...
case $target_vendor in
	k1om)
		...
		if test "$CC" != "x86_64-$target_vendor-linux-gcc"; then
			AC_MSG_WARN([Cross compiler not found in PATH.])
			MPSS_DEFAULT=/usr/linux-k1om-4.7/bin/x86_64-$target_vendor-linux-
			CROSS_VARS="ARCH=$target_vendor CROSS_COMPILE=${MPSS_DEFAULT}"
			CC=${MPSS_DEFAULT}gcc
			if test ! -x "$CC"; then
				AC_MSG_ERROR([MPSS GNU cross toolchain not found.])
			fi
			LD=${MPSS_DEFAULT}ld
			AR=${MPSS_DEFAULT}ar
			RANLIB=${MPSS_DEFAULT}ranlib
			OBJDUMP=${MPSS_DEFAULT}objdump
			STRIP=${MPSS_DEFAULT}strip
		else
         ...
         [AC_MSG_CHECKING([Linux kernel architecture])
          AS_IF([rm -f $PWD/build/arch
                 make -s --no-print-directory echoarch -f $PWD/build/Makefile \
                     LUSTRE_LINUX_CONFIG=$LINUX_CONFIG -C $LINUX $ARCH_UM $CROSS_VARS \
