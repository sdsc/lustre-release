...
[AC_MSG_CHECKING([whether to use any OFED Compact RDMA])
# set default
AC_ARG_WITH([o2ib],
...
if test $ENABLEO2IB -eq 0; then
	...
	if $o2ib_found; then
		if test \( -f ${O2IBPATH}/include/linux/compact-2.6.h \); then
			compatrdma_found=true
			AC_MSG_RESULT([yes])
			AC_DEFINE(HAVE_COMPACT_RDMA, 1, [compact rdma found])
		else
			AC_MSG_RESULT([no])
		fi
	fi
