...
mask_list=
allbits=0
fmt='#define LDLM_FL_%-20s  0x%016XULL // bit %2u
#define ldlm_is_%-18s    LDLM_TEST_FLAG( (_l), LDLM_FL_%s)
#define ldlm_set_%-18s   LDLM_SET_FLAG(  (_l), LDLM_FL_%s)
#define ldlm_clear_%-18s LDLM_CLEAR_FLAG((_l), LDLM_FL_%s)\n'
acc_fmt=''
tmpfile=[=(out-name)=]-$$.tmp
exec 8>&1 1> $tmpfile
[=
...
#define LDLM_FL_%-20s 0x%016XULL\n'
...
[= (shell "columns --spread=1 -I4 -S, --end=' };' <<_EOF_\n"
   len-list "_EOF_") =]
