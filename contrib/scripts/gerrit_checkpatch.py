...
CHECKPATCH_PATHS = os.getenv('CHECKPATCH_PATHS', 'checkpatch.pl').split(':')
CHECKPATCH_IGNORED_PATHS = set(['lustre/contrib/wireshark/packet-lustre.c',
                                'lustre/ptlrpc/wiretest.c',
                                'lustre/utils/wiretest.c'])
CHECKPATCH_IGNORED_KINDS = set(['LEADING_SPACE'])

REVIEW_HISTORY_PATH = os.getenv('REVIEW_HISTORY', 'REVIEW_HISTORY')

STYLE_LINK = 'https://wiki.hpdd.intel.com/display/PUB/Coding+Guidelines'
...
def parse_checkpatch_output(out, path_line_comments, warning_count):
    ...
    def add_comment(path, line, level, kind, message):
        ...
        if (path.endswith('.patch') or
            path in CHECKPATCH_IGNORED_PATHS or
            kind in CHECKPATCH_IGNORED_KINDS):
            return
    ...
    * Track reviewed revisions in history_path.
    ...
    def _post(self, path, obj):
        ...
        if not self.post_enabled:
            self._error("skipping POST to '%s'", url)
            return False
    ...
    def check_patch(self, patch):
        ...
        for path in CHECKPATCH_PATHS:
            ...
            self._debug("check: path = %s, out = '%s...', err = '%s...'",
                        path, out[:20], err[:20])
