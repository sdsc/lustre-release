...
AC_MSG_CHECKING([for RedHat kernel version])
	AS_IF([fgrep -q RHEL_RELEASE ${LINUX_OBJ}/include/linux/version.h], [
		RHEL_KERNEL="yes"
		rhelrelease=$((echo "#include <linux/version.h>";
		              echo "rhelrelease=RHEL_RELEASE") |
		              cpp -I ${LINUX_OBJ}/include |
		              grep "^rhelrelease=" | cut -d \" -f 2)
		RHEL_KERNEL_VERSION=$(echo ${LINUXRELEASE} |
		                      sed -e s/@<:@@<:@:punct:@:>@@:>@/./g |
		                      cut -d . -f -3 |
		                      sed -e s/\$/-$(expr 0${rhelrelease} + 1)/)
