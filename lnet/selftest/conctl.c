...
lst_session_info_ioctl(lstio_session_info_args_t *args)
{
	...
	if (!args->lstio_ses_idp ||	/* address for ouput sid */
	    !args->lstio_ses_keyp ||	/* address for ouput key */
	    !args->lstio_ses_featp ||	/* address for ouput features */
	    !args->lstio_ses_ndinfo ||	/* address for output ndinfo */
	    !args->lstio_ses_namep ||	/* address for ouput name */
	    args->lstio_ses_nmlen <= 0 ||
	    args->lstio_ses_nmlen > LST_NAME_SIZE)
		return -EINVAL;
