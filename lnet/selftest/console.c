...
lstcon_group_alloc(char *name, lstcon_group_t **grpp)
{
	...
	if (name != NULL) {
		...
		strncpy(grp->grp_name, name, sizeof(grp->grp_name)-1);
		grp->grp_name[sizeof(grp->grp_name)-1] = '\0';
	}
...
lstcon_batch_add(char *name)
{
	...
	strncpy(bat->bat_name, name, sizeof(bat->bat_name)-1);
	bat->bat_name[sizeof(bat->bat_name)-1] = '\0';
		   ...
		   int timeout, int force, lst_sid_t *sid_up)
{
	...
	strncpy(console_session.ses_name, name,
		sizeof(console_session.ses_name)-1);
	console_session.ses_name[sizeof(console_session.ses_name)-1] = '\0';
