...
int
srpc_add_bulk_page(srpc_bulk_t *bk, page_t *pg, int i, int nob)
{
...
#ifndef __KERNEL__
	LIBCFS_FREE(bk->bk_pages, sizeof(page_t *) * bk->bk_niov);
#endif
...
srpc_alloc_bulk(int cpt, unsigned bulk_npg, unsigned bulk_len, int sink)
{
	...
	memset(pages, 0, sizeof(page_t *) * bulk_npg);
	bk->bk_pages = pages;
#else
	UNUSED(pages);
#endif
