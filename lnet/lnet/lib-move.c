...
lnet_return_tx_credits_locked(lnet_msg_t *msg)
{
	...
	if (txni != NULL) {
		msg->msg_txni = NULL;
		printk(KERN_ERR "Decref from return_tx_credits\n");
...
routing_off:
	...
	if (rxni != NULL) {
		msg->msg_rxni = NULL;
		printk(KERN_ERR "Decref from return_rx_credits\n");
...
lnet_send(lnet_nid_t src_nid, lnet_msg_t *msg, lnet_nid_t rtr_nid)
{
 ...
 again:
	//if (local_ni != NULL) {
		//printk(KERN_ERR "Decref from send (local_ni)\n");
	...
	if (src_nid == LNET_NID_ANY) {
		...
		if (src_ni == NULL) {
			lnet_net_unlock(cpt);
                ...
                LASSERT (!msg->msg_routing);
		printk(KERN_ERR "Addref from send\n");
	...
	local_ni = lnet_net2ni_locked(LNET_NIDNET(dst_nid), cpt);
        ...
        if (local_ni != NULL) {
			...
			printk(KERN_ERR "Addref from send 2\n");
			lnet_ni_addref_locked(src_ni, cpt);
                } else if (src_ni == local_ni) {
			//printk(KERN_ERR "Decref from send (src_ni == local_ni)\n");
			//lnet_ni_decref_locked(local_ni, cpt);
		} else {
			//printk(KERN_ERR "Decref from send (else 1)\n");
			//lnet_ni_decref_locked(local_ni, cpt);
			printk(KERN_ERR "Decref from send (else 2)\n");
		...
		if (rc != 0) {
			printk(KERN_ERR "Decref from send (ENOMEM)\n");
			lnet_ni_decref_locked(src_ni, cpt);
			lnet_net_unlock(cpt);
		...
		if (lp == NULL) {
			if (src_ni != NULL) {
				printk(KERN_ERR "Decref from send (src_ni != NULL)\n");
				lnet_ni_decref_locked(src_ni, cpt);
			}
			lnet_net_unlock(cpt);
		...
		if (rtr_nid != lp->lp_nid) {
			...
			if (cpt2 != cpt) {
				if (src_ni != NULL) {
					printk(KERN_ERR "Decref from send (src_ni != NULL 2)\n");
					lnet_ni_decref_locked(src_ni, cpt);
				}
				lnet_net_unlock(cpt);
	   ...
	   void *private, int rdma_req)
{
	...
	printk(KERN_ERR "Addref from parse\n");
