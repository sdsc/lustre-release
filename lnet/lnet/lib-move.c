...
LNetDist(lnet_nid_t dstnid, lnet_nid_t *srcnidp, __u32 *orderp)
{
	...
	int			i, rc;
	char			*if_names[LNET_MAX_INTERFACES];
	...
	memset(if_names, 0, LNET_MAX_INTERFACES*sizeof(char *));
	for (i = 0; i < LNET_MAX_INTERFACES; i++) {
		LIBCFS_ALLOC(if_names[i], LNET_MAX_STR_LEN);
		if (if_names[i] == NULL)
			GOTO(out, rc = -ENOMEM);
	}
	...
	list_for_each(e, &the_lnet.ln_nis) {
		ni = list_entry(e, lnet_ni_t, ni_list);
			...
			GOTO(out, rc = local_nid_dist_zero ? 0 : 1);
                }
...
out:
	for (i = 0; (i < LNET_MAX_INTERFACES) && (if_names[i] != NULL); i++)
		LIBCFS_FREE(if_names[i], LNET_MAX_STR_LEN);
	return rc;
}
