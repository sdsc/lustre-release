...
again:
	...
	if (!peer->lp_multi_rail && local_net != NULL) {
		...
		/* grab th cpt ref count when we know the correct cpt */
		if (best_ni != NULL)
			lnet_ni_decref_locked(best_ni, cpt);
	}
...
send:
	 ...
	 * If it has changed then repeat the algorithm
	 */
	...
	lnet_peer_addref_locked(msg->msg_txpeer);
	...
	/*
	 * If we are routing the message then we don't need to cver write
