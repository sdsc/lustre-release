...
lnet_setpayloadbuffer(lnet_ni_t *ni, lnet_msg_t *msg)
{
	...
	if (ni == NULL || ni->ni_cksum_algo != CFS_HASH_ALG_NULL) {
		...
		cksum_size = cfs_crypto_hash_digestsize(ni->ni_cksum_algo);
	}

	msg->msg_niov = md->md_niov;
	if ((md->md_options & LNET_MD_KIOV) != 0) {
		lnet_kiov_t *kiov = msg->msg_kiov = md->md_iov.kiov;

		if (cksum_size == 0)
			goto no_cksum;

		for (i = 0; i < md->md_niov; i++) {
			unsigned long addr = kiov->kiov_offset;
			struct page *kpage = kiov->kiov_page;
		...
		struct iovec *iov = msg->msg_iov = md->md_iov.iov;

		if (cksum_size == 0)
			goto no_cksum;

		for (i = 0; i < md->md_niov; i++) {
			cfs_crypto_hash_update(hdesc, iov->iov_base,
					       iov->iov_len);
			iov++;
		}
		rc = cfs_crypto_hash_final(hdesc, msg->msg_cksum, &cksum_size);
