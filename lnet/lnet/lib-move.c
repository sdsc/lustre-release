...
static void
lnet_get_best_ni(struct lnet_net *local_net, int md_cpt,
		 struct lnet_ni **best_ni, int *best_credits,
...
again:
	...
	list_for_each_entry(peer_net, &peer->lp_peer_nets, lpn_on_peer_list) {
		...
		if (!local_net && !routing && !local_found) {
			 ...
			 * A peer can be reachable via multiple different
			 * gateways on different networks. If that's the
			 * case, we'll pick the best gw. If all gateways
			 * are equal then we'll round robin through the
			 * gateways.
			 */
			list_for_each_entry(lpni, &peer_net->lpn_peer_nis,
				...
				if (best_gw) {
					 ...
					 */
					if (lnet_compare_peers(best_gw,
							       net_gw) > 0)
						continue;
					if (best_gw->lpni_gw_seq <=
					    net_gw->lpni_gw_seq)
						continue;
				}
				best_gw = net_gw;
		 ...
		 *	3. Round Robin
		 */
		lnet_get_best_ni(local_net, md_cpt, &best_ni, &best_credits,
				 &shortest_distance);
	}
	...
	if (!best_ni) {
		...
		shortest_distance = UINT_MAX;
		best_credits = INT_MIN;
		lnet_get_best_ni(best_gw->lpni_net, md_cpt, &best_ni,
				 &best_credits, &shortest_distance);
