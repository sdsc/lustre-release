...
lnet_fail_nid (lnet_nid_t nid, unsigned int threshold)
{
        ...
        if (threshold != 0) {
                ...
                /* NB: we just use cpt = 0 to serialize
...
lnet_post_routed_recv_locked (lnet_msg_t *msg, int do_recv)
{
        ...
        rbp = lnet_msg2bufpool(msg, msg->msg_rx_cpt);
               ...
               unsigned int offset, unsigned int mlen)
{
        ...
        msg->msg_md = md;
        if (md->md_me != NULL) { /* commited for passive message */
                msg->msg_offset = offset;
                msg->msg_wanted = mlen;
        }
...
void
lnet_drop_delayed_msg(cfs_list_t *head, char *reason)
{
...
static int lnet_recv_put(lnet_ni_t *ni, lnet_msg_t *msg);

void
lnet_recv_delayed_msg(cfs_list_t *head)
{
        while (!cfs_list_empty(head)) {
		lnet_msg_t	  *msg;
...
static unsigned int
lnet_redir_put_intent(lnet_ni_t *ni, int index, lnet_hdr_t *hdr)
{
        unsigned int    intent;
        unsigned int    routed;
	...
	/* remote intent? */
		...
		if (!(ni->ni_npeers / LNET_CPT_NUMBER < lnet_ni_peer_low) ||
		     (lnet_redir_put == LNET_REDIR_ROUTED && routed))
                        return 0;
                /* fall through */
...
lnet_parse_put(lnet_ni_t *ni, lnet_msg_t *msg)
{
        ...
        hdr->msg.put.hdr_data   = le64_to_cpu(hdr->msg.put.hdr_data);
        ...
        mtable = lnet_mt_of_match(the_lnet.ln_portals[index],
                                   ...
                                   lnet_redir_put_intent(ni, index, hdr));
        ...
        else if ((rc & LNET_MATCHMD_NONE) != 0)
                msg->msg_delayed = 1;

        return (rc & LNET_MATCHMD_DROP) != 0 ? ENOENT : 0;
}
...
lnet_recv_put(lnet_ni_t *ni, lnet_msg_t *msg)
{
        ...
        if (msg->msg_md == NULL) { /* no match yet */
                lnet_process_id_t  src = {0};
                int                cpt = lnet_cpt_of_nid(msg->msg_sender);
                ...
                if (ni->ni_lnd->lnd_eager_recv != NULL &&
                    (rc = lnet_ni_eager_recv(ni, msg)) != 0) {
                ...
                rc = lnet_ptl_match_msg(the_lnet.ln_portals[put->ptl_index],
                                        LNET_MD_OP_PUT, src,
                                        hdr->payload_length,
                                        put->offset, put->match_bits, msg);
                if ((rc & LNET_MATCHMD_DROP) != 0) /* drop it */
                        return -ENOENT; /* negative value for event */

                if ((rc & LNET_MATCHMD_NONE) != 0) /* delayed */
                        return EAGAIN;
           ...
           void *private, int rdma_req)
{
        ...
        switch (type) {
        case LNET_MSG_ACK:
                rc = lnet_recv_ack(ni, msg);
                break;
        case LNET_MSG_PUT:
                rc = lnet_recv_put(ni, msg);
                break;
        case LNET_MSG_GET:
                rc = lnet_recv_get(ni, msg, rdma_req);
                break;
        case LNET_MSG_REPLY:
                rc = lnet_recv_reply(ni, msg);
                break;
        default:
                LBUG();
        }
