...
again:
	...
	list_for_each_entry(peer_net, &peer->lp_peer_nets, lpn_on_peer_list) {
		...
		while ((ni = lnet_get_next_ni_locked(local_net, ni))) {
			...
			ni_credits = lnet_ni_get_tx_credits(ni);
...
send:
	/*
	 * use lnet_cpt_of_nid() to determine the cpt to commit to the
	 * message. This ensures that we get a cpt which the best NI has
	 * on its list. If the cpt has changed then
