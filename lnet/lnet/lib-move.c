...
lnet_compare_routes(lnet_route_t *r1, lnet_route_t *r2)
{
	...
	if (p1->lp_ni->ni_peertimeout > 0) {
		 ...
		 * the last 10 seconds, it could be potentially dead or congested,
		 * so we prefer not to choose it even its status is still alive.
		 */
	   ...
	   void *private, int rdma_req)
{
	 ...
	 * it as alive */
	msg->msg_rxpeer->lp_last_alive =
	msg->msg_rxpeer->lp_last_query = cfs_time_current();
