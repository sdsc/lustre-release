...
#define LNET_NRB_LARGE_LOW      128
#define LNET_NRB_LARGE_HIGH     1024
...
static int large_router_buffers = 0;
CFS_MODULE_PARM(large_router_buffers, "i", int, 0444,
                "# of large messages to buffer in the router");
static int large_router_buffers_cpt = 0;
CFS_MODULE_PARM(large_router_buffers_cpt, "i", int, 0444,
		"# of large messages to buffer for each CPT");
...
static void
lnet_ni_notify_locked(lnet_ni_t *ni, lnet_peer_t *lp, int net_lock)
{
        int        alive;
        int        notifylnd;
...
/* NB expects lnet_net_lock(LNET_LOCK_EX) held */
void
...
lnet_add_route (__u32 net, unsigned int hops, lnet_nid_t gateway)
{
	...
	rc = lnet_nid2peer_locked(&route->lr_gateway, gateway, LNET_LOCK_EX);
		...
		if (rc == -EHOSTUNREACH) {
			CWARN("Gateway %s %d is not on local net\n",
...
lnet_parse_rc_info(lnet_rc_data_t *rcd)
{
	...
	lnet_peer_t      *gw   = rcd->rcd_gateway;
	lnet_route_t     *rtr;
...
lnet_router_checker_event (lnet_event_t *event)
{
	...
	if (event->type == LNET_EVENT_SEND) {
		/* NB: re-enable another ping, it's not protected but
		 * operations are logically serialized */
                lp->lp_ping_notsent = 0;
		if (event->status == 0)
			return;
	}

	/* NB: it's called with holding lnet_res_lock, it's the only place
	 * holding two locks at the same time, please take care of lock
	 * ordering if there is other use case in the future */
	lnet_net_lock(rcd->rcd_gw_cpt);
	...
	lnet_net_unlock(rcd->rcd_gw_cpt);
}

void
lnet_update_ni_status_locked(int net_lock)
{
        lnet_ni_t *ni;
	long       now;
	long       timeout;
	...
	now = cfs_time_current_sec();
		...
		lnet_ni_lock(ni, 1);
		/* timestamp could be changed, check it again */
		status = now < ni->ni_last_alive + timeout ?
			 LNET_NI_STATUS_UP : LNET_NI_STATUS_DOWN;

		LASSERT (ni->ni_status != NULL);

		if (ni->ni_status->ns_status == status) {
			lnet_ni_unlock(ni, 1);
			continue;
                }

		ni->ni_status->ns_status = status;
		CDEBUG(D_NET, "NI(%s:%lu) status changed to %s\n",
...
lnet_rc_data_t *
lnet_create_rc_data_locked(lnet_peer_t *gateway, int cpt)
{
        int               i;
        int               rc;
        lnet_ping_info_t *pi;
...
static void
lnet_ping_router_locked(lnet_peer_t *rtr, int cpt)
{
	...
	lnet_ni_notify_locked(rtr->lp_ni, rtr, cpt);
	...
	rcd = rtr->lp_rcd != NULL ?
	      rtr->lp_rcd : lnet_create_rc_data_locked(rtr, cpt);
...
lnet_router_checker(void *arg)
{
 ...
 rescan:
                ...
                cfs_list_for_each (entry, &the_lnet.ln_routers) {
                        rtr = cfs_list_entry(entry, lnet_peer_t, lp_rtr_list);
			tmp = lnet_cpt_of_nid_locked(rtr->lp_nid);
			...
			lnet_ping_router_locked(rtr, cpt);
			...
			if (version != the_lnet.ln_routers_version)
                ...
                if (the_lnet.ln_routing)
			lnet_update_ni_status_locked(cpt);
...
lnet_rtrpool_free_bufs(lnet_rtrbufpool_t *rbp)
{
	...
	if (rbp->rbp_nbuffers == 0) /* not initialized */
		return;
...
static int
lnet_nrb_large_estimate(void)
{
	int     large_pages = (LNET_MTU + CFS_PAGE_SIZE - 1) >> CFS_PAGE_SHIFT;
	long    nrb;
	long    npg;

	if (large_router_buffers_cpt > 0)
		return large_router_buffers_cpt;

	if (large_router_buffers_cpt < 0) {
		LCONSOLE_ERROR_MSG(0x10e, "large_router_buffers_cpt=%d "
				   "invalid when routing enabled\n",
				    large_router_buffers_cpt);
		return -1;
	}

	if (large_router_buffers > 0) {
		CWARN("Please use large_router_buffers_cpt instead of this\n");
		nrb = large_router_buffers / LNET_CPT_NUMBER;
		return max_t(int, 1, nrb);
	}

	if (large_router_buffers < 0) {
		LCONSOLE_ERROR_MSG(0x10e, "large_router_buffers=%d "
				   "invalid when routing enabled\n",
				   large_router_buffers);
		return -1;
	}

	/* about 40% of total pages */
	npg = (cfs_num_physpages >> 1) - (cfs_num_physpages >> 3);

	for (nrb = LNET_NRB_LARGE_LOW; nrb * large_pages < npg;)
		nrb += LNET_NRB_LARGE_LOW;

	nrb /= LNET_CPT_NUMBER;
	return min(max_t(int, LNET_NRB_LARGE_LOW, nrb), LNET_NRB_LARGE_HIGH);
}
...
lnet_alloc_rtrpools(int im_a_router)
{
	...
	nrb_tiny = lnet_nrb_tiny_estimate();
	if (nrb_tiny < 0)
		return -EINVAL;
	nrb_small = lnet_nrb_small_estimate();
	if (nrb_small < 0)
		return -EINVAL;
	nrb_large = lnet_nrb_large_estimate();
	if (nrb_large < 0)
		return -EINVAL;
...
lnet_notify (lnet_ni_t *ni, lnet_nid_t nid, int alive, cfs_time_t when)
{
	...
	lnet_ni_notify_locked(lp->lp_ni, lp, cpt);
...
lnet_router_checker (void)
{
	...
	if (the_lnet.ln_rc_state == LNET_RC_STATE_STOPPING)
		...
		/* Although it is called with holding lnet_res_lock in kernel,
		 * but nothing needs to be protected by lnet_res_lock */
