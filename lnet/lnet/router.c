	       ...
	       int fail_if_ignored)
{
	...
	if (rc != 0) {
		...
		if ((rc == -EHOSTUNREACH) &&
		    (!fail_if_ignored)) { /* gateway is not on a local net */
			return 0;	/* ignore the route entry */
		} else {
...
lnet_rtrpool_free_bufs(lnet_rtrbufpool_t *rbp)
{
	...
	while (!cfs_list_empty(&rbp->rbp_msgs)) {
		rb = cfs_list_entry(rbp->rbp_msgs.next,
...
lnet_rtrpool_adjust_bufs(lnet_rtrbufpool_t *rbp, int nbufs, int cpt)
{
	...
	/*  If we are called for less buffers than alredy in the pool, we
...
lnet_adjust_rtrpools(int tiny, int small, int large)
{
	...
	/* If routing is turned off, and we have never intialized the pools
