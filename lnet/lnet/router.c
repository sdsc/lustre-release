...
lnet_find_net_locked (__u32 net)
{
	...
	rn_list = &the_lnet.ln_remote_nets_hash[lnet_net2remotehash(net)];
...
lnet_add_route (__u32 net, unsigned int hops, lnet_nid_t gateway)
{
	...
	if (rc != 0) {
        ...
        if (rnet2 == NULL) {
                /* new network */
		cfs_list_add_tail(&rnet->lrn_list,
		       &the_lnet.ln_remote_nets_hash[lnet_net2remotehash(net)]);
                rnet2 = rnet;
        }
...
lnet_check_routes(void)
{
	...
	for (i = 0; i < LNET_REMOTE_NETS_HASH_SIZE; i++) {
		if (cfs_list_empty(&the_lnet.ln_remote_nets_hash[i]))
			continue;
