...
int
lnet_startup_lndnis(char *net, int from_mod_params, __s32 peer_to,
...
LNetCtl(unsigned int cmd, void *arg)
{
	...
	struct libcfs_ioctl_config_data *config;
	...
	case IOC_LIBCFS_ADD_NET:
		config = arg;
		LNET_MUTEX_LOCK(&the_lnet.ln_api_mutex);
		rc = lnet_startup_lndnis(config->ioc_config_u.net.intf,
					 0,
	...
	case IOC_LIBCFS_DEL_NET:
		config = arg;
		return lnet_shutdown_lndni(config->ioc_net);

	case IOC_LIBCFS_GET_NET:
	{
		struct libcfs_ioctl_net_config *net_config;
		config = arg;
		net_config = (struct libcfs_ioctl_net_config *)
			config->ioc_bulk;
	...
	case IOC_LIBCFS_CONFIG_RTR:
		...
		if (config->ioc_config_u.buffers.enable)
			return lnet_enable_rtrpools();
		return lnet_disable_rtrpools();

	case IOC_LIBCFS_ADD_BUF:
		config = arg;
		return lnet_adjust_rtrpools(config->ioc_config_u.buffers.tiny,
	...
	case IOC_LIBCFS_GET_BUF:
	{
		...
		config = arg;
		pool_cfg = (struct libcfs_ioctl_pool_cfg *)config->ioc_bulk;
...
lnet_create_ping_info(void)
{
	...
	if (the_lnet.ln_routing)
		pinfo->pi_features |= LNET_PING_FEAT_ROUTING;
	lnet_net_unlock(LNET_LOCK_EX);
