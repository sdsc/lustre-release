...
int
LNetDynNIInit(lnet_pid_t requested_pid, char *nets,
	      ...
	      __s32 credits)
{
	...
	lnet_net_lock(LNET_LOCK_EX);
	prepare = list_empty(&the_lnet.ln_nis);
	lnet_net_unlock(LNET_LOCK_EX);
...
LNetDynNIFini(__u32 net)
{
	...
	/* if ni_count == 1 that means that only LO is left on the active
	 * NIs.  In this case we want to clean up by calling LNetNIFini().
	 * The rason LNetNIFini is not called instead of
	 * lnet_shutdown_lndni() is because net might not exist, and we'd
	 * have to search for net on the active NIs, which
	 * lnet_shutdown_lndni() does.  To avoid repeating logic and
	 * making the code simpler to read, we shutdown the specified net
	 * and then if there is only 1 active NI left, then it must be the
	 * LO, and thus we clean up
	 */
	if (rc == 0 && ni_count == 1)
		LNetNIFini();
...
LNetCtl(unsigned int cmd, void *arg)
{
	...
	case IOC_LIBCFS_ADD_BUF:
		config = arg;
		LNET_MUTEX_LOCK(&the_lnet.ln_api_mutex);
		rc = lnet_rtrpools_adjust(config->ioc_config_u.buffers.tiny,
