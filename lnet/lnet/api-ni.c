...
static int
lnet_ping_info_create(lnet_ping_info_t **ping_info, int num_ni)
{
	unsigned int	infosz;
...
lnet_ping_info_to_ni(lnet_ping_info_t *ping_info)
{
	...
	for (i = 0; i < ping_info->pi_nnis; i++) {
		lnet_ni_status_t *ns = &ping_info->pi_ni[i];
		     ...
		     int ni_count)
{
	...
	rc = lnet_ping_info_create(ppinfo, ni_count);
	if (rc != 0)
		return rc;
...
static void
lnet_ping_md_unlink(void)
{
	int		timeout = 1;
	sigset_t	blocked = cfs_block_allsigs();

	LNetMDUnlink(the_lnet.ln_ping_target_md);
	LNetInvalidateHandle(&the_lnet.ln_ping_target_md);

	/* NB md could be busy; this just starts the unlink */
	for (;;) {
		lnet_net_lock(LNET_LOCK_EX);
		if ((the_lnet.ln_ping_info->pi_features & LNET_PING_FEAT_DEAD)
			!= 0) {
			lnet_net_unlock(LNET_LOCK_EX);
			break;
		}
		...
		cfs_pause(cfs_time_seconds(timeout));
		timeout *= 2;
	}
...
lnet_ping_target_update(lnet_ping_info_t *pinfo, lnet_handle_md_t md_handle)
{
	...
	/* set up the new ping information in the_lnet structure */
	lnet_net_lock(LNET_LOCK_EX);
	the_lnet.ln_ping_target_md = md_handle;
		    ...
		    __s32 peer_cr, __s32 peer_buf_cr, __s32 credits)
{
	...
	rc = lnet_parse_networks(&nilist, net, &ni_count);
	if (rc != 0)
		goto failed;
	...
	if (!from_mod_params) {
		 ...
		 * creatign a new ping info, to replace the old one */
		ni_count += lnet_get_ni_count();
		rc = lnet_ping_info_setup(&pinfo, &md_handle, ni_count);
		if (rc != 0)
			goto failed;
...
LNetCtl(unsigned int cmd, void *arg)
{
	...
	{
		struct libcfs_ioctl_lnet_stats *lnet_stats = arg;
