...
static int lnet_max_interfaces = LNET_MAX_INTERFACES;
...
static void lnet_assert_wire_constants(void)
{
	 ...
	 *
	 * LNet ping related checks added by hand in 2016.
	 */
	...
	/* Checks for struct lnet_ni_status_t and related constants */
	CLASSERT(LNET_NI_STATUS_INVALID == 0);
	CLASSERT(LNET_NI_STATUS_UP == 0x15aac0de);
	CLASSERT(LNET_NI_STATUS_DOWN == 0xdeadface);
	CLASSERT((int)sizeof(lnet_ni_status_t) == 16);
	CLASSERT((int)offsetof(lnet_ni_status_t, ns_nid) == 0);
	CLASSERT((int)sizeof(((lnet_ni_status_t *)0)->ns_nid) == 8);
	CLASSERT((int)offsetof(lnet_ni_status_t, ns_status) == 8);
	CLASSERT((int)sizeof(((lnet_ni_status_t *)0)->ns_status) == 4);
	CLASSERT((int)offsetof(lnet_ni_status_t, ns_unused) == 12);
	CLASSERT((int)sizeof(((lnet_ni_status_t *)0)->ns_unused) == 4);

	/* Checks for struct lnet_ping_info_t and related constants */
	...
	CLASSERT((int)offsetof(lnet_ping_info_t, pi_features) == 4);
	CLASSERT((int)sizeof(((lnet_ping_info_t *)0)->pi_features) == 4);
	CLASSERT((int)offsetof(lnet_ping_info_t, pi_pid) == 8);
	CLASSERT((int)sizeof(((lnet_ping_info_t *)0)->pi_pid) == 4);
	CLASSERT((int)offsetof(lnet_ping_info_t, pi_nnis) == 12);
	CLASSERT((int)sizeof(((lnet_ping_info_t *)0)->pi_nnis) == 4);
	CLASSERT((int)offsetof(lnet_ping_info_t, pi_ni) == 16);
...
lnet_ping_info_validate(struct lnet_ping_info *pinfo)
{
	...
	if (pinfo->pi_nnis < 1 || pinfo->pi_nnis > lnet_max_interfaces)
		return -ERANGE;
...
static void
lnet_ping_target_event_handler(lnet_event_t *event)
{
...
lnet_ping_target_setup(struct lnet_ping_buffer **ppbuf,
		       lnet_handle_md_t *ping_mdh, int ni_count, bool set_eq)
{
...
static void
lnet_ping_md_unlink(struct lnet_ping_buffer *pbuf, lnet_handle_md_t *ping_mdh)
{
...
lnet_ping_target_install_locked(struct lnet_ping_buffer *pbuf)
{
	...
	LNET_PING_BUFFER_SEQNO(pbuf) = ++the_lnet.ln_ping_target_seqno;
}
...
static void lnet_push_target_event_handler(struct lnet_event *ev)
{
	...
	if (pbuf->pb_info.pi_magic == __swab32(LNET_PROTO_PING_MAGIC))
		lnet_swap_pinginfo(&pbuf->pb_info);
...
static int lnet_push_target_init(void)
{
	...
	the_lnet.ln_push_target_nnis = LNET_MIN_INTERFACES;

	rc = lnet_push_target_resize();

	if (rc) {
		LNetEQFree(the_lnet.ln_push_target_eq);
		LNetInvalidateHandle(&the_lnet.ln_push_target_eq);
	}
...
static void lnet_push_target_fini(void)
{
	...
	LNetInvalidateHandle(&the_lnet.ln_push_target_md);
...
LNetCtl(unsigned int cmd, void *arg)
{
	...
	case IOC_LIBCFS_ADD_PEER_NI: {
		...
		mutex_lock(&the_lnet.ln_api_mutex);
		rc = lnet_add_peer_ni(cfg->prcfg_key_nid,
				      cfg->prcfg_cfg_nid,
				      cfg->prcfg_mr);
		mutex_unlock(&the_lnet.ln_api_mutex);
		return rc;
	...
	case IOC_LIBCFS_DEL_PEER_NI: {
		...
		mutex_lock(&the_lnet.ln_api_mutex);
		rc = lnet_del_peer_ni(cfg->prcfg_key_nid,
				      cfg->prcfg_cfg_nid);
		mutex_unlock(&the_lnet.ln_api_mutex);
		return rc;
	}
