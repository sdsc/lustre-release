...
static int
lnet_shutdown_lndni_prep(lnet_ping_info_t **ppinfo, lnet_handle_md_t *md_handle)
{
	int		ni_count;
	int		rc;
	...
	rc = lnet_ping_info_setup(ppinfo, md_handle, ni_count - 1, false);
	if (rc != 0)
		return rc;
...
static inline void
lnet_shutdown_lndni_fini(lnet_ping_info_t *pinfo, lnet_handle_md_t md_handle)
{
	lnet_ping_target_update(pinfo, md_handle);
}

static inline void
lnet_shutdown_lndni_clean(lnet_ping_info_t *pinfo, lnet_handle_md_t md_handle)
{
...
static void
lnet_shutdown_lndni_helper(struct lnet_ni *ni)
{
	/* proceed with shutting down the NI */
	lnet_net_lock(LNET_LOCK_EX);

	/* decrement the reference counter on ni which was
...
lnet_shutdown_lndni(__u32 net)
{
	...
	rc = lnet_shutdown_lndni_prep(&pinfo, &md_handle);
	if (rc != 0)
		return rc;

	found_ni = lnet_net2ni(net);
	if (found_ni == NULL) {
		lnet_shutdown_lndni_clean(pinfo, md_handle);
		return -EINVAL;
	}

	lnet_shutdown_lndni_helper(found_ni);

	lnet_shutdown_lndni_fini(pinfo, md_handle);
		    ...
		    int *ni_count)
{
	...
	while (!list_empty(nilist)) {
		...
		list_del(&ni->ni_list);
		rc = lnet_startup_lndni(ni, peer_timeout,
					peer_cr, peer_buf_cr, credits);
...
LNetNIInit(lnet_pid_t requested_pid)
{
	...
	ni = lnet_ni_alloc(LNET_MKNET(LOLND, 0), NULL, &net_head);
	if (ni == NULL)
		goto failed1;
