		     ...
		     int ni_count, bool set_eq)
{
	...
	if (set_eq) {
		/* We can have a tiny EQ since we only need to see the unlink
		 * event on teardown, which by definition is the last one! */
...
lnet_shutdown_lndni(__u32 net)
{
	 ...
	 * incremented when we called lnet_net2ni() */
...
static int
lnet_startup_lndnis(struct list_head *nilist, __s32 peer_to, __s32 peer_cr,
		    __s32 peer_buf_cr, __s32 credits, int *ni_count)
{
	int			rc = 0;
	struct lnet_ni		*ni;
	int			lnd_type;
	      ...
	      __s32 credits)
{
	...
	if (!prepare) {
		rc = lnet_startup_lndnis(&net_head, peer_to, peer_cr,
