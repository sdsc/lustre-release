...
lnet_ping_info_validate(struct lnet_ping_info *pinfo)
{
	...
	if (pinfo->pi_magic != LNET_PROTO_PING_MAGIC)
		return -EPROTO;
	if (!(pinfo->pi_features & LNET_PING_FEAT_NI_STATUS) == 0)
		return -EPROTO;
	/* Loopback + primary */
	if (pinfo->pi_nnis < 2 || pinfo->pi_nnis > lnet_max_interfaces)
		return -ERANGE;
...
lnet_ping_target_install_locked(struct lnet_ping_buffer *pbuf)
{
	 ...
	 */
	LASSERT(lnet_ping_info_validate(&pbuf->pb_info) == 0);
