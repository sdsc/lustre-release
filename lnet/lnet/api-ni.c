...
lnet_ni_credits_estimate(lnet_ni_t *ni)
{
	...
	LASSERT(ni->ni_credits > 0 || ni->ni_credits_cpt > 0);

	if (ni->ni_credits_cpt > 0)
		return ni->ni_credits_cpt;

	if (ni->ni_ncpts == 1)
		return ni->ni_credits;

	credits = ni->ni_credits / ni->ni_ncpts;
	credits = max(8 * ni->ni_peertxcredits, credits);
	credits = min(ni->ni_credits, credits);
...
# endif
		    ...
		    (ni->ni_credits == 0 && ni->ni_credits_cpt == 0)) {
