...
void
lnet_res_lh_initialize(struct lnet_res_container *rec, struct lnet_libhandle *lh)
{
	/* ALWAYS called with lnet_res_lock held */
...
static int
lnet_ping_info_setup(struct lnet_ping_info **ppinfo, struct lnet_handle_any *md_handle,
		     int ni_count, bool set_eq)
{
	struct lnet_process_id id = {LNET_NID_ANY, LNET_PID_ANY};
	struct lnet_handle_any me_handle;
...
static void
lnet_ping_md_unlink(struct lnet_ping_info *pinfo, struct lnet_handle_any *md_handle)
{
...
static void
lnet_ping_target_update(struct lnet_ping_info *pinfo, struct lnet_handle_any md_handle)
{
	struct lnet_ping_info *old_pinfo = NULL;
	struct lnet_handle_any old_md;
...
LNetNIInit(lnet_pid_t requested_pid)
{
	...
	struct lnet_handle_any md_handle;
...
LNetCtl(unsigned int cmd, void *arg)
{
	...
	case IOC_LIBCFS_PING:
		...
		rc = lnet_ping(id, data->ioc_u32[1], /* timeout */
			       (struct lnet_process_id __user *)data->ioc_pbuf1,
...
/**
 * Retrieve the struct lnet_process_id ID of LNet interface at \a index. Note that
 ...
 */
void
LNetSnprintHandle(char *str, int len, struct lnet_handle_any h)
{
...
static int
lnet_ping(struct lnet_process_id id, int timeout_ms,
	  struct lnet_process_id __user *ids, int n_ids)
{
	struct lnet_handle_any eqh;
	struct lnet_handle_any mdh;
