...
static int
startup_a_single_ni(struct lnet_ni *ni, __s32 peer_timeout,
		    ...
		    int *ni_count)
{
	...
	while (!list_empty(nilist)) {
		...
		list_del(&ni->ni_list);
		rc = startup_a_single_ni(ni, peer_timeout, peer_cr,
				    peer_buf_cr, credits);
		if (rc < 0)
			goto failed;
		...
		__s32 credits)
{
	...
	LASSERT(!list_empty(&net_head));
	ni = list_entry(net_head.next, lnet_ni_t, ni_list);
	rc = startup_a_single_ni(ni, peer_timeout, peer_cr, peer_buf_cr,
				 credits);
	if (rc < 0)
		goto failed1;
