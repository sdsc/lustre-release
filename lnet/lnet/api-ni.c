...
static int route_htable_size = LNET_REMOTE_NETS_HASH_DEFAULT;
CFS_MODULE_PARM(route_htable_size, "i", int, 0444,
		"size of remote route hash table");
...
LNetInit(void)
{
	 ...
	 * don't waste memory). */
	if (route_htable_size <= 0)
		route_htable_size = LNET_REMOTE_NETS_HASH_DEFAULT;
	else if (route_htable_size > LNET_REMOTE_NETS_HASH_MAX)
		route_htable_size = LNET_REMOTE_NETS_HASH_MAX;
	the_lnet.ln_remote_nets_hbits = max_t(int, 1,
					   order_base_2(route_htable_size) - 1);
