...
static int
lnet_create_ping_info(lnet_ping_info_t **ping_info, int num_ni)
{
	unsigned int	infosz;
	...
	if (the_lnet.ln_routing)
		(*ping_info)->pi_features |= LNET_PING_FEAT_ROUTING;
	lnet_net_unlock(LNET_LOCK_EX);
...
static inline void
lnet_free_ping_info(lnet_ping_info_t *pinfo)
{
	LIBCFS_FREE(pinfo,
...
lnet_destroy_ping_info(void)
{
	...
	lnet_free_ping_info(the_lnet.ln_ping_info);
		     ...
		     int ni_count)
{
	...
	if (ppinfo == NULL || md_handle == NULL) {
		CERROR("Bad function parameters\n");
		return -EINVAL;
	}

	rc = lnet_create_ping_info(ppinfo, ni_count);
	if (rc != 0)
		return rc;
