	...
	list_for_each_entry(net, &the_lnet.ln_nets, net_list) {
		if (net->net_id == net_id) {
			ni = list_entry(net->net_ni_list.next, struct lnet_ni,
					ni_netlist);
			//printk(KERN_ERR "Addref from net2ni\n");
...
lnet_nid2ni_locked(lnet_nid_t nid, int cpt)
{
	...
	list_for_each_entry(net, &the_lnet.ln_nets, net_list) {
		list_for_each_entry(ni, &net->net_ni_list, ni_netlist) {
			if (ni->ni_nid == nid) {
				//printk(KERN_ERR "Addref from nid2ni\n");
				//lnet_ni_addref_locked(ni, cpt);
				return ni;
			}
		}
...
lnet_islocalnid(lnet_nid_t nid)
{
	...
	//if (ni != NULL) {
		//printk(KERN_ERR "Decref from islocalnid\n");
...
lnet_ni_unlink_locked(lnet_ni_t *ni)
{
	if (!list_empty(&ni->ni_cptlist)) {
		...
		printk(KERN_ERR "Decref from unlink_locked\n");
...
lnet_shutdown_lndnets(void)
{
	...
	if (the_lnet.ln_loni != NULL) {
		printk(KERN_ERR "Decref from shutdown_lnd\n");
...
lnet_startup_lndni(struct lnet_ni *ni)
{
	...
	if (net->net_lnd->lnd_type == LOLND) {
		printk(KERN_ERR "Addref from startup_lnd\n");
		lnet_ni_addref(ni);
...
LNetCtl(unsigned int cmd, void *arg)
{
	...
	default:
		...
		//printk(KERN_ERR "Decref from LNetCtl\n");
		//lnet_ni_decref(ni);
		return rc;
	}
	/* not reached */
