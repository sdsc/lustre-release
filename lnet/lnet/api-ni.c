...
CFS_MODULE_PARM(number_routes, "i", int, 0444,
		"approx number of routes");
...
lnet_create_remote_nets_table(void)
{
	int		i;
	cfs_list_t	*hash;
	int		hash_size = LNET_REMOTE_NETS_HASH_SIZE;

	LASSERT(the_lnet.ln_remote_nets_hash == NULL);
	LIBCFS_ALLOC(hash, hash_size * sizeof(*hash));
	if (hash == NULL) {
...
LNetInit(void)
{
	...
	the_lnet.ln_num_routes = number_routes;
