...
lnet_ping_md_unlink(lnet_ping_info_t *pinfo, lnet_handle_md_t *md_handle)
{
	...
	for (;;) {
		if ((pinfo->pi_features & LNET_PING_FEAT_DEAD) != 0)
			break;
...
lnet_ping_event_handler(lnet_event_t *event)
{
	...
	if (event->unlinked)
		pinfo->pi_features |= LNET_PING_FEAT_DEAD;
}
...
lnet_shutdown_lndni(__u32 net)
{
	...
	/* delete all routes associated with this ni */
	lnet_del_route(LNET_NIDNET(found_ni->ni_nid), LNET_NID_ANY);
		    ...
		    __s32 peer_cr, __s32 peer_buf_cr, __s32 credits)
{
	...
	cur_ni_count = lnet_get_ni_count();
...
LNetCtl(unsigned int cmd, void *arg)
{
	...
	case IOC_LIBCFS_DEL_ROUTE:
		config = arg;
		LNET_MUTEX_LOCK(&the_lnet.ln_api_mutex);
		rc = lnet_del_route(config->ioc_net, config->ioc_nid);
		LNET_MUTEX_UNLOCK(&the_lnet.ln_api_mutex);
		return rc;
