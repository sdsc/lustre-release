...
void lnet_assert_wire_constants(void)
{
	 ...
	 * with gcc version gcc version 4.8.5 20150623 (Red Hat 4.8.5-4) (GCC)
	 */
	...
	/* Checks for struct lnet_handle_wire_t */
	...
	/* Checks for struct lnet_ni_status_t */
	...
	CLASSERT(LNET_PING_FEAT_BITS == 15);

	/* Checks for struct struct lnet_ping_info */
...
LNetCtl(unsigned int cmd, void *arg)
{
	...
	case IOC_LIBCFS_SET_MAX_INTF: {
		struct lnet_ioctl_set_value *max_intf;
		max_intf = arg;
		if (max_intf->sv_hdr.ioc_len != sizeof(*max_intf) ||
		    max_intf->sv_value < LNET_MIN_INTERFACES)
			return -EINVAL;
		mutex_lock(&the_lnet.ln_api_mutex);
		lnet_max_interfaces = max_intf->sv_value;
		mutex_unlock(&the_lnet.ln_api_mutex);
		return 0;
	}

	case IOC_LIBCFS_GET_MAX_INTF: {
		struct lnet_ioctl_set_value *max_intf;
		max_intf = arg;
		if (max_intf->sv_hdr.ioc_len != sizeof(*max_intf))
			return -EINVAL;
		max_intf->sv_value = lnet_max_interfaces;
		return 0;
	}
