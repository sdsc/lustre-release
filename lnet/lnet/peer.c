 ...
 *   Create a peer with its primary NI if only the key_nid is provided
 *   Add a NID to a peer identified by the key_nid. The peer identified
 *   by the key_nid must already exist.
...
lnet_add_peer_ni(lnet_nid_t key_nid, lnet_nid_t nid, bool mr)
{
	...
	/* The key_nid must always be specified */
	 ...
	 * key_nid as its primary nid.
	 */
	if (nid == LNET_NID_ANY)
		return lnet_peer_add(key_nid, flags);

	/* Look up the key_nid, which must exist. */
	...
	/*
	 * It's careless if key_nid isn't the primary nid for the
	 * peer, but not fatal.
	 */
	if (lp->lp_primary_nid != key_nid) {
		CDEBUG(D_NET, "key_nid %s is not primary for peer %s\n",
 ...
 *
 * The caller must hold ln_net_mutex. This prevents the peer from
...
void lnet_peer_push_event(struct lnet_event *ev)
{
	...
	/*
	 * If there was a peer ping failure that hasn't been cleared
	 * yet, see if we can update the data buffer as save us an
	 * additional roundtrip.
	 */
	if (lp->lp_state & LNET_PEER_PING_FAILED) {
		if (pbuf->pb_info.pi_nnis <= lp->lp_data->pb_nnis) {
			memcpy(&lp->lp_data->pb_info, &pbuf->pb_info,
			       LNET_PING_INFO_SIZE(pbuf->pb_info.pi_nnis));
			lp->lp_state &= ~LNET_PEER_PING_FAILED;
			lp->lp_state |= LNET_PEER_DATA_PRESENT;
			CDEBUG(D_NET, "Ping fail fixed by Push from %s\n",
			       libcfs_nid2str(lp->lp_primary_nid));
		}
		goto out;
	}
	...
	/*
	 * If a ping has been sent and we're waiting for the reply,
	 * then there is nothing more to be done: the ping reply will
	 * overwrite the buffer in question, swizzling it out only
	 * confuses matters even more.
	 *
	 * If a push has been sent and we're waiting for the ack, then
	 * lp_data points to the (global) push source and should not
	 * be overwritten.
	 */
	if (lp->lp_state & (LNET_PEER_PING_SENT | LNET_PEER_PUSH_SENT)) {
		CDEBUG(D_NET, "Ping/Push race from %s: Push %u dropped\n",
		       libcfs_nid2str(lp->lp_primary_nid),
		       LNET_PING_BUFFER_SEQNO(pbuf));
		goto out;
	}
	 ...
	 * the Push set PING_REQUIRED to force the discovery thread
	 * to fix the problem by pinging the peer.
	 */
...
static void lnet_discovery_event_handler(lnet_event_t *event)
{
	...
	/* non-0 means discovery needs to take another look */
	rc = -EAGAIN;
...
static int lnet_peer_data_present(struct lnet_peer *lp)
{
	 ...
	 *
	 * A special case occurs if the primary NID is the loopback
	 * NID.
	 */
	if (pbuf->pb_info.pi_nnis > 1)
...
static int lnet_peer_ping_failed(struct lnet_peer *lp)
{
	...
	mdh = lp->lp_data_mdh;
	LNetInvalidateHandle(&lp->lp_data_mdh);
	pbuf = lp->lp_data;
	lp->lp_data = NULL;
	lp->lp_state &= ~LNET_PEER_PING_FAILED;
	lp->lp_state |= LNET_PEER_PING_REQUIRED;
...
static int lnet_peer_send_ping(struct lnet_peer *lp)
{
	...
	rc = LNetMDBind(md, LNET_UNLINK, &lp->lp_data_mdh);
	if (rc != 0) {
	...
	rc = LNetGet(LNET_NID_ANY, lp->lp_data_mdh, id,
...
static int lnet_peer_send_push(struct lnet_peer *lp)
{
	...
	rc = LNetPut(LNET_NID_ANY, lp->lp_data_mdh,
