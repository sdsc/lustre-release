...
/* called with exclusive lock held */
static void
lnet_peer_ni_del_locked(struct lnet_peer_ni *lpni)
{
	...
	/*
	 * put peer ni on the zombie list. We need to do that because the
	 * peer ni can still be in use. With MR the cpt of the peer is not
	 * necessarily the same cpt that is currently locked. When we
	 * delete the peer we need to exclusively lock to modify the
	 * ptable. But before we could do that we need to unlock the
	 * currently locked cpt. In case of a delayed delete, there is no
	 * way for us to know the cpt unless we keep passing it along,
	 * which is not necessarily the right thing to do.
	 *
	 * The solution is to move the peer ni to a zombie list when it is
	 * deleted by DLC and when comes time to delete it no extra lock
	 * manipulation is required.
	 */
	list_add_tail(&lpni->lpni_on_global_zombie_list,
		      &the_lnet.ln_peer_ni_zombie_list);

	the_lnet.ln_peer_num_zombie_nis++;
...
lnet_peer_table_cleanup_locked(lnet_ni_t *ni, struct lnet_peer_table *ptable)
{
	...
	for (i = 0; i < LNET_PEER_HASH_SIZE; i++) {
		list_for_each_entry_safe(lpni, tmp, &ptable->pt_hash[i],
					 lpni_hashlist) {
			if (ni != NULL && ni->ni_net != lpni->lpni_net)
				continue;
...
static void
lnet_peer_ni_finalize_wait_locked(int cpt_locked)
{
	int	i = 3;

	while (!list_empty(&the_lnet.ln_peer_ni_zombie_list)) {
...
lnet_peer_tables_cleanup(lnet_ni_t *ni)
{
	...
	lnet_net_lock(LNET_LOCK_EX);
	lnet_peer_ni_finalize_wait_locked(LNET_LOCK_EX);
	lnet_net_unlock(LNET_LOCK_EX);
}
...
lnet_destroy_peer_ni_locked(struct lnet_peer_ni *lpni)
{
	...
	LASSERT(!list_empty(&lpni->lpni_on_global_zombie_list));

	lpni->lpni_net = NULL;

	/* remove the peer ni from the global zombie list */
	list_del_init(&lpni->lpni_on_global_zombie_list);
	the_lnet.ln_peer_num_zombie_nis--;
