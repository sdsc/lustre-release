...
static void
lnet_peer_del_routes_locked(lnet_ni_t *ni, cfs_list_t *peers, int cpt_locked)
{
	lnet_peer_t	*lp;
	lnet_peer_t	*tmp;
	...
	cfs_list_for_each_entry_safe(lp, tmp, peers,
				     lp_hashlist) {
		if (ni == lp->lp_ni) {
			/* Check if this peer is a gateway. */
			if (lp->lp_rtr_refcount) {
				lnet_nid_t gw_nid = lp->lp_nid;
...
static void
lnet_peer_hash_cleanup_locked(lnet_ni_t *ni, cfs_list_t *peers,
			      struct lnet_peer_table *ptable)
{
	lnet_peer_t	*lp;
	lnet_peer_t	*tmp;

	cfs_list_for_each_entry_safe(lp, tmp, peers, lp_hashlist) {
		lp = cfs_list_entry(peers->next, lnet_peer_t, lp_hashlist);
...
static void
lnet_peer_deathrow_wait_locked(struct lnet_peer_table *ptable, int cpt_locked)
{
	int		i;
	...
	for (i = 3; ptable->pt_zombies != 0; i++) {
		...
		if (IS_PO2(i)) {
			CDEBUG(D_WARNING,
			       "Waiting for %d:%d zombies on peer table\n",
			       ptable->pt_zombies, ptable->pt_number);
		}
...
lnet_peer_tables_cleanup(lnet_ni_t *ni)
{
	...
	/* If we are deleting all peers, we must be in shutdown. */
	if (ni == NULL)
		LASSERT(the_lnet.ln_shutdown);
	else {
		/* If just deleting the peers for a NI, get rid
		 * of any routes these peers are gateways for. */
		cfs_percpt_for_each(ptable, i, the_lnet.ln_peer_tables) {
			lnet_net_lock(i);
			...
			for (j = 0; j < LNET_PEER_HASH_SIZE; j++) {
				...
				lnet_peer_del_routes_locked(ni, peers, i);
			}
			lnet_net_unlock(i);
		}
	}
	...
	cfs_percpt_for_each(ptable, i, the_lnet.ln_peer_tables) {
		...
		for (j = 0; j < LNET_PEER_HASH_SIZE; j++) {
			...
			lnet_peer_hash_cleanup_locked(ni, peers, ptable);
		}
	...
	cfs_percpt_for_each(ptable, i, the_lnet.ln_peer_tables) {
		...
		lnet_net_lock(i);
		lnet_peer_deathrow_wait_locked(ptable, i);
...
int lnet_get_peers_info(__u32 count, __u64 *nid, char *aliveness,
			...
			__u32 *peer_tx_qnob)
{
	...
	peer_table = the_lnet.ln_peer_tables[*cpt_count];
	/* if the ptable is NULL then there are no more cpts to examine */
	if (peer_table == NULL)
		return -1;

	lnet_net_lock(*cpt_count);
