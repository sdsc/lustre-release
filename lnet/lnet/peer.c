...
static void
lnet_peer_table_cleanup_locked(struct lnet_ni *ni, struct lnet_peer_table *ptable)
{
	int i;
	struct lnet_peer *lp;
...
static void
lnet_peer_table_del_rtrs_locked(struct lnet_ni *ni, struct lnet_peer_table *ptable,
				int cpt_locked)
{
	struct lnet_peer *lp;
	struct lnet_peer *tmp;
...
lnet_peer_tables_cleanup(struct lnet_ni *ni)
{
	...
	while (!list_empty(&deathrow)) {
		lp = list_entry(deathrow.next, struct lnet_peer,
				lp_hashlist);
