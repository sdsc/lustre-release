...
static int
lnet_eq_wait_locked(time64_t *timeout)
__must_hold(&the_lnet.ln_eq_wait_lock)
{
	time64_t tms = *timeout;
	wait_queue_t wl;
	int wait;
	...
	if (tms < 0) {
		...
		schedule_timeout(nsecs_to_jiffies((u64)tms));
		tms -= jiffies_to_nsecs(jiffies - now);
 ...
 * \param timeout Time in milliseconds to wait for an event to occur on
 ...
 */
int
LNetEQPoll(lnet_handle_eq_t *eventqs, int neq, time64_t timeout,
