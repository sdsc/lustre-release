...
extern int lnet_parse_local(lnet_ni_t *ni, lnet_msg_t *msg);
extern int lnet_parse_forward_locked(lnet_ni_t *ni, lnet_msg_t *msg);
extern void lnet_ni_recv(lnet_ni_t *ni, void *private, lnet_msg_t *msg,
			 int delayed, unsigned int offset,
			 unsigned int mlen, unsigned int rlen);
extern void lnet_drop_message(lnet_ni_t *ni, int cpt, void *private,
			      unsigned int nob);
...
lnet_drop_rule_add(struct lnet_fault_attr *attr)
{
	...
	if (attr->u.drop.da_rate == 0) {
		CDEBUG(D_NET, "drop rate cannot be zero\n");
		RETURN(-EINVAL);
	}
	...
	if (attr->u.drop.da_wait != 0)
		rule->dr_start = cfs_time_current_sec() + attr->u.drop.da_wait;
	...
	CDEBUG(D_NET, "Added drop rule: src %s, dst %s, rate %d, delay %d\n",
	       ...
	       attr->u.drop.da_rate, attr->u.drop.da_wait);
	RETURN(0);
}
...
lnet_drop_rule_reset(void)
{
	...
	list_for_each_entry(rule, &the_lnet.ln_drop_rules, dr_link) {
		...
		rule->dr_drop_at = cfs_rand() % rule->dr_attr.u.drop.da_rate;
...
struct lnet_delay_rule {
	...
	/** the next time to delay a message */
...
lnet_delay_rule_add(struct lnet_fault_attr *attr)
{
	...
	if (attr->u.delay.la_rate == 0 || attr->u.delay.la_latency == 0) {
		CDEBUG(D_NET, "delay rate and latency cannot be zero\n");
		RETURN(-EINVAL);
	}
 ...
 * If \a dst is zero, then all rules have \a src as source will be removed
 *
 * When a drop is removed, all delayed messages of this rule will be processed
 * immediately.
 */
int
lnet_delay_rule_del(lnet_nid_t src, lnet_nid_t dst, bool shutdown)
{
	...
	list_for_each_entry_safe(rule, tmp, &the_lnet.ln_delay_rules, dl_link) {
		...
		CDEBUG(D_NET, "Remove delay rule: src %s->dst: %s (1/%d)\n",
...
lnet_delay_rule_reset(void)
{
	...
	list_for_each_entry(rule, &the_lnet.ln_delay_rules, dl_link) {
		...
		rule->dl_delay_at = cfs_rand() % rule->dl_attr.u.delay.la_rate;
