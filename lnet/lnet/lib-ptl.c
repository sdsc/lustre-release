...
lnet_ptl_enable_mt(struct lnet_portal *ptl, int cpt)
{
        ...
        if (likely(mtab->mt_deadline != 0)) {
                /* match table is already enabled, just update time stamp */
                mtab->mt_deadline = cfs_time_current_sec() + LNET_MT_DEADLINE;
                return;
        }
...
lnet_ptl_disable_mt(struct lnet_portal *ptl, int cpt)
{
	...
	int	i;
        ...
        if (mtab->mt_deadline == 0 ||
            mtab->mt_deadline > cfs_time_current_sec())
                return;
                 ...
                 __u64 mbits, unsigned int intent)
{
	...
	int	cpt;
        ...
        if (ptl->ptl_mtables[cpt]->mt_deadline == 0) {
                /* is there any active entry for this portal? */
                nmaps = ptl->ptl_mt_nmaps;
                /* map to an active mtable to avoid heavy "stealing" */
                if (nmaps != 0) {
			/* NB: there is possibility that ptl_mt_maps could
			 * be changed, but it wouldn't hurt anything */
                        cpt = ptl->ptl_rotor++;
...
int
lnet_ptl_match_msg(struct lnet_portal *ptl, int op_mask, lnet_process_id_t src,
                   unsigned int rlength, unsigned int roffset,
                   __u64 mbits, lnet_msg_t *msg)
{
	 ...
	 * It has more locking overhead and it should be rare */
		...
		mtab = ptl->ptl_mtables[tmp];
                if (mtab->mt_deadline == 0 && i != ncpts - 1)
                        continue;
...
LNetClearLazyPortal(int portal)
{
        ...
        lnet_drop_delayed_msg(&zombies, "Clearing lazy portal attr");
