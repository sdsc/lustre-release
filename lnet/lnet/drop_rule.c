...
lnet_drop_rule_add(struct lnet_drop_rule_attr *attr)
{
	...
	if (!(attr->dra_msg_mask & LNET_DROP_MSG_MASK)) {
		CDEBUG(D_NET, "invalid message type mask %x/%x\n",
		       attr->dra_msg_mask, LNET_DROP_MSG_MASK);
		return -EINVAL;
	}

	if (attr->dra_portal != -1 && attr->dra_portal >= MAX_PORTALS) {
		CDEBUG(D_NET, "invalid portal %d\n", attr->dra_portal);
		return -EINVAL;
	}
...
lnet_drop_rule_del(lnet_nid_t src, lnet_nid_t dst, bool all)
{
	...
	list_for_each_entry_safe(rule, tmp, &the_lnet.ln_drop_rules, dr_link) {
		if (all || (rule->dr_attr.dra_src == src &&
			    rule->dr_attr.dra_dst == dst))
...
drop_rule_match(struct lnet_drop_rule *rule, lnet_nid_t src,
		lnet_nid_t dst, int msg_type, int portal)
{
	bool	drop;
	...
	if (rule->dr_attr.dra_portal != -1 &&
	    rule->dr_attr.dra_portal != portal)
		return false;

	if (!(rule->dr_attr.dra_msg_mask & (1 << msg_type)))
		return false;
	...
	if (rule->dr_count % rule->dr_attr.dra_rate == 0)
		rule->dr_drop_at = rule->dr_count +
