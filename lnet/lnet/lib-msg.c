...
/* Don't need any lock but must be called before lnet_msg_commit */
void
...
lnet_msg_commit(lnet_msg_t *msg, int msg_type, int cpt)
{
        ...
        if (msg->msg_rx_committed) { /* routed message */
        ...
        case LNET_EVENT_SEND: /* active sending */
                LASSERT(msg_type == LNET_MSG_PUT || msg_type == LNET_MSG_GET);
...
lnet_finalize (lnet_ni_t *ni, lnet_msg_t *msg, int status)
{
        ...
        cpt = msg->msg_tx_committed ?
              msg->msg_tx_cpt : msg->msg_rx_cpt;
        lnet_net_lock(cpt);
