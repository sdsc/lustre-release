...
typedef struct
{
	...
	int              *kib_peertxcredits;    /* # concurrent sends to 1 peer_ni */
	int              *kib_peerrtrcredits;   /* # per-peer_ni router buffer credits */
	int              *kib_peercredits_hiw;  /* # when eagerly to return credits */
	int              *kib_peertimeout;      /* seconds to consider peer_ni dead */
...
/* flag to show a peer_ni can understand connrace protocol */
...
static inline bool
kiblnd_peer_win_race(kib_peer_ni_t *peer_ni, kib_msg_t *msg)
{
	if (!peer_ni->ibp_connecting)
		return true; /* no race */

	if (msg->ibm_credits & IBLND_CONNREQ_NOOP) {
		/* peer_ni can understand connrace protocol */

		if (msg->ibm_credits & IBLND_CONNREQ_WIN_WISH)
			return true; /* peer_ni has win wish */

		if (peer_ni->ibp_connrace_win)
			return false; /* I wish to win, reject peer_ni */
	}
	/* tie-break connection race in favour of the higher NID */
	return peer_ni->ibp_nid > peer_ni->ibp_ni->ni_nid;
}
...
void kiblnd_destroy_peer (kib_peer_ni_t *peer_ni);
bool kiblnd_reconnect_peer(kib_peer_ni_t *peer_ni, bool connrace_win);
