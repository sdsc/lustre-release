...
typedef struct
{
	...
	/* # threads on each CPT */
	int		 *kib_nscheds;
	int		 *kib_recovery_interval; /* recovery interval
						    (seconds) */
} kib_tunables_t;
...
#define IBLND_ROUTE_NEEDS_RECOVERY      5        /* needs recovery */
	...
	unsigned long           rt_last_changed; /* last state changed
						    timestamp */
...
struct kib_routes {
	...
	lnet_nid_t		rts_nid;
	/* route lists */
	struct kib_route	rts_route[LNET_MAX_INTERFACES];
	/* number of route entry */
	int			rts_nroute;
	/* next use index */
	int			rts_next;
};

struct kib_interface {
...
typedef struct kib_net
{
	...
	struct kib_interface		ibn_interfaces[LNET_MAX_INTERFACES];
...
struct kib_target {
	...
	int			tg_naddr;
	/* target address */
	__u32			tg_addr[LNET_MAX_INTERFACES];
};

struct kib_targetlist {
...
static inline void
kiblnd_set_route_state(struct kib_route *route, int state)
{
...
kib_peer_t *kiblnd_find_peer_locked(lnet_nid_t nid, struct kib_route *route);
...
void kiblnd_destroy_routes_locked(lnet_ni_t *ni);
int kiblnd_create_routes(lnet_ni_t *ni, lnet_nid_t nid);
struct kib_routes *kiblnd_find_routes_locked(lnet_ni_t *ni, lnet_nid_t nid);
struct kib_route *kiblnd_find_route(lnet_ni_t *ni, lnet_nid_t nid,
			       __u32 addr, kib_dev_t *dev);
struct kib_route *kiblnd_find_route_locked(lnet_ni_t *ni, lnet_nid_t nid,
			       __u32 addr, kib_dev_t *dev);
struct kib_route *kiblnd_next_route(lnet_ni_t *ni, lnet_nid_t nid);
struct kib_route *kiblnd_next_route_locked(lnet_ni_t *ni, lnet_nid_t nid);
