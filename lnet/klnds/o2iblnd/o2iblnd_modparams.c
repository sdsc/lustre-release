...
static int wr_sge = IBLND_WR_SGE;
CFS_MODULE_PARM(wr_sge, "i", int, 0444,
		"# scatter/gatter element per work request");
...
kiblnd_tunables_init (void)
{
	...
	if (*kiblnd_tunables.kib_wr_sge < 1) {
		CWARN("invalid wr_sge value %d, use default %d\n",
		      *kiblnd_tunables.kib_wr_sge, IBLND_WR_SGE);
		*kiblnd_tunables.kib_wr_sge = IBLND_WR_SGE;
	}
