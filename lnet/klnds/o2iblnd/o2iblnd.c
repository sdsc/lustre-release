...
kiblnd_get_peer_info(lnet_ni_t *ni, int index,
		     lnet_nid_t *nidp, int *count, int *cpt,
...
kiblnd_ctl(lnet_ni_t *ni, unsigned int cmd, void *arg)
{
	...
	case IOC_LIBCFS_GET_PEER: {
		int index = data->ioc_count, count = 0, cpt = 0;
		struct lnet_ioctl_peer *data_peer = NULL;
		__u32 active_conn = 0, waiting_conn = 0;
		__u32 connecting = 0, accepting = 0;
		lnet_nid_t nid = 0;
		...
		rc = kiblnd_get_peer_info(ni, index, &nid, &count, &cpt,
		...
		if (data_peer != NULL) {
			...
			data_peer->pr_lnd_u.pr_o2iblnd.peer_ref_count = count;
			data_peer->pr_lnd_u.pr_o2iblnd.connecting = connecting;
			data_peer->pr_lnd_u.pr_o2iblnd.accepting = accepting;
			data_peer->pr_lnd_u.pr_o2iblnd.active_conn = active_conn;
			data_peer->pr_lnd_u.pr_o2iblnd.waiting_conn = waiting_conn;
		} else {
			data->ioc_nid = nid;
	...
	case IOC_LIBCFS_GET_CONN: {
		...
		/* iWarp has no path MTU so its path_mtu is zero. All others this
		 * is not the case. */
		...
		if (data_conn != NULL) {
			...
			if (data_conn->conn_detail) {
				/* get more details on each connection */
				kiblnd_get_conn_detail(conn,
						data_conn->conn_lnd_u.o2iblnd_conn.q_type,
