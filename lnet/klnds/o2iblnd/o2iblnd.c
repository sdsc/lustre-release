	...
	read_lock_irqsave(&kiblnd_data.kib_global_lock, flags);
		...
		list_for_each(ptmp, &kiblnd_data.kib_peers[i]) {
			...
			*nidp = peer->ibp_nid;
			...
			details->cpt = lnet_cpt_of_nid(peer->ibp_nid);
			details->pid = LNET_PID_LUSTRE;
...
void
kiblnd_get_conn_detail(kib_conn_t *conn, struct lnet_ioctl_conn *data_conn)
{
	__u32 conn_queue_type = data_conn->conn_lnd_u.o2iblnd.queue_type;
	struct ioctl_tx_queue *tx_q = data_conn->conn_lnd_u.o2iblnd.tx_q;
	struct list_head *tmp;
	struct list_head *q;
	int i = 0;
	...
	list_for_each(tmp, q) {
		kiblnd_get_tx_info(list_entry(tmp, kib_tx_t, tx_list),
				   &tx_q[i]);
		i++;
		/*
	...
	data_conn->conn_lnd_u.o2iblnd.num_entries = i;
}
...
kiblnd_ctl(lnet_ni_t *ni, unsigned int cmd, void *arg)
{
	...
	case IOC_LIBCFS_GET_CONN: {
		...
		if (data_conn != NULL) {
			...
			data_conn->conn_lnd_u.o2iblnd.path_mtu = path_mtu;

			/* get more details on each connection */
			if (data_conn->conn_lnd_u.o2iblnd.queue_type > 0)
				kiblnd_get_conn_detail(conn, data_conn);
		} else {
