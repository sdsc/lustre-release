...
void
kiblnd_destroy_routes_locked(lnet_ni_t *ni)
{
	kib_net_t    *net    = (kib_net_t *)ni->ni_data;
	struct kib_routes *routes;

	while (!list_empty(&net->ibn_routes)) {
		routes = list_entry(net->ibn_routes.next, struct kib_routes,
				    rts_list);
		list_del(&routes->rts_list);
		LIBCFS_FREE(routes, sizeof(struct kib_routes));
	}
...
kiblnd_create_routes(lnet_ni_t *ni, lnet_nid_t nid)
{
	...
	LASSERT(routes->rts_nroute > 0 &&
		routes->rts_nroute < LNET_MAX_INTERFACES);
	...
	for (i = 0; i < routes->rts_nroute; i++) {
		...
		routes->rts_route[i].rt_addr =
			(tn) ? kiblnd_target_nid2addr(nid, i % tn) :
			LNET_NIDADDR(nid);
...
kiblnd_update_routes(lnet_ni_t *ni, lnet_nid_t nid)
{
	...
	list_for_each(tmp, &net->ibn_routes) {
		routes = list_entry(tmp, struct kib_routes, rts_list);
	...
	for (i = 0; i < routes->rts_nroute; i++) {
		if (i < oroutes) {
			...
			if (routes->rts_route[i].rt_addr != dest) {
				...
				kiblnd_set_route_state(&routes->rts_route[i],
						       IBLND_ROUTE_INIT);
			}
		} else {
			/* Add new route */
...
struct kib_route *
kiblnd_find_route_locked(lnet_ni_t *ni, lnet_nid_t nid, __u32 addr,
			 kib_dev_t *dev)
{
	kib_net_t     *net = (kib_net_t *)ni->ni_data;
	struct kib_routes  *routes = NULL;
	...
	if (kiblnd_create_routes(ni, nid) != 0)
		return NULL;
...
static void
kiblnd_recovery_route(struct kib_routes *routes, int force)
{
	int            i;
	unsigned long  flags;
	struct kib_route   *tmp_route  = NULL;
...
kiblnd_next_route_locked(lnet_ni_t *ni, lnet_nid_t nid)
{
	...
	struct kib_route   *route  = NULL;
	struct kib_route   *tmp_route  = NULL;
	struct kib_route   *alt_route  = NULL;
	int            first;
	...
	list_for_each(tmp, &net->ibn_routes) {
		...
		if (routes->rts_nid == nid) {
			...
			do {
				tmp_route =
				  &routes->rts_route[routes->rts_next];
				if (tmp_route->rt_dev->ibd_state ==
					IBLND_DEV_PORT_ACTIVE ||
				    tmp_route->rt_dev->ibd_state ==
					IBLND_DEV_PORT_DEGRADE) {
					if (tmp_route->rt_state <=
					    IBLND_ROUTE_ESTABLISHED) {
						route = tmp_route;
						alt_route = NULL;
					} else if(!alt_route) {
						alt_route = tmp_route;
				...
				if (tmp_route->rt_state ==
				   ...
				   time_after(jiffies,
					      tmp_route->rt_last_changed +
					      (*kiblnd_tunables.
						kib_recovery_interval * HZ))) {
					if (tmp_route->rt_dev->ibd_state ==
						IBLND_DEV_PORT_ACTIVE ||
					   tmp_route->rt_dev->ibd_state ==
						IBLND_DEV_PORT_DEGRADE) {
				...
				++routes->rts_next;
				routes->rts_next =
				  routes->rts_next % routes->rts_nroute;
			...
			if (!route && alt_route) {
				...
				route = alt_route;
				kiblnd_set_route_state(route,
						       IBLND_ROUTE_RECOVERY);
			}
...
kiblnd_create_peer(lnet_ni_t *ni, kib_peer_t **peerp, lnet_nid_t nid,
		   struct kib_route *route)
{
	kib_peer_t	*peer;
	kib_net_t	*net = ni->ni_data;
	struct kib_interface  *iface;
	int		cpt = lnet_cpt_of_nid(nid);
	unsigned long   flags;
...
kib_peer_t *
kiblnd_find_peer_locked(lnet_nid_t nid, struct kib_route *route)
{
...
kiblnd_destroy_conn (kib_conn_t *conn)
{
	...
	if (conn->ibc_state != IBLND_CONN_INIT) {
		kib_net_t *net = peer->ibp_ni->ni_data;
		struct kib_interface *iface = &net->ibn_interfaces
			[peer->ibp_route->rt_dev->ibd_index];
...
kiblnd_add_o2ibs(lnet_nid_t nid, int ipnaddr, __u32 iplen, char *ipbuf)
{
	...
	if (ipnaddr > LNET_MAX_INTERFACES)
		return -EINVAL;
	...
	LIBCFS_ALLOC(target, sizeof(struct kib_target));
...
kiblnd_init_fmr_poolset(kib_fmr_poolset_t *fps, int cpt, kib_net_t *net,
			int index, int pool_size, int flush_trigger)
{
	struct kib_interface *iface = &net->ibn_interfaces[index];
	kib_fmr_pool_t *fpo;
	int		rc;
...
int
kiblnd_fmr_pool_map(kib_dev_t *dev, kib_fmr_poolset_t *fps, __u64 *pages,
...
static int
kiblnd_create_pmr_pool(kib_dev_t *dev, kib_poolset_t *ps, int size,
		       kib_pool_t **pp_po)
{
...
static int
kiblnd_create_tx_pool(kib_dev_t *dev, kib_poolset_t *ps, int size,
		      kib_pool_t **pp_po)
{
        int            i;
        int            npg;
	...
	LIBCFS_CPT_ALLOC(tpo->tpo_tx_descs, lnet_cpt_table(), ps->ps_cpt,
        ...
        for (i = 0; i < size; i++) {
                ...
                tx->tx_pool = tpo;
		if (ps->ps_net->ibn_interfaces[dev->ibd_index].ibi_fmr_ps !=
		    NULL) {
...
kiblnd_net_init_pools(kib_net_t *net, int index, __u32 *cpts, int ncpts)
{
	...
	for (i = 0; i < ncpts; i++) {
		...
		rc = kiblnd_init_fmr_poolset(iface->ibi_fmr_ps[cpt], cpt, net,
					     index, kiblnd_fmr_pool_size(ncpts),
...
kiblnd_port_get_attr(kib_hca_dev_t *hdev)
{
	...
	if (rc == 0) {
		...
		if (ibdev->ibd_state == IBLND_DEV_PORT_ACTIVE) {
			...
			if (port_link_speed <
			    IBLND_DEV_LINK_SPEED_MAX(max_rate))
...
#endif
{
	struct kib_event  *work;
	kib_hca_dev_t  *hdev;

	work = container_of(workp, struct kib_event, ibe_work);
	hdev = work->ibe_dev;
...
kiblnd_shutdown (lnet_ni_t *ni)
{
	...
	for (index = 0; index < net->ibn_ninterfaces; index++) {
		...
		case IBLND_INIT_ALL:
			...
			i = 2;
			while (atomic_read(&net->ibn_interfaces[index].
					   ibi_npeers) != 0) {
				i++;
				/* 2**n? */
				CDEBUG(((i & (-i)) == i) ? D_WARNING : D_NET,
					...
					atomic_read(&net->ibn_interfaces[index].
						    ibi_npeers));
				set_current_state(TASK_UNINTERRUPTIBLE);
				schedule_timeout(cfs_time_seconds(1));
			}
			...
			write_lock_irqsave(g_lock, flags);
			LASSERT(net->ibn_interfaces[index].ibi_dev->ibd_nnets >
				0);
		...
		case IBLND_INIT_NOTHING:
			LASSERT(atomic_read(&net->ibn_interfaces[index].
					    ibi_nconns) == 0);

			if (net->ibn_interfaces[index].ibi_dev != NULL &&
			    net->ibn_interfaces[index].ibi_dev->ibd_nnets == 0)
				kiblnd_destroy_dev(net->ibn_interfaces[index].
						   ibi_dev);
	...
	write_lock_irqsave(g_lock, flags);
	kiblnd_destroy_routes_locked(ni);
	write_unlock_irqrestore(g_lock, flags);
...
kiblnd_startup (lnet_ni_t *ni)
{
        ...
        kib_net_t                *net;
	struct kib_interface		 *iface = NULL;
        struct timeval            tv;
        unsigned long             flags;
        int                       rc;
	int			  newdev;
	...
	for (index = 0; index < LNET_MAX_INTERFACES; index++) {
		...
		rc = kiblnd_net_init_pools(net, index, ni->ni_cpts,
					   ni->ni_ncpts);
		if (rc != 0) {
...
failed:
	if (net != NULL && iface != NULL && iface->ibi_dev == NULL &&
	    ibdev != NULL)
