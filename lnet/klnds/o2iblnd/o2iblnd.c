...
kiblnd_ctl(lnet_ni_t *ni, unsigned int cmd, void *arg)
{
	...
	case IOC_LIBCFS_GET_PEER: {
		...
		if (data_peer != NULL) {
			...
			data_peer->pr_lnd_u.pr_peer_details.peer_ref_count = count;

			data_peer->pr_lnd_u.pr_peer_details.connecting = connecting;
			data_peer->pr_lnd_u.pr_peer_details.accepting = accepting;
			data_peer->pr_lnd_u.pr_peer_details.active_conn = active_conn;
			data_peer->pr_lnd_u.pr_peer_details.waiting_conn = waiting_conn;
		} else {
			data->ioc_nid = nid;
