...
kiblnd_destroy_fmr_pool(kib_fmr_pool_t *fpo)
{
	...
	if (fpo->fpo_is_fmr) {
		if (fpo->fmr.fpo_fmr_pool != NULL)
			ib_destroy_fmr_pool(fpo->fmr.fpo_fmr_pool);
	} else {
		struct kib_fast_reg_descriptor *frd;
		int i;

		for (i = 0; !list_empty(&fpo->fast_reg.fpo_pool_list); i++) {
			frd = list_entry(fpo->fast_reg.fpo_pool_list.next,
...
kiblnd_create_fmr_pool(kib_fmr_poolset_t *fps, kib_fmr_pool_t **pp_fpo)
{
	...
	if (fpo->fpo_hdev->ibh_ibdev->alloc_fmr &&
		...
		rc = -ENOSYS;
		LCONSOLE_ERROR_MSG(rc, "IB device does not support FMRs nor "
					"FastRegs, can't register memory\n");
		goto out_dev_attr;
	}
	...
	if (fpo->fpo_is_fmr) {
		...
		fpo->fmr.fpo_fmr_pool = ib_create_fmr_pool(
						fpo->fpo_hdev->ibh_pd,
						&param);
		...
		for (i = 0; i < fps->fps_pool_size; i++) {
			LIBCFS_CPT_ALLOC(frd, lnet_cpt_table(), fps->fps_cpt,
					 sizeof(*frd));
			if (frd == NULL) {
				CERROR("Failed to allocate a new "
					"fast_reg descriptor\n");
				rc = -ENOMEM;
				goto out_desc;
			}
			...
			frd->frd_frpl = ib_alloc_fast_reg_page_list(
						fpo->fpo_hdev->ibh_ibdev,
						LNET_MAX_PAYLOAD/PAGE_SIZE);
			if (IS_ERR(frd->frd_frpl)) {
				rc = PTR_ERR(frd->frd_frpl);
				CERROR("Failed to allocate "
					"ib_fast_reg_page_list: %d\n", rc);
				goto out_middle;
			}

			frd->frd_mr = ib_alloc_fast_reg_mr(
						fpo->fpo_hdev->ibh_pd,
						LNET_MAX_PAYLOAD/PAGE_SIZE);
...
out_middle:
	if (frd->frd_mr != NULL)
		ib_dereg_mr(frd->frd_mr);
	if (frd->frd_frpl != NULL)
		ib_free_fast_reg_page_list(frd->frd_frpl);
	LIBCFS_FREE(frd, sizeof(*frd));
out_desc:
	while (!list_empty(&fpo->fast_reg.fpo_pool_list)) {
		frd = list_entry(fpo->fast_reg.fpo_pool_list.next,
				 struct kib_fast_reg_descriptor, frd_list);
		list_del(&frd->frd_list);
		ib_dereg_mr(frd->frd_mr);
		ib_free_fast_reg_page_list(frd->frd_frpl);
		LIBCFS_FREE(frd, sizeof(*frd));
