...
kiblnd_passive_connect(struct rdma_cm_id *cmid, void *priv, int priv_nob)
{
	...
	if (reqmsg->ibm_u.connparams.ibcp_max_frags >> IBLND_FRAG_SHIFT >
	    kiblnd_rdma_frags(version, ni)) {
		CWARN("Can't accept conn from %s (version %x): "
		      "max message size %d is too large (%d wanted)\n",
		      libcfs_nid2str(nid), version,
		      reqmsg->ibm_u.connparams.ibcp_max_frags >> IBLND_FRAG_SHIFT,
		      kiblnd_rdma_frags(version, ni));
		...
		goto failed;
	} else if ((reqmsg->ibm_u.connparams.ibcp_max_frags >> IBLND_FRAG_SHIFT <
		   (kiblnd_rdma_frags(version, ni))) &&
		   net->ibn_fmr_ps == NULL) {
		CWARN("Can't accept conn from %s (version %x): "
	...
	peer->ibp_max_frags = reqmsg->ibm_u.connparams.ibcp_max_frags >> IBLND_FRAG_SHIFT;
