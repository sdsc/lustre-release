...
kiblnd_get_idle_tx(kib_dev_t *dev, lnet_ni_t *ni, lnet_nid_t target)
{
	...
	struct kib_interface	*iface = &net->ibn_interfaces[dev->ibd_index];
...
kiblnd_fmr_map_tx(kib_net_t *net, kib_tx_t *tx, kib_rdma_desc_t *rd, int nob)
{
	...
	rc = kiblnd_fmr_pool_map(hdev->ibh_dev, fps, pages, npages, 0,
				 &tx->tx_u.fmr);
        if (rc != 0) {
              ...
              kib_rdma_desc_t *rd, int nfrags)
{
        ...
        kib_net_t          *net   = ni->ni_data;
	struct kib_interface    *iface =
		&net->ibn_interfaces[hdev->ibh_dev->ibd_index];
...
kiblnd_peer_notify (kib_peer_t *peer)
{
	...
	for (i = 0; i < routes->rts_nroute; i++) {
		...
		if (routes->rts_route[i].rt_state <= IBLND_ROUTE_ESTABLISHED &&
		    (routes->rts_route[i].rt_dev->ibd_state ==
...
kiblnd_passive_connect(struct rdma_cm_id *cmid, void *priv, int priv_nob)
{
	...
	route = kiblnd_find_route(ni, nid, ntohl(peer_addr->sin_addr.s_addr),
				  ibdev);
	if (route == NULL) {
...
kiblnd_connd (void *arg)
{
	...
	spin_lock_irqsave(&kiblnd_data.kib_connd_lock, flags);
		...
		if (!list_empty(&kiblnd_data.kib_connd_recovery)) {
			...
			kiblnd_launch_tx(route->rt_ni, NULL, route->rt_nid,
					 route);
