...
kiblnd_get_idle_tx(lnet_ni_t *ni, lnet_nid_t target)
{
	...
	/* Get peer max frags from peer table or default to ours */
	read_lock_irqsave(&kiblnd_data.kib_global_lock, flags);

	peer = kiblnd_find_peer_locked(target);
	if (peer != NULL)
		tx->tx_max_frags = peer->ibp_max_frags;
	else
		tx->tx_max_frags = IBLND_CFG_RDMA_FRAGS;
...
kiblnd_init_tx_msg (lnet_ni_t *ni, kib_tx_t *tx, int type, int body_nob)
{
	...
	LASSERT(tx->tx_nwrq >= 0);
	LASSERT(tx->tx_nwrq < IBLND_CFG_RDMA_FRAGS + 1);
...
kiblnd_launch_tx (lnet_ni_t *ni, kib_tx_t *tx, lnet_nid_t nid)
{
	...
	rc = kiblnd_create_peer(ni, &peer, nid, NULL);
	if (rc != 0) {
...
kiblnd_passive_connect (struct rdma_cm_id *cmid, void *priv, int priv_nob)
{
	...
	if (reqmsg->ibm_u.connparams.ibcp_queue_depth >
		...
		CERROR("Can't accept %s: queue depth too large: "
		       " %d (<=%d wanted)\n",
		       libcfs_nid2str(nid),
	...
	if (reqmsg->ibm_u.connparams.ibcp_max_frags >
		...
		goto failed;
		   ...
		   net->ibn_pmr_ps == NULL) {
		CERROR("Can't accept %s (version %x): "
		       "max_frags %d incompatible without FMR/PMR pool "
		       "(%d wanted)\n",
		       libcfs_nid2str(nid), version,
		       reqmsg->ibm_u.connparams.ibcp_max_frags,
		       IBLND_RDMA_FRAGS(version));
	...
	rc = kiblnd_create_peer(ni, &peer, nid, &reqmsg->ibm_u.connparams);
	if (rc != 0) {
	...
	ackmsg->ibm_u.connparams.ibcp_queue_depth  = reqmsg->ibm_u.connparams.ibcp_queue_depth;
	ackmsg->ibm_u.connparams.ibcp_max_msg_size = IBLND_MSG_SIZE;
	ackmsg->ibm_u.connparams.ibcp_max_frags    = reqmsg->ibm_u.connparams.ibcp_max_frags;
                  ...
                  __u64 incarnation, int why, kib_connparams_t *cp)
{
	...
	case IBLND_REJECT_RDMA_FRAGS:
		if (peer->ibp_max_frags <= cp->ibcp_max_frags) {
	...
	case IBLND_REJECT_MSG_QUEUE_SIZE:
		if (peer->ibp_queue_depth <= cp->ibcp_queue_depth) {
	...
	CNETERR("%s: retrying (%s), %x, %x, "
		...
		peer->ibp_queue_depth, peer->ibp_max_frags,
...
kiblnd_check_connreply (kib_conn_t *conn, void *priv, int priv_nob)
{
	...
	peer->ibp_queue_depth      = msg->ibm_u.connparams.ibcp_queue_depth;
	peer->ibp_max_frags        = msg->ibm_u.connparams.ibcp_max_frags;
...
kiblnd_active_connect (struct rdma_cm_id *cmid)
{
	...
	msg->ibm_u.connparams.ibcp_queue_depth  = peer->ibp_queue_depth;
	msg->ibm_u.connparams.ibcp_max_frags    = peer->ibp_max_frags;
