...
kiblnd_connect_peer (kib_peer_t *peer)
{
	...
	CDEBUG(D_NET, "%s: connection bound to %s:%pIS:%s\n",
               libcfs_nid2str(peer->ibp_nid), dev->ibd_ifname,
	       &srcaddr.sin_addr.s_addr, cmid->device->name);
...
kiblnd_passive_connect(struct rdma_cm_id *cmid, void *priv, int priv_nob)
{
	...
	LASSERT (ibdev != NULL);
		...
		CERROR("Peer's port (%pISc) is not privileged\n",
		       peer_addr);
                goto failed;
        }
		...
		CERROR("Can't accept %s on %s (%s:%d:%pSI): "
                       "bad dst nid %s\n", libcfs_nid2str(nid),
