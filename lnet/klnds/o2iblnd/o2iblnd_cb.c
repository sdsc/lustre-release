...
__must_hold(&conn->ibc_lock)
{
	...
	list_add(&tx->tx_list, &conn->ibc_active_txs);
		...
		kib_wr_t *bad = &tx->tx_wrq[tx->tx_nwrq - 1];
		kib_wr_t *wrq = &tx->tx_wrq[0];

		if (frd != NULL) {
			if (!frd->frd_valid) {
				wrq = &frd->frd_inv_wr;
#ifdef HAVE_IB_MAP_MR_SG
				wrq->wr.next = &frd->frd_fastreg_wr.wr;
			} else {
		...
		LASSERTF(bad->wr.wr_id == kiblnd_ptr2wreqid(tx, IBLND_WID_TX),
			 "bad wr_id "LPX64", opc %d, flags %d, peer: %s\n",
			 bad->wr.wr_id, bad->wr.opcode, bad->wr.send_flags,
			 libcfs_nid2str(conn->ibc_peer->ibp_nid));
#else
			...
			frd->frd_fastreg_wr.next = &tx->tx_wrq[0];
		}
