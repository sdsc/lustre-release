...
__must_hold(&conn->ibc_lock)
{
	...
	list_add(&tx->tx_list, &conn->ibc_active_txs);
		...
		LASSERTF(wrq->wr_id == kiblnd_ptr2wreqid(tx, IBLND_WID_TX),
			 "bad wr_id "LPX64", opc %d, flags %d, peer_ni: %s\n",
...
bool
kiblnd_reconnect_peer(kib_peer_ni_t *peer_ni, bool connrace_win)
{
	...
	LASSERT(!peer_ni->ibp_connrace_win);
	/*
	 * If I have lost connrace for enough times and want to win the race,
	 * I need to flag this peer_ni so I can use IBLND_CONNREQ_WIN_WISH for
	 * the next connection request, and reject incoming connection request
	 * from the peer_ni.
	 */
	if (peer_ni->ibp_nid > peer_ni->ibp_ni->ni_nid)
		peer_ni->ibp_connrace_win = connrace_win;
...
kiblnd_passive_connect(struct rdma_cm_id *cmid, void *priv, int priv_nob)
{
	...
	ni  = lnet_nid2ni(reqmsg->ibm_dstnid);
...
kiblnd_active_connect (struct rdma_cm_id *cmid)
{
	...
	kiblnd_pack_msg(peer_ni->ibp_ni, msg, version, connreq,
			peer_ni->ibp_nid, incarnation);
