...
_kgnilnd_proc_run_cksum_test(int caseno, int nloops, int nob)
{
	...
	for (i = 0; i < LNET_MAX_IOV; i++) {
		...
		src[i].kiov_page = alloc_page(__GFP_WAIT | __GFP_IO | __GFP_FS | __GFP_ZERO);
		...
		dest[i].kiov_page = alloc_page(__GFP_WAIT | __GFP_IO | __GFP_FS | __GFP_ZERO);
...
static struct file_operations kgn_cksum_test_fops = {
	.owner   = THIS_MODULE,
...
static struct file_operations kgn_stats_fops = {
	.owner   = THIS_MODULE,
...
{
	return single_open(file, kgnilnd_proc_peer_conns_seq_show, PDE_DATA(inode));
}

static struct file_operations kgn_peer_conns_fops = {
	.owner   = THIS_MODULE,
...
kgnilnd_proc_init(void)
{
	...
	pde = proc_create(GNILND_PROC_CKSUM_TEST, 0200, kgn_proc_root, &kgn_cksum_test_fops);
	if (pde == NULL) {
	...
	/* Initialize STATS */
	pde = proc_create(GNILND_PROC_STATS, 0644, kgn_proc_root, &kgn_stats_fops);
	if (pde == NULL) {
		CERROR("couldn't create proc entry %s\n", GNILND_PROC_STATS);
	...
	/* Initialize SMSG */
	pde = proc_create(GNILND_PROC_SMSG, 0444, kgn_proc_root, &kgn_smsg_fops);
	if (pde == NULL) {
		CERROR("couldn't create proc entry %s\n", GNILND_PROC_SMSG);
	...
	/* Initialize CONN */
	pde = proc_create(GNILND_PROC_CONN, 0444, kgn_proc_root, &kgn_conn_fops);
	if (pde == NULL) {
		CERROR("couldn't create proc entry %s\n", GNILND_PROC_CONN);
	...
	pde = proc_create(GNILND_PROC_PEER_CONNS, 0644, kgn_proc_root, &kgn_peer_conns_fops);
	if (pde == NULL) {
	...
	/* Initialize PEER */
	pde = proc_create(GNILND_PROC_PEER, 0444, kgn_proc_root, &kgn_peer_fops);
	if (pde == NULL) {
		CERROR("couldn't create proc entry %s\n", GNILND_PROC_PEER);
