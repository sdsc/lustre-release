...
kgnilnd_ctl(lnet_ni_t *ni, unsigned int cmd, void *arg)
{
	...
	case IOC_LIBCFS_GET_PEER: {
		...
		rc = kgnilnd_get_peer_info(index, &peer,
		...
		if (data_peer == NULL) {
			...
			data_peer->lnd_u.gnilnd.peer_status = peer_connecting;
			data_peer->lnd_u.gnilnd.peer_ref_count = peer_refcount;
		}
		...
		if (rc) {
			...
			if (data_peer == NULL) {
				...
				data_peer->lnd_u.gnilnd.dev_id = device_id;
				data_peer->lnd_u.gnilnd.peer_stamp = peerstamp;
				data_peer->lnd_u.gnilnd.fmaq_len = fmaq_len;
				data_peer->data->lnd_u.gnilnd.nfma = nfma;
				data_peer->lnd_u.gnilnd.tx_seq = tx_seq;
				data_peer->lnd_u.gnilnd.rx_seq = rx_seq;
				data_peer->lnd_u.gnilnd.nrdma = nrdma;
			}
		}
		break;
	}
