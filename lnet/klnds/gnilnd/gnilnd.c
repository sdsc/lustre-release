...
kgnilnd_find_or_create_conn_locked(kgn_peer_t *peer) {
	...
	kgnilnd_schedule_dgram(peer->gnp_cpt);
...
int kgnilnd_base_startup(void)
{
	...
	cfs_percpt_for_each(sched, i, kgnilnd_data.kgn_scheds) {
		...
		init_waitqueue_head(&sched->kgn_dgram_waitq);
		init_waitqueue_head(&sched->kgn_dgping_waitq);
	}
...
kgnilnd_base_shutdown(void)
{
	...
	cfs_percpt_for_each(sched, i, kgnilnd_data.kgn_scheds) {
		...
		wake_up_all(&sched->kgn_dgram_waitq);
		wake_up_all(&sched->kgn_dgping_waitq);
	}
...
kgnilnd_startup(lnet_ni_t *ni)
{
	...
	/* Total maximum number of threads per GNI device */
	...
	for (i = 0; i < ni->ni_ncpts; i++) {
		...
		if (net->gnn_dev->gnd_dgram_cpt == -1) {
			net->gnn_dev->gnd_dgram_cpt = cpt;
			...
			net->gnn_dev->gnd_dgram_cpt = cpt;
		} else if (net->gnn_dev->gnd_dgram_cpt == cpt) {
			dgram_mismatch = false;
		}
