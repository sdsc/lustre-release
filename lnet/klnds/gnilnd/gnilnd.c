...
kgnilnd_get_peer_info(int index, kgn_peer_t **found_peer, lnet_nid_t *id,
		      __u32 *nic_addr, int *peer_connecting, int *refcount,
		      int *accepting, int *connecting, int *active_conns,
		      int *waiting_conns)
{
	...
	for (i = 0; i < *kgnilnd_tunables.kgn_peer_hash_size; i++) {
		...
		list_for_each(ptmp, &kgnilnd_data.kgn_peers[i]) {
			...
			*peer_connecting  = peer->gnp_connecting;
...
kgnilnd_ctl(lnet_ni_t *ni, unsigned int cmd, void *arg)
{
	...
	case IOC_LIBCFS_GET_PEER: {
		...
		int accepting = 0, connecting = 0;
		int active_conns = 0, waiting_conns = 0;
		int tx_seq = 0, rx_seq = 0;
		int fmaq_len = 0, nfma = 0, nrdma = 0;
		struct lnet_ioctl_peer *data_peer = NULL;

		/* For the PEER_INFO case pass back a different data
		 * structure */
		...
		rc = kgnilnd_get_peer_info(index, &peer, &nid, &nic_addr,
					   &peer_connecting, &peer_refcount,
					   &accepting, &connecting,
					   &active_conns, &waiting_conns);
		if (rc)
			break;
		...
		if (data_peer == NULL) {
			...
			data_peer->pr_lnd_u.pr_peer_details.peer_ref_count = peer_refcount;
			data_peer->pr_lnd_u.pr_peer_details.active_conns = active_conns;
			data_peer->pr_lnd_u.pr_peer_details.waiting_conns = waiting_conns;

			data_peer->pr_lnd_u.pr_peer_details.pr_lnd.gnilnd.peer_status = peer_connecting;
		}
		...
		if (rc) {
			...
			if (data_peer != NULL)
				data_peer->pr_lnd_u.pr_peer_details.pr_lnd.gnilnd.peer_status |= (1 << 16);
			else
				data->ioc_flags |= (1 << 16);
			rc = 0;
		} else {
			/* still barf in the case of old PEER_INFO */
			if (data_peer == NULL) {
				...
				data_peer->pr_lnd_u.pr_peer_details.pr_lnd.gnilnd.dev_id = device_id;
				data_peer->pr_lnd_u.pr_peer_details.pr_lnd.gnilnd.peer_stamp = peerstamp;
				data_peer->pr_lnd_u.pr_peer_details.pr_lnd.gnilnd.fmaq_len = fmaq_len;
				data_peer->pr_lnd_u.pr_peer_details.pr_lnd.gnilnd.nfma = nfma;
				data_peer->pr_lnd_u.pr_peer_details.pr_lnd.gnilnd.tx_seq = tx_seq;
				data_peer->pr_lnd_u.pr_peer_details.pr_lnd.gnilnd.rx_seq = rx_seq;
				data_peer->pr_lnd_u.pr_peer_details.pr_lnd.gnilnd.nrdma = nrdma;
			}
		}
		break;
	}
