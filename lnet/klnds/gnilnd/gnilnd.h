...
typedef struct kgn_tunables {
	...
	int              *kgn_fast_reconn;      /* fast reconnection on connection timeout */
	int              *kgn_efault_lbug;      /* Should we LBUG on receiving an EFAULT */
	int		 *kgn_max_purgatory;    /* How many connections per peer to keep in purgatory */
...
typedef struct kgn_data {
	...
	unsigned long           free_pages_limit;     /* number of free pages reserve from fma block allocations */
} kgn_data_t;
