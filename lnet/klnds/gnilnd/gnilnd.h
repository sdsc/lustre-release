...
struct kgn_sched_info {
	unsigned long           kgn_alive;		/* scheduler thread alive stamp */
	short                   kgn_ready;		/* stuff to do in scheduler thread */
	wait_queue_head_t	kgn_waitq;		/* scheduler wakeup */
...
typedef struct kgn_device {
	...
	struct kgn_sched_info   **gnd_scheds;	  /* CPU partiton info mapped to this device */
	int			gnd_ncpts;	  /* Number of CPU partitons for this device */
...
void kgnilnd_schedule_device(struct kgn_sched_info *sched);
