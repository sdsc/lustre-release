...
kgnilnd_dgram_waitq(void *arg)
{
	...
	if (rc != 0) {
		CERROR("Can't set CPT affinity for kgnilnd_dg_%02d to %d: %d\n",
		       dev->gnd_id, dev->gnd_dgram_cpt, rc);
	}
	...
	while (!kgnilnd_data.kgn_shutdown) {
		...
		if (grc == GNI_RC_SUCCESS) {
			...
			prepare_to_wait(&sched->kgn_dgping_waitq, &mover_done,
					TASK_INTERRUPTIBLE);
			...
			finish_wait(&sched->kgn_dgping_waitq, &mover_done);
		}
	}
...
{
	struct kgn_cpt_info *sched = kgnilnd_data.kgn_scheds[(int) arg];
...
kgnilnd_dgram_mover(void *arg)
{
	...
	while (!kgnilnd_data.kgn_shutdown) {
		...
		prepare_to_wait(&sched->kgn_dgram_waitq, &wait, TASK_INTERRUPTIBLE);

		setup_timer(&timer, kgnilnd_dgram_poke_with_stick, sched->kgn_cpt);
		mod_timer(&timer, (long) jiffies + timeout);

		/* last second chance for others to poke us */
		did_something += xchg(&sched->kgn_dgram_ready, GNILND_DGRAM_IDLE);
		...
		finish_wait(&sched->kgn_dgram_waitq, &wait);
	}
