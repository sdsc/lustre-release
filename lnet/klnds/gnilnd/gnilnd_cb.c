...
void
kgnilnd_schedule_device(struct kgn_sched_info *sched)
{
	short already_live = 0;

	/* we'll only want to wake if the scheduler thread
	 * has come around and set ready to zero */
	already_live = cmpxchg(&sched->kgn_ready, GNILND_DEV_IDLE, GNILND_DEV_IRQ);
	if (!already_live) {
		wake_up_all(&sched->kgn_waitq);
	}
	return;
}
...
{
	struct kgn_sched_info *sched = (struct kgn_sched_info *) arg;
...
kgnilnd_device_callback(__u32 devid, __u64 arg)
{
	...
	if (dev->gnd_id == devid) {
		...
		for (i = 0; i < dev->gnd_ncpts; i++)
			kgnilnd_schedule_device(dev->gnd_scheds[i]);
	} else {
...
{
	struct kgn_sched_info *sched = tx->tx_conn->sched;
		...
		spinlock_t *state_lock, kgn_tx_list_state_t state)
{
	...
	/* rmb for gnd_ready */
	smp_rmb();
	if (conn->sched->kgn_ready == GNILND_DEV_LOOP) {
		rc = 0;
...
kgnilnd_queue_rdma(kgn_conn_t *conn, kgn_tx_t *tx)
{
	...
	kgnilnd_schedule_device(tx->tx_conn->sched);
}
...
int
kgnilnd_check_rdma_cq(struct kgn_sched_info *sched, kgn_device_t *dev)
{
	gni_return_t           rrc;
...
int
kgnilnd_check_fma_send_cq(struct kgn_sched_info *sched, kgn_device_t *dev)
{
	gni_return_t           rrc;
	__u64                  event_data;
	kgn_tx_ev_id_t         ev_id;
...
int
kgnilnd_check_fma_rcv_cq(struct kgn_sched_info *sched, kgn_device_t *dev)
{
	kgn_conn_t         *conn;
	gni_return_t        rrc;
	__u64               event_data;
...
int
kgnilnd_process_rdmaq(struct kgn_sched_info *sched, kgn_device_t *dev)
{
	int               found_work = 0;
	kgn_tx_t         *tx;
		...
		if (conn->gnc_state == GNILND_CONN_ESTABLISHED) {
			...
			if (conn->gnc_peer_error == -ETIMEDOUT) {
				...
				CNETERR("peer 0x%p->%s closed connection 0x%p due to timeout. "
				       ...
				       cfs_duration_sec(now - conn->sched->kgn_alive));
			}
...
int
kgnilnd_process_mapped_tx(struct kgn_sched_info *sched, kgn_device_t *dev)
{
	int		found_work = 0;
	int		rc = 0;
	kgn_tx_t	*tx;
...
int
kgnilnd_process_conns(struct kgn_sched_info *sched, kgn_device_t *dev, unsigned long deadline)
{
	int              found_work = 0;
	int              conn_sched;
	int              intent = 0;
...
kgnilnd_scheduler(void *arg)
{
	...
	snprintf(name, sizeof(name), "kgnilnd_sd_%02ld_%02ld_%02ld",
		 KGN_THREAD_DEV(id), KGN_THREAD_CPT(id),
		 KGN_THREAD_TID(id));
	cfs_daemonize(name);
	cfs_block_allsigs();
	...
	while (!kgnilnd_data.kgn_shutdown) {
		...
		(void)cmpxchg(&sched->kgn_ready, GNILND_DEV_IRQ, GNILND_DEV_LOOP);
		 ...
		 * need to take a break. We'll clear kgn_ready but we'll check
		 * one last time if there is an IRQ that needs processing */

		prepare_to_wait(&sched->kgn_waitq, &wait, TASK_INTERRUPTIBLE);
		...
		found_work += xchg(&sched->kgn_ready, GNILND_DEV_IDLE);
		...
		if ((busy_loops >= *kgnilnd_tunables.kgn_loops) ||
		   time_after_eq(jiffies, deadline)) {
			 ...
			 * not call wake_up() because kgn_ready == 1, but then
		...
		finish_wait(&sched->kgn_waitq, &wait);
	}
