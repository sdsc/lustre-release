	    ...
	    kgn_rdma_desc_t *sink, unsigned int nob, __u64 cookie)
{
	...
	GNIDBG_TX(D_NET, tx, "Post RDMA type 0x%02x conn %p dlvr_mode 0x%x cookie:"LPX64,
				    ...
				    struct list_head *souls)
{
	...
	/* If we have too many connections in purgatory we could run out of resources
	 * Limit the number of connections to a tunable number, Clean up to the minimum
	 * all in one fell swoop... there are situations where dvs will retry tx's and
	 * we can eat up several hundread connection requests at once.
	 */
	if (purgatory_conn_cnt > *kgnilnd_tunables.kgn_max_purgatory) {
		list_for_each_entry_safe (conn, connN, &peer->gnp_conns, gnc_list) {
			if (conn->gnc_in_purgatory && conn->gnc_state == GNILND_CONN_DONE) {
				CDEBUG(D_NET, "Dropping Held resource due to resource limits"
					      " being hit\n");
				kgnilnd_detach_purgatory_locked(conn, souls);
...
kgnilnd_send_mapped_tx(kgn_tx_t *tx, int try_map_if_full)
{
	 ...
	 * it's gnm_type will now be GNILND_MSG_PUT_DONE or GNILND_MSG_GET_DONE_REV.
	 */
	case GNILND_MSG_GET_REQ:
