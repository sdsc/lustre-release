...
ksocknal_find_conn_locked(ksock_peer_t *peer, ksock_tx_t *tx, int nonblk)
{
	...
	list_for_each(tmp, &peer->ksnp_conns) {
		...
		LASSERT(c->ksnc_proto && c->ksnc_proto->pro_match_tx);
...
ksocknal_find_connectable_route_locked(ksock_peer_t *peer)
{
	...
	list_for_each(tmp, &peer->ksnp_routes) {
		...
		if (!(!route->ksnr_retry_interval ||	/* first attempt */
			...
			CDEBUG(D_NET, "Too soon to retry route %pI4h (cnted %d, interval %ld, %ld secs later)\n",
			       &route->ksnr_ipaddr,
		      ...
		      unsigned long id)
{
	...
	task = kthread_run(fn, arg, "%s_%02lu", name, id);
	if (IS_ERR(task))
		return PTR_ERR(task);
...
ksocknal_connect(ksock_route_t *route)
{
	...
	for (;;) {
		 ...
		 */
		if (peer->ksnp_closing || route->ksnr_deleted || !wanted) {
			retry_later = 0;
			break;
		}
	...
	if (retry_later) {
		 ...
		 */
		if (rc == EALREADY || (!rc && peer->ksnp_accepting > 0)) {
			/*
 ...
 failed:
	...
	if (!list_empty(&peer->ksnp_tx_queue) && !peer->ksnp_accepting &&
	    !ksocknal_find_connecting_route_locked(peer)) {
		ksock_conn_t *conn;
...
ksocknal_connd_check_start(long sec, long *timeout)
{
	...
	rc = ksocknal_thread_start(ksocknal_connd, NULL, "socknal_cd%02d",
				   total);
