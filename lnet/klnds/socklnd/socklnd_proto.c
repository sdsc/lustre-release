...
ksocknal_recv_hello_v1(ksock_conn_t *conn, struct ksock_hello_msg *hello,int timeout)
{
	...
	rc = lnet_sock_read(sock, &hdr->src_nid,
			      ...
			      timeout);
	if (rc != 0) {
		CNETERR("Error %d reading rest of HELLO hdr from %pI4h\n",
			rc, &conn->ksnc_ipaddr);
	...
	if (hello->kshm_nips > LNET_MAX_INTERFACES) {
		CNETERR("Bad nips %d from ip %pI4h\n",
	...
	if (rc != 0) {
		CNETERR("Error %d reading IPs from ip %pI4h\n",
			rc, &conn->ksnc_ipaddr);
		LASSERT(rc < 0 && rc != -EALREADY);
		goto out;
	}
	...
	for (i = 0; i < (int) hello->kshm_nips; i++) {
		...
		if (hello->kshm_ips[i] == 0) {
			CNETERR("Zero IP[%d] from ip %pI4h\n",
				i, &conn->ksnc_ipaddr);
			rc = -EPROTO;
			break;
		}
