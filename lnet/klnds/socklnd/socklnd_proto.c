 ...
 *			    return the ACK that piggybacked by my message, or
 *			    NULL
			   ...
			   ksock_tx_t *tx_ack, __u64 cookie)
{
	...
	if (!tx->tx_msg.ksm_zc_cookies[0]) {
		/*
		 * NOOP tx has only one ZC-ACK cookie, can carry at
		 * least one more
		 */
...
ksocknal_send_hello_v1(ksock_conn_t *conn, ksock_hello_msg_t *hello)
{
	...
	if (rc) {
		CNETERR("Error %d sending HELLO payload (%d) to %pI4h/%d\n",
			rc, hello->kshm_nips, &conn->ksnc_ipaddr,
			conn->ksnc_port);
	}
out:
		       ...
		       int timeout)
{
	...
	if (hdr->type != cpu_to_le32(LNET_MSG_HELLO)) {
		CERROR("Expecting a HELLO hdr, but got type %d from %pI4h\n",
		       le32_to_cpu(hdr->type), &conn->ksnc_ipaddr);
		rc = -EPROTO;
		goto out;
	}
...
ksocknal_pack_msg_v2(ksock_tx_t *tx)
{
	...
	/*
	 * Don't checksum before start sending, because packet
	 * can be piggybacked with ACK
	 */
}
