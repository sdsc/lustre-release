...
{
	if (ksocknal_tunables.ksnd_sysctl)
...
{
	int rc = lnet_sock_getaddr(conn->ksnc_sock, true, &conn->ksnc_ipaddr,
				   &conn->ksnc_port);
	...
	rc = lnet_sock_getaddr(conn->ksnc_sock, false, &conn->ksnc_myipaddr,
			       NULL);
	if (rc) {
...
ksocknal_lib_zc_capable(ksock_conn_t *conn)
{
	 ...
	 * checksums
	 */
	return ((caps & NETIF_F_SG) && (caps & NETIF_F_ALL_CSUM));
}
...
#warning "XXX risk of kmap deadlock on multiple frags..."
	...
	if (conn->ksnc_msg.ksm_csum) {
		for (i = 0, sum = rc; sum > 0; i++, sum -= fragnob) {
			 ...
			 * stash the mapped address. But by doing it while
			 * the page is still mapped, the kernel just bumps the
			 * map count and returns me the address it stashed.
			 */
...
ksocknal_write_space(struct sock *sk)
{
	...
	CDEBUG(D_NET, "sk %p wspace %d low water %d conn %p%s%s%s\n",
	       ...
	       !conn ? "" : (conn->ksnc_tx_ready ? " ready" : " blocked"),
	       !conn ? "" : (conn->ksnc_tx_scheduled ? " scheduled" : " idle"),
