...
static struct proc_dir_entry *socklnd_proc_root;
...
ksocknal_proc_init(void)
{
	...
	socklnd_proc_root = proc_mkdir(SOCKLND_PROC_ROOT, init_net.proc_net);
	if (socklnd_proc_root == NULL) {
		CERROR("couldn't create proc dir %s\n", SOCKLND_PROC_ROOT);
		return -1;
	}

	entry = proc_create(SOCKLND_PROC_STATS, 0644, socklnd_proc_root,
			    &ksocknal_stats_fops);
	if (entry == NULL) {
...
ksocknal_proc_fini(void)
{
	...
	remove_proc_entry(SOCKLND_PROC_ROOT,  init_net.proc_net);
}
