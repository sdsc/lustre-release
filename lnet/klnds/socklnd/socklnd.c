...
ksocknal_base_startup (void)
{
	...
	cfs_percpt_for_each(sched, i, ksocknal_data.ksnd_schedulers) {
		...
		if (*ksocknal_tunables.ksnd_nscheds_cpt_max > 0) {
			nthrs = *ksocknal_tunables.ksnd_nscheds_cpt_max;
		} else {
...
ksocknal_start_schedulers(ksock_sched_t *sched)
{
	...
	if (sched->kss_nthreads == 0) {
		if (*ksocknal_tunables.ksnd_nscheds_cpt > 0) {
			nthrs = *ksocknal_tunables.ksnd_nscheds_cpt;
		} else {
			/* one thread for each connection type */
			nthrs = min(SOCKNAL_NSCHEDS, sched->kss_nthreads_max);
		}
	} else {
		LASSERT(sched->kss_nthreads <= sched->kss_nthreads_max);
		/* increase one thread if there are new interfaces
		 * in this network */
	...
	ni->ni_credits	      = *ksocknal_tunables.ksnd_credits;
        ...
        if (ni->ni_interfaces[0] == NULL) {
			...
			strncpy(net->ksnn_interfaces[i].ksni_name,
				ni->ni_interfaces[i], IFALIASZ - 1);
                }
                net->ksnn_ninterfaces = i;
        }

	/* call it before add it to ksocknal_data.ksnd_nets */
	if (ksocknal_search_new_ipif(net)) {
		rc = ksocknal_net_start_threads(net, ni->ni_cpts,
						ni->ni_ncpts);
		if (rc != 0)
			goto fail_1;
	}
