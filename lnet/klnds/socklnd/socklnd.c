...
ksocknal_associate_route_conn_locked(ksock_route_t *route, ksock_conn_t *conn)
{
	...
	/* Successful connection => further attempts can proceed immediately */
...
ksocknal_del_route_locked(ksock_route_t *route)
{
	...
	if (list_empty(&peer->ksnp_routes) && list_empty(&peer->ksnp_conns)) {
		/*
...
ksocknal_del_peer_locked(ksock_peer_t *peer, __u32 ip)
{
	...
	if (!nshared) {
		/*
		 * remove everything unless there are no explicit entries
		 * left
		 */
		list_for_each_safe(tmp, nxt, &peer->ksnp_routes) {
			route = list_entry(tmp, ksock_route_t, ksnr_list);
			...
			LASSERTF(!route->ksnr_share_count,
				 "route's share count is %d\n",
				 route->ksnr_share_count);
			ksocknal_del_route_locked(route);
		}
...
ksocknal_del_peer(lnet_ni_t *ni, lnet_process_id_t id, __u32 ip)
{
	...
	if (id.nid != LNET_NID_ANY) {
		lo = (int)(ksocknal_nid2peerlist(id.nid) -
...
ksocknal_match_peerip(ksock_interface_t *iface, __u32 *ips, int nips)
{
	...
	for (i = 0; i < nips; i++) {
		...
		this_netmatch = !(this_xor & iface->ksni_netmask);
		     ...
		     struct socket *sock, int type)
{
	...
	if (peer->ksnp_proto != conn->ksnc_proto ||
		...
		warn = peer->ksnp_incarnation != incarnation ?
		       "peer rebooted" : "wrong proto version";
		goto failed_2;
	}
...
ksocknal_base_startup(void)
{
	...
	for (i = 0; i < *ksocknal_tunables.ksnd_nconnds; i++) {
		...
		rc = ksocknal_thread_start(ksocknal_connd, (void *)i,
					   "socknal_cd", i);
		if (rc) {
			spin_lock_bh(&ksocknal_data.ksnd_connd_lock);
			ksocknal_data.ksnd_connd_starting--;
	...
	rc = ksocknal_thread_start(ksocknal_reaper, NULL, "socknal_reaper", 0);
	if (rc) {
...
ksocknal_debug_peerhash(lnet_ni_t *ni)
{
	...
	if (peer) {
		...
		CWARN("Active peer on shutdown: %s, ref %d, scnt %d, closing %d, accepting %d, err %d, zcookie "LPU64", txq %d, zc_req %d\n",
		      libcfs_id2str(peer->ksnp_id),
		      atomic_read(&peer->ksnp_refcount),
	...
	for (i = 0; i < net->ksnn_ninterfaces; i++) {
		...
		list_for_each_entry(tmp, &ksocknal_data.ksnd_nets, ksnn_list) {
			for (j = 0; !found && j < tmp->ksnn_ninterfaces; j++) {
				char *ifnam2 = &tmp->ksnn_interfaces[j].ksni_name[0];
...
ksocknal_start_schedulers(struct ksock_sched_info *info)
{
	...
	for (i = 0; i < nthrs; i++) {
		...
		index = (unsigned long)(sched - info->ksi_scheds);
		snprintf(name, sizeof(name), "socknal_sd%02d", info->ksi_cpt);
		rc = ksocknal_thread_start(ksocknal_scheduler, (void *)id,
					   name, index);
		if (!rc)
			continue;

		CERROR("Can't spawn thread %d for scheduler[%lu]: %d\n",
