...
ksocknal_get_peer_info(lnet_ni_t *ni, int index, lnet_process_id_t *id,
		       int *ref_count, int *active_conn, int *waiting_conn,
		       int *connecting, int *accepting, __u32 *myip,
		       __u32 *peer_ip, int *port, int *cpt, int *conn_count,
		       int *share_count)
{
	...
	for (i = 0; i < ksocknal_data.ksnd_peer_hash_size; i++) {
		list_for_each(ptmp, &ksocknal_data.ksnd_peers[i]) {
			...
			if (peer->ksnp_n_passive_ips == 0 &&
				...
				*id = peer->ksnp_id;
				*cpt = peer->ksnp_cpt;
				*ref_count = atomic_read(&peer->ksnp_refcount);
				*myip = 0;
				*peer_ip = 0;
				*port = 0;
				*conn_count = 0;
...
ksocknal_ctl(lnet_ni_t *ni, unsigned int cmd, void *arg)
{
	...
	case IOC_LIBCFS_GET_PEER: {
		...
		rc = ksocknal_get_peer_info(ni, index, &id, &ref_count,
					    &active_conn, &waiting_conn,
		...
		if (data_peer != NULL) {
			...
			data_peer->pr_lnd_u.pr_peer_details.cpt = cpt;
			data_peer->pr_lnd_u.pr_peer_details.peer_ref_count = ref_count;
			data_peer->pr_lnd_u.pr_peer_details.active_conns = active_conn;
			data_peer->pr_lnd_u.pr_peer_details.waiting_conns = waiting_conn;

			/* Driver specific data */
			data_peer->pr_lnd_u.pr_peer_details.pr_lnd.socklnd.shared_count = share_count;
			data_peer->pr_lnd_u.pr_peer_details.pr_lnd.socklnd.peer_ip = ip;
			data_peer->pr_lnd_u.pr_peer_details.pr_lnd.socklnd.peer_port = port;
			data_peer->pr_lnd_u.pr_peer_details.pr_lnd.socklnd.local_ip = myip;
			data_peer->pr_lnd_u.pr_peer_details.pr_lnd.socklnd.conn_count = conn_count;
			data_peer->pr_lnd_u.pr_peer_details.pr_lnd.socklnd.pid = id.pid;
		} else {
			data->ioc_nid	 = id.nid;
	...
	case IOC_LIBCFS_GET_CONN_INFO: {
		...
		if (data_conn != NULL) {
			...
			data_conn->conn_lnd_u.socklnd_conn.tx_buf_size = txmem;
			data_conn->conn_lnd_u.socklnd_conn.nagle = nagle;
			data_conn->conn_lnd_u.socklnd_conn.peer_ip = conn->ksnc_ipaddr;
			data_conn->conn_lnd_u.socklnd_conn.peer_port = conn->ksnc_port;
			data_conn->conn_lnd_u.socklnd_conn.local_ip = conn->ksnc_myipaddr;
			data_conn->conn_lnd_u.socklnd_conn.type = conn->ksnc_type;
			data_conn->conn_lnd_u.socklnd_conn.cpt = conn->ksnc_scheduler->kss_info->ksi_cpt;
			data_conn->conn_lnd_u.socklnd_conn.rx_buf_size = rxmem;
			data_conn->conn_lnd_u.socklnd_conn.pid = conn->ksnc_peer->ksnp_id.pid;
		} else {
