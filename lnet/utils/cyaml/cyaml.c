...
static int assign_type_value(struct cYAML *obj, const char *value)
{
	...
	if (strcmp(value, "null") == 0)
		obj->type = CYAML_TYPE_NULL;
	else if (strcmp(value, "false") == 0) {
		obj->type = CYAML_TYPE_FALSE;
		obj->valueint = 0;
	} else if (strcmp(value, "true") == 0) {
		obj->type = CYAML_TYPE_TRUE;
		obj->valueint = 1;
	} else if (*value == '-' || (*value >= '0' && *value <= '9')) {
		if (parse_number(obj, value) == 0) {
			obj->valuestring = strdup(value);
			obj->type = CYAML_TYPE_STRING;
		}
	} else {
		obj->valuestring = strdup(value);
		obj->type = CYAML_TYPE_STRING;
						   ...
						   struct cYAML_tree_node *tree)
{
	...
	tree->cur->type = CYAML_TYPE_ARRAY;
...
struct cYAML *cYAML_get_next_seq_item(struct cYAML *seq, struct cYAML **itm)
{
	...
	if ((*itm == NULL) && (seq->type == CYAML_TYPE_ARRAY)) {
		*itm = seq->child;
		return *itm;
	}
...
{
	return ((node != NULL) ? (node->type == CYAML_TYPE_ARRAY) : 0);
}
			       ...
			       struct cYAML **err_rc)
{
	...
	while (!done) {
		...
		*/
		...
		if (rc != CYAML_ERROR_NONE) {
			sprintf(err_str, "Failed to handle token:%d "
			       "[state=%d, rc=%d]",
