...
jt_lnet_route(int argc, char **argv)
{
	...
	/* There needs to be 4 arguments in all route commands. */
	if (argc < 4) {
		libcfs_err_noerrno(LIBCFS_MSG_ERROR,
			"Wrong number of arguments, got %d, expected >= 4\n",
			argc);
...
jt_lnet_route_buffers(int argc, char **argv)
{
	...
	if ((argc < 4) && (argc != 1)) {
		libcfs_err_noerrno(LIBCFS_MSG_ERROR,
		    "Wrong number of arguments, got %d, expected 3 or none\n",
		    argc);
		lnet_print_usage(argv[0]);
		return -1;
	}
