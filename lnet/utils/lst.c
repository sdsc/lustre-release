...
lst_print_stat(char *name, cfs_list_t *resultp,
	       int idx, int lnet, int bwrt, int rdwr, int type, int remote_ts)
{
        cfs_list_t        tmp[2];
	...
	int		      use_remote_ts = 0;
        ...
        do {
			...
			lst_print_stat(srp->srp_name, srp->srp_result,
				       idx, lnet, bwrt, rdwr, type, use_remote_ts);
...
static command_t lst_cmdlist[] = {
	...
	{"stat",                jt_lst_stat,            NULL,
	 ...
	 " [--remote_ts] [--timeout #] [--delay #] [--count #] GROUP [GROUP]"                         },
