...
command_t net_cmds[] = {
	{"add", jt_add_net, 0, "add a network\n"
	 ...
	 "\t--peer-buffer_credits: the number of buffer credits per peer\n"
...
static int jt_lnet_help(int argc, char **argv)
{
	print_help(lnet_cmds, "lnet", NULL);
}

static int jt_route_help(int argc, char **argv)
{
	print_help(route_cmds, "route", NULL);
}

static int jt_net_help(int argc, char **argv)
{
	print_help(net_cmds, "net", NULL);
}

static int jt_set_help(int argc, char **argv)
{
	print_help(set_cmds, "set", NULL);
}

static int jt_stats_help(int argc, char **argv)
{
	print_help(stats_cmds, "stats", NULL);
}

static int jt_peer_credits_help(int argc, char **argv)
{
	print_help(credits_cmds, "peer_credits", NULL);
}

static int jt_routing_help(int argc, char **argv)
{
	print_help(routing_cmds, "routing", NULL);
}
...
static int jt_set_routing(int argc, char **argv)
{
	...
	/* check if this is a request for help */
	if (argc > 1 && strcmp(argv[1], "help") == 0) {
		print_help(set_cmds, "set", "routing");
		return 0;
	}
...
static int jt_config_lnet(int argc, char **argv)
{
	...
	const char *const short_options = "a:h";
	const struct option long_options[] = {
		{ "all", 0, NULL, 'a' },
		{ "help", 0, NULL, 'h' },
		{ NULL, 0, NULL, 0 },
...
static int jt_unconfig_lnet(int argc, char **argv)
{
	...
	/* check if this is a request for help */
	if (argc > 1 && strcmp(argv[1], "help") == 0) {
		print_help(lnet_cmds, "lnet", "unconfigure");
		return 0;
	}
...
static int jt_add_net(int argc, char **argv)
{
	char *network = NULL, *intf = NULL, *cpt = NULL;
	long int pto = -1, pc = -1, pbc = -1, cre = -1;
	struct cYAML *err_rc = NULL;
	int rc, opt;
	optind = 0;

	const char *const short_options = "n:i:t:c:b:r:s:h";
	const struct option long_options[] = {
		{ "net", 1, NULL, 'n' },
		{ "if", 1, NULL, 'i' },
		{ "peer-timeout", 1, NULL, 't' },
		{ "peer-credits", 1, NULL, 'c' },
		{ "peer-buffer_credits", 1, NULL, 'b' },
		{ "credits", 1, NULL, 'r' },
		{ "cpt", 1, NULL, 's' },
		{ "help", 0, NULL, 'h' },
		{ NULL, 0, NULL, 0 },
	...
	rc = lustre_lnet_config_net(network, intf, pto, pc, pbc, cre, cpt, -1,
				    &err_rc);
...
static int jt_show_route(int argc, char **argv)
{
	...
	const char *const short_options = "n:g:h:p:dh";
	const struct option long_options[] = {
		{ "net", 1, NULL, 'n' },
		{ "gateway", 1, NULL, 'g' },
		{ "hop-count", 1, NULL, 'c' },
...
static int jt_show_net(int argc, char **argv)
{
	...
	const char *const short_options = "n:dh";
	const struct option long_options[] = {
		{ "net", 1, NULL, 'n' },
		{ "detail", 0, NULL, 'd' },
		{ "help", 0, NULL, 'h' },
	...
	while ((opt = getopt_long(argc, argv, short_options,
		...
		case 'd':
			...
			break;
		case 'h':
			print_help(route_cmds, "net", "show");
			return 0;
		default:
			return 0;
		}
