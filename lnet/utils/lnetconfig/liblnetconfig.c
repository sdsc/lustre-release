...
static lnet_nid_t *allocate_create_nid_array(char **nids, char *err_str)
{
	...
	if (nids == NULL) {
		snprintf(err_str, sizeof(err_str), "no NIDs to add");
		return NULL;
	}
				 ...
				 struct list_head *ip_ranges)
{
	 ...
	 * might not be specified. if specified interface needs to match the
	 * ip-range. If no ip-range then the interfaces are
	 * automaticlly matched if they are all up.
	...
	list_for_each_entry(intf_descr, intf_list,
		...
		if (cpt_expr != NULL) {
			...
			if (count > 0) {
				...
				LIBCFS_FREE(cpt_array,
					   sizeof(cpt_array[0] * count));
			} else {
				count = 0;
			}
		} else {
...
out:
	list_for_each_entry_safe(intf_descr, tmp, &nw_descr->nw_intflist,
		...
		LIBCFS_FREE(intf_descr, sizeof(*intf_descr));
	}
	...
	for (i = 0;; i++) {
		...
		if (detail) {
			...
			if (cYAML_create_number(tunables, "peer_timeout",
						lnd->lt_cmn.lct_peer_timeout)
							== NULL)
				goto out;

			if (cYAML_create_number(tunables, "peer_credits",
						lnd->lt_cmn.lct_peer_tx_credits)
							== NULL)
				goto out;

			if (cYAML_create_number(tunables,
						"peer_buffer_credits",
						lnd->lt_cmn.lct_peer_rtr_credits)
							== NULL)
				goto out;

			if (cYAML_create_number(tunables, "credits",
						lnd->lt_cmn.lct_max_tx_credits)
							== NULL)
				goto out;

			if (cYAML_create_number(tunables, "tcp bonding",
						ni_data->lic_tcp_bonding)
							== NULL)
				goto out;

			/* TODO: FUTURE: print LND specific tunables */
		      ...
		      struct cfs_expr_list **global_cpts)
{
	...
	if (tun != NULL) {
		...
		if (item != NULL)
			tunables->lt_cmn.lct_peer_timeout = item->cy_valueint;
		item = cYAML_get_object_item(tun, "peer_credits");
		if (item != NULL)
			tunables->lt_cmn.lct_peer_tx_credits = item->cy_valueint;
		item = cYAML_get_object_item(tun, "peer_buffer_credits");
		if (item != NULL)
			tunables->lt_cmn.lct_peer_rtr_credits = item->cy_valueint;
		item = cYAML_get_object_item(tun, "credits");
		if (item != NULL)
			tunables->lt_cmn.lct_max_tx_credits = item->cy_valueint;
				...
				struct cYAML **err_rc)
{
	...
	rc = lustre_lnet_del_peer_nid(nids[0], cfg_nid,
				      (seq_no) ? seq_no->cy_valueint : -1,
				      err_rc);
