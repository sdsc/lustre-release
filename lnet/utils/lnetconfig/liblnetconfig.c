				   ...
				   struct lnet_ioctl_peer_info *pinfo)
{
	...
	temp = pinfo->pr_lnd.gnilnd.dev_id;
	if (cYAML_create_number(item, "gni_device_id", temp) == NULL)
		return rc;

	temp = pinfo->pr_lnd.gnilnd.peer_status >> 16;
	if (temp != 0)
		state = "Down";
	else
			   ...
			   struct cYAML **show_rc, struct cYAML **err_rc)
{
	...
	for (i = 0;; i++) {
		...
		pinfo = data.pr_lnd_u.pr_peer_details;
