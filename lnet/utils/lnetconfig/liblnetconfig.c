			     ...
			     int seq_no, struct cYAML **err_rc)
{
	...
	char err_str[LIBCFS_MAX_STR_LEN];
	...
	data.ioc_config_u.route.priority = prio;
			  ...
			  int seq_no, struct cYAML **err_rc)
{
	...
	snprintf(err_str, LIBCFS_MAX_STR_LEN, "Success");
			   ...
			   struct cYAML **err_rc)
{
	...
	snprintf(err_str, LIBCFS_MAX_STR_LEN,
		 "out of Memory");
	...
	if (errno != ENOENT) {
		snprintf(err_str,
			 LIBCFS_MAX_STR_LEN,
			 "cannot get routes - %s. check dmesg.",
			 strerror(errno));
		rc = -errno;
		goto out;
	} else
	...
	if ((show_rc == NULL) || (rc != LUSTRE_CFG_RC_NO_ERR)) {
		...
		if ((*show_rc) != NULL) {
			...
			if (show_node != NULL && cYAML_is_sequence(show_node)) {
				...
				cYAML_insert_sibling((*show_rc)->child, route);
				free(root);
			} else
				cYAML_free_tree(root);
		} else
			   ...
			   int seq_no, struct cYAML **err_rc)
{
	...
	if ((intf == NULL) || (net == NULL)) {
		snprintf(err_str,
			 LIBCFS_MAX_STR_LEN,
			 "mandatory parameter not specified - %s",
			 (intf == NULL && net == NULL) ? "net, intf" :
			 (intf == NULL) ? "intf" : "net");
		rc = LUSTRE_CFG_RC_MISSING_PARAM;
		goto out;
	}
	...
	snprintf(buf, LIBCFS_MAX_STR_LEN - 1, "%s(%s)%s",
		 net, intf,
		 (smp) ? smp : "");

	buf[LIBCFS_MAX_STR_LEN - 1] = '\0';

	LIBCFS_IOC_INIT_V2(data);
	strncpy(data.ioc_config_u.net.intf, buf, LIBCFS_MAX_STR_LEN);
	data.ioc_config_u.net.peer_timeout = peer_to;
	data.ioc_config_u.net.peer_tx_credits = peer_cr;
	data.ioc_config_u.net.peer_rtr_credits = peer_buf_cr;
	data.ioc_config_u.net.max_tx_credits = credits;
			 ...
			 struct cYAML **show_rc, struct cYAML **err_rc)
{
	...
	int str_buf_len = LIBCFS_MAX_SHOW_NUM_CPT * 2;
	char str_buf[str_buf_len];
	char *pos = str_buf;
	char err_str[LIBCFS_MAX_STR_LEN];

	snprintf(err_str, LIBCFS_MAX_STR_LEN, "out of memory");
	...
	if (errno != ENOENT) {
		snprintf(err_str,
			 LIBCFS_MAX_STR_LEN,
			 "cannot get networks - %s. check dmesg.",
			 strerror(errno));
		rc = -errno;
		goto out;
	} else
	...
	if ((show_rc == NULL) || (rc != LUSTRE_CFG_RC_NO_ERR))
		...
		if ((*show_rc) != NULL) {
			...
			if (show_node != NULL && cYAML_is_sequence(show_node)) {
				...
				cYAML_insert_sibling((*show_rc)->child,
						     net_node);
				free(root);
			} else
				cYAML_free_tree(root);
...
{
	struct libcfs_ioctl_config_data *data;
	struct libcfs_ioctl_pool_cfg *pool_cfg = NULL;
	int rc = LUSTRE_CFG_RC_OUT_OF_MEM;
	char *buf;
	...
	int i;
	char err_str[LIBCFS_MAX_STR_LEN];
	char node_name[LIBCFS_MAX_STR_LEN];
	...
	for (i = 0;; i++) {
		...
		snprintf(node_name, LIBCFS_MAX_STR_LEN, "cpt[%d]", i);
	...
	if (pool_cfg != NULL) {
		...
		if (cYAML_create_number(item, "enable", pool_cfg->routing)
		    == NULL)
			goto out;
	}
	...
	if (errno != ENOENT) {
		snprintf(err_str,
			 LIBCFS_MAX_STR_LEN,
			 "cannot get routing information - %s. check dmesg.",
			 strerror(errno));
		rc = -errno;
		goto out;
	} else
				  ...
				  struct cYAML **err_rc)
{
	...
	struct cYAML *root = NULL, *peer = NULL, *first_peer = NULL;
	char err_str[LIBCFS_MAX_STR_LEN];
	bool ncpt_set = false;
			   ...
			   struct cYAML **err_rc)
{
	...
	if (rc != 0) {
		snprintf(err_str,
			 LIBCFS_MAX_STR_LEN,
			 "cannot get lnet statistics - %s. check dmesg.",
			 strerror(errno));
		rc = -errno;
		goto out;
	}
				  ...
				  struct cYAML **err_rc)
{
	struct cYAML *net, *intf, *tunables, *seq_no,
	      ...
	      *credits = NULL, *smp = NULL, *child;
	char devs[LIBCFS_MAX_STR_LEN];
	char *loc = devs;
	int size = LIBCFS_MAX_STR_LEN;
	int num;
				      ...
				      struct cYAML **err_rc)
{
	...
	rc = lustre_lnet_config_buffers((tiny) ? tiny->valueint : -1,
				    ...
				    struct cYAML **err_rc)
{
	...
	return lustre_lnet_show_routing((seq_no) ? seq_no->valueint : -1,
					show_rc, err_rc);
}
				 ...
				 struct cYAML **show_rc, struct cYAML **err_rc)
{
	...
	child = tree->child;
	while (child != NULL) {
		cb = lookup_fn(child->string, table);
		if (cb == NULL) {
			sprintf(err_str,
