...
fn_exit:
	if ((show_rc == NULL) || (rc != LUSTRE_CFG_RC_NO_ERR)) {
		...
		if ((*show_rc) != NULL) {
			 ...
			 */
			route = cYAML_get_object_item(*show_rc, "route");
			if ((route != NULL) && (cYAML_is_seq(route))) {
				cYAML_insert_child(route, root->child->child);
				free(root->child);
				free(root);
			} else if (route == NULL) {
...
fn_exit:
	if ((show_rc == NULL) || (rc != LUSTRE_CFG_RC_NO_ERR))
		...
		if ((*show_rc) != NULL) {
			 ...
			 * insert one.  Otherwise add to the one there
			 */
			net_node = cYAML_get_object_item(*show_rc, "net");
			if ((net_node != NULL) && (cYAML_is_seq(net_node))) {
				cYAML_insert_child(net_node, root->child->child);
				free(root->child);
				free(root);
			} else if (net_node == NULL) {
				cYAML_insert_sibling((*show_rc)->child, root->child);
				free(root);
			} else
				cYAML_free_tree(root);
			     ...
			     struct cYAML **err_rc)
{
	...
	if (cYAML_create_number(item, "enable", pool_cfg->routing) == NULL)
		goto fn_exit;
...
fn_exit:
	...
	if ((show_rc == NULL) || (rc != LUSTRE_CFG_RC_NO_ERR))
		...
		if ((*show_rc) != NULL) {
			 ...
			 * insert one.  Otherwise add to the one there
			 */
			pools_node = cYAML_get_object_item(*show_rc, "routing");
			if ((pools_node != NULL) && (cYAML_is_seq(pools_node))) {
				cYAML_insert_child(pools_node, root->child->child);
				free(root->child);
				free(root);
			} else if (pools_node == NULL) {
				cYAML_insert_sibling((*show_rc)->child, root->child);
				free(root);
			} else
				cYAML_free_tree(root);
				 ...
				 struct cYAML **show_rc, struct cYAML **err_rc)
{
	...
	while (child != NULL) {
		...
		if (cYAML_is_seq(child)) {
			while ((head = cYAML_get_next_seq_item(child, &item)) != NULL) {
