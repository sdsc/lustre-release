			   ...
			   struct cYAML **err_rc)
{
	...
	for (i = 0;; i++) {
		...
		if (cYAML_create_string(item, "net",
					libcfs_net2str(data.ioc_net)) == NULL)
			goto out;
			   ...
			   int seq_no, struct cYAML **err_rc)
{
	...
	snprintf(buf, sizeof(err_str) - 1, "%s(%s)%s",
		 net, intf,
		 (smp) ? smp : "");

	LIBCFS_IOC_INIT_V2(data);
	strncpy(data.ioc_config_u.net.intf, buf, sizeof(err_str));
			     ...
			     struct cYAML **err_rc)
{
	...
	for (i = 0;; i++) {
		...
		snprintf(node_name, sizeof(err_str), "cpt[%d]", i);
		...
		for (i = 0; i < LNET_NRBPOOLS; i++) {
			...
			if (cYAML_create_number(type_node, "mincredits",
					pool_cfg->pools[i].mincredits) == NULL)
				goto out;
		}
	}
