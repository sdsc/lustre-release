...
jt_ptl_drop_add(int argc, char **argv)
{
	...
	while (1) {
		...
		case 'd': /* dest NID/NET */
			...
			break;
		case 'r': /* drop rate */
			attr.dra_rate = strtol(optarg, NULL, 0);
			break;
		case 'l': /* latency before activating rule */
			attr.dra_delay = strtol(optarg, NULL, 0);
			break;
		case 'p': /* portal to filter */
			attr.dra_portal = strtol(optarg, NULL, 0);
			break;
		case 'm': { /* message types to filter */
			char  *tmp;
			__u32  mask;

			for (tmp = optarg, mask = 0; *tmp != '\0'; tmp++) {
				switch (*tmp) {
				case 'p':
				case 'P': /* LNET_MSG_PUT */
					mask |= 1 << LNET_DROP_PUT_BIT;
					break;
				case 'a':
				case 'A': /* LNET_MSG_GET */
					mask |= 1 << LNET_DROP_ACK_BIT;
					break;
				case 'g':
				case 'G': /* LNET_MSG_GET */
					mask |= 1 << LNET_DROP_GET_BIT;
					break;
				case 'r':
				case 'R': /* LNET_MSG_REPLY */
					mask |= 1 << LNET_DROP_REPLY_BIT;
					break;
				}
			}
			attr.dra_msg_mask |= mask;
			break; }
		default:
	...
	if (attr.dra_msg_mask == 0) /* all message types by default */
...
jt_ptl_drop_del(int argc, char **argv)
{
	...
	if (all) {
		...
		if (attr.dra_src == 0 || attr.dra_dst == 0) {
			fprintf(stderr, "Please provide both source and "
					"destination of drop rule\n");
			return -1;
		}
		data.ioc_inllen1 = sizeof(attr);
		data.ioc_inlbuf1 = (char *)&attr;
...
jt_ptl_drop_list(int argc, char **argv)
{
	...
	for (pos = 0;; pos++) {
		...
		libcfs_ioctl_unpack(&data, ioc_buf);
		printf("%s->%s (1/%d) ptl %d, msg %x, "
		       "PUT "LPU64", ACK "LPU64", GET "LPU64", REP "LPU64"\n",
		       libcfs_nid2str(attr.dra_src),
		       libcfs_nid2str(attr.dra_dst),
		       attr.dra_rate, attr.dra_portal, attr.dra_msg_mask,
