...
void LNetSnprintHandle(char *str, int str_len, struct lnet_handle_any handle);
...
int LNetMEAttach(unsigned int portal,
		 ...
		 struct lnet_handle_any *handle_out);

int LNetMEInsert(struct lnet_handle_any current_in,
		 struct lnet_process_id match_id_in,
		 __u64 match_bits_in,
		 __u64 ignore_bits_in,
		 enum lnet_unlink unlink_in,
...
int LNetMEUnlink(struct lnet_handle_any current_in);
/** @} lnet_me */
 ...
 * @{ */
int LNetMDAttach(struct lnet_handle_any current_in,
...
int LNetMDUnlink(struct lnet_handle_any md_in);
/** @} lnet_md */
 ...
 *
 * In addition to the struct lnet_handle_any, the LNet API defines two types
...
int LNetEQFree(struct lnet_handle_any eventq_in);

int LNetEQGet(struct lnet_handle_any eventq_in,
	      struct lnet_event *event_out);


int LNetEQWait(struct lnet_handle_any eventq_in,
	       struct lnet_event *event_out);

int LNetEQPoll(struct lnet_handle_any *eventqs_in,
	       int neq_in,
	       int timeout_ms,
	       struct lnet_event *event_out,
	       int *which_eq_out);
