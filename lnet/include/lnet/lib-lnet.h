...
static inline void
lnet_eq2handle(struct lnet_handle_any *handle_eq, struct lnet_eq *eq)
{
        if (eq == NULL) {
		LNetInvalidateHandle(handle_eq);
                return;
        }

	handle_eq->cookie = eq->eq_lh.lh_cookie;
}

static inline struct lnet_eq *
lnet_handle2eq(struct lnet_handle_any *handle_eq)
{
	/* ALWAYS called with resource lock held */
	struct lnet_libhandle *lh;

	lh = lnet_res_lh_lookup(&the_lnet.ln_eq_container, handle_eq->cookie);
	if (lh == NULL)
		return NULL;
...
static inline void
lnet_md2handle(struct lnet_handle_any *handle, struct lnet_libmd *md)
{
        handle->cookie = md->md_lh.lh_cookie;
}

static inline struct lnet_libmd *
lnet_handle2md(struct lnet_handle_any *handle)
{
	/* ALWAYS called with resource lock held */
	struct lnet_libhandle *lh;
	int cpt;
...
static inline void
lnet_me2handle(struct lnet_handle_any *handle_me, struct lnet_me *me)
{
	handle_me->cookie = me->me_lh.lh_cookie;
}

static inline struct lnet_me *
lnet_handle2me(struct lnet_handle_any *handle_me)
{
	/* ALWAYS called with resource lock held */
	struct lnet_libhandle *lh;
	int cpt;

	cpt = lnet_cpt_of_cookie(handle_me->cookie);
...
int lnet_notify(struct lnet_ni *ni, lnet_nid_t peer, int alive, cfs_time_t when);
void lnet_notify_locked(struct lnet_peer *lp, int notifylnd, int alive, cfs_time_t when);
...
void lnet_recv(struct lnet_ni *ni, void *private, struct lnet_msg *msg, int delayed,
...
struct lnet_msg *lnet_create_reply_msg(struct lnet_ni *ni, struct lnet_msg *get_msg);
void lnet_set_reply_msg_len(struct lnet_ni *ni, struct lnet_msg *msg, unsigned int len);
...
unsigned int lnet_kiov_nob (unsigned int niov, struct lnet_kiov *iov);
int lnet_extract_kiov(int dst_niov, struct lnet_kiov *dst,
		     int src_niov, struct lnet_kiov *src,
		     unsigned int offset, unsigned int len);
...
void lnet_copy_kiov2iov(unsigned int niov, struct kvec *iov,
			...
			unsigned int nkiov, struct lnet_kiov *kiov,
			unsigned int kiovoffset, unsigned int nob);
void lnet_copy_iov2kiov(unsigned int nkiov, struct lnet_kiov *kiov,
			unsigned int kiovoffset,
			unsigned int niov, struct kvec *iov,
			unsigned int iovoffset, unsigned int nob);
void lnet_copy_kiov2kiov(unsigned int ndkiov, struct lnet_kiov *dkiov,
			 unsigned int doffset,
			 unsigned int nskiov, struct lnet_kiov *skiov,
			 unsigned int soffset, unsigned int nob);
...
lnet_copy_kiov2flat(int dlen, void *dest, unsigned int doffset,
		    unsigned int nsiov, struct lnet_kiov *skiov,
		    unsigned int soffset, unsigned int nob)
{
	struct kvec diov = {/* .iov_base = */ dest, /* .iov_len = */ dlen};
...
static inline void
lnet_copy_flat2kiov(unsigned int ndiov, struct lnet_kiov *dkiov,
