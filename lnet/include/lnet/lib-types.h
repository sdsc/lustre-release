...
typedef struct lnet_msg {
	...
	/* NID of local NI, it's only for building event */
	lnet_nid_t		msg_sender;
	lnet_process_id_t	msg_target;
	__u32			msg_type;
	/* CPT for sending this message */
	unsigned int		msg_tx_cpt:15;
	/* commited to sending */
	unsigned int		msg_tx_committed:1;
	/* CPT for receiving this message */
...
/* credits and TX queue for each CPT of NI */
...
typedef struct lnet_ni {
	...
	/* protect ni_status and ni_npeers */
	cfs_spinlock_t		ni_lock;
	__u32			*ni_cpts;	/* bond NI on some CPTs */
	int			ni_ncpts;	/* number of CPTs */
	int			ni_redir_rotor;	/* redirect PUT rotor */
	/* reserved. how should we redirect PUT message on this NI */
        int			ni_redir_policy;
	/* max credit for each CPT */
	int			ni_maxtxcredits_cpt;
	/* # tx credits  */
	int			ni_maxtxcredits;
	/* lowest it's been */
	int			ni_mintxcredits;
	...
	int			ni_peertimeout;
	int			ni_npeers;	/* # peers on this NI */
	struct lnet_tx_queue	**ni_tx_queues;	/* TX queues */
	int			**ni_refs;	/* percpu partition refs */
...
enum {
	LNET_PROTO_PING_UNKNOWN		= 0,
	LNET_PROTO_PING_VERSION_1	= 1,
	LNET_PROTO_PING_VERSION		= 2,
};
...
typedef struct {
	/* chain on the_lnet.ln_zombie_rcd */
	cfs_list_t		rcd_list;
	lnet_handle_md_t	rcd_mdh;	/* ping buffer MD */
	unsigned int		rcd_gw_cpt;	/* CPT of gateway */
...
typedef struct lnet_peer {
	...
	/* returned ping version */
	unsigned int		lp_ping_version;
	int			lp_cpt;		/* CPT attached on */
	/* # times router went dead<->alive */
	int			lp_alive_count;
	long			lp_txqnob;	/* bytes queued for sending */
	cfs_time_t		lp_timestamp;	/* time of last ping attempt */
	...
	int			lp_rtr_refcount;
	cfs_list_t		lp_routes;	/* routes on this peer */
	lnet_rc_data_t		*lp_rcd;	/* router checker state */
} lnet_peer_t;

/* peer hash size, prime! */
...
typedef struct {
	...
	cfs_list_t		lr_gwlist;	/* chain on the gateway */
	__u32			lr_net;		/* remote net number */
        int			lr_seq;		/* sequence number for RR */
...
/* match-table hash bits of unique portal */
#define LNET_MT_HASH_BITS	8
/* match-table hash size of unique portal */
#define LNET_MT_HASH_SIZE	(1 << LNET_MT_HASH_BITS)
/* deadline (seconds) to disable matchtable, it's only for wildcard portal */
#define LNET_MT_DEADLINE	30

/* portal match table */
struct lnet_match_table {
	unsigned int		mt_cpt;		/* cpu-partition ID */
	unsigned int		mt_portal;	/* portal index */
	/*
	 * deadline to deativate the portal partition.
	 * Portal partition is marked as inactive if nobody posted any buffer
	 * on this CPU after pt_deadline.
	 */
	unsigned long		mt_deadline;
...
struct lnet_portal {
	...
	/* cpu-partition data for portals */
...
/* use 16 bits of PUT hdr_data as lnet PUT intent */
#define LNET_PUT_IT_BITS	16
#define LNET_PUT_IT_MASK	((1ULL << LNET_PUT_IT_BITS) - 1)

#define LNET_PUT_ITF_BITS	32
#define LNET_PUT_ITF_MASK	((1ULL << LNET_PUT_ITF_BITS) - 1)

enum {
	/* flag for local redirecting PUT */
	LNET_PUT_ITF_REDIR	= (1ULL << LNET_PUT_IT_BITS),
	/* flag for remote PUT intent */
	LNET_PUT_ITF_REMOTE	= (2ULL << LNET_PUT_IT_BITS),
};

enum {
	/* redirect PUT is off */
	LNET_REDIR_OFF		= 0,
	/* redirect all PUT messages */
	LNET_REDIR_ALWAYS	= 1,
	/*
	 * default policy for PUT redirecting, see more details
	 * in lnet_redir_put_intent()
	 */
	LNET_REDIR_AUTO		= 2,
	/* redirect all routed messages */
	LNET_REDIR_ROUTED	= 3,
};

#define LNET_LH_HASH_BITS	13
...
struct lnet_res_container {
	...
	cfs_list_t		rec_active;	/* inactive resource list */
#ifdef LNET_USE_LIB_FREELIST
	lnet_freelist_t		rec_freelist;	/* freelist for messages */
#endif
};

/* cpu-partition data of message */
...
# endif
#endif
	/* network & peer & credits lock */
	struct cfs_percpt_lock		*ln_net_lock;
	/* percpt peers table */
	struct lnet_peer_table		**ln_peer_tables;
	/* percpt message container */
