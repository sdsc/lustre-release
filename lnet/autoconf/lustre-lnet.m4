...
AC_ARG_WITH([o2ib],
	...
	[
		case $with_o2ib in
		yes)    O2IBPATHS=$(ofed_info | grep compat-rdma-devel | xargs rpm -ql | egrep -w 'compat-rdma$|openib$' | head -1)
		...
		*)      O2IBPATHS=$with_o2ib
		...
		O2IBPATHS=$(ofed_info | grep compat-rdma-devel | xargs rpm -ql | egrep -w 'compat-rdma$|openib$' | head -1)
		O2IBPATHS="$O2IBPATHS $LINUX $LINUX/drivers/infiniband"
		ENABLEO2IB=1
	])
