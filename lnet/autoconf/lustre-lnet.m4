...
AC_ARG_ENABLE([sriov-ofed],
	AC_HELP_STRING([--enable-sriov-ofed],
       ...
       [enable_srio_vofed='yes'],[enable_srio_vofed='no'])

SRIOV_ENABLED_OFED=0
if test "x$enable_srio_vofed" = 'xno'; then
	SRIOV_ENABLED_OFED=0
	AC_MSG_RESULT([no])
elif test "x$enable_srio_vofed" = 'xyes'; then
	SRIOV_ENABLED_OFED=1
	AC_DEFINE(HAVE_COMPAT_32_HEADERS, 1, [use compat-3.2 header files])
	AC_MSG_RESULT([yes])
else
	AC_MSG_RESULT([no])
fi
