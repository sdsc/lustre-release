		 ...
		 unsigned long long *sum, unsigned long long *sum_square)
{
	...
	if (cfs_get_param_path(&path, "lustre/mdt/MDS/mds/stats") != 0)
		return ERROR;
	...
	if (cfs_get_param_path(&path, "lustre/mdt/MDS/mds_readpage/stats") != 0)
		return ERROR;
	if (stats_values(path.gl_pathv[0], name_value, &tmp_nb_sample,
			 &tmp_min, &tmp_max,
			 &tmp_sum, &tmp_sum_square) == ERROR) {
	...
	if (cfs_get_param_path(&path, "lustre/mdt/MDS/mds_setattr/stats") != 0)
		return ERROR;
	if (stats_values(path.gl_pathv[0], name_value, &tmp_nb_sample,
			 &tmp_min, &tmp_max,
			 &tmp_sum, &tmp_sum_square) == ERROR) {
