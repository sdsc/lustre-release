...
static inline struct timespec current_time_ns(void)
{
	struct timespec now;
	clock_gettime(CLOCK_REALTIME, &now);
	return now;
}
#define CURRENT_TIME_NS current_time_ns()
...
out:
	...
	icino->ici_st.st_mtim = CURRENT_TIME_NS;
	return 0;
}
			   ...
			   struct intnl_stat *stbuf)
{
	...
	icino->ici_st.st_ctim = CURRENT_TIME_NS;
				   ...
				   size_t nbytes)
{
	...
	de =
	    incore_directory_probe(de,
				   ...
				   NULL,
				   &copy_info);
	icino->ici_st.st_atim = CURRENT_TIME_NS;
			...
			unsigned char type)
{
	...
	parent->ici_st.st_mtim = CURRENT_TIME_NS;
