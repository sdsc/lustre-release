...
AC_DEFUN([LB_CONFIG_RPMBUILD_OPTIONS],[
	...
	arg=''
	args=()
	for (( i=0 ; i<${#ac_configure_args} ; i++ )); do
		char=${ac_configure_args:i:1}
		if [[[ $quote ]]]; then
			if [[[ $char == \\ ]]]; then
				if [[[ $quote=='"' && ${ac_configure_args:i+1:1} == '"' ]]]; then
					let i++
					char=$quote
				fi
	...
	if test -n "$CROSS_PATH" ; then
		...
		if test x$CC_TARGET_ARCH = x"k1om-mpss-linux" ; then
			RPMBINARGS="$RPMBINARGS --define \"cross_requires mpss-bootimage\""
		else
	...
	if test x$enable_ldiskfs = xyes ; then
		RPMBINARGS="$RPMBINARGS --with ldiskfs"
	fi
