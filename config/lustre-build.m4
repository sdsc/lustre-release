...
AS_IF([test x$enable_server != xno],[
	# OK, admittedly this looks pretty strange.  This is a workaround
	# to handle the fact that some LB_CONFIG_MODULES tests need to run
	# after we have decided whether servers are going to be enabled,
	# but some tests need to be run before LB_CONFIG_LDISKFS and
	# LB_CONFIG_ZFS which are now _part_ of deciding whether we are going
	# to actually enable server support.  Eventually we need to unravel
	# the spaghetti here and split the tests under LB_CONFIG_MODULES
	# into at least two different groups.
	#
	# In other words, LB_CONFIG_MODULES currently needs to run before
	# LB_CONFIG_LDISKFS and LB_CONFIG_ZFS, but the tests under
	# LB_CONFIG_MODULES do not understand "enable_server=maybe".
	enable_server_backup=$enable_server
	enable_server=yes
	LB_CONFIG_MODULES
	enable_server=$enable_server_backup
])
LB_CONFIG_LDISKFS
LB_CONFIG_ZFS
