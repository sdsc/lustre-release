...
pgoff_t lov_stripe_pgoff(struct lov_stripe_md *lsm, pgoff_t stripe_index, int stripe)
{
	obd_off obd_size;

	obd_size = lov_stripe_size(lsm, stripe_index << PAGE_CACHE_SHIFT, stripe);
	return obd_size >> PAGE_CACHE_SHIFT;
}
