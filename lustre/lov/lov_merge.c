                      ...
                      struct ost_lvb *lvb, __u64 *kms_place)
{
	...
	CDEBUG(D_INODE, "MDT FID "DFID" initial value: s="LPU64
		...
		if (ll_nanotime_after(loi->loi_lvb.lvb_mtime,
				      loi->loi_lvb.lvb_mtime_ns,
				      current_mtime, current_mtime_ns)) {
			current_mtime = loi->loi_lvb.lvb_mtime;
			current_mtime_ns = loi->loi_lvb.lvb_mtime_ns;
		}
		if (ll_nanotime_after(loi->loi_lvb.lvb_atime,
				      loi->loi_lvb.lvb_atime_ns,
				      current_atime, current_atime_ns)) {
			current_atime = loi->loi_lvb.lvb_atime;
			current_atime_ns = loi->loi_lvb.lvb_atime_ns;
		}
		if (ll_nanotime_after(loi->loi_lvb.lvb_ctime,
				      loi->loi_lvb.lvb_ctime_ns,
				      current_ctime, current_ctime_ns)) {
                     ...
                     struct lov_stripe_md *lsm, int stripeno, int *set)
{
		...
		if (valid & OBD_MD_FLCTIME &&
		    ll_nanotime_after(src->o_ctime, src->o_ctime_ns,
				      tgt->o_ctime, tgt->o_ctime_ns)) {
			tgt->o_ctime = src->o_ctime;
			tgt->o_ctime_ns = src->o_ctime_ns;
		}
		if (valid & OBD_MD_FLMTIME &&
		    ll_nanotime_after(src->o_mtime, src->o_mtime_ns,
				      tgt->o_mtime, tgt->o_mtime_ns)) {
