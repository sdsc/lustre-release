                      ...
                      struct cl_dio_data *dio_data)
{
        ...
        LASSERTF(dio_data->cdd_size > 0, "cdd_size=%lu\n",
                 (unsigned long)dio_data->cdd_size);
        CDEBUG(D_VFSTRACE, "lov_dio: offset="LPU64" size=%lu addr=%lu pages=%p"
               " page_count=%d\n", (__u64)dio_data->cdd_offset,
               (unsigned long)dio_data->cdd_size, dio_data->cdd_addr,
        ...
        while (total_bytes > 0) {
                ...
                CDEBUG(D_VFSTRACE, "stripe_io: offset="LPU64" size=%lu addr=%lu"
