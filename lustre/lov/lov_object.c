 ...
 *
 * \param fiemap fiemap to hold all extents
 * \param lcl_fm_ext array of fiemap extents get from OSC layer
 * \param ost_index OST index to be written into the fm_device field for each
		    extent
 * \param ext_count number of extents to be copied
 * \param current_extent where to start copying in the extent array
 */
...
static obd_size fiemap_calc_fm_end_offset(struct ll_user_fiemap *fiemap,
					  struct lov_stripe_md *lsm,
					  obd_size fm_start, obd_size fm_end,
					  int *start_stripe)
{
...
/**
 * Get object's attributes (via @oinfo->oi_oa) by requesting OST_GETATTR RPC
 * for each underlying OSC objects.
 *
 * \param env	[in] lustre environment
 * \param obj	[in] object to be inquired
 * \param oinfo	[in] obd_info containing request info and result
 * \param set	[in] set holding requests of OST_GETATTR RPCs, if NULL, we'd
 *                   allocate it and free it after use
 *
 * \retval	0 on success and < 0 for error
 */
static int lov_object_obd_info_get(const struct lu_env *env,
				   struct cl_object *obj,
				   struct obd_info *oinfo,
				   struct ptlrpc_request_set *set)
{
	struct lov_stripe_md	*lsm;
	bool			locally_alloc_set = false;
	int			rc;
	ENTRY;
				   ...
				   int flags)
{
	...
	obdo->o_mode = S_IFREG;
	obdo->o_valid = OBD_MD_FLID | OBD_MD_FLTYPE;
