				 ...
				 struct cl_layout *layout)
{
	...
	struct lu_buf *buf = &layout->cl_buf;
	ssize_t rc;
	ENTRY;

	if (lsm == NULL) {
		layout->cl_is_released = false;
		layout->cl_generation = CL_LAYOUT_GEN_EMPTY;
		layout->cl_size = 0; /* no layout */

		RETURN(0);
	}

	layout->cl_generation = lsm->lsm_layout_gen;
	layout->cl_is_released = lsm_is_released(lsm);

	rc = lov_lsm_pack(lsm, buf->lb_buf, buf->lb_len);
	if (rc > 0) {
		layout->cl_size = rc;
	} else {
		/* buffer was not big enough, return the size */
		layout->cl_size = lov_lsm_pack(lsm, NULL, 0);
	}
