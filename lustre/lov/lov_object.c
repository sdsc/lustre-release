 ...
 */
static int fiemap_calc_last_stripe(struct lov_stripe_md *lsm, __u64 fm_start,
				   __u64 fm_end, int start_stripe,
				   int *stripe_count)
{
	int last_stripe;
 ...
 */
static __u64 fiemap_calc_fm_end_offset(struct fiemap *fiemap,
				       struct lov_stripe_md *lsm,
				       __u64 fm_start, __u64 fm_end,
				       int *start_stripe)
{
	__u64 local_end = fiemap->fm_extents[0].fe_logical;
			     ...
			     struct fiemap *fiemap, size_t *buflen)
{
	...
	for (cur_stripe = start_stripe; stripe_count > 0;
		...
		__u64	req_fm_len; /* Stores length of required mapping */
		__u64	len_mapped_single_call;
