				  ...
				  int stripe_idx)
{
	...
	if (rc != 0)
		return NULL;
	subdev = lovsub2cl_dev(dev->ld_target[ost_idx]);
	return  lov_sub_find(env, subdev, ofid, NULL);
}
 ...
 */
static int lov_obj_fiemap(const struct lu_env *env, struct cl_object *obj,
			  struct cl_ioc_fiemap *ifm)
{
	struct lov_stripe_md	*lsm;
	struct cl_object	*stripe_obj = NULL;
	 ...
	 */
	if (lsm->lsm_stripe_count > 1 && !(ifm->ioc_fiemap->fm_flags &
					   FIEMAP_FLAG_DEVICE_ORDER))
		GOTO(out, rc = -ENOTSUPP);
	ifm->ioc_fmkey->oa.o_oi = lsm->lsm_oi;

	if (!lsm_has_objects(lsm)) {
		if ((lsm != NULL) && lsm_is_released(lsm) &&
		    (fiemap->fm_start < fmkey->oa.o_size)) {
			/**
	...
	for (cur_stripe = start_stripe, i = 0; i < stripe_count;
		...
		/* find lobsub object */
		stripe_obj = lov_find_subobj(env, cl2lov(obj), lsm,
					     cur_stripe);
		if (IS_ERR(stripe_obj))
			GOTO(out, rc = PTR_ERR(stripe_obj));
		...
		do {
			...
			fmkey->oa.o_oi = lsm->lsm_oinfo[cur_stripe]->loi_oi;
			...
			*ifm->ioc_buflen =
			...
			rc = cl_object_ioctl(env, stripe_obj,
					     FSFILT_IOC_FIEMAP,
					     (unsigned long)ifm);
			if (rc != 0)
				GOTO(out, rc);
			    ...
			    unsigned int cmd, unsigned long arg)
{
	...
	case LL_IOC_LOV_GETSTRIPE:
		rc = lov_getstripe(obj, (struct lov_user_md __user *)arg);
		break;
	case FSFILT_IOC_FIEMAP:
		if (cl2lov(obj)->lo_type != LLT_RAID0)
			RETURN(-ENODATA);
		rc = lov_obj_fiemap(env, obj, (struct cl_ioc_fiemap *)arg);
		break;
	case IOC_FIND_CBDATA:
	default:
