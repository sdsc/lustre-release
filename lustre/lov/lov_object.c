...
loff_t lov_object_ra_size(struct cl_object *clob)
{
	...
	for (i = 0; i < lsm->lsm_stripe_count; i++) {
		u32 osc_ra_size, vallen = sizeof(u32);
		...
		rc = obd_get_info(NULL, tgt->ltd_exp, sizeof(KEY_RA_SIZE),
				  KEY_RA_SIZE, &vallen, &osc_ra_size);

		if (rc == 0 && (ra_size == 0 || osc_ra_size < ra_size))
			ra_size = osc_ra_size;
	}
