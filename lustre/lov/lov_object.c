...
struct cl_object *lov_find_subobj(const struct lu_env *env,
				  struct lov_object *lov,
...
static int lov_object_fiemap(const struct lu_env *env, struct cl_object *obj,
			     struct lov_stripe_md *lsm,
			     struct cl_ioc_fiemap *ifm)
{
	struct cl_object	*subobj = NULL;
	struct lov_obd		*lov = lu2lov_dev(obj->co_lu.lo_dev)->ld_lov;
	struct ll_fiemap_info_key *fmkey = ifm->ioc_fmkey;
	struct ll_user_fiemap	*fiemap = ifm->ioc_fiemap;
	struct ll_user_fiemap	*fm_local = NULL;
	struct ll_fiemap_extent	*lcl_fm_ext;
	int			count_local;
	bool			get_num_extents = false;
	int			ost_index = 0;
	int			actual_start_stripe;
	int			start_stripe;
	obd_size		fm_start;
	obd_size		fm_end;
	obd_size		fm_length;
	...
	if (fm_end_offset == -EINVAL)
		GOTO(out, rc = -EINVAL);
	if (fiemap_count_to_size(fiemap->fm_extent_count) >
	    *ifm->ioc_buflen) {
		fiemap->fm_extent_count =
			fiemap_size_to_count(*ifm->ioc_buflen);
	}
	if (fiemap->fm_extent_count == 0) {
		get_num_extents = true;
		count_local = 0;
	}
	/* Check each stripe */
	for (cur_stripe = start_stripe, i = 0; i < stripe_count;
		...
		do {
			...
			ifm->ioc_fiemap = fm_local;
			*ifm->ioc_buflen =
				fiemap_count_to_size(fm_local->fm_extent_count);

			rc = cl_object_ioctl(env, subobj, CL_IOC_FIEMAP,
					     (unsigned long)ifm);
			if (rc != 0)
				GOTO(out, rc);
inactive_tgt:
...
int lov_object_getattr(const struct lu_env *env, struct cl_object *obj,
		       struct cl_ioc_getattr *iga)
{
	struct cl_object	*subobj = NULL;
	struct lov_obd		*lov = lu2lov_dev(obj->co_lu.lo_dev)->ld_lov;
	struct obd_info		*oinfo = iga->iga_oinfo;
	...
	list_for_each(pos, &lovset->set_list) {
		...
		if (IS_ERR(subobj))
			GOTO(out, rc = PTR_ERR(subobj));
		iga->iga_oinfo = &req->rq_oi;
		rc = cl_object_ioctl(env, subobj, CL_IOC_GETATTR,
				     (unsigned long)iga);
		cl_object_put(env, subobj);
		if (rc != 0) {
	...
	if (!list_empty(&iga->iga_set->set_requests)) {
		LASSERT(rc == 0);
		LASSERT(iga->iga_set->set_interpret == NULL);
		iga->iga_set->set_interpret = lov_getattr_interpret;
		iga->iga_set->set_arg = (void *)lovset;
		GOTO(restore_iga, rc);
	}
out:
	if (rc)
...
static int lov_ioc_getattr(const struct lu_env *env, struct cl_object *obj,
			   struct lov_stripe_md *lsm,
			   struct cl_ioc_getattr *iga)
{
	...
	rc = lov_object_getattr(env, obj, iga);
	if (rc == 0)
...
static int lov_data_version(const struct lu_env *env, struct cl_object *obj,
			    struct lov_stripe_md *lsm,
			    struct ioc_data_version *idv)
{
	...
	if (idv->idv_flags & (LL_DV_RD_FLUSH | LL_DV_WR_FLUSH)) {
		oinfo.oi_oa->o_valid |= OBD_MD_FLFLAGS;
		oinfo.oi_oa->o_flags |= OBD_FL_SRVLOCK;
		if (idv->idv_flags & LL_DV_WR_FLUSH)
	...
	if (rc == 0) {
		...
		if (idv->idv_flags & LL_DV_WR_FLUSH &&
			...
			idv->idv_version = obdo->o_data_version;
	}
free_obdo:
	OBD_FREE_PTR(obdo);
out:
			    ...
			    unsigned int cmd, unsigned long arg)
{
	...
	case CL_IOC_LOV_GETSTRIPE:
		...
		break;
	case CL_IOC_FIEMAP:
		rc = lov_object_fiemap(env, obj, lsm,
				       (struct cl_ioc_fiemap *)arg);
		break;
	case CL_IOC_DATA_VERSION:
		rc = lov_data_version(env, obj, lsm,
				      (struct ioc_data_version *)arg);
		break;
	case CL_IOC_GETATTR:
		rc = lov_ioc_getattr(env, obj, lsm,
				     (struct cl_ioc_getattr *)arg);
		break;
