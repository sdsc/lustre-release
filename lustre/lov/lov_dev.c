			  ...
			  struct lu_context_key *key)
{
	...
	OBD_SLAB_ALLOC_PTR_GFP(info, lov_thread_kmem, CFS_ALLOC_NOFS);
	if (info != NULL)
                                  ...
                                  struct lu_context_key *key)
{
	...
	OBD_SLAB_ALLOC_PTR_GFP(info, lov_session_kmem, CFS_ALLOC_NOFS);
	if (info == NULL)
		info = ERR_PTR(-ENOMEM);
	return info;
}
			...
			struct cl_req *req)
{
	...
	ENTRY;
	OBD_SLAB_ALLOC_PTR_GFP(lr, lov_req_kmem, CFS_ALLOC_NOFS);
	if (lr != NULL) {
