...
static int lov_mdc_dev_init(const struct lu_env *env, struct lov_device *ld,
			    struct lu_device *next, __u32 idx, __u32 nr)
{
	...
	/* update LOVDOM array */
	ld->ld_md_tgts[nr].ldm_mdc = lu2cl_dev(next);
			      ...
			      struct obd_device *mdc, __u32 idx)
{
	...
	if (ld->ld_md_tgts_nr == LOV_MDC_TGT_MAX) {
		/* The maximum value of LOV_MDC_TGT_MAX is 32 now,
		 * if it become too small then all MD target handling
		 * must be rewritten in LOD manner, check lod_add_device()
