...
static int lfs_changelog(int argc, char **argv)
{
	...
	if (rc < 0) {
		...
		if (rec->cr_namelen)
			printf(" p=%s %.*s", fid_to_str(&rec->cr_pfid, fidstr),
		...
		if (rec->cr_flags & CLF_RENAME) {
			...
			if (!fid_is_zero(&rnm->cr_sfid))
				printf(" s=%s sp=%s %.*s",
				       fid_to_str(&rnm->cr_sfid, fidstr),
				       fid_to_str(&rnm->cr_spfid, fidstr),
	...
	for (path = argv + optind; *path != NULL; path++) {
		...
		if (!show_parents) {
			...
			while ((err = llapi_path2parent(*path, linkno, &fid,
				...
				printf("%s%s/%s", linkno != 0 ? "\t" : "",
				       fid_to_str(&fid, fidstr), name);
				linkno++;
			}
