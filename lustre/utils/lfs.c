			...
			int rc, bool h)
{
		...
		kbytes2str(dqb->dqb_bsoftlimit, strbuf, STRBUF_LEN, h);
		if (type == QC_GENERAL)
		...
		kbytes2str(dqb->dqb_bhardlimit, strbuf, STRBUF_LEN, h);
...
ug_output:
	...
	if (qctl.qc_valid == QC_GENERAL && qctl.qc_cmd != LUSTRE_Q_GETINFO &&
		...
		rc3 = print_obd_quota(mnt, &qctl, 0, human_readable,
				      &total_balloc);
		kbytes2str(total_balloc, strbuf, STRBUF_LEN, human_readable);
