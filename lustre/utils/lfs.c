...
static int lfs_ladvise(int argc, char **argv)
{
	...
	while ((c = getopt_long(argc, argv, short_opts,
		...
		case 'a':
			...
			if (advice_type == LU_LADVISE_INVALID) {
				fprintf(stderr, "error: invalid advice type "
					"'%s'\n", optarg);
	...
	if (advice_type == LU_LADVISE_INVALID) {
		fprintf(stderr, "error: please give an advice type\n");
		fprintf(stderr, "Valid types:");
	...
	if (argc <= optind) {
		fprintf(stderr, "error: please give one or more file names\n");
		return CMD_HELP;
	}

	if (end != LUSTRE_EOF && length != 0 && end != start + length) {
		fprintf(stderr, "error: conflicting arguments of -l and -e\n");
		return CMD_HELP;
	}
	...
	if (end <= start) {
		fprintf(stderr, "error: range [%llu, %llu] is invalid\n",
			start, end);
		return CMD_HELP;
	}
