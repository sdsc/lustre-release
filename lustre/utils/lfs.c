...
#define SSM_CMD_COMMON(_cmd, _tgt) \
...
#define SETSTRIPE_USAGE(_cmd, _tgt)		\
	SSM_CMD_COMMON(_cmd, _tgt)		\
	SSM_HELP_COMMON

#define MIGRATE_USAGE(_cmd, _tgt)					\
	SSM_CMD_COMMON(_cmd, _tgt)					\
	"                 [--block|-b] "_tgt"\n"			\
	"                 [--hsm|-H]\n"\
	"                 [--archive|-a NUM]\n"\
...
command_t cmdlist[] = {
	...
	{"migrate", lfs_setstripe, 0, "migrate file from one OST layout to "
	 ...
	 MIGRATE_USAGE("migrate  ", "<filename>")},
	{"mv", lfs_mv, 0,
			   ...
			   int archive_id)
{
	...
	if (param->lsp_pool)
		strncpy(hsm_param->lsp_pool, param->lsp_pool,
			sizeof(hsm_param->lsp_pool));
	else
...
static int lfs_setstripe(int argc, char **argv)
{
	...
	while ((c = getopt_long(argc, argv, "c:di:o:p:s:S:a:H",
				long_opts, NULL)) >= 0) {
		switch (c) {
		case 0:
			/* Long options. */
