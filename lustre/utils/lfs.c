...
static int migrate_copy_data(int fd_src, int fd_dst, size_t bufsz)
{
	...
	do {
		...
		if (wpos == rpos) {
			rc = llapi_lease_check(fd_src);
			if (rc <= 0) {
				rc = rc ? rc : -EAGAIN;
				fprintf(stderr, "File is being operated by "
					"others, migration aborted\n");
				goto out;
			}
		...
		if (wsize < 0) {
			rc = -errno;
			fprintf(stderr, "write failed on volatile: %s\n",
				strerror(-rc));
			goto out;
		}
		wpos += wsize;
		       ...
		       int stripe_pattern, char *pool_name)
{
	...
	rc = migrate_copy_data(fd, fdv, bufsz);
	if (rc < 0) {
		fprintf(stderr, "Data copy failed on %s\n", name);
		goto error;
	}
	...
	if (dv1 != dv2) {
		rc = -EAGAIN;
		fprintf(stderr, "data version changed on %s during migration\n",
			name);
		goto error;
	}
	...
	if (rc < 0) {
		fprintf(stderr, "giving up migration on %s: %s\n",
			name, strerror(-rc));
		have_rlck = false;
		goto error;
	}
	have_rlck = false;
	fd = -1; /* was closed by llapi_close_layout_swap */
