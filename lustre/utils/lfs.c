...
static int migrate_block(int fd, int fdv, const struct stat *st, size_t bufsz,
			 const char *name)
{
	__u64	dv1;
	int	gid;
		       ...
		       struct llapi_stripe_param *param)
{
	...
	if (migration_flags & MIGRATION_BLOCKS) {
		rc = migrate_block(fd, fdv, &st, bufsz, name);
	} else {
		rc = migrate_nonblock(fd, fdv, &st, bufsz, name);
		if (rc == 0) {
			have_rlck = false;
