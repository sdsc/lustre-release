...
static inline int name2uid(unsigned int *id, const char *name)
{
...
static inline int name2gid(unsigned int *id, const char *name)
{
	struct group *group;
...
static inline int uid2name(char **name, unsigned int id)
{
	struct passwd *passwd;
			      ...
			      bool human_readable)
{
	printf("Disk quotas for %s %s (%cid %u):\n",
	       qtype2name(qctl->qc_type), name,
	       *qtype2name(qctl->qc_type), qctl->qc_id);
...
static int get_id(__u32 *id, int type)
{
	switch (type) {
	case USRQUOTA:
		*id = geteuid();
...
all_output:
		...
		qctl.qc_idx = idx;
		qctl.qc_type = pass;
		rc = get_id(&qctl.qc_id, qctl.qc_type);
		LASSERT(rc == 0);
		switch (qctl.qc_type) {
		case USRQUOTA:
