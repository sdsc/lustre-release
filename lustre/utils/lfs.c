...
static int lfs_setstripe(int argc, char **argv)
{
	...
	if (llapi_layout_verify(layout, fname) == -1) {
		fprintf(stderr, "error: %s: file system %s does not support "
			"supplied layout\n", argv[0], fname);
		return errno;
	}
	...
	do {
		if (!migrate_mode) {
			...
			if ((stat(fname, &st) == 0) && (S_ISDIR(st.st_mode))) {
				result = llapi_layout_file_open(fname, O_WRONLY,
								0644, layout);
			} else {
				result = llapi_layout_file_create(fname,
								  O_WRONLY,
								  0644, layout);
			}
		} else
