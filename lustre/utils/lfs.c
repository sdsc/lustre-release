...
static int lfs_mv(int argc, char **argv)
{
	...
	while ((c = getopt_long(argc, argv, "i:", long_opts, NULL)) != -1) {
		...
		case 'i': {
			...
			if (*end != '\0') {
				fprintf(stderr, "%s: bad mdt index'%s'\n",
					argv[0], optarg);
				return CMD_HELP;
			}
			break;
	...
	if (param.mdtindex == -1) {
		fprintf(stderr, "%s mdtindex must be indicated\n", argv[0]);
		return CMD_HELP;
	}

	if (optind >= argc) {
		fprintf(stderr, "%s path is not provided\n", argv[0]);
		return CMD_HELP;
	}
	...
	if (rc != 0)
                fprintf(stderr, "error: migrate '%s' to mdt%x: %s\n",
                        argv[0], param.mdtindex, strerror(-rc));
	return rc;
}
