...
int lr_parse_line(void *priv, struct lr_info *info)
{
	...
	namelen = min(sizeof(info->name), rec->cr_namelen);
	strncpy(info->name, changelog_rec_name(rec), namelen);
	info->name[namelen] = '\0';
	if (namelen < rec->cr_namelen)
		fprintf(stderr, "Changelog name '%s' was truncated.\n",
	...
	if (rec->cr_flags & CLF_RENAME && !fid_is_zero(&rnm->cr_sfid)) {
		...
		snprintf(info->spfid, sizeof(info->spfid), DFID,
			 PFID(&rnm->cr_spfid));
		namelen = min(sizeof(info->sname), changelog_rec_snamelen(rec));
		strncpy(info->sname, changelog_rec_sname(rec), namelen);
		info->sname[namelen] = '\0';
		if (namelen < changelog_rec_snamelen(rec))
			fprintf(stderr, "Changelog name '%s' was truncated.\n",
