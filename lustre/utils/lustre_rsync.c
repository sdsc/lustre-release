 ...
 *      else mv .lustrerepl/[tfid] to (spfid,name2)
 *    Else if pfid is not present,
 *      if spfid is present, mv (spfid,sname) .lustrerepl/[tfid]
...
/* remove [info->pfid, info->tfid] from parents */
...
int lr_move(struct lr_info *info, struct lr_info *ext)
{
        ...
        if (info->version == 1) {
                memcpy(info->spfid, ext->pfid, sizeof(info->spfid));
                strncpy(info->sname, ext->name, sizeof(info->sname));
        }
        ...
        for (info->target_no = 0; info->target_no < status->ls_num_targets;
             info->target_no++) {
                ...
                if (special_src) {
                        lr_remove_pc(info->spfid, info->tfid);
                        if (!special_dest)
                                lr_cascade_move(info->tfid, info->dest, info);
                }
                if (special_dest)
                        lr_add_pc(info->pfid, info->tfid, info->name);
...
int lr_parse_line(void *priv, struct lr_info *info)
{
        ...
        info->version = changelog_version(rec);
        switch (info->version) {
        case 1:
                strncpy(info->name, ((struct changelog_rec_v1 *)rec)->cr_name,
                        sizeof(info->name));
                break;
        case 2:
                sprintf(info->spfid, DFID, PFID(&rec->cr_spfid));
                strncpy(info->name, rec->cr_name, sizeof(info->name));
                if (fid_is_sane(&rec->cr_spfid))
                        strncpy(info->sname,
                                rec->cr_name + strlen(rec->cr_name) + 1,
                                rec->cr_namelen - strlen(rec->cr_name) - 1);
                info->sname[rec->cr_namelen] = '\0';
                break;
        default:
                fprintf(stderr, "Current version %u, unable to handle version "
                        "%u changelog.\n", CHANGELOG_VERSION, info->version);
                return -1;
        }
        ...
        if (verbose > 1 && info->version > 1) {
                printf("Rec %lld: %d %s", info->recno, info->type,
                       info->name);
                if (fid_is_sane(&rec->cr_spfid))
                        printf(" %s", info->sname);
                printf("\n");
...
int lr_replicate()
{
        ...
        while (!quit && lr_parse_line(changelog_priv, info) == 0) {
                rc = 0;
                if (info->type == CL_RENAME && info->version == 1)
                        /* Rename operations have an additional changelog
                         * record of information if version == 1. */
