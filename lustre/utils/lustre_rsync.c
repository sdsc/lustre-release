...
int lr_add_pc(const char *pfid, const char *tfid, const char *name)
{
        ...
        if (!p)
                return -ENOMEM;
	strlcpy(p->pc_log.pcl_pfid, pfid, sizeof(p->pc_log.pcl_pfid));
	strlcpy(p->pc_log.pcl_tfid, tfid, sizeof(p->pc_log.pcl_tfid));
	strlcpy(p->pc_log.pcl_name, name, sizeof(p->pc_log.pcl_name));
