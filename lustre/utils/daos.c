...
static int
daos_event2id(daos_event_t *ev, daos_event_id_t *eid)
{
...
daos_event_fini(daos_event_t *ev)
{
	...
	rc = daos_event2id(ev, &evf.evf_id);
	if (rc != 0)
		return rc;
...
daos_event_abort(daos_event_t *ev)
{
	...
	rc = daos_event2id(ev, &eva.eva_id);
	if (rc != 0)
		return rc;
...
daos_event_next(daos_event_t *parent, daos_event_t *child)
{
	...
	rc = daos_event2id(parent, &evn.evn_parent);
	if (rc != 0)
		return NULL;
...
daos_noop(daos_event_t *ev, int latency)
{
	...
	rc = daos_event2id(ev, &unp.unp_ev);
	if (rc != 0)
		return rc;
