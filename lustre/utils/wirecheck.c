...
check_ptlrpc_body(void)
{
	...
	CHECK_DEFINE_X(LUSTRE_OSS_VERSION);
...
main(int argc, char **argv)
{
	...
	CHECK_VALUE(OSS_REPLY);
	CHECK_VALUE(OSS_GETATTR);
	CHECK_VALUE(OSS_SETATTR);
	CHECK_VALUE(OSS_READ);
	CHECK_VALUE(OSS_WRITE);
	CHECK_VALUE(OSS_CREATE);
	CHECK_VALUE(OSS_DESTROY);
	CHECK_VALUE(OSS_GET_INFO);
	CHECK_VALUE(OSS_CONNECT);
	CHECK_VALUE(OSS_DISCONNECT);
	CHECK_VALUE(OSS_PUNCH);
	CHECK_VALUE(OSS_OPEN);
	CHECK_VALUE(OSS_CLOSE);
	CHECK_VALUE(OSS_STATFS);
	CHECK_VALUE(OSS_SYNC);
	CHECK_VALUE(OSS_SET_INFO);
	CHECK_VALUE(OSS_QUOTACHECK);
	CHECK_VALUE(OSS_QUOTACTL);
	CHECK_VALUE(OSS_QUOTA_ADJUST_QUNIT);
	CHECK_VALUE(OSS_LAST_OPC);
