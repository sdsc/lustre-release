...
#define CHECK_CVALUE_64X(a)                                     \
        printf("        CLASSERT("#a" == 0x%.16llxULL);\n", a)
...
check_mdt_body(void)
{
        ...
        CHECK_VALUE_O(FMODE_READ);
        CHECK_VALUE_O(FMODE_WRITE);
        CHECK_VALUE_O(FMODE_EPOCH);
        CHECK_VALUE_O(FMODE_TRUNC);
        CHECK_VALUE_O(FMODE_SOM);
        CHECK_VALUE_O(FMODE_CLOSED);
...
check_obdo(void)
{
        ...
        CHECK_CVALUE_X(OBD_FL_CKSUM_RSVD1);
...
main(int argc, char **argv)
{
        ...
        CHECK_VALUE_X(OBD_STATFS_NODELAY);
        CHECK_VALUE_X(OBD_STATFS_FROM_CACHE);
        CHECK_VALUE_X(OBD_STATFS_PTLRPCD);
