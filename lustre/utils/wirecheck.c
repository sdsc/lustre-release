...
static void check_object_update(void)
{
	...
	CHECK_MEMBER(object_update, ou_padding2);
	CHECK_MEMBER(object_update, ou_batchid);
	CHECK_MEMBER(object_update, ou_fid);
	CHECK_MEMBER(object_update, ou_lens);
	CHECK_MEMBER(object_update, ou_bufs);
}
...
static void check_object_update_request(void)
{
	...
	CHECK_MEMBER(object_update_request, ourq_bufs);
}
...
main(int argc, char **argv)
{
	...
	CHECK_VALUE(OBJECT_UPDATE);
	CHECK_VALUE(OBJECT_UPDATE_LAST_OPC);
