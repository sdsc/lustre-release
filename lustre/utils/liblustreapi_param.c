...
/**
  ...
  *
  * The \a param is appended to /proc/{fs,sys}/{lnet,lustre}/ to
  ...
  */
static int
create_param_arg(char *name, size_t name_size, const char *param, va_list args)
{
	char pattern[PATH_MAX + 1] = "/proc/{fs,sys}/{lnet,lustre}/";
	const int len = 29; /* length of above /proc path prefix */
	glob_t glob_info;
	char *tmp;
	int rc;
...
/**
  ...
  *
  * The \param param is appended to /proc/{fs,sys}/{lnet,lustre}/ to
  * complete the absolute path to the file containing the parameter data
  ...
  */
char **
llapi_get_param(char *name, size_t name_size, size_t *list_size,
		const char *param, ...)
{
	...
	*list_size = 0;
	...
	if (create_param_arg(name, name_size, param, args) < 0)
		return list;
	...
	if (temp_errno == 0) {
		...
		if (offsets != NULL) {
			char *tmp = (char *) offsets + count * sizeof(offset);
	...
	if (temp_errno != 0) {
		/* Only free items on error. It's data is used to create the
		 * returned list */
		if (items != NULL)
			free(items);
	}
