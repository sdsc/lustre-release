...
int get_lustre_clilov_path(glob_t *path, char *fsname, char *file_path,
			   char *param)
{
	char buf[PATH_MAX], fs[PATH_MAX];
	int rc;

	if (fsname == NULL) {
		if (file_path == NULL) {
			fprintf(stderr, "invalid paramaters for poolpath\n");
			return -EINVAL;
		}
	...
	fprintf(stderr, "search for filesystem name %s\n", fs);

	rc = snprintf(buf, sizeof(buf), "%s-clilov-*", fs);
	if (rc >= sizeof(buf))
		return -E2BIG;

	fprintf(stderr, "param path filter %s\n", buf);

	rc = get_lustre_param_path(path, "lov", buf,
				   FILTER_BY_EXACT, param);
	if (rc != 0) {
		fprintf(stderr, "failed to get param path\n");
		return -ENOENT;
	}
