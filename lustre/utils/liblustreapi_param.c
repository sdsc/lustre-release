...
int find_target_obdpath(char *path, char *tgt, enum tgt_type type)
{
	...
	rc = cfs_get_param_path(&pattern, "%s/%s/target_obd",
...
int poolpath(char *fsname, char *pathname, char *pool_pathname)
{
	...
	rc = cfs_get_param_path(&pattern, "lov/%s-*/pools",
				fsname);
	if (rc != 0)
		return -ENOENT;
	      ...
	      char *buf, size_t buf_size)
{
	...
	rc = cfs_get_param_path(&path, "%s/%s/%s",
				type, inst, param);
	if (rc != 0)
		return -ENOENT;
...
clilovpath(const char *fsname, const char *const pathname, char *clilovpath)
{
	...
	rc = cfs_get_param_path(&pattern, "lov/%s-clilov-*",
				fsname);
	if (rc != 0)
		return -ENOENT;
