	       ...
	       char **mountopts, char *old_fsname)
{
	...
	char *optstring = "Bf:Ghi:L:m:no:p:qs:t:u:vV"
#ifndef TUNEFS
			  "b:c:d:k:MOrR";
#else
			  "eNQR::w";
	...
	while ((opt = getopt_long(argc, argv, optstring, long_opt, &longidx)) !=
		...
		case 'i': {
			...
			ldd->ldd_svindex = strtoul(optarg, &endptr, base);
			if (*endptr != '\0') {
	...
	if (mop->mo_flags & MO_RENAME) {
		...
		if (ldd->ldd_flags & LDD_F_WRITECONF) {
			fprintf(stderr, "Warning: ignore writeconf under "
				"rename mode for preserving the config log. "
				"If you do not want to keep the config log, "
				"please drop '--rename'\n");
			ldd->ldd_flags &= ~LDD_F_WRITECONF;
		}
	}
