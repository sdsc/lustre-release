...
/**
 * append_unique: append @key or @key=@val pair to @buf only if it's the first
 *                time the @key is appended
 *      @buf: buffer to hold @key or @key=@val
 *      @prefix: prefix string before @key
 *      @key: key string
 *      @val: value string if key required a value
 */
...
static void enable_default_ext4_features(struct mkfs_opts *mop, char *anchor, int user_spec)
{
        if (IS_OST(&mop->mo_ldd)) {
                append_unique(anchor, user_spec ? "," : " -O ",
                              "extents", NULL, sizeof(mop->mo_mkfsopts));
                append_unique(anchor, ",", "uninit_bg", NULL,
                              sizeof(mop->mo_mkfsopts));
        } else if (IS_MDT(&mop->mo_ldd)) {
                append_unique(anchor, user_spec ? "," : " -O ",
                              "dirdata", NULL, sizeof(mop->mo_mkfsopts));
                append_unique(anchor, ",", "uninit_bg", NULL,
                              sizeof(mop->mo_mkfsopts));
        } else {
                append_unique(anchor, user_spec ? "," : " -O ",
                              "uninit_bg", NULL, sizeof(mop->mo_mkfsopts));
        }

        /* Multiple mount protection enabled only if failover node specified */
        if (failover) {
                if (is_e2fsprogs_feature_supp("-O mmp") == 0)
                        append_unique(anchor, ",", "mmp", NULL,
                                      sizeof(mop->mo_mkfsopts));
                else
        ...
        if (is_e2fsprogs_feature_supp("-O dir_nlink") == 0)
                append_unique(anchor, ",", "dir_nlink", NULL,
                              sizeof(mop->mo_mkfsopts));
        ...
        if (is_e2fsprogs_feature_supp("-O huge_file") == 0)
                append_unique(anchor, ",", "huge_file", NULL,
                              sizeof(mop->mo_mkfsopts));
        ...
        if (mop->mo_device_sz / (L_BLOCK_SIZE >> 10) >= 0x100002000ULL &&
                ...
                append_unique(anchor, ",", "64bit", NULL,
                              sizeof(mop->mo_mkfsopts));
        ...
        if (is_e2fsprogs_feature_supp("-O flex_bg") == 0) {
                ...
                append_unique(anchor, ",", "flex_bg", NULL,
                              sizeof(mop->mo_mkfsopts));

                if (IS_OST(&mop->mo_ldd)) {
                        snprintf(tmp_buf, sizeof(tmp_buf), " -G %u",
                                 (1 << 20) / L_BLOCK_SIZE);
                        strscat(anchor, tmp_buf,
                                sizeof(mop->mo_mkfsopts));
                }
        }
 ...
 *                  string in the end
 *      @start: where the option string starts
...
again:
        while (end && *end != '-')
                ++end;
        /* special extended option key "stripe-width" */
        if ((strncmp(end, "-width", 6) == 0) &&
            ((end - start) >= 6) && (strncmp(end - 6, "stripe-width", 12) == 0)) {
                ++end;
                goto again;
        }
        ...
        if ((mop->mo_ldd.ldd_mount_type == LDD_MT_EXT3) ||
            ...
            (mop->mo_ldd.ldd_mount_type == LDD_MT_LDISKFS2)) {
                ...
                if ((start = strstr(mop->mo_mkfsopts, "-O")) != NULL) {
                        ...
                        enable_default_ext4_features(mop, start, 1);
                } else {
                        enable_default_ext4_features(mop,
                                mop->mo_mkfsopts + strlen(mop->mo_mkfsopts), 0);
                }
                ...
                if (IS_OST(&mop->mo_ldd) && mop->mo_device_sz > 100 * 1024 &&
                    mop->mo_device_sz * 1024 / L_BLOCK_SIZE <= 0xffffffffULL) {
                        ...
                        append_unique(start, ext_opts ? "," : " -E ",
                                      "resize", buf, sizeof(mop->mo_mkfsopts));
                        ext_opts = 1;
                }
