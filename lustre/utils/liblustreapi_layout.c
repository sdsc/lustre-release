...
__llapi_layout_from_lum(const struct lov_user_md_v3 *lum)
{
	...
	if (lum->lmm_stripe_size == 0)
		layout->llot_stripe_size = LLAPI_LAYOUT_USE_FS_DEFAULT;
	else
		layout->llot_stripe_size = lum->lmm_stripe_size;

	if (lum->lmm_stripe_count == 0)
		layout->llot_stripe_count = LLAPI_LAYOUT_USE_FS_DEFAULT;
	else if (lum->lmm_stripe_count == -1)
		layout->llot_stripe_count = LLAPI_LAYOUT_USE_ALL_OSTS;
	else
...
__llapi_layout_to_lum(const llapi_layout_t *layout)
{
	...
	if (layout->llot_stripe_size == LLAPI_LAYOUT_USE_FS_DEFAULT)
		lum->lmm_stripe_size = 0;
	else
		lum->lmm_stripe_size = layout->llot_stripe_size;

	if (layout->llot_stripe_count == LLAPI_LAYOUT_USE_FS_DEFAULT)
		lum->lmm_stripe_count = 0;
	else
		lum->lmm_stripe_count = layout->llot_stripe_count;

	if (layout->llot_stripe_offset == LLAPI_LAYOUT_USE_FS_DEFAULT)
...
llapi_layout_t *llapi_layout_alloc(void)
{
	...
	layout->llot_stripe_size = LLAPI_LAYOUT_USE_FS_DEFAULT;
	layout->llot_stripe_count = LLAPI_LAYOUT_USE_FS_DEFAULT;
	layout->llot_stripe_offset = LLAPI_LAYOUT_USE_ALL_OSTS;
...
llapi_layout_t *llapi_layout_by_path(const char *path)
{
	...
	if (fd >= 0) {
		if (fgetxattr(fd, XATTR_LUSTRE_LOV, lov_buf,
			      sizeof(lov_buf)) < 0) {
			close(fd);
			errno = ENOTTY;
			return NULL;
...
int llapi_layout_stripe_count_get(const llapi_layout_t *layout, uint64_t *count)
{
	...
	if (layout->llot_stripe_count == -1)
		*count =  LLAPI_LAYOUT_USE_ALL_OSTS;
	else if (layout->llot_stripe_count == 0)
		*count =  LLAPI_LAYOUT_USE_FS_DEFAULT;
	else
...
{
	if (stripe_count == LLAPI_LAYOUT_USE_ALL_OSTS)
		stripe_count = -1;
	else if (stripe_count == LLAPI_LAYOUT_USE_FS_DEFAULT)
		stripe_count = 0;
...
int llapi_layout_stripe_size_get(const llapi_layout_t *layout, uint64_t *size)
{
	...
	if (layout->llot_stripe_size == 0)
		*size = LLAPI_LAYOUT_USE_FS_DEFAULT;
	else
...
{
	if (stripe_size == LLAPI_LAYOUT_USE_FS_DEFAULT)
		stripe_size = 0;
...
int llapi_layout_ost_index_set(llapi_layout_t *layout, int stripe_number,
			       uint64_t ost_index)
{
	if (ost_index == LLAPI_LAYOUT_USE_FS_DEFAULT)
		ost_index = -1;
			       ...
			       uint64_t *index)
{
	...
	if (layout->llot_objects[stripe_number].l_ost_idx == -1)
		*index = LLAPI_LAYOUT_USE_FS_DEFAULT;
	else
