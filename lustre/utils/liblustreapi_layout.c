...
{
	size_t objects_sz, stripe_count = lum->lmm_stripe_count;
	struct llapi_layout *layout;
	...
	if (lum->lmm_magic == LOV_USER_MAGIC_V1) {
		const struct lov_user_md_v1 *lum_v1 = (void *)lum;
		...
		if (objects_sz != 0) {
			memcpy(layout->llot_objects, lum->lmm_objects,
			       objects_sz);
			      ...
			      const char *path, char *fsname)
{
	...
	/* User doesn't care above stripe layout */
	if (!layout->llot_objects_are_valid)
		return true;

	/* Deal with stripe count -1 case */
