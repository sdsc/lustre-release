...
static struct llapi_layout *__llapi_layout_alloc(unsigned int num_stripes)
{
	...
	if (layout != NULL &&
	    __llapi_layout_objects_realloc(layout, num_stripes) < 0) {
		free(layout);
		layout = NULL;
	}
...
#define P2_BOUNDARY_ROUNDUP(n, p2)	(((n) + (p2)) & ~((p2) - 1))
			       ...
			       uint64_t ost_index)
{
	...
	if (stripe_number == 0 && ost_index == LLAPI_LAYOUT_DEFAULT) {
		...
		if (__llapi_layout_objects_realloc(layout,
				P2_BOUNDARY_ROUNDUP(stripe_number, 8)) < 0)
			return -1;
