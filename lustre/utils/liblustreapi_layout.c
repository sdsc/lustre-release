...
struct llapi_layout {
	...
	char		llot_poolname[LOV_MAXPOOLNAME + 1];
...
llapi_layout_from_lum(const struct lov_user_md_v3 *lum)
{
	...
	if (lum->lmm_magic == LOV_USER_MAGIC_V3) {
		snprintf(layout->llot_poolname, sizeof(layout->llot_poolname),
			 "%s", lum->lmm_pool_name);
...
llapi_layout_to_lum(const struct llapi_layout *layout)
{
	...
	strncpy(lum->lmm_pool_name, layout->llot_poolname,
 ...
 * \retval	-1 if arguments are invalid
 */
int llapi_layout_poolname_get(const struct llapi_layout *layout, char *dest,
			       size_t n)
{
	if (layout == NULL || layout->llot_magic != LLAPI_LAYOUT_MAGIC ||
	    dest == NULL) {
	...
	strncpy(dest, layout->llot_poolname, n);
	return 0;
}
 ...
 * \param[in] poolname	pool name to set
 *
 * \retval	0 on success
 * \retval	-1 if arguments are invalid or pool name is too long
 */
int llapi_layout_poolname_set(struct llapi_layout *layout,
			       const char *poolname)
{
	char *ptr;
	...
	strncpy(layout->llot_poolname, poolname,
		sizeof(layout->llot_poolname));
	return 0;
}
