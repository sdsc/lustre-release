...
__llapi_layout_from_lov_user_md(const struct lov_user_md_v3 *lum)
{
	size_t stripe_count = lum->lmm_stripe_count;
	struct llapi_layout *layout;
	size_t objects_sz;
	...
	/* If the number of stripes is zero we end up copying nothing.
	 * In this case the objects are still considered valid */
...
/**
 * Private helper to copy the data from a llapi_layout to a
 * newly allocated lov_user_md. The current version of this API
...
__llapi_layout_to_lum(const struct llapi_layout *layout)
{
	...
	lum_size = lov_user_md_size(layout->llot_stripe_count, magic);
	lum = malloc(lum_size);
	if (lum == NULL)
		return NULL;
...
struct llapi_layout *llapi_layout_by_fd(int fd)
{
	...
	if (fgetxattr(fd, XATTR_LUSTRE_LOV, lum, lum_len) < 0) {
		...
		layout = __llapi_layout_from_lov_user_md(lum);
	}
			      ...
			      const char *path, char *fsname)
{
	...
	if (get_param_obdvar(fsname, path, "lov", "numobd",
			     data, sizeof(data)) < 0)
		return -1;
	...
	/* We need to test if the stripe offset belongs to the file system
	 * and the pool */
			   ...
			   const struct llapi_layout *layout)
{
	...
	if (llapi_layout_ost_index_valid(layout, path, fsname)) {
		errno = EINVAL;
		return -1;
	}
	...
	if (layout != NULL) {
		struct lov_user_md *lum = __llapi_layout_to_lum(layout);
		...
		if (fd != -1) {
			if (fsetxattr(fd, XATTR_LUSTRE_LOV, (void *)lum,
				      sizeof(*lum), 0) < 0)  {
				/* Chase errno before close stomps on it. */
				tmp = errno;
				close(fd);
				errno = tmp;
				fd = -1;
