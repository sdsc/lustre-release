 ...
 */
static void
llapi_layout_swab_lov_user_md(struct lov_user_md *lum, int lmm_object_count)
{
	int i;
	...
	if (lum->lmm_magic == LOV_MAGIC_V3)
		lod = lumv3->lmm_objects;
	else
		lod = lum->lmm_objects;

	for (i = 0; i < lmm_object_count; i++)
...
llapi_layout_from_lum(const struct lov_user_md *lum, size_t lmm_object_count)
{
	struct llapi_layout *layout;
	size_t objects_sz;

	objects_sz = lmm_object_count * sizeof(lum->lmm_objects[0]);

	layout = __llapi_layout_alloc(lmm_object_count);
	if (layout == NULL)
		return NULL;
...
struct llapi_layout *llapi_layout_get_by_fd(int fd, uint32_t flags)
{
	...
	lmm_object_count = llapi_layout_objects_in_lum(lum, bytes_read);
	...
	if (fstat(fd, &st) < 0)
		goto out;
	if (!S_ISDIR(st.st_mode) && lmm_object_count != lum->lmm_stripe_count) {
		errno = EINTR;
		goto out;
	}
	...
	if (lum->lmm_magic == __swab32(LOV_MAGIC_V1) ||
	    lum->lmm_magic == __swab32(LOV_MAGIC_V3))
		llapi_layout_swab_lov_user_md(lum, lmm_object_count);

	layout = llapi_layout_from_lum(lum, lmm_object_count);
...
static bool llapi_layout_stripe_count_is_valid(uint64_t stripe_count)
{
...
static bool llapi_layout_stripe_index_is_valid(uint64_t stripe_index)
{
