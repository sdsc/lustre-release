...
__llapi_layout_from_lum(const struct lov_user_md_v3 *lum)
{
	...
	layout->llot_generation		= lum->u.lum_layout_gen;
...
/**
 * Read generation number of \a layout.
 */
int llapi_layout_generation(const lustre_layout_t *layout)
{
	if (layout == NULL) {
		errno = EINVAL;
		return -1;
	}
	/* Distinguish valid -1 return value (meaning on-disk generation
	 * number hasn't been read yet) from an error. */
	if (layout->llot_generation == -1)
		errno = 0;
	return layout->llot_generation;
}
...
int llapi_layout_obj_id(const lustre_layout_t *layout, int stripe_number)
{
	...
	return layout->llot_objects[stripe_number].l_object_id;
}
