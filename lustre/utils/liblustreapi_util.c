 ...
 *
 * We want to extract only the \a string subset of the version from this file.
 */
static get_version_string(const char *string, char *version, int version_size)
{
	char buffer[4096];
	char *ptr;
	int rc;
	...
	if (ptr != NULL) {
		...
		if (tmp != NULL)
			*tmp = '\0';
		if (strlcpy(version, ptr + strlen(string), version_size) >=
		    version_size) {
			errno = EOVERFLOW;
			return -1;
		}
	} else {
		/* This probably shouldn't be used by the caller since an
		 * error is being returned, but just in case. */
		strlcpy(version, "unknown.version", version_size);
