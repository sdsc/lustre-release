...
static int zfs_check_mmp(struct mkfs_opts *mop)
{
	FILE *f;
	...
	if (hostid == 0) {
		if (mop->mo_flags & MO_NOMMP_CHECK) {
			fprintf(stderr, "WARNING: spl_hostid not set. ZFS has "
				"no multi-mount-protection\n");
		} else {
			fatal();
			fprintf(stderr, "spl_hostid not set. Either set "
				"/etc/hostid and reload spl or call with "
				"--force-nohostid\n");
			return EINVAL;
		}
	}
