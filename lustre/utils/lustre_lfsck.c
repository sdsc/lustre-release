			      ...
			      int types, bool multipe_devices)
{
	...
	rc = glob("/proc/fs/lustre/mdd/*", GLOB_NOSORT, NULL, &param);
	if (rc != 0) {
		if (multipe_devices || rc != GLOB_NOMATCH ||
		...
		rc = glob("/proc/fs/lustre/obdfilter/*", GLOB_NOSORT,
			  NULL, &param);
		if (rc != 0) {
			fprintf(stderr, "Fail to get device name: rc = %d\n."
				"You can specify the device explicitly "
	...
	if (ptr == NULL) {
		fprintf(stderr, "Detect invalid device name %s. "
	...
	if (!multipe_devices && param.gl_pathc > 1) {
		fprintf(stderr,
