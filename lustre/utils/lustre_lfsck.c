...
int jt_lfsck_start(int argc, char **argv)
{
	...
	while ((opt = getopt_long(argc, argv, optstring, long_opt_start,
		...
		case 'm':
			if (strcmp(optarg, "otable") == 0) {
				start.ls_method = LM_OTABLE;
			} else if (strcmp(optarg, "namespace") == 0) {
				start.ls_method = LM_NAMESPACE;
			} else {
				fprintf(stderr,
	...
	if (rc < 0) {
		/* XXX: Old version kernel service may be in compatible with new
		 *	version user space tools, need to be processed here. */
