...
yaml_document_delete(yaml_document_t *document)
{
    ...
    while (!STACK_EMPTY(0, document->nodes)) {
        yaml_node_t node = POP(0, document->nodes);
        yaml_free(node.tag);
        switch (node.type) {
            ...
            case YAML_SEQUENCE_NODE:
                STACK_DEL(0, node.data.sequence.items);
                break;
            case YAML_MAPPING_NODE:
                STACK_DEL(0, node.data.mapping.pairs);
                break;
            default:
                assert(0);  /* Should not happen. */
        }
    }
    STACK_DEL(0, document->nodes);
