...
void print_usage()
{
		...
		" writes that to/{proc,sys}/fs/lustre/llite/.../stats_track_gid"
...
char* get_path_stats(int with_llstat, char* stats_path)
{
	...
	if (glob("/{proc/{fs,sys},sys/{fs,class,kernel/debug}}/lustre/llite/*",
		 GLOB_DOOFFS, NULL, &stats_glob_buffer) != 0) {
		fprintf(stderr, "Error: Couldn't find lustre/llite entries\n");
                exit(1);
        }
