			...
			const struct hsm_action_item *hai, long hal_flags)
{
	...
	if (rc < 0) {
		CT_ERROR("'%s' seek to read to "LPU64" (len %ld)"
			 " failed (%s)\n",
			 src, hai->hai_extent.offset, (long)src_st.st_size,
			 strerror(errno));
		rc = -errno;
		goto out;
	}
...
out:
	...
	if ((hai->hai_action == HSMA_RESTORE) &&
		...
		if (rc < 0) {
			rc = -errno;
			CT_ERROR("'%s' final truncate to %ld failed (%s)\n",
				 dst, (long)src_st.st_size, strerror(-rc));
			err_major++;
		}
	}
