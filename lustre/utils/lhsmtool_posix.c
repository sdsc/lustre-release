...
static int ct_parseopts(int argc, char * const *argv)
{
	...
	while ((c = getopt_long(argc, argv, "A:b:c:hiMp:qrv",
		...
		default:
			rc = -EINVAL;
			CT_ERROR(rc, "unrecognized option ");
			return rc;
		}
	}
...
static int ct_save_stripe(int src_fd, const char *src, const char *dst)
{
	...
	if (rc < 0) {
		rc = -errno;
		CT_ERROR(rc, "cannot write %zd bytes in '%s'",
			 xattr_size, lov_file);
		close(fd);
		goto err_cleanup;
	}
			...
			const struct hsm_action_item *hai, long hal_flags)
{
	...
	if (rc < 0) {
		...
		CT_ERROR(rc, "progress for copying '%s'->'%s' failed",
			 src, dst);
		goto out;
	}
	...
	while (wpos < rlen) {
		...
		if (time(0) >= last_print_time + opt.o_report_int) {
			...
			if (rc < 0) {
				/* Action has been canceled or something wrong
				 * is happening. Stop copying data. */
				CT_ERROR(rc, "progress for copying '%s'->'%s'"
					 " failed", src, dst);
				goto out;
			}
		}
...
out:
	...
	if ((hai->hai_action == HSMA_RESTORE) &&
		...
		if (rc < 0) {
			rc = -errno;
			CT_ERROR(rc,
				 "cannot do final truncate to %jd on '%s'",
				 (intmax_t)src_st.st_size, dst);
			err_major++;
		}
	}
			...
			int dst_fd)
{
	...
	if (fchmod(dst_fd, st.st_mode) < 0 ||
		...
		rc = -errno;
		CT_ERROR(rc, "fchmod fchown or futimes on '%s' failed", src);
		return rc;
	}
	...
	while (name < list + list_len) {
		...
		if (!is_restore ||
			...
			if (rc < 0 && errno != EOPNOTSUPP) {
				rc = -errno;
				CT_ERROR(rc, "cannot fsetxattr '%s' on '%s'",
					 name, dst);
				return rc;
			}
		}
...
static int ct_archive(const struct hsm_action_item *hai, const long hal_flags)
{
	...
	if (src_fd < 0) {
		rc = -errno;
		CT_ERROR(rc, "cannot open for read of '%s'", src);
		goto fini_major;
	}
	...
	if (dst_fd == -1) {
		rc = -errno;
		CT_ERROR(rc, "cannot open for write of '%s'", dst);
		goto fini_major;
	}
	...
	if (opt.o_shadow_tree) {
		...
		if (rc < 0) {
			CT_ERROR(rc, "fid2path on '%s' failed", buf);
			rcf = rcf ? rcf : rc;
			goto fini_minor;
		}
...
static int ct_process_item(struct hsm_action_item *hai, const long hal_flags)
{
	...
	if (opt.o_verbose >= LLAPI_MSG_INFO || opt.o_dry_run) {
		...
		if (rc < 0)
			CT_ERROR(rc, "fid2path on '%s' failed", fid);
		else
...
static int ct_import_recurse(const char *relpath)
{
	...
	while (1) {
		...
		if (rc != 0) {
			CT_ERROR(rc, "importing '%s' failed", newpath);
...
static int ct_run(void)
{
	...
	if (opt.o_daemonize) {
		...
		if (rc < 0) {
			rc = -errno;
			CT_ERROR(rc, "cannot start as daemon");
			return rc;
		}
	}
	...
	while (1) {
		...
		if (rc == -ESHUTDOWN) {
			...
			CT_WARN("message receive: (%s)", strerror(-rc));
			err_major++;
...
static int ct_cleanup(void)
{
	...
	if (close(arc_fd) < 0) {
		rc = -errno;
		CT_ERROR(rc, "cannot close backend mount point");
		return rc;
	}
