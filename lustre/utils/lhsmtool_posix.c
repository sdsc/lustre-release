...
#define CT_ERROR(format, ...) \
	llapi_printf(LLAPI_MSG_ERROR, format, ## __VA_ARGS__)
#define CT_DEBUG(format, ...) \
	llapi_printf(LLAPI_MSG_DEBUG, format, ## __VA_ARGS__)
#define CT_WARN(format, ...) \
	llapi_printf(LLAPI_MSG_WARN, format, ## __VA_ARGS__)
#define CT_TRACE(format, ...) \
	llapi_printf(LLAPI_MSG_INFO, format, ## __VA_ARGS__)

void usage(const char *name, int rc)
{
	const char *cmd = basename(name);
...
int ct_parseopts(const int argc, char *const *argv)
{
	...
	while ((c = getopt_long(argc, argv, "A:b:c:hiMp:qruv",
		...
		case 'A':
			if ((opt.o_archive_cnt >= MAX_ARCHIVE_CNT) ||
				...
				CT_ERROR("%s: archive number must be less than "
					 "%d\n",
		...
		case 0:
			break;
		default:
			CT_ERROR("%s: option '%s' unrecognized,"
	...
	case CA_IMPORT:
		/* src dst fsname */
		if (argc != optind + 3) {
			CT_ERROR("%s: --import requires 2 arguments not %d\n",
				 argv[0]);
			usage(argv[0], 1);
		}
	...
	if (argc != optind + 1) {
		CT_ERROR("%s: no fsname specified\n", argv[0]);
		usage(argv[0], 1);
	}
	opt.o_fsname = argv[optind];

	CT_TRACE("%s: action=%d src=%s dst=%s fsname=%s\n", argv[0],
		 opt.o_action, opt.o_src, opt.o_dst,
		 opt.o_fsname);

	if (!opt.o_dryrun && !opt.o_hsmroot) {
		CT_ERROR("%s: no --hsm_root specified\n", argv[0]);
		usage(argv[0], 1);
	}
...
static int get_lov_rules(const char *src, const char *parent,
...
use_default:
	...
	if (rc < 0) {
		CT_ERROR("%s: cannot get stripe info for '%s' (%d)\n",
			 parent, src, rc);
		return rc;
	}
	return 0;
...
static int ct_restore_stripe(const char *src, const char *dst,
			     const int fd_src, const int fd_dst,
			     const char *parent)
{
	int			 rc;
	struct lov_user_md_v3	*lum;
	int			 lum_size, xattr_sz;
	...
	if (rc < 0) {
		CT_ERROR("%s: cannot get stripe rules\n", src);
		goto out;
	}
	...
	if (rc == -1) {
		CT_ERROR("%s: cannot set stripe (%d)\n", dst, strerror(errno));
		rc = -errno;
	}
...
fini_major:
	...
	rc = ct_fini(rootpath, &copy, hai, ct_flags, rc);
	       ...
	       const long hal_flags)
{
	...
	fd_dst = llapi_create_volatile(parent, O_LOV_DELAY_CREATE);
	if (fd_dst < 0) {
	...
	/* build the 2 path names from the 2 FID */
	ct_path_target(src, sizeof(src), opt.o_hsmroot, &hai->hai_fid);
	ct_path_lustre(dst, sizeof(dst), rootpath, &hai->hai_dfid);

	CT_TRACE("%s: restore file to '%s' parent is %s\n", src, dst, parent);
	...
	rc = ct_restore_stripe(src, dst, fd_src, fd_dst, parent);
	if (rc < 0) {
	...
	*sub_seqmax = 0;
	...
	while ((rc = readdir_r(dir, &ent, &cookie)) == 0) {
		...
		if (sscanf(ent.d_name, "%hx", &sub_seq) != 1) {
			CT_TRACE("%s: unexpected dirname format, skip entry.\n",
				 ent.d_name);
			continue;
		}
