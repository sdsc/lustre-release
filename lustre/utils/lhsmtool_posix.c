...
static int ct_save_stripe(int src_fd, const char *src, const char *dst)
{
	...
	lum = (struct lov_user_md *)lov_buff;
	lum->lmm_layout_gen = 0;
	...
	rc = write(fd, lov_buff, xattr_size);
	if (rc < 0) {
			  ...
			  size_t *lum_size)
{
	...
	*lum_size = (size_t)rc;
	...
	if (lum->lmm_magic == LOV_USER_MAGIC_V3) {
		rc = llapi_search_ost(fs_name, lum->lmm_pool_name, NULL);
		if (rc < 0) {
...
static int ct_restore_stripe(const char *src, const char *dst, int dst_fd)
{
	...
	rc = ct_load_stripe(src, lum, &lum_size);
	if (rc) {
	...
	rc = fsetxattr(dst_fd, XATTR_LUSTRE_LOV, lum, lum_size, XATTR_CREATE);
	if (rc < 0) {
...
static int ct_ftruncate_sync(int fd, off_t length)
{
	int	rc;
...
out:
	...
	if ((hai->hai_action == HSMA_RESTORE) &&
		 ...
		 */
		rc = ct_ftruncate_sync(dst_fd, src_st.st_size);
		if (rc < 0) {
			rc = -errno;
			 ...
			 int dst_fd, bool is_restore)
{
	...
	do {
		...
		size = flistxattr(src_fd, names, names_sz);
		if (size < 0) {
			free(names);
			if (errno != ERANGE)
				return -errno;
	...
	while (ptr < names + size) {
		rc = fgetxattr(src_fd, ptr, buf, buf_sz);
		if (rc < 0) {
			rc = -errno;
			goto out;
		}
...
static int ct_begin(struct hsm_copyaction_private **hcp,
		    const struct hsm_action_item *hai)
{
	char	 src[PATH_MAX];
	int	 rc;

	rc = llapi_hsm_action_begin(hcp, opt.o_mnt, hai, false);
	if (rc < 0) {
...
static int ct_fini(struct hsm_copyaction_private **hcp,
		   const struct hsm_action_item *hai, int flags, int ct_rc)
{
	...
	rc = llapi_hsm_action_end(hcp, &hai->hai_extent, flags, abs(ct_rc));
	if (rc == -ECANCELED)
...
static int ct_fid2parent_path(char *parent, size_t parent_len,
...
static int ct_restore(const struct hsm_action_item *hai, const long hal_flags)
{
	...
	rc = ct_fid2parent_path(parent, sizeof(parent), opt.o_mnt,
				&hai->hai_fid);
	if (rc)
		goto fini;
	...
	snprintf(dst, sizeof(dst), "%s/{VOLATILE}="DFID, parent, PFID(&dfid));

	CT_TRACE("'%s' restore data to '%s' parent is '%s'\n", src, dst,
		 parent);
			   ...
			   int errval)
{
	...
	rc = llapi_hsm_action_begin(&hcp, opt.o_mnt, hai, true);
	if (rc)
		return rc;
...
static int ct_setup(void)
{
	...
	if (arc_fd < 0) {
		CT_ERROR("Can't open archive at '%s': %s.\n", opt.o_hsm_root,
			 strerror(errno));
		return -errno;
	}
	...
	if (rc) {
		CT_ERROR("Can't find filesystem mounted at: %s.\n", opt.o_mnt);
		return -rc;
	}
...
static int ct_cleanup(void)
{
	...
	if (close(arc_fd) < 0) {
		CT_ERROR("Can't close archive: %s.\n", strerror(errno));
		return -errno;
	}
