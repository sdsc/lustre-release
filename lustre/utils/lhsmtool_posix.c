...
static int ct_save_stripe(const char *src, const char *dst)
{
	...
	if (rc < 0) {
		CT_ERROR("'%s' cannot write %d bytes (%s)\n",
			 lov_file, lum_size, strerror(errno));
		close(fd);
		rc = -errno;
		goto out;
	}
			     ...
			     const struct lov_user_md_v3 *lum)
{
	...
	rc = fsetxattr(dst_fd, XATTR_LUSTRE_LOV, (void *)lum, xattr_sz,
		       XATTR_CREATE);
...
static int ct_copy_data(const char *mnt, const char *src, const char *dst,
			...
			size_t *file_size)
{
	...
	while (wpos < rlen) {
		...
		if (opt.o_bw != 0) {
			...
			sleep_time = excess * 1000000 / opt.o_bw;
			...
			int dst_fd)
{
	...
	if (fchmod(dst_fd, st.st_mode) == -1 ||
	    fchown(dst_fd, st.st_uid, st.st_gid) == -1 ||
	    futimes(dst_fd, times) == -1)
			 ...
			 int dst_fd, bool restore)
{
	...
	while (ptr < names + size) {
		...
		if ((restore == false) ||
			...
			rc = fsetxattr(dst_fd, ptr, buf, rc, 0);
...
static int ct_begin(const char *mnt, const struct hsm_action_item *hai,
		    struct hsm_copy *copy)
{
	char	 src[PATH_MAX];
	int	 rc;

	ct_path_lustre(src, sizeof(src), mnt, &hai->hai_fid);
	rc = llapi_hsm_copy_start(mnt, copy, hai);
	if (rc < 0)
...
static int ct_fini(const char *mnt, struct hsm_copy *copy,
		   const struct hsm_action_item *hai, int flags, int ct_rc)
{
	...
	ct_path_lustre(lstr, sizeof(lstr), mnt, &hai->hai_fid);
	hp.hp_cookie = hai->hai_cookie;
	...
	hp.hp_flags = flags;
	rc = llapi_hsm_copy_end(mnt, copy, &hp);
	if (rc == -ECANCELED)
...
static int ct_archive(const char *mnt, const struct hsm_action_item *hai,
	       const long hal_flags)
{
	 ...
	 */
	ct_path_lustre(src, sizeof(src), mnt, &hai->hai_dfid);
	ct_path_archive(dst, sizeof(dst), opt.o_hsmroot, &hai->hai_fid);
	...
	rc = ct_copy_data(mnt, src, dst, src_fd, dst_fd,
			  hai, hal_flags, NULL);
	if (rc < 0) {
	...
	if (rename_needed == true) {
		...
		ct_path_archive(src, sizeof(src), opt.o_hsmroot, &hai->hai_fid);
		rc = rename(dst, src);
		if (rc < 0) {
	...
	if (opt.o_shadow) {
		...
		rc = llapi_fid2path(mnt, buf, src + strlen(src),
...
out:
	...
	rc = ct_fini(mnt, &copy, hai, ct_flags, rcf);
	return rc;
}

static int ct_restore(const char *mnt, struct hsm_action_item *hai,
		      const long hal_flags)
{
	struct hsm_copy		 copy;
	char			 src[PATH_MAX];
	...
	rc = llapi_fid2path(mnt, fid, released,
			    sizeof(released), &recno, &linkno);
	if (rc != 0) {
		CT_ERROR(DFID": cannot get path for FID (%s)\n",
			 PFID(&hai->hai_fid), strerror(-rc));
		goto fini;
	}
	/* fid2path returns a relative path */
	sprintf(parent, "%s/%s", mnt, released);
	/* remove file name */
	ptr = strrchr(parent, '/');
	...
	dst_fd = llapi_create_volatile(parent, volatile_flags);
	if (dst_fd < 0) {
		CT_ERROR("'%s' cannot create volatile file (%s)\n",
			 parent, strerror(-dst_fd));
		goto fini;
	}

	if (use_default_stripe == false) {
		rc = ct_restore_stripe(dst, dst_fd, lum);
		if (rc < 0) {
	...
	rc = llapi_fd2fid(dst_fd, &hai->hai_dfid);
	if (rc < 0) {
	...
	if (opt.o_xattr) {
		rc = ct_copy_xattr(src, dst, src_fd, dst_fd, true);
		if (rc < 0) {
...
fini:
	rc = ct_fini(mnt, &copy, hai, flags, rc);
...
static int ct_remove(const char *mnt, const struct hsm_action_item *hai,
		     const long hal_flags)
{
	struct hsm_copy		copy;
	char			dst[PATH_MAX];
...
fini:
	rc = ct_fini(mnt, &copy, hai, 0, rc);
	return rc;
}

static int ct_process_item(const char *mnt, struct hsm_action_item *hai,
			   const long hal_flags)
{
	int			rc = 0;
	...
	if (opt.o_verbose >= LLAPI_MSG_INFO || opt.o_dryrun) {
		...
		rc = llapi_fid2path(mnt, fid, path,
	...
	case HSMA_ARCHIVE:
		rc = ct_archive(mnt, hai, hal_flags);
		break;
	case HSMA_RESTORE:
		rc = ct_restore(mnt, hai, hal_flags);
		break;
	case HSMA_REMOVE:
		rc = ct_remove(mnt, hai, hal_flags);
		break;
	case HSMA_CANCEL:
...
static void *ct_thread(void *data)
{
	...
	rc = ct_process_item(cttd->mnt, cttd->hai, cttd->hal_flags);
...
static int ct_process_item_async(const char *mnt,
...
static int ct_import_one(const char *src, const char *dst)
{
	...
	ct_path_archive(newarc, sizeof(newarc), opt.o_hsmroot, &fid);
...
static int ct_rebind_one(const lustre_fid *old_fid, const lustre_fid *new_fid)
{
	...
	ct_path_archive(src, sizeof(src), opt.o_hsmroot, old_fid);
	ct_path_archive(dst, sizeof(dst), opt.o_hsmroot, new_fid);
...
static int ct_daemon(void)
{
	...
	rc = llapi_hsm_copytool_start(&ctdata, opt.o_mnt, 0, opt.o_archive_cnt,
				      opt.o_archive_id);
	if (rc < 0) {
	...
	while (1) {
		...
		rc = llapi_search_rootpath(mnt, hal->hal_fsname);
		if (rc < 0) {
		...
		if (strcmp(mnt, opt.o_mnt) != 0) {
			CT_ERROR("'%s' invalid root path, expecting: %s\n",
				 mnt, opt.o_mnt);
			opt.err_major++;
			if (opt.o_err_abort)
				break;
		...
		while (++i <= hal->hal_count) {
			...
			rc = ct_process_item_async(mnt, hai,
						   hal->hal_flags);
			if (rc < 0)
