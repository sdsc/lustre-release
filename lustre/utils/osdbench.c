...
struct thread_job {
	struct lu_fid fid;
	struct lu_fid fid2; /* last_rcvd */
	struct lu_fid fid3; /* lov_objids */
	int use_index;
	int thread_id;
};
...
void *mdd_create_mknod(void *arg)
{
	...
	printf("created parent %llu/%u\n",
		dir_fid.f_seq, (unsigned)dir_fid.f_oid);	
	/* insert ./.. into the directory */
	our = obe_new(device);
	snprintf(filename, sizeof(filename), ".");
	direntry.rec_fid = &tj->fid;
...
/* this test simulates mdd_create() for 1-striped files */
void *mdt_create_mknod(void *arg)
{
	struct thread_job *tj = arg;
	struct obe_request *our;
	struct lu_buf lb, lb2, lbv;
	struct lu_attr attr, attr2;
	int i, j, rc, offset, offset2;
	struct lu_fid dir_fid;
	struct dt_insert_rec direntry;
	char filename[128];
	__u64 ver;
	...
	offset2 = tj->thread_id * packsize * 8;
	...
	for (j = 0; j < (total / threads / packsize); j++) {
		...
		for (i = 1; i <= packsize; i++) {
			...
			obe_add_update(our, i, xattr_set, &tj->fid, &lb,
				       XATTR_NAME_LINK, 0);
			obe_add_update(our, i, xattr_set, &tj->fid, &lbv,
			...
			lb2.lb_buf = filename;
			lb2.lb_len = 8;
			obe_add_update(our, i, write, &tj->fid3, &lb2,
				       offset2 + (i * 8));
		}
...
static struct obe_test tests[] = {
	...
	{ "mdt_create_mknod", mdt_create_mknod},
...
int main(int argc, char *argv[])
{
	...
	fid3.f_oid = 2;
	fid3.f_ver = 0;
	rc = obe_create_object(&fid3, S_IFREG, DFT_REGULAR, 0, 0);
	if (rc < 0) {
		fprintf(stderr, "can't create regular object\n");
		exit(-1);
	}
