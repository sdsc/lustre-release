...
int jt_lcfg_listparam(int argc, char **argv)
{
        ...
        for (i = rc; i < argc; i++) {
                ...
                if (strncmp(path, "/proc", strlen("/proc")) == 0) {
                        fprintf(stderr, "list_param: specifying parameters via 
...
int jt_lcfg_getparam(int argc, char **argv)
{
        ...
        for (i = rc; i < argc; i++) {
                 ...
                 * Paths must begin with /proc. */
                if (strncmp(path, "/proc", strlen("/proc")) == 0) {
                        fprintf(stderr, "get_param: specifying parameters via 
                                full paths is deprecated.\n");
                        fp = open(path, O_RDONLY);
#if LUSTRE_VERSION_CODE >= OBD_OCD_VERSION(2,6,50,0)
#warning "remove deprecated full path tunable access"
...
int jt_lcfg_setparam(int argc, char **argv)
{
        ...
        for (i = rc; i < argc; i++) {
                 ...
                 * Paths must begin with /proc. */
                if (strncmp(path, "/proc", strlen("/proc")) == 0) {
                        fprintf(stderr, "set_param: specifying parameters via 
                                full paths is deprecated.\n");
                        fp = open(path, O_RDONLY);
#if LUSTRE_VERSION_CODE >= OBD_OCD_VERSION(2,6,50,0)
#warning "remove deprecated full path tunable access"
