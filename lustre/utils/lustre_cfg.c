...
static char *display_name(char *filename, size_t filename_size, bool po_show_type)
{
	struct stat st;
	char *tmp;
	char *suffix = NULL;
...
static int lprocfs_param_pattern(const char *pattern, char *buf, size_t bufsize)
{
	...
	if (strstr(pattern, "../") || strstr(patternm, "./")) {
		fprintf(stderr, "error: parameter '%s' not allowed\n", pattern);
		return -EINVAL;
	}
