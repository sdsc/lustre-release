	...
	for (i = 0;;i++) {
		...
		if (detail) {
			...
			if (!cYAML_create_number(route, "priority",
						 data.ioc_config_u.route.priority))
				goto fn_exit;

			if (!cYAML_create_string(route, "state",
						 data.ioc_config_u.route.flags ? "up" : "down"))
				goto fn_exit;
		}
	}
			 ...
			 cYAML **show_rc, cYAML **err_rc)
{
	...
	for (i = 0;;i++) {
		...
		if (detail) {
			...
			if (!cYAML_create_number(tunables, "peer_buffer_credits",
						data->ioc_config_u.net.peer_buf_cr))
				goto fn_exit;
					 ...
					 cYAML **err_rc)
{
	...
	do {
		for (i = 0;; i++) {
			...
			rc = l_ioctl(LNET_DEV_ID, IOC_LIBCFS_GET_PEER_INFO, &peer_info);
			if (rc != 0)
				break;
			...
			if (!cYAML_create_string(peer, "nid",
						 libcfs_nid2str(peer_info.ioc_nid)))
				goto fn_exit;
		...
		if (errno != ENOENT) {
			snprintf(err_str,
				LNET_MAX_STR_LEN,
				"IOC_LIBCFS_GET_PEER_INFO failed: %s", strerror(errno));
			rc = -errno;
			goto fn_exit;
		}
				  ...
				  cYAML **err_rc)
{
	...
	if (tunables) {
		...
		peer_buf_cr = cYAML_get_object_item(tunables, "peer_buffer_credits");
	...
	return lustre_lnet_config_net((net) ? net->valuestring : NULL,
				      ...
				      (peer_buf_cr) ? peer_buf_cr->valueint : -1,
				 ...
				 cYAML **show_rc, cYAML **err_rc)
{
	...
	while (cur) {
		...
		if ((rc) || (!cb)) {
			sprintf(err_str, "Failed to process request: %s [%d, %p]",
				cur->string, rc, cb);
