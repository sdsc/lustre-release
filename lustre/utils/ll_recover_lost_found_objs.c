...
static int traverse_lost_found(char *src_dir, const char *mount_path)
{
	...
	struct obd_group_info *grp_info;
		...
		sprintf(seq_name, (fid_seq_is_rsvd(ff_group) ||
			fid_seq_is_mdt0(ff_group)) ?  LPU64 : LPX64i,
			fid_seq_is_idif(ff_group) ? 0 : ff_group);
		...
		sprintf(obj_name, (fid_seq_is_rsvd(parent_fid.ff_seq) ||
			...
			fid_seq_is_idif(parent_fid.ff_seq)) ? LPU64 : LPX64i,
			ff_objid);

		grp_info = find_or_create_grp(&grp_info_list, ff_group,
					      mount_path);
		if (grp_info == NULL) {
			closedir(dir_ptr);
			return 1;
