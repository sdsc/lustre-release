...
static void print_obd_line(char *s)
{
	...
	if (sscanf(s, buf, obd_name) == 0)
		goto try_mdc;
	if (cfs_get_param_path(&path, "osc/%s/ost_conn_uuid",
			       obd_name) != 0)
		goto try_mdc;
...
try_mdc:
	...
	if (sscanf(s, buf, obd_name) == 0)
		goto fail;
	if (cfs_get_param_path(&path, "mdc/%s/mds_conn_uuid",
			       obd_name) != 0)
		goto fail;
	fp = fopen(path.gl_pathv[0], "r");
	if (fp == NULL)
...
int jt_obd_list(int argc, char **argv)
{
	...
	rc = cfs_get_param_path(&path, "devices");
	if (rc != 0)
		return -errno;
                                 ...
                                 char *ostname)
{
        ...
        case LCFG_POOL_ADD: {
                do {
                        rc = llapi_search_ost(fsname, poolname, ostname);
			if (rc == -ENODEV) {
				fprintf(stderr, "llapi_search_ost returned -ENODEV for POOL_ADD\n");
                                return rc;
			}
                        if (rc != 1)
                                sleep(2);
