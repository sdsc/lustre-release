...
char *obdo_print(struct obdo *obd)
{
	...
	snprintf(buf, sizeof(buf), "id: %#llx\ngrp: ""%#llx\natime: %llu\n"
		"mtime: %llu\nctime: %llu\nsize: %llu\nblocks: %llu"
		"\nblksize: %u\nmode: %o\nuid: %d\ngid: %d\nflags: %x\n"
		"misc: %x\nnlink: %d,\nvalid %#llx\n",
...
int jt_obd_create(int argc, char **argv)
{
	...
	ostid_set_seq_echo(&data.ioc_obdo1.o_oi);
		...
		data.ioc_obdo1.o_valid = OBD_MD_FLTYPE | OBD_MD_FLMODE |
			...
			fprintf(stderr, "error: %s: oid not valid #%d:%#llx\n",
...
int jt_obd_test_setattr(int argc, char **argv)
{
	...
	__u64 objid = 3;
        ...
        if (verbose != 0)
		printf("%s: setting %lld attrs (objid %#llx): %s",
	...
	ostid_set_seq_echo(&data.ioc_obdo1.o_oi);
        ...
        if (!rc) {
                ...
                --i;
                if (verbose != 0)
			printf("%s: %lld attrs in %.3fs (%.3f attr/s): %s",
...
int jt_obd_destroy(int argc, char **argv)
{
	...
	ostid_set_seq_echo(&data.ioc_obdo1.o_oi);
		...
		data.ioc_obdo1.o_valid = OBD_MD_FLID | OBD_MD_FLMODE;
                ...
                shmem_bump(1);
                if (rc < 0) {
			fprintf(stderr, "error: %s: objid %#llx: %s\n",
...
int jt_obd_test_getattr(int argc, char **argv)
{
	...
	__u64 objid = 3;
        ...
        next_time.tv_usec = start.tv_usec;
        if (verbose != 0)
		printf("%s: getting %lld attrs (objid %#llx): %s",
                       jt_cmdname(argv[0]), count, objid, ctime(&start.tv_sec));
...
int jt_obd_test_brw(int argc, char **argv)
{
	...
	data.ioc_offset = (repeat_offset ? 0 : thr_offset);
        ...
        if (verbose != 0)
		printf("%s: %s %llux%d pages (obj %#llx, off %llu): %s",
        ...
        for (i = 1, next_count = verbose; i <= count && shmem_running(); i++) {
			...
			shmem_lock ();
			printf("%s: %s number %d @ %lld:%llu for %d\n",
...
repeat:
		...
		printf("default_stripe_size: %llu\n",
                       desc.ld_default_stripe_size);
		printf("default_stripe_offset: %llu\n",
