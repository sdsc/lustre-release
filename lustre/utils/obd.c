...
/**
 * clear the fileset of a nodemap
 *
 * \param	argc		number of args
 * \param	argv[]		variable string arguments
 *
...
int jt_nodemap_clear_fileset(int argc, char **argv)
{
	...
	if (nodemap_name == NULL) {
		fprintf(stderr, "usage: nodemap_clear_fileset --name <name>\n");
		return -1;
	}

	rc = nodemap_cmd(LCFG_NODEMAP_CLEAR_FILESET, NULL, 0, argv[0],
			 nodemap_name, NULL);
	if (rc != 0) {
		errno = -rc;
		fprintf(stderr, "error: %s: cannot clear fileset of nodemap "
				"'%s': rc = %d\n",
			jt_cmdname(argv[0]), nodemap_name, rc);
	}
