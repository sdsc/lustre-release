...
int jt_obd_md_common(int argc, char **argv, int cmd)
{
        ...
        while (shmem_running()) {
                ...
                data.ioc_obdo2.o_oi.oi_id = child_base_id;
			...
			fid_ostid_pack(&fid, &data.ioc_obdo1.o_oi);
                }
...
int jt_obd_create(int argc, char **argv)
{
	...
	if (argc < 5) {
        ...
        for (i = 1, next_count = verbose; i <= count && shmem_running(); i++) {
                ...
                data.ioc_obdo1.o_oi.oi_id = base_id;
		...
		if (be_verbose(verbose, &next_time, i, &next_count, count))
			printf("%s: #%d is object id "DOSTID"\n",
				jt_cmdname(argv[0]), i,
				POSTID(&data.ioc_obdo1.o_oi));
        }
        return rc;
}
