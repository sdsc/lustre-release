...
int ldiskfs_make_lustre(struct mkfs_opts *mop)
{
	...
	if ((mop->mo_ldd.ldd_mount_type == LDD_MT_EXT3) ||
		 ...
		 *	LMA size: 24(lustre_mdt_attrs) + 16(xattr_entry) +
		 *		  11(trusted.lma)
		 *	link EA size (MDT): 24(link_ea_header) + (filename) +
		 *			    18(link_ea_entry) +
		 *			    16(xattr_entry) + 12(trusted.link)
		 *	and some margin for alignement, ACLs and other EAs.
		 *
		 * If we say the average filename length is about 32 bytes,
		 * the calculation looks like:
		 * 156 + (32+32+24*N) + (24+27) + (24+18+32~+28) < 512*2^m
		 * {m=0,1,2,3}
		 */
