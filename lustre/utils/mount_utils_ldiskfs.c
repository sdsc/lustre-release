...
static int mkdir_extend_dir_to_htree(char *dir)
{
...
static int __l_mkdir(char * filepnm, int mode , struct mkfs_opts *mop)
{
        ...
        if (mop->mo_ldd.ldd_flags & LDD_F_IAM_DIR)
                return mkdir_extend_dir_to_htree(filepnm);
        return 0;
}

/* Write the server config as binary on-disk files in the filesystem */
 ...
 */
static int __ldiskfs_set_lustre_tunables(char *source, struct mount_opts *mop,
                                         int fan_out)
{
        glob_t glob_info = { 0 };
...
set_params:
        ...
        if (fan_out) {
                ...
                for (i = 0; rc == 0 && i < glob_info.gl_pathc; i++){
                        ...
                        rc = __ldiskfs_set_lustre_tunables(real_path, mop, 0);
                }
...
int ldiskfs_set_lustre_tunables(char *dev, struct mount_opts *mop)
{
        return __ldiskfs_set_lustre_tunables(dev, mop, 1);
}
