...
static inline int llapi_stripe_size_is_valid(int64_t size)
{
	return size >= 0 && (size & (LOV_MIN_STRIPE_SIZE - 1)) == 0;
}

static inline int llapi_stripe_size_is_too_big(int64_t size)
{
	return size >= (1ULL << 32);
}

static inline int llapi_stripe_count_is_valid(int64_t count)
{
	return count >= -1 && count <= LOV_MAX_STRIPE_COUNT;
}

static inline int llapi_stripe_index_is_valid(int64_t index)
{
