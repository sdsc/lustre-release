...
/*
 * cal_total_bytes: calculates total bytes that need to be
 * written into or read from the filesystem.
 */
static unsigned long long cal_total_bytes(const char *op)
{
		     ...
		     time_t time_st, unsigned long dir_num)
{
	...
	total_bytes = cal_total_bytes("write");
	if (total_bytes <= 0) {
		    ...
		    time_t time_st, unsigned long dir_num)
{
	...
	total_bytes = cal_total_bytes("read");
	if (total_bytes <= 0) {
		fprintf(stderr, "\n%s: unable to calculate total bytes\n",
			progname);
		return 1;
