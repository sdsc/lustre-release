			 ...
			 const char *param, const char *value)
{
	...
	if (options[0] != '\0') {
		avail_len = options_len - strlen(options) - 1;
		strlcat(options, ",", 1);
	}
	avail_len = options_len - strlen(options) - 1;
	strlcat(options, param, avail_len);
	avail_len = options_len - strlen(options) - 1;
	if (value)
		strlcat(options, value, avail_len);
	return 0;
}
...
int main(int argc, char *const argv[])
{
	...
	rc = append_option(options, g_pagesize, "device=", mop.mo_source);
	if (rc != 0)
		goto out_osd;
