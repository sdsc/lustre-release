...
static int append_option(char *options, const char *param,
			 const char *value, size_t options_len)
{
	...
	*flagp = 0;
	...
	while ((opt = strsep(&nextopt, ","))) {
		...
		if (val && strncmp(arg, "md_stripe_cache_size", 20) == 0) {
			...
			rc = append_option(options, opt, NULL, options_len);
			if (rc != 0)
		       ...
		       const char *params, size_t options_len)
{
	...
	while ((ptr = strstr(ptr, PARAM_MGSNODE)) != NULL) {
		...
		rc = append_option(options, ptr, NULL, options_len);
		if (rc != 0)
		     ...
		     char *options, size_t options_len)
{
	...
	rc = append_option(options, ldd->ldd_mount_opts, NULL, options_len);
	if (rc != 0)
		goto out_parse_ldd;
	...
	if (!mop->mo_have_mgsnid) {
		...
		if (ldd->ldd_flags & LDD_F_SV_TYPE_MGS) {
			rc = append_option(options, "mgs", NULL, options_len);
			if (rc != 0)
				goto out_parse_ldd;
			mop->mo_have_mgsnid++;
		} else {
	...
	if (ldd->ldd_flags & LDD_F_VIRGIN) {
		rc = append_option(options, "virgin", NULL, options_len);
		if (rc != 0)
			goto out_parse_ldd;
	}
	if (ldd->ldd_flags & LDD_F_UPDATE) {
		rc = append_option(options, "update", NULL, options_len);
		if (rc != 0)
			goto out_parse_ldd;
	}
	if (ldd->ldd_flags & LDD_F_WRITECONF) {
		rc = append_option(options, "writeconf", NULL, options_len);
		if (rc != 0)
			goto out_parse_ldd;
	}
	if (ldd->ldd_flags & LDD_F_NO_PRIMNODE) {
		rc = append_option(options, "noprimnode", NULL, options_len);
		if (rc != 0)
			goto out_parse_ldd;
	}
	...
	while (start && *start != '\0') {
		...
		rc = append_option(options, "param=", cur, options_len);
		if (rc != 0)
			goto out_parse_ldd;
	}

	/* svname must be last option */
	rc = append_option(options, "svname=", ldd->ldd_svname, options_len);
	if (rc != 0)
		goto out_parse_ldd;
...
int main(int argc, char *const argv[])
{
	...
	rc = parse_options(&mop, options, &flags, g_pagesize);
        if (rc) {
	...
	if (!client) {
		...
		rc = parse_ldd(mop.mo_source, &mop, options, g_pagesize);
		if (rc)
			goto out_mo_source;
	}
	...
	rc = append_option(options, "device=", mop.mo_source, g_pagesize);
	if (rc != 0)
		goto out_osd;
