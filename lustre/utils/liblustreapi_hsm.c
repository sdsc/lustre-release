...
static int create_restore_volatile(struct hsm_copyaction_private *hcp)
{
	...
	rc = fid_parent(hcp->ct_priv->mnt, &hai->hai_fid, parent,
			sizeof(parent));
	if (rc)
		return rc;
 ...
 * \param ct_priv  Copytool handle acquired at registration.
			   ...
			   const struct hsm_action_item *hai, bool is_error)
{
	...
	if (hai->hai_action == HSMA_RESTORE) {
		rc = create_restore_volatile(priv);
		if (rc)
			goto err_out;
	}

	rc = ioctl(ct_priv->mnt_fd, LL_IOC_HSM_COPY_START, &priv->copy);
	if (rc) {
		rc = -errno;
		goto err_out;
	}
			 ...
			 const struct hsm_extent *he, int flags, int errval)
{
	...
	rc = ioctl(priv->ct_priv->mnt_fd, LL_IOC_HSM_COPY_END, &priv->copy);
	if (rc) {
		rc = -errno;
		goto err_cleanup;
	}
