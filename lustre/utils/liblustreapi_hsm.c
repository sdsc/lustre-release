...
int write_json_event(struct llapi_json_item_list **event)
{
	...
	if (rc < 0) {
		...
		if (rc == -EPIPE || rc == -EAGAIN) {
			fprintf(stderr, "Write to event fd failed: no reader\n");
		} else {
			fprintf(stderr, "Write to event fd failed: %s\n",
				strerror(abs(rc)));
			return rc;
		}
	}
		    ...
		    __u64 total, __u64 current)
{
	...
	if (llapi_json_add_item(&json_items, "data_fid", LLAPI_JSON_STRING, strfid))
		goto err;

	snprintf(strfid, sizeof(strfid), DFID_NOBRACE, PFID(&hai->hai_fid));
	if (llapi_json_add_item(&json_items, "source_fid", LLAPI_JSON_STRING, strfid))
		goto err;
