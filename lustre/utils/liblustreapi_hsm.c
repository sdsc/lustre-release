...
err:
	rc = -1;
	llapi_error(LLAPI_MSG_ERROR, errno, "error in "
		    ...
		    __u64 total, __u64 current)
{
	...
	if (llapi_json_add_item(&json_items, "data_fid",
				LLAPI_JSON_STRING, strfid))
		goto err;

	snprintf(strfid, sizeof(strfid), DFID_NOBRACE, PFID(&hai->hai_fid));
	if (llapi_json_add_item(&json_items, "source_fid",
				LLAPI_JSON_STRING, strfid))
		goto err;
...
err:
	...
	fprintf(stderr, "\nFATAL ERROR IN llapi_hsm_log_error: rc %d,"
			 "errno %d\n", rc, errno);
