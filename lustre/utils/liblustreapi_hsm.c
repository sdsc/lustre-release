...
int log_ct_registration(struct hsm_copytool_private **priv,
			ct_event_type_t event_type)
{
	int				rc;
	...
	if (llapi_hsm_get_agent_uuid(ct->mnt, agent_uuid, sizeof(agent_uuid)))
		goto err;
...
int log_ct_progress(struct hsm_copyaction_private **phcp,
		    ...
		    ct_progress_type_t progress_type,
