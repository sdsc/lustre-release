...
static int ct_mdc_getinfo(const struct hsm_copytool_private *ct,
			  ...
			  lstat_t *st)
{
	...
	if (lmd_size < XATTR_SIZE_MAX)
		lmd_size = XATTR_SIZE_MAX;
	...
	snprintf((void *)lmd, lmd_size, DFID_NOBRACE, PFID(fid));
			   ...
			   bool is_error)
{
	...
	if (hai->hai_action == HSMA_RESTORE) {
		rc = ct_mdc_getinfo(hcp->ct_priv, &hai->hai_fid, &hcp->stat);
		if (rc < 0)
			goto err_out;
