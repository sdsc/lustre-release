...
uint32_t sk_gen_params(struct sk_cred *skc, const bool initiator)
{
	...
	skc->sc_kctx.skc_host_random = cpu_to_be32(random);
 ...
 * contents.
 *
 * If the derviced key size is greater than the HMAC algorithm size it will be
 ...
 *
 * For ski mode:
 * Session HMAC Key = HMAC("Integrity", KDF derived Session Key)
 *
 * For skpi mode:
 * Session HMAC Key = HMAC("Integrity", KDF derived Session Key)
 * Session Encryption Key = HMAC("Encrypt", KDF derived Session Key)
 *
 * \param[in,out]	skc		Shared key credentials structure with
 *
 * \return	-1		failure
...
int sk_compute_keys(struct sk_cred *skc)
{
	...
	bufs[0].value = integrity;
	bufs[0].length = strlen(integrity);
	rc = sk_sign_bufs(&kctx->skc_session_key, bufs, 1,
	...
	/* The encryption key may be a different size than the hash algorithm
	 * so we use the key derivation function leaving room for the counter
	 * in array element 0 */
	bufs[0].value = NULL;
	bufs[0].length = 0;
	bufs[1].value = encrypt;
	bufs[1].length = strlen(encrypt);

	kctx->skc_encrypt_key.length =
		sk_crypt_types[kctx->skc_crypt_alg].sct_bytes;
	kctx->skc_encrypt_key.value = malloc(kctx->skc_encrypt_key.length);
	if (!kctx->skc_encrypt_key.value)
		return -ENOMEM;

	return sk_kdf(&kctx->skc_encrypt_key, &kctx->skc_session_key, bufs,
		      2, kctx->skc_hmac_alg);
}
