...
void sk_config_cpu_to_disk(struct sk_keyfile_config *config)
{
	...
	config->skc_p_len = htobe32(config->skc_p_len);
...
void sk_config_disk_to_cpu(struct sk_keyfile_config *config)
{
	...
	config->skc_p_len = be32toh(config->skc_p_len);
 ...
 * \param[in]	p_len		Sizeof of p in bytes from key
 *
			       ...
			       const uint32_t flags, int *p_len)
{
	...
	if (p_len)
		*p_len = config->skc_p_len / 8;
	skc->sc_p.length = config->skc_p_len / 8;
