 ...
 * \return	-1	failure
 */
static key_serial_t sk_load_key(const void *payload, const int psize,
				const char *description)
{
	...
	key = add_key("user", description, payload, psize,
...
int sk_load_keyfile(char *path, enum sk_key_type type)
{
	...
	case (SK_TYPE_MGS):
		...
		if (rc >= SK_DESCRIPTION_SIZE)
			goto out;
		if (sk_load_key(config, sizeof(*config), description) == -1)
			goto out;
		break;
	case (SK_TYPE_SERVER):
		...
		/* Server must also load client key so fall through */
	case (SK_TYPE_CLIENT):
		...
		if (config->skc_fsname) {
			...
			if (rc >= SK_DESCRIPTION_SIZE)
				goto out;
			if (sk_load_key(config, sizeof(*config),
					description) == -1)
				goto out;
		}
		...
		break;
	default:
		printerr(0, "Invalid key type, not loading key\n");
		goto out;
		break;
	}
...
int sk_validate_config(const struct sk_keyfile_config *config)
{
	...
	if (config->skc_expire < 60 || config->skc_expire > INT_MAX) {
		 ...
		 * also prevent values over INT_MAX because where appears
	...
	if (config->skc_session_keylen % 8 != 0 ||
	    config->skc_session_keylen > SK_SESSION_MAX_KEYLEN * 8) {
		printerr(0, "Invalid session key length must be a multiple of 8"
			 " and less then %d bits\n", SK_SESSION_MAX_KEYLEN * 8);
		return -1;
	}
	if (config->skc_shared_keylen % 8 != 0 ||
	    config->skc_shared_keylen > SK_MAX_KEYLEN * 8){
 ...
 * at \a hash
 *
 * \param[in]		string	Null terminated string to hash using SHA256
 * \param[in,out]	hash	gss_buffer_desc to hold the result
 *
 * \return	-1	failure
 * \return	0	success
 */
static int sk_hash_string(const char *string, gss_buffer_desc *hash)
{
	...
	if (!hash->value || hash->length < EVP_MD_size(md))
		goto out_err;
 ...
 */
uint32_t sk_verify_hash(const char *string, const gss_buffer_desc *current_hash)
{
			       ...
			       const uint32_t flags)
{
	...
	skc->sc_nodemap_hash.length = SK_HASH_LEN;
	...
	if (sk_hash_string(config->skc_nodemap, &skc->sc_nodemap_hash)) {
...
static uint32_t sk_gen_initiator_params(struct sk_cred *skc)
{
	...
	skc->sc_params = DH_generate_parameters(skc->sc_session_keylen,
...
{
	/* Only generate parameters when session_key is required.  These should
	 * already be zeroed by sk_create_cred but just in case they were
	 * altered make sure they are empty */
 ...
 * Only SHA256 is supported here based on the HLD.
 *
...
{
	HMAC_CTX hctx;
	void *key = skc->sc_kctx.skc_shared_key.value;
	long keylen = skc->sc_kctx.skc_shared_key.length;
	unsigned int hashlen = SK_HASH_LEN;
	int i;
	int rc = -1;
	...
	if (HMAC_Init_ex(&hctx, key, keylen, EVP_sha256(), NULL) != 1) {
		   ...
		   const int numbufs, gss_buffer_desc *hmac)
{
	...
	if (sk_sign_bufs(skc, bufs, numbufs, &bufs_hmac)) {
...
{
	gss_buffer_desc *session = &skc->sc_kctx.skc_session_key;
	...
	session->length = DH_size(skc->sc_params);
	session->value = malloc(session->length);
	if (!session->value) {
		printerr(0, "Failed to allocate memory for computed session "
			 "key\n");
		goto out_err;
	}

	status = DH_compute_key(session->value, remote_pub_key, skc->sc_params);
	if (status == -1) {
		printerr(0, "DH_compute_key() failed: %s\n",
			 ERR_error_string(ERR_get_error(), NULL));
		goto out_err;
	} else if (status < session->length) {
		printerr(0, "DH_compute_key() returned a short key of %d "
			 "bytes, expected: %lu\n", status, session->length);
...
int sk_serialize_kctx(struct sk_cred *skc, gss_buffer_desc *ctx_token)
{
	...
	printerr(2, "Serialized buffer of %lu bytes for kernel\n", bufsize);
...
int sk_decode_netstring(gss_buffer_desc *bufs, int numbufs, gss_buffer_desc *ns)
{
	...
	printerr(2, "Decoded netstring of %lu bytes\n", ns->length);
	return i;
			...
			gss_buffer_desc *ns)
{
	...
	for (i = 0; i < numbufs; i++) {
		/* size */
		rc = snprintf((char *) ptr, size, "%lu:", bufs[i].length);
		ptr += rc;
	...
	printerr(2, "Encoded netstring of %lu bytes\n", ns->length);
	return 0;
}
