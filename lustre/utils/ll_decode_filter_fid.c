	...
	for (i = 1; i < argc; i++) {
		...
		if (size > sizeof(struct filter_fid_old)) {
			...
			printf("%s: objid=%llu seq=%llu parent="DFID
			       " stripe=%u\n", argv[i],
			       (unsigned long long)le64toh(ffo->ff_objid),
			       (unsigned long long)le64toh(ffo->ff_seq),
			       (unsigned long long)le64toh(ffo->ff_parent.f_seq),
			...
			printf("%s: parent="DFID" stripe=%u\n", argv[i],
			       (unsigned long long)le64toh(ff->ff_parent.f_seq),
			       le32toh(ff->ff_parent.f_oid), 0, /* ver */
			       /* this is stripe_nr actually */
			       le32toh(ff->ff_parent.f_stripe_idx));
		}
	}
