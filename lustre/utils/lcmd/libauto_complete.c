 ...
 */
callback_ll_t *loaded_cbs = NULL;
cYAML *g_usr_cmds = NULL;
...
{
	callback_ll_t *new_node, *cur;

	cur = loaded_cbs;

	new_node = calloc(sizeof(char),
			  sizeof(callback_ll_t));
	if (!new_node)
		return -1;
...
static e_match_rc_t yaml_get_help_info(cYAML **last, char **cmd, char **help, const char *text)
{
	cYAML *cj = *last;
	...
	if ((cj->string) &&
		(strncmp(cj->string, "lnet_cmd_value", strlen("lnet_cmd_value")) == 0)) {
...
static e_match_rc_t yaml_find_obj(char *token, cYAML *obj, cYAML **ret,
...
static e_match_rc_t yaml_match(char *token, cYAML_ll_t **start, cmd_cb_t *cb,
			       char **cmd_match, int *entire_word_matched)
{
	cYAML *obj = (*start)->obj;
	cYAML *found = NULL;
	cYAML *value = NULL, *back = NULL;
	e_match_rc_t rc = EN_MATCH_NO_MATCH;
	cYAML_ll_t *cjl = NULL;
	...
	*cmd_match = NULL;
	...
	if ((rc == EN_MATCH_NO_MATCH) || (strlen(token) == 0)) {
		...
		if (back) {
			...
			if (strlen(token) != 0) {
				rc = yaml_find_obj(token, cjl->obj->child, &found,
						   entire_word_matched);
...
char *cmd_generator(const char *text, int state)
{
	char **tokens;
	int i = 0;
	e_match_rc_t rc = EN_MATCH_MULTI_MATCH;
	cYAML_ll_t *ll = NULL;
	static cYAML *last = NULL;
	char *cmd = NULL;
	char *help = NULL;
	...
	if (!state) {
		...
		if (tokens) {
			/* feed the text word for word into the YAML matcher function.
			* Feed NULL to indicate end of matching */
			while (tokens[i] != NULL) {
				/* The YAML matcher function will return cYAML struct, which can
				* be passed to a function to print out the help info */
			...
			if ((rc != EN_MATCH_MULTI_MATCH) && (strlen(text) == 0)) {
				rc = yaml_match((char*)text, &ll, NULL, &cmd, &entire_word_matched);
				if (rc == EN_MATCH_ERROR)
					goto function_exit;
			}
		...
		if ((cmd) && (rc == EN_MATCH_MATCH) && !entire_word_matched && (rl_completion_invoking_key == TAB)) {
	...
	if (last) {
		...
		while (last && (rc = yaml_get_help_info(&last, &cmd, &help, text)) == EN_MATCH_NO_MATCH) {
		}
...
int parse_and_callback_tokenized(char **tokens, cYAML **err_rc)
{
	...
	if (tokens) {
		/* feed the text word for word into the YAML matcher function. */
		while (tokens[i] != NULL) {
			/* The assumption is that there is only one callback per command */
			rc = yaml_match(tokens[i], &ll, &cb,
					&cmd, NULL);
			if ((rc == EN_MATCH_ERROR) || (rc == EN_MATCH_NO_MATCH)) {
				/* if at anytime we get a NO_MATCH, then parsing has failed */
				success = -1;
				...
				if (!kvl_node) {
					success = -1;
					cYAML_build_error(success, -1, "lcmd", "system",
							  "Out Of Memory", err_rc);
					goto function_exit;
				}
...
void cmd_display(char **matches, int num_matches, int max_length)
{
	...
	if (rl_completion_invoking_key == LIST) {
		...
		for (i = 0; i < num_matches+1; i++) {
			...
			fprintf(rl_outstream, LIST_FORMAT"\n", cmd, width, "", help);
			free(cmd);
			free(help);
		}
	} else
