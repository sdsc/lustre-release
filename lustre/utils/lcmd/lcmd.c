...
int main(int argc, char **argv)
{
	...
	if (argc > 1) {
		while ((cOpt = getopt_long(argc, argv, short_options,
			...
			case 'c':
				...
				r = optind-2-i;
				remove_cmd_line_entry(cmd_line, r, cmd_line_size--);
				i = argc - cmd_line_size;
				r = optind-1-i;
				remove_cmd_line_entry(cmd_line, r, cmd_line_size--);
				break;
	...
	if (init_auto_complete(lustre_cmds, cmd_lib ? cmd_lib : LUSTRE_CMD_LIB)) {
		CYAML_BUILD_PRINT_ERROR(-1, -1, "build", "auto_complete",
					"Failed to initialize auto-complete library",
					&yaml_err);
		return -1;
	}
	...
	for(;;) {
		...
		if (strcmp(input, "reload cmd lib") == 0) {
			if (init_auto_complete(lustre_cmds, cmd_lib ? cmd_lib : LUSTRE_CMD_LIB)) {
				fprintf(stderr, "Failed to initialize auto-complete library\n");
				break;
			}
		}
