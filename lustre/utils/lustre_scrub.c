...
static void usage_start(void)
{
        fprintf(stderr, "Start OI Scrub.\n"
                ...
                "-h: Help information.\n"
                "-a: async commit for OI file updating, 'yes'|'no'(default).\n"
...
static inline char *scrub_ctime(__u64 src)
{
...
scrub_info_to_str(char *str, struct scrub_header *header, int size)
{
        ...
        rc = snprintf(str, size, "Information:\nversion: %d\nkeysize: %d\n"
                      "success count: %u\n"
                      "last completed time: %s"
                      "initial start time: %s"
                      "latest start time: %s",
                      header->sh_version, header->sh_keysize,
                      header->sh_success_count,
                      scrub_ctime(header->sh_time_last_complete),
                      scrub_ctime(header->sh_time_initial_start),
                      scrub_ctime(header->sh_time_latest_start));
        if (rc <= 0)
                return -ENOSPC;
        size -= rc;
        str  += rc;
        ...
        if (ninfo->sin_time_last_checkpoint == 0)
                goto dryrun;
        ...
        rc = snprintf(str, size, "\nStatistics for normal mode:\n"
                      ...
                      scrub_ctime(ninfo->sin_time_last_checkpoint), key_str1,
                      ninfo->sin_items_scanned, ninfo->sin_items_updated,
                      ninfo->sin_items_failed, ninfo->sin_time,
        ...
        if (header->sh_current_status == SS_REBUILDING ||
            header->sh_current_status == SS_DRURUN_SACNNING) {
                rc = snprintf(str, size, "real-time speed: %u objs/sec\n",
                              ninfo->sin_speed);
                if (rc <= 0)
                        return -ENOSPC;
                size -= rc;
...
dryrun:
        if (dinfo->sid_time_last_checkpoint == 0)
                goto out;
        ...
        rc = snprintf(str, size, "\nStatistics for dryrun mode:\n"
                      ...
                      "run time: %u sec\n"
                      "average speed: "LPU64" objs/sec\n",
                      scrub_ctime(dinfo->sid_time_last_checkpoint), key_str1,
                      key_str2, dinfo->sid_items_scanned,
                      dinfo->sid_items_unmatched, dinfo->sid_items_failed,
                      dinfo->sid_items_updated_prior, dinfo->sid_time,
...
int jt_scrub_start(int argc, char *argv[])
{
        ...
        char *optstring = "M:ha:e:i:m:n:rs:w:";
        ...
        while ((opt = getopt_long(argc, argv, optstring, long_opt_start,
                ...
                case 'a':
                        if (strcmp(optarg, "yes") == 0) {
                                ...
                                fprintf(stderr, "Invalid async commit option: "
                                        "%s. The valid ones are: 'yes'|'no'"
                                        "(default).\n", optarg);
                                return -EINVAL;
                        }
                        ssp.ssp_valid |= SSP_ASYNC;
                        break;
                case 'e':
...
int jt_scrub_show(int argc, char *argv[])
{
        ...
        printf("Show OI Scrub on the MDT device %s:\nCurrent Status: %s\n",
               device, scrub_status_names[header->sh_current_status] != NULL ?
