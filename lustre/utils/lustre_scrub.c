...
static void usage_start(void)
{
        fprintf(stderr, "Start Lustre Scrub.\n"
                ...
                SP_CHECKPOING_INTERVAL_MAX, SP_CHECKPOING_INTERVAL_DEF,
                SP_SPEED_NO_LIMIT);
}
...
int jt_scrub_start(int argc, char **argv)
{
        ...
        start.ss_sponsor = SCRUB_TRIGGERED_BY_COMMAND;
        ...
        while ((opt = getopt_long(argc, argv, optstring, long_opt_start,
                ...
                case 'i':
                        val = atoi(optarg);
                        if (val < 1 || val > SP_CHECKPOING_INTERVAL_MAX) {
                                fprintf(stderr,
                                        "Invalid checkpoint interval %d. "
                                        "The valid value should be [1 - %d].\n",
                                        val, SP_CHECKPOING_INTERVAL_MAX);
                                return -EINVAL;
                        }
                        start.ss_checkpoint_interval = val;
                        start.ss_valid |= SSV_CHECKPOING_INTERVAL;
                        break;
                case 'm':
        ...
        printf("Started Lustre Scrub on the MDT device %s:", device);
        if (start.ss_active == 0) {
                printf(" NONE");
        } else {
                for (i = 0; i < 3; i++) {
                        if (start.ss_active & scrub_types_names[i].type) {
                                printf(" %s", scrub_types_names[i].name);
                                start.ss_active &= ~scrub_types_names[i].type;
                        }
                }
                if (start.ss_active != 0)
                        printf(" unknown(0x%x)", start.ss_active);
        }
        printf("\n");
        return 0;
}
...
int jt_scrub_stop(int argc, char **argv)
{
        ...
        printf("Stopped Lustre Scrub on the MDT device %s.\n", device);
        return 0;
}
