...
static int find_target_obdpath(char *fsname, char *path)
{
	...
	rc = cfs_get_procpath(pattern, PATH_MAX,
                     ...
                     unsigned int result_size)
{
	...
	rc = cfs_get_procpath(file, PATH_MAX, "{lnet,lustre}/%s",
                 param_path);
	if (rc != 0)
		return -ENOENT;
                            ...
                            char *value, unsigned int val_len)
{
	...
	rc = cfs_get_procpath(buf, PATH_MAX, "lustre/devices");
	if (rc != 0)
		return -ENOENT;
...
static int poolpath(char *fsname, char *pathname, char *pool_pathname)
{
	...
	rc = cfs_get_procpath(pattern, PATH_MAX, "lustre/lov/%s-*/pools",
				fsname);
	if (rc != 0)
		return -ENOENT;
...
int llapi_ping(char *obd_type, char *obd_name)
{
	...
	rc = cfs_get_procpath(path, MAX_STRING_SIZE, "lustre/%s/%s/ping",
                 obd_type, obd_name);
	if (rc != 0)
		return -ENOENT;
                         ...
                         void *args, llapi_cb_t cb)
{
	...
	rc = cfs_get_procpath(name, PATH_MAX, "lustre/devices");
	if (rc != 0)
		return -ENOENT;
