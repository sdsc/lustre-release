                            ...
                            char *value, unsigned int val_len)
{
	...
	rc = llapi_get_param(&list, &count, "devices");
	if (rc != 0) goto out;
	...
	rc = llapi_get_param(&list, &count, "%s.%s.%s", obd_type, dev,
			     param_name);
	if (rc == 0)
		strncpy(value, list[count - 1], val_len);
out:
	if (list != NULL)
		free(list);
	return rc;
			 ...
			 const char *param, char *buf, size_t buf_size)
{
	...
	rc = llapi_get_param(&list, &count, "%s.%s.%s", type, inst, param);
	if ((rc == 0) && (count > 0) && (list[0] != NULL))
		strncpy(buf, list[0], strlen(list[0]));
...
static int get_mds_md_size(const char *path)
{
	...
	/* Get the max ea size from llite /proc or /sys. */
...
int llapi_search_ost(char *fsname, char *poolname, char *ostname)
{
	...
	if (poolname == NULL) {
		if (len != 0) {
			rc = llapi_get_param(&list, &count,
					     "lov.%s-*.target_obd", fsname);
			if((rc != 0) && (rc != -ENODEV)) rc = -EINVAL;
		} else {
			rc = -EINVAL;
		}
	} else {
		rc = llapi_get_param(&list, &count, "lov.%s-*.pools.%s", fsname,
				     poolname);
		if (rc != 0) {
			if (rc == -ENODEV) {
				if (llapi_get_param(&list, &count,
						    "lov.%s-*.target_obd",
						    fsname) == -ENODEV)
					rc = -ENODEV;
				else
                          ...
                          int list_size, char *buffer, int buffer_size)
{
	...
	strlcpy(fsname, poolname, sizeof(fsname));
        ...
        *pool = '\0';
        ...
        if (fsname == NULL) {
                rc = llapi_search_fsname(pathname, fsname);
                if (rc != 0)
                        return rc;
        }

        llapi_printf(LLAPI_MSG_NORMAL, "Pool: %s.%s\n", fsname, pool);
	rc = snprintf(path, sizeof(path), "lov.%s-*.pools.%s", fsname, pool);

	if (rc >= sizeof(path))
		return -EOVERFLOW;

	rc = llapi_get_param(&list, &count, path);
        if (rc != 0) {
	...
	for (i = 0 ; i < count; i++) {
                ...
                if (used + strlen(list[i]) + 1 > buffer_size) {
                        rc = -EOVERFLOW;
                        break;
                }

                strcpy(buffer + used, list[i]);
                members[nb_entries] = buffer + used;
                used += strlen(list[i]) + 1;
                nb_entries++;
                rc = nb_entries;
        }
...
{
        char fsname[PATH_MAX + 1], rname[PATH_MAX + 1];
	char buf[PATH_MAX + 1], pathname[PATH_MAX + 1];
        char *ptr;
        DIR *dir;
        struct dirent pool;
		...
		snprintf(pathname, sizeof(pathname), "lov.%s-*.pools",
			      buf);
		...
		if (fsname == NULL) {
			rc = llapi_search_fsname(pathname, buf);
			if (rc != 0)
				return rc;
		} else {
			strcpy(buf, fsname);
	...
	rc = get_proc_path(buf, strlen(buf), pathname);
	if (rc != 0) {
                return rc;
        }
                                  ...
                                  int *ost_count, enum tgt_type type)
{
	...
	rc = llapi_get_param(&list, &count, "%s.%s.target_obd",
	...
	for (i = 0 ; i < count; i++) {
		if (uuidp && (index < *ost_count)) {
			int tmp = sscanf(list[i], format, &index, uuidp[index].uuid);
			if(tmp < 2) break;
                }
                index++;
        }
...
/*
 * Given the path to a stripe attribute /proc or /sys file, tries to open and
                           ...
                           unsigned int *attr)
{
	...
	rc = llapi_get_param(&list, &count, fpath);
	if ((rc == 0) && (count > 0) && (list[0] != NULL))
		*attr = atoi(list[0]);
                              ...
                              unsigned int *soffset)
{
	...
	if (fsname == NULL) {
        ...
        if (scount) {
		snprintf(fpath, PATH_MAX, "lov.%s-clilov-*.stripecount", dpath);
        ...
        if (ssize) {
		snprintf(fpath, PATH_MAX, "lov.%s-clilov-*.stripesize", dpath);
        ...
        if (soffset) {
		snprintf(fpath, PATH_MAX, "lov.%s-clilov-*.stripeoffset", dpath);
...
int llapi_ping(char *obd_type, char *obd_name)
{
	...
	char buffer[MAX_STRING_SIZE];
        ...
        snprintf(buffer, MAX_STRING_SIZE, "%s.%s.ping", obd_type, obd_name);

        rc = get_proc_path(path, strlen(path), buffer);
        if (rc != 0) {
                return rc;
        }
