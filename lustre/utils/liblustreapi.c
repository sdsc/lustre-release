                          ...
                          int list_size, char *buffer, int buffer_size)
{
	...
	for (i = 0 ; i < count; i++) {
		...
		strncpy(buffer + used, list[i], strlen(list[i]));
                       ...
                       char *buffer, int buffer_size)
{
	...
	char buf[PATH_MAX + 1], pathname[PATH_MAX + 1];
		...
		if (fsname == NULL) {
			...
			strncpy(buf, fsname, strlen(fsname));
		}
		snprintf(pathname, sizeof(pathname), "lov.%s-*.pools",
			      buf);
        }
                                  ...
                                  int *ost_count, enum tgt_type type)
{
	...
	for (i = 0 ; i < count; i++) {
		if (uuidp && (index < *ost_count)) {
			int tmp = sscanf(list[i], format, &index,
					 uuidp[index].uuid);
			if (tmp < 2)
				break;
                }
                              ...
                              unsigned int *soffset)
{
	...
	if (fsname == NULL) {
        ...
        if (soffset) {
		snprintf(fpath, PATH_MAX, "lov.%s-clilov-*.stripeoffset",
			 dpath);
