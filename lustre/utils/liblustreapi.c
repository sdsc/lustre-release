...
int llapi_file_open2(const char *name, int flags, int mode,
		     struct llapi_stripe_param *param)
{
	...
	if (param->lsp_nr_osts > 0) {
		...
		if (!found) {
			llapi_error(LLAPI_MSG_ERROR, -EINVAL,
				    "stripe offset '%d' is not in the "
				    "target list", param->lsp_stripe_offset);
			return -EINVAL;
		}
                    ...
                    int stripe_count, int stripe_pattern)
{
	...
	return llapi_file_open2(name, flags, mode, &param);
}

int llapi_file_create2(const char *name, struct llapi_stripe_param *param)
{
	int fd;

	fd = llapi_file_open2(name, O_CREAT | O_WRONLY, 0644, param);
	if (fd < 0)
		return fd;
                      ...
                      int stripe_offset, int stripe_count, int stripe_pattern)
{
	...
	return llapi_file_create2(name, &param);
}
...
int llapi_dir_create(const char *name, int flags,
		     struct llapi_stripe_param *param)
{
			  ...
			  int stripe_count, int stripe_pattern, char *pool_name)
{
	...
	return llapi_dir_create(name, flags, &param);
}
