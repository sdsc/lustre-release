 ...
 * \retval	-errno on error.
 */
int llapi_create_volatile_param(char *directory, int mdt_idx,
				...
				const struct llapi_stripe_param *stripe_param)
{
	...
	if (mdt_idx == -1)
		...
		snprintf(filename, sizeof(filename),
			 LUSTRE_VOLATILE_HDR":%.4X:%.4X", mdt_idx, rnumber);
...
{
	return llapi_create_volatile_param(directory, idx, open_flags,
