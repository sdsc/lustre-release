...
static int open_parent(const char *path)
{
	...
	fd_parent = open(parent_path, O_RDONLY | O_NOCTTY);
	free(path_copy);
			...
			void *data, struct dirent64 *de)
{
	...
	if (decision == 0) {
		...
		if (ret == 0 && param->fp_mdt_uuid != NULL) {
			if (fd != NULL) {
				...
				if (tmp_fd > 0) {
					ret = llapi_file_fget_mdtidx(tmp_fd,
						     &param->fp_file_mdt_index);
					close(tmp_fd);
				} else {
					ret = -errno;
	...
	if (param->fp_obd_uuid || param->fp_mdt_uuid) {
		...
		if (lustre_fs && !param->fp_got_uuids) {
			ret = setup_target_indexes((fd != NULL) ? *fd :
						   fd_parent, path, param);
			if (ret)
				return ret;
			    ...
			    struct dirent64 *de)
{
	...
	if (fd != NULL) {
		...
		tmp_fd = open(path, O_RDONLY | O_NOCTTY);
		if (tmp_fd > 0) {
			ret = llapi_file_fget_mdtidx(tmp_fd, &mdtidx);
			close(tmp_fd);
		} else {
			ret = -errno;
		}
			...
			struct dirent64 *de)
{
	...
	if (param->fp_obd_uuid) {
		param->fp_quiet = 1;
		ret = setup_obd_uuid((fd !=NULL) ? *fd : fd_parent, path, param);
		if (ret)
			return ret;
	}
...
dump:
	if (!(param->fp_verbose & VERBOSE_MDTINDEX))
		llapi_lov_dump_user_lmm(param, path, (fd != NULL) ? 1 : 0);
