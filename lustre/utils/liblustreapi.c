				...
				int stripe_count, int stripe_pattern)
{
	...
	if (stripe_count < -1 || stripe_count > LOV_MAX_STRIPE_COUNT) {
		rc = -EINVAL;
		cfs_error(CFS_MSG_ERROR, rc, "error: bad stripe count %d",
			  stripe_count);
		return rc;
	}
                         ...
                         int stripe_count, int stripe_pattern, char *pool_name)
{
        ...
        if (fd < 0) {
                rc = -errno;
		cfs_error(CFS_MSG_ERROR, rc, "unable to open '%s'", name);
                return rc;
        }
...
static int poolpath(char *fsname, char *pathname, char *pool_pathname)
{
	...
	rc = cfs_first_match(pattern, buffer);
        if (rc)
                return rc;
                          ...
                          int list_size, char *buffer, int buffer_size)
{
	...
	cfs_printf(CFS_MSG_NORMAL, "Pool: %s.%s\n", fsname, pool);
	sprintf(path, "%s/%s", pathname, pool);
	fd = fopen(path, "r");
	if (fd == NULL) {
		rc = -errno;
		cfs_error(CFS_MSG_ERROR, rc, "Cannot open %s", path);
                return rc;
        }
                       ...
                       char *buffer, int buffer_size)
{
        ...
        if (ptr != NULL) {
                ...
                if (!realpath(name, rname)) {
                        rc = -errno;
			cfs_error(CFS_MSG_ERROR, rc, "invalid path '%s'",
				  name);
			return rc;
		}
		...
		if (rc != 0) {
			cfs_error(CFS_MSG_ERROR, rc, "'%s' is not"
	...
	cfs_printf(CFS_MSG_NORMAL, "Pools from %s:\n", fsname);
...
static int common_param_init(struct find_param *param, char *path)
{
        ...
        if (param->lmd == NULL) {
		cfs_error(CFS_MSG_ERROR, -ENOMEM,
			  "error: allocation of %lu bytes for ioctl",
			  (unsigned long) (sizeof(lstat_t) + param->lumlen));
                return -ENOMEM;
        }
                 ...
                 struct lov_user_mds_data *lmd, int lumlen)
{
        ...
        if (ret) {
                if (errno == ENOTTY) {
                        ...
                        if (ret) {
                                ret = -errno;
				cfs_error(CFS_MSG_ERROR, ret,
					  "error: %s: lstat failed for %s",
					  __func__, path);
			}
		} else if (errno == ENOENT) {
			ret = -errno;
			cfs_error(CFS_MSG_WARN, ret,
                                   ...
                                   cfs_dirent_t *de)
{
        ...
        if (!d && errno != ENOTDIR) {
                ret = -errno;
		cfs_error(CFS_MSG_ERROR, ret, "%s: Failed to open '%s'",
			  __func__, path);
                return ret;
        } else if (!d && !parent) {
...
int llapi_file_get_lov_uuid(const char *path, struct obd_uuid *lov_uuid)
{
        ...
        if (fd < 0) {
                rc = -errno;
		cfs_error(CFS_MSG_ERROR, rc, "error opening %s", path);
                return rc;
        }
                                  ...
                                  int *ost_count, enum tgt_type type)
{
        ...
        if (fp == NULL) {
                rc = -errno;
		cfs_error(CFS_MSG_ERROR, rc, "error: opening '%s'", buf);
                return rc;
        }
...
int llapi_get_obd_count(char *mnt, int *count, int is_mdt)
{
        ...
        if (!root) {
                rc = -errno;
		cfs_error(CFS_MSG_ERROR, rc, "open %s failed", mnt);
                return rc;
        }
	...
	if (!param->obduuid && !param->quiet && !param->obds_printed)
        ...
        while (fgets(buf, sizeof(buf), fp) != NULL) {
                ...
                if (param->obduuid) {
                        ...
                        /* Print everything */
			cfs_printf(CFS_MSG_NORMAL, "%s", buf);
                }
        }
...
retry_get_uuids:
		...
		cfs_error(CFS_MSG_ERROR, ret, "get ost uuid failed");
                goto out_free;
        }
                           ...
                           unsigned int *attr)
{
        ...
        if (f == NULL) {
                rc = -errno;
		cfs_error(CFS_MSG_ERROR, rc, "Cannot open '%s'", fpath);
		return rc;
	}
	...
	if (fgets(line, sizeof(line), f) != NULL) {
		*attr = atoi(line);
		...
		cfs_error(CFS_MSG_ERROR, errno, "Cannot read from '%s'",
			     fpath);
                rc = 1;
        }
                                     ...
                                     int raw, char *pool_name)
{
        ...
        if (is_dir && lum->lmm_object_seq == FID_SEQ_LOV_DEFAULT) {
			...
			cfs_printf(CFS_MSG_NORMAL, "(Default) ");
	}
	...
	if (verbose & VERBOSE_COUNT) {
		if (verbose & ~VERBOSE_COUNT)
			cfs_printf(CFS_MSG_NORMAL, "%sstripe_count:   ",
				   prefix);
                if (is_dir) {
			...
			cfs_printf(CFS_MSG_NORMAL, "%hd%c",
	...
	if (verbose & VERBOSE_SIZE) {
		if (verbose & ~VERBOSE_SIZE)
			cfs_printf(CFS_MSG_NORMAL, "%sstripe_size:    ",
				   prefix);
                if (is_dir && !raw && lum->lmm_stripe_size == 0) {
                        ...
                        rc = sattr_cache_get_defaults(NULL, path, NULL, &ssize,
                                                      NULL);
                        if (rc == 0)
				cfs_printf(CFS_MSG_NORMAL, "%u%c", ssize,
					   nl);
			else
				cfs_error(CFS_MSG_ERROR, rc,
					  "Cannot determine default"
			...
			cfs_printf(CFS_MSG_NORMAL, "%u%c",
				   lum->lmm_stripe_size, nl);
		}
	}

	if ((verbose & VERBOSE_DETAIL) && !is_dir) {
		cfs_printf(CFS_MSG_NORMAL, "lmm_stripe_pattern: %x%c",
			   lum->lmm_pattern, nl);
	}

	if ((verbose & VERBOSE_GENERATION) && !is_dir) {
		if (verbose & ~VERBOSE_GENERATION)
			cfs_printf(CFS_MSG_NORMAL, "%slayout_gen:     ",
				   prefix);
		cfs_printf(CFS_MSG_NORMAL, "%u%c",
	...
	if (verbose & VERBOSE_OFFSET) {
		if (verbose & ~VERBOSE_OFFSET)
			cfs_printf(CFS_MSG_NORMAL, "%sstripe_offset:  ",
				   prefix);
		if (is_dir)
			cfs_printf(CFS_MSG_NORMAL, "%d%c",
	...
	if ((verbose & VERBOSE_POOL) && (pool_name != NULL)) {
		if (verbose & ~VERBOSE_POOL)
			cfs_printf(CFS_MSG_NORMAL, "%spool:           ",
				   prefix);
		cfs_printf(CFS_MSG_NORMAL, "%s%c", pool_name, nl);
	}
			...
			if ((obdindex == OBD_NOT_FOUND) || (obdindex == idx)) {
				...
				sprintf(fmt, "%s%s%s\n",
					...
					(fid_seq_is_rsvd(gr) ||
					 fid_seq_is_mdt0(gr)) ?
					 "%14llu" : "%#14llx", "%s");
				cfs_printf(CFS_MSG_NORMAL, fmt, idx, oid,
					   oid, gr,
					   obdindex == idx ? " *" : "");
			}
		}
                             ...
                             char *path, int is_dir)
{
        ...
        default:
		cfs_printf(CFS_MSG_NORMAL, "unknown lmm_magic:  %#x "
...
obd_matches:
        ...
        while (!decision) {
                ...
                if (ret) {
                        if (errno == ENOENT) {
				cfs_error(CFS_MSG_ERROR, -ENOENT,
					  "warning: %s: %s does not exist",
					  __func__, path);
				goto decided;
			} else {
        ...
        if (decision != -1) {
		cfs_printf(CFS_MSG_NORMAL, "%s", path);
		if (param->zeroend)
			cfs_printf(CFS_MSG_NORMAL, "%c", '\0');
		else
			cfs_printf(CFS_MSG_NORMAL, "\n");
        }
                     ...
                     struct obd_uuid *uuid_buf)
{
		...
		cfs_error(CFS_MSG_ERROR, rc, "error: %s: opening '%s'",
			  __func__, path);
                return rc;
        }
                            ...
                            char *obd_uuid, void *args)
{
        ...
        if (rc == ENOTCONN) {
		cfs_printf(CFS_MSG_NORMAL, "%s inactive.\n", obd_name);
	} else if (rc) {
		cfs_error(CFS_MSG_ERROR, rc, "error: check '%s'",
			  obd_name);
	} else {
		cfs_printf(CFS_MSG_NORMAL, "%s active.\n", obd_name);
        }
}
                         ...
                         cfs_dirent_t *de)
{
        ...
        if (rc) {
				...
				cfs_error(CFS_MSG_ERROR, -ENODATA,
	...
	if (rc) {
		rc = -errno;
		cfs_error(CFS_MSG_ERROR, rc, "error: chmod %s (%hu)",
			  path, st->st_mode);
        }
...
static int rmtacl_notify(int ops)
{
	...
	int found = 0, fd = 0, rc = 0;
		...
		if (rc < 0) {
			rc = -errno;
			cfs_error(CFS_MSG_ERROR, rc, "ioctl %d\n", fd);
			goto out;
                }
...
static int do_rmtacl(int argc, char *argv[], int ops, int (output_func)(char *))
{
        ...
        if (output_func) {
			...
			cfs_error(CFS_MSG_ERROR, rc,
                ...
                if (pid < 0) {
                        rc = -errno;
			cfs_error(CFS_MSG_ERROR, rc, "Can't fork\n");
                        close(fd[0]);
                        close(fd[1]);
                        return rc;
                } else if (!pid) {
        ...
        if (fp == NULL) {
                rc = -errno;
		cfs_error(CFS_MSG_ERROR, rc, "fdopen %d failed\n", fd[0]);
                kill(pid, SIGKILL);
                close(fd[0]);
                return rc;
        }
        ...
        if (waitpid(pid, &status, 0) < 0) {
                rc = -errno;
		cfs_error(CFS_MSG_ERROR, rc, "waitpid %d failed\n", pid);
                return rc;
        }
                      ...
                      int want_error)
{
        ...
        if (rc == -1)
                ...
                rc = 0;
        if (rc && want_error)
		cfs_error(CFS_MSG_ERROR, rc, "ioctl %d err %d", opc, rc);
                   ...
                   int buflen, long long *recno, int *linkno)
{
        ...
        if (rc) {
                if (rc != -ENOENT)
			cfs_error(CFS_MSG_ERROR, rc, "ioctl err %d", rc);
        } else {
                         ...
                         int archive_count, int *archives)
{
        ...
        for (rc = 0; rc < archive_count; rc++) {
			...
			cfs_err_noerrno(CFS_MSG_ERROR,
					"Maximum of %lu archives supported",
					(unsigned long) sizeof(ct->archives));
                        goto out_err;
                }
