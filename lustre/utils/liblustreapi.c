			   ...
			   const char *param, char *buf, size_t buf_size)
{
	...
	return get_param_value("llite", inst, FILTER_BY_EXACT, param, buf, buf_size);
}
			 ...
			 const char *param, char *buf, size_t buf_size)
{
	...
	return get_param_value("lov", uuid.uuid, FILTER_BY_EXACT, param,
			       buf, buf_size);
}
			 ...
			 const char *param, char *buf, size_t buf_size)
{
	...
	return get_param_value("lmv", uuid.uuid, FILTER_BY_EXACT, param,
			       buf, buf_size);
}
...
int llapi_search_ost(char *fsname, char *poolname, char *ostname)
{
	...
	if (poolname == NULL) {
		if (len > 0) {
			...
			rc = get_lustre_param_path(&param, "lov", fsname,
						   FILTER_BY_FS_NAME,
						   "target_obd");
			if (rc == 0) {
		...
		rc = poolpath(fsname, NULL, buffer);
		if (rc == 0)
			strncat(buffer, poolname, sizeof(buffer));
	}
        if (rc)
                return rc;
                          ...
                          int list_size, char *buffer, int buffer_size)
{
	...
	rc = snprintf(buf, sizeof(buf), "%s/%s", pathname, pool);
	if (rc >= sizeof(buf))
		return -EOVERFLOW;
	fd = fopen(buf, "r");
	if (fd == NULL) {
...
int llapi_poollist(const char *name)
{
	...
	char **list, *buffer = NULL, *fsname = NULL;
	...
	rc = get_param_value("lov", name, type, "numobd",
			     data, sizeof(data));
	if (rc < 0)
		goto err;
	obdcount = atoi(data);
                                  ...
                                  int *ost_count, enum tgt_type type)
{
	...
	rc = find_target_obdpath(buf, name.uuid, type);
	if (rc != 0)
		return -ENOENT;
...
static int setup_obd_uuid(DIR *dir, char *dname, struct find_param *param)
{
	...
	/* Now get the ost uuids */
	rc = find_target_obdpath(buf, obd_uuid.uuid,
...
{
	char filter[PATH_MAX + 1];
	char val[PATH_MAX + 1];
	int rc;

	snprintf(filter, PATH_MAX, "%s-clilov-*", fsname);

	if (scount) {
		rc = get_param_value("lov", filter, FILTER_BY_EXACT,
...
int llapi_ping(char *obd_type, char *obd_name)
{
	...
	rc = cfs_get_param_path(&path, "%s/%s/ping",
				obd_type, obd_name);
	if (rc != 0)
		return rc;
                         ...
                         void *args, llapi_cb_t cb)
{
	...
	rc = cfs_get_param_path(&param, "devices");
	if (rc != 0)
		return -ENOENT;
