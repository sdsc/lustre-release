...
static int llapi_stripe_limit_check(struct llapi_stripe_param *param)
{
	unsigned long long stripe_size = param->lsp_stripe_size;
	int stripe_offset = param->lsp_stripe_offset;
	int stripe_count = param->lsp_stripe_count;
	int page_size, rc;
	...
	if (stripe_offset < -1 || stripe_offset > LOV_V1_INSANE_STRIPE_COUNT) {
		rc = -EINVAL;
			  ...
			  struct llapi_stripe_param *param)
{
	...
	unsigned int lum_size = sizeof(*lum);
	int fd, rc = 0;
	...
	if (param->lsp_nr_osts > 0) {
		char ostname[MAX_OBD_NAME + 1];
		bool found;
		int i;

		/* list of targets is exclusive with pool */
		if (pool_name != NULL) {
			llapi_error(LLAPI_MSG_ERROR, -EINVAL,
				    "cannot have --pool and --ost "
				    "on the same time");
			return -EINVAL;
		}

		/* if stripe offset is set, make sure it's one of the OSTs. */
		found = param->lsp_stripe_offset == -1;

		for (i = 0; i < param->lsp_nr_osts; i++) {
		...
		/* set stripe count to # of targets in the list */
		param->lsp_stripe_count = param->lsp_nr_osts;

		/* sizeof lum */
		lum_size += param->lsp_nr_osts * sizeof(lum->lmm_objects[0]);
	}
			...
			if (strcmp(pool_name, fsname) != 0) {
				*ptr = '.';
				llapi_err_noerrno(LLAPI_MSG_ERROR,
					"%s: pool '%s' is not found on "
					"filesystem '%s'",
					__func__, pool_name, fsname);
				return -EINVAL;
			}
			pool_name = ptr + 1;
		}
...
retry_open:
	...
	if (fd < 0) {
		rc = -errno;
		llapi_error(LLAPI_MSG_ERROR, rc, "%s: unable to open '%s'",
			    __func__, name);
		return rc;
	}

	rc = llapi_stripe_limit_check(param);
	if (rc != 0)
		goto out;
	...
	} else if (param->lsp_nr_osts > 0) {
		int i;

		lum->lmm_magic = LOV_USER_MAGIC_V4;
		for (i = 0; i < param->lsp_nr_osts; i++)
			lum->lmm_objects[i].l_ost_idx = param->lsp_osts[i];
	}
	...
	if (ioctl(fd, LL_IOC_LOV_SETSTRIPE, lum)) {
		...
		llapi_err_noerrno(LLAPI_MSG_ERROR,
				  "error on ioctl %lu for '%s' (%d): %s",
				  (unsigned long)LL_IOC_LOV_SETSTRIPE, name,
				  fd, errmsg);
	}
out:
	if (rc) {
                    ...
                    int stripe_count, int stripe_pattern)
{
	...
	return llapi_file_open_param(name, flags, (mode_t)mode, &param);
}

int llapi_file_create_param(const char *name, struct llapi_stripe_param *param)
{
	int fd;

	fd = llapi_file_open_param(name, O_CREAT | O_WRONLY, 0644, param);
	if (fd < 0)
		return fd;
                      ...
                      int stripe_offset, int stripe_count, int stripe_pattern)
{
	...
	return llapi_file_create_param(name, &param);
}
...
int llapi_dir_create(const char *name, struct llapi_stripe_param *param)
{
...
int llapi_dir_create_pool(const char *name, int flags, int stripe_offset,
			  int stripe_count, int stripe_pattern, char *pool_name)
{
	struct llapi_stripe_param param = { 0 };

	param.lsp_stripe_count = stripe_count;
	param.lsp_stripe_pattern = stripe_pattern;
	param.lsp_stripe_offset = stripe_offset;
	param.lsp_pool = pool_name;

	return llapi_dir_create(name, &param);
}
