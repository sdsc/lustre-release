...
/** Convert a v1 changelog record into an extended one. */
static void changelog_extend_v1(struct changelog_ext_rec *ext)
{
	...
	ext->cr_flags |= CLF_EXT_VERSION;
}

/** Convert a v2 record into an extended one. */
static void changelog_extend_v2(struct changelog_ext_rec_v2 *ext)
{
	...
	memmove(ext->cr_name, rec->cr_name, rec->cr_namelen);
	memmove(ext->cr_jobid, rec->cr_jobid, sizeof(ext->cr_jobid));
	fid_zero(&ext->cr_sfid);
	fid_zero(&ext->cr_spfid);
	ext->cr_flags |= CLF_EXT_VERSION;
}
 ...
 */
static void changelog_extend_rec(struct changelog_ext_rec_v2 *rec, int format)
{
	changelog_rec_remap((struct changelog_rec_v2 *)rec, format);
	if (format == CHANGELOG_REC_FMT_V1)
		changelog_extend_v1((struct changelog_ext_rec *)rec);
	else
		changelog_extend_v2(rec);
}
...
int llapi_changelog_recv(void *priv, struct changelog_ext_rec_v2 **rech)
{
	...
	format = cp->flags & CHANGELOG_FLAG_JOBID ? CHANGELOG_REC_FMT_V2 :
...
repeat:
	...
	/* Our message can be of type changelog_rec, changelog_ext_rec,
	 * changelog_rec_v2 or changelog_ext_rec_v2.
	 * Use pointer math to skip kuch_hdr and point directly to the message
	 * payload.
	 */
	*rech = (struct changelog_ext_rec_v2 *)(kuch + 1);
	changelog_extend_rec(*rech, format);
