...
int llapi_dir_set_default_stripe(const char *name, int stripe_offset,
				 int stripe_count, int stripe_pattern,
				 const char *pool_name)
{
	...
	if (pool_name != NULL) {
		...
		memcpy(lum.lum_pool_name, pool_name, strlen(pool_name));
	}
	...
	if (ioctl(fd, LL_IOC_LMV_SET_DEFAULT_STRIPE, &lum)) {
		char *errmsg = "stripe already set";
		rc = -errno;
		if (errno != EEXIST && errno != EALREADY)
			errmsg = strerror(errno);
