			   ...
			   char *buf, size_t buf_size)
{
	...
	return get_lustre_param_value("llite", inst, FILTER_BY_EXACT, param,
				      buf, buf_size);
}
                                  ...
                                  int *ost_count, enum tgt_type type)
{
	...
	rc = find_target_obdpath(buf, sizeof(buf), name.uuid, type);
	if (rc != 0)
		return -ENOENT;
...
static int setup_obd_uuid(DIR *dir, char *dname, struct find_param *param)
{
	...
	/* Now get the ost uuids */
	rc = find_target_obdpath(buf, sizeof(buf), obd_uuid.uuid,
