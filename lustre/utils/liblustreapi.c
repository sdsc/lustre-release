...
int llapi_search_ost(char *fsname, char *poolname, char *ostname)
{
	...
	if (poolname == NULL) {
		...
		if (rc == 0) {
			...
			fprintf(stderr, "poolpath failed with %d\n", rc);
		}
	}
	cfs_free_param_data(&param);
	if (rc) {
		fprintf(stderr, "llapi_search_ost failed to get parameter path\n");
		return rc;
	}

        fd = fopen(buffer, "r");
	if (fd == NULL) {
		fprintf(stderr, "llapi_search_ost failed to open %s\n", buffer);
                return -errno;
	}
                                  ...
                                  int *ost_count, enum tgt_type type)
{
	...
	rc = get_lustre_param_path(&param, type == LOV_TYPE ? "lov" : "lmv",
				   name.uuid, FILTER_BY_EXACT, "target_obd");
	if (rc != 0)
		return -ENOENT;
...
static int setup_obd_uuid(DIR *dir, char *dname, struct find_param *param)
{
	...
	/* Now get the ost uuids */
	rc = get_lustre_param_path(&param_data,
                              ...
                              unsigned int *soffset)
{
	...
	if (scount) {
		rc = get_lustre_param_value("lov", filter, FILTER_BY_EXACT,
