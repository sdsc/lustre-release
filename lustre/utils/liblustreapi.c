			  ...
			  int stripe_count, int stripe_pattern, char *pool_name)
{
	...
	gid = getegid();
	if (uid != 0 || gid != 0) {
		fprintf(stderr, "only sysadmin can create remote dir\n");
		return -EACCES;
	}
	...
	if (rc) {
		llapi_error(LLAPI_MSG_ERROR, rc,
			    "error: IOC_LMV_SETSTRIPE pack failed for '%s'.",
			    name);
		free(dirpath);
		free(namepath);
		return rc;
	...
	if (ioctl(fd, LL_IOC_LMV_SETSTRIPE, buf)) {
		...
		llapi_err_noerrno(LLAPI_MSG_ERROR,
				  "error on ioctl "LPX64" for '%s' (%d): %s",
				  (__u64)LL_IOC_LMV_SETSTRIPE, name, fd,
				  errmsg);
	}
...
int llapi_direntry_remove(char *dname)
{
	...
	gid = getegid();
	if (uid != 0 || gid != 0) {
		fprintf(stderr, "only sysadmin can remove remote dir\n");
		return -EACCES;
	}
	...
	if (rc) {
		llapi_error(LLAPI_MSG_ERROR, rc,
			    "error: IOC_REMOVE_ENTRY pack failed for '%s'.",
			    filename);
		free(dirpath);
		free(namepath);
		return rc;
	...
	if (ioctl(fd, LL_IOC_REMOVE_ENTRY, buf)) {
...
static int common_param_init(struct find_param *param, char *path)
{
	...
	/* On phase 1, striped dir would not be supported,
	 * so we only allocate 1 stripe here, and it will
	 * need more for real dir stripe */
	param->lmvlen = lmv_user_md_size(1, LMV_MAGIC_V1);
	param->lmv_md = malloc(param->lmvlen);
	if (param->lmv_md == NULL) {
		llapi_error(LLAPI_MSG_ERROR, -ENOMEM,
			    "error: allocation of %d bytes for ioctl",
			    param->lmvlen);
		return -ENOMEM;
...
{
	struct lmv_user_md *lmv = (struct lmv_user_md *)param->lmv_md;
	int ret = 0;
				...
				if (dent->d_type == DT_DIR) {
					...
					if (!dir) {
						...
						llapi_error(LLAPI_MSG_ERROR,
						 ret, "%s: Failed to open '%s'",
						      __func__, path);
					} else {
						ret = cb_get_dirstripe(path,
								    dir, param);
						closedir(dir);
					}
				} else {
                             ...
                             char *path, int is_dir)
{
	...
	if (param->get_lmv)
		magic = (__u32)param->lmv_md->lum_magic;
	else
	...
	case LMV_USER_MAGIC: {
		...
		lum = (struct lmv_user_md *)param->lmv_md;
