...
/**
 * Generate a non-null integer for use as a group identifier.
 *
 * \param[out] rnd	Where to store the GID
 * \retval 0 on success
...
retry:
	sz_ret = read(fd, rnd, sz_exp);
	if (sz_ret < sz_exp) {
		rc = -errno;
		fprintf(stderr, "cannot read %zd bytes from /dev/urandom: %s\n",
			sz_exp, strerror(-rc));
		goto out;
	}
...
int llapi_fswap_layouts(int fd1, int fd2, __u64 dv1, __u64 dv2, __u64 flags)
{
	...
	rc = llapi_grouplock_random_id(&grp_id);
	if (rc < 0)
		return rc;
