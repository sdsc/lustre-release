...
int llapi_poollist(const char *name)
{
        /* list of pool names (assume that pool count is smaller
           than OST count) */
        char *list[FIND_MAX_TGTS];
        char *buffer;
        /* fsname-OST0000_UUID < 32 char, 1 per OST */
        int bufsize = FIND_MAX_TGTS * 32;
        int i, nb;
        ...
        if ((name[0] == '/') || (strchr(name, '.') == NULL))
                ...
                nb = llapi_get_poollist(name, list, FIND_MAX_TGTS, buffer,
                                        bufsize);
        else
                /* name is a pool name (<fsname>.<poolname>) */
                nb = llapi_get_poolmembers(name, list, FIND_MAX_TGTS, buffer,
                                           bufsize);
...
static int llapi_get_target_uuids(int fd, struct obd_uuid *uuidp,
                                  int *ost_count, enum lov_type type)
{
 ...
 * param->obdindexes */
static int setup_indexes(DIR *dir, struct obd_uuid *obduuids, int num_obds,
                         int **obdindexes, int *obdindex, enum lov_type type)
{
        struct obd_uuid *uuids = NULL;
        int obdcount = INIT_ALLOC_NUM_TGTS;
...
static int setup_target_indexes(DIR *dir, struct find_param *param)
{
        int ret = 0;

        if (param->mdtuuid) {
                ret = setup_indexes(dir, param->mdtuuid, param->num_mdts,
                              &param->mdtindexes, &param->mdtindex, LMV_TYPE);
                if (ret)
                        return ret;
        }
        if (param->obduuid) {
                ret = setup_indexes(dir, param->obduuid, param->num_obds,
                        ...
                        void *data, cfs_dirent_t *de)
{
        ...
        if (param->obduuid || param->mdtuuid) {
                ...
                if (lustre_fs && !param->got_uuids) {
                        ret = setup_target_indexes(dir ? dir : parent, param);
                        if (ret)
                                return ret;
