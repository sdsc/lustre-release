...
typedef cYAML_handler_error_t (*yaml_token_handler)(yaml_token_t *token,
						    cYAML_tree_node_t *);

static cYAML_handler_error_t yaml_parse_error(yaml_token_t *token,
					      cYAML_tree_node_t *tree);
static cYAML_handler_error_t yaml_stream_start(yaml_token_t *token,
					       cYAML_tree_node_t *tree);
static cYAML_handler_error_t yaml_stream_end(yaml_token_t *token,
					     cYAML_tree_node_t *tree);
static cYAML_handler_error_t yaml_not_supported(yaml_token_t *token,
						cYAML_tree_node_t *tree);
static cYAML_handler_error_t yaml_document_start(yaml_token_t *token,
						 cYAML_tree_node_t *tree);
static cYAML_handler_error_t yaml_document_end(yaml_token_t *token,
					       cYAML_tree_node_t *tree);
static cYAML_handler_error_t yaml_blk_seq_start(yaml_token_t *token,
						cYAML_tree_node_t *tree);
static cYAML_handler_error_t yaml_blk_mapping_start(yaml_token_t *token,
						    cYAML_tree_node_t *tree);
static cYAML_handler_error_t yaml_block_end(yaml_token_t *token,
					    cYAML_tree_node_t *tree);
static cYAML_handler_error_t yaml_key(yaml_token_t *token,
				      cYAML_tree_node_t *tree);
static cYAML_handler_error_t yaml_value(yaml_token_t *token,
					cYAML_tree_node_t *tree);
static cYAML_handler_error_t yaml_scalar(yaml_token_t *token,
					 cYAML_tree_node_t *tree);
...
void cYAML_ll_free(cYAML_ll_t *ll)
{
	cYAML_ll_t *node;
	node = ll;
	while (node) {
...
int cYAML_ll_push(cYAML *obj, cYAML_ll_t **list)
{
...
cYAML *cYAML_ll_pop(cYAML_ll_t **list)
{
	cYAML_ll_t *pop = *list;
	cYAML *obj = NULL;
...
int cYAML_ll_check_instances(cYAML_ll_t *ll, cYAML *obj, int instances)
{
	cYAML_ll_t *node = ll;
	int i = instances;
	...
	while (node) {
		if ((node->obj->string) &&
		    !strcmp(node->obj->string, obj->string)) {
			i--;
			if (i < 0)
				return -1;
		}
...
cYAML *cYAML_get_object_item(cYAML *parent, const char *name)
{
	cYAML *node;
...
void cYAML_tree_recursive_walk(cYAML *node, cYAML_walk_cb cb,
				      bool cb_first,
				      void *usr_data,
				      void **out)
{
...
cYAML *cYAML_find_object(cYAML *root, const char *name)
{
	cYAML *found = NULL;
...
void cYAML_clean_usr_data(cYAML *node, cYAML_user_data_free_cb free_cb)
{
	cYAML_tree_recursive_walk(node, clean_usr_data, false, free_cb, NULL);
}

void cYAML_free_tree(cYAML *node)
{
...
void cYAML_print_tree(cYAML *node, int level)
{
	cYAML_print_tree_helper(stderr, node, level);
}

void cYAML_print_tree2file(FILE *f, cYAML *node, int level)
{
	cYAML_print_tree_helper(f, node, level);
}

cYAML *cYAML_create_object(cYAML *parent, char *key)
{
...
cYAML *cYAML_create_string(cYAML *parent, char *key, char *value)
{
	cYAML *node = calloc(sizeof(char),
			     sizeof(cYAML));
	if (!node)
...
cYAML *cYAML_create_number(cYAML *parent, char *key, double value)
{
	cYAML *node = calloc(sizeof(char),
			     sizeof(cYAML));
	if (!node)
...
void cYAML_insert_sibling(cYAML *root, cYAML *sibling)
{
	cYAML *last = NULL;
	if (!root || !sibling)
		return;
...
cYAML *cYAML_build_tree(char *yaml_file,
			const char *yaml_blk,
