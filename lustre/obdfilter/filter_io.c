...
repeat:
		...
		for (i = 0; i < page_count; i++, lnb++, page++) {
			if (i > 0) {
                                /* Sigh, alloc_pages only init the first page, but
                                 * all pages needs to be put to cache, so it needs
                                 * to initialize pages after the first page. (only
                                 * _page_count needs to be reset right now)
                                 * Copy these 5 lines of code from prep_new_page()
                        ...
                        rc = add_to_page_cache_lru(page, inode->i_mapping,
                                                 lnb->offset >> CFS_PAGE_SHIFT,
                                                 gfp_mask);
                        if (unlikely(rc)) {
                                int j;
