 ...
 **/
static int filter_alloc_lnb_pages(struct obd_device *obd, struct dentry *dentry,
                                  ...
                                  int page_count)
{
        ...
        CDEBUG(D_CACHE, "%s:allocate pages order %d ino %lu\n",
               obd->obd_name, order, dentry->d_inode->i_ino);
        ...
        for (i = 0; i < page_count; i++, lnb++, page++) {
                ...
                rc = ll_add_to_page_cache_lru(page, dentry->d_inode->i_mapping,
 ...
 **/
static int filter_get_lnb_pages(struct obd_device *obd, struct dentry *dentry,
                                ...
                                int localreq, int *pages)
{
        ...
        for (i = 0; i < count; i++, lnb++) {
                page = find_lock_page(dentry->d_inode->i_mapping,
                                      lnb->offset >> CFS_PAGE_SHIFT);
                if (page == NULL) {
                        if (nlnb == NULL)
                                nlnb = lnb;
                        ...
                        if (page_count > 0) {
                                rc = filter_alloc_lnb_pages(obd, dentry, nlnb,
                                                          localreq, page_count);
                                if (rc)
                                        GOTO(out, rc);
                                nlnb = NULL;
        ...
        if (page_count > 0 && rc == 0)
                rc = filter_alloc_lnb_pages(obd, dentry, nlnb, localreq,
                                            page_count);
out:
        if (rc)
 ...
 **/
int filter_alloc_norder_pages(struct obd_device *obd, struct dentry *dentry,
                              ...
                              int localreq)
{
        ...
        while (count < page_count) {
                int pages;
                rc = filter_get_lnb_pages(obd, dentry, nlnb, order, localreq,
                                          &pages);
                if (rc) {
                        if (rc != -ENOMEM)
                                return rc;
                              ...
                              struct lustre_capa *capa)
{
        ...
        for (i = 0, nlnb = res; i < *npages;) {
                ...
                rc = filter_alloc_norder_pages(obd, dentry, nlnb, order, 0);
                if (rc)
                        GOTO(cleanup, rc);

                for (j = 0, lnb = nlnb; j < count; j++, lnb++) {
                        LASSERT(lnb->page != NULL);
                        LASSERT(lnb->dentry != NULL);
                               ...
                               struct lustre_capa *capa)
{
    ...
    recreate_out:
        ...
        for (i = 0, nlnb = res; i < *npages;) {
                 ...
                 * performance, it tries to get continguous pages for niobuf.
                 * See LU-410 for details */
                rc = filter_alloc_norder_pages(obd, dentry, nlnb, order,
                                               localreq);
                if (rc)
                        GOTO(cleanup, rc);
