                              ...
                              struct ptlrpc_request *r, int increase_only)
{
 ...
 disk_update:
	...
	if ((inode->i_mtime.tv_sec > lvb->lvb_mtime ||
	     (inode->i_mtime.tv_sec == lvb->lvb_mtime &&
	      inode->i_mtime.tv_nsec > lvb->lvb_mtime_ns)) || !increase_only) {
	...
	if ((inode->i_atime.tv_sec > lvb->lvb_atime ||
	     (inode->i_atime.tv_sec == lvb->lvb_atime &&
	      inode->i_atime.tv_nsec > lvb->lvb_atime_ns)) || !increase_only) {
	...
	if ((inode->i_ctime.tv_sec > lvb->lvb_ctime ||
	     (inode->i_ctime.tv_sec == lvb->lvb_ctime &&
	      inode->i_ctime.tv_nsec > lvb->lvb_ctime_ns)) || !increase_only) {
