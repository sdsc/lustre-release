...
set_last_id:
                ...
                LTIME_S(iattr.ia_atime) = LONG_MIN + 24 * 3600;
                LTIME_S(iattr.ia_mtime) = LONG_MIN + 24 * 3600;
                LTIME_S(iattr.ia_ctime) = LONG_MIN + 24 * 3600;
                err = fsfilt_setattr(obd, dchild, handle, &iattr, 0);
                if (err)
                        CERROR("unable to initialize a/c/m time of the newly"
                                "created inode\n");
