...
struct lu_context_key osdbench_thread_key;
...
LU_KEY_INIT_FINI(osdbench, struct obe_thread_info);

LU_TYPE_INIT_FINI(osdbench, &osdbench_thread_key);

LU_CONTEXT_KEY_DEFINE(osdbench, LCT_DT_THREAD);

static struct lu_device_type_operations obe_device_type_ops = {
	.ldto_init		= osdbench_type_init,
	.ldto_fini		= osdbench_type_fini,

	.ldto_start		= osdbench_type_start,
	.ldto_stop		= osdbench_type_stop,
};
...
{
	return lu_context_key_get(&env->le_ctx, &osdbench_thread_key);
}
...
static int obe_replay_create_update(const struct lu_env *env,
				    ...
				    struct thandle *th, int exec)
{
	struct obdo		*lobdo = &obe_oti_get(env)->oti_obdo1;
	struct obdo		*wobdo = &obe_oti_get(env)->oti_obdo2;
				       ...
				       struct thandle *th, int exec)
{
	...
	if (exec == 0)
		rc = dt_declare_xattr_set(env, o, &b, name, fl, th);
	else
		rc = dt_xattr_set(env, o, &b, name, fl, th);
	return rc;
}
...
static int osdbench_execute(const struct lu_env *env, struct obe_tx *obt)
{
...
static int osdbench_abort(const struct lu_env *env, struct obe_tx *obt)
{
	struct obe_update *obu, *tmp;
	int rc;
	ENTRY;
...
osdbench_create_tx(const struct lu_env *env, struct obe_device *obe)
{
	struct obe_tx *tx;
	ENTRY;
...
static int
osdbench_perform(const struct lu_env *env, struct obe_device *obe,
		 struct object_update_request *ureq)
{
	...
	for (i = 0; i < ureq->ourq_count; i++) {
		...
		if (update->ou_batchid != batchid) {
			...
			if (obt != NULL)
				osdbench_execute(env, obt);

			obt = osdbench_create_tx(env, obe);
			LASSERT(!IS_ERR(obt));
		...
		obu->obu_object = obe_locate(env, obe, &update->ou_fid);
		LASSERT(!IS_ERR(obu->obu_object));
		...
		if (rc < 0) {
			osdbench_abort(env, obt);
			obt = NULL;
			break;
		}
	}
	if (obt != NULL)
		rc = osdbench_execute(env, obt);

	RETURN(rc);
}

static int
osdbench_iocontrol(unsigned int cmd, struct obd_export *exp, int len,
		   void *karg, void *uarg)
{
	...
	OBD_ALLOC_LARGE(ureq, data->ioc_plen1);
	if (ureq == NULL) {
		CERROR("can't allocate %d\n", data->ioc_plen1);
		GOTO(out, rc = -ENOMEM);
	}

	if (copy_from_user(ureq, data->ioc_pbuf1, data->ioc_plen1))
		GOTO(out, rc = -EFAULT);

	if (unlikely(ureq->ourq_magic != UPDATE_REQUEST_MAGIC))
		GOTO(out, rc = -EINVAL);

	rc = osdbench_perform(env, obe, (void *)data->ioc_pbuf1);

	EXIT;
out:
	if (ureq)
		OBD_FREE_LARGE(ureq, data->ioc_plen1);
	if (env) {
		lu_env_fini(env);
		OBD_FREE_PTR(env);
	}
...
static int osdbench_setup(struct obd_device *obddev, struct lustre_cfg *lcfg)
{
	...
	*((struct obe_device **)p) = obe;
	...
	/* LCONSOLE_INFO("%s: connected to %s\n", obddev->obd_name,
			 tgt->obd_name); */

	RETURN(rc);
}

static int osdbench_cleanup(struct obd_device *obddev)
{
	void *p = &obddev->u.obt;
	struct obe_device *obe = *((struct obe_device **)p);
	int rc;
...
static int osdbench_connect(const struct lu_env *env, struct obd_export **exp,
...
static int osdbench_disconnect(struct obd_export *exp)
{
	int                     rc;
	ENTRY;
...
static struct obd_ops osdbench_obd_ops = {
	.o_owner       = THIS_MODULE,
	.o_iocontrol   = osdbench_iocontrol,
	.o_connect     = osdbench_connect,
	.o_disconnect  = osdbench_disconnect,
	.o_setup       = osdbench_setup,
	.o_cleanup     = osdbench_cleanup
};
...
# ifdef HAVE_SERVER_SUPPORT
	rc = class_register_type(&osdbench_obd_ops, NULL, true, NULL,
				 LUSTRE_OSDBENCH_NAME, NULL);
	LU_CONTEXT_KEY_INIT(&osdbench_thread_key);
	lu_context_key_register(&osdbench_thread_key);
#else
	RETURN(-ENOTSUP);
# endif
...
{
# ifdef HAVE_SERVER_SUPPORT
	lu_context_key_degister(&osdbench_thread_key);
