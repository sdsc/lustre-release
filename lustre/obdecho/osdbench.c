			  ...
			  const struct lu_object_conf *unused)
{
	...
	obe = container_of0(o->lo_dev, struct obe_device, obe_top_dev.dd_lu_dev);
...
static int
osdbench_execute(const struct lu_env *env, struct obe_tx *obt)
{
	...
	list_for_each_entry_safe(obu, tmp, &obt->obt_updates_list, obu_list) {
		...
		rc = obe_updates_vec[type](env, obu->obu_object, obu->obu_update,
					   obt->obt_th, 1);
		LASSERT(rc == 0);
		 ...
		 struct object_update_request *ureq)
{
	...
	for (i = 0; i < ureq->ourq_count; i++) {
		...
		list_add(&obu->obu_list, &obt->obt_updates_list);
		   ...
		   void *karg, void *uarg)
{
	...
	rc = osdbench_perform(env, obe, ureq);

	EXIT;
out:
	if (ureq)
		OBD_FREE(ureq, data->ioc_plen1);
	if (env) {
		lu_env_fini(env);
		OBD_FREE_PTR(env);
	}
