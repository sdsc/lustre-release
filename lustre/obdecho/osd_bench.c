				    ...
				    struct thandle *th, int exec)
{
	...
	struct dt_index_features *feat = &obe_oti_get(env)->oti_dif;
	...
	if (unlikely(IS_ERR(wobdo)))
		return PTR_ERR(wobdo);
	if (unlikely(wobdo == NULL))
		return -EINVAL;
	if (unlikely(size != sizeof(*wobdo)))
		return -EINVAL;
	...
	if (type != NULL && !IS_ERR(type)) {
		...
		if (dof.dof_type == DFT_INDEX) {
			...
			feat->dif_keysize_max = 16;
			feat->dif_ptrsize = 4;
			feat->dif_recsize_max = 8;
		}
	}
...
static int obe_replay_insert_bin_update(const struct lu_env *env,
					struct dt_object *o,
					struct object_update *u,
					struct thandle *th, int exec)
{
	struct obe_thread_info	*info = obe_oti_get(env);
	...
	if (o->do_index_ops == NULL) {
		...
		if (unlikely(rc < 0)) {
			CERROR("can't use as index with %d/%d: rc = %d\n",
				(int)dif->dif_keysize_min,
				(int)dif->dif_recsize_min, rc);
			return rc;
		}
	}
...
static int obe_replay_delete_bin_update(const struct lu_env *env,
					struct dt_object *o,
					struct object_update *u,
					struct thandle *th, int exec)
{
	...
	if (exec == 0)
		rc = dt_declare_delete(env, o, (struct dt_key *)name, th);
	else
		rc = dt_delete(env, o, (struct dt_key *)name, th);
...
static osd_update_func obe_updates_vec[] = {
	...
	[OUT_INDEX_BIN_INSERT]	= obe_replay_insert_bin_update,
	[OUT_INDEX_BIN_DELETE]	= obe_replay_delete_bin_update,
};
