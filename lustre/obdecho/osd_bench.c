...
enum {
	LPROC_OBE_START		= (OUT_START * 2),
	LPROC_OBE_CREATE	= (OUT_CREATE * 2),
	LPROC_OBE_DECL_CREATE	= (OUT_CREATE * 2) + 1,
	LPROC_OBE_DESTROY	= (OUT_DESTROY * 2),
	LPROC_OBE_DECL_DESTROY	= (OUT_DESTROY * 2) + 1,
	LPROC_OBE_REF_ADD	= (OUT_REF_ADD * 2),
	LPROC_OBE_DECL_REF_ADD	= (OUT_REF_ADD * 2) + 1,
	LPROC_OBE_REF_DEL	= (OUT_REF_DEL * 2),
	LPROC_OBE_DECL_REF_DEL	= (OUT_REF_DEL * 2) + 1,
	LPROC_OBE_ATTR_SET	= (OUT_ATTR_SET * 2),
	LPROC_OBE_DECL_ATTR_SET	= (OUT_ATTR_SET * 2) + 1,
	LPROC_OBE_ATTR_GET	= (OUT_ATTR_GET * 2),
	LPROC_OBE_DECL_ATTR_GET	= (OUT_ATTR_GET * 2) + 1,
	LPROC_OBE_XATTR_SET	= (OUT_XATTR_SET * 2),
	LPROC_OBE_DECL_XATTR_SET	= (OUT_XATTR_SET * 2) + 1,
	LPROC_OBE_XATTR_GET	= (OUT_XATTR_GET * 2),
	LPROC_OBE_DECL_XATTR_GET	= (OUT_XATTR_GET * 2) + 1,
	LPROC_OBE_INDEX_LOOKUP	= (OUT_INDEX_LOOKUP * 2),
	LPROC_OBE_DECL_INDEX_LOOKUP	= (OUT_INDEX_LOOKUP * 2) + 1,
	LPROC_OBE_INDEX_INSERT	= (OUT_INDEX_INSERT * 2),
	LPROC_OBE_DECL_INDEX_INSERT	= (OUT_INDEX_INSERT * 2) + 1,
	LPROC_OBE_INDEX_DELETE	= (OUT_INDEX_DELETE * 2),
	LPROC_OBE_DECL_INDEX_DELETE	= (OUT_INDEX_DELETE * 2) + 1,
	LPROC_OBE_WRITE	= (OUT_WRITE * 2),
	LPROC_OBE_DECL_WRITE	= (OUT_WRITE * 2) + 1,
	LPROC_OBE_XATTR_DEL	= (OUT_XATTR_DEL * 2),
	LPROC_OBE_DECL_XATTR_DEL	= (OUT_XATTR_DEL * 2) + 1,
	LPROC_OBE_PUNCH	= (OUT_PUNCH * 2),
	LPROC_OBE_DECL_PUNCH	= (OUT_PUNCH * 2) + 1,

	LPROC_OBE_LOCATE,
	LPROC_OBE_LOCATE_NEW,
	LPROC_OBE_LAST,
};
...
struct obe_device {
	...
	struct rw_semaphore	 obe_sem;
	struct list_head	 obe_env_list;
	struct proc_dir_entry	*obe_proc_entry;
	struct lprocfs_stats    *obe_stats;
};

struct obe_env {
	struct list_head	 obv_list;
	struct task_struct	*obv_task;
	struct lu_env		 obv_env;
};
...
static int obe_execute(const struct lu_env *env, struct obe_device *obe,
			struct obe_tx *obt)
{
	...
	lprocfs_counter_add(obe->obe_stats, LPROC_OBE_START, timediff);
	...
	list_for_each_entry(obu, &obt->obt_updates_list, obu_list) {
		...
		timediff = cfs_timeval_sub(&end, &start, NULL);
		lprocfs_counter_add(obe->obe_stats, type * 2, timediff);
	}
		 ...
		 struct object_update_reply *urep)
{
	...
	for (i = 0; i < ureq->ourq_count; i++) {
		...
		timediff = cfs_timeval_sub(&end, &start, NULL);
		rc = (type == OUT_CREATE ?
			LPROC_OBE_LOCATE_NEW : LPROC_OBE_LOCATE);
		lprocfs_counter_add(obe->obe_stats, rc, timediff);
		...
		do_gettimeofday(&end);
		timediff = cfs_timeval_sub(&end, &start, NULL);
		lprocfs_counter_add(obe->obe_stats, type * 2 + 1,
				    timediff);
...
static struct lu_env *obe_get_env(struct obe_device *obe)
{
	struct obe_env *cur;
	struct lu_env *env = NULL;
	int rc;

	down_read(&obe->obe_sem);
	list_for_each_entry(cur, &obe->obe_env_list, obv_list) {
		if (cur->obv_task == current) {
			env = &cur->obv_env;
			break;
		}
	}
	...
	rc = lu_env_init(&cur->obv_env, LCT_DT_THREAD);
	if (rc) {
		OBD_FREE_PTR(cur);
		return NULL;
	}
	cur->obv_task = current;
	down_write(&obe->obe_sem);
	list_add(&cur->obv_list, &obe->obe_env_list);
	up_write(&obe->obe_sem);
...
struct lprocfs_vars lprocfs_obe_obd_vars[] = { { NULL } };
...
static struct obe_device *obe_dev_prepare(struct dt_device *dt)
{
	...
	obe->obe_osd = dt;
	init_rwsem(&obe->obe_sem);
	INIT_LIST_HEAD(&obe->obe_env_list);
	...
	type = class_search_type(LUSTRE_OSDBENCH_NAME);
	if (type == NULL)
		goto out;

	obe->obe_proc_entry = lprocfs_register("obe", type->typ_procroot,
					       lprocfs_obe_obd_vars,
					       &obe->obe_top_dev);
	if (IS_ERR(obe->obe_proc_entry)) {
		obe->obe_proc_entry = NULL;
		goto out;
	}

	obe->obe_stats = lprocfs_alloc_stats(LPROC_OBE_LAST, 0);
	if (obe->obe_stats != NULL) {
		rc = lprocfs_register_stats(obe->obe_proc_entry, "stats",
					    obe->obe_stats);
		if (rc)
			goto out;
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_START,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"start", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_DECL_CREATE,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"decl_create", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_CREATE,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"create", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_DECL_DESTROY,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"decl_destroy", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_DESTROY,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"destroy", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_DECL_REF_ADD,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"decl_refadd", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_REF_ADD,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"refadd", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_DECL_REF_DEL,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"decl_refdel", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_REF_DEL,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"refdel", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_DECL_ATTR_SET,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"decl_attr_set", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_ATTR_SET,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"attr_set", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_DECL_ATTR_GET,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"decl_attr_get", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_ATTR_GET,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"attr_get", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_DECL_XATTR_SET,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"decl_xattr_set", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_XATTR_SET,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"xattr_set", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_DECL_XATTR_GET,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"decl_xattr_get", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_XATTR_GET,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"xattr_get", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_DECL_INDEX_LOOKUP,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"decl_lookup", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_INDEX_LOOKUP,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"lookup", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_DECL_INDEX_INSERT,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"decl_insert", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_INDEX_INSERT,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"insert", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_DECL_INDEX_DELETE,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"decl_delete", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_INDEX_DELETE,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"delete", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_DECL_WRITE,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"decl_write", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_WRITE,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"write", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_DECL_XATTR_DEL,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"decl_xattr_del", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_XATTR_DEL,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"xattr_del", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_DECL_PUNCH,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"decl_punch", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_PUNCH,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"punch", "usec");

		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_LOCATE,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"locate", "usec");
		lprocfs_counter_init(obe->obe_stats, LPROC_OBE_LOCATE_NEW,
				LPROCFS_CNTR_AVGMINMAX|LPROCFS_CNTR_STDDEV,
				"new", "usec");
	}
...
{
	struct obe_env *cur, *tmp;
	if (obe->obe_stats)
		lprocfs_free_stats(&obe->obe_stats);
	if (obe->obe_proc_entry)
		lprocfs_remove(&obe->obe_proc_entry);
	list_for_each_entry_safe(cur, tmp, &obe->obe_env_list, obv_list) {
		lu_env_fini(&cur->obv_env);
		OBD_FREE_PTR(cur);
	}
