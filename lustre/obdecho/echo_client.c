...
static int echo_md_root_get(const struct lu_env *env,
                                           ...
                                           struct lustre_cfg *cfg)
{
		...
		rc = echo_md_root_get(env, emd, &ed->ed_root_fid);
		if (rc != 0) {
...
static int echo_md_los_fini(const struct lu_env *env, struct obd_device *obddev)
{
	struct echo_device	*ed = obd2echo_dev(obddev);
	ENTRY;
	...
	if (ed->ed_next_ismd) {
#ifdef HAVE_SERVER_SUPPORT
		struct md_device	*md = lu2md_dev(ed->ed_next);
