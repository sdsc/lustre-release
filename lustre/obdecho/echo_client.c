...
echo_md_root_get(const struct lu_env *env, struct echo_md_device *emd,
		 struct local_oid_storage *los, struct lu_fid *root_fid)
{
	struct lu_fid			 fid;
	int				 rc = 0;
	ENTRY;
	...
	fid.f_oid = 1;
	fid.f_ver = 0;
	rc = local_oid_storage_init(env, emd->emd_bottom, &fid, &los);
	if (rc != 0)
		RETURN(rc);

	lu_echo_root_fid(&fid);
	if (echo_md_seq_site(emd)->ss_node_id == 0) {
		rc = echo_md_local_file_create(env, emd, los,
                                           ...
                                           struct lustre_cfg *cfg)
{
		...
		rc = echo_md_root_get(env, emd, ed->ed_los, &ed->ed_root_fid);
		if (rc != 0) {
