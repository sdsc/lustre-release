                                           ...
                                           struct lustre_cfg *cfg)
{
        ...
        if (tgt == NULL) {
                CERROR("Can not find %s device for echo client\n",
        ...
        if (ed->ed_next_ismd) {
                ...
                if (found == 0) {
                        CERROR("%s is not lu device! type %s \n",
                               lustre_cfg_string(cfg, 1), tgt_type_name);
                        GOTO(out, rc = -EINVAL);
                }
                                   ...
                                   struct md_attr *ma)
{
        ...
        rc = seq_client_alloc_fid(ed->ed_cl_seq, fid, env);
        if (rc < 0) {
                CERROR("Can not alloc fid for %s\n", lname->ln_name);
                return rc;
        }
        ...
        ec_child = lu_object_find_at(env, &ed->ed_cl.cd_lu_dev,
                                     fid, NULL);
        if (IS_ERR(ec_child)) {
                CERROR("Can not find childs "DFID"\n", PFID(fid));
                return PTR_ERR(ec_child);
        }

        child = lu_object_locate(ec_child->lo_header, ld->ld_type);
        if (child == NULL) {
                CERROR("Can not locate childs "DFID"\n", PFID(fid));
                GOTO(out_put, rc = -EINVAL);
        }

        CDEBUG(D_INFO, "Start creating object "DFID" %s %p \n",
               PFID(lu_object_fid(&parent->mo_lu)),
               lname->ln_name, parent);

        rc = mdo_create(env, parent, lname, lu2md(child), spec, ma);
        if (rc) {
                CERROR("Can not create child "DFID"\n", PFID(fid));
                GOTO(out_put, rc);
        }
        CDEBUG(D_INFO, "End creating object "DFID" %s %p rc %d\n",
               PFID(lu_object_fid(&parent->mo_lu)),
               lname->ln_name, parent, rc);
out_put:
                             ...
                             int *max_lmm_size)
{
        ...
        if (strcmp(ld->ld_type->ldt_name, LUSTRE_MDD_NAME)) {
                /*FIXME: non-MDD, use fix lmd?*/
                ma->ma_lmm = (void*)&info->eti_lmd;
                ma->ma_lmm_size = sizeof(info->eti_lmd);
                *max_lmm_size = 0;
                RETURN(0);
        }
                                 ...
                                 int stripe_count, int stripe_offset)
{
        ...
        if (name != NULL) {
                ...
                rc = echo_md_create_internal(env, ed, lu2md(parent), lname,
                                             spec, ma);
                GOTO(out_free, rc);
        }
                               ...
                               __u64 id, int count)
{
        ...
        for (i = 0; i < count; i++) {
                ...
                if (IS_ERR(ec_child)) {
                        CERROR("Can't find child %s\n", lname->ln_name);
                        RETURN(PTR_ERR(ec_child));
                }

                child = lu_object_locate(ec_child->lo_header, ld->ld_type);
                if (child == NULL) {
                        CERROR("Can not locate childs %s\n", lname->ln_name);
                        lu_object_put(env, ec_child);
                        rc = -EINVAL;
                        break;
                }

                CDEBUG(D_INFO, "Start getattr object "DFID"\n",
                       PFID(lu_object_fid(child)));
                rc = mo_attr_set(env, lu2md(child), ma);
                if (rc) {
                        CERROR("Can not getattr child "DFID"\n",
                                PFID(lu_object_fid(child)));
                        lu_object_put(env, ec_child);
                        break;
                }
                CDEBUG(D_INFO, "End getattr object "DFID"\n",
                       PFID(lu_object_fid(child)));
                id ++;
                lu_object_put(env, ec_child);
        }
                              ...
                              __u64 id, int count)
{
        ...
        for (i = 0; i < count; i++) {
                ...
                CDEBUG(D_INFO, "Start lookup object "DFID" %s %p \n",
                       PFID(lu_object_fid(parent)), lname->ln_name, parent);
                rc = mdo_lookup(env, lu2md(parent), lname, fid, NULL);
                if (rc) {
                        CERROR("Can not lookup child %s\n", name);
                        break;
                }
                CDEBUG(D_INFO, "End lookup object "DFID" %s %p \n",
                       PFID(lu_object_fid(parent)), lname->ln_name, parent);
                                    ...
                                    struct md_attr *ma)
{
        ...
        CDEBUG(D_INFO, "Start destroy object "DFID" %s %p \n",
               PFID(lu_object_fid(&parent->mo_lu)),
               lname->ln_name, parent);
        ...
        if (rc) {
                lu_object_put(env, ec_child);
                CERROR("Can not unlink child %s\n", lname->ln_name);
                GOTO(out_put, rc);
        }
        CDEBUG(D_INFO, "End destroy object "DFID" %s %p \n",
               PFID(lu_object_fid(&parent->mo_lu)),
               lname->ln_name, parent);
out_put:
        lu_object_put(env, ec_child);
                                    ...
                                    int path_len)
{
        ...
        if (rc) {
                CERROR("get root error %d\n", rc);
                RETURN(ERR_PTR(rc));
        }

        parent = lu_object_find_at(env, &ed->ed_cl.cd_lu_dev, fid, NULL);
        if (IS_ERR(parent)) {
                CERROR("Can not find the parent "DFID"\n", PFID(fid));
                RETURN(parent);
        }
        ...
        while (1) {
                ...
                lu_object_put(env, parent);
                if (IS_ERR(child)) {
                        CERROR("lookup %s under parent "DFID" error %d\n",
                           ...
                           struct obd_ioctl_data *data)
{
        ...
        if (IS_ERR(parent)) {
                CERROR("Can not resolve the path %s\n", path);
                cl_env_put(env, &refcheck);
                RETURN(PTR_ERR(parent));
        }
        ...
        case ECHO_MD_MKDIR: {
                ...
                rc = echo_create_md_object(env, ed, parent, name, namelen,
...
static int echo_client_setup(const struct lu_env *env, struct obd_device *obddev,
                             struct lustre_cfg *lcfg)
{
