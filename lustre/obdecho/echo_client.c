			  ...
			  const struct cl_io *unused)
{
	...
	OBD_SLAB_ALLOC_PTR_GFP(el, echo_lock_kmem, CFS_ALLOC_NOFS);
	if (el != NULL) {
					   ...
					   struct lu_device *dev)
{
	...
	/* we're the top dev. */
	LASSERT(hdr == NULL);
	OBD_SLAB_ALLOC_PTR_GFP(eco, echo_object_kmem, CFS_ALLOC_NOFS);
	if (eco != NULL) {
				  ...
				  struct lu_context_key *key)
{
	...
	OBD_SLAB_ALLOC_PTR_GFP(info, echo_thread_kmem, CFS_ALLOC_NOFS);
	if (info == NULL)
		info = ERR_PTR(-ENOMEM);
	return info;
}
				   ...
				   struct lu_context_key *key)
{
	...
	OBD_SLAB_ALLOC_PTR_GFP(session, echo_session_kmem, CFS_ALLOC_NOFS);
	if (session == NULL)
