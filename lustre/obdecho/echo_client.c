                             ...
                             struct obd_device *obddev, struct lustre_cfg *lcfg)
{
        ...
        if (!strcmp(tgt->obd_type->typ_name, LUSTRE_MDT_NAME)) {
                lu_update_ctx_tags(LCT_REMEMBER | LCT_NOREF | LCT_MD_THREAD);
                lu_update_ses_tags(LCT_SESSION | LCT_REMEMBER | LCT_NOREF);
                RETURN(0);
        }
...
static int echo_client_cleanup(struct obd_device *obddev)
{
        ...
        if (ed->ed_next_ismd) {
                lu_clear_ctx_tags(LCT_REMEMBER | LCT_NOREF | LCT_MD_THREAD);
                lu_clear_ses_tags(LCT_SESSION | LCT_REMEMBER | LCT_NOREF);
                RETURN(0);
        }
