                           ...
                           struct obd_ioctl_data *data)
{
        ...
        env = cl_env_refill(env, ECHO_MD_CTX_TAG, ECHO_MD_SES_TAG);
        if (IS_ERR(env)) {
                cl_env_put(env, &refcheck);
                RETURN(PTR_ERR(env));
        }
                             ...
                             struct obd_device *obddev, struct lustre_cfg *lcfg)
{
        ...
        if (!strcmp(tgt->obd_type->typ_name, LUSTRE_MDT_NAME)) {
                lu_update_ctx_tags(ECHO_MD_CTX_TAG);
                lu_update_ses_tags(ECHO_MD_SES_TAG);
                RETURN(0);
        }
...
static int echo_client_cleanup(struct obd_device *obddev)
{
        ...
        if (ed->ed_next_ismd) {
                lu_clear_ctx_tags(ECHO_MD_CTX_TAG);
                lu_clear_ses_tags(ECHO_MD_SES_TAG);
                RETURN(0);
        }
