...
int sptlrpc_enc_pool_get_pages(struct ptlrpc_bulk_desc *desc)
{
	...
	if (desc->bd_u.bd_kiov.bd_enc_vec != NULL)
		return 0;

	OBD_ALLOC(desc->bd_u.bd_kiov.bd_enc_vec,
		  desc->bd_iov_count * sizeof(*desc->bd_u.bd_kiov.bd_enc_vec));
	if (desc->bd_u.bd_kiov.bd_enc_vec == NULL)
		return -ENOMEM;
...
again:
	if (unlikely(page_pools.epp_free_pages < desc->bd_iov_count)) {
        ...
        for (i = 0; i < desc->bd_iov_count; i++) {
                LASSERT(page_pools.epp_pools[p_idx][g_idx] != NULL);
                desc->bd_u.bd_kiov.bd_enc_vec[i].kiov_page =
...
void sptlrpc_enc_pool_put_pages(struct ptlrpc_bulk_desc *desc)
{
	...
	for (i = 0; i < desc->bd_iov_count; i++) {
		LASSERT(desc->bd_u.bd_kiov.bd_enc_vec[i].kiov_page != NULL);
	...
	OBD_FREE(desc->bd_u.bd_kiov.bd_enc_vec,
		 desc->bd_iov_count * sizeof(*desc->bd_u.bd_kiov.bd_enc_vec));
	desc->bd_u.bd_kiov.bd_enc_vec = NULL;
}
