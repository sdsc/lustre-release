...
void ptlrpc_fill_bulk_md (lnet_md_t *md, struct ptlrpc_bulk_desc *desc, int mdcnt)
{
        CLASSERT (PTLRPC_MAX_BRW_PAGES < LI_POISON);

        LASSERT (mdcnt < PTLRPC_BULK_OPS_SIZE);
        LASSERT (desc->bd_iov_count <= PTLRPC_MAX_BRW_PAGES);
        LASSERT (!(md->options & (LNET_MD_IOVEC | LNET_MD_KIOV | LNET_MD_PHYS)));

        md->options |= LNET_MD_KIOV;
        if (desc->bd_enc_iov)
                md->start = &desc->bd_enc_iov[mdcnt * LNET_MAX_IOV];
        else
                md->start = &desc->bd_iov[mdcnt * LNET_MAX_IOV];
        md->length = MAX(0, desc->bd_iov_count - mdcnt * LNET_MAX_IOV);
        md->length = MIN(LNET_MAX_IOV, md->length);
}
...
{
        LASSERT (mdcnt < PTLRPC_BULK_OPS_SIZE);
        LASSERT (desc->bd_iov_count > mdcnt * LNET_MAX_IOV);
        LASSERT (!(md->options & (LNET_MD_IOVEC | LNET_MD_KIOV | LNET_MD_PHYS)));
        ...
        md->start = &desc->bd_iov[mdcnt * LNET_MAX_IOV];
        md->length = MIN(LNET_MAX_IOV, desc->bd_iov_count - mdcnt * LNET_MAX_IOV);
}
