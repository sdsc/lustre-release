...
/* lu_nodemap flags */
enum nm_flag_shifts {
	NM_FL_ROOT = 0,		/* allow root access */
	NM_FL_TRUST = 1,	/* trust client ids */
};
				     ...
				     struct lu_nodemap *nodemap)
{
	...
	nr->ncr.ncr_flags = nodemap->nmf_trust_client_ids << NM_FL_TRUST |
...
static void nodemap_idmap_rec_init(union nodemap_rec *nr, const __u32 id_fs)
{
...
static void nodemap_global_rec_init(union nodemap_rec *nr, const bool active)
{
...
typedef int (*nm_idx_cb_t)(struct lu_env *env, struct dt_object *idx,
			       struct nodemap_key *nk, union nodemap_rec *nr);
 ...
 * \retval
 */
static enum nm_idx_type nodemap_process_keyrec(struct nodemap_config *config,
					       struct nodemap_key *key,
					       union nodemap_rec *rec,
					       struct lu_nodemap **recent_nm)
{
	struct lu_nodemap	*nodemap = NULL;
	enum nm_idx_type	 type;
	__u32			 nodemap_id;
	lnet_nid_t		 nid[2];
	__u32			 map[2];
	int			 rc;
	...
	if (type == NODEMAP_RANGE_IDX || type == NODEMAP_UIDMAP_IDX ||
		...
		if (nodemap == NULL) {
			CWARN("can't find nodemap for range/idmap, exiting\n");
			GOTO(out, rc = -EINVAL);
		}
	...
	case NODEMAP_CLUSTER_IDX:
		...
		nodemap->nmf_allow_root_access =
				rec->ncr.ncr_flags & (1 << NM_FL_ROOT);
		nodemap->nmf_trust_client_ids =
				rec->ncr.ncr_flags & (1 << NM_FL_TRUST);
	...
	case NODEMAP_GLOBAL_IDX:
		config->nmc_nodemap_active = rec->ngr.ngr_is_active;
		break;
	default:
