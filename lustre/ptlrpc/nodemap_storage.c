...
#define NM_FL_ENC	0
#define NM_FL_HMAC	1
#define NM_FL_BLOCK	2
#define NM_FL_ROOT	3
#define NM_FL_TRUST	4
				     ...
				     struct lu_nodemap *nodemap)
{
	...
	nr->val.n.flags = nodemap->nmf_trust_client_ids << NM_FL_TRUST |
		nodemap->nmf_allow_root_access << NM_FL_ROOT |
		nodemap->nmf_block_lookups << NM_FL_BLOCK |
		nodemap->nmf_hmac_required << NM_FL_HMAC |
		nodemap->nmf_encryption_required << NM_FL_ENC;
}
...
{
	nk->uid.global = cpu_to_le64(nm_idx_set_type(0, NODEMAP_GLOBAL_IDX));
}

static void nodemap_prep_global_rec(struct nodemap_rec *nr, const bool active)
{
	nr->val.g.is_active = active;
}
...
typedef int (*nm_idx_action_t)(struct lu_env *env, struct dt_object *idx,
			       struct nodemap_key *nk, struct nodemap_rec *nr);

/**
 * Iterates through all the registered nodemap_config_files and calls the
 * given action with the ncf as a parameter, as well as the given key and rec.
 *
 * \param	action_f	action function to call
 * \param	nk		key of the record to act upon
 * \param	nr		record to act upon, NULL for the delete action
 */
static int nodemap_idx_action(nm_idx_action_t action_f, struct nodemap_key *nk,
			      struct nodemap_rec *nr)
{
	...
	list_for_each_entry(ncf, &ncf_list_head, ncf_list) {
		rc = action_f(&env, ncf->ncf_obj, nk, nr);
		if (rc != 0) {
			CWARN("%s: error writing to nodemap config: %d\n",
				  ...
				  struct lu_nodemap **recent_nodemap)
{
	...
	CWARN("found rec type %d, nodemap_id %d\n", type, nodemap_id);
	...
	if (type == NODEMAP_RANGE_IDX || type == NODEMAP_UIDMAP_IDX ||
		...
		if (nodemap == NULL) {
			CWARN("couldn't find nodemap for range/idmap, exiting\n");
			GOTO(out, rc = -EINVAL);
		}
	...
	case NODEMAP_CLUSTER_IDX:
		...
		if (nodemap == NULL) {
			if (nodemap_id == LUSTRE_NODEMAP_DEFAULT_ID) {
				nodemap = nodemap_create(rec->val.n.name,
							 config, 1);
			...
			CWARN("nodemap %s exists with id %d, loading id %d\n",
				nodemap->nm_name, nodemap->nm_id, nodemap_id);
			GOTO(out, rc = -EINVAL);
		}

		nodemap->nm_squash_uid = le32_to_cpu(rec->val.n.squash_uid);
		nodemap->nm_squash_gid = le32_to_cpu(rec->val.n.squash_gid);
		nodemap->nmf_encryption_required = rec->val.n.flags &
							(1 << NM_FL_ENC);
		nodemap->nmf_hmac_required = rec->val.n.flags &
							(1 << NM_FL_HMAC);
		nodemap->nmf_block_lookups = rec->val.n.flags &
							(1 << NM_FL_BLOCK);
		nodemap->nmf_allow_root_access = rec->val.n.flags &
							(1 << NM_FL_ROOT);
		nodemap->nmf_trust_client_ids = rec->val.n.flags &
							(1 << NM_FL_TRUST);
	...
	case NODEMAP_GLOBAL_IDX:
		config->nmc_nodemap_active = rec->val.g.is_active;
		break;
	default:
