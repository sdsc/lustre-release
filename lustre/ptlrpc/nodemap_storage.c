				     ...
				     const struct lu_nodemap *nodemap)
{
	...
	nr->ncr.ncr_flags = cpu_to_le32(
		nodemap->nmf_trust_client_ids << NM_FL_TRUST |
...
static void nodemap_idmap_rec_init(union nodemap_rec *nr, __u32 id_fs)
{
...
static void nodemap_range_rec_init(union nodemap_rec *nr, lnet_nid_t nid[2])
{
				  ...
				  struct lu_nodemap **recent_nodemap)
{
	...
	case NODEMAP_CLUSTER_IDX:
		...
		if (nodemap == NULL) {
			...
			CWARN("error loading nodemap %s, id mismatch\n", rec->ncr.ncr_name);
		...
		nodemap->nmf_allow_root_access = flags & (1 << NM_FL_ROOT);
		nodemap->nmf_trust_client_ids = flags & (1 << NM_FL_TRUST);
