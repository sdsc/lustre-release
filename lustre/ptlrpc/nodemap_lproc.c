...
/**
 * Clear the fileset of nodemap.
 *
 * \param[in] file      proc file
 * \param[in] buffer    string, "<nodemap name>"
 * \param[in] count     \a buffer length
 * \param[in] off       unused
 * \retval              \a count on success
 * \retval              negative number on error
 */
static ssize_t
lprocfs_clear_nodemap_fileset_seq_write(struct file *file,
					const char __user *buffer,
					size_t count, loff_t *off)
{
...
LPROC_SEQ_FOPS_WO_TYPE(nodemap, clear_nodemap_fileset);
...
static struct lprocfs_vars lprocfs_nm_module_vars[] = {
	...
	{
		.name		= "clear_nodemap_fileset",
		.fops		= &nodemap_clear_nodemap_fileset_fops,
	},
	{
