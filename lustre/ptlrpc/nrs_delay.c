 ...
 */
static int delay_req_compare(cfs_binheap_node_t *e1, cfs_binheap_node_t *e2)
{
...
static cfs_binheap_ops_t nrs_delay_heap_ops = {
	.hop_enter	= NULL,
	.hop_exit	= NULL,
					     ...
					     bool peek, bool force)
{
	...
	if (likely(nrq != NULL)) {
		...
		if (cfs_time_current_sec() < nrq->nr_u.delay.req_start_time) {
			nrq = NULL;
		} else if (likely(!peek)) {
			     ...
			     struct ptlrpc_nrs_request *nrq)
{
	...
	nrq->nr_u.delay.req_start_time = cfs_time_current_sec() + cfs_rand() %
			       ...
			       struct ptlrpc_nrs_request *nrq)
{
	...
	CDEBUG(D_RPCTRACE, "NRS: finished handling delayed request from %s,"
			   "after a delay of %ld seconds\n",
			 ...
			 enum ptlrpc_nrs_ctl opc, void *arg)
{
	...
	case NRS_CTL_DELAY_RD_MIN: {
		...
		*(__u32 *)arg = delay_data->min_delay;
		}
		break;

	case NRS_CTL_DELAY_WR_MIN: {
		struct nrs_delay_data	*delay_data = policy->pol_private;
		if (*(__u32 *)arg > delay_data->max_delay)
			RETURN(-EINVAL);

		delay_data->min_delay = *(__u32 *)arg;
		}
		break;
	...
	case NRS_CTL_DELAY_RD_MAX: {
		...
		*(__u32 *)arg = delay_data->max_delay;
		}
		break;

	case NRS_CTL_DELAY_WR_MAX: {
		struct nrs_delay_data	*delay_data = policy->pol_private;
		if (*(__u32 *)arg < delay_data->min_delay)
			RETURN(-EINVAL);

		delay_data->max_delay = *(__u32 *)arg;
		}
		break;
	...
	case NRS_CTL_DELAY_RD_PCT: {
		...
		*(__u32 *)arg = delay_data->delay_pct;
		}
		break;

	case NRS_CTL_DELAY_WR_PCT: {
		struct nrs_delay_data	*delay_data = policy->pol_private;
		if (*(__u32 *)arg < 0 || *(__u32 *)arg > 100)
			RETURN(-EINVAL);

		delay_data->delay_pct = *(__u32 *)arg;
		}
		break;
	}
	RETURN(0);
 ...
 */
static ssize_t
lprocfs_nrs_delay_seq_write_common(const char *buffer, unsigned int bufsize,
				   size_t count, const char *var_name,
				   ...
				   enum ptlrpc_nrs_ctl opc, bool single)
{
	...
	strncpy(tmp, "reg_", tmpsize);
	strncat(tmp, var_name, tmpsize - strlen(tmp) - 1);
	count_copy = count;
	val_str = lprocfs_find_named_value(kernbuf, tmp, &count_copy);
	if (val_str != kernbuf) {
		val_reg = simple_strtol(val_str, NULL, 10);
		queue |= PTLRPC_NRS_QUEUE_REG;
	}

	/* look for "hp_<var_name>" in kernbuf */
	strncpy(tmp, "hp_", tmpsize);
	strncat(tmp, var_name, tmpsize - strlen(tmp) - 1);
	count_copy = count;
	val_str = lprocfs_find_named_value(kernbuf, tmp, &count_copy);
	if (val_str != kernbuf) {
		if (!nrs_svc_has_hp(svc))
			GOTO(free_bufs, rc = -ENODEV);

		val_hp = simple_strtol(val_str, NULL, 10);
	...
	if (queue == 0) {
		...
		val_reg = simple_strtol(kernbuf, NULL, 10);
	...
	if ((queue & PTLRPC_NRS_QUEUE_REG) != 0) {
		if (val_reg < min_val || val_reg > max_val)
			GOTO(free_bufs, rc = -EINVAL);
...
free_bufs:
	...
	return rc == -ENODEV && rc2 == -ENODEV ? -ENODEV : rc;
}
 ...
 * request minimum delay on all PtlRPC serivices to 2 seconds, and
 *
 ...
 */
static ssize_t
ptlrpc_lprocfs_nrs_delay_min_seq_write(struct file *file, const char *buffer,
 ...
 * request maximum delay on all PtlRPC serivices to 10 seconds, and
 *
 ...
 */
static ssize_t
ptlrpc_lprocfs_nrs_delay_max_seq_write(struct file *file, const char *buffer,
				       size_t count, loff_t *off)
{
	struct seq_file			*m = file->private_data;
	struct ptlrpc_service		*svc = m->private;
 ...
 */
static ssize_t
ptlrpc_lprocfs_nrs_delay_pct_seq_write(struct file *file, const char *buffer,
				       size_t count, loff_t *off)
{
	struct seq_file		*m = file->private_data;
	struct ptlrpc_service	*svc = m->private;
