				  ...
				  bool is_default)
{
	...
	if (is_default || default_nodemap == NULL) {
		...
		nodemap->nmf_deny_squashed_access = 0;
		...
		nodemap->nmf_deny_squashed_access =
				default_nodemap->nmf_deny_squashed_access;
...
/**
 * Set the nmf_deny_squashed_access flag to true or false.
 * \param	name		nodemap name
 * \param	deny_squashed	if true, squashed users will get EACCES
 * \retval	0 on success
 *
 */
int nodemap_set_deny_squashed(const char *name, bool deny_squashed)
{
	...
	nodemap->nmf_deny_squashed_access = deny_squashed;
...
EXPORT_SYMBOL(nodemap_set_deny_squashed);
