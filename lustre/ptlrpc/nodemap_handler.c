 ...
 * \param	save_to_index	should be false when loading range from index
 * \retval	0		success
 * \retval	-ENOMEM
 *
 */
...
int nodemap_add_range(const char *name, const lnet_nid_t nid[2])
{
	...
	rc = nodemap_add_range_helper(active_config, nodemap, nid, 1);
	mutex_unlock(&active_config_lock);
	nodemap_putref(nodemap);
out:
	return rc;
...
{
	/* nodemap_destroy takes active_config_lock, can't lock here */
 ...
 *
 * \param      nid             nid to classify
 * \param[out] user_buf        buffer to write the nodemap name to
 * \param      user_buf_len    length of buffer to write into
 *
 * \retval     0               success
 * \retval     non-zero        error copying nodemap name to buffer
 */
