...
/**
 * Returns true if clients in this nodemap are allowed to use setfacl.
 * Currently only nodemaps with the trusted and admin flags are allowed.
 *
 * \param	lu_nodemap	nodemap to check
 * \retval	0 if nodemap not allowed to setfacl
 */
bool nodemap_allow_setfacl(const struct lu_nodemap *nodemap)
{
	return nodemap->nmf_trust_client_ids && nodemap->nmf_allow_root_access;
}
EXPORT_SYMBOL(nodemap_allow_setfacl);
 ...
 * \retval	new size of ACLs, or -EINVAL on a bad \a size parameter
 */
size_t nodemap_map_acl(struct lu_nodemap *nodemap, struct lu_buf *buf,
		    enum nodemap_tree_type tree_type, size_t size)
{
	posix_acl_xattr_header	*header = (posix_acl_xattr_header *)buf->lb_buf;
