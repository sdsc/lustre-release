		     ...
		     enum nodemap_tree_type tree_type, __u32 id)
{
	...
	CWARN("nodemap info: %s(id=%d, p=%p) trust %d admin %d squash u %u, g %d\n",
		nodemap->nm_name, nodemap->nm_id, nodemap,
...
int nodemap_set_allow_root(const char *name, bool allow_root)
{
	...
	CWARN("updating nodemap %s (id=%d,p=%p) admin %d, was %d\n",
		nodemap->nm_name, nodemap->nm_id, nodemap, allow_root,
		nodemap->nmf_allow_root_access);
