...
static void
ptlrpc_nrs_req_add_nolock(struct ptlrpc_service_part *svcpt,
 ...
 */
static void
ptlrpc_nrs_hpreq_add_nolock(struct ptlrpc_service_part *svcpt,
			    struct ptlrpc_request *req)
{
	ENTRY;
	LASSERT(svcpt != NULL);
	...
	if (req->rq_hp == 0) {
		...
		req->rq_hp = 1;
		ptlrpc_nrs_req_add_nolock(svcpt, req);
		if (opc != OBD_PING)
...
static int
ptlrpc_nrs_policy_register(struct ptlrpc_service_part *svcpt,
			   ...
			   unsigned long flags, void *arg)
{
	...
	case PTLRPC_NRS_QUEUE_REG:
		rc = nrs_policy_register(ptlrpc_svcpt2nrs(svcpt, 0),
					 desc, flags, arg);
		if (rc != 0 || queue == PTLRPC_NRS_QUEUE_REG)
			break;

	case PTLRPC_NRS_QUEUE_HP:
		rc = nrs_policy_register(ptlrpc_svcpt2nrs(svcpt, 1),
					 desc, flags, arg);
		break;
	}
...
static struct ptlrpc_nrs_pol_desc ptlrpc_nrs_crr_desc;
static struct ptlrpc_nrs_pol_desc ptlrpc_nrs_crrn_desc;
static struct ptlrpc_nrs_pol_desc ptlrpc_nrs_orr_desc;
static struct ptlrpc_nrs_pol_desc ptlrpc_nrs_trr_desc;

static struct ptlrpc_nrs_pol_desc_type nrs_pol_std_type[] = {
	{ &ptlrpc_nrs_fifo_desc,	PTLRPC_NRS_TYPE_FIFO },
	{ &ptlrpc_nrs_crr_desc,		PTLRPC_NRS_TYPE_CRR },
	{ &ptlrpc_nrs_crrn_desc,	PTLRPC_NRS_TYPE_CRRN },
	{ &ptlrpc_nrs_orr_desc,		PTLRPC_NRS_TYPE_ORR },
	{ &ptlrpc_nrs_trr_desc,		PTLRPC_NRS_TYPE_TRR },
	{ NULL,				PTLRPC_NRS_TYPE_LAST },
};

static inline struct ptlrpc_nrs_pol_desc *
nrs_type2desc(enum ptlrpc_nrs_pol_type type)
{
	__u8	i;

	for(i = 0; nrs_pol_std_type[i].ptd_type != PTLRPC_NRS_TYPE_LAST; i++)
		if (nrs_pol_std_type[i].ptd_type == type)
			return nrs_pol_std_type[i].ptd_desc;
	LBUG();
}

static int
ptlrpc_nrs_register_policies(struct ptlrpc_service_part *svcpt,
			     enum ptlrpc_nrs_queue_type queue)
{
	int				rc;
	unsigned long			flags;
	...
	for (type = PTLRPC_NRS_TYPE_FIRST; type < PTLRPC_NRS_TYPE_LAST;
	     type <<= 1) {
		if (svcpt->scp_nrs_supported & type) {
			if (type == PTLRPC_NRS_TYPE_FIFO)
				flags = PTLRPC_NRS_FL_FALLBACK |
					PTLRPC_NRS_FL_REG_START;
			else
				flags = 0;

			rc = ptlrpc_nrs_policy_register(svcpt, queue,
							nrs_type2desc(type),
							flags, NULL);
			if (rc != 0)
				GOTO(failed, rc);
...
static int
ptlrpc_nrs_policy_unregister(struct ptlrpc_service_part *svcpt,
			     enum ptlrpc_nrs_queue_type queue, int type)
{
	...
	case PTLRPC_NRS_QUEUE_BOTH:
	case PTLRPC_NRS_QUEUE_REG:
		rc = nrs_policy_unregister(ptlrpc_svcpt2nrs(svcpt, 0), type);
		if (queue == PTLRPC_NRS_QUEUE_REG || rc != 0)
			break;

	case PTLRPC_NRS_QUEUE_HP:
		rc = nrs_policy_unregister(ptlrpc_svcpt2nrs(svcpt, 1), type);
		break;
	}
		       ...
		       enum ptlrpc_nrs_queue_type queue)
{
	...
	case PTLRPC_NRS_QUEUE_REG:
		...
		if (queue == PTLRPC_NRS_QUEUE_REG)
			break;
	case PTLRPC_NRS_QUEUE_HP:
		OBD_CPT_ALLOC_PTR(svcpt->scp_hpreq_nrs,
				  svcpt->scp_service->srv_cptable,
				  svcpt->scp_cpt);
		if (svcpt->scp_hpreq_nrs == NULL)
			RETURN(-ENOMEM);
	...
	/* Register NRS policies; this uses \a ptlrpc_service::scp_nrs_supported
	 * so should happen after it has been set */
	rc = ptlrpc_nrs_register_policies(svcpt, queue);
	RETURN(rc);
}
...
ptlrpc_svcpt_nrs_cleanup(struct ptlrpc_service_part *svcpt)
{
	...
	while (!cfs_list_empty(&nrs->nrs_policy_list)) {
		...
		rc = ptlrpc_nrs_policy_unregister(svcpt, PTLRPC_NRS_QUEUE_REG,
						  policy->pol_type);
		LASSERT(rc == 0);
	}

	if (ptlrpc_nrs_svcpt_has_hp(svcpt)) {
		nrs = ptlrpc_svcpt2nrs(svcpt, 1);
		nrs->nrs_stopping = 1;
		while (!cfs_list_empty(&nrs->nrs_policy_list)) {
			policy = cfs_list_entry(nrs->nrs_policy_list.next,
						...
						pol_list);
			rc = ptlrpc_nrs_policy_unregister(svcpt,
							  PTLRPC_NRS_QUEUE_HP,
							  policy->pol_type);
			LASSERT(rc == 0);
		}
		OBD_FREE_PTR(svcpt->scp_hpreq_nrs);
	}
}
...
{
	int		cptid = ptlrpc_nrspol2cptid(policy);
	char		cptidstr[4];

	strcpy(orrd->od_objname, policy->pol_type == PTLRPC_NRS_TYPE_ORR ?
			...
			struct ptlrpc_service_conf *conf)
{
	...
	ptlrpc_service_for_each_part(svcpt, i, svc) {
		svcpt->scp_nrs_supported = conf->psc_nrs.nc_supp;
		rc = ptlrpc_svcpt_nrs_setup(svcpt, conf->psc_nrs.nc_type);
		if (rc != 0) {
			ptlrpc_server_nrs_cleanup(svc);
			break;
		}
	}
	RETURN(rc);
...
ptlrpc_server_nrs_cleanup(struct ptlrpc_service *svc)
{
	...
	/* Clean up NRS heads on all service CPTs */
	ptlrpc_service_for_each_part(svcpt, i, svc)
			  ...
			  struct ptlrpc_request *req)
{
	...
	nrs_resource_get_safe(ptlrpc_svcpt2nrs(svcpt, req->rq_hp),
		   ...
		   struct ptlrpc_request *req, __u8 hp)
{
	...
	if (!nrq->nr_enqueued) {
		if (hp)
			ptlrpc_nrs_hpreq_add_nolock(svcpt, req);
		else
			ptlrpc_nrs_req_add_nolock(svcpt, req);
	}
			  ...
			  enum ptlrpc_nrs_ctl opc, void *arg)
{
	...
	case PTLRPC_NRS_QUEUE_BOTH:
	case PTLRPC_NRS_QUEUE_REG:
		rc = nrs_policy_ctl(ptlrpc_svcpt2nrs(svcpt, 0), type, opc,
				    arg);
		if (rc != 0 || queue == PTLRPC_NRS_QUEUE_REG)
			break;

	case PTLRPC_NRS_QUEUE_HP:
		rc = nrs_policy_ctl(ptlrpc_svcpt2nrs(svcpt, 1), type, opc,
				    arg);
		break;
	}
...
nrs_crrn_start(struct ptlrpc_nrs_policy *policy)
{
	...
	/* TODO: Change names for each hash on different service/CPTs */
