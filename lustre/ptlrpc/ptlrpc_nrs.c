...
static int
nrs_policy_ctl_locked(ptlrpc_nrs_policy_t *policy, unsigned opc, void *arg)
{
...
static void
nrs_policy_stop_final(ptlrpc_nrs_policy_t *policy)
{
	ptlrpc_nrs_t *nrs = policy->pol_nrs;
...
static int
nrs_policy_stop_locked(ptlrpc_nrs_policy_t *policy)
{
	ptlrpc_nrs_t *nrs = policy->pol_nrs;
...
static int
nrs_policy_start_locked(ptlrpc_nrs_policy_t *policy)
{
	ptlrpc_nrs_t *nrs = policy->pol_nrs;
	int	      rc = 0;
	...
	if (policy->pol_flags & PTLRPC_NRS_FL_FALLBACK) {
		...
		if (nrs->nrs_policy_primary != NULL) {
			ptlrpc_nrs_policy_t *tmp = nrs->nrs_policy_primary;
...
static void
nrs_policy_get_locked(ptlrpc_nrs_policy_t *policy)
{
	policy->pol_ref++;
}

static ptlrpc_nrs_policy_t *
nrs_policy_find_locked(ptlrpc_nrs_t *nrs, int type)
{
...
static void
nrs_policy_put_locked(ptlrpc_nrs_policy_t *policy)
{
	LASSERT(policy->pol_ref > 0);
...
static void
nrs_policy_put(ptlrpc_nrs_policy_t *policy)
{
...
static void
nrs_resource_put(ptlrpc_nrs_resource_t *res)
{
	ptlrpc_nrs_policy_t *policy = res->res_policy;

	/* not always have op_res_put */
	if (policy->pol_ops->op_res_put != NULL) {
		ptlrpc_nrs_resource_t *parent;
...
static ptlrpc_nrs_resource_t *
nrs_resource_get(ptlrpc_nrs_policy_t *policy, ptlrpc_nrs_request_t *nrq)
{
	ptlrpc_nrs_resource_t *res = NULL;
	ptlrpc_nrs_resource_t *tmp = NULL;
	int		       rc;
...
static void
nrs_resource_get_safe(ptlrpc_nrs_t *nrs, ptlrpc_nrs_request_t *nrq,
		      ptlrpc_nrs_resource_t **resp)
{
	ptlrpc_nrs_policy_t   *primary	= NULL;
	ptlrpc_nrs_policy_t   *fallback = NULL;
...
static void
nrs_resource_put_safe(ptlrpc_nrs_resource_t **resp)
{
	ptlrpc_nrs_policy_t *pols[NRS_RES_MAX];
	ptlrpc_nrs_t	    *nrs = NULL;
	int		     i;
...
static ptlrpc_nrs_resource_t *
nrs_request_resource(ptlrpc_nrs_request_t *nrq)
{
...
static ptlrpc_nrs_policy_t *
nrs_request_policy(ptlrpc_nrs_request_t *nrq)
{
...
static void
nrs_request_enqueue(ptlrpc_nrs_request_t *nrq)
{
...
static ptlrpc_nrs_policy_t *
nrs_request_dequeue(ptlrpc_nrs_request_t *nrq)
{
	ptlrpc_nrs_policy_t *policy;
...
static void
nrs_request_start(ptlrpc_nrs_request_t *nrq)
{
	ptlrpc_nrs_policy_t *policy = nrs_request_policy(nrq);
...
static void
nrs_request_stop(ptlrpc_nrs_request_t *nrq)
{
	ptlrpc_nrs_policy_t *policy = nrs_request_policy(nrq);
...
static int
nrs_policy_ctl(ptlrpc_nrs_t *nrs, int type, unsigned int opc, void *arg)
{
	ptlrpc_nrs_policy_t	 *policy;
	ptlrpc_nrs_policy_info_t *info;
	int    rc = 0;
	...
	case PTLRPC_NRS_CTL_GET_INFO:
		...
		info = (ptlrpc_nrs_policy_info_t *)arg;
	...
	case PTLRPC_NRS_CTL_ORR_RD_QUANTUM: {
		struct ptlrpc_service	*svc = nrs->nrs_service;
		nrs_orr_data_t		*orrd = policy->pol_private;
		nrs_orr_info_t		*info = (nrs_orr_info_t *)arg;
	...
	case PTLRPC_NRS_CTL_ORR_WR_QUANTUM: {
		...
		cfs_spin_lock(&svc->srv_rq_lock);
		orrd->od_quantum = *(nrs_orr_quantum_t *)arg;
		cfs_spin_unlock(&svc->srv_rq_lock);
		}
		break;
...
static int
nrs_policy_unregister(ptlrpc_nrs_t *nrs, int type)
{
...
static int
nrs_policy_register(ptlrpc_nrs_t *nrs,
		    ptlrpc_nrs_policy_desc_t *desc,
...
{
	ptlrpc_nrs_t	     *nrs = ptlrpc_server_nrs(svc, hp);
	ptlrpc_nrs_policy_t  *policy;
	ptlrpc_nrs_request_t *nrq;
...
static ptlrpc_nrs_policy_desc_t ptlrpc_nrs_fifo_desc;
static ptlrpc_nrs_policy_desc_t ptlrpc_nrs_crr_desc;
static ptlrpc_nrs_policy_desc_t ptlrpc_nrs_crr2_desc;
static ptlrpc_nrs_policy_desc_t ptlrpc_nrs_orr_desc;
static ptlrpc_nrs_policy_desc_t ptlrpc_nrs_trr_desc;
			     ...
			     ptlrpc_nrs_queue_type_t queue)
{
	...
	if (svc->srv_nrs_supported & PTLRPC_NRS_TYPE_CRR2) {
		rc = ptlrpc_nrs_policy_register(svc, queue,
						&ptlrpc_nrs_crr2_desc, 0, NULL);
		if (rc != 0)
			GOTO(failed, rc);
	}
...
{
	ptlrpc_nrs_request_t  *nrq = &req->rq_nrq;
...
ptlrpc_nrs_req_prioritize(struct ptlrpc_request *req)
{
	...
	ptlrpc_nrs_t	      *nrs = ptlrpc_server_nrs(svc, 1);
	ptlrpc_nrs_request_t  *nrq = &req->rq_nrq;
	ptlrpc_nrs_resource_t *res1[NRS_RES_MAX];
	ptlrpc_nrs_resource_t *res2[NRS_RES_MAX];
...
ptlrpc_nrs_policy_control(struct ptlrpc_service *svc,
			  ptlrpc_nrs_queue_type_t queue,
			  int type, unsigned opc, void *arg)
{
	int	rc;
...
/** NRS polices */
static int
nrs_fifo_start(ptlrpc_nrs_policy_t *policy)
{
	nrs_fifo_head_t *head;
...
static void
nrs_fifo_stop(ptlrpc_nrs_policy_t *policy)
{
	nrs_fifo_head_t *head = policy->pol_private;
...
static int
nrs_fifo_res_get(ptlrpc_nrs_policy_t *policy, ptlrpc_nrs_request_t *nrq,
		 ptlrpc_nrs_resource_t *parent, ptlrpc_nrs_resource_t **resp)
{
	/* just use the object embedded in fifo request */
	*resp = &((nrs_fifo_head_t *)policy->pol_private)->fh_res;
	return 1; /* done */
}
...
nrs_fifo_req_poll(ptlrpc_nrs_policy_t *policy, void *arg)
{
	...
	return cfs_list_empty(&head->fh_list) ? NULL :
	       cfs_list_entry(head->fh_list.next, ptlrpc_nrs_request_t,
			      nr_u.fifo.fr_list);
}
...
nrs_fifo_req_add(ptlrpc_nrs_policy_t *policy, ptlrpc_nrs_request_t *nrq)
{
	...
	head = container_of(nrs_request_resource(nrq), nrs_fifo_head_t, fh_res);
	/* they are for debug */
...
static void
nrs_fifo_req_start(ptlrpc_nrs_policy_t *policy, ptlrpc_nrs_request_t *nrq)
{
...
static void
nrs_fifo_req_stop(ptlrpc_nrs_policy_t *policy, ptlrpc_nrs_request_t *nrq)
{
	struct ptlrpc_request *req = container_of(nrq, struct ptlrpc_request,
						  rq_nrq);
...
static ptlrpc_nrs_ops_t nrs_fifo_ops = {
	.op_policy_init		= NULL,
	.op_policy_fini		= NULL,
...
static int
nrs_crr_start(ptlrpc_nrs_policy_t *policy)
{
	nrs_crr_data_t *cdat;
...
static void
nrs_crr_stop(ptlrpc_nrs_policy_t *policy)
{
	nrs_crr_data_t *cdat = policy->pol_private;
...
static int
nrs_crr_res_get(ptlrpc_nrs_policy_t *policy, ptlrpc_nrs_request_t *nrq,
		ptlrpc_nrs_resource_t *parent, ptlrpc_nrs_resource_t **resp)
{
	struct ptlrpc_request *req = container_of(nrq, struct ptlrpc_request,
						  rq_nrq);
	...
	if (parent == NULL) {
		*resp = &((nrs_crr_data_t *)policy->pol_private)->cd_res;
		return 0;
	} else {
...
nrs_crr_req_poll(ptlrpc_nrs_policy_t *policy, void *arg)
{
	...
	return node == NULL ? NULL :
	       container_of(node, ptlrpc_nrs_request_t, nr_u.crr.cr_node);
}
...
nrs_crr_req_del(ptlrpc_nrs_policy_t *policy, ptlrpc_nrs_request_t *nrq)
{
	...
	if (node == NULL) { /* no more request */
		...
		nrq = container_of(node, ptlrpc_nrs_request_t,
				   nr_u.crr.cr_node);
...
static void
nrs_crr_req_start(ptlrpc_nrs_policy_t *policy, ptlrpc_nrs_request_t *nrq)
{
	struct ptlrpc_request *req = container_of(nrq, struct ptlrpc_request,
						  rq_nrq);
...
static void
nrs_crr_req_stop(ptlrpc_nrs_policy_t *policy, ptlrpc_nrs_request_t *nrq)
{
	struct ptlrpc_request *req = container_of(nrq, struct ptlrpc_request,
						  rq_nrq);
...
static ptlrpc_nrs_ops_t nrs_crr_ops = {
	.op_policy_init		= NULL,
	.op_policy_fini		= NULL,
...
nrs_crr2_hop_keycmp(const void *key, cfs_hlist_node_t *hnode)
{
	...
	nrs_crr2_client_t *cli = cfs_hlist_entry(hnode, nrs_crr2_client_t,
						 cc_hnode);
...
{
	return cfs_hlist_entry(hnode, nrs_crr2_client_t, cc_hnode);
}
...
static int
nrs_crr2_start(ptlrpc_nrs_policy_t *policy)
{
	nrs_crr2_net_t *net;
...
static void
nrs_crr2_stop(ptlrpc_nrs_policy_t *policy)
{
	nrs_crr2_net_t *net = policy->pol_private;
...
static int
nrs_crr2_res_get(ptlrpc_nrs_policy_t *policy, ptlrpc_nrs_request_t *nrq,
		 ptlrpc_nrs_resource_t *parent, ptlrpc_nrs_resource_t **resp)
{
	nrs_crr2_net_t	      *net;
	nrs_crr2_client_t     *cli;
	nrs_crr2_client_t     *tmp;
	struct ptlrpc_request *req;

	if (parent == NULL) {
		*resp = &((nrs_crr2_net_t *)policy->pol_private)->cn_res;
		return 0;
	}

	net = container_of(parent, nrs_crr2_net_t, cn_res);
...
static void
nrs_crr2_res_put(ptlrpc_nrs_policy_t *policy, ptlrpc_nrs_resource_t *res)
{
	nrs_crr2_net_t	  *net;
	nrs_crr2_client_t *cli;
	...
	cli = container_of(res, nrs_crr2_client_t, cc_res);
	net = container_of(res->res_parent, nrs_crr2_net_t, cn_res);
...
static ptlrpc_nrs_ops_t nrs_crr2_ops = {
	.op_policy_init		= NULL,
	.op_policy_fini		= NULL,
	.op_policy_start	= nrs_crr2_start,
	.op_policy_stop		= nrs_crr2_stop,
	.op_policy_ctl		= NULL, /* not used */
	.op_res_get		= nrs_crr2_res_get,
	.op_res_put		= nrs_crr2_res_put,
	.op_req_poll		= nrs_crr2_req_poll,
	.op_req_enqueue		= nrs_crr2_req_add,
	.op_req_dequeue		= nrs_crr2_req_del,
...
static ptlrpc_nrs_policy_desc_t ptlrpc_nrs_crr2_desc = {
	.pd_name		= "crr2",
	.pd_type		= PTLRPC_NRS_TYPE_CRR2,
	.pd_ops			= &nrs_crr2_ops,
};

/* Object-based Round Robin policy (ORR) */
...
orr_req_compare(cfs_binheap_node_t *e1, cfs_binheap_node_t *e2)
{
	...
	nrq1 = container_of(e1, ptlrpc_nrs_request_t, nr_u.orr.or_node);
	nrq2 = container_of(e2, ptlrpc_nrs_request_t, nr_u.orr.or_node);
...
static void
nrs_orr_key_fill(nrs_orr_data_t *orrd, ptlrpc_nrs_request_t *nrq,
		 ptlrpc_nrs_pol_type_t type, nrs_orr_key_t *key)
{
	struct ptlrpc_request  *req;
	...
	if (type == PTLRPC_NRS_TYPE_ORR) {
		...
		LASSERT(body != NULL);
		/* TODO: This really needs a call to ost_validate_obdo(), to
...
#ifdef HAVE_SERVER_SUPPORT
	{
		...
		/* assert in case lsd is obtained differently in the future */
		LASSERT(lsd != NULL);
		key->ok_idx = lsd->lsd_ost_index;
	}
#else
	/* TODO: Can we do something better than this here? This is just to fix
	 * builds made with --disable-server. */
	key->ok_idx = 0;
#endif
}

/* Is the RPC type handled by ORR ? */
static int
nrs_orr_req_supported(nrs_orr_data_t *orrd, ptlrpc_nrs_request_t *nrq)
{
	struct ptlrpc_request  *req;
	__u32			opc;
...
nrs_orr_log2phys(struct obd_export *exp, struct obdo *oa,
		 nrs_orr_req_range_t *log, nrs_orr_req_range_t *phys);

/* Set ORR range values in RPC */
static void
nrs_orr_set_range(ptlrpc_nrs_request_t *nrq, nrs_orr_req_range_t *range)
{
...
static int
nrs_orr_get_range(ptlrpc_nrs_request_t *nrq, __u8 phys)
{
	struct ptlrpc_request  *req = container_of(nrq, struct ptlrpc_request,
						   rq_nrq);
	struct ptlrpc_service  *svc = req->rq_rqbd->rqbd_service;
	...
	if (phys) {
		...
		if (rc)
			/* TODO: Decide better action; maybe add a cdebug here
			 */
			GOTO(out, rc);
	}
...
static int nrs_orr_hop_keys_eq(nrs_orr_key_t *k1, nrs_orr_key_t *k2)
{
...
{
	return cfs_hash_djb2_hash(key, sizeof(nrs_orr_key_t), mask);
}

static void *
nrs_orr_hop_key(cfs_hlist_node_t *hnode)
{
	nrs_orr_object_t *orro = cfs_hlist_entry(hnode, nrs_orr_object_t,
						 oo_hnode);
...
nrs_orr_hop_keycmp(const void *key, cfs_hlist_node_t *hnode)
{
	...
	return nrs_orr_hop_keys_eq(&orro->oo_key, (nrs_orr_key_t *)key);
}

static void *
nrs_orr_hop_object(cfs_hlist_node_t *hnode)
{
	return cfs_hlist_entry(hnode, nrs_orr_object_t, oo_hnode);
}
...
{
	nrs_orr_data_t		*orrd;
	nrs_orr_object_t	*orro;

	orro = cfs_hlist_entry(hnode, nrs_orr_object_t, oo_hnode);
	orrd = container_of(orro->oo_res.res_parent, nrs_orr_data_t, od_res);

	LASSERTF(cfs_atomic_read(&orro->oo_ref) == 0,
		 "Busy NRS orr policy for object with objid "LPX64" at OST "
		 "with OST index %d, with %d refs\n", orro->oo_key.ok_id,
...
#ifdef LPROCFS
/* TODO: Change these ORR functions */
int nrs_orr_lprocfs_init(ptlrpc_nrs_policy_t *policy);
void nrs_orr_lprocfs_fini(ptlrpc_nrs_policy_t *policy);
#else
int nrs_orr_lprocfs_init(ptlrpc_nrs_policy_t *policy) { return 0; }
void nrs_orr_lprocfs_fini(ptlrpc_nrs_policy_t *policy) { }
#endif

static int
nrs_orr_start(ptlrpc_nrs_policy_t *policy)
{
	nrs_orr_data_t	       *orrd;
	int			rc = 0;
	...
	orrd->od_cache = cfs_mem_cache_create(policy->pol_nrs->nrs_queue_type ==
					      ...
					      sizeof(nrs_orr_object_t), 0, 0);
	if (orrd->od_cache == NULL)
		GOTO(failed, rc = -ENOMEM);

	/* Hash for finding objects by nrs_orr_key_t */
...
static int
nrs_orr_res_get(ptlrpc_nrs_policy_t *policy, ptlrpc_nrs_request_t *nrq,
		ptlrpc_nrs_resource_t *parent, ptlrpc_nrs_resource_t **resp)
{
	nrs_orr_data_t	       *orrd;
	nrs_orr_object_t       *orro;
	nrs_orr_object_t       *tmp;
	nrs_orr_key_t		key;

	ENTRY;

	/* nrs_orr_data_t is requested */
	if (parent == NULL) {
		*resp = &((nrs_orr_data_t *)policy->pol_private)->od_res;
		RETURN(0);
	}

	orrd = container_of(parent, nrs_orr_data_t, od_res);
	...
	/* nrs_orr_object_t is requested */
	nrs_orr_key_fill(orrd, nrq, policy->pol_type, &key);
...
static void
nrs_orr_res_put(ptlrpc_nrs_policy_t *policy, ptlrpc_nrs_resource_t *res)
{
	nrs_orr_data_t		*orrd;
	nrs_orr_object_t	*orro;
	...
	orro = container_of(res, nrs_orr_object_t, oo_res);
	LASSERT(res->res_parent != NULL);
	orrd = container_of(res->res_parent, nrs_orr_data_t, od_res);
...
static ptlrpc_nrs_request_t *
nrs_orr_req_poll(ptlrpc_nrs_policy_t *policy, void *arg)
{
	nrs_orr_data_t		*orrd = policy->pol_private;
	cfs_binheap_node_t	*node = cfs_binheap_root(orrd->od_binheap);

	return node == NULL ? NULL :
		container_of(node, ptlrpc_nrs_request_t, nr_u.orr.or_node);
}

/* Sort-adds a request to the binary heap */
static int
nrs_orr_req_add(ptlrpc_nrs_policy_t *policy, ptlrpc_nrs_request_t *nrq)
{
	nrs_orr_data_t		*orrd;
	nrs_orr_object_t	*orro;
	int			 rc;
...
static void
nrs_orr_req_del(ptlrpc_nrs_policy_t *policy, ptlrpc_nrs_request_t *nrq)
{
	nrs_orr_data_t		*orrd;
	nrs_orr_object_t	*orro;
...
static void
nrs_orr_req_start(ptlrpc_nrs_policy_t *policy, ptlrpc_nrs_request_t *nrq)
{
	nrs_orr_object_t	*orro = container_of(nrs_request_resource(nrq),
						     nrs_orr_object_t, oo_res);

	LASSERT(nrq->nr_u.orr.or_round <= orro->oo_round);

	/* NB: resource control, credits etc can be added to here */
	CDEBUG(D_RPCTRACE, "NRS ORR start %s request for object with ID "LPU64""
	       " from OST with index %d, with round "LPU64"\n",
	       nrs_request_policy(nrq)->pol_name, nrq->nr_u.orr.or_key.ok_id,
	       nrq->nr_u.orr.or_key.ok_idx, nrq->nr_u.orr.or_round);
}

static void
nrs_orr_req_stop(ptlrpc_nrs_policy_t *policy, ptlrpc_nrs_request_t *nrq)
{
	nrs_orr_object_t	*orro = container_of(nrs_request_resource(nrq),
						     nrs_orr_object_t, oo_res);
	...
	/* NB: resource control, credits etc can be added to here */
	CDEBUG(D_RPCTRACE, "NRS ORR stop %s request for object with ID "LPU64" "
	       "from OST with index %d, with round "LPU64"\n",
	       nrs_request_policy(nrq)->pol_name, nrq->nr_u.orr.or_key.ok_id,
	       nrq->nr_u.orr.or_key.ok_idx, nrq->nr_u.orr.or_round);
}

static ptlrpc_nrs_ops_t nrs_orr_ops = {
	.op_policy_init		= NULL,
	.op_policy_fini		= NULL,
		 ...
		 nrs_orr_req_range_t *log, nrs_orr_req_range_t *phys)
{
	 ...
	 * information and assume the nrs_orr_fiemap_t struct that is allocated
...
/* Target-based Round Robin policy (TRR) */
...
nrs_trr_hop_exit(cfs_hash_t *hs, cfs_hlist_node_t *hnode)
{
	...
	LASSERTF(cfs_atomic_read(&orro->oo_ref) == 0,
		 "Busy NRS TRR policy at OST with index %d, with %d refs\n",
		 orro->oo_key.ok_idx, cfs_atomic_read(&orro->oo_ref));
...
static int
nrs_trr_start(ptlrpc_nrs_policy_t *policy)
{
	nrs_orr_data_t	       *orrd;
	int			rc = 0;
...
static void
nrs_trr_req_start(ptlrpc_nrs_policy_t *policy, ptlrpc_nrs_request_t *nrq)
{
	nrs_orr_object_t	*orro = container_of(nrs_request_resource(nrq),
						     nrs_orr_object_t, oo_res);
	...
	/* NB: resource control, credits etc can be added to here */
	CDEBUG(D_RPCTRACE, "NRS TRR start %s request from OST with index %d,"
	       "with round "LPU64"\n", nrs_request_policy(nrq)->pol_name,
	       nrq->nr_u.orr.or_key.ok_idx, nrq->nr_u.orr.or_round);
}

static void
nrs_trr_req_stop(ptlrpc_nrs_policy_t *policy, ptlrpc_nrs_request_t *nrq)
{
	nrs_orr_object_t	*orro = container_of(nrs_request_resource(nrq),
						     nrs_orr_object_t, oo_res);
	...
	/* NB: resource control, credits etc can be added to here */
	CDEBUG(D_RPCTRACE, "NRS TRR stop %s request from OST with index %d,"
	       "with round "LPU64"\n", nrs_request_policy(nrq)->pol_name,
	       nrq->nr_u.orr.or_key.ok_idx, nrq->nr_u.orr.or_round);
}

/* Reuse much of the ORR functionality for TRR */
static ptlrpc_nrs_ops_t nrs_trr_ops = {
	.op_policy_init		= NULL,
	.op_policy_fini		= NULL,
