...
{
	struct ptlrpc_nrs	  *nrs = ptlrpc_server_nrs(svcpt, hp);
	struct ptlrpc_nrs_policy  *policy;
	struct ptlrpc_nrs_request *nrq;
			   ...
			   unsigned long flags, void *arg)
{
	...
	case PTLRPC_NRS_QUEUE_REG:
		rc = nrs_policy_register(ptlrpc_server_nrs(svcpt, 0),
					 desc, flags, arg);
		if (rc != 0 || queue == PTLRPC_NRS_QUEUE_REG)
			break;

	case PTLRPC_NRS_QUEUE_HP:
		rc = nrs_policy_register(ptlrpc_server_nrs(svcpt, 1),
					 desc, flags, arg);
		break;
	}
			     ...
			     enum ptlrpc_nrs_queue_type queue)
{
	...
	if (svcpt->scp_nrs_supported & PTLRPC_NRS_TYPE_FIFO) {
	...
	if (svcpt->scp_nrs_supported & PTLRPC_NRS_TYPE_CRR) {
		rc = ptlrpc_nrs_policy_register(svcpt, queue,
	...
	if (svcpt->scp_nrs_supported & PTLRPC_NRS_TYPE_CRRN) {
		rc = ptlrpc_nrs_policy_register(svcpt, queue,
	...
	if (svcpt->scp_nrs_supported & PTLRPC_NRS_TYPE_ORR) {
		rc = ptlrpc_nrs_policy_register(svcpt, queue,
						&ptlrpc_nrs_orr_desc, 0, NULL);
		if (rc != 0)
			GOTO(failed, rc);
	}
	if (svcpt->scp_nrs_supported & PTLRPC_NRS_TYPE_TRR) {
		rc = ptlrpc_nrs_policy_register(svcpt, queue,
			     ...
			     enum ptlrpc_nrs_queue_type queue, int type)
{
	...
	case PTLRPC_NRS_QUEUE_BOTH:
	case PTLRPC_NRS_QUEUE_REG:
		rc = nrs_policy_unregister(ptlrpc_server_nrs(svcpt, 0), type);
		if (queue == PTLRPC_NRS_QUEUE_REG || rc != 0)
			break;

	case PTLRPC_NRS_QUEUE_HP:
		rc = nrs_policy_unregister(ptlrpc_server_nrs(svcpt, 1), type);
		break;
	}
			...
			enum ptlrpc_nrs_queue_type queue)
{
	...
	case PTLRPC_NRS_QUEUE_REG:
		...
		if (queue == PTLRPC_NRS_QUEUE_REG)
			break;
	case PTLRPC_NRS_QUEUE_HP:
		OBD_ALLOC_PTR(svcpt->scp_hpreq_nrs);
		if (svcpt->scp_hpreq_nrs == NULL)
			RETURN(-ENOMEM);
		nrs = ptlrpc_server_nrs(svcpt, 1);
		nrs->nrs_svcpt = svcpt;
			  ...
			  struct ptlrpc_request *req)
{
	...
	nrs_resource_get_safe(ptlrpc_server_nrs(svcpt, req->rq_hp),
...
ptlrpc_nrs_req_prioritize(struct ptlrpc_request *req)
{
	...
	struct ptlrpc_nrs		*nrs = ptlrpc_server_nrs(svcpt, 1);
	struct ptlrpc_nrs_request	*nrq = &req->rq_nrq;
			  ...
			  enum ptlrpc_nrs_ctl opc, void *arg)
{
	...
	case PTLRPC_NRS_QUEUE_BOTH:
	case PTLRPC_NRS_QUEUE_REG:
		rc = nrs_policy_ctl(ptlrpc_server_nrs(svcpt, 0), type, opc,
				    arg);
		if (rc != 0 || queue == PTLRPC_NRS_QUEUE_REG)
			break;

	case PTLRPC_NRS_QUEUE_HP:
		rc = nrs_policy_ctl(ptlrpc_server_nrs(svcpt, 1), type, opc,
				    arg);
		break;
	}
