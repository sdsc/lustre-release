...
int keyblock_init(struct keyblock *kb, char *alg_name, int alg_mode);
void keyblock_free(struct keyblock *kb);
int keyblock_dup(struct keyblock *new, struct keyblock *kb);
int get_bytes(char **ptr, const char *end, void *res, int len);
int get_rawobj(char **ptr, const char *end, rawobj_t *res);
int get_keyblock(char **ptr, const char *end, struct keyblock *kb,
		 __u32 keysize);
void buf_to_sg(struct scatterlist *sg, void *ptr, int len);
__u32 gss_crypt_generic(struct crypto_blkcipher *tfm, int decrypt, void *iv,
...
int gss_digest_hmac(struct crypto_hash *tfm, rawobj_t *key, rawobj_t *hdr,
		    ...
		    rawobj_t *cksum);
int gss_digest_norm(struct crypto_hash *tfm, struct keyblock *kb, rawobj_t *hdr,
		    int msgcnt, rawobj_t *msgs, int iovcnt, lnet_kiov_t *iovs,
		    rawobj_t *cksum);
