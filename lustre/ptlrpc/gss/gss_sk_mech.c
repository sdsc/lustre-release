...
/* Starting number for reverse contexts */
#define SK_IV_REV_START (1LL << 63)
...
{
	__u64 tmp_iv;
	skh->skh_version = cpu_to_be64(SK_MSG_VERSION);
...
{
	if (be64_to_cpu(skh->skh_version) != SK_MSG_VERSION)
...
void sk_construct_rfc3686_iv(__u8 *iv, __u32 nonce, __u64 partial_iv)
{
	...
	memcpy(iv + CTR_RFC3686_NONCE_SIZE, &partial_iv, CTR_RFC3686_IV_SIZE);
	memcpy(iv + CTR_RFC3686_NONCE_SIZE + CTR_RFC3686_IV_SIZE,
	       &ctr, sizeof(ctr));
}
