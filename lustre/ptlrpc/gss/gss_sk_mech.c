...
#define SK_BLOCK_MASK(len, blocksize) \
	((len + blocksize - 1) & (~(blocksize - 1)))
...
static char *sk_crypt_name[] = {
	[SK_CRYPT_AES_CTR] = "ctr(aes)",
};
...
static struct sk_hmac_type sk_hmac_types[] = {
	[SK_HMAC_MD5]    = { "hmac(md5)", 16, },
	[SK_HMAC_SHA1]   = { "hmac(sha1)", 20, },
	[SK_HMAC_SHA256] = { "hmac(sha256)", 32, },
	[SK_HMAC_SHA512] = { "hmac(sha512)", 64, },
};
			     ...
			     unsigned long *endtime)
{
	...
	*endtime = (unsigned long)skc->sc_expire;
	return GSS_S_COMPLETE;
}
		    ...
		    rawobj_t *token)
{
	...
	cipher.len = token->len - sht->sht_size;
	...
	LASSERT(cipher.len + sht->sht_size <= token->len);
			     ...
			     int adj_nob)
{
	...
	for (i = 0; i < desc->bd_iov_count; i++) {
		...
		if (adj_nob) {
			...
			LASSERT(ciov->kiov_len + cnob <=
				desc->bd_nob_transferred);
			LASSERT(piov->kiov_len <= ciov->kiov_len);
		}
