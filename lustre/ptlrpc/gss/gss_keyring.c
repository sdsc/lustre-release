 ...
 */
static void* key_fetch_payload(struct key *key, unsigned int index)
{
	void *key_ptr = NULL;
...
static void bind_key_ctx(struct key *key, struct ptlrpc_cli_ctx *ctx)
{
	...
	LASSERT(!key_fetch_payload(key, 0));
...
{
	LASSERT(key_fetch_payload(key, 0) == ctx);
	LASSERT(test_bit(PTLRPC_CTX_CACHED_BIT, &ctx->cc_flags) == 0);
...
{
	struct ptlrpc_cli_ctx *ctx = key_fetch_payload(key, 0);
