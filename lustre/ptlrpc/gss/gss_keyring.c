...
/* Mapping structure to provide backward compatibility for accessing
 * the various keyrings. */
struct _keyring_compat {
...
#define DUMP_PROCESS_KEYRINGS(cred)					\
{									\
	CWARN("DUMP PK: %s[%u,%u/%u](<-%s[%u,%u/%u]): "			\
		"a %d, t %d, p %d, s %d, u %d, us %d, df %d\n",		\
		cred->comm, cred->pid, cred->uid, cred->fsuid,		\
		cred->parent->comm, cred->parent->pid,			\
		cred->parent->uid, cred->parent->fsuid,			\
		cred->request_key_auth ?				\
		cred->request_key_auth->serial : 0,			\
		cred->thread_keyring ?					\
		cred->thread_keyring->serial : 0,			\
		cred->process_keyring ?					\
		cred->process_keyring->serial : 0,			\
		cred->session_keyring ?					\
		cred->session_keyring->serial : 0,			\
		cred->user->uid_keyring ?				\
		cred->user->uid_keyring->serial : 0,			\
		cred->user->session_keyring ?				\
		cred->user->session_keyring->serial : 0,		\
		jit_keyring()						\
	);								\
}
...
/* Prime the keyring compatibility structure. */
static void keyring_init(struct _keyring_compat *compat)
{
#if HAVE_STRUCT_CRED
	const struct cred *cred = current_cred();

	LASSERT(compat);
	*compat = (struct _keyring_compat) {
		.jit_keyring		= cred->jit_keyring,
		.process_keyring	= cred->tgcred->process_keyring,
		.session_keyring	= cred->tgcred->session_keyring,
		.thread_keyring		= cred->thread_keyring,
		.user			= cred->user,
	};
#else
	const struct task_struct *tsk = cfs_current();

	LASSERT(compat);
	*compat = (struct _keyring_compat) {
		.jit_keyring		= tsk->jit_keyring,
		.process_keyring	= tsk->signal->process_keyring,
		.session_keyring	= tsk->signal->session_keyring,
		.thread_keyring		= tsk->thread_keyring,
		.user			= tsk->user,
	};
#endif
...
int gss_kt_instantiate(struct key *key, const void *data, size_t datalen)
{
	...
	OBD_ALLOC_PTR(cred);
	keyring_init(cred);
	LASSERT(cred->session_keyring);

	lockdep_off();
	rc = key_link(cred->session_keyring, key);
	lockdep_on();
	if (unlikely(rc)) {
		CERROR("failed to link key %08x to keyring %08x: %d\n",
		       key->serial,
		       cred->session_keyring->serial, rc);
		OBD_FREE_PTR(cred);
		RETURN(rc);
	}
