...
static int ptlrpcd_init(void)
{
        ...
        nthreads = 1;
#endif
        CWARN("ptlrpcd: %d threads, bind policy %s\n",
              nthreads,
              (ptlrpcd_bind_policy == PDB_POLICY_NONE) ? "NONE" :
              (ptlrpcd_bind_policy == PDB_POLICY_FULL) ? "FULL" :
              (ptlrpcd_bind_policy == PDB_POLICY_PAIR) ? "PAIR" :
              (ptlrpcd_bind_policy == PDB_POLICY_NEIGHBOR) ? "NEIGHBOR" :
              (ptlrpcd_bind_policy == PDB_POLICY_NODE) ? "NODE" :
              "UNKNOWN");
