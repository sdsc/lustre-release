 ...
 * A value other than 0 implies it was tuned.
 */
...
/*
 * ptlrpcd_partner_group_size defaults to 2, but setting it to -1 here
 * allows us to determine if it has been tuned.
 */
static int ptlrpcd_partner_group_size = -1;
 ...
 */
static struct ptlrpcd_ctl *ptlrpcd_rcv_thread;
...
int ptlrpcd_start(struct ptlrpcd *pd, int index, const char *name)
{
	...
	if (pd == NULL) {
		LASSERT(index == -1);
		pc = ptlrpcd_rcv_thread;
		pc->pc_cpt = CFS_CPT_ANY;
	} else {
		LASSERT(index >= 0 && index < pd->pd_nthreads);
		pc = &pd->pd_threads[index];
		pc->pc_cpt = pd->pd_cpt;
...
static void ptlrpcd_fini(void)
{
	...
	if (ptlrpcd_rcv_thread != NULL) {
		ptlrpcd_stop(ptlrpcd_rcv_thread, 0);
		ptlrpcd_free(ptlrpcd_rcv_thread);
		ptlrpcd_rcv_thread = NULL;
	}
...
static int ptlrpcd_init(void)
{
	...
	if (ptlrpcd_bind_policy != 0)
		CWARN("The ptlrpcd_bind_policy parameter is obsolete\n");

	if (ptlrpcd_partner_group_size < 0) {
		/*
		 * If ptlrpcd_bind_policy was tuned use that value,
		 * default to 2 to create pairs.
		 */
...
#ifdef CONFIG_NUMA
			ptlrpcd_partner_group_size = 0; /* Full CPT */
#else
			ptlrpcd_partner_group_size = 3; /* Triplets */
#endif
			break;
		}
	} else if (ptlrpcd_partner_group_size > max_ptlrpcds) {
		ptlrpcd_partner_group_size = max_ptlrpcds;
	}
	 ...
	 */
	set_bit(LIOD_RECOVERY, &ptlrpcd_rcv_thread->pc_flags);
	rc = ptlrpcd_start(NULL, -1, "ptlrpcd_rcv");
	if (rc < 0)
		GOTO(out, rc);
