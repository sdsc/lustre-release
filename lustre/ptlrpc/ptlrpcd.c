...
struct ptlrpcd {
	...
	struct ptlrpcd_ctl pd_thread_brw;
...
ptlrpcd_select_pc(struct ptlrpc_request *req, pdl_policy_t policy, int index)
{
	...
	if (req != NULL && req->rq_bulk != NULL)
		return &ptlrpcds->pd_thread_brw;
...
static void ptlrpcd_fini(void)
{
	...
	if (ptlrpcds != NULL) {
		...
		ptlrpcd_stop(&ptlrpcds->pd_thread_brw, 0);
		ptlrpcd_free(&ptlrpcds->pd_thread_brw);
...
static int ptlrpcd_init(void)
{
	...
	snprintf(name, sizeof(name) - 1, "ptlrpcd_brw");
	rc = ptlrpcd_start(-1, nthreads, name, &ptlrpcds->pd_thread_brw);
	if (rc < 0)
		GOTO(out, rc);
