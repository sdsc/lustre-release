...
static void *
nrs_crrn_hop_object(cfs_hlist_node_t *hnode)
{
...
/**
 * Removes an nrs_crrn_client object from the hash and frees its memory, if the
 * object has no active users.
 */
static void
nrs_crrn_hop_put(cfs_hash_t *hs, cfs_hlist_node_t *hnode)
{
	...
	if (cfs_atomic_dec_return(&cli->cc_ref) > 1)
		return;

	cfs_hash_lock(hs, 0);
	cfs_hash_dual_bd_get_and_lock(hs, (void *)&cli->cc_nid, bds, 1);

	/**
	 * Another thread may have won the race and taken a reference on the
	 * nrs_orr_object.
	 */
	if (cfs_atomic_read(&cli->cc_ref) > 1)
		goto lost_race;
...
static void
nrs_crrn_hop_put_locked(cfs_hash_t *hs, cfs_hlist_node_t *hnode)
{
	struct nrs_crrn_client	*cli = cfs_hlist_entry(hnode,
						       struct nrs_crrn_client,
						       cc_hnode);
...
static cfs_hash_ops_t nrs_crrn_hash_ops = {
	...
	.hs_put_locked	= nrs_crrn_hop_put_locked,
};
...
nrs_crrn_start(struct ptlrpc_nrs_policy *policy)
{
	...
	net->cn_cli_hash = cfs_hash_create("nrs_nid_hash",
					   ...
					   &nrs_crrn_hash_ops,
					   CFS_HASH_RW_BKTLOCK |
					   CFS_HASH_REHASH | CFS_HASH_COUNTER |
					   CFS_HASH_SHRINK |
		 ...
		 struct ptlrpc_nrs_resource **resp, bool moving_req)
{
	...
	/**
	 * XXX: Is a slab cache warranted here as for ORR/TRR? The main
	 * reason for not doing this is to avoid populating the list of slab
	 * caches shown at /proc/slabinfo with a high number of entries; e.g. if
	 * CRR-N is started on all services, there would be two entries (one for
	 * the regular, and one for the high-priority NRS head), for each CPT of
	 * each service. This may be fine, although maybe a better solution could
	 * be to have a centrally-located slab cache for all CRR-N policy
	 * instances.
	 */
