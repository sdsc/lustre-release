...
/**
 * Protects \e nrs_pols_list, and nrs_heads_list
 */
static CFS_DEFINE_MUTEX(nrs_core_mutex);
 ...
 * \param[in]  nrq  The request that is beign handled.
...
ptlrpc_nrs_hpreq_add_nolock(struct ptlrpc_request *req)
{
	...
	struct ptlrpc_nrs		*nrs = ptlrpc_svcpt2nrs(svcpt, true);
			    ...
			    enum ptlrpc_nrs_queue_type queue, char *name)
{
	...
	case PTLRPC_NRS_QUEUE_REG:
		rc = nrs_policy_unregister(ptlrpc_svcpt2nrs(svcpt, false),
					   name);
		if (queue == PTLRPC_NRS_QUEUE_REG || rc != 0)
			break;

	case PTLRPC_NRS_QUEUE_HP:
		rc = nrs_policy_unregister(ptlrpc_svcpt2nrs(svcpt, true),
					   name);
		break;
	}
		       ...
		       struct ptlrpc_service_conf *conf)
{
	 ...
	 */
	nrs = ptlrpc_svcpt2nrs(svcpt, false);
	...
	nrs = ptlrpc_svcpt2nrs(svcpt, true);
	rc = svcpt_nrs_setup0(nrs, svcpt);
...
/**
 * Adds a policy descriptor to NRS core's global policy list, \e nrs_pol_list
 *
 * \param[in] desc The policy descriptor
 *
 * \retval -EEXIST The policy descriptor \desc has already been added
 * \retval	 0 The policy descriptor \a desc was added successfully
 */
static int
...
/**
 * Removes a policy descriptor from NRS core's global policy list,
 * \e nrs_pol_list
 *
 * \param[in] desc The policy descriptor to remove
 *
 * \retval -ENODEV Policy descriptor \a desc has not been registered with NRS
 *		   core
 ...
 *
 * \pre cfs_mutex_is_locked(&nrs_core_mutex)
 */
static int
nrs_policy_unregister_all_locked(struct ptlrpc_nrs_pol_desc *desc)
{
	...
	/**
	 * TODO: During registration, this should alway happen; i.e. always
	 * remove it; FIXME
	 *
	 * Only remove the policy deecriptor from NRS core when the policy has
	 * been removed from all service partitions of all services.
	 */
...
/**
 * \name Exported API
 *TODO: Check Doxygen usage ^^
 */

/**
 * Registers a new policy with NRS core.
 *
 * Used for policies that register exeternally with NRS core, ie. ones that are
 * not part of nrs_pols_builtin. The function will only succeed if policy
 * registration with all service partitions is successful.
 *
 ...
 *
 * \param[in] desc The descriptor of teh policy to unregister
 *
 * \retval -ve error
 * \retval   0 success
 */
...
{
	struct ptlrpc_nrs	*nrs = ptlrpc_svcpt2nrs(svcpt, false);
 ...
 * \param[in] hp    Whether to enqueue teh request on the regular or
 *		    high-priority NRS head.
 */
void
 ...
 * \param[in]	  queue Whether to carry out teh command on the policy which
			  ...
			  enum ptlrpc_nrs_ctl opc, void *arg)
{
	...
	case PTLRPC_NRS_QUEUE_BOTH:
	case PTLRPC_NRS_QUEUE_REG:
		rc = nrs_policy_ctl(ptlrpc_svcpt2nrs(svcpt, false), name, opc,
				    arg);
		if (rc != 0 || queue == PTLRPC_NRS_QUEUE_REG)
			break;

	case PTLRPC_NRS_QUEUE_HP:
		rc = nrs_policy_ctl(ptlrpc_svcpt2nrs(svcpt, true), name, opc,
				    arg);
		break;
	}
...
ptlrpc_nrs_init(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(nrs_pols_builtin); i++) {
		/**
		 * No need to take nrs_core_mutex as there is noe contention at
		 * this early stage.
		 */
		rc = nrs_policy_add_locked(nrs_pols_builtin[i]);
		/**
...
/**
 * TODO: how to merge this Doxygen header and the one in the *.h files? Does using
 * 'name' here automatically link them?
 * \name fifo
 *
 * FIFO policy
 */
