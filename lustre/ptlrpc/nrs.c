...
/**
 * Obtains an NRS request from \a policy for handling, and optionally removes
 * the request.
 *
 ...
 * \param[in] rem   when set, the request is also removed from the policy
					    ...
					    bool rem, bool force)
{
	...
	nrq = policy->pol_desc->pd_ops->op_req_get(policy, rem, force);
 ...
 */
static inline void nrs_request_dequeue(struct ptlrpc_nrs_request *nrq)
{
	struct ptlrpc_nrs_policy *policy;
 ...
 */
static inline void nrs_request_start(struct ptlrpc_nrs_request *nrq)
{
			   ...
			   bool force)
{
	...
	cfs_list_for_each_entry(policy, &nrs->nrs_policy_queued,
				pol_list_queued) {
		nrq = nrs_request_get(policy, rem, force);
		if (likely(nrq != NULL))
