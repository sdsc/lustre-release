 ...
 * by \e nrs_core_mutex.
 */
static CFS_LIST_HEAD(nrs_core_heads_list);

/**
 * List of all policies registered with NRS core; protected by \e nrs_core_mutex
 */
static CFS_LIST_HEAD(nrs_core_pols_list);

/**
 * Protects \e nrs_core_pols_list, \e nrs_core_heads_list, external policy
 * registration/unregistration, and lprocfs operations on policies.
 */
DEFINE_MUTEX(nrs_core_mutex);
...
/**
 * Registers all compatible policies in nrs_core_pols_list, for NRS head \a nrs.
 *
...
nrs_register_policies_locked(struct ptlrpc_nrs *nrs)
{
	...
	cfs_list_for_each_entry(desc, &nrs_core_pols_list, pd_list) {
		if (nrs_policy_compatible(svc, desc)) {
			...
			/**
			 * If this is the regular NRS head for the first service
			 * partition, register the policy's lprocfs interface
			 * for this service.
			 */
			if (svcpt != svc->srv_parts[0] ||
...
svcpt_nrs_setup0(struct ptlrpc_nrs *nrs, struct ptlrpc_service_part *svcpt)
{
	...
	cfs_list_add_tail(&nrs->nrs_heads, &nrs_core_heads_list);
...
again:
	...
	/**
	 * If this is the last service partition, remove any lprocfs interfaces
	 * for all compatible policies.
	 */
	if (svcpt != svc->srv_parts[svc->srv_ncpts - 1])
		goto not_last;
...
nrs_policy_exists_locked(struct ptlrpc_nrs_pol_desc *desc)
{
	...
	cfs_list_for_each_entry(tmp, &nrs_core_pols_list, pd_list) {
		if (!strcmp(tmp->pd_name, desc->pd_name))
			return true;
	}
	return false;
}
...
nrs_policy_unregister_locked(struct ptlrpc_nrs_pol_desc *desc)
{
	...
	cfs_list_for_each_entry(nrs, &nrs_core_heads_list, nrs_heads) {
...
again:
	...
	cfs_list_add_tail(&desc->pd_list, &nrs_core_pols_list);
fail:
	mutex_unlock(&nrs_core_mutex);
 ...
 * array, to NRS core's list of policies \e nrs_core_pols_list.
 *
 * \retval 0 All policy descriptors in \e nrs_pols_builtin have been added
 *	     successfully to \e nrs_core_pols_list
 */
int
