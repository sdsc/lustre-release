...
static int nrs_tbf_opcode_parse(struct nrs_tbf_cmd *cmd, const char *id)
{
	...
	strncpy(cmd->u.tc_start.ts_opcodes_str, id, strlen(id) + 1);
...
static int nrs_tbf_id_parse(struct nrs_tbf_cmd *cmd, char *token)
{
	...
	/* Skip '{' */
	token++;
	/* '}' has already been replaced to '\0' when splitting pairs */
	...
	if (!rc)
		cmd->u.tc_start.ts_valid_types |= NRS_TBF_FLAG_OPCODE;
...
static void nrs_tbf_cmd_fini(struct nrs_tbf_cmd *cmd)
{
	if (cmd->tc_cmd == NRS_CTL_TBF_START_RULE) {
		...
		if (cmd->u.tc_start.ts_valid_types & NRS_TBF_FLAG_NID)
			nrs_tbf_nid_cmd_fini(cmd);
		if (cmd->u.tc_start.ts_valid_types & NRS_TBF_FLAG_OPCODE)
			nrs_tbf_opcode_cmd_fini(cmd);
	}
}
...
nrs_tbf_parse_value_pairs(struct nrs_tbf_cmd *cmd, char *buffer)
{
	...
	while (val != NULL && strlen(val) != 0) {
		/* Only id pair could contain space */
		if (strncmp(val, "id", 2) == 0) {
			token = strsep(&val, "}");

			/* Should be followed by space or nothing */
			if (val[0] == ' ')
				val++;
			else if (val[0] != '\0')
				return -EINVAL;
	...
	switch (cmd->tc_cmd) {
	case NRS_CTL_TBF_START_RULE:
		if (cmd->u.tc_start.ts_valid_types == 0)
			return -EINVAL;
