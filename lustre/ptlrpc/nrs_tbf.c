		   ...
		   bool no_block)
{
	...
	OBD_CPT_ALLOC_GFP(rule, nrs_pol2cptab(policy), nrs_pol2cptid(policy),
			  sizeof(*rule), no_block ? GFP_ATOMIC : __GFP_IO);
	if (rule == NULL)
		GOTO(out, rc = -ENOMEM);
		...
		struct nrs_tbf_cmd *cmd)
{
	...
	case NRS_CTL_TBF_START_RULE:
		...
		rule = nrs_tbf_rule_start(policy, head, cmd, true);
		if (IS_ERR(rule))
			rc = PTR_ERR(rule);
		break;
				 ...
				 struct nrs_tbf_head *head)
{
	...
	rule = nrs_tbf_rule_start(policy, head, &start, false);
