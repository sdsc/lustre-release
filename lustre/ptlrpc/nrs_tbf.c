...
static void
_nrs_tbf_cli_reset_value(struct nrs_tbf_head *head,
...
{
	_nrs_tbf_cli_reset_value(head, cli, true);
}

static void
_nrs_tbf_cli_reset(struct nrs_tbf_head *head,
		   ...
		   bool inited)
{
	...
	_nrs_tbf_cli_reset_value(head, cli, inited);
}
...
nrs_tbf_cli_reset(struct nrs_tbf_head *head,
		  ...
		  struct nrs_tbf_client *cli)
{
	_nrs_tbf_cli_reset(head, rule, cli, true);
}
		  ...
		  const char *name)
{
	...
	cfs_list_for_each(pos, &head->th_list) {
		rule = cfs_list_entry(pos, struct nrs_tbf_rule, tr_linkage);
		LASSERT((rule->tr_flags & NTRS_STOPPING) == 0);
		if (strlen(name) == strlen(rule->tr_name) &&
		 ...
		 struct ptlrpc_request *req)
{
	...
	head->th_ops->o_client_init_info(cli, req);
	...
	_nrs_tbf_cli_reset(head, rule, cli, false);
}
...
static int nrs_tbf_jobid_hop_keycmp(const void *key, cfs_hlist_node_t *hnode)
{
	struct nrs_tbf_client *cli = cfs_hlist_entry(hnode,
						     ...
						     tc_hnode);
	return (strlen(cli->tc_jobid) == strlen(key)) &&
		!strncmp(cli->tc_jobid, key, strlen(key));
}
...
static void nrs_tbf_jobid_hop_exit(cfs_hash_t *hs, cfs_hlist_node_t *hnode)
{
	...
	LASSERTF(cfs_atomic_read(&cli->tc_ref) == 0,
		 "Busy TBF object from client with jobid %s, with %d refs\n",
		 cli->tc_jobid, cfs_atomic_read(&cli->tc_ref));
...
static cfs_hash_t *nrs_tbf_jobid_hash_create(void)
{
	return cfs_hash_create("nrs_tbf_hash",
...
nrs_tbf_jobid_hash_lookup(cfs_hash_t *hash,
			  struct ptlrpc_request *req)
{
	return cfs_hash_lookup(hash, lustre_msg_get_jobid(req->rq_reqmsg));
}

static struct nrs_tbf_client *
nrs_tbf_jobid_hash_findadd_unique(cfs_hash_t *hash,
				  struct nrs_tbf_client *cli)
{
	return cfs_hash_findadd_unique(hash, cli->tc_jobid,
				       &cli->tc_hnode);
}

static void
nrs_tbf_jobid_client_init_info(struct nrs_tbf_client *cli,
			     struct ptlrpc_request *req)
{
...
static int
nrs_tbf_jobid_start_default_rule(struct ptlrpc_nrs_policy *policy,
			       struct nrs_tbf_head *head)
{
	struct nrs_tbf_cmd	 start;
	struct nrs_tbf_rule	*rule = NULL;
...
nrs_tbf_jobid_list_free(cfs_list_t *jobid_list)
{
	...
	cfs_list_for_each_safe(pos, next, jobid_list) {
		jobid = cfs_list_entry(pos, struct nrs_tbf_jobid, tj_linkage);
...
nrs_tbf_jobid_list_match(cfs_list_t *jobid_list, char *id)
{
	...
	cfs_list_for_each(pos, jobid_list) {
		jobid = cfs_list_entry(pos, struct nrs_tbf_jobid, tj_linkage);
		if (strlen(id) == strlen(jobid->tj_id) &&
		    strncmp(id, jobid->tj_id, strlen(jobid->tj_id)) == 0)
			return 1;
	}
	return 0;
}
...
{
	return snprintf(buff, length, "%s {%s} %llu, ref %ld\n",
			rule->tr_name,
			rule->tr_jobids_str,
			rule->tr_rpc_rate,
			rule->tr_ref - 1);
...
struct nrs_tbf_ops nrs_tbf_jobid_ops = {
	...
	.o_rule_start_default = nrs_tbf_jobid_start_default_rule,
	.o_hash_create = nrs_tbf_jobid_hash_create,
	.o_hash_lookup = nrs_tbf_jobid_hash_lookup,
	.o_hash_findadd_unique = nrs_tbf_jobid_hash_findadd_unique,
	.o_client_init_info = nrs_tbf_jobid_client_init_info,
...
static cfs_hash_t *nrs_tbf_nid_hash_create(void)
{
	return cfs_hash_create("nrs_tbf_hash",
			       NRS_TBF_NID_BITS,
			       NRS_TBF_NID_BITS,
...
static struct nrs_tbf_client *
nrs_tbf_nid_hash_lookup(cfs_hash_t *hash,
			struct ptlrpc_request *req)
{
	return cfs_hash_lookup(hash, &req->rq_peer.nid);
}

static struct nrs_tbf_client *
nrs_tbf_nid_hash_findadd_unique(cfs_hash_t *hash,
				struct nrs_tbf_client *cli)
{
	return cfs_hash_findadd_unique(hash, &cli->tc_nid,
				       &cli->tc_hnode);
}

static int
nrs_tbf_nid_start_default_rule(struct ptlrpc_nrs_policy *policy,
			       struct nrs_tbf_head *head)
{
	struct nrs_tbf_cmd	 start;
	struct nrs_tbf_rule	*rule = NULL;
...
static void
nrs_tbf_nid_client_init_info(struct nrs_tbf_client *cli,
			     struct ptlrpc_request *req)
{
...
struct nrs_tbf_ops nrs_tbf_nid_ops = {
	...
	.o_rule_start_default = nrs_tbf_nid_start_default_rule,
	.o_hash_create = nrs_tbf_nid_hash_create,
	.o_hash_lookup = nrs_tbf_nid_hash_lookup,
	.o_hash_findadd_unique = nrs_tbf_nid_hash_findadd_unique,
	.o_client_init_info = nrs_tbf_nid_client_init_info,
...
static int nrs_tbf_start(struct ptlrpc_nrs_policy *policy, char *arg)
{
	...
	} else if ((strlen(arg) == strlen(NRS_TBF_TYPE_JOBID)) &&
		  (strcmp(arg, NRS_TBF_TYPE_JOBID) == 0)) {
	...
	head->th_cli_hash = head->th_ops->o_hash_create();
	...
	rc = head->th_ops->o_rule_start_default(policy, head);
	if (rc)
			   ...
			   bool moving_req)
{
	...
	cli = head->th_ops->o_hash_lookup(head->th_cli_hash, req);
	if (cli != NULL) {
	...
	tmp = head->th_ops->o_hash_findadd_unique(head->th_cli_hash, cli);
	if (tmp != cli) {
		cfs_atomic_dec(&cli->tc_ref);
		nrs_tbf_cli_fini(cli);
		cli = tmp;
