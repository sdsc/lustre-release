		       ...
		       struct ptlrpc_request *req)
{
	...
	cli = nrs_tbf_jobid_hash_lookup(hs, &bd, jobid, &version);
	cfs_hash_bd_unlock(hs, &bd, 1);

	return cli;
}

/* Return remain number to shrink */
static int nrs_tbf_jobid_hash_purge(struct nrs_tbf_head *head, int nr)
{
	int			 bnr;
	int			 start;
	int			 i;
	...
	start = head->th_purge_start;
	bnr = (nr == ~0) ? -1 : nr / CFS_HASH_NBKT(head->th_cli_hash) + 1;
again:
	cfs_hash_for_each_bucket(head->th_cli_hash, &bd, i) {
		...
		count = bnr;
		cfs_hash_bd_lock(head->th_cli_hash, &bd, 1);
		bkt = cfs_hash_bd_extra_get(head->th_cli_hash, &bd);
		cfs_list_for_each_entry_safe(cli, tmp, &bkt->ntb_lru,
					     tc_lru) {
			LASSERT(cfs_atomic_read(&cli->tc_ref) == 0);

			cfs_hash_bd_get(head->th_cli_hash, &cli->tc_jobid,
					&bd2);
			LASSERT(bd.bd_bucket == bd2.bd_bucket);

			cfs_hash_bd_del_locked(head->th_cli_hash,
					       &bd2, &cli->tc_hnode);
			cfs_list_move(&cli->tc_lru, &dispose);
			did_sth = true;
		...
		while (!cfs_list_empty(&dispose)) {
			cli = container_of0(dispose.next,
	...
	/* race on head->th_purge_start, but nobody cares */
	head->th_purge_start = i % CFS_HASH_NBKT(head->th_cli_hash);
			  ...
			  struct nrs_tbf_client *cli)
{
	...
	jobid = cli->tc_jobid;
	cfs_hash_bd_get_and_lock(hs, (void *)jobid, &bd, 1);
	ret = nrs_tbf_jobid_hash_lookup(hs, &bd, jobid, &version);
	if (ret == NULL) {
	...
	if (ret == cli) {
		/**
		 * Only check when adding new entry.
		 * Maybe on a seperate thread?
		 */
		size = cfs_atomic_read(&head->th_hash_size) + 1;
		size -= tbf_jobid_cache_size;
		if (size > 0) {
			cfs_atomic_sub(size, &head->th_hash_size);
			size = nrs_tbf_jobid_hash_purge(head, size);
			if (size > 0)
				cfs_atomic_add(size, &head->th_hash_size);
		}
	}
	return ret;
}
		      ...
		      struct nrs_tbf_client *cli)
{
	...
	cfs_hash_bd_get(head->th_cli_hash, &cli->tc_jobid, &bd);
	bkt = cfs_hash_bd_extra_get(head->th_cli_hash, &bd);
	if (!cfs_hash_bd_dec_and_lock(head->th_cli_hash, &bd, &cli->tc_ref))
		return;
	if (cfs_list_empty(&cli->tc_lru)) /* TODO: remove me */
...
#define NRS_TBF_JOBID_BKT_BITS 5
		      ...
		      struct nrs_tbf_head *head)
{
	...
	cfs_atomic_set(&head->th_hash_size, 0);
			   ...
			   bool moving_req)
{
	...
	if (tmp != cli) {
		...
		cfs_atomic_inc(&head->th_hash_size);
	}
out:
	*resp = &cli->tc_res;
