...
int ptlrpc_register_bulk(struct ptlrpc_request *req)
{
	...
	/* A mbits is only used for a single request from the client.
	 * For retried bulk transfers, a new mbits will be allocated in
	 * in ptlrpc_check_set() if it needs to be resent, so it is not
	 * using the same RDMA match bits after an error.
	 *
	 * For multi-bulk RPCs, rq_mbits is the last mbits needed for bulks.
	 * The first bulk mbits is power-of-two aligned before rq_mbits.
	 * LU-1431 */
	mbits = req->rq_mbits & ~((__u64)desc->bd_md_max_brw - 1);
	...
	/* Set rq_mbits to matchbits of the final bulk so that server can
	 * infer the number of bulks that were prepared */
	req->rq_mbits = --mbits;
	LASSERTF(desc->bd_last_mbits == (req->rq_mbits & PTLRPC_BULK_OPS_MASK),
		 "bd_last_mbits = x"LPU64", rq_mbits = x"LPU64"\n",
		 desc->bd_last_mbits, req->rq_mbits);
...
int ptl_send_rpc(struct ptlrpc_request *request, int noreply)
{
	...
	if (request->rq_bulk != NULL) {
		if (request->rq_resend && request->rq_nr_resend == 0)
			ptlrpc_update_bulk_mbits(request);
		else
