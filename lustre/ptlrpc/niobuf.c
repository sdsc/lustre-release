...
int ptlrpc_start_bulk_transfer(struct ptlrpc_bulk_desc *desc)
{
	...
	if (exp_connect_multibulk(exp)) {
		xid = desc->bd_req->rq_xid &
			(~((exp_brw_size(exp) >> LNET_MTU_BITS) - 1));
...
void ptlrpc_abort_bulk(struct ptlrpc_bulk_desc *desc)
{
	...
	mdunlink_iterate_helper(desc->bd_mds,
				exp_brw_size(desc->bd_export) >> LNET_MTU_BITS);
...
int ptlrpc_register_bulk(struct ptlrpc_request *req)
{
	...
	brw_size = req->rq_import->imp_obd->u.cli.cl_max_pages_per_rpc <<
			CFS_PAGE_SHIFT;
	xid = req->rq_xid & (~((brw_size >> LNET_MTU_BITS) - 1));
	...
	for (posted_md = 0; posted_md < total_md; posted_md++, xid++) {
		...
		if (rc != 0) {
			CERROR("%s: LNetMEAttach failed for md %d: rc =  %d\n",
			       desc->bd_export->exp_obd->obd_name, posted_md,
			       rc);
			break;
		}
		...
		if (rc != 0) {
			CERROR("%s: LNetMDAttach failed for md %d: rc = %d\n",
			       desc->bd_export->exp_obd->obd_name, posted_md,
			       rc);
	...
	if (rc != 0) {
		...
		spin_unlock(&desc->bd_lock);
		LASSERT(desc->bd_md_count >= 0);
		mdunlink_iterate_helper(req->rq_bulk->bd_mds,
					brw_size >> LNET_MTU_BITS);
	...
	if (desc->bd_md_count != total_md)
		CWARN("%s: Peer %s touched %d buffers while I was "
		      "registering\n",
...
int ptlrpc_unregister_bulk(struct ptlrpc_request *req, int async)
{
	...
	brw_size = req->rq_import->imp_obd->u.cli.cl_max_pages_per_rpc <<
			CFS_PAGE_SHIFT;
	mdunlink_iterate_helper(desc->bd_mds, brw_size >> LNET_MTU_BITS);
