...
int ptlrpc_disconnect_import(struct obd_import *imp, int noclose)
{
	...
	/* import don't finish a recovery or FULL - notify server,
	 * to release a resources */
...
{
	/* import may be FULL but pinger don't found a link fail 
	 * so - reping it */
	if (ptlrpc_obd_ping(imp->imp_obd) == 0)
		RETURN(0);
	/* ping failed - wait recovery */
	return ptlrpc_recover_import(imp, NULL, 0);
}
