 ...
 */
static int
nrs_orr_init(struct ptlrpc_nrs_policy *policy)
{
	if (policy->pol_nrs->nrs_svcpt->scp_service->srv_ncpts > 1) {
		bool is_orr = strncmp(policy->pol_name, NRS_POL_NAME_ORR,
				      NRS_POL_NAME_MAX) == 0;
...
nrs_orr_start(struct ptlrpc_nrs_policy *policy)
{
	...
	if (strncmp(policy->pol_name, NRS_POL_NAME_ORR,
		    NRS_POL_NAME_MAX) == 0) {
