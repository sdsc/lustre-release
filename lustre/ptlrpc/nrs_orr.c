...
#define NRS_ORR_HFLAGS		 (CFS_HASH_RW_BKTLOCK | CFS_HASH_REHASH | \
				  CFS_HASH_SHRINK | CFS_HASH_ASSERT_EMPTY)

#define NRS_TRR_HBITS_LOW	 3
#define NRS_TRR_HBITS_HIGH	 8
#define NRS_TRR_HBBITS		 2
#define NRS_TRR_HFLAGS		 (CFS_HASH_SPIN_BKTLOCK | CFS_HASH_REHASH | \
				  CFS_HASH_SHRINK | CFS_HASH_ASSERT_EMPTY)
...
static void
nrs_orr_hop_put_locked(cfs_hash_t *hs, cfs_hlist_node_t *hnode)
{
	struct nrs_orr_object *orro = cfs_hlist_entry(hnode,
						      struct nrs_orr_object,
						      oo_hnode);
...
nrs_orr_start(struct ptlrpc_nrs_policy *policy)
{
	...
	/**
	 * TODO: Change this to true when ofd_get_info()-related issue is
	 * resolved, or disable physical offsets altogether for the moment.
	 */
