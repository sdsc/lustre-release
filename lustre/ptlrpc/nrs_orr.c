 ...
 */
static void
nrs_orr_genobjname(struct ptlrpc_nrs_policy *policy, char *name)
{
	snprintf(name, NRS_ORR_OBJ_NAME_MAX, "%s%s%s%d",
		 "nrs_", policy->pol_name,
...
#define NRS_ORR_HFLAGS		 (CFS_HASH_RW_BKTLOCK | CFS_HASH_REHASH |    \
				  CFS_HASH_COUNTER | CFS_HASH_SHRINK |	     \
...
nrs_orr_start(struct ptlrpc_nrs_policy *policy)
{
	...
	if (strcmp(policy->pol_name, NRS_POL_NAME_ORR) == 0) {
