	...
	if (likely(!peek && nrq != NULL)) {
		...
		if (strncmp(policy->pol_desc->pd_name, NRS_POL_NAME_ORR,
			...
			CDEBUG(D_RPCTRACE,
			       ...
			       "with id "LPX64", from OST with index %u, with "
			       "round "LPU64"\n", NRS_POL_NAME_ORR,
			       nrq->nr_u.orr.or_key.ok_id,
			     ...
			     struct ptlrpc_nrs_request *nrq)
{
	...
	if (strncmp(policy->pol_desc->pd_name, NRS_POL_NAME_ORR,
		    NRS_POL_NAME_MAX) == 0)
		CDEBUG(D_RPCTRACE,
		       "NRS: finished handling %s request for object with id "
		       LPX64" from OST with index %u, with round "LPU64"\n",
		       NRS_POL_NAME_ORR, nrq->nr_u.orr.or_key.ok_id,
