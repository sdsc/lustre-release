...
struct ptlrpc_nrs_request * nrs_orr_req_poll(struct ptlrpc_nrs_policy *policy)
{
	...
	/** The return value is asserted against NULL in nrs_request_poll() */
	return container_of(node, struct ptlrpc_nrs_request, nr_node);
}
					     ...
					     unsigned long count, void *data)
{
	 ...
	 * ptlrpc_nrs_pol_ops::op_policy_ctl(). -ENODEV is returned only if the
	 * operation fails with -ENODEV on all heads that have been specified
	 * by the command; if at least one operation succeeds, success is
	 * returned.
	 */
	if ((queue & PTLRPC_NRS_QUEUE_REG) != 0) {
		rc = ptlrpc_nrs_policy_control(svc, PTLRPC_NRS_QUEUE_REG,
