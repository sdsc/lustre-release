 ...
 * ------------------------------------------------
 *
 * it might not be precise and strictly unique but should be good enough.
 */
#define REQS_ALL_BITS		64
#define REQS_SEC_BITS		32
#define REQS_USEC_BITS		16
#define REQS_SEQ_BITS		8
#define REQS_CPT_BITS		8

void ptlrpc_req_add_history(struct ptlrpc_service_part *svcpt,
			    struct ptlrpc_request *req)
{
	...
	CLASSERT(REQS_CPT_BITS + REQS_SEQ_BITS +
		 REQS_USEC_BITS + REQS_SEC_BITS == REQS_ALL_BITS);

	tmp = sec << (REQS_USEC_BITS + REQS_SEQ_BITS + REQS_CPT_BITS);
	tmp |= usec << (REQS_SEQ_BITS + REQS_CPT_BITS);

	if (svcpt->scp_hist_seq_time < tmp) {
		svcpt->scp_hist_seq_time = tmp;
		svcpt->scp_hist_seq_num = 0; /* reset it */
	}

	req->rq_history_seq = svcpt->scp_hist_seq_time;

	/* it's unlikely to overflow 8 bits sequence in 16 usecs, but... */
	tmp = (svcpt->scp_hist_seq_num++) & ((1U << REQS_SEQ_BITS) - 1);
	req->rq_history_seq |= tmp << REQS_CPT_BITS;

	/* it's unlikely to have more than 256 partitions, but... */
	tmp = svcpt->scp_cpt & ((1U << REQS_CPT_BITS) - 1);
	req->rq_history_seq |= tmp;
