...
static void corrupt_bulk_data(struct ptlrpc_bulk_desc *desc)
{
	...
	for (i = 0; i < desc->bd_iov_count; i++) {
		if (desc->bd_u.bd_kiov.bd_vec[i].kiov_len == 0)
			continue;

		ptr = kmap(desc->bd_u.bd_kiov.bd_vec[i].kiov_page);
		off = desc->bd_u.bd_kiov.bd_vec[i].kiov_offset & ~CFS_PAGE_MASK;
		ptr[off] ^= 0x1;
		kunmap(desc->bd_u.bd_kiov.bd_vec[i].kiov_page);
		return;
	}
}
                          ...
                          struct ptlrpc_bulk_desc *desc)
{
	...
	LASSERT(IS_BULK_DESC_KIOV(desc->bd_type));
        ...
        for (i = 0, nob = 0; i < desc->bd_iov_count; i++) {
                if (desc->bd_u.bd_kiov.bd_vec[i].kiov_len +
