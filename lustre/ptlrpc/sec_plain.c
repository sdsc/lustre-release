...
static void corrupt_bulk_data(struct ptlrpc_bulk_desc *desc)
{
	...
	for (i = 0; i < desc->bd_iov_count; i++) {
		if (GET_KIOV(desc)[i].kiov_len == 0)
			continue;

		ptr = kmap(GET_KIOV(desc)[i].kiov_page);
		off = GET_KIOV(desc)[i].kiov_offset & ~CFS_PAGE_MASK;
		ptr[off] ^= 0x1;
		kunmap(GET_KIOV(desc)[i].kiov_page);
		return;
	}
}
