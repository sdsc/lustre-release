 ...
 */
int nm_member_del(struct lu_nodemap *nodemap, struct obd_export *exp)
{
 ...
 */
static int nm_member_get_exports_cb(cfs_hash_t *hs, cfs_hash_bd_t *bd,
				  struct hlist_node *hnode, void *data)
{
 ...
 */
static void nm_member_revoke_exp_locks(struct obd_export *exp)
{
 ...
 * nodemap whose NID ranges contain the export's NID.
 *
...
void nm_member_reclassify_nodemap(struct lu_nodemap *nodemap)
{
	...
	list_for_each_entry_safe(exp, tmp_exp, &reclassify_exports,
		...
		if (nodemap->nm_id != new_nodemap->nm_id) {
			...
			nm_member_add(new_nodemap, exp);

			nm_member_revoke_exp_locks(exp);
		}
	}
}
