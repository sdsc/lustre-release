...
{
	struct ptlrpc_nrs_policy       *policy = data;
	struct ptlrpc_service_part     *svcpt = ptlrpc_nrspol2svcpt(policy);
	int				rc;
	int				val;
				   ...
				   unsigned long count, void *data)
{
	...
	rc = ptlrpc_nrs_policy_control(svcpt, PTLRPC_NRS_QUEUE_BOTH,
				       policy->pol_type,
				       PTLRPC_NRS_CTL_ORR_WR_SUPP_REQ,
				       supp);
	if (rc < 0)
		return rc;
...
ptlrpc_lprocfs_register_svc_parts(struct ptlrpc_service *svc)
{
	...
	svc->srv_cpt_procroot = lprocfs_register("cpt", svc->srv_procroot, NULL,
						 NULL);
	if (IS_ERR(svc->srv_cpt_procroot))
		return PTR_ERR(svc->srv_cpt_procroot);

	ptlrpc_service_for_each_part(svcpt, i, svc) {
		snprintf(cptidstr, sizeof(cptidstr), "%d", svcpt->scp_cpt);
                svcpt->scp_procroot = lprocfs_register(cptidstr,
						       svc->srv_cpt_procroot,
						       NULL, NULL);
                                     ...
                                     struct ptlrpc_service *svc)
{
	...
	if (rc)
                CERROR("Failed to initalize lprocfs for %s service CPTs\n",
		       svc->srv_name);
}
