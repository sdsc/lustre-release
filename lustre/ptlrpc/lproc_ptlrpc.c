...
/**
 * Max valid comand string is "65535 both"; 65535 because
 * nrs_crrn_client::cc_quantum and nrs_orr_object::oo_quantum are __u16.
 */
#define LPROCFS_NRS_WR_QUANTUM_MAX_CMD	(sizeof("65535 both"))
 ...
 * carried out only on the regular NRS head's policy instance. Command strings
 ...
 * TODO: the CRR-N version of this, ptlrpc_lprocfs_rd_nrs_crrn_quantum() is
...
no_hp:
	 ...
	 *	Regular requests:
	 *
	 *	  - RR quantum size (# of RPCs): 256
	 *
	 *	High-priority requests:
	 *
	 *	  - RR quantum size (# of RPCs): 32
	 */
	*eof = 1;
...
/**
 * Sets the value of the Round Robin quantum (i.e. the maximum batch size)
 * for ORR/TRR policy instances of a service. By default, the operation is
 * carried out only on the regular NRS head's policy instance. Command strings
 * are of the format "<value> [both|hp]"; in case where the optional [both|hp]
 * token is specified, the operation is carried out on either both NRS heads or
 * the high-priority NRS head respectively.
 ...
 * For example:
 *
 * lclt set_param *.*,*.nrs_orr_quantum="128"
 * lclt set_param *.*,*.nrs_trr_quantum="512 both"
 *
 * policy instances in the ptlrpc_nrs_pol_state::NRS_POL_STATE_STOPPED state are
 * are skipped later by nrs_orr_ctl().
 * TODO: the CRR-N version of this, ptlrpc_lprocfs_wr_nrs_crrn_quantum() is
 * almost identical; it can be reworked and then be reused for ORR/TRR.
 */
static int
...
/**
 * Max valid command string is "physical both"
 */
#define LPROCFS_NRS_WR_ORR_OFFSET_MAX_CMD	sizeof("physical both")

/**
 * Sets the type of offsets used to order RPCs in ORR/TRR policy instances.
 * Command strings are of the format "<physical|logical> [both|hp]; in cases
 * where the optional [both|hp] token is specified, the operation is carried out
 * on either both NRS heads or the high-priority NRS head respectively.
 *
 * For example:
 *
 * lclt set_param ost.OSS.ost_io.nrs_orr_offset_type=physical
 * lclt set_param ost.OSS.ost_io.nrs_trr_offset_type="logical both"
 *
 * policy instances in the ptlrpc_nrs_pol_state::NRS_POL_STATE_STOPPED state are
 * are skipped later by nrs_orr_ctl().
 */
				      ...
				      unsigned long count, void *data)
{
	...
	if (count >= LPROCFS_NRS_WR_ORR_OFFSET_MAX_CMD)
		GOTO(out, rc = -EINVAL);

	OBD_ALLOC(cmd, LPROCFS_NRS_WR_ORR_OFFSET_MAX_CMD);
	if (cmd == NULL)
		GOTO(out, rc = -ENOMEM);
	...
	if (strcmp(token, "physical") == 0)
		physical = true;
	else if (strcmp(token, "logical") == 0)
		physical = false;
	else
		GOTO(out, rc = -EINVAL);
...
out:
	if (cmd_copy)
		OBD_FREE(cmd_copy, LPROCFS_NRS_WR_ORR_OFFSET_MAX_CMD);

	RETURN(rc < 0 ? rc : count);
}

/**
 * Translates snum nrs_orr_supp values to a corresponding string.
 */
static const char *
...
no_hp:
	...
	/**
	 * Supported RPC types information is a
	 * [OST_READ|OST_WRITE|OST_READ and OST_WRITE] string, and output is in
	 * YAML format.
	 *
	 * For example:
	 *
	 *	Regular requests:
	 *
	 *	  - Supported RPC types: OST_READ and OST_WRITE
	 *
	 *	High-priority requests:
	 *
	 *	  - Supported RPC types: OST_READ
	 */
	*eof = 1;
...
/**
 * Max valid command string is "readwrite both"
 */
#define LPROCFS_NRS_WR_ORR_SUPP_MAX_CMD		sizeof("readwrite both")

/* Set the currectly handled type of RPCs for an ORR/TRR policy.
 * Inputs are "read", "write", "readwrite".
 */
/**
 * Sets the type of RPCs handled by ORR/TRR policy instances. Command strings
 * are of the format "<read|write|readwrite> [both|hp]; in cases
 * where the optional [both|hp] token is specified, the operation is carried out
 * on either both NRS heads or the high-priority NRS head respectively; if the
 * optional token is omitted, the operation is carried only on the regular NRS
 * head.
 *
 * For example:
 *
 * lclt set_param ost.OSS.ost_io.nrs_orr_supported=readwrite
 * lclt set_param ost.OSS.ost_io.nrs_trr_supported="write both"
 *
 * policy instances in the ptlrpc_nrs_pol_state::NRS_POL_STATE_STOPPED state are
 * are skipped later by nrs_orr_ctl().
 */
				    ...
				    unsigned long count, void *data)
{
	...
	if (count >= LPROCFS_NRS_WR_ORR_SUPP_MAX_CMD)
		GOTO(out, rc = -EINVAL);

	OBD_ALLOC(cmd, LPROCFS_NRS_WR_ORR_SUPP_MAX_CMD);
	if (cmd == NULL)
		GOTO(out, rc = -ENOMEM);
...
dflt_queue:
	 ...
	 */
	if (queue == PTLRPC_NRS_QUEUE_REG || queue == PTLRPC_NRS_QUEUE_BOTH) {
		rc = ptlrpc_nrs_policy_control(svc, PTLRPC_NRS_QUEUE_REG, orr_data->name,
	...
	if (queue == PTLRPC_NRS_QUEUE_HP || queue == PTLRPC_NRS_QUEUE_BOTH) {
		rc = ptlrpc_nrs_policy_control(svc, PTLRPC_NRS_QUEUE_HP, orr_data->name,
					       NRS_CTL_ORR_WR_SUPP_REQ, false,
					       &supp);
		if (rc == -ENODEV)
			rc = 0;
...
out:
	if (cmd_copy)
		OBD_FREE(cmd_copy, LPROCFS_NRS_WR_ORR_SUPP_MAX_CMD);
