				   ...
				   int count, int *eof, void *data)
{
	...
	*eof = 1;
	return snprintf(page, count, "%d MB\n",
			(svc->srv_nbuf_max / 1024) * svc->srv_buf_size / 1024);
}
				    ...
				    unsigned long count, void *data)
{
	 ...
	 * memory. */
	if (val > PTLRPC_NBUFS_MEM_MAX_DEFAULT_MB)
		return -ERANGE;
	spin_lock(&svc->srv_lock);
	svc->srv_nbuf_max = val * 1024 / (svc->srv_buf_size / 1024);
	spin_unlock(&svc->srv_lock);
					  ...
					  int *eof, void *data)
{
	...
	*eof = 1;
	ptlrpc_service_for_each_part(svcpt, i, svc) {
		n += snprintf(page, count, "network buffers:\n\tcreated: %d\n"
			"\tavailable: %d\n\tidle: %d\n"
			"\thistory: %d\n\tmax allowed: %d\n\tsize: %d\n"
			"\ttotal memory used: "LPU64" MB\n"
			"requests:\n\tactive: %d\n\thp active: %d\n"
			"\tincoming: %d\n",
			svcpt->scp_nrqbds_total, svcpt->scp_nrqbds_posted,
			svcpt->scp_nrqbds_idle, svcpt->scp_hist_nrqbds,
			svc->srv_nbuf_max, svc->srv_buf_size,
			(__u64)svcpt->scp_nrqbds_total * svc->srv_buf_size / 1024 / 1024,
			svcpt->scp_nreqs_active, svcpt->scp_nhreqs_active,
