			 ...
			 struct ptlrpc_service_part *svcpt, int cpt)
{
	...
	cfs_atomic_set(&svcpt->scp_rpc_number, 0);
...
static void ptlrpc_service_summary_cb(ulong_ptr_t data)
{
	...
	for (i = 0; i < svc->srv_ncpts; i++) {
		...
		cpt_data = cfs_atomic_read(&svcpt->scp_rpc_number);
		new_data += cpt_data;
		cfs_atomic_sub(cpt_data, &svcpt->scp_rpc_number);
	}
			     ...
			     struct ptlrpc_thread *thread)
{
	...
	cfs_atomic_inc(&svcpt->scp_rpc_number);
