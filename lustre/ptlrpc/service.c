...
void ptlrpc_server_drop_request(struct ptlrpc_request *req)
{
	...
	if (req->rq_at_linked) {
		...
		/* NB: it's unnecessary to re-check because implicit barriers
		 * in ptlrpc_at_check_timed() can guarantee order of changing
		 * req::rq_refcount and req::rq_at_linked, but we still check
		 * again in case people want to change ptlrpc_at_check_timed()
		 * in the future, please see LU-1721 */
...
static int ptlrpc_at_check_timed(struct ptlrpc_service_part *svcpt)
{
	...
	if (first > at_early_margin) {
        ...
        while (count > 0) {
			 ...
			 */
			/* NB: do this after ptlrpc_at_remove_timed() to
			 * make sure ptlrpc_server_drop_request() can see
			 * changes in order */
