...
ptlrpc_service_summary_update(struct ptlrpc_service *svc, int new_data)
{
	...
	summary->ss_last_numer = new_data;

	cfs_list_for_each(l, &summary->ss_history_head) {
		period = cfs_list_entry(l, struct ptlrpc_summary_period,
					sp_linkage);
		if (period->sp_carry_times < summary->ss_width - 1) {
			period->sp_carry_times++;
			period->sp_carry += new_data;
			break;
		} else {
			tmp = period->sp_statistic;
			period->sp_statistic = period->sp_carry +
					       new_data;
...
static void ptlrpc_service_summary_cb(ulong_ptr_t data)
{
	...
	struct ptlrpc_service_summary *summary = &svc->srv_summary;
	int new_data = cfs_atomic_read(&summary->ss_rpc_number);
	ENTRY;

	ptlrpc_service_summary_update(svc, new_data);
	cfs_atomic_sub(new_data, &summary->ss_rpc_number);
...
ptlrpc_service_summary_fini(struct ptlrpc_service *svc)
{
	...
	cfs_list_for_each_safe(l, tmp, &summary->ss_history_head) {
		period = cfs_list_entry(l, struct ptlrpc_summary_period,
					sp_linkage);
		cfs_list_del(&period->sp_linkage);
		OBD_FREE_PTR(period);
	}
...
static int ptlrpc_service_summary_init(struct ptlrpc_service *svc)
{
	...
	summary->ss_width = summary_width;
	length = summary->ss_timeout;
	CFS_INIT_LIST_HEAD(&summary->ss_history_head);
	for (i = 0; i < summary->ss_depth; i++) {
		length *= summary->ss_width;
		OBD_ALLOC_PTR(period);
		if (period == NULL)
			GOTO(fail, rc = -ENOMEM);
		period->sp_depth = i;
		period->sp_length = length;
		cfs_list_add_tail(&period->sp_linkage,
				  &summary->ss_history_head);
	}

	cfs_atomic_set(&summary->ss_rpc_number, 0);
			     ...
			     struct ptlrpc_thread *thread)
{
	...
	cfs_atomic_inc(&svc->srv_summary.ss_rpc_number);
