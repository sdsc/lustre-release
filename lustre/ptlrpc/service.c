...
static int ptlrpc_server_check_resend_in_progress(struct ptlrpc_request *req)
{
	...
	__u64 req_xid = req->rq_xid;

	if (!(lustre_msg_get_flags(req->rq_reqmsg) & MSG_RESENT) ||
	    (cfs_atomic_read(&req->rq_export->exp_rpc_count) == 0))
		return 0;

	/* We cannot compare rq_xid of bulk requests as it is increased on
	 * client upon resending, use reqmsg last_xid value. */
	if (req->rq_bulk_write || req->rq_bulk_read) {
		req_xid = lustre_msg_get_last_xid(req->rq_reqmsg);
		DEBUG_REQ(D_RPCTRACE, req,
			  "Got bulk resent XID "LPU64"/"LPU64"\n",
			  req->rq_xid, req_xid);
	}
	...
	cfs_list_for_each_entry(tmp, &req->rq_export->exp_reg_rpcs,
				rq_exp_list) {
		/* Found duplicate one */
		if (tmp->rq_xid == req_xid ||
		    req_xid == lustre_msg_get_last_xid(tmp->rq_reqmsg))
			goto found;
	}
...
found:
	DEBUG_REQ(D_RPCTRACE, req, "Found duplicate req in processing\n");
	DEBUG_REQ(D_RPCTRACE, tmp, "Request is being processed\n");
	return -EBUSY;
}
