...
struct ptlrpc_bulk_frag_ops ptlrpc_bulk_kiov_nopin_ops = {
					      ...
					      struct ptlrpc_bulk_frag_ops *ops)
{
	...
	LASSERT((is_bulk_desc_kiov(type) && ops->add_kiov_frag != NULL) ||
		(is_bulk_desc_iovec(type) && ops->add_iov_frag != NULL));
			  ...
			  void *frag, int len)
{
	...
	iovec = &BD_GET_IOVEC(desc, desc->bd_iov_count);
...
inline void ptlrpc_release_page_pin(struct ptlrpc_bulk_desc *desc)
{
	int i;
