 ...
 *         handler = ptlrpc_alloc_work();
 *         while (1)
 *                 ptlrpc_run_work();
 *         ptlrpc_destroy_work();
 *    the above code snippet is correct in theory.
 * 2. ptlrpc_run_work() can be called by multiple processes meanwhile, but
 *    it will only be queued once in any time;
 */
...
#define PTLRPC_WORK_MAGIC 0x6655436b676f4f44 /* magic code for work async arg */
                        ...
                        int (*cb)(const struct lu_env *, void *), void *cbdata)
{
        ...
        args = (typeof(args))&req->rq_async_args;
...
EXPORT_SYMBOL(ptlrpc_alloc_work);
...
EXPORT_SYMBOL(ptlrpc_destroy_work);
