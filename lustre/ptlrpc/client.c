...
/**
 * If request has a new allocated XID (new request or EINPROGRESS resend), generate matchbits
 * from this XID, otherwise allocate a new matchbits for request to ensure previous bulk
 * fails and avoid problems with lost replies and therefore several transfers landing into
 * the same buffer from different sending attempts.
 */
void ptlrpc_set_bulk_mbits(struct ptlrpc_request *req)
{
	...
	/* For multi-bulk RPCs, rq_mbits is the last mbits needed for bulks so that server can
	 * infer the number of bulks that were prepared, see LU-1431 */
	req->rq_mbits &= ~((__u64)bd->bd_md_max_brw - 1);
	req->rq_mbits += (bd->bd_iov_count + LNET_MAX_IOV - 1) / LNET_MAX_IOV - 1;
}
