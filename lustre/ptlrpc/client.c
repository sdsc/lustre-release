...
static int ptlrpc_send_new_req(struct ptlrpc_request *req)
{
	...
	/* find the known replied XID but skip for the CONNECT request,
	 * because we don't want send server a known replied XID which
	 * is greater than the XID of replay requests. */
	if (!ptlrpc_req_is_connect(req))
