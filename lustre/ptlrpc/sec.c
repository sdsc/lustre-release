...
char *sptlrpc_secflags2str(__u32 flags, char *buf, int bufsize)
{
		...
		strncat(buf, "reverse,", bufsize-1-strlen(buf));
        if (flags & PTLRPC_SEC_FL_ROOTONLY)
		strncat(buf, "rootonly,", bufsize-1-strlen(buf));
        if (flags & PTLRPC_SEC_FL_UDESC)
		strncat(buf, "udesc,", bufsize-1-strlen(buf));
        if (flags & PTLRPC_SEC_FL_BULK)
		strncat(buf, "bulk,", bufsize-1-strlen(buf));
        if (buf[0] == '\0')
