				 ...
				 struct thandle *th)
{
	...
	if (lu_object_exists(&dt->do_lu)) {
		...
		rc = out_ref_del_pack(env, lu_object_fid(&dt->do_lu),
		...
		rc = out_object_destroy_pack(env, lu_object_fid(&dt->do_lu),
					     &update->dur_buf,
					     update->dur_batchid);
		if (rc != 0)
			GOTO(out, rc);
	...
	rc = out_create_pack(env, lu_object_fid(&dt->do_lu), attr, hint, dof,
			     &update->dur_buf, update->dur_batchid);
	if (rc != 0)
		GOTO(out, rc);
out:
				  ...
				  struct dt_object *dt, struct thandle *th)
{
	...
	rc = out_ref_add_pack(env, lu_object_fid(&dt->do_lu),
			      &update->dur_buf, update->dur_batchid);
			    ...
			    const struct lu_attr *attr, struct thandle *th)
{
	...
	rc = out_attr_set_pack(env, lu_object_fid(&dt->do_lu), attr,
			       &update->dur_buf, update->dur_batchid);
			       ...
			       struct lustre_capa *capa)
{
	...
	rc = out_index_lookup_pack(env, lu_object_fid(&dt->do_lu), rec, key,
				   &update->dur_buf);
	if (rc != 0) {
		CERROR("%s: Insert update error: rc = %d\n",
				 ...
				 struct thandle *th)
{
	...
	rc = out_index_insert_pack(env, lu_object_fid(&dt->do_lu), rec, key,
				   &update->dur_buf, update->dur_batchid);
	return rc;
}
				 ...
				 struct thandle *th)
{
	...
	rc = out_index_delete_pack(env, lu_object_fid(&dt->do_lu), key,
				   &update->dur_buf, update->dur_batchid);
	return rc;
}
				    ...
				    loff_t pos, struct thandle *th)
{
	...
	rc = out_write_pack(env, lu_object_fid(&dt->do_lu), buf, pos,
			    &update->dur_buf, update->dur_batchid);
