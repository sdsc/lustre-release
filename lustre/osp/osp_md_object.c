...
out_swabbed:
	update_reply_set_swabbed(reply, 0);
out:
	if (req != NULL)
		ptlrpc_req_finished(req);
			       ...
			       struct lustre_capa *capa)
{
	...
	if (size < sizeof(*fid)) {
		...
		GOTO(out_swabbed, rc = -EINVAL);
	}

	if (ptlrpc_rep_need_swab(req) && update_reply_need_swab(reply, 0))
...
out_swabbed:
	/* set swabbed before returning an error, or it may be swabbed again */
	update_reply_set_swabbed(reply, 0);
out:
	if (req != NULL)
		ptlrpc_req_finished(req);
