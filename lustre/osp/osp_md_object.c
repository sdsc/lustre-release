		       ...
		       struct ptlrpc_request **reqp)
{
	...
	/* FIXME: If UPDATE_RECREATE is set, it means the update(create remote
                    ...
                    struct thandle *th)
{
	...
	if (th->th_sync == 0)
		th->th_sync = 1;
	/* Only one thread will access this list, no need lock */
...
/**
 * FIXME: We probably do not need these read/write lock for remote MD object.
 */
                                    ...
                                    struct dt_object *dt, unsigned role)
{
        ...
        cfs_down_read_nested(&obj->opo_sem, role);
                                     ...
                                     struct dt_object *dt, unsigned role)
{
        ...
        cfs_down_write_nested(&obj->opo_sem, role);
...
static void osp_it_fini(const struct lu_env *env, struct dt_object *dt,
                        struct dt_it *di)
{
...
static void osp_it_put(const struct lu_env *env,
                       struct dt_object *dt,
                       struct dt_it *di)
{
        return;
...
static int osp_it_next(const struct lu_env *env,
...
static int osp_it_key_size(const struct lu_env *env,
                           struct dt_object *dt,
                           const struct dt_it *di)
{
        LBUG();
...
static int osp_it_rec(const struct lu_env *env, struct dt_object *dt,
                      const struct dt_it *di, struct dt_rec *lde,
                      __u32 attr)
{
        LBUG();
        return 0;
...
static __u64 osp_it_store(const struct lu_env *env, struct dt_object *dt,
                         const struct dt_it *di)
{
        LBUG();
        return 0;
...
static int osp_it_load(const struct lu_env *env, struct dt_object *dt,
...
static int osp_it_key_rec(const struct lu_env *env, struct dt_object *dt,
                            ...
                            const struct dt_index_features *feat)
{
	...
	/* FIXME: check whether the object is DIR or regular file */
	return 0;
}
                              ...
                              void *policy)
{
	...
	struct dt_device       *dt_dev = lu2dt_dev(dt->do_lu.lo_dev);
        ...
        req = ldlm_enqueue_pack(osp->opd_exp);
        if (IS_ERR(req))
                RETURN(PTR_ERR(req));
