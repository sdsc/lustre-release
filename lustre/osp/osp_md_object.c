 ...
 *
 * If there are mulitple MDTs in one filesystem, one operation might
 * includes modifications in several MDTs. In such cases, clients
 ...
 *
 * It marks necessary flags for created object. In DNE phase I,
 ...
 *
 * osp_md_object_xx_lock will only lock the remote object in local cache,
 * which uses the semaphore (opo_sem) inside the osp_object to lock the
 * object. Note: it will not lock the object in the whole cluster, which
 * will rely on the ldlm lock.
 *
 * \param[in] env	execution environment
 ...
 *
 * Look up record by key under a remote index object, and it packs lookup update
 * into RPC and send to the remote OUT and waits for the lookup result.
 *
 * \param[in] env	execution environment
 ...
 *                      record. Note: it is impossible to return 0.
 */
 ...
 *
 * Do nothing in this method for now. Becase in DNE phase I, remote updates
 ...
 *
 * Advance the pointer of the iterator to the next entry. It shares the similar
 ...
 *
 * Get the key at current iterator poisiton. Because this will only be
 * used for remote directory, it will convert entry to lu_dirent and
 * return the name(lde_name) of the directory entry.
 *
 * \param[in] env	execution environment
 * \param[in] di	iterator of this iteration
 *
 ...
 *
 * Get the key size at current iterator poisiton. Because this will only
 * be used for remote directory, it will convert the entry to lu_dirent and
 * return the name size(lde_namelen) of the directory entry.
 *
 * \param[in] env	execution environment
 * \param[in] di	iterator of this iteration
 *
 ...
 *
 * Get the record at current iterator position. Because this will only be used
 * for remote directory, it will use lu_dirent_calc_size() to calculate the
 * record size.
 *
 * \param[in] env	execution environment
 * \param[in] di	iterator of this iteration
 ...
 *
 * Try to initialize the index api pointer for the given object. It
 * is the entry point of the index api, i.e. we must call this method
 * first to initialize the index object, then call other index methods.
 *
 * \param[in] env	execution environment
