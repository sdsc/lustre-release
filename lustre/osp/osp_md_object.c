				    ...
				    loff_t pos, struct thandle *th)
{
	...
	if (!(obj->opo_ooa->ooa_attr.la_valid & LA_SIZE)) {
					...
					void *data, int index, int rc)
{
	...
	if (rc < 0 && obj->opo_ooa != NULL) {
		...
		obj->opo_ooa->ooa_attr.la_valid = 0;
		spin_unlock(&obj->opo_lock);
		CDEBUG(D_INFO, "invalid "DFID" attr\n",
			    ...
			    struct thandle *th, int ignore_quota)
{
	...
	*pos += buf->lb_len;
	...
	LASSERT(obj->opo_ooa != NULL);
	LASSERT(obj->opo_ooa->ooa_attr.la_valid & LA_SIZE);
	if (obj->opo_ooa->ooa_attr.la_size < *pos)
