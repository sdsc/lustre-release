...
static int osp_updates_reply_handler(const struct lu_env *env,
				     ...
				     struct update_request *update)
{
	...
	/* Only one thread can operate this list, no need lock */
	cfs_list_for_each_entry(callback, &update->ur_cb_list,
				uc_list) {
		rc1 = callback->uc_callback(env, req, callback->uc_idx,
					    callback->uc_args);
		if (rc1 != 0) {
			CDEBUG(D_INFO, "%s: %dth cb failed: rc = %d\n",
			   ...
			   struct ptlrpc_request **reqp)
{
	...
	rc = osp_updates_reply_handler(env, req, update);
...
static void osp_destroy_update_req(struct update_request *update)
{
	...
	/* Only 1 thread can operate on the list, no need lock */
 ...
 * description is in lustre_idl.h) during delcare phase, all of updates
					 ...
					 int index, void *arg)
{
	...
	rc = osp_get_attr_from_req(env, req, attr, index);
	if (rc)
		return rc;
					...
					struct thandle *th)
{
	...
	lu_attr_to_obdo(&osi->osi_obdo, attr);
				   ...
				   struct thandle *th)
{
	...
	lu_attr_to_obdo(&osi->osi_obdo, (struct lu_attr *)attr);
	lustre_set_wire_obdo(&osi->osi_obdo, &osi->osi_obdo);
	obdo_cpu_to_le(&osi->osi_obdo, &osi->osi_obdo);
				    ...
				    struct thandle *th)
{
	...
	fid = (struct lu_fid *)lu_object_fid(&dt->do_lu);
	rc = osp_insert_update(env, update, OBJ_XATTR_SET, fid, 3, sizes, bufs);
			   ...
			   struct lustre_capa *capa)
{
	...
	struct dt_device      *dt_dev = lu2dt_dev(dt->do_lu.lo_dev);
	struct update_request *update = NULL;
	struct lu_attr        *tmp_attr = &osp_env_info(env)->osi_attr;
	struct ptlrpc_request *req = NULL;
	int rc;
	ENTRY;
	...
	rc = osp_get_attr_from_req(env, req, tmp_attr, 0);
	if (rc)
		GOTO(out, rc);
