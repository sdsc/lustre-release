...
{
	return th->th_storage_th->th_dev != NULL &&
	       th->th_storage_th->th_dev->dd_ops == &osp_dt_ops;
}
...
#define osp_update_rpc_pack(env, name, rc, ubuf, op, ...)		\
do {                                                                    \
	...
	struct object_update_request *ureq;				\
									\
	ureq = (ubuf)->ub_req;						\
	max_update_length = (ubuf)->ub_req_size -			\
	...
	if (rc == -E2BIG) {						\
		...
		rc1 = osp_extend_update_buffer(env, ubuf);		\
		if (rc1 != 0) {						\
			rc = rc1;					\
			break;						\
		}							\
...
int osp_insert_async_request(const struct lu_env *env,
			     enum update_type op, struct osp_object *obj,
			     int count, __u16 *lens, const void **bufs,
			     void *data, osp_update_interpreter_t interpterer);
