...
{
	return container_of0(d, struct lwp_device, lpd_dt_dev.dd_lu_dev);
}

static inline struct lu_device *lwp2lu_dev(struct lwp_device *d)
{
	return &d->lpd_dt_dev.dd_lu_dev;
}
	      ...
	      struct lu_device_type *ldt, struct lustre_cfg *cfg)
{
	...
	lwp->lpd_dt_dev.dd_lu_dev.ld_ops = &lwp_lu_ops;
	lwp->lpd_dt_dev.dd_ops = NULL;
	lwp->lpd_obd->obd_lu_dev = &lwp->lpd_dt_dev.dd_lu_dev;
					  ...
					  struct lustre_cfg *lcfg)
{
	...
	if (m == NULL) {
		...
		l = lwp2lu_dev(m);
		dt_device_init(&m->lpd_dt_dev, t);
		rc = lwp_init0(env, m, t, lcfg);
		if (rc != 0) {
