				   ...
				   struct llog_rec_hdr *h)
{
	...
	LASSERTF(rec->lur_count == 1, "lur_count = %d\n",
		 rec->lur_count);
				   ...
				   void *data)
{
	...
	do {
		...
		if (osp_sync_low_in_flight(d) &&
		    d->opd_syn_prev_done != 0 && b != NULL &&
		    ((id >= d->opd_syn_last_committed_id && inbatch) ||
		     inbatch > 512)) {
