			      ...
			      struct dt_txn_commit_cb *dcb, int err)
{
	...
	CDEBUG(D_HA, "%s: %Lu committed\n", d->opd_obd->obd_name,
	       cb->ospc_transno);

	cfs_spin_lock(&d->opd_syn_lock);
	if (cb->ospc_transno > d->opd_syn_last_committed_id)
		d->opd_syn_last_committed_id = cb->ospc_transno;
			   ...
			   struct thandle *th, __u64 transno)
{
	...
	if (cfs_time_before(cfs_time_current(), d->opd_syn_next_commit_cb))
		return 0;
	...
	if (cb == NULL)
		return -ENOMEM;
	cb->ospc_dev = d;
	cb->ospc_transno = transno;

	dcb = &cb->ospc_cb;
	dcb->dcb_func = osp_sync_local_commit_cb;

	cfs_spin_lock(&d->opd_syn_lock);
	if (cfs_time_aftereq(cfs_time_current(), d->opd_syn_next_commit_cb)) {
		d->opd_syn_next_commit_cb = cfs_time_shift(1);
		CDEBUG(D_HA, "%s: add commit cb at %Lu, next at %Lu\n",
		       d->opd_obd->obd_name,
		       (unsigned long long) cfs_time_current(),
		       (unsigned long long) d->opd_syn_next_commit_cb);
...
void osp_sync_force(const struct lu_env *env, struct osp_device *d)
{
	...
	if (rc == 0) {
		printk("%s: sync forced, %lu changes\n",
			d->opd_obd->obd_name, d->opd_syn_changes);
		cfs_spin_lock(&d->opd_syn_lock);
		id = d->opd_syn_last_used_id;
		cfs_spin_unlock(&d->opd_syn_lock);
		rc = osp_sync_add_commit_cb(env, d, th, id);
			    ...
			    const struct lu_attr *attr)
{
	...
	cfs_spin_lock(&d->opd_syn_lock);
	osi->osi_hdr.lrh_id = d->opd_syn_last_used_id++;
	cfs_spin_unlock(&d->opd_syn_lock);
	...
	rc = osp_sync_add_commit_cb(env, d, th, osi->osi_hdr.lrh_id);
...
int osp_sync_init(const struct lu_env *env, struct osp_device *d)
{
	...
	CFS_INIT_LIST_HEAD(&d->opd_syn_committed_there);
	d->opd_syn_next_commit_cb = cfs_time_shift(1);
	CDEBUG(D_HA, "%s: next commit cb at %Lu (now %Lu) \n",
	       d->opd_obd->obd_name,
	       (unsigned long long) d->opd_syn_next_commit_cb,
	       (unsigned long long) cfs_time_current());
