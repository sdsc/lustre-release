				 ...
				 struct osp_device *d)
{
	...
	req->rq_request_portal = OUT_OST_PORTAL;
	CFS_INIT_LIST_HEAD(&req->rq_exp_list);
	req->rq_svc_thread = (void *) OSP_JOB_MAGIC;
	...
	CDEBUG(D_OTHER, "%s: send update %p\n", d->opd_obd->obd_name, u);
				   ...
				   void *data)
{
	...
	do {
		 ...
		 */
		if (d->opd_syn_update)
			lwi = LWI_TIMEOUT(CFS_HZ, osp_sync_batch_expired, d);
		...
		if (osp_sync_running(d) && d->opd_syn_force_sync) {
			...
			CDEBUG(D_ERROR, "%s: flush updates\n", d->opd_obd->obd_name);
