...
static int osp_sync_process_single(const struct lu_env *env,
				   struct osp_device *d,
				   struct llog_handle *llh,
				   struct llog_rec_hdr *rec)
{
...
static int osp_sync_interpret_batched(const struct lu_env *env,
				      struct ptlrpc_request *req,
				      void *aa, int rc)
{
	struct osp_device	*d = req->rq_cb_data;
	...
	if (req->rq_transno == 0 && rc == 0)
		CDEBUG(D_ERROR, "reply req %p/%d, rc %d, transno %u\n", req,
		       cfs_atomic_read(&req->rq_refcount),
		       rc, (unsigned) req->rq_transno);
	...
	if (rc) {
		 ...
		 * error happened, we'll try to repeat on next boot ?
		 */
		CDEBUG(rc == -EIO ? D_OTHER : D_ERROR,
		       "%s: req %p/%Lx, rc = %d\n",
		       d->opd_obd->obd_name, req, req->rq_transno, rc);
		...
		if (req->rq_transno == 0) {
			...
			LASSERT(req->rq_async_args.pointer_arg[0] != NULL);
			OBD_FREE(req->rq_async_args.pointer_arg[0],
				 sizeof(struct llog_cookie) * UPDATE_MAX_OPS);
			req->rq_async_args.pointer_arg[0] = NULL;
		}

		wake_up(&d->opd_syn_waitq);
	}

	CDEBUG(D_OTHER, "%s: req %p/%Lx\n", d->opd_obd->obd_name,
		req, req->rq_transno);
...
static int osp_sync_send_batched(const struct lu_env *env,
				 struct osp_device *d)
{
	struct ptlrpc_request	*req;
	struct update_request	*u;
	int			 rc;
	ENTRY;
	...
	rc = osp_prep_update_req(env, d, u->ur_buf, UPDATE_BUFFER_SIZE, &req);
	LASSERT(rc == 0);

	CDEBUG(D_OTHER, "%s: send update %d/%ld\n", d->opd_obd->obd_name,
		u->ur_buf->ub_count, update_buf_size(u->ur_buf));

	d->opd_syn_batches++;
	d->opd_syn_updates_batched += u->ur_buf->ub_count;

	osp_destroy_update_req(d->opd_syn_update);
	d->opd_syn_update = NULL;

	LASSERT(d->opd_syn_update_cookies != NULL);
	LASSERT(req->rq_async_args.pointer_arg[0] == NULL);
	req->rq_async_args.pointer_arg[0] = d->opd_syn_update_cookies;
	d->opd_syn_update_cookies = NULL;
	...
	req->rq_interpret_reply = osp_sync_interpret_batched;
	req->rq_commit_cb = osp_sync_request_commit_cb;
	req->rq_cb_data = d;
...
static int osp_sync_process_batched(const struct lu_env *env,
				    struct osp_device *d,
				    struct llog_handle *llh,
				    struct llog_rec_hdr *rec)
{
	...
	/*
	 * the only thread processing the log (which is per OST)
	 * so no concurrency and no need in locking
	 */

repeat:
	update = d->opd_syn_update;
	if (update == NULL) {
		update = osp_create_update_req(&d->opd_dt_dev);
		if (IS_ERR(update))
			RETURN(PTR_ERR(update));
		d->opd_syn_update = update;
		CDEBUG(D_OTHER, "new batch %p\n", update);

		OBD_ALLOC(d->opd_syn_update_cookies,
			  sizeof(struct llog_cookie) * UPDATE_MAX_OPS);
		LASSERT(d->opd_syn_update_cookies);
	...
	CDEBUG(D_OTHER, "%s: adding %x into %p\n", d->opd_obd->obd_name,
	       rec->lrh_type, update);

	if (rec->lrh_type == MDS_UNLINK_REC) {
		/* case MDS_UNLINK_REC is kept for compatibility */
		LBUG();
		/*struct llog_unlink_rec	*ur = (struct llog_unlink_rec *)rec;
		rc = osp_insert_update(env, update, OBJ_DESTROY, &ur->lur_fid,
				       0, NULL, NULL);*/

	} else if (rec->lrh_type == MDS_UNLINK64_REC) {
		struct llog_unlink64_rec *ur = (struct llog_unlink64_rec *)rec;
		/* XXX: such a crappy crap! we need this conversion to support
		 *	IDIFs, notice 0 in the call to ostid_to_fid() */
		fid_to_ostid(&ur->lur_fid, &ostid);
		rc = ostid_to_fid(&fid, &ostid, 0);
		LASSERT(rc == 0);
		rc = osp_insert_update(env, update, OBJ_DESTROY, &fid,
				       0, NULL, NULL);

	} else if (rec->lrh_type == MDS_SETATTR64_REC) {
		struct llog_setattr64_rec *ur = (struct llog_setattr64_rec *)rec;
		int			   size = sizeof(struct obdo);
		char			  *buf;

		/* XXX: such a crappy crap! we need this conversion to support
		 *	IDIFs, notice 0 in the call to ostid_to_fid() */
		rc = ostid_to_fid(&fid, &ur->lsr_oi, 0);
		LASSERT(rc == 0);

		osi->osi_obdo.o_valid = LA_UID | LA_GID;
		osi->osi_obdo.o_uid = ur->lsr_uid;
		osi->osi_obdo.o_gid = ur->lsr_gid;

		lustre_set_wire_obdo(NULL, &osi->osi_obdo, &osi->osi_obdo);
		obdo_cpu_to_le(&osi->osi_obdo, &osi->osi_obdo);

		buf = (char *)&osi->osi_obdo;
		rc = osp_insert_update(env, update, OBJ_ATTR_SET, &fid, 1,
				       &size, &buf);
	} else {
		CERROR("unknown record type: %x\n", rec->lrh_type);
		LBUG();
	...
	if (rc == 0) {
		int i = update->ur_buf->ub_count - 1;
		LASSERT(i < UPDATE_MAX_OPS);
		d->opd_syn_update_cookies[i].lgc_lgl = llh->lgh_id;
		d->opd_syn_update_cookies[i].lgc_subsys = LLOG_MDS_OST_ORIG_CTXT;
		d->opd_syn_update_cookies[i].lgc_index = rec->lrh_index;

	} else if (rc == -E2BIG) {
		/* the buffer is full, fire the RPC off */
		rc = osp_sync_send_batched(env, d);
		goto repeat;
				   ...
				   struct llog_rec_hdr *rec)
{
	...
	if (unlikely(rec->lrh_type == LLOG_GEN_REC)) {
		...
		if (!memcmp(&d->opd_syn_generation, &gen->lgr_gen,
			...
			LCONSOLE(D_INFO, "%s: %d OLD updates in %d batches\n",
				 d->opd_obd->obd_name, d->opd_syn_updates_batched,
				 d->opd_syn_batches);
	...
	/* XXX: use batching if the peer supports */
	if (rec->lrh_type == MDS_UNLINK_REC)
		rc = osp_sync_process_single(env, d, llh, rec);
	else
		rc = osp_sync_process_batched(env, d, llh, rec);
...
static void osp_sync_cancel_committed(const struct lu_env *env,
				      struct osp_device *d,
				      struct llog_handle *llh,
				      struct ptlrpc_request *req)
{
	...
	if (lustre_msg_get_opc(req->rq_reqmsg) != UPDATE_OBJ) {
		/*
		 * legacy way - dedicated RPC per change
		 */
		struct ost_body *body;
	...
	u = req_capsule_client_get(&req->rq_pill, &RMF_UPDATE);

	CDEBUG(D_OTHER, "%s: req %p/%Lx, %d updates\n", d->opd_obd->obd_name,
		req, req->rq_transno, u->ub_count);

	cookies = req->rq_async_args.pointer_arg[0];
	LASSERT(cookies != NULL);

	for (i = 0; i < u->ub_count; i++) {
		size = update_get_reply_buf(reply, &tmp, i);

		/* if any error (except ENOENT) we just leave
		 * the record around till the next reboot */
		if (size != 0 && size != -ENOENT) {
			CERROR("update #%d failed: rc = %d\n",
				i, size);
			continue;
		}

		/* updated was applied successfully or
		 * no object was found - cancel the
		 * record */
		rc = llog_cat_cancel_records(env, llh, 1, cookies + i);
		if (rc)
			CERROR("%s: can't cancel: %d\n", obd->obd_name, rc);
	}

	OBD_FREE(cookies, sizeof(struct llog_cookie) * UPDATE_MAX_OPS);
	req->rq_async_args.pointer_arg[0] = NULL;
}
				       ...
				       struct osp_device *d)
{
	...
	cfs_list_for_each_entry_safe(req, tmp, &list, rq_exp_list) {
		...
		osp_sync_cancel_committed(env, d, llh, req);
...
static int osp_sync_batch_expired(void *data)
{
	struct osp_device *d = data;
	d->opd_syn_force_sync = 1;
	return 1;
}
				   ...
				   void *data)
{
	...
	do {
		...
		/*
		 * wait a bit and if no more updates to the batch, flush it
		 */
		if (d->opd_syn_update)
			lwi = LWI_TIMEOUT(HZ, osp_sync_batch_expired, d);

		l_wait_event(d->opd_syn_waitq,
			     !osp_sync_running(d) ||
			     osp_sync_can_process_new(d, rec) ||
			     d->opd_syn_force_sync ||
			     !cfs_list_empty(&d->opd_syn_committed_there),
			     &lwi);

		if (osp_sync_running(d) && d->opd_syn_force_sync) {
			d->opd_syn_force_sync = 0;
			CDEBUG(D_OTHER, "%s: flush updates\n",
			       d->opd_obd->obd_name);
			osp_sync_send_batched(env, d);
		}
...
static int osp_sync_thread(void *_arg)
{
	...
	if (d->opd_syn_rpc_in_progress > 0) {
		LASSERT(d->opd_syn_update != NULL);
		LASSERT(d->opd_syn_rpc_in_progress == 1);
		osp_destroy_update_req(d->opd_syn_update);
		LASSERT(d->opd_syn_update_cookies);
		OBD_FREE(d->opd_syn_update_cookies,
			 sizeof(struct llog_cookie) * UPDATE_MAX_OPS);
		spin_lock(&d->opd_syn_lock);
		d->opd_syn_rpc_in_progress--;
		spin_unlock(&d->opd_syn_lock);
...
static void osp_sync_id_traction_fini(struct osp_device *d)
{
	...
	LCONSOLE(D_INFO, "%s: %d updates in %d batches\n", d->opd_obd->obd_name,
		 d->opd_syn_updates_batched, d->opd_syn_batches);
