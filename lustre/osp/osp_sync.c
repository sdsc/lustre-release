 ...
 *
 * \retval		1 - should keep running, 0 - should stop
 */
 ...
 * \param[in] d		OSP device
 *
 * \retval		0 - still running, 1 - stopped
 */
 ...
 * \param[in] d		OSP device
 *
 * \retval		1 - there are changes, 0 - no changes
 */
 ...
 * XXX: shouldn't we actually have two different functions instead?
 *
 *
 * \param[in] env	LU environment provided by the caller
 * \param[in] d		OSP device
 ...
 * XXX: this is pretty expensive and can be imported later using batching.
 *
 ...
 *
 * \retval		new request on success, ERR_PTR() if failed
 */
 ...
 * specific bits and send RPC. The function is used to handle llog_unlink_rec
 * which were used in the older versions of Lustre. Current version uses
 * llog_unlink_rec64.
 *
 * \param[in] d		OSP device
 * \param[in] llh	llog handle where the record is stored
 ...
 *
 * The function prepares a new RPC, initializes it with unlink(destroy)
 * specific bits and send RPC. Depending on the target (MDT or OST) two
 * different protocols are used. For MDT we use OUT (basically OSD API
 ...
 * Depending on the type the handler is called which responsibility is to
 * prepare and send a RPC to apply the change.
 ...
 *
 * The function walks through the list of the committed RPCs and cancel
 ...
 * The mechanism is very similar to transno thing used by MDT service: it's
 * an single ID stream which can be assigned by any OSP to its llog records.
 * The tricky part is that ID is stored in per-transaction data and re-used
 * by all the OSPs involved in that transaction. Then all these OSPs are
 * waken up utilizing a single OSD commit callback.
 *
 ...
 *
 * Decrease a refcounter on the tracker used by OSP device passed. If no
 * more users left, then the tracker is released.
 *
 * \param[in] d		OSP device
 */
static void osp_sync_id_traction_fini(struct osp_device *d)
 ...
 *
 * Generates a new ID using the tracker associated with OSP device passed,
 * unless ID passed isn't 0. Unconditially adds OSP device to the wakeup
 * list, so OSP won't miss when a transaction using the ID is committed.
 * Notice ID is 32bit, but llog doesn't support >2^32 records anyway.
 *
 * \param[in] d		OSP device
 ...
 *
 * if OSP don't have any llog records she's waiting to commit, then it is
 * possible to unsibscribe from wakeups from the tracking using this method.
 *
