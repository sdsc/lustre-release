...
#include "../obdclass/llog_internal.h"
...
int llog_get_cat_list_2(const struct lu_env *env, struct dt_device *d,
                      int idx, int count, struct llog_catid *idarray);
int llog_put_cat_list_2(const struct lu_env *env, struct dt_device *d,
                      int idx, int count, struct llog_catid *idarray);
#if 0
...
static int osp_sync_thread(void *_arg)
{
        ...
        rc = llog_cat_process_or_fork(&env, llh, osp_sync_process_queues, d, 0, 0, 0);
...
static int osp_sync_llog_init(const struct lu_env *env, struct osp_device *d)
{
        ...
        rc = llog_get_cat_list_2(env, d->opd_storage, d->opd_index, 1,
                               &osi->osi_cid);
        if (rc) {
        ...
        rc = llog_put_cat_list_2(env, d->opd_storage, d->opd_index, 1,
                               &osi->osi_cid);
        if (rc) {
                CERROR("rc: %d\n", rc);
                GOTO(out_close, rc);
