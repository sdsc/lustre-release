...
static void osp_sync_request_commit_cb(struct ptlrpc_request *req)
{
	...
	LASSERT(cfs_list_empty(&jra->jra_link));

	ptlrpc_request_addref(req);

	spin_lock(&d->opd_syn_lock);
	cfs_list_add(&jra->jra_link, &d->opd_syn_committed_there);
	spin_unlock(&d->opd_syn_lock);
...
{
	struct osp_job_req_args *jra = ptlrpc_req_async_args(req);
					       ...
					       const struct req_format *format)
{
	...
	jra = ptlrpc_req_async_args(req);
	CFS_INIT_LIST_HEAD(&jra->jra_link);
				       ...
				       struct osp_device *d)
{
	...
	while (!cfs_list_empty(&list)) {
		jra = cfs_list_entry(list.next, struct osp_job_req_args,
				     jra_link);
		LASSERT(jra->jra_magic == OSP_JOB_MAGIC);
		cfs_list_del_init(&jra->jra_link);
