 ...
 * \retval		NULL if the creation failed.
 */
 ...
 *
 * \retval		object(dt_object) found or created if it
 * \retval		successfully get the object.
 * \retval		ERR_PTR(errno) if not get the object.
 */
 ...
 * \retval		-EFAULT if only part of buffer is written.
 * \retval		other negative errno if write failed.
 */
 ...
 * \retval		negative errno if initialization failed
 */
 ...
 *
 * \retval		0 if initialization succeed
 * \retval		negative error if initialization failed
 */
 ...
 * \retval		negative errno if disconnection failed
 */
 ...
 * \retval		negative errno if cleanup failed
 */
 ...
 *
 * \retval		0 if the config log record is executed correctly.
 * \retval		negative errno if the record execution fails.
 */
 ...
 * \retval		impossible to return other value right now.
 */
 ...
 * \retval		negative errno if get statfs failed.
 */
 ...
 * \retval		negative errno if synchronization fails
 */
 ...
 * \retval		negative errno connection failed
 */
...
/**
 * Initialize OSP device according the parameters in the config log(@cfg).
 *
 ...
 * \retval		negative errno if OSP initialization failed.
 */
 ...
 * \retval		interface. So return NULL for now.
 */
 ...
 * which is only used in lprocfs helpers by obd_statfs.
 *
 * \param[in] env	execution environment.
 ...
 *                      that accepts using a cached statfs. It is not used in
 ...
 * \retval		negative errno if statfs failed.
 */
 ...
 *
 * \retval		0 if the event is handled successsfully.
 * \retval		negative errno if the event is being handled
 *                      unsuccessfully.
 */
 ...
 * \retval		negative errno the ioctl is being handled
 *                      unsuccessfully.
 */
 ...
 * \retval		negative errno if get information unsuccessfully.
 */
 ...
 * \retval		1 allocate FID successfully and also request
 *                      a new sequence from seq meta server.
 * \retval		negative errno allocate FID failed.
 */
