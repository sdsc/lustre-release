				    ...
				    void *args, int rc)
{
	...
	case LNE_LAYOUT_QUERY: {
		...
		ler = req_capsule_server_get(&req->rq_pill, &RMF_GENERIC_DATA);
			...
			struct lov_stripe_md *lsm)
{
	...
	if (KEY_IS(KEY_LFSCK_EVENT)) {
		...
		LASSERT(lai->lai_ler->ler_event == LNE_LAYOUT_QUERY);
	...
	} else if (KEY_IS(KEY_LFSCK_EVENT_IN)) {
		struct osp_device		*osp	=
					lu2osp_dev(exp->exp_obd->obd_lu_dev);
		struct lfsck_event_request	*ler	= val;
		void				*key	= osp->opd_lfsck_key;

		if (key != NULL)
			ler->u.ler_status = lfsck_query(key, LT_LAYOUT);
		else
			ler->u.ler_status = 0;
	}
			      ...
			      void *val, struct ptlrpc_request_set *set)
{
	...
	if (KEY_IS(KEY_LFSCK_EVENT)) {
		struct lfsck_async_info    *lai = val;
		struct lfsck_event_request *ler = lai->lai_ler;
		struct osp_device	   *osp =
					lu2osp_dev(exp->exp_obd->obd_lu_dev);
		struct osp_async_info_args *oaia;
		struct ptlrpc_request	   *req;
		...
		if (ler->ler_event == LNE_LAYOUT_STOP)
			osp->opd_lfsck_key = NULL;
		...
		if (ler->ler_event == LNE_LAYOUT_START) {
			LASSERT(lai->lai_key != NULL);

			osp->opd_lfsck_key = lai->lai_key;
		}
		...
		if (key != NULL)
			rc = lfsck_in_notify(env, key, ler, NULL);
	}
