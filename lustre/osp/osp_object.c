				...
				enum dt_declare_intents intents)
{
	...
	/* XXX: Currently, we only support OST target. */
	if (!o->opo_for_ost)
		RETURN(0);
	...
	spin_lock(&o->opo_async_getattr_lock);
	/* If need to refresh the cached attr, clear the ::la_valid,
	 * then the subsequent attr_get will trigger new RPC to get
	 * new attr from OST; or if DDI_PREFETCH is specified also,
	 * then it will pre-fetch new attr as following. */
	if (intents & DDI_REFRESH)
		la->la_valid = 0;
	...
	if (intents & DDI_PREFETCH && la->la_valid == 0)
		need_rpc = true;

	if (intents & DDI_NEED_PFID && !(la->la_valid & LA_PFID))
		need_rpc = true;
	...
	osp_getattr_fill_oa(&body->oa, o, intents & DDI_NEED_PFID);
			...
			struct lu_attr *attr, struct lustre_capa *capa)
{
	...
	body = req_capsule_client_get(&req->rq_pill, &RMF_OST_BODY);
	osp_getattr_fill_oa(&body->oa, o, attr->la_valid & LA_PFID);
	ptlrpc_request_set_replen(req);
			 ...
			 struct lustre_capa *capa)
{
	...
	body = req_capsule_client_get(&req->rq_pill, &RMF_OST_BODY);
	osp_getattr_fill_oa(&body->oa, o, true);
	ptlrpc_request_set_replen(req);
	rc = ptlrpc_queue_wait(req);
	if (rc != 0)
		GOTO(out, rc);
