 ...
 *
 * If \a unlink is true, then remove the entry from the cache.
 ...
 * This transaction handle is usually used for LFSCK to repair inconsistent
 * OST object owner. Otherwise, the OSP will handle the normal chown/chgrp
 * cases via llog. This function will declare the credits for generating
 * MDS_SETATTR64_REC llog.
 *
 * \param[in] env	pointer to the thread context
 * \param[in] dt	pointer to the OSP layer dt_object
