...
/**
 * NOT prepare set_{attr,xattr} RPC for remote transaction.
 *
 * According to our current transaction/dt_object_lock framework, the
 * transaction sponsor will start the transaction firstly, then try to
 * acquire related dt_object_lock if needed. Under such general rule,
 * if we want to prepare the set_{attr,xattr} RPC in the RPC declare
 * phase, then related attr/xattr should be known without dt_object_lock.
 * But such condition maybe not true for some remote transaction case.
 * For example:
 *
