 ...
 *
 * If \a unlink is specified, then remove the entry from the cache.
 ...
 * \param[in] unlink	true for if remove the extended attribute entry
 *			from the cache
 *
 ...
 * Find the named extended attribute in the OSP object attributes cache.
 *
 * It calls the osp_oac_xattr_find_locked() with the osp_object::opo_lock held.
 *
 * \param[in] obj	pointer to the OSP object
 * \param[in] name	the name of the extended attribute
 * \param[in] unlink	true for if remove the extended attribute entry
		      ...
		      struct osp_xattr_entry **poxe, int len)
{
	...
	spin_lock(&obj->opo_lock);
	old = osp_oac_xattr_find_locked(ooa, oxe->oxe_buf, namelen, true);
	list_add_tail(&oxe->oxe_list, &ooa->ooa_xattr_list);
	spin_unlock(&obj->opo_lock);
 ...
 *
 * When gets the reply for the async mode RPC for getting OSP object
 * attributes, this function will be called to handle the result.
 *
 * \param[in] env	pointer to the thread context
 * \param[in] reply	pointer to the RPC reply
 * \param[in] req	pointer to the RPC request
 ...
 * based shared asynchronous request queue. The osp_attr_get_interpterer() will
 * be registered as the interpreter function that will called when gets reply
 ...
 * to the peer to get the attribute synchronously and, if successful,
 * add it to the OSP attributes cache.
 * (\see lustre/osp/osp_trans.c for OUT RPC.)
 *
 * \param[in] env	pointer to the thread context
 * \param[in] dt	pointer to the OSP layer dt_object
 ...
 *
 * Declare that the caller will set attribute to the specified OST object.
 *
 * If the transaction is a remote transaction (\see osp_trans_create()
 * for remote transaction), then it will add an OUT_ATTR_SET sub-request to
 * the per OSP-transaction based request queue via osp_md_declare_attr_set().
 * This handle is usually used for LFSCK to repair inconsistent OST object
 * owner. Otherwise, the OSP will handle the normal chown/chgrp cases via llog.
 * This function will declare the credits for generating MDS_SETATTR64_REC llog.
 *
 * \param[in] env	pointer to the thread context
 * \param[in] dt	pointer to the OSP layer dt_object
 ...
 *
 * When gets the reply for the async mode RPC for getting OSP object extended
 * attribute, this function will be called to handle the result.
 *
 * \param[in] env	pointer to the thread context
 * \param[in] reply	pointer to the RPC reply
 * \param[in] req	pointer to the RPC request
 ...
 * the extended attribute synchronously and, if successful, add it to the
 * OSP attributes cache.
 * (\see lustre/osp/osp_trans.c for OUT RPC.)
 *
 ...
 *
 * The OST object destrory will be handled via llog asynchronously. This
 ...
 *
 * The new records will be filled in a serial of pages. The OSP side
