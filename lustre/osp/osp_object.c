...
osp_oac_xattr_find_or_add(struct osp_object *obj, const char *name, size_t len)
{
	...
	if (len)
		size = sizeof(*oxe) + namelen + 1 + len;
	else
		size = sizeof(*oxe) + namelen + 1 + OXE_DEFAULT_LEN;
				 ...
				 struct lu_buf *buf, const char *name)
{
	...
	/* Currently, only supports pre-fetch EA for OST-object.
	 * Pre-fetching MDT-object's EA without lock is not safe. */
...
rpc:
	...
	LASSERT(rbuf->lb_len > 0);
	rc = rbuf->lb_len;

	/* Currently, only caching OST-object EA is safe. */
	if (!is_ost_obj(&dt->do_lu))
		GOTO(out, rc);
