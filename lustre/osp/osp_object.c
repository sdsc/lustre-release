 ...
 * fetch those attributes when every get_attr()/get_xattr() called. And
 * for LFSCK to scan the whole system, such sychronous mode scanning is
 * almost unacceptable.
 *
 ...
 * but also every stripe on OST of the file has each own FID. This function
 * will assign the FID to the stripe The FID to be used is reserved when
 * create declare.
 *
