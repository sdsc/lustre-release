 ...
 * on the peer side remotely. It is inefficient to send RPC to peer to
 * fetch those attributes when every get_attr()/get_xattr() called. For
 * a large system, the LFSCK synchronous mode scanning is prohibitively
 * inefficient.
 *
 ...
 * attributes cache will be initialized when pre-fetches the
 * attribute or extended attribute via osp_declare_attr_get()
 ...
 *
 * If @unlink is specified, then remove the entry from the cache.
 ...
 * Find the named extended attribute in the OSP object attributes cache.
 *
 * It is the warp function of osp_oac_xattr_find_locked() with the
 * osp_object::opo_lock lock held firstly.
 *
 * \param[in] obj	pointer to the OSP object
 * \param[in] name	the name of the extended attribute
 * \param[in] unlink	whether remove the found extended attribute entry
...
/**
 * Find the name extended attribute in the OSP object attributes cache.
 *
 ...
 *
 * There is an old extended attributed entry with the same name,
 * remove it from the cache and return it via the parameter @poxe.
 *
 * \param[in] obj	pointer to the OSP object
 ...
 *
 * To declare that the caller will get attribute from the specified OST object.
 *
 ...
 *
 * To declare that the caller will set attribute to the specified OST object.
 *
 ...
 * a MDS_SETATTR64_REC record in the llog. There is dedicated thread to
 * handle the llog asychronously.
 ...
 *
 * To declare that the caller will get extended attribute from the specified
 * OST object.
 *
 ...
 * attributes cache.
 *
 * There is a race condition: some other has added the named extended
 ...
 *
 * To declare that the caller will set extended attribute to the specified
 * MDT/OST object.
 *
 ...
 *
 * To declare that the caller will delete extended attribute on the specified
 * MDT/OST object.
 *
 ...
 *
 * To declare that the caller will create the OST object.
 *
 * If the transaction is a remote transaction (please refer to the
 ...
 *
 * To declare that the caller will destroy the specified OST object.
 *
