				      ...
				      void *arg, int rc)
{
	...
	list_for_each_entry_safe(oar, next, &dt_update->dur_cb_items,
		...
		/* The peer may only handled some requests (indicated by the
		 * 'count') in the packaged OUT RPC, we can only get results
 ...
 *    the up layer caller (such as the LFSCK engine) will call the
