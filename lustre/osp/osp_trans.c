			   ...
			   osp_dummy_update_interpterer_t interpterer)
{
	...
	CFS_INIT_LIST_HEAD(&odui->odui_list);
	odui->odui_obj = obj;
...
static void osp_dummy_update_item_fini(const struct lu_env *env,
				       struct osp_dummy_update_item * odui)
{
	LASSERT(cfs_list_empty(&odui->odui_list));
				      ...
				      void *args, int rc)
{
	...
	cfs_list_for_each_entry_safe(odui, next, &update->ur_callback_list,
				     odui_list) {
		cfs_list_del_init(&odui->odui_list);
				 ...
				 struct update_request *update)
{
	...
	if (rc != 0) {
		...
		cfs_list_for_each_entry_safe(odui, next,
...
again:
	rc = out_insert_update(env, update, op,
			       (struct lu_fid *)lu_object_fid(osp2lu_obj(obj)),
			       count, lens, bufs);
	if (rc == -E2BIG) {
	...
	if (rc == 0)
		cfs_list_add_tail(&odui->odui_list, &update->ur_callback_list);
