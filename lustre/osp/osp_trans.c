				      ...
				      void *arg, int rc)
{
	...
	list_for_each_entry_safe(odui, next, &update->ur_cb_items, odui_list) {
				...
				void *arg, int rc)
{
	struct osp_async_update_args	*oaua	= arg;
	struct update_request		*update = oaua->oaua_update;
	struct dt_txn_commit_cb		*cb	= update->ur_dtcc;

	LASSERT(cb != NULL);
	LASSERT(cb->dcb_func != NULL);

	cb->dcb_func(env, NULL, cb, rc);
	out_destroy_update_req(update);
...
/* with osp::opd_dummy_th_mutex held */
...
again:
	...
	if (rc == 0)
		list_add_tail(&odui->odui_list, &update->ur_cb_items);
		    ...
		    struct thandle *th)
{
	...
	if (update->ur_buf->ub_count > 0) {
		if (update->ur_dtcc != NULL) {
			...
			/* In phase I, if the transaction includes remote
			 * updates, the local update should be synchronized,
			 * so it will set th_sync = 1 */
			th->th_sync = 1;
