				...
				struct ptlrpc_request *req, void *arg, int rc)
{
	...
	list_for_each_entry_safe(ouc, next, &our->our_cb_items, ouc_list) {
		...
		if (rc1 < 0)
			CERROR("rc1 %d index %d count %d lens %d\n",
			       rc1, index, count, reply->ourp_lens[index]);
...
osp_get_next_request(struct osp_updates *ou, struct osp_update_request **ourp)
{
	...
	list_for_each_entry_safe(our, tmp, &ou->ou_list, our_list) {
		...
		CDEBUG(D_HA, "our %p version "LPU64" rpc_version "LPU64"\n",
				  ...
				  struct osp_device *osp)
{
	...
	osp->opd_update->ou_refresh_version = osp->opd_update->ou_version;
	spin_unlock(&osp->opd_update->ou_lock);

	up_write(&ctxt->loc_handle->lgh_lock);
	CDEBUG(D_HA, "%srestart version from "LPU64"\n",
	       osp->opd_obd->obd_name, osp->opd_update->ou_refresh_version); 
	return rc;
}
