		    ...
		    struct ptlrpc_request **reqp)
{
	...
	osp_set_req_replay(env, osp2top(osp), req);
	req->rq_allow_intr = 1;
			       ...
			       struct osp_update_request *our)
{
	...
	if (!oth->ot_super.th_wait_submit && !oth->ot_super.th_sync) {
		...
		req->rq_allow_replay = our->our_allow_replay;
				  ...
				  struct osp_object *obj)
{
	...
	if (is_for_recovery(env) || osp2top(osp)->ld_obd->obd_recovering)
		oth->ot_our->our_allow_replay = 1;
	spin_unlock(&oth->ot_our->our_list_lock);
	spin_unlock(&ou->ou_lock);
