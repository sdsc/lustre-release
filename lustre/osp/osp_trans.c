 ...
 * get_(x)attr, and so on. To reduce the number of RPCs, we allow multiple
 * operations to be packaged together in single OUT RPC.
 *
 * For the asynchronous idempotent operations, the RPCs will be inserted into
 * a shared asynchronous request queue - osp_device::opd_async_requests. Once
 * the queue is full, we will send the packaged OUT RPC and purge the queue.
 *
 * When the request is inserted into the shared queue, it will register an
 * interpreter. When the packaged OUT RPC is replied (or failed), it will
				      ...
				      void *arg, int rc)
{
	...
	list_for_each_entry_safe(oar, next, &dt_update->dur_cb_items,
		...
		/* The peer may has handled part of sub-requests (indicated
		 * via the 'count') in the OUT RPC, we can only get results
 ...
 * into a single OUT RPC, and give it to the background ptlrpcd daemon.
 *
 * \param[in] env	pointer to the thread context
 ...
 * queue that is attached to the osp_device.
 *
 * The function generates a new osp_async_request with the given parameters,
 ...
 * the shared queue.
 *
 * NOTE: hold the osp::opd_async_requests_mutex.
 *
 * \param[in] env		pointer to the thread context
 * \param[in] op		operation type
 * \param[in] obj		pointer to the operation target
 * \param[in] count		array size of the subsequent 'lens'/'bufs'
 ...
 *    LFSCK purpose, then it is a remote transaction. For a remote
 *    transaction, the up layer caller will call dt_trans_create()
 *    on the OSP dt_device to use osp_trans_create() that creates
 *    the transaction handler and returns it to the caller.
 *
 * 2) If the transcation contains both local and remote updates,
 *    such as cross MDTs create under DNE mode, then the up layer
 ...
 *    used by the whole transaction in the subsequent sub-operations.
 *
 * \param[in] env	pointer to the thread context
 ...
 * asynchronously; otherwise, the cross MDTs transaction will be synchronized.
 *
 ...
 *
 * If the transaction is a remote one, then related remote updates
 * will be triggered in osp_trans_stop(); otherwise the transaction
 ...
 *
 * If the transaction is a remote one, or the update handler is
 * marked as 'tu_sent_after_local_trans', then related remote
