...
static int osp_precreate_send(const struct lu_env *env, struct osp_device *d)
{
	...
	/* Sometimes OST might not be able to precreate the object
	 * OSP ask, but local fid client(cl_seq) already created
	 * those FIDs(osp_precreate_fids), so it need rollback the
	 * current fid in client seq to keep it same as last_created_fid,
	 * otherwise OSP might get un-continguous fid in next precreation.
	 * Since there is only one precreate thread per OSP, we do not need
	 * worry race here. */
					 ...
					 struct osp_device *d)
{
	...
	*last_fid = d->opd_last_used_fid;
	/* Freshed FS, no need cleanup orphans */
	 ...
	 * the last create fid on OST. See the comments in
...
static int osp_precreate_thread(void *_arg)
{
	...
	while (osp_precreate_running(d)) {
		...
		while (osp_precreate_running(d)) {
			...
			/* got connected */
			if (d->opd_obd->u.cli.cl_seq == NULL)
				continue;
