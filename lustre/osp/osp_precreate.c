 ...
 * The function fills data from the reply if succesfull and scheduler another
 * update.
 *
 ...
 *
 * If cached statfs data claim no free space, but OSP has got a request to destroy
 * an object (so release some space probably), then we may need to refresh cached
 * statfs data sooner then planned. The function checks there is no statfs update
 * going and schedules immediate update if so.
 ...
 * or the caller can't used it for a new objects anymore. Find the details
 * in osp_fid_end_seq(). This is a protected version of that function.
 *
 * \param[in] env	LU environment provided by the caller
 * \param[in] osp	OSP device
 *
 ...
 *
 * If the target/connection is healhy, the function finds how many objects
 * should be precreated. Then allocates, prepares and schedules precreate
 * RPC synchronously. Upon reply the function wake ups the threads waiting
 * for the new objects on this target. If the target wasn't able to create
 * all the objects requested, then the next precreate will be asking less
 * objects (i.e. slow precreate down).
 *
 * \param[in] env	LU environment provided by the caller
 * \param[in] osp	OSP device
 *
 ...
 *
 * Then function is called in a contex of a dedicated thread handling
 ...
 *
 * When a striping is being create late, it's possible that size is
 * already initialized on the file. Then the new striping should
 * inherit size from the file. The function sets size on the object
 * using the regular protocol (OST_PUNCH).
