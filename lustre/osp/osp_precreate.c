...
static int osp_precreate_send(struct osp_device *d)
{
	...
	if (unlikely(d->opd_pre_status)) {
		CERROR("%s: don't send new precreate: rc = %d\n",
...
static int osp_precreate_connection_from_mds(struct osp_device *d)
{
	...
	if (req == NULL) {
		CERROR("%s: can't allocate request\n", d->opd_obd->obd_name);
		RETURN(-ENOMEM);
	}
	...
	if (rc) {
		ptlrpc_request_free(req);
		CERROR("%s: can't pack request: rc = %d\n",
		       d->opd_obd->obd_name, rc);
		RETURN(rc);
	}
...
void osp_pre_update_status(struct osp_device *d, int rc)
{
	...
	if (likely(msfs->os_type)) {
		...
		if ((msfs->os_ffree < 32) || (msfs->os_bavail < used)) {
			...
			if (old != -ENOSPC)
				CERROR("%s: status %lu blocks, %lu free, %lu "
				       "used, %lu avail -> %d: rc = %d\n",
				       d->opd_obd->obd_name,
			...
			d->opd_pre_grow_count = OST_MIN_PRECREATE;
			cfs_waitq_signal(&d->opd_pre_waitq);
			CERROR("%s: no space: %lu blocks, %lu free, %lu used, "
			       "%lu avail -> %d: rc = %d\n",
			       d->opd_obd->obd_name,
			       (unsigned long)msfs->os_blocks,
			       (unsigned long)msfs->os_bfree,
