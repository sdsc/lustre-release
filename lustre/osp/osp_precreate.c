...
static int osp_precreate_thread(void *_arg)
{
	...
	CERROR("clear opd_precreate_initialized before enter running loop\n");
	while (osp_precreate_running(d)) {
		...
		d->opd_precreate_initialized = 0;
		CERROR("clear opd_precreate_initialized\n");
	}
...
int osp_precreate_reserve(const struct lu_env *env, struct osp_device *d)
{
	...
	while ((rc = d->opd_pre_status) == 0 || rc == -ENOSPC ||
		...
		if (precreated > d->opd_pre_reserved &&
			...
			CERROR("set opd_precreate_initialized normally\n");
			break;
		}
		spin_unlock(&d->opd_pre_lock);
		...
		if (unlikely(rc == -ENOSPC)) {
			...
			if (d->opd_syn_changes +
				...
				d->opd_precreate_initialized = 1;
				CERROR("set opd_precreate_initialized for "
				       "-ENOSPC error\n");
				break;
			}
		}
