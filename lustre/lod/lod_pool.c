...
{
	return cfs_hash_djb2_hash(key, strnlen(key, LOV_MAX_POOLNAME), mask);
}
 ...
 *
 * The caller may not have a ref on pool if it got the pool without
 * calling lod_find_pool() (e.g. go through the lod pool list)
 *
