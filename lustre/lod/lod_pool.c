 ...
 * that RAID-1 mirroring could select targets from independent fault domains,
 * or pools could define target performance characteristics so that
 * applicatins could select IOP-optimized storage or stream-optimized
 * storage for a particular output file.
 *
 * This file handles creation, lookup, and removal of pools, as well as
 * adding and removing pools.  Pools are accessed by name in the pool
...
/**
 * Get a reference on the specified \a pool.
 *
...
/**
 * Drop a reference on the specified \a pool and free its memory if this
 * was the last reference count on this structure.
 *
 ...
 * it is explicitly destroyed by the sysadmin.
 *
 ...
 *
 * The currently referenced pool target in is given by op_array[lpi_idx].
 *
 * \param[in] seq	new sequence file structure to initialize
 * \param[in] v		(unused)
 */
 ...
 * \retval	-ve error number on failure
 */
 ...
 * array size.  If pool count not is specied (\a count == 0), then use
 * POOL_INIT_COUNT will be used.  Allocating an non-zero iniial array
 ...
 *
 * Remove one target named \a ostname from \a poolname.  The \a ostname is
 * searched for in the lod_device lod_ost_bitmap array, to get ensure the
 ...
 *
 * The caller may no have a ref on pool if it got the pool without calling
 * lod_find_pool() (e.g. go through the lod pool list)
 *
