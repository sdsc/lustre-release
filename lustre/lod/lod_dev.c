				...
				struct top_thandle *top_th)
{
	...
	master_st = list_entry(&top_th->tt_sub_trans_list,
 ...
 */
static void lod_cancel_master_log_commit(struct thandle *th,
					 void *cookie)
{
	struct top_thandle *top_th = container_of(th, struct top_thandle,
						  tt_super);
	struct top_thandle *master_th;
	struct lod_cancel_log_thread *slave_lclt = cookie;
	ENTRY;

	if (!top_th->tt_cancel_master)
		RETURN_EXIT;

	LASSERT(top_th->tt_cancel_data != NULL);
	master_th = (struct top_thandle *)top_th->tt_cancel_data;

	/* Move this transaction to the slave cancel list */
	lod_add_thandle_to_cancel_list(slave_lclt, master_th);
				 ...
				 struct top_thandle *committed_th)
{
	...
	master_st = list_entry(&committed_th->tt_sub_trans_list,
			       struct sub_thandle, st_list);
	...
	th = dt_trans_create(env, committed_th->tt_super.th_dev);
	if (IS_ERR(th))
		RETURN(PTR_ERR(th));
	...
	rc = dt_trans_start_local(env, committed_th->tt_super.th_dev, th);
	if (rc < 0)
		GOTO(out_trans, rc);

	top_th = container_of(th, struct top_thandle, tt_super);
	top_th->tt_cancel_data = (void *)committed_th;
	top_th->tt_cancel_master = 1;

	rc = llog_cat_cancel_records(env, master_ctxt->loc_handle, 1,
				     &master_st->st_update->stu_cookie, th);
out_trans:
	dt_trans_stop(env, committed_th->tt_super.th_dev, th);
				...
				struct lod_device *lod)
{
	...
	/* Add commit callback for master log cancellation */
	lod->lod_cancel_master_commit_cb.dtc_txn_commit =
				lod_cancel_master_log_commit;
	lod->lod_cancel_master_commit_cb.dtc_cookie =
				&lod->lod_cancel_slave_thread;
	lod->lod_cancel_master_commit_cb.dtc_flags = DT_TXN_CALLBACK_TOP;
	lod->lod_cancel_master_commit_cb.dtc_tag = LCT_MD_THREAD;
	INIT_LIST_HEAD(&lod->lod_cancel_master_commit_cb.dtc_linkage);
	dt_txn_callback_add(lod->lod_child, &lod->lod_cancel_master_commit_cb);
...
static struct thandle *lod_trans_create(const struct lu_env *env,
					struct dt_device *dt)
{
	return top_trans_create(env, dt2lod_dev(dt)->lod_child);
}
...
/**
 * Register callback for lod transaction.
 *
 * register commit callback for top thandle. It will add commit callback
 * for all of its sub thandle, so these callbacks will be called when these
 * sub thandle are committed.
 *
 * \param[in]th		thandle of top thandle
 */
static void lod_trans_register_callback(struct thandle *th)
{
	struct top_thandle	*top_th = container_of(th, struct top_thandle,
						       tt_super);
	struct sub_thandle	*st;
	...
	list_for_each_entry(st, &top_th->tt_sub_trans_list, st_list) {
		...
		stu->stu_txn_cb.dtc_txn_commit = lod_sub_update_txn_commit;
		stu->stu_txn_cb.dtc_cookie = st;
		stu->stu_txn_cb.dtc_flags = DT_TXN_CALLBACK_TOP;
		stu->stu_txn_cb.dtc_tag = LCT_MD_THREAD;
		INIT_LIST_HEAD(&stu->stu_txn_cb.dtc_linkage);

		txn_callback_add(st->st_sub_th, &stu->stu_txn_cb);
	}
}
			  ...
			  struct thandle *th)
{
	...
	if (top_th->tt_multiple_node)
		lod_trans_register_callback(th);
