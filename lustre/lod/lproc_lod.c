...
static int lod_failout_seq_show(struct seq_file *m, void *v)
{
	struct obd_device *dev = m->private;
	struct lod_device *lod;
...
static ssize_t
lod_failout_seq_write(struct file *file, const char *buffer,
		      size_t count, loff_t *off)
{
	struct seq_file		*m	= file->private_data;
	struct obd_device	*dev	= m->private;
