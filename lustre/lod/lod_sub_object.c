				  ...
				  struct thandle *th)
{
	...
	sub_th = lod_get_sub_trans(env, th, dt);
	if (IS_ERR(sub_th))
 ...
 */
static void lu_attr_cpu_to_le(struct lu_attr *dst_attr, struct lu_attr *src_attr)
{
			  ...
			  struct thandle *th)
{
	...
	lod_updates_pack(env, create, rc, th, lu_object_fid(&dt->do_lu),
			 lti_attr, hint, dof);
	if (rc < 0)
		RETURN(rc);
			   ...
			   struct thandle *th)
{
	...
	lod_updates_pack(env, ref_add, rc, th,
			   ...
			   struct thandle *th)
{
	...
	lod_updates_pack(env, ref_del, rc, th,
			 lu_object_fid(&dt->do_lu));
	if (rc < 0)
		RETURN(rc);
			   ...
			   struct thandle *th)
{
	...
	lod_updates_pack(env, object_destroy, rc, th,
			 lu_object_fid(&dt->do_lu));
	if (rc < 0)
		RETURN(rc);
				...
				struct lustre_capa *capa, int ign)
{
	...
	lod_updates_pack(env, index_insert, rc, th, lu_object_fid(&dt->do_lu),
			 rec, key);
	if (rc < 0)
		return rc;
			  ...
			  struct lustre_capa *capa)
{
	...
	lod_updates_pack(env, index_delete, rc, th, lu_object_fid(&dt->do_lu),
			 name);
	if (rc < 0)
		RETURN(rc);
			     ...
			     struct thandle *th, struct lustre_capa *capa)
{
	...
	lod_updates_pack(env, xattr_set, rc, th, lu_object_fid(&dt->do_lu),
			 buf, name, fl);
	if (rc < 0)
		RETURN(rc);
			    ...
			    struct lustre_capa *capa)
{
	...
	lod_updates_pack(env, attr_set, rc, th, lu_object_fid(&dt->do_lu),
			 lti_attr);
	if (rc < 0)
		RETURN(rc);
			     ...
			     struct lustre_capa *capa)
{
	...
	lod_updates_pack(env, xattr_del, rc, th, lu_object_fid(&dt->do_lu),
			 name);
	if (rc < 0)
		RETURN(rc);
			 ...
			 int rq)
{
	...
	lod_updates_pack(env, write, rc, th, lu_object_fid(&dt->do_lu),
			 buf, *pos);
	if (rc < 0)
		RETURN(rc);
