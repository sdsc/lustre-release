...
int lod_get_lov_ea(const struct lu_env *env, struct lod_object *mo)
{
	...
	struct dt_object       *next = dt_object_child(&mo->ldo_obj);
	int			rc;
	ENTRY;
...
{
	struct lod_thread_info	*info = lod_env_info(env);
	struct lod_device	*md = lu2lod_dev(mo->ldo_obj.do_lu.lo_dev);
	...
	LASSERT(mo);
	LASSERT(mo->ldo_stripe == NULL);
	LASSERT(mo->ldo_stripenr > 0);
	LASSERT(mo->ldo_stripe_size > 0);

	i = sizeof(struct dt_object *) * mo->ldo_stripenr;
	OBD_ALLOC(mo->ldo_stripe, i);
	if (mo->ldo_stripe == NULL)
		GOTO(out, rc = -ENOMEM);
	mo->ldo_stripes_allocated = mo->ldo_stripenr;

	for (i = 0; i < mo->ldo_stripenr; i++) {
		...
		mo->ldo_stripe[i] = container_of(n, struct dt_object, do_lu);
	}
		       ...
		       const struct lu_buf *buf)
{
	...
	mo->ldo_stripe_size = le32_to_cpu(lmm->lmm_stripe_size);
	mo->ldo_stripenr = le16_to_cpu(lmm->lmm_stripe_count);
	mo->ldo_layout_gen = le16_to_cpu(lmm->lmm_layout_gen);
...
int lod_load_striping(const struct lu_env *env, struct lod_object *mo)
{
	...
	if (mo->ldo_stripe) {
		...
		/* check validity */
		for (i = 0; i < mo->ldo_stripenr; i++)
			LASSERTF(mo->ldo_stripe[i], "stripe %d is NULL\n", i);
		GOTO(out, rc = 0);
	}
