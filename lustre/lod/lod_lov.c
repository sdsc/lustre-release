...
static int lod_get_ea(const struct lu_env *env, struct lod_object *lo,
		      const char *name)
{
	struct lod_thread_info	*info = lod_env_info(env);
	struct dt_object	*next = dt_object_child(&lo->ldo_obj);
...
inline int lod_get_lov_ea(const struct lu_env *env, struct lod_object *lo)
{
	return lod_get_ea(env, lo, XATTR_NAME_LOV);
}

inline int lod_get_lmv_ea(const struct lu_env *env, struct lod_object *lo)
{
	return lod_get_ea(env, lo, XATTR_NAME_LMV);
}

inline int lod_get_default_lmv_ea(const struct lu_env *env, struct lod_object *lo)
{
