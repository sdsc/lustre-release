		   ...
		   char *type, int active)
{
	...
	rc = lfsck_add_target(env, lod->lod_child, d, exp, index, is_osc);
	if (rc != 0)
...
static void __lod_del_device(const struct lu_env *env, struct lod_device *lod,
			     struct lod_tgt_descs *ltd, unsigned idx, bool is_osc)
{
		 ...
		 struct lod_tgt_descs *ltd, bool is_osc)
{
	...
	mutex_lock(&ltd->ltd_mutex);
	cfs_foreach_bit(ltd->ltd_tgt_bitmap, idx)
		__lod_del_device(env, lod, ltd, idx, is_osc);
	mutex_unlock(&ltd->ltd_mutex);
	lod_putref(lod, ltd);
		   ...
		   unsigned gen)
{
	...
	__lod_del_device(env, lod, ltd, idx, true);
	EXIT;
out:
	mutex_unlock(&ltd->ltd_mutex);
	lod_putref(lod, ltd);
