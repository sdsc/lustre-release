			   ...
			   struct thandle *th)
{
	...
	if (lo->ldo_striping_cached == 0 || lo->ldo_def_striping == NULL)
		RETURN(0);

	rc = lod_md_swap_le(info, &lo->ldo_def_striping->lsd_md,
			    lo->ldo_def_striping->lsd_mdsize);
	if (rc != 0)
		RETURN(rc);

	info->lti_buf.lb_buf = info->lti_ea_store;
	info->lti_buf.lb_len = lo->ldo_def_striping->lsd_mdsize;
...
/**
 * Swap user md \alum into in-disk byte order(little endian) and store it
 * into \ainfo->lti_ea_store.
 *
 * \param[in] lum		user md in CPU byte sequence
 * \param[in] lum_size		the size of \alum
 * \param[in] buf		buffer to store disk byte order
 */
int lod_md_swap_le(struct lod_thread_info *info,
		   struct lov_user_md *lum, unsigned int lum_size)
{
	ENTRY;

	if (info->lti_ea_store_size < lum_size) {
		int rc;

		rc = lod_ea_store_resize(info, lum_size);
		if (rc != 0)
			RETURN(rc);
	}

	memcpy(info->lti_ea_store, lum, lum_size);
	if (lum->lmm_magic == cpu_to_le32(lum->lmm_magic))
		RETURN(0);

	/* Host is not using little endian, need to swap */
