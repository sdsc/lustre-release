	       ...
	       const char *name)
{
	...
	if (unlikely(info->lti_ea_store == 0)) {
...
int lod_get_lmv_ea(const struct lu_env *env, struct lod_object *lo)
{
	return lod_get_ea(env, lo, XATTR_NAME_LMV);
}

int lod_get_default_lmv_ea(const struct lu_env *env, struct lod_object *lo)
{
