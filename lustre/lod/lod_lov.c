 ...
 *
 * Connects to the OSP device passed, initializes all the internal
 * structures related to the device and adds it to the target table.
 *
 ...
 *
 * Given few processes can use the device at the moment it's requested for
 * removal and this can take quite a while (due to non-local nature), we
 * just mark the device as dead. When the last reference is released, the
 * device will be finally removed.
 *
 * \param[in] env		environment passed by the caller
 ...
 *
 * Remove a device identified by the name passed from the target table.
 * Given the device can be used, the real deletion happens in lod_putref().
 *
 * \param[in] env		environment passed by the caller
 * \param[in] lod		LOD device to be connected to the new OSP
 ...
 * demand. The memory is released when the correspondent thread is finished.
 *
 ...
 *
 * Generate and store striping information in the form of LOV EA to the
 * given object. An external exclusive locking required to be done by
 * the caller. The transaction must be started. FLDB service should be
 * running as well, it's used to map FID to the target, which is stored
 * in LOV EA.
 *
 * \param[in] env		environment passed by the caller
 ...
 *
 * \retval			0 if the index is presetn
 ...
 * already. An external exclusive locking should be ensured by
 * the caller. FLDB service must be running - to be able to map
 * a FID to the targets and find appropriate device representing
 * that target.
 *
 * \param[in] env		environment passed by the caller
 ...
 *
 * Parses striping information in \buf and instantiates
 * the objects representing the stripes.
 *
 * \param[in] env		environment passed by the caller
 * \param[in] lo		LOD object
 ...
 *
 * A protected version of lod_load_striping_locked() - loads the striping
 * information from storage, parses that and instantiate LU objects to
 ...
 * Verify striping.
 *
 * Checks the validity of all fields including the magic, stripe size,
 * stripe count, stripe offset and that the pool is present. Also checks
 * all the target indices points to the existing targets.
 *
