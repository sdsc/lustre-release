...
{
	cfs_down_read(&ltd->ltd_rw_sem);
	cfs_mutex_lock(&ltd->ltd_mutex);
	ltd->ltd_refcount++;
	cfs_mutex_unlock(&ltd->ltd_mutex);
}
	...
	if (ltd->ltd_refcount == 0 && ltd->ltd_death_row) {
		...
		cfs_list_for_each_entry_safe(tgt_desc, tmp, &kill, ltd_kill) {
			...
			if (ltd == &lod->lod_ost_descs) {
				...
				if (rc)
					CERROR("%s: qos_del_tgt() failed with "
					       "%d for %s\n",
					       lod2obd(lod)->obd_name, rc,
					     obd_uuid2str(&tgt_desc->ltd_uuid));
				rc = obd_fid_fini(tgt_desc->ltd_exp);
				if (rc)
 ...
 * \param mdt_index - is the group of the OSP.
		   ...
		   char *type, int active)
{
	...
	CDEBUG(D_CONFIG, "osp:%s idx:%d gen:%d mdt_idx %d\n", osp, index, gen,
	       mdt_index);
	...
	if (strcmp(LUSTRE_OSC_NAME, type) == 0) {
		...
		data->ocd_group = mdt_index;
		ltd = &lod->lod_ost_descs;
	} else {
		struct obd_import *imp = obd->u.cli.cl_import;
		/*
		 * XXX These flags needs to be revisited in later phase.
		 */
	...
	if (!strcmp(LUSTRE_OSC_NAME, type)) {
		...
		rc = obd_fid_init(exp, LUSTRE_SEQ_DATA);
		if (rc) {
			CERROR("%s: fid init error %d\n", obd->obd_name, rc);
			GOTO(out_pool, rc);
		}
		...
		if (index == 0) {
			struct md_site *ms =
				lu_site2md(lod2lu_dev(lod)->ld_site);
			ms->ms_control_exp = class_export_get(exp);
			ms->ms_server_fld->lsf_control_exp = exp;
		}
	}
		   ...
		   unsigned gen)
{
	...
	if (!obd_uuid_equals(&uuid, &LTD_TGT(ltd, idx)->ltd_uuid)) {
		CERROR("%s: LOD target UUID %s at index %d does not match %s"
			"\n", obd->obd_name,
			obd_uuid2str(&LTD_TGT(ltd, idx)->ltd_uuid), idx, osp);
		GOTO(out, rc = -EINVAL);
	}
