 ...
 * all the stripes. The stripes pointing back with ".." and proper LinkEA. The
 * master object gets LMV EA which identifies it as a striped directory. The
 * function allocates FIDs for all the stripes.
 *
 * \param[in] env	execution environment
 * \param[in] dt	object
 ...
 * warpper for lod_prep_md_striped_create(). The only additional functionality
 ...
 * striped one, then stripe objects are created properly and appropriate EA is
 * set (LOV EA storing all the stripes directly or LMV EA storing just a small
 * header with striping configuration).
 *
 * See the details of the method in OSD API description.
 */
 ...
 * (if it exists) in the parent directory object. We do always cache default
 * striping for the regular files (stored in LOV EA) and we do cache default
 * striping for the directories if requested by \child_mode (when a new
