			...
			struct lu_buf *buf, bool resize)
{
	...
	while (rc == 0) {
		...
		if (ent->lde_name[0] == '.') {
			...
			CDEBUG(lod->lod_failout ? D_ERROR : D_INFO,
		...
		if (len == ent->lde_namelen) {
			...
			if (!fid_is_zero(&lmv1->lmv_stripe_fids[index])) {
				...
				CDEBUG(lod->lod_failout ? D_ERROR : D_INFO,
				       "%s: both the shard "DFID" and "DFID
				       " for the striped directory "DFID
				       " claim the same LMV EA index %d, %s\n",
