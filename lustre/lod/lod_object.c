...
{
	memcpy(slave_lmv, master_lmv, sizeof(*slave_lmv));
	slave_lmv->lmv_magic = cpu_to_le32(LMV_DIRECTORY_STRIPE_SHARD);
	return;
}
			   ...
			   const struct lu_buf *buf)
{
	...
	if (le32_to_cpu(lmv1->lmv_hash_type) == LMV_HASH_TYPE_MIGRATION)
		RETURN(0);

	if (le32_to_cpu(lmv1->lmv_magic) == LMV_DIRECTORY_STRIPE_SHARD) {
