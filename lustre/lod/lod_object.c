				     ...
				     struct thandle *th)
{
	...
	if (dof->dof_type == DFT_REGULAR) {
		 ...
		 * did not find it initially, then return -EREMOTE,
		 * so client will retrieve the default stripeEA and
		 * re-create the directory.
		 *
		 * Note: if dah_eadata != NULL, it means creating the
		 * striped direcotry with specified stripeEA, then it
		 * should ignore the default stripeEA */
		if (hint->dah_eadata == NULL &&
