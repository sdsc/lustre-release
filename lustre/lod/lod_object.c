		    ...
		    struct lu_buf *lmv_buf)
{
	...
	lmm_size = lmv_mds_md_size(stripenr, LMV_MAGIC);
	...
	lmm1->lmv_stripe_count = cpu_to_le32(stripenr);
			       ...
			       struct thandle *th)
{
	...
	while (allocated < stripenr) {
		...
		CDEBUG(D_INFO, "prefer_idx %d idx %d mdt cnt %d allocated %d"
		       "last allocated %d\n", prefer_idx, idx, lod->lod_mdtnr,
		       allocated, idx_array[allocated - 1]);
	...
	for (i = 0; i < stripenr; i++) {
		...
		if (i != 0) {
			...
			rc = dt_declare_insert(env, dto,
				    (struct dt_rec *)lu_object_fid(&dto->do_lu),
					 ...
					 struct lod_object *lp)
{
	...
	if (rc < sizeof(struct lmv_user_md)) {
		...
		lp->ldo_dir_def_stripe_offset = -1;
		GOTO(unlock, rc = 0);
	}
			...
			umode_t child_mode)
{
	...
	if (S_ISDIR(child_mode)) {
		...
		/* If user specified stripes for this directory */
		if (ah->dah_eadata != NULL && ah->dah_eadata_len != 0) {
			...
			if (rc == 0 && lum1->lum_stripe_count > 1) {
				...
				CDEBUG(D_INFO, "set stripe EA nr:%hu off:%u\n",
				       lc->ldo_stripenr, lc->ldo_stripe_offset);
			}
			...
			CDEBUG(D_INFO, "inherite EA nr:%hu off:%u\n",
			       lc->ldo_stripenr, lc->ldo_stripe_offset);
		}
