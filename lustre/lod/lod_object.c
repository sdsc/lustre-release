					    ...
					    bool declare)
{
	struct lod_thread_info *info = lod_env_info(env);
	struct lod_object *lo = lod_dt_obj(dt);
	const struct lod_default_striping *lds = &info->lti_def_striping;
	...
	if (lds->lds_dir_def_striping_set &&
				      ...
				      umode_t mode)
{
	if (lds->lds_def_striping_set && S_ISREG(mode)) {
		...
		CDEBUG(D_INFO, "striping from default [%hu %u %d %s]\n",
		...
		CDEBUG(D_INFO, "striping from default [%hu %d %u]\n",
			...
			umode_t child_mode)
{
	...
	if (d->lod_md_root == NULL) {
		...
		if (!IS_ERR(root)) {
			...
			/* enable OSP attribute cache on slave MDTs */
			dt_declare_xattr_get(env, dt_object_child(root), NULL,
					     XATTR_NAME_LOV);
		}
	}
