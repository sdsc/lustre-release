				      ...
				      struct thandle *th)
{
	...
	LASSERT(lum->lum_stripe_count > 0);
					    ...
					    struct lustre_capa *capa)
{
	...
	CDEBUG(D_OTHER, "set default striping: count # %u offset %d\n",
	      (unsigned)le32_to_cpu(lum->lum_stripe_count),
			 ...
			 struct lustre_capa *capa)
{
	...
	} else if (strcmp(name, XATTR_NAME_DEFAULT_LMV)) {
		if (!S_ISDIR(attr))
			RETURN(-ENOTDIR);
