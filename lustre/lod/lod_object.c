				  ...
				  const struct lmv_user_md_v1 *lum)
{
	...
	if (unlikely(lum->lum_magic != LMV_USER_MAGIC))
		GOTO(out, rc = -EINVAL);

	if (unlikely(lum->lum_stripe_count == 0))
		GOTO(out, rc = -EINVAL);

	if (unlikely(lum->lum_stripe_count > lod->lod_mdtnr + 1))
		GOTO(out, rc = -EINVAL);
out:
	if (rc != 0)
		    ...
		    struct lu_buf *lmv_buf)
{
	...
	LASSERT(lo->ldo_dir_striped && lo->ldo_stripenr > 0);
			   ...
			   const struct lu_buf *buf)
{
	...
	OBD_ALLOC(stripe, sizeof(stripe[0]) * (lmv1->lmv_stripe_count - 1));
	if (stripe == NULL)
		RETURN(-ENOMEM);
...
out:
	...
	lo->ldo_stripenr = lmv1->lmv_stripe_count - 1;
	lo->ldo_stripes_allocated = lmv1->lmv_stripe_count - 1;
	if (rc != 0)
				      ...
				      struct thandle *th)
{
	...
	LASSERT(lum->lum_magic == LMV_USER_MAGIC &&
	...
	for (i = 1; i < stripe_count; i++) {
		...
		idx = (idx_array[i - 1] + 1) % (lod->lod_mdtnr + 1);

		for (j = 0; j < lod->lod_mdtnr;
		     j++, idx = (idx + 1) % (lod->lod_mdtnr + 1)) {
			bool already_allocated = false;
			int k;

			CDEBUG(D_INFO, "try idx %d, mdt cnt %d,"
			       " allocated %d, last allocated %d\n", idx,
			       lod->lod_mdtnr, i, idx_array[i - 1]);
			...
			umode_t child_mode)
{
	...
	if (S_ISDIR(child_mode)) {
		...
		if (lp->ldo_dir_striping_cached) {
			...
			CDEBUG(D_INFO, "inherit default EA nr:%d off:%d\n",
		...
		if (ah->dah_eadata != NULL && ah->dah_eadata_len != 0) {
			...
			if (rc == 0 && lum1->lum_stripe_count > 1) {
				 ...
				 * stripe_count > 1 */
				lc->ldo_stripenr = lum1->lum_stripe_count - 1;
				lc->ldo_dir_stripe_offset =
						   lum1->lum_stripe_offset;
			...
			lc->ldo_stripenr = 0;
			lc->ldo_dir_stripe_offset = (__u32)(-1);
		}
				     ...
				     bool declare)
{
	...
	if (lo->ldo_dir_def_striping_set &&
		...
		v1->lum_stripe_count = lo->ldo_stripenr + 1;
		v1->lum_stripe_offset = lo->ldo_dir_stripe_offset;
	...
	if (lo->ldo_dir_striping_cached &&
		...
		memset(v1, 0, sizeof(*v1));
		v1->lum_magic = cpu_to_le32(LMV_USER_MAGIC);
		v1->lum_stripe_count = lo->ldo_dir_def_stripenr + 1;
...
static int lod_object_unlock_internal(const struct lu_env *env,
				      ...
				      ldlm_policy_data_t *policy, int count)
{
	struct lod_object	*lo = lod_dt_obj(dt);
	int			rc = 0;
	int			i;
	...
	LASSERT(count <= lo->ldo_stripenr);
	for (i = 0; i < count; i++) {
		int	rc1;

		rc1 = dt_object_unlock(env, lo->ldo_stripe[i], einfo, policy);
		if (rc1 < 0)
			rc = rc == 0 ? rc1 : rc;
	}
