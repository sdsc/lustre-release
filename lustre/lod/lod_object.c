		    ...
		    struct lu_buf *lmv_buf)
{
	...
	if (lo->ldo_dir_striping_cached) {
		lmv_buf->lb_buf = info->lti_mea_store;
		lmv_buf->lb_len = lmm_size;
		RETURN(0);
	}

	if (info->lti_mea_store_size < lmm_size) {
		rc = lod_mea_store_resize(info, lmm_size);
		if (rc != 0)
			RETURN(rc);
	}

	lmm_common = info->lti_mea_store;
	lmm_common->lmv_magic = cpu_to_le32(LMV_MAGIC);
	lmm_common->lmv_stripe_count = cpu_to_le32(stripenr);
	lmm_common->lmv_hash_type = cpu_to_le32(MEA_MAGIC_ALL_CHARS);
	rc = lod_fld_lookup(env, lod, lu_object_fid(&dt->do_lu),
			    &lmm_common->lmv_master_mdt_index,
			    LU_SEQ_RANGE_MDT);
	if (rc != 0)
		RETURN(rc);
	...
	for (i = 0; i < stripenr; i++) {
		...
		lmm1 = (struct lmv_mds_md_v1 *)info->lti_mea_store;
		dto = lo->ldo_stripe[i];
		LASSERT(dto != NULL);
		fid_cpu_to_le(&lmm1->lmv_data[i], lu_object_fid(&dto->do_lu));
	}
			 ...
			 struct lustre_capa *capa)
{
	...
	if (S_ISDIR(attr)) {
		if (strncmp(name, XATTR_NAME_LOV,
			...
			union lmv_mds_md *lmv = buf->lb_buf;
				     ...
				     struct thandle *th)
{
	...
	if (dof->dof_type == DFT_REGULAR) {
		...
		struct lov_user_md_v3 *v3 =
				(struct lov_user_md_v3 *)info->lti_ea_store;
				      ...
				      void *policy, int count)
{
	struct lod_device	*lod = lu2lod_dev(dt->do_lu.lo_dev);
	struct lod_tgt_descs	*ltd = &lod->lod_mdt_descs;
	struct lmv_mds_md_common *lmv_common = einfo->ei_cbdata;
	struct lmv_mds_md_v1	*lmv1 = einfo->ei_cbdata;
	int			rc = 0;
	int			i;
	ENTRY;

	LASSERT(count <= lmv_common->lmv_stripe_count);
	for (i = 1; i < count; i++) {
		struct lod_tgt_desc	*tgt;
		...
		rc = lod_fld_lookup(env, lod, &lmv1->lmv_data[i],
		...
		slave = dt_locate_at(env, tgt->ltd_tgt, &lmv1->lmv_data[i],
				     dt->do_lu.lo_dev->ld_site->ls_top_dev);
		LASSERTF(!IS_ERR(slave), "can not find "DFID", slave "DFID"\n",
			 PFID(lu_object_fid(&dt->do_lu)),
			 PFID(&lmv1->lmv_data[i]));
			   ...
			   void *policy)
{
	...
	LASSERT(lmv_common->lmv_magic == LMV_MAGIC_V1);
	/* lock the slave objects */
	for (i = 1; i < lmv_common->lmv_stripe_count; i++) {
		struct lod_tgt_desc	*tgt;
		struct dt_object	*slave;
