 ...
 *
 * \retval 0		on success
 * \retval -ENOSPC	if no OST object are available at all
 ...
 * weights, then select the OSTs with ther weights used as the probability.
 ...
 *
 * \retval 0		on success
 * \retval -EAGAIN	if no enough OSTs are found for requested stripe count
 * \retval -EINVAL	requested OST index is invalid
 * \retval negative	errno on failure
 */
