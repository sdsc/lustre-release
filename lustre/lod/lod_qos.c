 ...
 *
 * Add a new OST target to the structure representing an OSS. R-sort the list
 ...
 * used is based on weights and it's trying to ensure the space is used
 ...
 * passed and less then 75% of the requested number of the stripe can be
 ...
 * function.
 * The algorithm is two-step: first of all, it finds OSTs available and
 * calculate total weight for those. Then the OSTs the greatest weights are
 * selected.
 *
 * \param[in] env	execution environment for this thread
 * \param[in] lo	LOD object
 ...
 * resulting configuration in the object to be used by the allocator later.
 ...
 * containing configuration hints can be provided optionally.
 ...
 * internal concurrency (QoS is serialized). The caller must ensure
