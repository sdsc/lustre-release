...
/**
 * Add a new target to QoS target table
 *
 * The function adds a new OST target to the a structure representing
 * an OSS and re-sort the list of known OSSs by the number of OSTs
 * living on them. The OSS list is protected internally and no external
 * locking is required.
 *
 * \param[in] lod		lod device
 * \param[in] ost_desc		OST description
 *
 * \retval 			0 if suceeed, -ENOMEM if allocation failed
 */
 ...
 * if no OSTs left on that.
 *
 * \param[in] lod		lod device
 * \param[in] ost_desc		OST description
 *
 * \retval 			0 if suceeed, -ENOENT if no OSS was found
 */
 ...
 *
 * Requests statfs data from the given target, verifies it's active and not
 * read-only. If so, then it can be used to place new OST objects.
 * The function also maintains the number of active/inactive targets,
 * sets dirty flags to notify if the number of active/inactive changes,
 * so others can run re-balance procedures. No external locking is
 * required.
 *
 * \param[in] env	LU environment provided by the caller
 * \param[in] d		LOD device
 * \param[in] index	index of OST target to check
 * \param[out] sfs	buffer for statfs dat
 *
 * \retval		0 if the target is good, negative error code otherwise
 */
...
/**
 * Re-calculate per-object penalties for OSSs and OSTs
 *
 * Re-calculate penalties when the configuration changes, active targets
 * change, after statfs refresh (all these are reflected by lq_dirty flag).
 *
 * \param[in] lod	LOD device
 *
 * \retval		0 on success, -EAGAIN if the number of OSTs isn't enough
 */
...
/**
 * Calculate optimal round-robin order with regard to OSSs
 *
 * The function places all the OSTs from the given pool in a special array
 * the round-robin allocation will be used. The placement algoright interleaves
 * OSTs from the different OSSs so that RR allocation can balance OSSs evenly.
 * The function resorts when the number of active targets changes (because of
 * a new target or activation/deactivation).
 *
 * \param[in] lod	LOD device
 * \param[in] src_poll	OST pool
 * \param[in] lqr	round-robin list
 *
 * \retval		0 on success, -ENOMEM if failed to allocate the array
 */
 ...
 *
 * The function instantiate LU representation for a new object on the
 * specified device. Also it declares an intention to create that
 * object on a storage.
 *
 * Notice lu_object_annon() is used which is a trick with regard to LU/OSD
 ...
 * Check is OST full
 *
 * The function checks the given statfs data whether such an OST
 * should be considered full.
 *
 * \param[in] msfs	statfs data
 *
 * \retval		false - not full, true - full
 */
 ...
 *
 * Every time a new striping is being created we fill a special temporary
 * array to remember all the targets already used for the striping and do
 * not use them again. The function allocates (or exentends if exists) this
 * temporary array and clear it. The array is stored in the environment.
 *
 * \param[in] env	LU environment provided by the caller
 * \param[in] stripes	number of items needed in the array
 *
 * \retval		0 on success, -ENOMEM if fails
 */
 ...
 *
 * Mark the given target is used for a new striping being created. Using
 * function lod_qos_is_ost_used() a caller can check that.
 *
 * \param[in] env	LU environment provided by the caller
 ...
 * object (should be prepared by the caller), but can change if the flag
 ...
 * required. The function succeed even if a single stripe is allocated. The
 * function is trying to allocate the stripes on the "fast" targets (meaning
 * they do have objects precreated and there is no need to wait for them). The
 * full OSTs are not considered (see lod_qos_dev_is_full() for the details).
 *
 * \param[in] env	LU environment provided by the caller
 ...
 *
 * \retval		0 on success
 * \retval		-ENOSPC if no enough OSTs are found
 * \retval		negative error if fails for another reason
 */
 ...
 *
 * Allocates a new striping using the OSTs provided. The full OSTs are
 * not considered. The exact order of OSTs is not important. The allocation
 * procedure prefers the targets with ready precreated objects. The number
 * of the stripes needed and stripe offset are taken from the object. If that
 * number can not be met, then the function returns a failure and then it's the
 * caller's responsbility to release the stripes allocated. All the internal
...
/**
 * Check wiether QoS allocation should be used
 *
 * A simple helper to decide when QoS allocation should be used:
 * if it's just a single available target or the used space is
 * evenly distributed among the targets at the moment.
 *
 * \param[in] lod	LOD device
 *
 * \retval		true - should be used, false - should not
 */
 ...
 * used is based on weights and it's trying to unsure the space is used
 * evenly by OSTs and OSSs. The striping configuration (# of stripes, offset,
 * pool) is taken from the object and supposed to be prepared by the caller.
 * If LOV_USES_DEFAULT_STRIPE is not passed and prepared configuration can't
 * be met due to lack of enough OSTs, then allocation fails. If the flag is
 * passed and less then #stripes*0.75 can be allocated, then allocation fails.
 * No concurrent allocation is allowed on the object and this should be
 * ensured by the caller. All the internal structures are protected by the
 * function.
 * The algorightm is two-step: first of all, it finds OSTs available and
 * calculate total weight for those. Then the OSTs the greater weights are
 * taken for the striping.
 *
 * \param[in] env	LU environment provided by the caller
 * \param[in] lo	LOD object
 * \param[out] stripe	striping created
...
/**
 * Find stripecound the caller can use
 *
 * Sometimes suggested stripecount can't be reached for a number of
 * the reasons: lack of enough active OSTs, the backend does not
 * support EAs that large. IOW, the function finds the maximal possible
 * stripecount not greater than passed one. If the passed one is 0, then
 * the filesystem's default one is used.
 *
 * \param[in] lod	LOD device
 * \param[in] magic	the format if striping
 * \param[in] stripe_count	count the caller would like to use
 *
 * \retval		the final max stripecount
 */
 ...
 *
 * When the caller passes a fully-defined striping (i.e. everything
 * including OST object FIDs are defined), then we still need to instantiate
 * LU-cache with the objects representing the stripes defined.
 *
 * \param[in] env	LU environment provided by the caller
 ...
 *
 * The caller gets a suggested striping configuration from the number of
 * the sources: per-directory default one, from the applications. Then
 * it needs to verify the suggested striping is valid, apply missing bits
 * and store the resulting configuration in the object to be used by the
 * allocater later. Should not be called concurrently against the same
 * object. It's OK to provide with a fully-defined striping.
 *
 * \param[in] env	LU environment provided by the caller
 * \param[in] lo	LOD object
 * \param[in] buf	buffer containing the striping
 ...
 *
 * The function creates a new striping for the object. A buffer
 * contaning configuration hints can be provided optionally.
 * The function tries QoS algorithm first unless the space distributed
 * evenly among OSTs, but by default RR algorithm is preferred due to
 * internal concurrency (QoS is serialized). The caller should ensure
 * no concurrent calls to the function are made against the same object.
 *
 * \param[in] env	LU environment provided by the caller
 * \param[in] lo	LOD object
 * \param[in] attr	attributes OST objects will be declared with
 * \param[in] buf	suggested striping configuration or NULL
 * \param[in] thandle	transaction handle
 */
