 ...
 *
 * This function adds a new OST target to the a structure representing an OSS
 * and re-sorts the list of known OSSs by the number of OSTs attached to each
 * OSS. The OSS list is protected internally and no external locking is
 * required.
 *
 ...
 * object on a storage.
 *
 * Note lu_object_annon() is used which is a trick with regard to LU/OSD
...
/**
 * Acceptable stripecount.
 *
 * Returns an acceptable stripecount depending on flag LOV_USES_DEFAULT_STRIPE:
 * all stripes or 3/4 of stripes.
 *
 * \param[in] stripe_cnt	stripes
 ...
 * \retval negative	negated errno if fails for another reason
 */
 ...
 * vary depending on OST status. The allocation procedure prefers the targets
 * with precreated objects ready. The number of the stripes needed and stripe
 * offset are taken from the object. If that number can not be met, then the
 * function returns a failure and then it's the caller's responsbility to
...
/**
 * Check weither QoS allocation should be used.
 *
 ...
 * allocation algorithm should be used.
 *
 * \param[in] lod	LOD device
 *
...
/**
 * Find stripe count the caller can use.
 *
 ...
 *
 * The caller gets a suggested striping configuration from the number of the
 * sources including per-directory default and applications. Then it needs to
 * verify the suggested striping is valid, apply missing bits and store the
 * resulting configuration in the object to be used by the allocater later.
 * Should not be called concurrently against the same object. It's OK to provide
 * with a fully-defined striping.
 *
 * \param[in] env	execution environment for this thread
 * \param[in] lo	LOD object
 * \param[in] buf	buffer containing the striping
