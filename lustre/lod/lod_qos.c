...
/* alloc objects on osts with specified OSTs */
			      ...
			      struct thandle *th)
{
	...
	LASSERT(lum != NULL && lum->lmm_magic == LOV_USER_MAGIC_V4);
	...
	if (i == lo->ldo_stripenr) {
		CERROR("%s: start index %d not found\n",
	...
	for (i = 0; i < lo->ldo_stripenr;
		...
		__u32 ost_idx = lum->lmm_objects[array_idx].l_ost_idx;
				...
				const struct lu_buf *buf)
{
	...
	case LOV_USER_MAGIC_V4:
		if (LOV_OFFSET_IS_DEFAULT(v1->lmm_stripe_offset))
	...
	default:
		CERROR("%s: don't recognize magic %X\n",
			...
			struct thandle *th)
{
	...
	if (likely(lo->ldo_stripe == NULL)) {
		...
		if (lum != NULL && lum->lmm_magic == LOV_USER_MAGIC_V4) {
			rc = lod_alloc_ost_list(env, lo, stripe, flag, lum, th);
		} else if (LOV_OFFSET_IS_DEFAULT(lo->ldo_def_stripe_offset)) {
