...
repeat_find:
	...
	for (i = 0; i < ost_count;
		...
		if (specified_osts) {
			...
			for (j = 0; j < lum->lmm_stripe_count; j++)
				if (ost_idx == lum->lmm_objects[j].l_ost_idx)
					break;

			if (j == lum->lmm_stripe_count) /* not found */
				continue;
		}
			...
			struct thandle *th)
{
	...
	if (likely(lo->ldo_stripe == NULL)) {
		...
		if (LOV_OFFSET_IS_DEFAULT(lo->ldo_def_stripe_offset)) {
