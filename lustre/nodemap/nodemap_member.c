...
static void *member_hs_key(struct hlist_node *hnode)
{
	...
	exp = hlist_entry(hnode, struct obd_export, exp_target_data.ted_nodemap_member);
...
{
	return hlist_entry(hnode, struct obd_export, exp_target_data.ted_nodemap_member);
}
...
void member_add(struct lu_nodemap *nodemap, struct obd_export *exp)
{
	...
	if (hlist_unhashed(&exp->exp_target_data.ted_nodemap_member) == 0) {
		CWARN("export %p %s already hashed, failed to add to nodemap %s\n", exp, exp->exp_client_uuid.uuid, nodemap->nm_name);
		return;
	}
