...
{
	LASSERT(member != NULL);
	LASSERT(cfs_atomic_read(&member->mem_refcount) > 0);
...
static void *member_hs_key(cfs_hlist_node_t *hnode)
{
	struct lu_nodemap_member *member;

	member = cfs_hlist_entry(hnode, struct lu_nodemap_member, mem_hash);

	return member->mem_id;
}

static int member_hs_keycmp(const void *key, cfs_hlist_node_t *hnode)
{
	struct lu_nodemap_member	*member;
...
static void *member_hs_hashobject(cfs_hlist_node_t *hnode)
{
	return cfs_hlist_entry(hnode, struct lu_nodemap_member, mem_hash);
}

static void member_hs_get(cfs_hash_t *hs, cfs_hlist_node_t *hnode)
{
	struct lu_nodemap_member *member;
				  ...
				  cfs_hlist_node_t *hnode, void *data)
{
	...
	member = cfs_hlist_entry(hnode, struct lu_nodemap_member, mem_hash);
	if (member->mem_exp != NULL)
		ldlm_revoke_export_locks(member->mem_exp);
