			      ...
			      struct thandle *th)
{
	...
	if (atomic_read(&obj->oo_zil_in_progress) > 0) {
		/* XXX: protect destroy vs. osd_zil_get_data() */
		/* once Isaac's patch with postponed destroy is landed
		 * then that infrastructure can be used here */
		printk("!!!! in-use by ZIL, pid %u\n", current->pid);
...
out:
	if (osd_use_zil(oh, rc))
		out_object_destroy_pack(env, &oh->ot_buf,
			...
			const struct lu_attr *la, struct thandle *handle)
{
	...
	if (osd_use_zil(oh, rc))
		out_attr_set_pack(env, &oh->ot_buf,
				  lu_object_fid(&dt->do_lu), la,
				  atomic_inc_return(&obj->oo_version));
			     ...
			     struct thandle *th)
{
	...
	if (osd_use_zil(oh, rc))
		out_create_pack(env, &oh->ot_buf, fid, attr, hint, dof,
				atomic_inc_return(&obj->oo_version));
			      ...
			      struct thandle *handle)
{
	...
	if (osd_use_zil(oh, rc))
		out_ref_add_pack(env, &oh->ot_buf,
				 lu_object_fid(&dt->do_lu),
				 atomic_inc_return(&obj->oo_version));
			      ...
			      struct thandle *handle)
{
	...
	if (osd_use_zil(oh, rc))
		out_ref_del_pack(env, &oh->ot_buf,
				 lu_object_fid(&dt->do_lu),
				 atomic_inc_return(&obj->oo_version));
