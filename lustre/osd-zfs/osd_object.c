			     ...
			     struct thandle *th)
{
	...
	db = osd_create_type_f(dof->dof_type)(env, obj, attr, oh);
	if (IS_ERR(db)) {
		rc = PTR_ERR(db);
		db = NULL;
		GOTO(out, rc);
	...
	rc = -sa_handle_get(osd->od_os, obj->oo_db->db_object, NULL,
			    SA_HDL_PRIVATE, &obj->oo_sa_hdl);
	if (rc)
		GOTO(out, rc);
	...
	rc = __osd_attr_init(env, osd, obj->oo_sa_hdl, oh->ot_tx, attr, parent);
	if (rc)
		GOTO(out, rc);
