			   ...
			   const struct lu_object_conf *conf)
{
	...
	result = osd_fid_lookup(env, osd, lu_object_fid(l), &oid);
	if (result == 0) {
		LASSERT(obj->oo_db == NULL);
		result = __osd_obj2dbuf(env, osd->od_objset.os, oid,
					&obj->oo_db, osd_obj_tag);
		if (result == 0) {
			LASSERT(obj->oo_db);
			result = osd_object_init0(env, obj);
		} else {
			CERROR("Error during lookup "DFID"/"LPX64": %d\n",
				PFID(lu_object_fid(l)), oid, result);
		}
					 ...
					 struct osd_thandle *oh)
{
	...
	if (obj->oo_xattr != ZFS_NO_OBJECT) {
		...
		while ((rc = -zap_cursor_retrieve(zc, za)) == 0) {
			...
			if (rc) {
				CERROR("%s: error during xattr lookup: %d\n",
				       osd->od_svname, rc);
				goto out_err;
			}
			      ...
			      struct thandle *th)
{
	...
	if (rc) {
		CERROR("%s: zap_remove() failed with error %d\n",
		       osd->od_svname, rc);
		GOTO(out, rc);
	}
	...
	if (rc) {
		CERROR("%s: __osd_object_destroy() failed with error %d\n",
		       osd->od_svname, rc);
		GOTO(out, rc);
	}
