			...
			const struct lu_attr *la, struct thandle *handle)
{
	...
	if (valid & LA_FLAGS) {
		...
		/* Note: even there might be LUSTRE_DEAD_FL flags here,
		 * oo_destroyed will be set in osd_object_destroy */
		...
		if (la->la_flags & LUSTRE_LMA_FL_MASKS) {
			...
			buf.lb_buf = lma;
			buf.lb_len = sizeof(info->oti_buf);
			rc1 = osd_xattr_get(env, &obj->oo_dt, &buf,
					   XATTR_NAME_LMA);
			if (rc1 > 0) {
				lma->lma_incompat =
				...
				rc1 = osd_xattr_set_internal(env, obj, &buf,
							    XATTR_NAME_LMA,
							    LU_XATTR_REPLACE,
							    oh);
			}
