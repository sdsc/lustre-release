		  ...
		  uint64_t parent, const char *name, uint64_t *child)
{
	...
	if (*child != 0) {
		rc = zap_create_claim_norm(o->od_os, *child,
		...
		oid = zap_create_norm(o->od_os, DMU_OT_DIRECTORY_CONTENTS,
				      14, /* ZFS fzap_default_block_shift */
				      DMU_OT_SA, DN_MAX_BONUSLEN, tx);
	}
	...
	/* XXX: what kind of error handling we want here? */
		  ...
		  const struct lu_fid *fid)
{
	...
	if (unlikely(fid_is_local_file(fid) || fid_seq_is_llog(fid_seq(fid))) ||
