				       ...
				       struct osd_thandle *oh)
{
	...
	size_t	new_size = oh->ot_update_max_size + UPDATE_BUFFER_SIZE_ADD;
	...
	memcpy(ureq, oh->ot_update, oh->ot_update_max_size);

	OBD_FREE_LARGE(oh->ot_update, oh->ot_update_max_size);

	oh->ot_update = ureq;
	oh->ot_update_max_size = new_size;
