...
#define UPDATE_BUFFER_SIZE_ADD	4096
			     ...
			     struct update_buffer *ubuf)
{
	...
	size_t	new_size = ubuf->ub_req_size + UPDATE_BUFFER_SIZE_ADD;
...
#define osd_zil_update_pack(env, buf, oh, obj, name, ...)		\
({									\
	...
	int				 ret;				\
									\
	while (rc == 0 && oh->ot_buf.ub_req != NULL) {			\
		ureq = oh->ot_buf.ub_req;				\
		max = oh->ot_buf.ub_req_size-object_update_request_size(ureq);\
		...
		if (ret == -E2BIG) {					\
			...
			rc1 = extend_update_buffer(env, &oh->ot_buf);	\
			if (rc1 != 0) {					\
				ret = rc1;				\
				break;					\
			}						\
