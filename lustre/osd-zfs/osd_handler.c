			 ...
			 struct dt_device_param *param)
{
	...
	bshift = osd_spa_maxblockshift(dmu_objset_spa(uos->os));
	param->ddp_grant_frag = udmu_blk_insert_cost(uos, bshift);
}
...
static void osd_recordsize_changed_cb(void *arg, uint64_t newval)
{
	...
	LASSERT(newval <= osd_spa_maxblocksize(dmu_objset_spa(uos->os)));
