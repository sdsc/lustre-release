				 ...
				 uint64_t nrblocks, uint64_t est_maxblockshift)
{
	...
	/* Average space/dnode more than maximum block size, use max block
	 * size to estimate free dnodes from adjusted free blocks count.
	 * OSTs typically use multiple blocks per dnode so this case applies. */
	if (est_usedobjs <= est_usedblocks) {
		est_totobjs = nrblocks;

	/* Average space/dnode smaller than min dnode size (probably due to
	 * metadnode compression), use min dnode size to estimate object count.
	 * MDTs may use only one block per node so this case applies. */
	 ...
	 *
	 * We know nrblocks is below 2^(64 - blkbits) bits, and est_usedobjs
	 * is under 48 bits due to DN_MAX_OBJECT_SHIFT, which means that
	 * multiplying them may get as large as 2 ^ 96 for the minimum
	 * blocksize of 64KB allowed above.
	 *
	 * The ratio of dnodes per block (est_usedobjs / est_usedblocks) is
	 * under 2^(blkbits - DNODE_SHIFT) = blocksize / 512 due to the
	 * limit checks above, so we can safely compute this first.  We care
	 * more about accuracy on the MDT (many dnodes/block) which is good
	 * because this is where truncation errors are smallest.  Since both
	 * nrblocks and dn_per_block are a function of blkbits, their product
	 * is at most 2^(64 - blkbits) * 2^(blkbits - DNODE_SHIFT) = 2^(64 - 9)
	 * so we can safely use 7 bits to compute a fixed-point fraction and
	 * est_totobjs can still fit in 64 bits. */
	} else {
