...
void osd_zfs_acct_fini(const struct lu_env *env, struct osd_device *o)
{
	...
	/* all the deltas should be applied already */
	LASSERTF(atomic_read(&o->od_quota_updates) == 0,
		 "%s: %u quota updates\n", o->od_svname,
		 atomic_read(&o->od_quota_updates));
...
/*
 * the function mimics original dmu_tx_create_assigned()
 * which is not exported
 */
static dmu_tx_t *
osd_dmu_tx_create_assigned(struct osd_device *osd, uint64_t txg)
{
	dmu_tx_t *tx;
	...
	tx->tx_dir = NULL;
	tx->tx_objset = NULL;
	tx->tx_pool = spa_get_dsl(dmu_objset_spa(osd->od_objset.os));
	tx->tx_txg = txg;
	tx->tx_lastsnap_txg = 0;
	tx->tx_anyobj = TRUE;
			       ...
			       struct hlist_node *node, void *data)
{
	...
	LASSERT(d->hcb_txg != 0);
	tx = osd_dmu_tx_create_assigned(osd, d->hcb_txg);

	rc = -zap_increment_int(osd->od_objset.os, d->hcb_zapid, za->zic_id,
				atomic_read(&za->zic_num), tx);
	if (unlikely(rc != 0))
...
#endif
{
	...
	LASSERT(zat->zat_txg == tx->tx_txg);

	cbdata.hcb_osd = osd;
	cbdata.hcb_txg = zat->zat_txg;
	...
	atomic_dec(&osd->od_quota_updates);
...
int osd_zfs_acct_trans_start(const struct lu_env *env, struct osd_thandle *oh)
{
	...
	/* XXX: we set blocks_modified=0 because otherwise
	 *	dsl_sync_task_sync() can skip calling the callback
	 *	which will result in the lost deltas and broken
	 *	accounting. I think we should be OK here given
	 *	every involved tx declares the changes to the ZAPs
	 *	storing accounting information. -bzzz */
	if (add_work) {
		spa_t *spa = dmu_objset_spa(osd->od_objset.os);
		LASSERT(ac->zat_osd == NULL);
		ac->zat_osd = osd;
		ac->zat_txg = oh->ot_tx->tx_txg;
		...
		atomic_inc(&osd->od_quota_updates);
	}
