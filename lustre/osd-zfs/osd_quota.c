				    ...
				    struct osd_thandle *oh)
{
	...
	rc = -zap_lookup(osd->od_os, oid, buf, sizeof(uint64_t), 1, &v);
	if (rc == -ENOENT) {
		v = 0;
	...
	if (tmp == za) {
		...
		if (rc == -ENOENT) {
			 ...
			 * cached value. */
			v = 0;
			rc = -zap_update(osd->od_os, oid, buf,
			       ...
			       struct hlist_node *node, void *data)
{
	...
	rc = -zap_lookup(d->hcb_osd->od_os, d->hcb_zapid,
			       ...
			       struct hlist_node *node, void *data)
{
	...
	rc = -zap_increment_int(osd->od_os, d->hcb_zapid, za->zic_id,
...
int osd_zfs_acct_trans_start(const struct lu_env *env, struct osd_thandle *oh)
{
	...
	if (add_work) {
		spa_t *spa = dmu_objset_spa(osd->od_os);
			   ...
			   struct dt_rec *dtrec, __u32 attr)
{
	 ...
	 * track inode usage */
	zic = osd_zfs_lookup_by_id(it->oiq_hash, it->oiq_id);
	if (zic != NULL) {
		/* found in the cache */
		rec->ispace = atomic_read(&zic->zic_num);
	} else {
		 rc = -zap_lookup(osd->od_os, it->oiq_obj->oo_db->db_object,
				  za->za_name, sizeof(uint64_t), 1,
				  &rec->ispace);
		 if (rc == -ENOENT) {
			/* user/group has not created any file yet */
