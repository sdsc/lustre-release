 ...
 */
int osd_zfs_acct_id(const struct lu_env *env, cfs_hash_t *hash,
 ...
 * at the beginning it initializes the cache from correspoded ZAP
 */
				    ...
				    struct osd_thandle *oh)
{
	...
	/*
	 * any concurrent thread is running in the zame txg, so no on-disk
	...
	if (rc == 0) {
		...
		CERROR("%s: can't access accounting zap %Lu\n",
		       osd->od_svname, oid);
		return;
	}
...
static void *acct_key(cfs_hlist_node_t *hnode)
{
	struct zfs_id_change	*ac;

	ac = cfs_hlist_entry(hnode, struct zfs_id_change, zic_hash);
	return &ac->zic_id;
}

static int acct_hashkey_keycmp(const void *key, cfs_hlist_node_t *compared_hnode)
{
	struct zfs_id_change	*ac;
	const __u64		*id = key;

	ac = cfs_hlist_entry(compared_hnode, struct zfs_id_change, zic_hash);
	return *id == ac->zic_id;
}

static void *acct_hashobject(cfs_hlist_node_t *hnode)
{
	return cfs_hlist_entry(hnode, struct zfs_id_change, zic_hash);
}
			       ...
			       cfs_hlist_node_t *node, void *data)
{
	...
	za = cfs_hlist_entry(node, struct zfs_id_change, zic_hash);

#if 1
	/*
	 * extra checks to ensure our cache matches on-disk state
	 */
	sprintf(buf, "%llx", za->zic_id);
	...
	if (atomic_read(&za->zic_num) != v) {
		CERROR("*** INVALID ACCOUNTING FOR %Lu: %d != %Ld (rc %d)\n",
			za->zic_id, atomic_read(&za->zic_num), v, rc);
		/* XXX: to cache with automated testing */
		LBUG();
	}
#endif
...
void osd_zfs_acct_fini(const struct lu_env *env, struct osd_device *o)
{
	...
	cfs_hash_for_each_safe(o->od_acct_usr, osd_zfs_delete_item, &cbdata);
	...
	cfs_hash_for_each_safe(o->od_acct_grp, osd_zfs_delete_item, &cbdata);
			       ...
			       cfs_hlist_node_t *node, void *data)
{
	...
	rc = -zap_increment_int(osd->od_objset.os, d->hcb_zapid,
				za->zic_id, atomic_read(&za->zic_num),
				d->hcb_tx);
	if (unlikely(rc != 0))
...
static void osd_zfs_acct_update(void *arg1, void *arg2, dmu_tx_t *tx)
{
	...
	struct osd_zfs_acct_txg	*zat = arg2;
	struct hash_cbdata	 cbdata;
