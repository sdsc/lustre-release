...
int osd_zil_get_data(void *arg, lr_write_t *lr, char *buf, zio_t *zio)
{
	...
	if (buf != NULL) { /* immediate write */
		...
		lr->lr_blkptr.blk_pad[1] = lu_object_fid(&obj->oo_dt.do_lu)->f_seq;
		lr->lr_blkptr.blk_fill = lu_object_fid(&obj->oo_dt.do_lu)->f_oid;
		...
		if (rc == 0) {
			...
			lr->lr_length |= (uint64_t)lu_object_fid(&obj->oo_dt.do_lu)->f_oid << 32;
			lr->lr_blkoff = lu_object_fid(&obj->oo_dt.do_lu)->f_seq;
...
we need to put lu_fid somewhere:
 - lr_foid must be some existing dnode
 - length can be taken from blkptr (LSIZE(&lr_blkptr)
 - lr_length must be >=512(1<<9) and <= 12M(1<<24)
   so, we need only 24-9=15bits, say 16 for 32M blocksize
   so we can occupy 64-16=48bit to store f_oid
 - lr_blkoff can store lu_fid.f_seq
typedef struct {
		       ...
		       struct dt_object *dt, uint64_t offset, uint64_t size)
{
	...
	while (size) {
		...
		lr->lr_offset = offset;
		lr->lr_length = len;
		lr->lr_blkoff = atomic_inc_return(&(obj)->oo_version);
				    ...
				    struct osd_thandle *oh)
{
	...
	wobdo = object_update_param_get(u, 0, &size);
				    ...
				    struct osd_thandle *oh)
{
	...
	if (lb.lb_buf == NULL) {
		/* data should be read from the log (indirect) */
		...
		lb.lb_len = *tmp;
		if (oh->ot_assigned != 0) {
			/* XXX: do not reallocate every time */
			lu_buf_check_and_alloc(&info->oti_lb, lb.lb_len);
			LASSERT(info->oti_lb.lb_buf != NULL);
				...
				boolean_t byteswap)
{
	...
	struct lu_fid		fid;
	uint64_t	pos, offset_in_log, version;
	void		*buf = (lr + 1);
	__u64	len;
	int	size;
	int	rc;
	...
	OBD_ALLOC_LARGE(ubuf.ub_req, size);
	LASSERT(ubuf.ub_req != NULL);
	ubuf.ub_req_size = size;
	...
	update = update_buffer_get_update(ubuf.ub_req, 0);
	LASSERT(update != NULL);
	...
	if (0)
	printk("@@@ move %s: ver=%u len=%u to "DFID", at %u\n",
	...
	rc = out_update_pack(cb->env, update, ubuf.ub_req_size, OUT_WRITE, &fid,
			     ARRAY_SIZE(sizes), sizes, bufs);
	LASSERT(rc == 0);
	update->ou_batchid = version;
	ubuf.ub_req->ourq_count = 1;

	/* copy record to the stable log */
	rc = osd_zil_write_to_log(cb, cb->log_dnode, ubuf.ub_req,
				object_update_request_size(ubuf.ub_req), NULL);
	LASSERT(rc == 0);

	OBD_FREE_LARGE(ubuf.ub_req, size);
...
static int osd_replay_log(struct osd_zil_cbdata *cb)
{
	...
	LASSERT(rc == 0);
	printk("!!! found log with %u bytes\n", (unsigned)attr->la_size);

	/* the buffer should be big enough for a full block */
	/* XXX: max.blocksize should be taken from the object set */
	OBD_ALLOC(buf, 256 * 1024);
	LASSERT(buf != NULL);
	...
	while (offset < attr->la_size) {
		...
		/* read the record */
		rc = dmu_read(osd->od_os, oid, offset, reclen, buf, 0);
		LASSERT(rc == 0);

		rc = osd_zil_parse_update(cb, buf, offset);
		if (rc != 0)
			break;
...
static int osd_destroy_log(struct osd_zil_cbdata *cb)
{
	return 0;
}
...
int osd_zil_replay(const struct lu_env *env, struct osd_device *o)
{
	 ...
	 * XXX: update the sequence so ZIL replay is atomic
	 */
	zil_replay(o->od_os, &cb, osd_zil_replay_vector);
	printk("%s: ZIL - %llu blocks, %llu recs, synced %llu\n",
		o->od_svname, o->od_zilog->zl_parse_blk_count,
		o->od_zilog->zl_parse_lr_count,
		dp->dp_tx.tx_synced_txg);
	printk("%s: %u in %u/%u tx in %u groups, %d discarded\n",
...
#if 0
	if (o->od_zilog->zl_parse_lr_count > 0 || 1) {
