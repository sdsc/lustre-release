 ...
 * 1) copy ZIL records to a regular files
 ...
 *    in contrast with ZPL, Lustre's transactions race each against other.
 *    say, transaction T1 changes object O1, then T2 changes O2, then T1
 ...
 *    and rollback to mimic TXG behavior: remember the stable state with
 ...
 * made with osd_commit_write(). the idea is to save memory during replay
 * and not require everything to fit memory.
 */
...
int osd_zil_replay(const struct lu_env *env, struct osd_device *o)
{
	...
	/* something could went from ZIL into the stable log at previous try
