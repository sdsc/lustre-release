 ...
 *
 * the order of the updates is tracked using per-object version. every update
 * to an object increments its version which goes to ZIL. every time an object
 * leaves the cache the global (per-osd) version is updated if it has a smaller
 * value. when an object gets cached, it's initial version is initialized from
 * the global version. this way we ensure any update to an object gets a bigger
 * version, so the order is preserved. notice the version is growing in
 * non-monothonical manner.
 *
 * we have to introduce additional locking to protect data being synced via ZIL
 * from modification from the upper layers. it's range locks, as sometimes the
 * original writes in ZIL aren't block-aligned. let's say we got two OST_WRITE
 * and those both went to block B, while in ZIL we got two records. the writes
 * were large enough to postpone data inlining till we really need to commit
 * ZIL. so at ZIL commit time we'll get two get_data() callbacks in a row and
 * both will be trying to lock the same block B to protect the data.
 */
...
struct zil_update {
	...
	struct list_head	 zu_on_zos; /* all updates of tx */
				    ...
				    struct osd_thandle *oh)
{
	...
	if (lb.lb_buf == NULL) {
		...
		if (oh->ot_assigned != 0) {
			...
			rc = -dmu_read(osd->od_os, info->oti_datalog,
...
static void osd_zil_add_update(struct zil_object_state *zos, struct zil_update *new)
{
	...
	list_for_each_entry_reverse(zup, &zos->zos_updates, zu_on_zos) {
		if (new->zu_version > zup->zu_version) {
			...
			/*printk("ADD %s "DFID" %llu (after %llu)\n",
			       update_op_str(new->zu_update->ou_type),
			       PFID(&new->zu_fid), new->zu_version,
			       zup->zu_version);*/
			return;
		}
	}
	list_add(&new->zu_on_zos, &zos->zos_updates);
	/*printk("ADD %s "DFID" %llu\n", update_op_str(new->zu_update->ou_type),
		PFID(&new->zu_fid), new->zu_version);*/
}
 ...
 */
static int osd_zil_parse_update(struct osd_zil_cbdata *cb, void *buf)
{
...
{
	struct osd_thread_info	*info = osd_oti_get(cb->env);
	...
	info->oti_datalog = cb->dlog_dnode;
	...
	list_for_each_entry_safe(zup, tmp, &ztx->zt_updates, zu_list) {
		...
		if (zz != zup) {
			...
			CDEBUG(D_OTHER, "tx %p "DFID" : %llu != %llu\n",
			       ...
			       zz->zu_version);
			/*printk("  SKIP %s "DFID" ver %llu (now %llu, first %llu)\n",
				update_op_str(u->ou_type), PFID(&zup->zu_fid),
				zup->zu_version, zup->zu_zos->zos_version,
				zz->zu_version);*/
		...
		/*printk("  apply %s "DFID" ver %llu (%llu now)\n",
			update_op_str(u->ou_type), PFID(&zup->zu_fid),
			zup->zu_version, zup->zu_zos->zos_version);*/
...
static void osd_zil_dump_updates(struct osd_zil_cbdata *cb)
{
	struct zil_update	*zup;
	struct zil_tx		*ztx;

	list_for_each_entry(ztx, &cb->zil_txs, zt_list) {
		printk("tx %p\n", ztx);
		list_for_each_entry(zup, &ztx->zt_updates, zu_list) {
			struct object_update	*u;
			u = zup->zu_update;
			printk("  %s "DFID" ver %llu (%llu now)\n",
...
static int osd_zil_apply_updates(struct osd_zil_cbdata *cb)
{
...
static int osd_zil_create_file(struct osd_zil_cbdata *cb, char *name,
				uint64_t *dnode)
{
	struct osd_device	*osd = cb->osd;
	...
	rc = __osd_attr_init(cb->env, osd, oid, tx, &la, osd->od_root);
...
static int osd_zil_check_and_create_log(struct osd_zil_cbdata *cb)
{
	int rc;
	...
	rc = osd_zil_create_file(cb, "log", &cb->log_dnode);
	if (rc < 0) {
...
static int osd_zil_check_and_create_data_log(struct osd_zil_cbdata *cb)
{
	int rc;
	...
	rc = osd_zil_create_file(cb, "dlog", &cb->dlog_dnode);
	if (rc < 0) {
...
static int osd_zil_write_to_log(struct osd_zil_cbdata *cb, uint64_t oid,
				void *buf, int reclen, __u64 *ret_offset)
{
	struct osd_device	*osd = cb->osd;
	dmu_tx_t		*tx;
	struct lu_attr		*la = &cb->la;
	__u64			 offset;
	int			 rc;

	rc = __osd_object_attr_get(cb->env, osd, oid, la);
	if (rc < 0)
		return rc;
	offset = la->la_size;
				 ...
				 boolean_t byteswap)
{
	...
	rc = osd_zil_check_and_create_log(cb);
	if (rc < 0)
		GOTO(out, rc);

	/* copy record to the stable log */
	size = lr->lr_common.lrc_reclen - sizeof(*lr);
	rc = osd_zil_write_to_log(cb, cb->log_dnode, lr + 1, size, NULL);

out:
	if (cb->result == 0)
		cb->result = rc;
	RETURN(SET_ERROR(-cb->result));
}
				...
				boolean_t byteswap)
{
	...
	rc = osd_zil_check_and_create_data_log(cb);
	if (rc < 0)
		GOTO(out, rc);
	...
	rc = osd_zil_write_to_log(cb, cb->dlog_dnode, buf, len, &offset_in_log);
	if (rc < 0) {
	...
	rc = out_update_pack(cb->env, update, size, OUT_WRITE, &fid,
	...
	/* copy record to the stable log */
	rc = osd_zil_write_to_log(cb, cb->log_dnode, ureq,
				...
				boolean_t byteswap)
{
	...
	rc = osd_zil_write_to_log(cb, cb->log_dnode, NULL, 0, NULL);
...
static int osd_replay_log(struct osd_zil_cbdata *cb)
{
	struct osd_device	*osd = cb->osd;
	struct zpl_direntry	 zde;
	struct lu_attr		*attr = &cb->la;
	struct lu_buf		 lb = { 0 };
	uint64_t		 oid, offset;
	uint32_t		 reclen;
	int			 rc;
	...
	/* get the size */
	rc = __osd_object_attr_get(cb->env, cb->osd, oid, attr);
	if (rc < 0)
		RETURN(rc);
	...
	cb->zil_dev = osd_zil_dev_prepare(cb->env, osd);
	if (cb->zil_dev == NULL) {
	...
	while (offset < attr->la_size) {
		...
		rc = osd_zil_parse_update(cb, lb.lb_buf);
		if (rc != 0)
			break;
	...
	osd_zil_free_records(cb->env, cb);

	cfs_hash_for_each_safe(cb->hash, osd_zil_release_state, NULL);
	lu_site_purge(cb->env, cb->zil_dev->zl_top_dev.dd_lu_dev.ld_site, ~0);

	LCONSOLE_INFO("%s: LOG - %u in %u/%u tx in %u groups, %d discarded\n",
		      osd->od_svname, cb->update_nr, cb->tx_nr, cb->iterations,
		      cb->barriers, cb->discarded);

out:
	lu_buf_free(&lb);
	if (cb->zil_dev != NULL)
		osd_zil_dev_fini(cb->env, cb->zil_dev);
	if (cb->hash != NULL)
...
static int osd_destroy_log(struct osd_zil_cbdata *cb, const char *name)
{
	struct osd_device	*osd = cb->osd;
	struct zpl_direntry	 zde;
	dmu_tx_t		*tx;
...
/*
 * 1) move ZIL to stable log (which is subject to snapshot/rollback)
 *    crash, continue to move ZIL records to the stable log
 * 2) take a snapshot
 *    crash, ZIL is empty, the log is not, restart from (2)
 * 3) replay from the stable log
 *    crash, find the snapshot, rollback to that, restart from (3)
 * 4) remove the log
 *    crash, find the snapshot, rollback to that restart from (3)
 * 5) discard the snapshot
 *
 */
...
int osd_zil_replay(const struct lu_env *env, struct osd_device *o)
{
	...
	if (rc < 0) {
		CERROR("%s: can't lookup the log: rc = %d\n", o->od_svname, rc);
		RETURN(rc);
	}
	...
	/* replay from the log */
	rc = osd_replay_log(&cb);
	if (rc != 0)
		GOTO(out, rc);

	/* destroy the logs */
	rc = osd_destroy_log(&cb, "log");
	if (rc != 0)
		GOTO(out, rc);
	rc = osd_destroy_log(&cb, "dlog");
	if (rc != 0)
		GOTO(out, rc);
