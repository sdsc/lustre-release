 ...
 *  - punch support
 *  - unlinked list
 *  - more sync records to make groups less
 *  - indirect read twice
 */
...
static int osd_replay_destroy_update(const struct lu_env *env,
				     ...
				     struct osd_thandle *oh)
{
	return dt_destroy(env, o, &oh->ot_super);
}
...
static int osd_replay_ref_add_update(const struct lu_env *env,
				     ...
				     struct osd_thandle *oh)
{
	return dt_ref_add(env, o, &oh->ot_super);
}
...
static int osd_replay_ref_del_update(const struct lu_env *env,
				     ...
				     struct osd_thandle *oh)
{
	return dt_ref_del(env, o, &oh->ot_super);
}
				      ...
				      struct osd_thandle *oh)
{
	...
	return dt_attr_set(env, o, attr, &oh->ot_super);
}
				       ...
				       struct osd_thandle *oh)
{
	...
	return dt_xattr_set(env, o, &b, name, fl, &oh->ot_super);
}
				       ...
				       struct osd_thandle *oh)
{
	...
	return dt_xattr_del(env, o, name, &oh->ot_super);
}
				    ...
				    struct osd_thandle *oh)
{
	...
	return dt_insert(env, o, (struct dt_rec *)&rec, (struct dt_key *)name,
			 &oh->ot_super, 1);
}
				    ...
				    struct osd_thandle *oh)
{
	...
	return dt_delete(env, o, (struct dt_key *)name, &oh->ot_super);
}
...
static int
osd_zil_read_bp(zilog_t *zilog, blkptr_t *bp, uint64_t object,
		uint64_t offset, void *wbuf)
{
	...
	LASSERT(!BP_IS_HOLE(bp));

	SET_BOOKMARK(&zb, osd_dmu_objset_id(zilog->zl_os), object,
			ZB_ZIL_LEVEL, offset / BP_GET_LSIZE(bp));

	rc = -arc_read(NULL, zilog->zl_spa, bp, arc_getbuf_func, &abuf,
				   ...
				   struct osd_thandle *oh)
{
	...
	if (IS_ERR(lb.lb_buf))
		rc = PTR_ERR(lb.lb_buf);

	if (rc == -ENODATA) {
		/* data should be read from the block specified (indirect) */
		tmp = object_update_param_get(u, 2, &size);
		LASSERT(tmp != NULL && size == sizeof(blkptr));
		memcpy(&blkptr, tmp, sizeof(blkptr));
		tmp = object_update_param_get(u, 3, &size);
		LASSERT(tmp != NULL && size == 8);
		lb.lb_len = *tmp;
		LASSERT(!BP_IS_HOLE(&blkptr));
		LASSERT(BP_GET_LSIZE(&blkptr) == lb.lb_len);
		lu_buf_check_and_alloc(&info->oti_lb, lb.lb_len);
		if (info->oti_lb.lb_buf == NULL)
			GOTO(out, rc = -ENOMEM);
		lb.lb_buf = info->oti_lb.lb_buf;
		rc = osd_zil_read_bp(osd->od_zilog, &blkptr,
				     obj->oo_db->db_object, pos, lb.lb_buf);
		LASSERT(rc == 0);
		CDEBUG(D_HA, "replay indirect %u@%lu\n",
		       (int)lb.lb_len, (unsigned long)pos);
	} else if (rc < 0) {
		/* XXX */
		LBUG();
	} else {
		LASSERT(size > 0);
		lb.lb_len = size;
		CDEBUG(D_HA, "replay direct %u@%lu\n",
		       (int)size, (unsigned long)pos);
	}
...
static int osd_replay_punch_update(const struct lu_env *env,
				   struct osd_replay_state *ors,
				   struct dt_object *o,
				   struct object_update *u,
				   struct osd_thandle *oh)
...
static osd_replay_update osd_replay_update_vec[] = {
	...
	[OUT_PUNCH]		= osd_replay_punch_update,
 ...
 */
static int osd_zil_parse_update(struct osd_replay_state *ors,
				void *buf, int bufsize)
{
	...
	for (i = 0; i < ureq->ourq_count; i++) {
		...
		zos = osd_zil_find_or_create_state(ors->ors_hash,
						   &update->ou_fid);
		if (zos == NULL)
			GOTO(out, rc = -ENOMEM);
		zup->zu_zos = zos;
...
static struct thandle *osd_zil_trans_create(struct osd_replay_state *ors)
{
	...
	tx = dmu_tx_create_assigned(dmu_objset_pool(osd->od_os),
				    ors->ors_txg);
	if (tx == NULL)
	...
	th = &oh->ot_super;
	th->th_dev = &osd->od_dt_dev;
	th->th_result = 0;
	th->th_tags = 0;
	RETURN(th);
}

static int osd_zil_trans_stop(const struct lu_env *env, struct dt_device *dt,
			  struct thandle *th)
{
	struct osd_thandle	*oh;
	...
	/* XXX: should we postpone this till after ZIL replay is over? */
	...
	list_for_each_entry_safe(zup, tmp, &ztx->zt_updates, zu_list) {
		...
		if (zz != zup) {
			...
			CDEBUG(D_OTHER, "tx %p "DFID" : %llu != %llu\n",
			       zup, PFID(&zup->zu_update->ou_fid),
			       zup->zu_version, zz->zu_version);
			continue;
		}
		...
		zo = zil_locate(env, ors, &zup->zu_update->ou_fid);
		if (IS_ERR(zo))
			GOTO(out, rc = PTR_ERR(zo));

		rc = osd_replay_update_vec[u->ou_type](env, ors, zo, u, oh);
		if (unlikely(rc < 0))
			GOTO(out, rc);

		zup->zu_zos->zos_version = zup->zu_version;

		lu_object_put(env, &zo->do_lu);
...
out:
	rc2 = osd_zil_trans_stop(env, dt, th);
	ztx->zt_repeats++;
	...
	list_for_each_entry(ztx, &ors->ors_zil_txs, zt_list) {
		...
		list_for_each_entry(zup, &ztx->zt_updates, zu_list) {
			...
			printk(KERN_INFO "  %s "DFID" ver %llu (%llu now)\n",
			       ...
			       PFID(&zup->zu_update->ou_fid), zup->zu_version,
			       zup->zu_zos->zos_version);
		}
	}
}
				 ...
				 boolean_t byteswap)
{
	...
	rc = osd_zil_parse_update(ors, lr + 1, size);
				...
				boolean_t byteswap)
{
	...
	size = sizeof(struct object_update_request) +
		...
		sizeof(struct object_update_param) * 4 +
		sizeof(__u64) +
		sizeof(__u64) +
		sizeof(lr->lr_blkptr) + 32;

	sizes[0] = 0;
	sizes[1] = sizeof(__u64);
	sizes[2] = sizeof(lr->lr_blkptr);
	...
	rc = osd_zil_parse_update(ors, ureq, size);
	LASSERT(rc == 0);
...
void osd_zil_make_itx(struct osd_thandle *oh)
{
	...
	zil_itx_assign(osd->od_zilog, itx, oh->ot_tx);

	/* the buffer has been consumed */
	oh->ot_update->ourq_count = 0;
}
...
static int osd_replay_log(struct osd_replay_state *ors)
{
	...
	/* we create a holder TX which basically pins TXG for this
	 * group of updates */
	tx = dmu_tx_create(osd->od_os);
	if (tx == NULL)
		GOTO(out, rc = -ENOMEM);
...
int osd_zil_replay(const struct lu_env *env, struct osd_device *o)
{
	...
	ors.ors_hash = cfs_hash_create("obj", HASH_POOLS_CUR_BITS,
				       HASH_POOLS_MAX_BITS, HASH_POOLS_BKT_BITS,
