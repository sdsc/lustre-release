...
static int osd_zil_replay_create(struct osd_replay_state *ors, lr_acl_v0_t *lr,
				boolean_t byteswap)
{
...
{
	struct osd_thread_info	*info = osd_oti_get(ors->ors_env);
	...
	info->oti_datalog = ors->ors_dlog_dnode;
				...
				boolean_t byteswap)
{
	...
	rc = osd_zil_write_to_log(ors, ors->ors_dlog_dnode, buf, len, &offset_in_log);
	if (rc < 0) {
...
static int osd_replay_log(struct osd_replay_state *ors)
{
	...
	lu_site_purge(ors->ors_env, ors->ors_zil_dev->zl_top_dev.dd_lu_dev.ld_site, ~0);

	LCONSOLE_INFO("%s: LOG - %u in %u/%u tx in %u groups, %d discarded\n",
		      osd->od_svname, ors->ors_update_nr, ors->ors_tx_nr, ors->ors_iterations,
