...
static int osd_zil_replay_tx(struct osd_zil_cbdata *cb, struct zil_tx *ztx)
{
	...
	list_for_each_entry(zup, &ztx->zt_updates, zu_list) {
		...
		zup->zu_obj = zil_locate(env, cb, &zup->zu_fid, 0);
...
int osd_zil_replay(const struct lu_env *env, struct osd_device *o)
{
	...
	if (snapname == NULL)
		RETURN(-ENOMEM);
	sprintf(snapname, "%s@rollback", o->od_mntdev);
...
#if 0
	zo = lu2zil_obj(lo);
	LASSERTF(version == 0 || zo->zo_version == 0 || version > zo->zo_version,
		 "unexpected version: %llu, was %llu\n",
		 version, zo->zo_version);
	if (version > 0)
		zo->zo_version = version;
#endif
