	...
	while (len > 0) {
		...
		if (sz_in_block == bs) {
			...
			while (sz_in_block > 0) {
				...
				lnb->lnb_rc = 0;
				if (sz_in_block == bs)
					lnb->lnb_dentry = (void *)abuf;
				else
					lnb->lnb_dentry = NULL;

				/* this one is not supposed to fail */
				lnb->lnb_page = kmem_to_page(abuf->b_data +
							off_in_block);
				LASSERT(lnb->lnb_page);
			...
			while (sz_in_block > 0) {
				...
				lnb->lnb_page = alloc_page(OSD_GFP_IO);
				if (unlikely(lnb->lnb_page == NULL))
					GOTO(out_err, rc = -ENOMEM);

				LASSERT(lnb->lnb_page->mapping == NULL);
				lnb->lnb_page->mapping = (void *)obj;
				...
				struct thandle *th)
{
	...
	for (i = 0; i < npages; i++) {
		 ...
		 * grant. */
		if ((lnb->lnb_flags & OBD_BRW_NOQUOTA) ||
		    (lnb->lnb_flags & (OBD_BRW_FROM_GRANT | OBD_BRW_SYNC)) ==
		    OBD_BRW_FROM_GRANT)
			ignore_quota = true;
		if (size == 0) {
			/* first valid lnb */
...
retry:
	...
	if (flags & QUOTA_FL_OVER_USRQUOTA)
		lnb->lnb_flags |= OBD_BRW_OVER_USRQUOTA;
	if (flags & QUOTA_FL_OVER_GRPQUOTA)
		lnb->lnb_flags |= OBD_BRW_OVER_GRPQUOTA;
	...
	for (i = 0; i < npages; i++) {
		...
		if (lnb->lnb_page->mapping == (void *)obj) {
			dmu_write(osd->od_objset.os, obj->oo_db->db_object,
				lnb[i].lnb_file_offset, lnb->lnb_len,
				kmap(lnb->lnb_page), oh->ot_tx);
			kunmap(lnb->lnb_page);
		} else if (lnb->lnb_dentry) {
			LASSERT(((unsigned long)lnb->lnb_dentry & 1) == 0);
		...
		if (new_size < lnb[i].lnb_file_offset + lnb->lnb_len)
			new_size = lnb[i].lnb_file_offset + lnb->lnb_len;
	}
			...
			struct niobuf_local *lnb, int npages)
{
	...
	for (i = 0; i < npages; i++) {
		buf.lb_buf = kmap(lnb->lnb_page);
		buf.lb_len = lnb->lnb_len;
		offset = lnb[i].lnb_file_offset;

		CDEBUG(D_OTHER, "read %u bytes at %u\n",
			(unsigned) lnb->lnb_len,
			(unsigned) lnb[i].lnb_file_offset);
		lnb->lnb_rc = osd_read(env, dt, &buf, &offset, NULL);
		kunmap(lnb->lnb_page);

		if (lnb->lnb_rc < buf.lb_len) {
