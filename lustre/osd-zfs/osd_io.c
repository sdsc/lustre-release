		       ...
		       struct dt_object *dt, uint64_t offset, uint64_t size)
{
	...
	out_write_pack(env, &oh->ot_buf,
			lu_object_fid(&dt->do_lu), &lb, offset, 0);
				    ...
				    struct osd_thandle *oh)
{
	...
	obdo = object_update_param_get(u, 0, &size);
	LASSERT(obdo != NULL && size == sizeof(*obdo));
	la_from_obdo(&attr, obdo, obdo->o_valid);
				    ...
				    struct osd_thandle *oh)
{
	...
	fl = object_update_param_get(u, 2, &size);
	LASSERT(fl != NULL && size == 4);
...
static int osd_zil_replay_update(struct osd_zil_cbdata *cb, lr_update_t *lr,
				boolean_t byteswap)
{
...
void osd_zil_fini(struct osd_device *o)
{
	...
	if (o->od_recs_in_log > 0)
		CWARN("@@@@@@@ %d bytes/rec ave\n",
		      (int)(o->od_bytes_in_log / o->od_recs_in_log));
...
void osd_zil_make_itx(struct osd_thandle *oh)
{
	...
	lr = (lr_update_t *)&itx->itx_lr;
	lr->lr_oid = 1;
	buf = (char *)(lr + 1);
	memcpy(buf, oh->ot_buf.ub_req, len);

	osd->od_recs_in_log++;
	osd->od_bytes_in_log += len;
...
#if 0
	{
	struct object_update_request	*r;
	r = oh->ot_buf.ub_req;
	printk("!!!!! %u updates in %d/%d at %p\n", r->ourq_count,
		(int)object_update_request_size(oh->ot_buf.ub_req),
		(int)oh->ot_buf.ub_req_size, r);
	}
#endif
