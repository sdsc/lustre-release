	...
	while (len > 0) {
		...
		for (i = 0; i < numbufs; i++) {
			...
			while (tocpy > 0) {
				...
				/* db_data might be NULL here if dbuf was just
				 * created, we have to issue an I/O (dbuf_read())
				 * against the buffer to fill db_data */
			...
			struct niobuf_local *lnb, int npages)
{
	...
	for (i = 0; i < npages; i++) {
		...
		/* wait until I/O on dbuf is done */
		ptr = ((unsigned long)lnb[i].lnb_data) & ~1UL;
		if (ptr != 0) {
			...
			if (unlikely(db->db_state == DB_UNCACHED))
				rc = -EIO;
			mutex_exit(&db->db_mtx);
			dbp = (dmu_buf_t *)db;
		}
		if (unlikely(rc < 0)) {
			/* fail all the remaning pages */
			lnb[i].lnb_rc = rc;
			continue;
		};

		LASSERT(dbp->db_data != NULL);
		LASSERT(dbp->db_offset <= lnb[i].lnb_file_offset);
		lnb[i].lnb_page = kmem_to_page(dbp->db_data +
					       lnb[i].lnb_page_offset);
		lnb[i].lnb_page_offset = lnb[i].lnb_page_offset & ~PAGE_MASK;
