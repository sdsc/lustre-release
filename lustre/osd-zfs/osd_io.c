    ...
    uint64_t size, int sync)
{
	...
	while (size) {
		...
		if (write_state == WR_COPIED && dmu_read(osd->od_os,
		    o->oo_db->db_object, offset, len, lr+1, DMU_READ_NO_PREFETCH) != 0) {
			zil_itx_destroy(itx);
			itx = zil_itx_create(TX_WRITE, sizeof (*lr));
			lr = (lr_write_t *)&itx->itx_lr;
			write_state = WR_NEED_COPY;
		...
		//printk("^^^^ new zil %p in txg %Lu: %u/%u type %u to %Lu\n",
				...
				boolean_t byteswap)
{
	...
	printk("REPLAY %u/%u to %Lu\n", (unsigned)off, (unsigned)len, lr->lr_foid);
...
zil_replay_func_t osd_zil_replay_vector[TX_MAX_TYPE] = {
	...
	(zil_replay_func_t)osd_zil_replay_err,	/* TX_CREATE */
