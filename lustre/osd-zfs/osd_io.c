			   ...
			   const char *name, const struct lu_buf *buf)
{
	...
	namelen = strlen(name);
	len = namelen + 1 + buf->lb_len;
	itx = zil_itx_create(TX_ACL_V0, sizeof(*lr) + len);
	lr = (lr_acl_v0_t *)&itx->itx_lr;
	lr->lr_foid = o->oo_db->db_object;
	b = (char *)lr + sizeof(*lr);
	strcpy(b, name);
	b = b + namelen + 1;
	memcpy(b, buf->lb_buf, buf->lb_len);
...
static int osd_zil_replay_create(struct osd_device *o, lr_acl_v0_t *lr,
				boolean_t byteswap)
{
...
static int osd_zil_replay_acl_v0(struct osd_device *osd, lr_acl_v0_t *lr,
				 boolean_t byteswap)
{
	int namelen, vallen;
	char *name, *val;
	...
	name = (char *)lr + sizeof(*lr);
	namelen = strlen(name);
	val = name + namelen + 1;
	vallen = lr->lr_common.lrc_reclen - sizeof(*lr) - namelen - 1;

	err = sa_handle_get(osd->od_os, lr->lr_foid, lr,
			    SA_HDL_PRIVATE, &sa);

	err = __osd_xattr_load(osd, lr->lr_foid, &nv);
	LASSERT(err == 0);

	err = -nvlist_remove(nv, name, DATA_TYPE_BYTE_ARRAY);
	LASSERT(err == 0 || err == -ENOENT);

	err = -nvlist_add_byte_array(nv, name, val, vallen);
	LASSERT(err == 0);

	err = -nvlist_size(nv, &sa_size, NV_ENCODE_XDR);
	LASSERT(err == 0);

	dxattr = sa_spill_alloc(KM_SLEEP);
	LASSERT(dxattr != NULL);

	err = -nvlist_pack(nv, &dxattr, &sa_size,
				NV_ENCODE_XDR, KM_SLEEP);
	LASSERT(err == 0);

	CDEBUG(D_HA, "REPLAY %s to %llu\n", name, lr->lr_foid);
...
restart:
	...
	if (err) {
		...
		err = -sa_update(sa, SA_ZPL_DXATTR(osd), dxattr, sa_size, tx);
		LASSERT(err == 0);
		/* Ensure the replayed seq is updated */
...
zil_replay_func_t osd_zil_replay_vector[TX_MAX_TYPE] = {
	...
	(zil_replay_func_t)osd_zil_replay_acl_v0,	/* TX_ACL_V0 */
};
