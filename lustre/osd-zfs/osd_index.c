			       ...
			       struct lu_fid *fid)
{
	...
	if (rc < 0) {
		CERROR("%s: can't get LINKEA: rc = %d\n",
		       osd_obj2dev(osd_dt_obj(o))->od_svname, rc);
		GOTO(out, rc);
	}
...
#error "remove OSD_ZFS_INSERT_DOTS_FOR_TESTING below"
#endif
...
#ifdef OSD_ZFS_INSERT_DOTS_FOR_TESTING
	if (rc == -ENOENT && name[0] == '.')
		rc = 0;
#endif
	if (rc && rc != -ENOENT)
				     ...
				     struct lustre_capa *capa)
{
	...
	it = (struct osd_zap_it *)osd_zap_it_init(env, dt, unused, capa);
	if (!IS_ERR(it))
		it->ozi_pos = 0;
 ...
 */
static int osd_zap_retrieve_skip_dots(struct osd_zap_it *it,
				      zap_attribute_t *za)
{
	int rc, isdot;
...
static int osd_dir_it_next(const struct lu_env *env, struct dt_it *di)
{
	 ...
	 */
	rc = osd_zap_retrieve_skip_dots(it, za);
...
#else
#error "remove extra check above"
#endif
