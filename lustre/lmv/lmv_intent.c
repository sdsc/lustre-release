...
update:
		...
		lsm->lsm_md_oinfo[i].lmo_size = body->size;

		/* subtract nlink of . and .. from non-master stripe */
		lsm->lsm_md_oinfo[i].lmo_nlink = body->nlink;

		lsm->lsm_md_oinfo[i].lmo_atime = body->atime;
		lsm->lsm_md_oinfo[i].lmo_ctime = body->ctime;
		lsm->lsm_md_oinfo[i].lmo_mtime = body->mtime;
		...
		if (req)
			ptlrpc_req_finished(req);
release_lock:
		size += lsm->lsm_md_oinfo[i].lmo_size;

		if (i != 0)
			nlink += lsm->lsm_md_oinfo[i].lmo_nlink - 2;
		else
			nlink += lsm->lsm_md_oinfo[i].lmo_nlink;

		if (lsm->lsm_md_oinfo[i].lmo_atime > atime)
			atime = lsm->lsm_md_oinfo[i].lmo_atime;

		if (lsm->lsm_md_oinfo[i].lmo_ctime > ctime)
			ctime = lsm->lsm_md_oinfo[i].lmo_ctime;

		if (lsm->lsm_md_oinfo[i].lmo_mtime > mtime)
			mtime = lsm->lsm_md_oinfo[i].lmo_mtime;
		    ...
		    __u64 extra_lock_flags)
{
	...
	if (it->it_flags & MDS_OPEN_BY_FID && fid_is_sane(&op_data->op_fid2)) {
		if (op_data->op_mea1) {
			...
			index = raw_name2idx(lsm->lsm_md_hash_type,
					     lsm->lsm_md_stripe_count,
					     op_data->op_name,
					     op_data->op_namelen);
			LASSERT(index < lsm->lsm_md_stripe_count);
			op_data->op_fid1 = lsm->lsm_md_oinfo[index].lmo_fid;
		}
