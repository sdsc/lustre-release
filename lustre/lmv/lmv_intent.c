...
static bool lmv_is_known_hash_type(const struct lmv_stripe_md *lsm)
{
		  ...
		  __u64 extra_lock_flags)
{
	...
	int			rc = 0;
	bool			try_all_stripes = false;
	ENTRY;

	/* If it is unknown hash type, it will not return, instead
	 * it will walk through all of stripes */
	tgt = lmv_locate_mds(lmv, op_data, &op_data->op_fid1);
	if (IS_ERR(tgt) && (PTR_ERR(tgt) != -EBADFD || lsm == NULL ||
			    lmv_is_known_hash_type(lsm)))
		RETURN(PTR_ERR(tgt));

	/* Both migrating dir and unknown hash dir need to walk through
	 * all of sub-stripes */
	...
	if (*reqp == NULL) {
		...
		RETURN(rc);
	} else if (it_disposition(it, DISP_LOOKUP_NEG) && try_all_stripes) {
		...
		for (idx = 0;
		     ...
		     it_disposition(it, DISP_LOOKUP_NEG); idx++) {
