...
update:
		...
		lsm->lsm_oinfo[i].lmo_nlink = i != 0 ?
					      body->nlink - 2 : body->nlink;

		CDEBUG(D_INODE, "Fresh size %lu nlink %lu from "DFID"\n",
...
release_lock:
	 ...
	 */
	CDEBUG(D_INODE, "Return refreshed attrs: size = %lu nlink %lu for "
	       DFID"\n", size, nlink, PFID(&lsm->lsm_oinfo[0].lmo_fid));
	if (mbody != NULL) {
		mbody->size = size;
		mbody->nlink = nlink;
	}
...
int lmv_revalidate_slaves_by_req(struct obd_export *exp,
				 ...
				 int extra_lock_flags)
{
	...
	return lmv_revalidate_slaves(exp, body, lsm, cb_blocking,
				     extra_lock_flags);
}
		    ...
		    __u64 extra_lock_flags)
{
	...
	/* Update all other stripes(nlink, size etc) if needed */
	if (op_data->op_mea2)
		rc = lmv_revalidate_slaves_by_req(exp, *reqp, op_data->op_mea2,
		      ...
		      __u64 extra_lock_flags)
{
	...
	if (op_data->op_mea2)
		rc = lmv_revalidate_slaves_by_req(exp, *reqp,
						  op_data->op_mea2,
						  cb_blocking,
						  extra_lock_flags);
	if (*reqp == NULL)
