			  ...
			  int extra_lock_flags)
{
	...
	for (i = 0; i < lsm->lsm_md_stripe_count; i++) {
		...
		if (rc > 0 && req == NULL) {
			...
			/* Slave is still valid on cache */
			lock = ldlm_handle2lock(lockh);
			LASSERT(lock != NULL);
			lock_res_and_lock(lock);
			lock_inode = igrab(lock->l_resource->lr_lvb_inode);
			LASSERT(lock_inode != NULL);
			CDEBUG(D_INODE, "slave "DFID" is still valid.\n",
			       PFID(&fid));
			LASSERTF(inode == lock_inode, DFID" lmo_root %lu"
				 " != lock_inode %lu", PFID(&fid),
				 inode->i_ino, lock_inode->i_ino);
			iput(lock_inode);
...
update:
			if (unlikely(body->nlink < 2)) {
				CERROR("nlink %d < 2 corrupt stripe %d "DFID":"
				       DFID"\n", body->nlink, i,
