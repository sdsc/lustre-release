...
int lmv_name_to_stripe_index(__u32 lmv_hash_type, unsigned int max_mdt_index,
			     const char *name, int namelen)
{
	int	idx;
	__u32	hashtype = lmv_hash_type & LMV_HASH_TYPE_MASK;

	LASSERT(namelen > 0);
	if (max_mdt_index <= 1)
		return 0;

	if (lmv_hash_type & LMV_MIGRATING_FLAG) {
		idx = 0;
		return 0;
	}
			     ...
			     struct lookup_intent *it)
{
	 ...
	 * has been deleted */
	if (lock->l_lvb_data == NULL || lock->l_lvb_len == 0) {
		LDLM_LOCK_PUT(lock);
		return -ESTALE;
	}
	...
	LDLM_LOCK_PUT(lock);
	rc = lmv_unpack_md(lmv->exp, &new_lsm, lmm,
			   (int)le32_to_cpu(lmm->lmv_md_v1.lmv_stripe_count));
	if (rc < 0)
		return rc;
...
lmv_locate_target_for_name(struct lmv_obd *lmv, struct lmv_stripe_md *lsm,
			   ...
			   mdsno_t *mds, ldlm_blocking_callback cb_blocking)
{
	struct lmv_tgt_desc	*tgt;
	const struct lmv_oinfo	*oinfo;
	int			rc;

	rc = lmv_refresh_lsm(lmv, lsm, cb_blocking);
	if (rc != 0)
		return ERR_PTR(rc);
...
{
	if (body == NULL) {
		/* body == NULL means we need to invalidate the lsm */
		lsm->lsm_md_invalid = 1;
		CDEBUG(D_INFO, "Invalidate LMV layout for "DFID"\n",
		       PFID(&lsm->lsm_md_master_fid));
		return 0;
	}
