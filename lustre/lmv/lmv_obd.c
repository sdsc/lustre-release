		...
		int stripe_count)
{
	...
	if (*lmmp != NULL && lsm == NULL) {
		lmm_common = (struct lmv_mds_md_common *)(*lmmp);
		lmm_size = lmv_mds_md_size(lmm_common->lmv_stripe_count,
					   lmm_common->lmv_magic);
		if (lmm_size == 0)
			RETURN(-EINVAL);
	...
	if (*lmmp == NULL && lsm == NULL) {
		...
		if (*lmmp == NULL)
			RETURN(-ENOMEM);
		lmm_common = (struct lmv_mds_md_common *)(*lmmp);
		lmm_common->lmv_stripe_count = stripe_count;
		lmm_common->lmv_magic = LMV_MAGIC;
		RETURN(lmm_size);
	}

	/* pack lmm */
	LASSERT(lsm != NULL);
	lmm_size = lmv_mds_md_size(lsm->lsm_count, lsm->lsm_md_magic);
	if (*lmmp == NULL) {
		OBD_ALLOC(*lmmp, lmm_size);
		if (*lmmp == NULL)
			RETURN(-ENOMEM);
	...
	lmm_common = (struct lmv_mds_md_common *)(*lmmp);
	lmm_common->lmv_magic = cpu_to_le32(lsm->lsm_md_magic);
	lmm_common->lmv_stripe_count = cpu_to_le32(lsm->lsm_count);
	lmm_common->lmv_master_mdt_index = cpu_to_le32(lsm->lsm_master);
	lmm_common->lmv_hash_type = cpu_to_le32(lsm->lsm_hash_type);
	cplen = strlcpy(lmm_common->lmv_pool_name, lsm->lsm_md_pool_name,
			sizeof(lmm_common->lmv_pool_name));
	if (cplen >= sizeof(lmm_common->lmv_pool_name))
		GOTO(free, rc = -E2BIG);

	switch (lsm->lsm_md_magic) {
	case LMV_MAGIC_V1:
		rc = lmv_pack_md_v1(lsm, (struct lmv_mds_md_v1 *)lmmp);
		break;
	default:
		rc = -EINVAL;
		break;
			    ...
			    const struct lmv_mds_md_v1 *lmm1)
{
	...
	stripe_count = le32_to_cpu(lmm1->lmv_common.lmv_stripe_count);
		  ...
		  const union lmv_mds_md *lmm, int stripe_count)
{
	...
	lsm->lsm_md_magic = le32_to_cpu(lmm_common->lmv_magic);
	lsm->lsm_count = le32_to_cpu(lmm_common->lmv_stripe_count);
	lsm->lsm_master = le32_to_cpu(lmm_common->lmv_master_mdt_index);
	lsm->lsm_hash_type = le32_to_cpu(lmm_common->lmv_hash_type);
	lsm->lsm_layout_version = le32_to_cpu(lmm_common->lmv_layout_version);
	cplen = strlcpy(lsm->lsm_md_pool_name, lmm_common->lmv_pool_name,
	...
	case LMV_MAGIC_V1:
		...
		break;
	default:
		CERROR("%s: unrecorganize magic %x\n", exp->exp_obd->obd_name,
		       le32_to_cpu(lmm_common->lmv_magic));
		rc = -EINVAL;
		break;
	}
