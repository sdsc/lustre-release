...
choose_hash:
	...
	case MEA_MAGIC_HASH_SEGMENT:
		...
		break;
	case MEA_MAGIC_FULL_NAME_HASH:
		c = mea_full_name_hash(count, name, namelen);
		break;
	default:
		...
		struct lu_fid *fid)
{
	...
	sidx = raw_name2idx(lsm->lsm_md_hash_type, lsm->lsm_md_stripe_count,
			    op_data->op_name, op_data->op_namelen);

	LASSERT(sidx < lsm->lsm_md_stripe_count);
	*fid = lsm->lsm_md_oinfo[sidx].lmo_fid;
	op_data->op_mds = lsm->lsm_md_oinfo[sidx].lmo_mds;
	tgt = lmv_get_target(lmv, lsm->lsm_md_oinfo[sidx].lmo_mds);
                    ...
                    struct ptlrpc_request **request)
{
	...
	if (op_data->op_mea2) {
		...
		op_data->op_fid2 = lsm->lsm_md_oinfo[index].lmo_fid;
	}
		   ...
		   struct cl_attr *attr)
{
	...
	for (i = 0; i < lsm->lsm_md_stripe_count; i++) {
		CDEBUG(D_INFO, ""DFID" size %llu nlink %u atime "LPU64
		       ...
		       PFID(&lsm->lsm_md_oinfo[i].lmo_fid),
		       lsm->lsm_md_oinfo[i].lmo_size,
		       lsm->lsm_md_oinfo[i].lmo_nlink,
		       lsm->lsm_md_oinfo[i].lmo_atime,
		       lsm->lsm_md_oinfo[i].lmo_ctime,
		       lsm->lsm_md_oinfo[i].lmo_mtime);
		attr->cat_size += lsm->lsm_md_oinfo[i].lmo_size;
		if (i != 0)
			attr->cat_nlink += lsm->lsm_md_oinfo[i].lmo_nlink - 2;
		else
			attr->cat_nlink = lsm->lsm_md_oinfo[i].lmo_nlink;

		if (attr->cat_atime < lsm->lsm_md_oinfo[i].lmo_atime)
			attr->cat_atime = lsm->lsm_md_oinfo[i].lmo_atime;
		if (attr->cat_ctime < lsm->lsm_md_oinfo[i].lmo_ctime)
			attr->cat_ctime = lsm->lsm_md_oinfo[i].lmo_ctime;
		if (attr->cat_mtime < lsm->lsm_md_oinfo[i].lmo_mtime)
			attr->cat_mtime = lsm->lsm_md_oinfo[i].lmo_mtime;
	}
	return 0;
}
