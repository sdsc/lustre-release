 ...
 * so that the request for the next set of directory pages from the
 * server can be made properly.
 *
 * A lu_dirpage is laid out as follows, where e=ldp_hash_end and each
 * "ent" is a struct lu_dirent.  It has size up to LU_PAGE_SIZE.  The
 * ldp_hash_end value is used to find the next lu_dirpage in a directory
 * listing that spans multiple pages (two in this example):
 *
 * .-----------------   -----.
 * |s|e|f|p|ent|ent| ... |ent|
 ...
 * larger than LU_PAGE_SIZE, a single host page if filled with multiple
 * lu_dirpages.  After reading the lu_dirpages from the MDS, the
 * ldp_hash_end of the first lu_dirpage refers to the one immediately
 * after it in the same CFS_PAGE:
 *
 * .------------------   -----.
 * |s|e0|f|p|ent|ent| ... |ent|
 * |--v---------------   -----|
 * |s|e1|f|p|ent|ent| ... |ent|
 ...
 * |    another CFS_PAGE      |
 *
 * We want to adjust this structure so it can be used as a single
 * logical lu_dirpage.  Because the code that finds the next lu_dirpage
 * assumes pages of size LU_PAGE_SIZE, it will use the ldp_hash_end from
 * the first lu_dirpage.  Therefore we need to modify that value so that
 * it refers to the lu_dirpage that comes after the _last_ lu_dirpage in
 * the CFS_PAGE.  This function does so by copying the ldp_hash_end
 * value from the last lu_dirpage (e' in the diagram) to the first (e0).
 */
...
static void lmv_adjust_dirpages(struct page **pages, int nrdpgs, int nlupgs)
{
	...
	for (i = 0; i < nrdpgs; i++) {
		...
		while (--nlupgs > 0) {
			 ...
			 * first entry of next lu_dirpage. In this way several
			 * lu_dirpages can be stored into one client page. */
			end_dirent = ((void *)end_dirent) +
				     le16_to_cpu(end_dirent->lde_reclen);
