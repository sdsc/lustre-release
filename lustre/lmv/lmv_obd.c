			  ...
			  struct lmv_mds_md_v1 *lmm1)
{
	...
	lmm1->lmv_master_mdt_index = cpu_to_le32(lsm->lsm_master_mdt_index);
	lmm1->lmv_hash_type = cpu_to_le32(lsm->lsm_hash_type);
		...
		int stripe_count)
{
	...
	if (*lmmp != NULL && lsm == NULL) {
		...
		lmm_size = lmv_mds_md_size(stripe_count, (*lmmp)->lmv_magic);
		if (lmm_size == 0)
			RETURN(-EINVAL);
			    ...
			    const struct lmv_mds_md_v1 *lmm1)
{
	...
	lsm->lsm_master_mdt_index = le32_to_cpu(lmm1->lmv_master_mdt_index);
	lsm->lsm_hash_type = le32_to_cpu(lmm1->lmv_hash_type);
	lsm->lsm_layout_version = le32_to_cpu(lmm1->lmv_layout_version);
	...
	CDEBUG(D_INFO, "unpack lsm count %d, master %d hash_type %d"
	       ...
	       lsm->lsm_master_mdt_index, lsm->lsm_hash_type,
