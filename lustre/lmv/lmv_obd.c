...
/*
 * Fix up an array of lu_dirpages so that the request for the next set
 * of pages from the server can be made properly.
 *
 * A host VM page (of size CFS_PAGE_SIZE) may contain more than one
 * lu_dirpage (of size LU_PAGE_SIZE), such as on platforms that use 64k
 * pages.  In that case, overwrite the ldp_hash_end and ldp_flags of the
 * first lu_dirpage with those of the last.  This is needed because
 * those are the values used to request the next set of directory
 * pages from the server, but the code that does so will only look at
 * the first lu_dirpage.
 */
#if CFS_PAGE_SIZE > LU_PAGE_SIZE
static void lmv_fix_dirpages(struct lu_dirpage *dp, int nlupgs)
{
	...
	while (nlupgs--) {
		...
		/* Advance dp to next lu_dirpage in host VM page. */
		dp = (struct lu_dirpage *)((char *)dp + LU_PAGE_SIZE);

		/* Check if we've reached the end of a CFS_PAGE or if
		 * there are no more lu_dirpages to process. */
		if (!((unsigned long)dp & ~CFS_PAGE_MASK) || nlupgs == 0)
			break;

		/* Save the hash and flags of this dirpage. */
		hash_end = dp->ldp_hash_end;
		flags = dp->ldp_flags;
