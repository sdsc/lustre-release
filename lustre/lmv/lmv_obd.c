                        ...
                        struct ptlrpc_request **request)
{
        ...
        nrdpgs = ((*request)->rq_bulk->bd_nob_transferred + CFS_PAGE_SIZE - 1)
                 >> CFS_PAGE_SHIFT;
        nxpgs = (*request)->rq_bulk->bd_nob_transferred >> LU_PAGE_SHIFT;
        ...
        for (i = 0; i < nrdpgs; i++) {
                ...
                if (obj) {
                        ...
                        if ((tgt0_idx != nr - 1) &&
                            (le64_to_cpu(dp->ldp_hash_end) == DIR_END_OFF)) {
...
#if CFS_PAGE_SIZE > LU_PAGE_SIZE
                if (((char *)dp + LU_PAGE_SIZE < (char *)first + CFS_PAGE_SIZE)
                    && nxpgs > 0) {
                        ...
                        if (tmp) {
                                 ...
                                 * first entry of next page, in this way several
                                 * pages can be integrated into one normal dir
                                 * page on client.
                                 */
                                tmp = ((void *)tmp) +
