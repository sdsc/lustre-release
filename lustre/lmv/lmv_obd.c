...
/**
 * Get the minum dir entry, whose hash value is larger or equal to @hash_offset,
 * and it is only being called for striped directory.
 *
				 ...
				 __u64 offset, struct page **ppage)
{
	...
	entry_page = alloc_page(GFP_KERNEL);
	if (entry_page == NULL)
		RETURN(-ENOMEM);
	...
	area += sizeof(*dp);
	left = PAGE_CACHE_SIZE - sizeof(*dp);
	ent = area;
	do {
		__u16			esize;
		...
		if (min_ent == NULL) {
			...
			hash_offset = le64_to_cpu(MDS_DIR_END_OFF);
			GOTO(out, rc);
		}

		esize = le16_to_cpu(min_ent->lde_reclen);

		/* the last entry lde_reclen is 0, but it might not
		 * the end of this entry of this temporay entry */
		if (esize == 0)
			esize = lu_dirent_calc_size(min_ent->lde_namelen,
						    min_ent->lde_attrs);
		if (esize > left) {
			last_ent->lde_reclen = 0;
		...
		if (strncmp(ent->lde_name, ".",
		    ...
		    strncmp(ent->lde_name, "..",
			   le16_to_cpu(ent->lde_namelen)) == 0) {
			if (le16_to_cpu(ent->lde_namelen) == 1)
		...
		left -= esize;
		ent->lde_reclen = cpu_to_le16(esize);
		last_ent = ent;
		ent = (void *)ent + esize;
		hash_offset = le64_to_cpu(min_ent->lde_hash);
