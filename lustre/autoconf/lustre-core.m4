...
# 2.6.33 through 3.4 have quotactl_ops->quota_sync with 3 arguments
AC_DEFUN([LC_HAVE_QUOTA_SYNC_3ARGS],
[AC_MSG_CHECKING([if quotactl_ops->quota_sync takes 3 arguments])
LB_LINUX_TRY_COMPILE([
        #include <linux/fs.h>
        #include <linux/quota.h>
],[
	struct quotactl_ops qops = {};
	qops.quota_sync(NULL, 0, 0);
],[
        AC_DEFINE(HAVE_QUOTA_SYNC_3ARGS, 1, [quotactl_ops->quota_sync takes 3 arguments])
        AC_MSG_RESULT([yes])
],[
        AC_MSG_RESULT([no])
])
...
#
# 2.6.37 (fd8c37ec) changed ext4's set_opt to take a long as its first arg
#
AC_DEFUN([LC_HAVE_EXT4_SET_OPT_TAKES_LONG],
[AC_MSG_CHECKING([if ext4's set_opt takes long as its first argument])
tmp_flags="$EXTRA_KCFLAGS"
EXTRA_KCFLAGS="-Ifs"
LB_LINUX_TRY_COMPILE([
        #include <ext4/ext4.h>
        ...
        AC_MSG_RESULT([yes])
        AC_DEFINE(HAVE_EXT4_SET_OPT_TAKES_LONG, 1,
                [ext4's set_opt takes long as its first argument])
],[
        AC_MSG_RESULT([no])
])
EXTRA_KCFLAGS="$tmp_flags"
...
AC_DEFUN([LC_PROG_LINUX],
	 ...
	 LC_WRITEBACK_INODES_NEEDS_SB_UMOUNT_LOCK
         ...
         LC_HAVE_EXT4_SET_OPT_TAKES_LONG
