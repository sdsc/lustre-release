...
/* Sortcut to the list of hal_records of a copytool char device */
#define ct_cdev_hal_records_list ct_cdev_dummy_hal_record.hrec_list
/* The copytool char device struct
 * (note that ct_cdev_hal_records_list might be used like another member of that
 * struct)
 */
struct ct_cdev {
	/* To register the char device */
	struct miscdevice	 ct_cdev_misc;
	char			 ct_cdev_name[CTDEV_NAME_LEN_MAX];
	struct list_head	 ct_cdev_list;
	struct kref		 ct_cdev_ref;

	/* To operate the char device */
	struct hal_record	 ct_cdev_dummy_hal_record;
	struct rw_semaphore	 ct_cdev_hal_records_rwsem;
	struct workqueue_struct	*ct_cdev_rd_wake_up_wqueue;
	struct work_struct	 ct_cdev_rd_wake_up_work;
	wait_queue_head_t	 ct_cdev_rd_queue;
	wait_queue_head_t	 ct_cdev_wr_queue;
	atomic_t		 ct_cdev_readers_number;
	atomic_t		 ct_cdev_queue_size;
};
...
/**
 * Retrieve the fsname of an obd_device from it obd_name member
 */
static void obd_name2fsname(char *fsname, char *obd_name)
{
	char tmp[MAX_OBD_NAME];
	char *minus; /* will point to a '-' char */
	int i;

	snprintf(tmp, MAX_OBD_NAME, obd_name);
	...
	snprintf(fsname, LUSTRE_MAXFSNAME + 1, tmp);
}

/**
 * Get the fsname a copytool char device is working for
 * \retval	a pointer to the fsname the device is working for if it exists
 *		NULL otherwise
 */
static inline char *ct_cdev_fsname(struct ct_cdev *device)
{
	char *fsname = strchr(device->ct_cdev_name, '-');

	return (fsname == NULL) ? NULL : fsname + 1;
}
 ...
 *		NULL otherwise
 */
static struct ct_cdev *obd2ct_cdev_locked(struct obd_device *obd)
{
	char fsname[LUSTRE_MAXFSNAME + 1];
	struct ct_cdev *device;

	obd_name2fsname(fsname, obd->obd_name);

	list_for_each_entry(device, &device_list, ct_cdev_list) {
...
{
	return (atomic_read(&device->ct_cdev_queue_size) + count +
 ...
 */
static bool something_to_read(struct reader *reader)
{
	...
	return !list_is_last(&reader->reader_last_rec->hrec_list,
			     &reader->reader_ct_cdev->ct_cdev_hal_records_list);
}
...
static void wake_up_readers(struct work_struct *work)
{
	struct ct_cdev *device = container_of(work, struct ct_cdev,
					      ct_cdev_rd_wake_up_work);

	wake_up_interruptible(&device->ct_cdev_rd_queue);
}

/**
 * Does nothing
 */
static ssize_t ct_cdev_write(struct file *file, const char __user *buff,
...
/**
 * Adds a message to a copytool char device. The device is chosen according to
 * the obd provided.
 *
			    ...
			    int flags)
{
	...
	CDEBUG(D_HSM, "ct_cdev: Writing %lu bytes on channel %i...\n", count,
	       hal->hal_archive_id);
	...
	if (atomic_read(&device->ct_cdev_readers_number) == 0)
		/* Nothing to do */
		RETURN(count);
	...
	if (hrec == NULL)
		RETURN(-ENOMEM);
	/* Allocate the hsm_action_list separately because it might be too big
	 * to allocate at once with the hal_record (cf. HAL_MAXSIZE) */
	OBD_ALLOC(hrec->hrec_hal, hal_size(hal));
	if (hrec->hrec_hal == NULL) {
		OBD_FREE_PTR(hrec);
		RETURN(-ENOMEM);
	}
	hrec->hrec_ct_cdev = device;
	memcpy(hrec->hrec_hal, hal, hal_size(hal));

repeat:
	if (wait_event_interruptible(device->ct_cdev_wr_queue,
	...
	down_write(&device->ct_cdev_hal_records_rwsem);

	if (atomic_read(&device->ct_cdev_readers_number) == 0) {
		up_write(&device->ct_cdev_hal_records_rwsem);
		OBD_FREE_PTR(hrec);
		RETURN(count);
	}
	...
	for (i = atomic_read(&device->ct_cdev_readers_number); i > 1; i--)
		kref_get(&hrec->hrec_ref);

	/* Publish the hal_record using an rcu method (cf. read) */
	list_add_tail_rcu(&hrec->hrec_list, &device->ct_cdev_hal_records_list);
	/* Update the queue size */
	atomic_add(sizeof(struct hal_record) + count,
		   &device->ct_cdev_queue_size);

	up_write(&device->ct_cdev_hal_records_rwsem);

	queue_work(device->ct_cdev_rd_wake_up_wqueue,
		   &device->ct_cdev_rd_wake_up_work);

	CDEBUG(D_HSM, "ct_cdev: Wrote %lu bytes for the obd '%s'\n", count,
	       obd->obd_name);
	RETURN(count);
}
...
static struct ct_cdev *inode2ct_cdev(struct inode *inode)
{
	...
	list_for_each_entry(device, &device_list, ct_cdev_list) {
		if (device->ct_cdev_misc.minor == minor)
			break;
	}
	up_read(&device_rwsem);
...
repeat:
		down_read(&device->ct_cdev_hal_records_rwsem);

		reader->reader_last_rec = \
			list_last_entry(&device->ct_cdev_hal_records_list,
				        struct hal_record, hrec_list);
		if (!kref_get_unless_zero(&reader->reader_last_rec->hrec_ref)) {
			/* Unlikely to happen */
			up_read(&device->ct_cdev_hal_records_rwsem);
			goto repeat;
		}

		atomic_inc(&device->ct_cdev_readers_number);
...
{
	struct hal_record *hrec = container_of(ref, struct hal_record, hrec_ref);
	struct ct_cdev *device = hrec->hrec_ct_cdev;
	ENTRY;

	down_write(&device->ct_cdev_hal_records_rwsem);

	atomic_sub(sizeof(struct hal_record) + hal_size(hrec->hrec_hal),
		   &device->ct_cdev_queue_size);
	list_del(&hrec->hrec_list);

	up_write(&device->ct_cdev_hal_records_rwsem);

	wake_up_interruptible(&device->ct_cdev_wr_queue);
	OBD_FREE(hrec->hrec_hal, hal_size(hrec->hrec_hal));
	OBD_FREE_PTR(hrec);
	EXIT;
}
...
static int ct_cdev_release(struct inode *inode, struct file *file)
{
	...
	if (file->f_mode & FMODE_READ) {
		...
		last_rec = list_last_entry(&device->ct_cdev_hal_records_list,
					   struct hal_record, hrec_list);
		/* Removing the reader */
		atomic_dec(&device->ct_cdev_readers_number);
...
repeat:
	if (wait_event_interruptible(device->ct_cdev_rd_queue,
	 ...
	 * relies on rcu. */
	rcu_read_lock();
	list_for_each_entry_continue_rcu(hrec, &device->ct_cdev_hal_records_list,
					 hrec_list) {
		last_rec = hrec;
		if (!right_channel(hrec, *ppos))
			continue;
		if (hal_size(hrec->hrec_hal) > count - rd_bytes) {
		...
		rcu_read_unlock();
		if (copy_to_user(buff, hrec->hrec_hal,
				 hal_size(hrec->hrec_hal)))
			RETURN(-EFAULT);
		/* Acquire the lock again for the list-traversal operation */
		rcu_read_lock();

		buff += hal_size(hrec->hrec_hal);
		rd_bytes += hal_size(hrec->hrec_hal);
	}
	rcu_read_unlock();
	...
	CDEBUG(D_HSM, "ct_cdev: Read %lu bytes", rd_bytes);
	RETURN(rd_bytes);
}
...
static unsigned int ct_cdev_poll(struct file *file, poll_table *wait)
{
	...
	if (file->f_mode & FMODE_READ) {
		struct reader *reader = file->private_data;
		struct ct_cdev *device = reader->reader_ct_cdev;
		poll_wait(file, &device->ct_cdev_rd_queue, wait);
...
int ct_cdev_init(struct obd_device *obd)
{
	...
	obd_name2fsname(fsname, obd->obd_name);
	snprintf(device->ct_cdev_name, CTDEV_NAME_LEN_MAX, "copytool-%s",
		 fsname);
	device->ct_cdev_misc.minor = MISC_DYNAMIC_MINOR;
	device->ct_cdev_misc.name = device->ct_cdev_name;
	device->ct_cdev_misc.fops = &ct_cdev_fops;

	/* Init dummy hal_record */
	kref_init(&device->ct_cdev_dummy_hal_record.hrec_ref);

	/* Reader/Writer semaphore */
	init_rwsem(&device->ct_cdev_hal_records_rwsem);

	/* hal_records list */
	INIT_LIST_HEAD(&device->ct_cdev_hal_records_list);

	/* Readers queue */
	init_waitqueue_head(&device->ct_cdev_rd_queue);
	/* Readers waker workqueue */
	device->ct_cdev_rd_wake_up_wqueue =
		create_singlethread_workqueue(device->ct_cdev_name);
	if (device->ct_cdev_rd_wake_up_wqueue == NULL) {
		rc = -ENOMEM;
		goto out_free;
	}

	/* Readers waking work */
	INIT_WORK(&device->ct_cdev_rd_wake_up_work, wake_up_readers);

	/* Writers queue */
	init_waitqueue_head(&device->ct_cdev_wr_queue);

	/* Global readers count */
	atomic_set(&device->ct_cdev_readers_number, 0);

	/* Size of the elements in the list */
	atomic_set(&device->ct_cdev_queue_size, 0);

	kref_init(&device->ct_cdev_ref);

	list_add(&device->ct_cdev_list, &device_list);

	/* Now we can actually register the device */
	rc = misc_register(&device->ct_cdev_misc);
	if (rc != 0)
		goto out_cleanup;
...
out_cleanup:
	destroy_workqueue(device->ct_cdev_rd_wake_up_wqueue);
out_free:
	OBD_FREE_PTR(device);
	up_write(&device_rwsem);
	RETURN(rc);
...
static void ct_cdev_deregister(struct kref *kref)
{
	...
	list_del(&device->ct_cdev_list);

	misc_deregister(&device->ct_cdev_misc);
	flush_workqueue(device->ct_cdev_rd_wake_up_wqueue);
	destroy_workqueue(device->ct_cdev_rd_wake_up_wqueue);
	OBD_FREE_PTR(device);
...
void ct_cdev_exit(struct obd_device* obd)
{
	...
	down_write(&device_rwsem);
	device = obd2ct_cdev_locked(obd);
	kref_put(&device->ct_cdev_ref, ct_cdev_deregister);
	up_write(&device_rwsem);
	EXIT;
}
