...
struct ct_cdev {
	...
	struct mutex		 hal_release_record_mutex;
#endif
...
{
	char *fsname = strchr(device->name, '-');
...
repeat:
	if (wait_event_interruptible(device->wr_queue,
				     ...
				     flags & O_NONBLOCK)) {
		OBD_FREE(hrec, hrec->hrec_hal_sz);
		OBD_FREE_PTR(hrec);
		RETURN(-ERESTARTSYS);
	}
...
#ifdef HAVE_KREF_GET_UNLESS_ZERO
repeat:
		...
		mutex_lock(&device->hal_release_record_mutex);

		reader->reader_last_rec =
			list_last_entry(&device->hal_records_list,
					struct hal_record, hrec_list);

		mutex_unlock(&device->hal_release_record_mutex);
#endif
...
static void cleanup_hal_record(struct kref *ref)
{
	...
	OBD_FREE(hrec->hrec_hal, hrec->hrec_hal_sz);
	OBD_FREE_PTR(hrec);
	EXIT;
}
...
#ifndef HAVE_KREF_GET_UNLESS_ZERO
	/* Init the mutex to release records */
	mutex_init(&device->hal_release_record_mutex);
#endif
