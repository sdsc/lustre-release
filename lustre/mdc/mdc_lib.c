...
void mdc_close_pack(struct ptlrpc_request *req, struct md_op_data *op_data)
{
	...
	mdc_setattr_pack_rec(rec, op_data);
	/*
	 * Some client may zero out the timestampes when losing ibits lock,
	 * and there was a defect on server side which allows the atime
	 * to be overwritten by stale atime packed in close RPC.
	 *
	 * We clear out the ATTR_ATIME flag on client proactively to avoid
	 * zeroing out the atime on old unpatched server. See LU-8041.
	 */
	if (rec->sa_atime == 0)
