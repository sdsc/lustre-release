...
static void mdc_adjust_dirpages(struct page **pages, int ncfspgs, int nlupgs)
{
	int i;
...
#else
#define mdc_adjust_dirpages(pages, ncfspgs, nlupgs) do {} while (0)
...
static int mdc_read_page(void *data, struct page *page0)
{
	...
	if (op_data->op_mea1 != NULL) {
		...
		lmo = &op_data->op_mea1->lsm_oinfo[op_data->op_stripe_offset];
		inode = lmo->lmo_root;
		fid = &lmo->lmo_fid;
	} else {
	...
	if (rc == 0) {
		...
		nrdpgs = (req->rq_bulk->bd_nob_transferred +
			    PAGE_CACHE_SIZE - 1) >> PAGE_CACHE_SHIFT;
		nlupgs = req->rq_bulk->bd_nob_transferred >>
							LU_PAGE_SHIFT;
		...
		mdc_adjust_dirpages(page_pool, nrdpgs, nlupgs);
			...
			struct md_callback *cb_op, struct page **ppage)
{
	...
	*ppage = NULL;
	...
	if (op_data->op_mea1 != NULL) {
		...
		dir = (struct inode *)op_data->op_data;
	}
	LASSERT(dir != NULL);
		   ...
		   struct md_callback *cb_op, struct lu_dirent **entp)
{
	...
	if (op_data->op_offset == MDS_DIR_END_OFF) {
		*entp = NULL;
		RETURN(0);
	}

	rc = mdc_readpage(exp, op_data, cb_op, &page);
	if (rc != 0)
		RETURN(rc);
	...
	for (ent = lu_dirent_start(dp); ent != NULL;
		...
		index++;
		if (ent->lde_hash > op_data->op_offset)
			break;
	}
	kunmap(page);
	
	if (ent == NULL) {
		__u64 orig_offset = op_data->op_offset;

		/* current page is finished, but still not find entry,
		 * try next page. */
		...
		mdc_release_page(page,
			         le32_to_cpu(dp->ldp_flags) & LDF_COLLIDE);
		op_data->op_offset = dp->ldp_hash_end;
		rc = mdc_readpage(exp, op_data, cb_op, &page);
		if (rc != 0)
			RETURN(rc);
		   ...
		   struct md_callback *cb_op, struct lu_dirent **entp)
{
	...
	dp = page_address(page);
	if (dp->ldp_hash_end < op_data->op_offset)
		GOTO(out, *entp = NULL);
