...
static int mdc_read_page_remote(void *data, struct page *page0)
{
	...
	if (rc == 0) {
		...
		body = req_capsule_server_get(&req->rq_pill, &RMF_MDT_BODY);
		/* Checked by mdc_read_page_remote() */
		LASSERT(body != NULL);
		if (body->valid & OBD_MD_FLSIZE) {
			if (op_data->op_mea1 != NULL) {
				...
				inode2 = (struct inode *)op_data->op_data;
				LASSERT(inode2 != NULL);
				rp->rp_cb->md_update_inode(inode2, body->size);
			}
		}
...
*mdc_read_page_remote(struct obd_export *exp, const struct lmv_oinfo *lmo,
		      const unsigned long index, struct obd_capa *oc)
{
			...
			struct page **ppage)
{
	...
	lmo = &op_data->op_mea1->lsm_oinfo[op_data->op_stripe_offset];
	page = mdc_read_page_remote(exp, lmo,
