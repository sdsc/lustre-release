                              ...
                              struct ptlrpc_request *req)
{
        ...
        if (body->valid & OBD_MD_FLRMTPERM) {
                ...
                perm = req_capsule_server_swab_get(pill, &RMF_PACKAGED_XATTR,
        ...
        *request = NULL;
        ...
        if (op_data->op_valid & OBD_MD_FLRMTPERM) {
                ...
                req_capsule_set_size(&req->rq_pill, &RMF_PACKAGED_XATTR,
                                     RCL_SERVER,sizeof(struct mdt_remote_perm));
        }
        ptlrpc_request_set_replen(req);
                      ...
                      struct lustre_md *md)
{
        ...
        if (body->valid & OBD_MD_FLRMTPERM) {
                ...
                if (!md->remote_perm)
                        GOTO(out, rc = -EPROTO);
        } else if (body->valid & OBD_MD_PACKAGED_XATTR &&
                   body->packaged_xattr > 0) {
                void *data = req_capsule_server_sized_get(pill,
                                                          &RMF_PACKAGED_XATTR,
                                                          body->packaged_xattr);
                int left = body->packaged_xattr;

                if (unlikely(data == NULL))
                        GOTO(out, rc = -EPROTO);

                while (left > 0) {
                        __u32 type = be32_to_cpu(*((__u32 *)data));
                        __s32 size = be32_to_cpu(*((__s32 *)data + 1));

                        data = (__u32 *)data + 2;
                        left -= 8;
                        switch (type) {
                        case PXT_ACL:
                                rc = mdc_unpack_acl(data, size, &md->posix_acl);
                                if (rc)
                                        GOTO(out, rc);
                                ...
                                break;
                        case PXT_DEFACL:
                                rc = mdc_unpack_acl(data, size, &md->def_acl);
                                if (rc)
                                        GOTO(out, rc);
                                ...
                                break;
                        default:
                                CWARN("Unknown packaged xattr type: %d\n",type);
                                GOTO(out, rc = -EINVAL);
                        }
        ...
        } else if (body->valid & OBD_MD_FLACL && body->packaged_xattr >= 0) {
                void *data;

                if (body->packaged_xattr == 0)
                        ...
                        rc = mdc_unpack_acl(data, body->packaged_xattr,
                                            &md->posix_acl);
                }
                if (rc)
                        GOTO(out, rc);
