...
static int remember_copytool(struct obd_uuid uuid, int archive_mask)
{
				 ...
				 struct hsm_action_list *hal)
{
	...
	/* TODO: swab hal if needed */
	if (hal->hal_magic == __swab32(HAL_MAGIC))
		lustre_swab_hal(hal);
	CDEBUG(D_HSM, " Received hsm_action_list l=%lu actions=%d on %s\n",
	       hal_size(hal), hal->hal_count, hal->hal_fsname);
			      ...
			      struct ptlrpc_request_set *set)
{
	...
	if (KEY_IS(KEY_READ_ONLY)) {
        ...
        if (KEY_IS(KEY_HSM_COPYTOOL_SEND)) {
                rc = mdc_hsm_copytool_send(imp->imp_obd, val);
                RETURN(rc);
        }
