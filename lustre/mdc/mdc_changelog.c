...
static long chlg_write(struct file *file, const char __user *buff, size_t count,
		       loff_t *off)
{
	struct chlg_reader_state	*crs = file->private_data;
	char				*kbuff;
...
int mdc_changelog_cdev_init(struct obd_device *obd)
{
	...
	CDEBUG(D_CONSOLE, "Registered %s for OBD device %p (%d)\n",
	       entry->ced_name, obd, entry->ced_misc.minor);
