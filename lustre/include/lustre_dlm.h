 ...
 * It's two main roles are:
 *   - to provide locking assuring consistency of data on all Lustre nodes
 *   - to assure a way for clients to cache state protected by a lock by not
 *     actually releasing the lock until a conflicting lock is requested.
 *
 * @{
 */
 ...
 *  Defines what lock modes are compatible with what other lock modes.
 */
...
struct ldlm_pool_ops {
	/** Recalculate pool \param pl usage */
	int (*po_recalc)(struct ldlm_pool *pl);
	/** Cancel at least \param nr locks from pool \pl */
...
/**
 * LDLM pools related, type of lock pool n the namespace.
 * Greedy means release cached locks agressively
 */
typedef enum {
...
struct ldlm_ns_bucket {
	 ...
	 * Used by AT code to avoid spurious client evictions due to
	 * unresponsiveness where in fact it is network or overall system load
	 * is at fault
	 */
...
struct ldlm_namespace {
	...
	*/
	 ...
	 * LRU lock list.
	 * Unused locks are locks with zero r/w refcounts. This list is only
	 * used on clients for lock caching purposes.
	 * When we want to release some locks voluntarily or if server want
	...
	/** Definition of how eagerly we need to release unused locks from LRU*/
	ldlm_appetite_t		ns_appetite;

	/**
	 * If more than \a ns_contended_locks found, the resource is considered
	 * to be contended. Lock enqueues might specify that no contended locks
	 * should be granted
	 */
...
/**
 * Returns 1 if namespace \param ns is a client namespace
 */
...
/**
 * Returns 1 if namespace \param ns is a server namespace
 */
 ...
 * LDLM lock structure
 *
 * Represents actual ldlm lock and its state in memory
 */
struct ldlm_lock {
	/**
	 * Local lock handle.
	...
	/**
	 * Lock completion handler pointer
	...
	/**
	 * Set when lock is sent blocking AST. Time in seconds when timeout
 ...
 * LDLM user is responsible for creation of a mapping between objects it wants
 * to be protected and resource names.
 *
 * A resource can only hold locks of a single lock type.
 */
struct ldlm_resource {
	...
	*/
	...
	/**
	 * Type of locks this resource can hold.
	 * Only one type per resource.
	 */
 ...
 * Also used during deciding of lock granting and cancelling.
 */
...
/**
 * Update LVBO on a resource taking into account data from reqest \param r
 */
...
/* ldlm_pool.c */
/** \defgroup ldlm_pools
 * Various ldlm pool related functions. There are not used outside of ldlm
 * @{
 */
