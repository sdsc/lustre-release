 ...
 */
static inline void layout_lock_set_locked(struct layout_lock *ll, __u64 cookie,
                                          ldlm_mode_t mode, int count)
{
        ENTRY;
...
/**
 * initialize a layout lock struct
 ...
 * \retval no return value
 */
static inline void layout_lock_set(struct layout_lock *ll, __u64 cookie,
                                   ldlm_mode_t mode, int count)
{
        cfs_mutex_down(&ll->ll_lh_sem);
        layout_lock_set_locked(ll, cookie, mode, count);
        cfs_mutex_up(&ll->ll_lh_sem);
}

/**
 * get a reference on a known layout lock and take a lock on it
 * \param struct layout_lock *ll layout lock
 * \retval 1 if lock already used
 * \retval 0 if lock is empty
 */
static inline int layout_lock_get_ref_if_used(struct layout_lock *ll)
{
        ENTRY;

        cfs_mutex_down(&ll->ll_lh_sem);
        if (lustre_handle_is_used(&ll->ll_lh)) {
                /* we already know the lock, we take a ref and inc the ref
                 * counter */
                ldlm_lock_addref(&ll->ll_lh, ll->ll_lmode);
                cfs_atomic_inc(&ll->ll_lrefcount);
                RETURN(1);
        }
        RETURN(0);
}
 ...
 *  \retval 0 success
 */
static inline int layout_lock_put(struct layout_lock *ll)
{
        ENTRY;
