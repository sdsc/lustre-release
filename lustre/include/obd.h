...
/**
 * Get a reference on a known layout lock. Caller must own ll_lh_sem
 *
 * \param struct layout_lock *ll layout lock
 * \retval 1 if lock already used
 * \retval 0 if lock is empty
 */
static inline int layout_lock_getref_if_used_locked(struct layout_lock *ll)
{
        if (!lustre_handle_is_used(&ll->ll_lh))
                return 0;
        /* we already know the lock, we take a ref and inc the ref counter */
 ...
 * \param struct layout_lock *ll layout lock
 */
static inline void layout_lock_put(struct layout_lock *ll)
{
        cfs_mutex_down(&ll->ll_lh_sem);
        if (lustre_handle_is_used(&ll->ll_lh)) {
