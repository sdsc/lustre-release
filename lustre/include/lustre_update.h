...
update_op_size(unsigned int params_count)
{
	return offsetof(struct update_op, uop_params_off) +
	       params_count * sizeof(__u16);
}
...
update_records_size(const struct update_records *record)
{
	...
	return offsetof(struct update_records, ur_ops) +
	       update_ops_size(&record->ur_ops, record->ur_update_count) +
