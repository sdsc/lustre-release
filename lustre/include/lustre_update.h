...
#define MAX_UPDATE_PARAMS		3
struct update_op {
	struct lu_fid uop_fid;
	__u16	uop_type;
	__u16	uop_params_off[MAX_UPDATE_PARAMS];
};

/* All of updates in the mulitple_update_record */
struct update_ops {
	__u32			uo_count;
	struct update_op	uo_op[0];
};

static inline int update_ops_size(const struct update_ops *ops)
{
	return sizeof(*ops) + ops->uo_count * sizeof(ops->uo_op[0]);
}
...
update_ops_get_op(const struct update_ops *ops, int index)
{
	if (index > ops->uo_count)
		return NULL;

	return (struct update_op *)&ops->uo_op[index];
}
...
int out_update_pack(const struct lu_env *env, struct object_update *update,
		    ...
		    int count, int *lens, const char **bufs, __u64 batchid);
...
int out_object_destroy_pack(const struct lu_env *env,
			    struct object_update *update, int max_update_length,
			    const struct lu_fid *fid, int cookie_size,
			    const void *cookie, __u64 batchid);
