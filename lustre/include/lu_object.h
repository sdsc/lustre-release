...
static inline int lu_object_is_immutable(const struct lu_object_header *h)
{
	return test_bit(LU_OBJECT_IMMUTABLE, &h->loh_flags);
}

static inline void lu_object_set_immutable(struct lu_object_header *h)
{
	set_bit(LU_OBJECT_IMMUTABLE, &h->loh_flags);
}

static inline void lu_object_clear_immutable(struct lu_object_header *h)
{
	clear_bit(LU_OBJECT_IMMUTABLE, &h->loh_flags);
}
