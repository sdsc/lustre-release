...
static inline void lu_attr_to_obdo(struct obdo *obdo, struct lu_attr *attr)
{
	...
	obdo->o_blkbits = attr->la_blkbits;
...
static inline void obdo_to_lu_attr(struct lu_attr *attr, struct obdo *obdo)
{
	...
	attr->la_blkbits = obdo->o_blkbits;
...
/**
 * Check whether object exists on stable storage.
 */
 ...
 */
static inline int lu_object_remote(const struct lu_object *o)
{
	return o->lo_header->loh_attr & LOHA_REMOTE;
}
