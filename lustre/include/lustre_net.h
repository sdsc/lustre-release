...
ptlrpc_nrs_req_can_move(struct ptlrpc_request *req)
{
	 ...
	 * ptlrpc_nrs_req_initialize() (e.g. performing a fiemap call for ORR
	 * and TRR policies); not checking this would mean
	 ...
	 * obd_export::exp_rpc_lock, we should avoid sleeping (we would have
	 * to, if the sleeping operation had not been performed during
	 * ptlrpc_nrs_req_initialize()). Perhaps this can be improved.
	 */
...
struct ptlrpc_service_part {
	...
	struct proc_dir_entry	       *scp_procroot;
