...
#define BULK_OP_ACTIVE	0x1
#define BULK_OP_PASSIVE 0x2
#define BULK_OP_PUT	0x4
#define BULK_OP_GET	0x8
#define BULK_BUF_IOVEC  0x10
#define BULK_BUF_KIOV	0x20

#define BULK_GET_SOURCE (BULK_OP_PASSIVE | BULK_OP_GET)
#define BULK_PUT_SINK   (BULK_OP_PASSIVE | BULK_OP_PUT)
#define BULK_GET_SINK   (BULK_OP_ACTIVE | BULK_OP_GET)
#define BULK_PUT_SOURCE (BULK_OP_ACTIVE | BULK_OP_PUT)

#define IS_BULK_GET_SOURCE(type)		\
  (((type & BULK_GET_SOURCE) == BULK_GET_SOURCE))
#define IS_BULK_PUT_SINK(type)		\
  (((type & BULK_PUT_SINK) == BULK_PUT_SINK))
#define IS_BULK_GET_SINK(type)		\
  (((type & BULK_GET_SINK) == BULK_GET_SINK))
#define IS_BULK_PUT_SOURCE(type)		\
  (((type & BULK_PUT_SOURCE) == BULK_PUT_SOURCE))
#define IS_BULK_DESC_IOVEC(type)		\
  (((type & BULK_BUF_IOVEC) | (type & BULK_BUF_KIOV)) == BULK_BUF_IOVEC)
#define IS_BULK_DESC_KIOV(type)			\
  (((type & BULK_BUF_IOVEC) | (type & BULK_BUF_KIOV)) == BULK_BUF_KIOV)

#define IS_BULK_OP_ACTIVE(type)			\
  (((type & BULK_OP_ACTIVE) | (type & BULK_OP_PASSIVE)) == BULK_OP_ACTIVE)
#define IS_BULK_OP_PASSIVE(type)		\
  (((type & BULK_OP_ACTIVE) | (type & BULK_OP_PASSIVE)) == BULK_OP_PASSIVE)
