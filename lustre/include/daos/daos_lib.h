...
typedef struct {
	/** called with eq_lock */
	int (*op_abort)(void *kparam, void *uparam, int unlinked);
	/** called w/o eq_lock */
	int (*op_complete)(void *kparam, void *uparam, int error, int unlinked);
} daos_kevent_ops_t;

struct daos_kevent;

void daos_event_complete(struct daos_kevent *ev, int error);
int daos_event_dispatch(daos_event_id_t eid, void *uparam, void *kparam,
			daos_kevent_ops_t *ops, struct daos_kevent **evpp);
