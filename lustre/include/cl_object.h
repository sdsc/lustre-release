...
struct cl_layout {
	...
	bool		cl_is_released;
	/** size of layout. If cl_buf is not big enough, it won't
	 * copy the layout but just return the size. */
	size_t          cl_size;
	/** the buffer to return the layout in lov_mds_md format.
	 * To support getxattr(), the layout generation in lov_mds_md is
	 * cleared so the caller should use cl_generation instead. */
