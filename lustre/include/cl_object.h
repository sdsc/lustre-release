...
/**
 * DIO data propagated between layers.
 */
struct cl_dio_data {
	enum cl_req_type        cdd_cmd;
	struct page	      **cdd_pages;
	size_t                  cdd_size;
	loff_t                  cdd_offset; /* offset in file */
	int			cdd_pgoff;  /* offset in first page */
	int                     cdd_page_count;
	struct obdo             cdd_oa;
...
struct cl_io_operations {
	 ...
	 */
	int (*cio_is_cacheable)(const struct lu_env *env,
				const struct cl_io_slice *slice);
        /**
