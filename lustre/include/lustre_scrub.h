...
/* Return value for ::next() call. Negative value for non-fatal error. */
...
/* Arguments from MDD layer to OSD layer for scrub initialization. */
struct dt_scrub_args {
        ...
        __u16   dsa_param_flags;

        /* Param: interval time (seconds) between two checkpoints. */
        __u16   dsa_param_checkpoint_interval;
};
