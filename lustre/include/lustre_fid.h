 ...
 *
 * FID
 *   File IDentifier generated by client from range allocated by the SEQ
 *   service. First 0x400 sequences [2^33, 2^33 + 0x400] are reserved for
 *   system use. Note that on ldiskfs MDTs that IGIF FIDs can use inode
 *   numbers starting at 12, but this is in the IGIF SEQ rangeand does not
 *   conflict with assigned FIDs.
 *
 * IGIF
 ...
 *   used on MDT0 in a CMD filesystem, because there are not expected to be
 *   any OLD formatted CMD filesystems. Belongs to sequence in [12, 2^32 - 1]
 *   range, where sequence number is inode number, and inode generation is
 *   used as OID.
 *   NOTE: This assumes no more than 232-1 inodes exist in the MDT filesystem,
 *   which is the maximum possible for an ldiskfs backend. NOTE: This assumes
 *   that the reserved ext3/ext4/ldiskfs inode numbers [0-11] are never
 *   visible to clients, which has always been true.
 *
 * IDIF
 ...
 *      objid & 0xffffffff
 *
 *   that is, it consists of lower 32 bits of object ID.
 *   NOTE: This assumes that no more than 248-1 objects have ever been created
 *   on an OST, and that no more than 65535 OSTs are in use.  Both are very
 *   reasonable assumptions (can uniquely map all objects on an OST that
 *   created 1M objects per second for 9 years, or combinations thereof).
 *
 * OST_MDT0
 *   Surrogate FID used to identify an existing object on OLD formatted OST
 *   filesystem. Belongs to the reserved sequence 0, and is used internally
 *   prior to the introduction of FID-on-OST, at which point IDIF will be
 *   used to identify objects as residing on a specific OST.
 *
 * ECHO
 *   For testing OST IO performance the object sequence 1 is used. This is
 *   compatible with both OLD and NEW.1 namespaces, as this SEQ number is in
 *   the ext3/ldiskfs reserved inode range and does not conflict with IGIF
 *   sequence numbers.
 *
 * LLOG
 *   For Lustre Log objects the object sequence 2 is used. This is compatible
 *   with both OLD and NEW.1 namespaces, as this SEQ number is in the
 ...
 *   production CMD release, as the objects in this range conflict across all
 ...
 *   FID must map onto the o_id and o_gr in a manner that ensures existing
 *   objects are identified consistently for IO, as well as onto the lock
 *   namespace to ensure both IDIFs map onto the same objects for IO as well
 *   as resources in the DLM.
 *
 * DLM OLD OBIF/IDIF:
 *
 ...
 *  larger than the 233 reserved sequence numbers for IDIF, so it is possible
