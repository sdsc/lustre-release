...
{
	if(!fid_seq_is_mdt0(ostid_seq(ostid))) {
		CERROR("Invalid ostid "DOSTID"\n", POSTID(ostid));
		return;
	}
	fid->f_seq = fid_idif_seq(ostid_id(ostid), ost_idx);
	fid->f_oid = ostid_id(ostid);    /* truncate to 32 bits by assignment */
	fid->f_ver = ostid_id(ostid) >> 48; /* in theory, not currently used */
}
...
{
	if (ostid->oi_fid.f_ver != 0 || fid_seq_is_mdt0(ostid_seq(ostid))) {
		CERROR("bad ostid, "DOSTID"\n", POSTID(ostid));
		return;
	}
	*fid = ostid->oi_fid;
}
