...
struct mgs_nidtbl_entry {
        ...
        __u8            mne_bufcnt;     /* # of __u64 buffer for NIDs */
        __u64           mne_nids[0];    /* variable size buffer for NIDs. */
};
extern void lustre_swab_mgs_nidtbl_entry(struct mgs_nidtbl_entry *oinfo);

struct mgs_config_body {
        char     mcb_name[MTI_NAME_MAXLEN]; /* logname */
        __u64    mcb_offset;    /* index of config log */
        __u16    mcb_type;      /* type of log: CONFIG_T_[CONFIG|RECOVER] */
        __u16    mcb_bits;      /* bits unit size of config log */
