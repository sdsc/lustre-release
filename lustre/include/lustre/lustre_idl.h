...
static inline int lmv_mds_md_stripe_count_get(const union lmv_mds_md *lmm)
{
	...
	case LMV_MAGIC_V1:
		return lmm->lmv_md_v1.lmv_stripe_count;
	case LMV_USER_MAGIC:
		return lmm->lmv_user_md.lum_stripe_count;	
	default:
		return -EINVAL;
	}
}
					      ...
					      unsigned int stripe_count)
{
	switch (lmm->lmv_magic) {
	case LMV_MAGIC_V1:
		lmm->lmv_md_v1.lmv_stripe_count = stripe_count;
		break;
	case LMV_USER_MAGIC:
		lmm->lmv_user_md.lum_stripe_count = stripe_count;
		break;
	default:
		return -EINVAL;
	}
