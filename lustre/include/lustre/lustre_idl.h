...
typedef enum {
	OBJECT_UPDATE	= 1000,
	OBJECT_UPDATE_LAST_OPC
} update_cmd_t;

#define OBJECT_UPDATE_FIRST_OPC    OBJECT_UPDATE
...
/* All versions of lmv_mds_md_vx should start with lmv_mds_md_common */
struct lmv_mds_md_common {
	...
	__u32 lmv_padding;		/* Used for directory restriping */
	char lmv_pool_name[LOV_MAXPOOLNAME];	/* pool name */
};

struct lmv_mds_md_v1 {
	struct lmv_mds_md_common lmv_common;
	struct lu_fid lmv_data[0];	     /* FIDs for each stripe */
};

union lmv_mds_md {
	__u32			 lmv_magic;
	struct lmv_mds_md_common lmv_md_common;
...
static inline int lmv_mds_md_size(int stripes, int lmm_magic)
{
	switch (lmm_magic) {
	case LMV_MAGIC_V1:
		return sizeof(struct lmv_mds_md_v1) + stripes *
 ...
 */
#define	OBJECT_UPDATE_PER_RPC_MAX	10

/**
 * Maximum number of buffers per update
 */
#define	OBJECT_UPDATE_PARAMS_MAX	8
