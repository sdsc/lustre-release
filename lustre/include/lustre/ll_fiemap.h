...
struct ll_fiemap_extent {
	uint64_t fe_logical;  /* logical offset in bytes for the start of the
				  * extent from the beginning of the file */
	uint64_t fe_physical; /* physical offset in bytes for the start of
				  * the extent from the beginning of the disk */
	uint64_t fe_length;   /* length in bytes for this extent */
	uint64_t fe_reserved64[2];
	uint32_t fe_flags;    /* FIEMAP_EXTENT_* flags for this extent */
	uint32_t fe_device;   /* device number for this extent */
	uint32_t fe_reserved[2];
};
