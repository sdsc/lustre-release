...
extern int llapi_obd_statfs(char *path, uint32_t type, uint32_t index,
...
extern int llapi_target_iterate(int type_num, char **obd_type, void *args,
				llapi_cb_t cb);
extern int llapi_get_connect_flags(const char *mnt, uint64_t *flags);
...
extern int llapi_get_data_version(int fd, uint64_t *data_version,
				  uint64_t flags);
...
extern int llapi_hsm_state_set_fd(int fd, uint64_t setmask,
				  uint64_t clearmask,
				  uint32_t archive_id);
extern int llapi_hsm_state_set(const char *path, uint64_t setmask,
			       uint64_t clearmask,
			       uint32_t archive_id);
