...
/**
 * Read the generation number of \a layout.
 * The generation number is a read-only attribute that can be used to
 * detect if the layout of a file has changed.  This attribute is only
 * meaningful if \a layout was initialized with
 * llapi_layout_lookup_by{path,fd,fid}(), otherwise the return value
 * will be -1 and errno will be 0.
 *
 * \retval 0+ The generation number
 * \retval -1 Error with status code in errno, except as noted above.
 */
