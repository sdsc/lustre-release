...
#define __OBD_MALLOC(ptr, cptab, cpt, size, gfp_mask)                         \
do {                                                                          \
        unsigned __flags = ((gfp_mask) | CFS_ALLOC_ZERO);                     \
                                                                              \
        (ptr) = (cptab) == NULL ?                                             \
                cfs_malloc(size, __flags) :                                   \
...
#define OBD_ALLOC_GFP(ptr, size, gfp_mask)                                    \
        __OBD_MALLOC(ptr, NULL, 0, size, gfp_mask)

#define OBD_CPT_ALLOC_GFP(cptab, cpt, ptr, size, gfp_mask)                    \
        __OBD_MALLOC(ptr, cptab, cpt, size, gfp_mask)

#define OBD_ALLOC(ptr, size)                            \
        OBD_ALLOC_GFP(ptr, size, CFS_ALLOC_STD)
#define OBD_CPT_ALLOC(ptr, cptab, cpt, size)            \
        OBD_CPT_ALLOC_GFP(ptr, cptab, cpt, size, CFS_ALLOC_STD)
...
do {                                                                          \
        (ptr) = cptab == NULL ?                                               \
                cfs_vmalloc(size, CFS_ALLOC_ZERO) :                           \
                cfs_cpt_vmalloc(cptab, cpt, size, CFS_ALLOC_ZERO);            \
        if (unlikely((ptr) == NULL)) {                                        \
...
#define __OBD_SLAB_ALLOC(ptr, slab, cptab, cpt, type, size)                   \
do {                                                                          \
        unsigned __flags = (type | CFS_ALLOC_ZERO);                           \
                                                                              \
        LASSERT(ergo((type & CFS_ALLOC_ATOMIC) == 0,                          \
                      !cfs_in_interrupt()));                                  \
        (ptr) = (cptab) == NULL ?                                             \
                cfs_mem_cache_alloc(slab, size, __flags) :                    \
                cfs_mem_cache_cpt_alloc(slab, cptab, cpt, size, __flags);     \
        if (likely((ptr) != NULL &&                                           \
                   (!HAS_FAIL_ALLOC_FLAG || obd_alloc_fail_rate == 0 ||       \
...
#define OBD_SLAB_ALLOC_GFP(ptr, slab, type, size)                             \
        __OBD_SLAB_ALLOC(ptr, slab, NULL, 0, type, size)
#define OBD_CPT_SLAB_ALLOC_GFP(ptr, slab, cptab, cpt, type, size)             \
        __OBD_SLAB_ALLOC(ptr, slab, cptab, cpt, type, size)
...
#define OBD_SLAB_CPT_ALLOC(ptr, slab, cptab, cpt, size)                       \
        OBD_SLAB_CPT_ALLOC_GFP(ptr, slab, cptab, cpt, CFS_ALLOC_STD , size)
...
#define OBD_SLAB_CPT_ALLOC_PTR_GFP(ptr, slab, ctab, cpt, gfp)                 \
        OBD_SLAB_CPT_ALLOC_GFP(ptr, slab, cptab, cpt, gfp, sizeof *(ptr))
...
#define OBD_PAGE_CPT_ALLOC(ptr, cptab, cpt, gfp_mask)           \
        __OBD_PAGE_CPT_ALLOC(ptr, cptab, cpt, gfp_mask)
