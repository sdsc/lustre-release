...
do {									      \
	u_int32_t __flags;						      \
	__flags = ((flags) & GFP_ATOMIC) ?			      \
		  (flags) : ((flags)|__GFP_WAIT);			      \
	(ptr) = (cptab) == NULL ?					      \
		kmalloc(size, __flags) :				      \
...
do {									      \
	u_int32_t __type;						      \
	__type = ((type)&GFP_ATOMIC) ?					      \
		  (type) : ((type)|__GFP_WAIT);				      \
	LASSERT(ergo(__type != GFP_ATOMIC, !cfs_in_interrupt()));	      \
	(ptr) = (cptab) == NULL ?					      \
...
do {									      \
	(ptr) = (cptab) == NULL ?					      \
		alloc_page(((gfp_mask) & GFP_ATOMIC) ?	      \
				(gfp_mask) : ((gfp_mask)|__GFP_WAIT)) :   \
		cfs_page_cpt_alloc(cptab, cpt, ((gfp_mask)&GFP_ATOMIC) ?\
				(gfp_mask) : ((gfp_mask)|__GFP_WAIT));    \
        if (unlikely((ptr) == NULL)) {                                        \
