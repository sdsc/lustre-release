...
#define __OBD_MALLOC(ptr, cptab, cpt, size, flags)			      \
do {									      \
	(ptr) = (cptab) == NULL ?					      \
		cfs_malloc(size, flags) :				      \
...
#define OBD_ALLOC(ptr, size)						      \
	OBD_ALLOC_GFP(ptr, size, CFS_ALLOC_IO | CFS_ALLOC_ZERO)
#define OBD_CPT_ALLOC(ptr, cptab, cpt, size)				      \
	OBD_CPT_ALLOC_GFP(ptr, cptab, cpt, size,			      \
...
#define __OBD_SLAB_ALLOC(ptr, slab, cptab, cpt, size, flags)		      \
do {									      \
	LASSERT(ergo(((flags) & CFS_ALLOC_ATOMIC) == 0,			      \
		      !cfs_in_interrupt()));				      \
	(ptr) = (cptab) == NULL ?					      \
		cfs_mem_cache_alloc(slab, size, flags) :		      \
		cfs_mem_cache_cpt_alloc(slab, cptab, cpt, size, flags);	      \
	if (likely((ptr) != NULL &&					      \
		   (!HAS_FAIL_ALLOC_FLAG || obd_alloc_fail_rate == 0 ||	      \
...
#define OBD_SLAB_ALLOC_GFP(ptr, slab, size, flags)			      \
	__OBD_SLAB_ALLOC(ptr, slab, NULL, 0, size, flags)
#define OBD_CPT_SLAB_ALLOC_GFP(ptr, slab, cptab, cpt, size, flags)	      \
	__OBD_SLAB_ALLOC(ptr, slab, cptab, cpt, size, flags)
...
#define OBD_SLAB_ALLOC(ptr, slab, size)					      \
	OBD_SLAB_ALLOC_GFP(ptr, slab, size, CFS_ALLOC_IO | CFS_ALLOC_ZERO)
#define OBD_SLAB_CPT_ALLOC(ptr, slab, cptab, cpt, size)			      \
	OBD_SLAB_CPT_ALLOC_GFP(ptr, slab, cptab, cpt, size,		      \
			       CFS_ALLOC_IO | CFS_ALLOC_ZERO)
...
#define OBD_PAGE_CPT_ALLOC(ptr, cptab, cpt, gfp_mask)			      \
	__OBD_PAGE_ALLOC(ptr, cptab, cpt, gfp_mask)
