...
struct dt_device_operations {
	 ...
	 * persistently. I.e. the transaction will survive an event like power
	 * off if the callback did run. The number of callbacks isn't limited,
	 * but you should note that some disk filesystems do handle the commit
	 * callbacks in the thread hadling commit/flush of all the transactions,
	 * meaning that new transactions are blocked from commit and flush until
	 * all the callbacks are done. Also, note few callbacks can be running
	 * concurrently using few CPU cores. The callbacks will be running in a
	 * special environment which can not be used to pass data around.
	 *
	 * \param[in] th	transaction handle
	 *
	 * \retval		0 on success, negative error code if failed
