...
struct dt_device_operations {
	 ...
	 *
	 * \retval		0 on success
	 * \retval		negative errno if fails
         */
	 ...
	 * hadling commit/flush of all the transactions, meaning that new
...
struct dt_object_operations {
	 ...
	 * with ->do_attr_get() soon. This allows OSD to implement prefetching
	 * logic in an object-oriented manner. The implementation can be noop.
	 * This method should avoid expensive delays such as waiting on disk
	 * I/O, otherwise the goal of enabling a performance optimization would
	 * be defeated.
	 *
	 * \param[in] env	execution environment for this thread
	 * \param[in] dt	object
	 ...
	 * This enables the layer below to prepare resources (e.g. journal
	 * credits in ext4).  This method should be called between creating
	 * the transaction and starting it.
	 * Note that the la_valid field of \a attr specifies which attributes
	 * will change.
	 ...
	 * with ->do_xattr_get() soon. This allows OSD to implement prefetching
	 * logic in an object-oriented manner. The implementation can be noop.
	 * This method should avoid expensive delays such as waiting on disk
	 * I/O, otherwise the goal of enabling a performance optimization would
	 * be defeated.
	 ...
	 * \param[in] env	execution environment for this thread
	 * \param[in] dt	object
	 * \param[in] buf	buffer in which to store the value
	 * \param[in] name	name of the extended attribute
	 * \param[in] capa	[not used]
	 *
	 * \retval		0 on success
	 * \retva		-ERANGE if \a buf is too small
	 * \retval		negative errno if fails
	 * \retval		value's size if \a buf is NULL or has zero size
	 */
	 ...
	 *
	 * Notify the underlying filesystem that the extended attribute may
	 * be deleted in this transaction.
	 * This enables the layer below to prepare resources (e.g. journal
	 * credits in ext4).  This method should be called between creating
	 * the transaction and starting it.
	 * The object need not exist.
	 ...
	 * then the method should reserve an object for the given
	 * credentials and return an error if quota is over. If object
	 * creation later fails for some reason, then the reservation should
	 ...
	 * in the LU-object header attributes. If the layer implementing this
	 * method is responsible for quota, then the method should maintain
	 * object accounting for the given credentials.
	 *
	 * \param[in] env	execution environment for this thread
	 * \param[in] dt	object
	 * \param[in] attr	attributes of the new object
	 ...
	 * be changed in this transaction.
	 * This enables the layer below to prepare resources (e.g. journal
	 * credits in ext4).  This method should be called between creating
	 * the transaction and starting it.
	 * The object need not exist.
	 ...
	 * ->do_sync() to maintain overall consisency, in which case it's
...
struct dt_body_operations {
	 ...
	 * Note: the method doesn't check whether output array is large enough.
	 *
	 * \param[in] env	execution environment for this thread
	 * \param[in] dt	object
	 * \param[in] pos	position in the object to start
	 ...
	 * \param[in] env	execution environment for this thread
	 * \param[in] dt	object
	 * \param[in] lb	array of descriptors to fill data in
	 * \param[in] nr	size of the array
	 *
	 * \retval		0 on success
	 * \retval		negative errno if fails
	 ...
	 *
	 * Notify the underlying filesystem that space may be deallocated
...
struct dt_index_operations {
	 ...
	 *
	 * Notify the underlying filesystem that new key/value may be
	 * inserted in this transaction.
	 * This enables the layer below to prepare resources (e.g. journal
	 * credits in ext4).  This method should be called between creating
	 * the transaction and starting it.
	 * key/value format and size is subject to ->do_index_try().
	 *
	 * \param[in] env	execution environment for this thread
	 * \param[in] dt	object
	 ...
	 *
	 * Notify the underlying filesystem that key/value may be deleted
	 * in this transaction.
	 * This enables the layer below to prepare resources (e.g. journal
	 * credits in ext4).  This method should be called between creating
	 * the transaction and starting it.
	 ...
	 * associated values, get key/value size, etc.
		 ...
		 * \param[in] capa	not used
		 *
		 * \retval 		iterator pointer on success
		 * \retval		ERR_PTR(errno) if fails
                 */
		 ...
		 * \param[in] di	iterator
		 *
		 * \retval		pointer to the key on success
		 * \retval		ERR_PTR(errno) if fails
		 */
		 ...
		 * \param[in] di	iterator
		 *
		 * \retval		size of the key on success
		 * \retval		negative errno if fails
		 */
		 ...
		 * buffer must be big enough(as negotiated with ->do_index_try()
		 * or ->rec_size()). The caller can specify she is interested
		 * only in part of the record, using attr argument (see LUDA_*
		 ...
		 * Return record size.
		 *
		 * Returns size of the record at the current position. \a attr
		 * can be used to specify only the parts of the record need to
		 * be returned. (see LUDA_* definitions for the details).
		 *
		 * \param[in] env	execution environment for this thread
		 * \param[in] di	iterator
		 * \param[in] attr	part of the record to return
		 *
		 * \retval 		size of the record on success
		 * \retval		negative errno if fails
		 */
		 ...
		 *
		 * \retval		posizve if current position points to
