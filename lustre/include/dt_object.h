...
struct update_request {
	...
	cfs_spinlock_t		ur_cb_list_lock;
	cfs_list_t		ur_list;
	int			ur_flags;
	int			ur_rc;
				     ...
				     int *size)
{
	...
	ptr = (char *)update + cfs_size_round(
		     offsetof(struct update, u_bufs[0]));
	for (i = 0; i < index; i++) {
...
struct thandle {
	...
	/* In DNE, one transaction can be dis-assemblied into
