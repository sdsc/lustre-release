...
struct dt_device_operations {
	 ...
	 * ->dt_trans_start() and updates or not) so that a \thandle and other
	 ...
	 * will be called when the transaction it registered to is stored
	 * persistently. I.e. the transaction will survive an event like a regular
	 * power-off case if the callback did run. The number of callbacks isn't
	 * artifically limited, but you should note that some disk filesystems do
	 * handle the commit callbacks in the thread hadling commit/flush of all
	 * the transactions, meaning that new transactions are blocked from commit
	 * and flush until all the callbacks are done. Also, note few callbacks
	 * can be running concurrently using few CPU cores. The callbacks will be
	 * running in a special environment which can not be used to pass data
	 * around.
	 *
	 * \param[in] th	transaction handle
	 *
...
/**
 * A dt-object provides with the common operations to create/destroy,
...
struct dt_object_operations {
	 ...
	 * resources (e.g. journal credits in ext4). Note la_valid specify
	 ...
	 * prepare resources (e.g. journal credits in ext4).
	 ...
	 * (e.g. journal credits in ext4) for this change.
	 ...
	 * object wasn't created for a reason, then the reservation should
	 * be released properly (usually in ->dt_trans_stop()).
	 *
	 * \param[in] env	lu environment provided by the caller
	 ...
	 * The method should be called between ->do_trans_create() and
	 * ->do_trans_start() so the layers below can make reservations
	 * (e.g. journal credits in ext4) to destroy the object passed.
	 * There is no requirement for the object to exist.
	 *
	 * \param[in] env	lu environment provided by the caller
	 * \param[in] dt	object
	 ...
	 * The method should be called between ->do_trans_create() and
	 * ->do_trans_start() so the layers below can make reservations
	 * (e.g. journal credits in ext4) to increment nlink count.
	 *
	 * \param[in] env	lu environment provided by the caller
	 * \param[in] dt	object
	 *
	 ...
	 * The method should be called between ->do_trans_create() and
	 * ->do_trans_start() so the layers below can make reservations
	 * (e.g. journal credits in ext4) to decrement nlink count.
	 *
	 * \param[in] env	lu environment provided by the caller
	 * \param[in] dt	object
	 *
	 ...
	 * if the object isn't clean, then many disk filesystems will be doing
	 * ->do_sync() to maintain overall consisency, so it's still very
	 * expensive.
	 *
	 * \param[in] env	lu environment provided by the caller
	 * \param[in] dt	object
...
struct dt_body_operations {
	 ...
	 * (e.g. journal credits in ext4) to do write operation. The
	 * object is not supposed to exist.
	 * If the layer implementing this method is responsbile for quota,
	 * then the method should reserve space for the given credentials
	 * and return an error if quota is over. If later the write wasn't
	 * done for a reason, then the reserve should be released properly
	 ...
	 * other components (e.g. networking) can transfer data from or to the
	 ...
	 * The caller should specify if the buffers are used to read or modify
	 ...
	 * Note: the method doesn't check whether output array is enough.
	 *
	 * \param[in] env	LU environment provided by the caller
	 * \param[in] dt	object
	 * \param[in] pos	position in the object to start
	 ...
	 * all the resources (e.g. journal credits in ext4) to complete
	 * the operation.
	 * If the layer implementing this method is responsbile for quota,
	 ...
	 * in lazy manner, when actual allocation happens.
	 * If the layer implementing this method is responsible for quota,
	 * then the method should maintain space accounting for the given
	 * credentials.
	 *
	 ...
	 *
	 * This method should be called between ->do_trans_create() and
	 * ->do_trans_start() to let OSD prepare the resources (e.g. journal
...
struct dt_index_operations {
	 ...
	 * Thus it's the caller responsibility to provide the method with
	 ...
	 * or below. The object should should have been created with type
	 * DFT_INDEX or DFT_DIR.
	 *
	 * \param[in] env	LU environment provided by the caller
	 * \param[in] dt	object
	 ...
	 * \retval		another negative if failed
	 */
	 ...
	 *
	 * This method should be called between ->dt_trans_create() and
	 * ->dt_trans_start() to let the layers below prepare the resourses
	 * required. key/value format and size is subject to ->do_index_try().
	 *
	 * \param[in] env	LU environment provided by the caller
	 * \param[in] dt	object
	 ...
	 * have been negotiated before using ->do_index_try(), no additonal
	 ...
	 * \param[in] handle	transaction handle
	 * \param[in] capa	not used
	 * \param[in] ignore_quota	not used
	 *
	 * \retval		0 on success, negative error code if failed
	 */
	 ...
	 * additonal information can be specified to the method.
	 *
	 * \param[in] env	LU environment provided by the caller
	 * \param[in] dt	object
	 * \param[in] key	key
	 ...
	 */
         ...
         * Iterator interface
	 *
	 * Set of the methods to iterate over an existing index, list the
	 * keys stored and associated values, get key/value size, etc.
         */
        struct dt_it_ops {
		/**
		 ...
		 * \param[in] capa	not used
		 *
		 * \retval iterator ptr	on success
		 * \retval ERR_PTR() if failed
                 */
		 ...
		 * \retval positive	at the record with least key not larger
		 *			than the key
		 ...
		 * \param[in] di	iterator
		 *
		 * \retval positive	no more records
		 * \retval 0		success, the next record is found
		 * \retval negative	failed, error code
		 */
		 ...
		 * retain data until ->get(), ->load() and ->fini() method
		 * is called.
		 *
		 * \param[in] env	LU environment provided by the caller
		 * \param[in] di	iterator
		 *
		 * \retval ptr		pointer to the key
		 * \retval ERR_PTR()	if failed
		 */
		 ...
		 * \param[in] di	iterator
		 *
		 * \retval positive	size of the key
		 * \retval negative	an error code if failed
		 */
		 ...
		 * buffer is supposed to be big enough (as negotiated with
		 * ->do_index_try() or ->rec_size()). The caller can specify she
		 * is interested only in part of the record, using attr argument
		 * (see LUDA_* definitions for the details).
		 * XXX: it looks like the only actual user for @attr is LFSCK.
		 *	given upgrade restrictions, can we get rid of this in
		 *	the next version?
		 *
		 * \param[in] env	LU environment provided by the caller
		 * \param[in] di	iterator
		 ...
		 *
		 * \retval		0 on success, <0 error code if failed
		 */
		...
		/**
		 * Return a record size
		 *
		 * Returns size of the record at the current position.
		 *
		 * \param[in] env	LU environment provided by the caller
		 * \param[in] di	iterator
		 * \param[in] attr	interesting part of the value
		 *
		 * \retval positive	size of the record
		 * \retval negative	an error code if failed
		 */
		...
		/**
		 * Initialize a position using cookie/hash
		 *
		 ...
		 *
		 * \retval >0		current position points to record with
		 *			least cookie not larger than cookie
		 * \retval =0		current position matches cookie
		 * \retval <0		error code if failed
		 */
