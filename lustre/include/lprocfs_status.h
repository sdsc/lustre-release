...
struct lprocfs_counter {
	 ...
	 * LPROCFS_STAT_FLAGS_SOFTIRQ_SAFE flag, its counter need
	 * lc_array_sum[1]
	 */
...
enum lprocfs_stats_flags {
	...
	/* stats have no percpu area and need locking */
	LPROCFS_STATS_FLAG_NOPERCPU	= 0x0001,
	/* stats need alloc its all percpu data at the beginning, so that
	 * no memory allocation happens in irq context */
	LPROCFS_STATS_FLAG_SOFTIRQ_SAFE	= 0x0010,
};
...
lprocfs_stats_counter_size(struct lprocfs_stats *stats)
{
	...
	/* softirq safe stats need lc_array_sum[1] */
