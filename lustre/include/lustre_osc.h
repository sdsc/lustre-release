...
struct osc_object {
	...
	const struct osc_type_operations *oo_type_ops;
	bool			oo_initialized;
};
...
static inline void osc_build_res_name(struct osc_object *osc,
				      struct ldlm_res_id *resname)
{
	return osc->oo_type_ops->oto_build_res_name(osc, resname);
}
...
{
	return obj->oo_type_ops->oto_dlmlock_at_pgoff(env, obj, index, flags);
}
