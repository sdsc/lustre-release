...
#define OBD_COUNTER_INCREMENT(obd, op)					       \
	do {								       \
		unsigned int _off;					       \
		_off = (obd)->obd_cntr_base + OBD_COUNTER_OFFSET(op);	       \
		lprocfs_counter_incr((obd)->obd_stats, _off);		       \
	} while (0)
...
#define MD_COUNTER_OFFSET(op)						       \
	((offsetof(struct md_ops, m_ ## op) -				       \
	 ...
	 sizeof(((struct md_ops *)NULL)->m_getstatus))

#define NUM_MD_STATS							       \
	(sizeof(struct md_ops) / sizeof(((struct md_ops *)NULL)->m_getstatus))

#define EXP_MD_COUNTER_INCREMENT(exp, op)				       \
	do {								       \
		unsigned int _off;					       \
		_off = (exp)->exp_obd->md_cntr_base + MD_COUNTER_OFFSET(op);   \
