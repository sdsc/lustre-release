...
static inline int exp_connect_lvb_type(struct obd_export *exp)
{
	LASSERT(exp != NULL);
	return !!(exp->exp_connect_flags & OBD_CONNECT_LVB_TYPE);
}

static inline int imp_connect_lvb_type(struct obd_import *imp)
{
	struct obd_connect_data *ocd;
