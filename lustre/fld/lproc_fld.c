...
static void *fldb_seq_start(struct seq_file *p, loff_t *pos)
{
	...
	/* XXX zfs osd_index_it_get() will LASSERT() on non zero keys. */
	if (*pos != 0)
		return NULL;
	...
	rc = lu_env_init(&fsp->fsp_env, LCT_MD_THREAD);
	if (rc != 0)
		GOTO(out_fsp, rc);

	iops = &obj->do_index_ops->dio_it;
	fsp->fsp_it = iops->init(&fsp->fsp_env, obj, 0, NULL);
	if (IS_ERR(fsp->fsp_it))
		GOTO(out_env, rc = PTR_ERR(fsp->fsp_it));
