...
int fldname_to_index(char *name, int *index)
{
	...
	dash = name + strlen(name);
	for (; dash > name && *dash != '-'; dash--)
		;
	if (dash == name)
		RETURN(-EINVAL);
			       ...
			       struct lu_server_fld *fld)
{
	...
	/* Update only happenes during initalization, i.e. local FLDB
	 * does not exist yet */
	if (!fld->lsf_new)
		RETURN(0);
...
static int fld_handle_read(struct tgt_session_info *tsi)
{
	...
	req_capsule_extend(tsi->tsi_pill, &RQF_FLD_READ);
...
TGT_FLD_HDL_VAR(0,	FLD_QUERY,	fld_handler),
};
