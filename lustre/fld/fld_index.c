 ...
 *      \retval  -ve error
 */
int fld_index_create(struct lu_server_fld *fld, const struct lu_env *env,
		     const struct lu_seq_range *new, struct thandle *th)
{
	...
	if (delete)
		fld_cache_delete_nolock(fld->lsf_cache, range);
	rc = fld_cache_insert_nolock(fld->lsf_cache, new);
...
int fld_index_init(struct lu_server_fld *fld, const struct lu_env *env,
		   struct dt_device *dt)
{
