 ...
 *    that a master LMV EA claims as a slave LMV EA by wrong, so we can ignore
 *    such race case and the reverse case.
 *
 ...
 *    compared with the name in the linkEA. Although both the linkEA and the
 *    LMV EA can be wrong, but it is almost impossible that they hit the same
 *    data corruption. So if they match each other, then trust them. Otherwise,
 *    if they does not match or the linkEA is lost, then need to check with the
 *    name entry on the master.
 *
 ...
 *	  wrong hash map, then it will change the master hash type to UNKNOWN
 *	  as described above. The worst case is euqal to the case without LFSCK.
 *
 * 3.2) If the master hash hash is invalid, and nor LMV_HASH_TYPE_UNKNOWN, then
 *	trust the first shard with valid hash type. If the shard is worng also,
 *	means there are double failures, then as the LFSCK processing, other
 *	LFSCK instances on the other MDTs may find bad hash map, and then, the
 *	master hash type will be changed to UNKNOWN as described in the 3).
 *
 * 3.3) If the master name hash is LMV_HASH_TYPE_UNKNOWN, then it is possible
 *	that some other LFSCK instances on other MDT found bad hash map, then
 *	changed the master hash type to UNKNOWN as described in the 3). But it
 *	also maybe because of data corruption in master LMV EA. To make such
 *	two cases to be distinguishable, when the LFSCK changes the master
 *	hash type to LMV_HASH_TYPE_UNKNOWN, it will mark in the master LMV EA
 *	(new lmv flags). The the subsequent LFSCK checking can distinguish
 *	them: for former case, turst the master LMV EA with nothing to be done;
 *	otherwise, trust the first shard with valid hash type as the 3.2) does.
 *
 * 4) lmv_stripe_count: For the shard of a striped directory, if its index has
 *    been verified as the 2), then the stripe count must be larger than its
 *    index. For the master object, by scanning all shards' index, the LFSCK
 *    cannot the highest index, and the stripe count must be larger than the
 *    knowns the highest index. If the stipe count in the LMV EA matches above
 *    two rules, then it is may be trustable. If both the master claimed stripe
 *    count and the slave claimed stripe count match each own rule, but they
 *    are not the same, then trust the larger one. Because some name entries
 *    (and related shards) maybe lost, there is no way to know what the exactly
 *    stripe count should be, we have to use the known largest one.
 *
 ...
 *	It is very very race case, can be ignored.
 *
 * 5.2) The parent directory is a striped directory. Then the LFSCK needs to
 *	re-generate the master LMV EA: the lmv_master_mdt_index is from the
 *	MDT device index; the lmv_hash_type is from the first valid shard;
 *	the lmv_stripe_count will be calculated via scanning all the shards.
 *	Unfortunately, if someone has created other file(s) under the master
 *	object before the master LMV EA re-generated, then the LFSCK cannot
 *	recover the striped directory, because we needs to keep such file(s)
 *	to be visible to client. Under such case, the LFSCK will mark the
 *	master object as read only. It is the administrator's duty to handle
 *	the bad striped directory in further.
 */
...
/**
 * Mark the specified object as read-only vis set LUSTRE_IMMUTABLE_FL.
 *
			       ...
			       const struct lu_fid *fid, __u32 index)
{
	...
	len = sprintf(name2, DFID":%u", PFID(fid), index);
	name2[len] = '\0';
...
static int lfsck_record_lmv(const struct lu_env *env,
			    ...
			    struct lfsck_namespace_req *lnr,
			    struct lmv_mds_md_v1 *lmv,
			    int offset, __u32 flags, __u32 flags2)
{
	struct lfsck_lmv	   *llmv  = lnr->lnr_lmv;
	...
	if (index >= llmv->ll_stripes_allocated) {
		void *data;
		int count = index + 1;
		int size = sizeof(struct lfsck_slave_lmv_rec) *
			   llmv->ll_stripes_allocated;

		OBD_ALLOC_LARGE(data,
				sizeof(struct lfsck_slave_lmv_rec) * count);
		if (data == NULL) {
			llmv->ll_failed = 1;
		...
		memcpy(data, llmv->ll_recs, size);
		OBD_FREE_LARGE(llmv->ll_recs, size);
		llmv->ll_hash_type = LMV_HASH_TYPE_UNKNOWN;
		llmv->ll_stripes_allocated = count;
		llmv->ll_recs = data;
	}

	lslr = (struct lfsck_slave_lmv_rec *)llmv->ll_recs + index;
	...
	if (fid_is_zero(&lslr->lslr_fid)) {
		...
		if (llmv->ll_max_filled_slot < index)
			llmv->ll_max_filled_slot = index;
	...
	case LSLF_BAD_INDEX2:
		/* The conflict one is a normal valid object. */
		switch (flags) {
		case LSLF_NONE:
...
no_lmvea:
			if (llmv->ll_lmv.lmv_hash_type &
				 ...
				 *	LFSCK did not found such conflict,
				...
				rc = lfsck_remove_dirent(env, com, dir,
							 fid, index);
				if (rc > 0)
					rc = lfsck_namespace_trace_add(env, com,
								       fid, 0);
			}
		...
		case LSLF_BAD_INDEX1:
			 ...
			 * index in the lmv. Recursive once or twice. */
			rc = lfsck_record_lmv(env, com, lnr, lmv, index,
					LSLF_BAD_INDEX2, lslr->lslr_flags);
		...
		break;
	case LSLF_NO_LMVEA:
		/* The conflict one has no slave LMV EA. */
		switch (flags) {
		case LSLF_NONE:
...
none:
			if (llmv->ll_lmv.lmv_hash_type &
				...
				/* Otherwise, remove the conflict name entry,
				 * and add its FID in the LFSCK tracing file
				 * for further processing. */
		...
		break;
	case LSLF_DANGLING:
		/* The conflict one is a dangling name entry. */
		switch (flags) {
		case LSLF_NONE:
		case LSLF_BAD_INDEX2:
		case LSLF_NO_LMVEA:
			/* Remove the conflict dangling name entry. */
			rc = lfsck_replace_lmv(env, com, lslr, lnr,
					       lmv, index, flags);
			break;
		case LSLF_DANGLING:
		...
		case LSLF_BAD_INDEX1:
			...
			if (rc == -ERANGE || rc == -EEXIST) {
				 ...
				 * the conflict dangling name entry. */
				rc = lfsck_replace_lmv(env, com, lslr, lnr,
						       lmv, index, flags);
			}
	...
	case LSLF_BAD_INDEX1: {
		...
		struct lu_fid		 *fid2 = &info->lti_fid2;

		index = lslr->lslr_index;

		memset(lmv2, 0, sizeof(*lmv2));
		lmv2->lmv_magic = LMV_MAGIC_STRIPE;
		lmv2->lmv_stripe_count = lslr->lslr_stripe_count;
		lmv2->lmv_master_mdt_index = offset;
		lmv2->lmv_hash_type = lslr->lslr_hash_type;

		*fid2 = lnr->lnr_fid;
		lnr->lnr_fid = lslr->lslr_fid;

		/* The conflict one has another possible slot,
		 * try is recursive once. */
		rc = lfsck_record_lmv(env, com, lnr, lmv2, index,
				      LSLF_BAD_INDEX2, flags);
		if (rc == 0) {
			lslr->lslr_fid = *fid2;
			lslr->lslr_flags = flags;
			if (lmv != NULL) {
				...
				if (llmv->ll_hash_type ==
						LMV_HASH_TYPE_UNKNOWN &&
				    lmv_is_known_hash_type(lmv->lmv_hash_type))
					llmv->ll_hash_type = lmv->lmv_hash_type;

				if (lslr->lslr_stripe_count <=
						LFSCK_LMV_MAX_STRIPES &&
				    llmv->ll_max_stripe_count <
						lslr->lslr_stripe_count)
					llmv->ll_max_stripe_count =
						lslr->lslr_stripe_count;
...
/**
 * Check whether the given entry match shard's name rule or not.
 *
 * The valid shard name/type should be:
 * 1) The type must be S_ISDIR
				     ...
				     __u16 type, const struct lu_fid *fid)
{
	...
	if (idx < 0 || idx >= LFSCK_LMV_MAX_STRIPES)
		return -EINVAL;
			       ...
			       struct lmv_mds_md_v1 *lmv, bool locked)
{
	...
	lfsck_buf_init(buf, lmv2, sizeof(*lmv2));
...
out:
	CDEBUG(D_LFSCK, "%s: namespace LFSCK set LMV EA for the object "
	       DFID" on the MDT %x remotely with event %u: rc = %d\n",
	       lfsck_lfsck2name(lfsck), PFID(fid),
	       ltd != NULL ? ltd->ltd_index : -1, event, rc);
 ...
 * been created some non-shard objects under the master MDT-object. If such
 * case happend, then the LFSCK cannot re-generate the lost master LMV EA to
 * those non-shard objects to be visible client.
 *
 * \param[in] env	pointer to the thread context
 * \param[in] com	pointer to the lfsck component
				      ...
				      const struct lu_fid *cfid, __u32 cidx)
{
	...
	sprintf(name, DFID":%u", PFID(cfid), cidx);
	...
	do {
		 ...
		 *	the subdir to be invisible for ever. */
 ...
 * to be visible to clients. Then the LFSCK will mark the striped directory
 ...
 * directory directly, then we will set the master LMV EA and generate a
 * new re-scan (the striped directory) that will be handled by the LFSCK
					 ...
					 __u32 cidx, __u32 flags)
{
	...
	OBD_ALLOC_LARGE(data, sizeof(struct lfsck_slave_lmv_rec) * count);
	if (data == NULL) {
		OBD_FREE_PTR(llu);
 ...
 *
 * Ideally, we can use general dt_xattr_set() API for that, but because the
 * LFSCK on current MDT may only knows part of the LMV EA elements' values,
 * and there will other additional actions after the LMV EA updating on the
 * remote MDT. So the LFSCK will use LFSCK_NOTIFY RPC for these purpose via
 * single RPC.
 *
 * \param[in] env	pointer to the thread context
 * \param[in] com	pointer to the lfsck component
					...
					const char *name)
{
	...
	idx = lfsck_find_mdt_idx_by_fid(env, lfsck, pfid);
	if (idx < 0)
		GOTO(log, rc = idx);
	...
	parent = lfsck_object_find_by_dev(env, dev, pfid);
	if (IS_ERR(parent))
...
log:
	CDEBUG(D_LFSCK, "%s: namespace LFSCK assistant found bad name hash"
	       ...
	       PFID(pfid), name,
	       idx < 0 ? llmv->ll_lmv.lmv_master_mdt_index : idx, rc);
 ...
 * unmatched name entry, the LFSCK will will try to fix it.
 *
 * \param[in] env	pointer to the thread context
 * \param[in] com	pointer to the lfsck component
					...
					struct lfsck_lmv *llmv)
{
	...
	if (lfsck_is_invalid_slave_lmv(clmv)) {
		rc = lfsck_namespace_trace_add(env, com, cfid,
					       LLF_UNCERTAION_LMV);
	...
	if (rc != 0) {
		...
		rc1 = lfsck_namespace_trace_add(env, com, cfid,
						LLF_UNCERTAION_LMV);
	...
	rc = sprintf(name2, DFID":%u", PFID(cfid), clmv->lmv_master_mdt_index);
	name2[rc] = '\0';

	rc = lfsck_links_get_first(env, obj, name, pfid);
	if (rc == 0 && strcmp(name, name2) == 0) {
		llmv->ll_lmv_verified = 1;
...
/**
 * Verify the name entry under the striped directory.
 *
 * All the name entry under the given striped directory or its shard have
 * been scanned, the LFSCK has got the global knownledge about the LMV EA
 * consistency.
 *
 ...
 *			striped directory or its shard
 *
 * \retval		zero for succeed
 * \retval		negative error number on failure
 */
					...
					struct lfsck_namespace_req *lnr)
{
	...
	if (llmv->ll_lmv_slave) {
		if (llmv->ll_lmv_verified) {
			...
			up_write(&com->lc_sem);
			lfsck_namespace_trace_del(env, com,
						  lfsck_dto2fid(lnr->lnr_obj));
		}

		RETURN(0);
	}

	/* Either the striped directory has been disabled or only part of the
	 * striped directory have been scanned. */
	 ...
	 * so it cannot make reparation about the uncompleted knowledge. */
	if (llmv->ll_failed) {
		down_write(&com->lc_sem);
	...
	if (lmv->lmv_stripe_count > LFSCK_LMV_MAX_STRIPES) {
		/* The stripe count in the master LMV EA is wrong. */
		if (llmv->ll_max_filled_slot >= llmv->ll_max_stripe_count)
			/* The max known stripe index is not smaller than
			 * the max known stripe count in slave LMV EA, then
			 * trust the max known stripe index. */
			stripe_count = llmv->ll_max_filled_slot + 1;
		else
			/* The max known stripe index is smaller than the
			 * max known stripe count in slave LMV EA, then
			 * trust the max known stripe count in slave LMV EA,
			 * because there may be orphan shards not found yet. */
			stripe_count = llmv->ll_max_stripe_count;
	} else if (lmv->lmv_stripe_count <= llmv->ll_max_filled_slot) {
		/* The max known stripe index is not smaller than the
		 * stripe count in the master LMV EA, then trust the
		 * max known stripe index. */
		stripe_count = llmv->ll_max_filled_slot + 1;
	} else {
		stripe_count = lmv->lmv_stripe_count;
	}
	...
	if (llmv->ll_lmv_updated) {
		rc = lfsck_namespace_update_lmv(env, com, lnr->lnr_obj,
						lmv, false);
		if (rc != 0)
			RETURN(rc);
	...
	for (i = 0; i <= llmv->ll_max_filled_slot; i++) {
		struct dt_object *obj = NULL;
		struct lfsck_slave_lmv_rec *lslr =
			(struct lfsck_slave_lmv_rec *)llmv->ll_recs + i;
		char *name = info->lti_tmpbuf;
		char *name2 = info->lti_tmpbuf;
		...
		len = sprintf(name, DFID":%u", PFID(cfid), i);
		name[len] = '\0';
		cname = lfsck_name_get_const(env, name, len);
		memcpy(lnr->lnr_name, name, len);
		...
		if (idx == lfsck_dev_idx(lfsck->li_bottom)) {
			...
			obj = lfsck_object_find_by_dev(env, ltd->ltd_tgt, cfid);
		}
		...
		case LSLF_BAD_INDEX2:
			...
			rename = true;
			len = sprintf(name2, DFID":%u", PFID(cfid),
				      lslr->lslr_index);
			name2[len] = '\0';
...
repair:
		...
		if (lmvea) {
			...
			if (rc1 == 0) {
				...
				up_write(&com->lc_sem);
				rc1 = lfsck_namespace_trace_add(env, com,
						cfid, LLF_RECHECK_NAMEHASH);
			}
		}

		if (rename) {
			rc1 = lfsck_namespace_repair_dirent(env, com,
					lnr->lnr_obj, obj, lnr->lnr_name, name2,
		...
		if (linkea) {
			...
			rc1 = linkea_add_buf(&ldata, cname,
					     lfsck_dto2fid(lnr->lnr_obj));
			if (rc1 != 0)
				goto next;
...
next:
		CDEBUG(D_LFSCK, "%s: namespace LFSCK repair the shard "DFID
		      "of the striped directory "DFID" on the MDT %x with "
		      "dangling %s, rename %s, lmvea %s, linkea %s: rc = %d\n",
		      lfsck_lfsck2name(lfsck), PFID(cfid), PFID(&lnr->lnr_fid),
		      idx, create ? "yes" : "no", rename ? "yes" : "no",
		      lmvea ? "yes" : "no", linkea ? "yes" : "no", rc1);
					  ...
					  struct lfsck_namespace_req *lnr)
{
	...
	if (idx == lfsck_dev_idx(lfsck->li_bottom)) {
		...
		if (unlikely(strcmp(lnr->lnr_name, dotdot) == 0)) {
			rc = lfsck_namespace_trace_add(env, com, pfid, 0);
...
dangling:
		...
		if (rc == 0)
			rc = lfsck_record_lmv(env, com, lnr, NULL, stripe,
					      LSLF_DANGLING, LSLF_NONE);

		GOTO(out, rc);
	}

	stripe = lfsck_is_valid_master_name_entry(env, lnr->lnr_name,
			lnr->lnr_namelen, lfsck_object_type(obj), &lnr->lnr_fid);
	if (stripe < 0) {
		type = LNIT_BAD_DIRENT;
