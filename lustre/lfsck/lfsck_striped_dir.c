					...
					struct lfsck_namespace_req *lnr)
{
	...
	for (i = 0; i <= llmv->ll_max_filled_slot; i++) {
		...
		len = snprintf(name, LFSCK_TMPBUF_LEN - 1, DFID":%u",
			       PFID(cfid), i);
		name[len] = '\0';
		...
		case LSLF_BAD_INDEX2:
			...
			rename = true;
			len = snprintf(name2, LFSCK_TMPBUF_LEN - 1, DFID":%u",
