...
struct lfsck_rec_lmv_save *lfsck_rec_lmv_save_buffer;

/**
 * Record the slave LMV EA in the lfsck_lmv::ll_recs.
 *
 * If the lfsck_lmv::ll_recs slot corresponding to the given @offset is free,
 * then fill the slot with the given @lnr/@lmv/@flags directly (maybe need to
 * extend the lfsck_lmv::ll_recs buffer).
 *
 * If the lfsck_lmv::ll_recs slot corresponding to the given @offset is taken
...
none:
			if (llmv->ll_lmv.lmv_hash_type &
				...
				lrls = lfsck_rec_lmv_save_buffer + *depth - 1;
					...
					struct lfsck_namespace_req *lnr)
{
	...
	if (llmv->ll_lmv_slave) {
		if (llmv->ll_lmv_verified) {
			...
			up_write(&com->lc_sem);
			lfsck_namespace_trace_del(env, com, lfsck_dto2fid(dir));
		}
		...
		if (lmvea) {
			...
			if (dt_object_remote(obj) != 0) {
				...
				if (rc1 == 0) {
					...
					up_write(&com->lc_sem);
					rc1 = lfsck_namespace_trace_add(env,
							com, cfid,
							LLF_RECHECK_NAMEHASH);
				}
			}
...
next:
		CDEBUG(D_LFSCK, "%s: namespace LFSCK repair the shard "
		      DFID"of the striped directory "DFID" on the MDT %x "
		      "with dangling %s/%s, rename %s/%s, llinkea %s/%s, "
		      "lmvea %s/%s: rc = %d\n", lfsck_lfsck2name(lfsck),
		      PFID(cfid), PFID(&lnr->lnr_fid), idx,
 ...
 *    the lfsck_lmv::ll_recs. In this step, if the 'shard' (may be fake
 ...
 *    thread will scan the lfsck_lmv::ll_recs, and for every shard in
