...
static int lfsck_object_needs_lfsck(const struct lu_env *env,
				    struct lfsck_instance *lfsck,
		   ...
		   struct dt_object *obj)
{
	...
	rc = lfsck_object_needs_lfsck(env, lfsck, obj);
	if (rc <= 0)
		GOTO(out, rc);
		   ...
		   struct dt_device *next, bool master)
{
	...
	if (master) {
		...
		if (rc == -ENOENT)
			rc = dt_lookup(env, root,
			       (struct dt_rec *)(&lfsck->li_global_root_fid),
			       (const struct dt_key *)"REMOTE_PARENT_DIR",
			       BYPASS_CAPA);
...
cfs_module(lfsck, "2.5.0", lfsck_init, lfsck_exit);
