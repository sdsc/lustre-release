...
/**
 * Requst the specified ibits lock for the given object.
 *
				   ...
				   const char *name)
{
	...
	/* 7a. insert name into parent dir */
	rec->rec_fid = cfid;
	rc = dt_declare_insert(env, parent, (const struct dt_rec *)rec,
			       (const struct dt_key *)name, th);
	if (rc != 0)
	...
	/* 7b. insert name into parent dir */
	rc = dt_insert(env, parent, (const struct dt_rec *)rec,
		       (const struct dt_key *)name, th, BYPASS_CAPA, 1);
	if (rc != 0)
		GOTO(stop, rc);
				      ...
				      struct dt_object *parent)
{
	...
	while (rc == 0) {
		...
		while (off < 7 && ent->lde_name[off] <= '9' &&
		       ent->lde_name[off] >= '0')
			off++;
...
#define LFSCK_UNCERTAION_INCONSISTENCY 1

/**
 * Check whether the @child back references the @parent.
 *
 * The child's FID is stored in the bookmark file. If the child back
 * references the parent via its ".." entry, then insert the name to
 * the .lustre/lost+found; otherwise, if the child back references
 * another parent2, then:
 * 1) If the parent2 recognizes the child, then update the bookmark file;
 * 2) Otherwise, the LFSCK cannot know whether there will be parent3 that
 ...
 *			which FID is stored in the bookmark file
 ...
 *			it is referenced by the name entry under the parent2.
 *
 * \retval		positive number for uncertain inconsistency
 * \retval		0 for success
 * \retval		negative error number on failure
 */
static int lfsck_verify_lpf_child1(const struct lu_env *env,
				   struct lfsck_instance *lfsck,
				   struct dt_object *parent,
				   struct dt_object *child,
				   const char *name,
				   struct lu_fid *fid)
{
	...
	if (rc != 0)
		GOTO(linkea, rc = LFSCK_UNCERTAION_INCONSISTENCY);
...
linkea:
	...
	if (rc == 0) {
		/* It is almost impossible that the bookmark file and
		 * child's linkEA hit the same data corruption. Trust
		 * the linkEA then. */
		if (lu_fid_eq(fid2, &LU_LPF_FID) && strcmp(name, name2) == 0) {
			...
			*fid = *fid2;
			...
			/* Update the parent's @name entry. It is NOT
			 * necessary to make this updating and above
			 * updating to be within single transaction. */
			rc = lfsck_update_name_entry(env, lfsck, parent, name,
						     lfsck_dto2fid(child),
						     S_IFDIR);
		...
		if (parent2 == NULL || IS_ERR(parent2)) {
			...
			RETURN(LFSCK_UNCERTAION_INCONSISTENCY);
		}
	...
	lfsck_ibits_unlock(&lh, LCK_PR);
	if (rc != 0)
		GOTO(put, rc = LFSCK_UNCERTAION_INCONSISTENCY);

	if (!lu_fid_eq(fid, lfsck_dto2fid(child)))
		GOTO(put, rc = LFSCK_UNCERTAION_INCONSISTENCY);
 ...
 * Check whether the @child back references the @parent.
 *
 * The child's FID is stored in the .lustre/lost+found/ sub-directory name
 * entry. If the child back references the parent via its ".." entry, then
 * update the bookmark file, otherwise, if the child back references another
 * parent2, then:
 * 1) If the parent2 recognizes the child, then remove the sub-directory
 *    from .lustre/lost+found/;
 * 2) Otherwise, if the parent2 does not recognizes the child, trust the
 *    sub-directory name entry and update the child;
 * 3) Otherwise, if we do not know whether the parent2 recognizes the child
 ...
 * \param[in] parent	pointer to the lost+found object
 * \param[in] child	pointer to the lost+found sub-directory object
 *			which FID is stored in the lost+found sub-directory
 *			name entry
 * \param[in] name	the name for lost+found sub-directory object
 * \param[out] fid	pointer to the buffer to hold the FID of the object
 *			(called it as parent2) that is referenced via the
 ...
 * \retval		negative error number on failure
 */
static int lfsck_verify_lpf_child2(const struct lu_env *env,
				   struct lfsck_instance *lfsck,
				   struct dt_object *parent,
				   struct dt_object *child,
				   const char *name,
	...
	if (rc == 0) {
		...
		if (lu_fid_eq(fid2, &LU_LPF_FID) && strcmp(name, name2) == 0) {
			...
			*fid = *fid2;
			...
			/* Update the bookmark file. It is NOT necessary
			 * to make this updating and above updating to be
			 * within single transaction. */
			lfsck->li_bookmark_ram.lb_lpf_fid =
					*lfsck_dto2fid(child);
			rc = lfsck_bookmark_store(env, lfsck);
...
update:
	...
	rc = lfsck_update_name_entry(env, lfsck, child, dotdot, &LU_LPF_FID,
				     S_IFDIR);
 ...
 * be verified firstly to allow the users it during the LFSCK.
 *
 * \param[in] env	pointer to the thread context
 * \param[in] lfsck	pointer to the lfsck instance
 *
...
int lfsck_verify_lpf(const struct lu_env *env, struct lfsck_instance *lfsck)
{
	...
	if (rc == 0) {
		...
		if (unlikely(!dt_object_exists(child2) ||
			...
			goto check_child1;
		} else if (unlikely(!dt_try_as_dir(env, child2))) {
			GOTO(put, rc = -ENOTDIR);
		}

		if (child1 == NULL) {
			rc = lfsck_verify_lpf_child2(env, lfsck, parent,
						     child2, name, pfid);
		} else if (!lu_fid_eq(cfid, &bk->lb_lpf_fid)) {
			rc = lfsck_verify_lpf_child1(env, lfsck, parent, child1,
						     name, pfid);
			if (!lu_fid_eq(pfid, &LU_LPF_FID))
				rc = lfsck_verify_lpf_child2(env, lfsck,
						parent, child2, name, pfid);
		} else {
			if (lfsck->li_lpf_obj == NULL) {
			...
			cname = lfsck_name_get_const(env, name, strlen(name));
			rc = lfsck_verify_linkea(env, dev, child2, cname,
						 &LU_LPF_FID);
		}
			      ...
			      struct lfsck_component *com, int status)
{
	...
	if (unlikely(status != LS_SCANNING_PHASE2))
		return 0;
