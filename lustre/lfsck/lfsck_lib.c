...
lfsck_trans_callback_init(const struct lu_env *env, struct lfsck_component *com,
			  dt_cb_t func, int type)
{
...
void lfsck_trans_callback_fini(struct lfsck_trans_callback *ltc)
{
	lfsck_component_put(ltc->ltc_env, ltc->ltc_com);
	OBD_FREE_PTR(ltc);
}
