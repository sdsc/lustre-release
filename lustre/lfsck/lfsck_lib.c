				  ...
				  const char *name)
{
	...
	struct dt_insert_rec	*dir	= &lfsck_env_info(env)->lti_dir;
	loff_t			 pos	= 0;
			    ...
			    struct lfsck_instance *lfsck)
{
	...
	while (!list_empty(&lfsck->li_list_scan)) {
		com = list_entry(lfsck->li_list_scan.next,
				 struct lfsck_component,
				 lc_link);
	...
	while (!list_empty(&lfsck->li_list_double_scan)) {
		com = list_entry(lfsck->li_list_double_scan.next,
				 struct lfsck_component,
				 lc_link);
		lfsck_component_cleanup(env, com);
	}

	while (!list_empty(&lfsck->li_list_idle)) {
		com = list_entry(lfsck->li_list_idle.next,
				 struct lfsck_component,
				 lc_link);
		lfsck_component_cleanup(env, com);
	}
				 ...
				 void *args, int rc)
{
	...
	case LE_PEER_EXIT:
		if (rc != 0 && rc != -EALREADY)
			CWARN("%s: fail to notify %s %x for %s: "
			      "event = %d, rc = %d\n",
			      lfsck_lfsck2name(com->lc_lfsck),
			      (lr->lr_flags & LEF_TO_OST) ? "OST" : "MDT",
	...
	case LE_QUERY: {
		...
		if (reply == NULL) {
			rc = -EPROTO;
			CERROR("%s: invalid return value of query for %s: "
			       "rc = %d\n", lfsck_lfsck2name(com->lc_lfsck),
			       lad->lad_name, rc);
			spin_lock(&ltds->ltd_lock);
			list_del_init(phase_list);
			     ...
			     struct lfsck_tgt_desc *ltd, __u16 type)
{
	...
	if (com != NULL) {
		...
		rc = lfsck_async_request(env, ltd->ltd_exp, lr, set,
					 ...
					 laia, LFSCK_NOTIFY);
		if (rc != 0) {
			CERROR("%s: fail to notify %s %x for co-stop for %s: "
			       "rc = %d\n", lfsck_lfsck2name(lfsck),
			       (lr->lr_flags & LEF_TO_OST) ? "OST" : "MDT",
			       ltd->ltd_index, lad->lad_name, rc);
			lfsck_tgt_put(ltd);
...
unlock:
	...
	if (ltd != NULL) {
		...
		ltd->ltd_dead = 1;
		spin_unlock(&ltds->ltd_lock);
		lfsck_stop_notify(env, lfsck, ltds, ltd, LT_NAMESPACE);
