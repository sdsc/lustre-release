	...
	if (master) {
		...
		if (lfsck_dev_idx(key) == 0) {
			...
			rc = dt_lookup(env, obj, (struct dt_rec *)fid,
				(const struct dt_key *)".lustre", BYPASS_CAPA);
			if (rc != 0)
				GOTO(out, rc);
			...
			cname = lfsck_name_get_const(env, ".lustre",
						     strlen(".lustre"));
			...
			*pfid = *fid;
			rc = dt_lookup(env, obj, (struct dt_rec *)fid,
				(const struct dt_key *)"lost+found", BYPASS_CAPA);
			if (rc != 0)
				GOTO(out, rc);
			...
			cname = lfsck_name_get_const(env, "lost+found",
						     strlen("lost+found"));
