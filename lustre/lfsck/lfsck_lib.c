				  ...
				  const char *name)
{
	...
	buf = lfsck_buf_get_const(env, ldata.ld_buf->lb_buf,
				  ldata.ld_leh->leh_len);
	rc = dt_declare_xattr_set(env, child, buf, XATTR_NAME_LINK, 0, th);
	if (rc != 0)
		GOTO(stop, rc);
	...
	/* 5a. insert linkEA for child. */
	rc = dt_xattr_set(env, child, buf, XATTR_NAME_LINK, 0, th, BYPASS_CAPA);
	if (rc != 0)
		GOTO(stop, rc);
				   ...
				   const char *name)
{
	...
	/* 5a. insert name into parent dir remotely. */
	dir->dir_fid = cfid;
	rc = dt_declare_insert(env, parent, (const struct dt_rec *)dir,
			       (const struct dt_key *)name, th);
	if (rc != 0)
	...
	/* 6a. increase parent nlink remotely. */
	rc = dt_declare_ref_add(env, parent, th);
	if (rc != 0)
		GOTO(stop, rc);
	...
	/* 5b. insert name into parent dir remotely. */
	rc = dt_insert(env, parent, (const struct dt_rec *)dir,
		       (const struct dt_key *)name, th, BYPASS_CAPA, 1);
	if (rc != 0)
		GOTO(stop, rc);
	...
	dt_write_lock(env, parent, 0);
	/* 6b. increase parent nlink remotely. */
	rc = dt_ref_add(env, parent, th);
	dt_write_unlock(env, parent);
