 ...
 * \retval		negative error number on failure
 */
static int lfsck_verify_lpf_by_bookmark(const struct lu_env *env,
					struct lfsck_instance *lfsck,
					struct dt_object *parent,
					struct dt_object *child,
					const char *name,
 ...
 * \retval		negative error number on failure
 */
static int lfsck_verify_lpf_by_nameentry(const struct lu_env *env,
					 struct lfsck_instance *lfsck,
					 struct dt_object *parent,
					 struct dt_object *child,
					 const char *name,
...
int lfsck_verify_lpf(const struct lu_env *env, struct lfsck_instance *lfsck)
{
	...
	if (rc == 0) {
		...
		if (child1 == NULL) {
			rc = lfsck_verify_lpf_by_nameentry(env, lfsck, parent,
							   child2, name, pfid);
		} else if (!lu_fid_eq(cfid, &bk->lb_lpf_fid)) {
			rc = lfsck_verify_lpf_by_bookmark(env, lfsck, parent,
							  child1, name, pfid);
			if (!lu_fid_eq(pfid, &LU_LPF_FID))
				rc = lfsck_verify_lpf_by_nameentry(env, lfsck,
...
check_child1:
	if (child1 != NULL)
		rc = lfsck_verify_lpf_by_bookmark(env, lfsck, parent, child1,
						  name, pfid);
...
static int __init lfsck_init(void)
{
	...
	OBD_ALLOC(lfsck_rec_lmv_save_buffer,
		  sizeof(struct lfsck_rec_lmv_save) *
		  LFSCK_REC_LMV_MAX_DEPTH);
	if (lfsck_rec_lmv_save_buffer == NULL)
		return -ENOMEM;
...
static void __exit lfsck_exit(void)
{
	...
	OBD_FREE(lfsck_rec_lmv_save_buffer,
		 sizeof(struct lfsck_rec_lmv_save) *
		 LFSCK_REC_LMV_MAX_DEPTH);
}
