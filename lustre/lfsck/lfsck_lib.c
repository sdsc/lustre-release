...
#define LFSCK_UNCERTAIN_INCONSISTENCY 1
				   ...
				   struct lu_fid *fid)
{
	...
	if (rc != 0)
		GOTO(linkea, rc = LFSCK_UNCERTAIN_INCONSISTENCY);
...
linkea:
	...
	if (rc == 0) {
		...
		if (parent2 == NULL || IS_ERR(parent2)) {
			...
			RETURN(LFSCK_UNCERTAIN_INCONSISTENCY);
		}
	...
	lfsck_ibits_unlock(&lh, LCK_PR);
	if (rc != 0)
		GOTO(put, rc = LFSCK_UNCERTAIN_INCONSISTENCY);
