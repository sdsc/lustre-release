...
struct list_head lfsck_ost_orphan_list;
struct list_head lfsck_mdt_orphan_list;
...
void lfsck_component_get(struct lfsck_component *com)
{
...
static void lfsck_tgt_descs_fini(struct lfsck_tgt_descs *ltds)
{
	...
	for (idx = 0; idx < TGT_PTRS; idx++) {
		if (ltds->ltd_tgts_idx[idx]) {
			OBD_FREE_LARGE(ltds->ltd_tgts_idx[idx],
				       sizeof(ltds->ltd_tgts_idx[0]));
			      ...
			      bool is_osc, bool locked)
{
	...
	if (ltds->ltd_tgts_idx[index / TGT_PTRS_PER_BLOCK] == NULL) {
		OBD_ALLOC_LARGE(ltds->ltd_tgts_idx[index / TGT_PTRS_PER_BLOCK],
				sizeof(ltds->ltd_tgts_idx[0]));
		if (ltds->ltd_tgts_idx[index / TGT_PTRS_PER_BLOCK] == NULL)
			GOTO(unlock, rc = -ENOMEM);
	}
