...
static void lfsck_unpack_ent(struct lu_dirent *ent, __u64 *cookie, __u16 *type)
{
	struct luda_type	*lt;
	...
	*cookie = le64_to_cpu(ent->lde_hash);
	...
	LASSERT(ent->lde_attrs & LUDA_TYPE);
				   ...
				   struct lfsck_instance *lfsck)
{
	...
	do {
		...
		rc = iops->rec(env, di, (struct dt_rec *)ent,
			       lfsck->li_args_dir);
		lfsck_unpack_ent(ent, &lfsck->li_cookie_dir, &type);
		if (rc != 0) {
...
int lfsck_master_engine(void *args)
{
	...
	if (lfsck->li_master &&
		 ...
		 * directroy. So go ahead until hit failure when really uses
		 * the directory. */
		if (rc != 0)
...
again:
	...
	while (!list_empty(head)) {
		...
		*gen = lad->lad_touch_gen;
		list_move_tail(phase_list, head);
...
int lfsck_assistant_engine(void *args)
{
	...
	while (1) {
		...
		if (lad->lad_to_double_scan) {
			...
			/* flush all async updating before phase2. */
