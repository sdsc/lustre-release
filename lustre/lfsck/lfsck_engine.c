 ...
 * 2) We do not scan the .lustre and its children, because the LFSCK has done
 *    that when the LFSCK engine start.
 * 3) Global /ROOT needs to be scanned, backend root needs not to be scanned.
 * 4) If the @obj is neither IGIF nor normal FID, then needs not to be scanned.
 * 5) If it is a remote object, then scanning the object will be done on the
 *    MDT on which the object really resides.
 * 6) If the local object has normal FID, then needs to be scanned. Otherwise,
 * 7) If the object has linkEA, then needs to be scanned. Otherwise,
 * 8) Check the object's parent, set the parent as current @obj, repeat 3)-8).
 *
