				     ...
				     int fl, __u32 ost_idx)
{
	...
	if (le32_to_cpu(lmm->lmm_pattern) & LOV_PATTERN_F_HOLE) {
		...
		if (i == count)
			lmm->lmm_pattern =
				cpu_to_le32(le32_to_cpu(lmm->lmm_pattern) &
					    ~LOV_PATTERN_F_HOLE);
	}
				     ...
				     __u32 ost_idx, __u32 ea_off, bool reset)
{
	...
	if (fl == LU_XATTR_CREATE || reset) {
		struct dt_device_param *param = &lfsck_env_info(env)->lti_param;
		...
		dt_conf_get(env, lfsck->li_next, param);
		lmm->lmm_stripe_size =
				cpu_to_le32(param->ddp_default_stripe_size);
