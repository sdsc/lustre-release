				    ...
				    struct dt_device *cdev, __u32 ea_off)
{
	...
	pfid->ff_parent.f_ostidx = cpu_to_le32(ea_off);
					...
					__u32 ea_off)
{
	...
	if (fid_is_zero(pfid)) {
		...
		ff->ff_parent.f_ostidx = cpu_to_le32(ea_off);
					...
					struct lu_attr *la)
{
	 ...
	 * MDT-object's FID::f_ver, instead it is the OST-object index in its
	 * parent MDT-object's layout EA. */
	pfid->ff_parent.f_ostidx = cpu_to_le32(llr->llr_lov_idx);
	buf = lfsck_buf_get(env, pfid, sizeof(struct filter_fid));
