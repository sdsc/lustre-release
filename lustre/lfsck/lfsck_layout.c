...
static int lfsck_layout_assistant(void *args)
{
	...
	while (1) {
		while (!list_empty(&llmd->llmd_list)) {
			...
			spin_lock(&llmd->llmd_lock);
			llr = list_entry(llmd->llmd_list.next,
					 struct lfsck_layout_req,
					 llr_list);
...
cleanup1:
	if (!llmd->llmd_exit) {
		LASSERT(list_empty(&llmd->llmd_list));
	} else {
