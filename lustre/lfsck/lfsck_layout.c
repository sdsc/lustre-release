				    ...
				    struct dt_device *cdev, __u32 ea_off)
{
	...
	rc = dt_trans_start_local(env, cdev, handle);
	if (rc != 0)
		GOTO(stop, rc);
					...
					__u32 ea_off)
{
	...
	/* The transaction will contains both the MDT-object (parent) and
	 * the OST-object (child) modifications, so we need to locate the
	 * object based on lfsck->li_next. */

	lpf = lfsck_object_find_by_dev(env, next,
				       lfsck_dto2fid(lfsck->li_lpf_obj));
	if (IS_ERR(lpf))
		GOTO(log, rc = PTR_ERR(lpf));

	if (unlikely(!dt_try_as_dir(env, lpf)))
		GOTO(log, rc = -ENOTDIR);
	...
	if (fid_is_zero(pfid)) {
		...
		cobj = lfsck_object_find_by_dev(env, next, cfid);
		if (IS_ERR(cobj))
			GOTO(log, rc = PTR_ERR(cobj));
	}

	pobj = lfsck_object_find_by_dev(env, next, pfid);
	if (IS_ERR(pobj))
	 ...
	 *
	 * Set the @fl as LU_XATTR_CREATE | LU_XATTR_REPLACE to tell
	 * low layer NOT to create stripes. */
	lfsck_buf_init(&lov_buf, ea_buf->lb_buf, size);
	rc = dt_declare_xattr_set(env, pobj, &lov_buf, XATTR_NAME_LOV,
				  LU_XATTR_CREATE | LU_XATTR_REPLACE, th);
	if (rc != 0)
		GOTO(stop, rc);
	...
	rc = dt_trans_start_local(env, next, th);
	if (rc != 0)
		GOTO(stop, rc);
	...
	if (rc == 0)
		...
		rc = lfsck_layout_extend_lovea(env, lfsck, th, pobj, cfid,
				&lov_buf, LU_XATTR_CREATE | LU_XATTR_REPLACE,
				ltd->ltd_index, ea_off, true);
...
log:
	...
	if (lpf != NULL && !IS_ERR(lpf))
		lfsck_object_put(env, lpf);
						   ...
						   struct lu_buf *buf)
{
	...
	struct dt_object_format 	*dof	= &info->lti_dof;
	struct dt_device		*next	= com->lc_lfsck->li_next;
	struct ost_id			*oi	= &info->lti_oi;
	struct dt_object		*parent = NULL;
	struct dt_device		*cdev	= lfsck_obj2dev(llr->llr_child);
	...
	if (lfsck_obj2dev(llr->llr_parent->llo_obj) == next) {
		parent = llr->llr_parent->llo_obj;
	} else {
		parent = lfsck_object_find_by_dev(env, next,
				lfsck_dto2fid(llr->llr_parent->llo_obj));
		if (IS_ERR(parent))
	...
	if (IS_ERR(o))
		GOTO(unlock1, rc = PTR_ERR(o));

	o = lu_object_locate(o->lo_header, d->ld_type);
	if (unlikely(o == NULL))
		GOTO(unlock1, rc = -EINVAL);
	...
	handle = dt_trans_create(env, next);
	if (IS_ERR(handle))
		GOTO(unlock1, rc = PTR_ERR(handle));
	...
	rc = dt_trans_start_local(env, next, handle);
	if (rc != 0)
		GOTO(stop, rc);
