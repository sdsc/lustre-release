					...
					struct lfsck_layout_req *llr)
{
	...
	if (llr->llr_req != NULL)
		ptlrpc_req_finished(llr->llr_req);
					  ...
					  struct obd_export *exp)
{
	...
	rc = obd_get_info(env, exp, strlen(KEY_LFSCK_QUERY),
			  KEY_LFSCK_QUERY, &size, &status, NULL);
	if (rc != 0)
		return rc;
	if (status == 0)
		return 1;
	...
	while (1) {
		...
		if (llmd->llmd_to_post) {
			...
			if (llmd->llmd_post_result > 0) {
				lcr->lcr_type = LCT_LAYOUT_NEXT;
			} else if (llmd->llmd_post_result == 0) {
				lcr->lcr_type = LCT_LAYOUT_STOP;
				if (lfsck->li_paused)
					lcr->lcr_status = LS_CO_PAUSED;
				else
					lcr->lcr_status = LS_CO_PAUSED;
			} else {
				lcr->lcr_type = LCT_LAYOUT_STOP;
				lcr->lcr_status = LS_CO_FAILED;
			}
				   ...
				   int result, bool init)
{
	...
	lfsck->li_notify(lfsck->li_notify_data, LNE_LAYOUT_PHASE1_DONE, result);
	return rc;
}
					  ...
					  struct lfsck_component *com)
{
	...
	lfsck->li_notify(lfsck->li_notify_data, LNE_LAYOUT_PHASE2_DONE, rc);
