...
lfsck_layout_req_init(struct lfsck_layout_object *parent,
		      struct dt_object *child, __u32 idx)
{
	struct lfsck_layout_req *llr;
...
again:
	...
	if (rc == -ERANGE) {
		rc = dt_xattr_get(env, tobj, &LU_BUF_NULL, XATTR_NAME_LOV,
				  BYPASS_CAPA);
		if (unlikely(rc == -ENOENT || rc == -ENODATA || rc == 0))
			GOTO(out, rc = LLI_UNMATCHED_PAIR);
				     ...
				     int count, __u16 gen)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (rc != 0) {
			lu_object_put_nocache(env, &cobj->do_lu);
			goto next;
		}
		...
		llr = lfsck_layout_req_init(llo, cobj, index);
		if (llr == NULL) {
			lu_object_put_nocache(env, &cobj->do_lu);
			rc = -ENOMEM;
			goto next;
