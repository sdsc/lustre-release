...
struct lfsck_layout_master_data {
	...
	atomic_t		llmd_async_rpcs_in_flight;
					...
					struct dt_txn_commit_cb *cb, int rc)
{
	...
	if (atomic_dec_and_test(&llmd->llmd_async_rpcs_in_flight))
					...
					struct lu_attr *la)
{
	...
	atomic_inc(&llmd->llmd_async_rpcs_in_flight);
	rc = dt_trans_start(env, dev, handle);
	if (rc != 0) {
		atomic_dec(&llmd->llmd_async_rpcs_in_flight);
		GOTO(stop, rc);
	}
					  ...
					  struct lfsck_component *com, bool init)
{
	...
	l_wait_event(thread->t_ctl_waitq,
		     ...
		     (list_empty(&llmd->llmd_req_list) &&
		      atomic_read(&llmd->llmd_async_rpcs_in_flight) == 0),
		     &lwi);
					     ...
					     struct lfsck_component *com)
{
	...
	LASSERT(list_empty(&llmd->llmd_req_list));
	LASSERT(atomic_read(&llmd->llmd_async_rpcs_in_flight) == 0);
...
int lfsck_layout_setup(const struct lu_env *env, struct lfsck_instance *lfsck)
{
	...
	if (lfsck->li_master) {
		...
		atomic_set(&llmd->llmd_async_rpcs_in_flight, 0);
		com->lc_data = llmd;
	} else {
