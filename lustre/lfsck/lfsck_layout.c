 ...
 *
 * Under some data curruption cases, the MDT-object of the file may be lost,
 ...
 *
 * On the other hand, the LFSCK may has created some OST-object for reparing
 * dangling LOV EA reference, but as the LFSCK processing, it may found that
 * the old OST-object is there and should replace the former new created OST
 * object. Unfortunately, some others have modified such new created object.
