				     ...
				     struct lu_buf *lov_ea, __u32 idx)
{
	...
	for (i = 0; i < count; i++, objs++) {
		...
		if (lu_fid_eq(cfid, tfid)) {
			 ...
			 *
			 * The local MDT maybe not know whether the remote
			 * MDT-object has been removed or not. Then trying
			 * to get XATTR_NAME_FID EA can be used to detect
			 * its existence. Normally, the XATTR_NAME_FID EA
			 * is only used for OST-object, so if the remote
			 * MDT-object contains such EA, then it should be
			 * either wrong or not a valid MDT-object. */
