...
#define LLI_BITS			3
#define LLI_SIZE			(1 << LLI_BITS)
#define LLI_MASK			(~(LLI_SIZE - 1))

static const char lfsck_layout_name[] = "lfsck_layout";

struct lfsck_layout_inconsistency {
	__u64		lli_count[LLIT_MAX];
	spinlock_t	lli_lock;
};
...
struct lfsck_layout_master_data {
	...
	struct lfsck_layout_inconsistency llmd_lli[LLI_SIZE];
			      ...
			      struct lfsck_component *com)
{
	...
	if (lfsck->li_master) {
		...
		for (i = 0; i < LLIT_MAX; i++) {
			for (j = 0; j < LLI_SIZE; j++) {
				ram->ll_objs_repaired[i] +=
						llmd->llmd_lli[j].lli_count[i];
				llmd->llmd_lli[j].lli_count[i] = 0;
			}
		}
	}
...
static void lfsck_layout_trans_callback(struct lu_env *env, struct thandle *th,
					struct dt_txn_commit_cb *cb, int rc)
{
	...
	if (rc < 0) {
		...
		struct lfsck_layout_inconsistency *lli;

		lli = &llmd->llmd_lli[task_cpu(current) & ~LLI_MASK];
		spin_lock(&lli->lli_lock);
		lli->lli_count[type - 1]++;
					...
					struct lu_attr *la)
{
	...
	ltc = lfsck_trans_callback_init(env, com, lfsck_layout_trans_callback,
					LLIT_DANGLING);
			      ...
			      struct lfsck_component *com, bool init)
{
	...
	if (com->lc_lfsck->li_master) {
		...
		for (i = 0; i < LLI_SIZE; i++)
			memset(&llmd->llmd_lli[i].lli_count, 0,
			       sizeof(llmd->llmd_lli[i].lli_count));
	}
			     ...
			     struct lfsck_component *com, char *buf, int len)
{
	...
	if (lfsck->li_master) {
		...
		for (i = 0; i < LLIT_MAX; i++) {
			...
			for (j = 0; j < LLI_SIZE; j++)
				repaired[i] += llmd->llmd_lli[j].lli_count[i];
		}
	} else {
		for (i = 0; i < LLIT_MAX; i++)
			repaired[i] = lo->ll_objs_repaired[i];
...
int lfsck_layout_setup(const struct lu_env *env, struct lfsck_instance *lfsck)
{
	...
	if (lfsck->li_master) {
		...
		for (i = 0; i < LLI_SIZE; i++) {
			memset(&llmd->llmd_lli[i].lli_count, 0,
			       sizeof(llmd->llmd_lli[i].lli_count));
			spin_lock_init(&llmd->llmd_lli[i].lli_lock);
		}
		com->lc_data = llmd;
	} else {
...
again1:
	...
	if (rc == -ENODATA) {
		...
		if (la->la_ctime == 0) {
			...
			if (unlikely(la->la_mode & S_IXOTH)) {
				dt_read_unlock(env, obj);
	...
	rec->lor_fid.f_ver = 0;
	rc = lfsck_fid_match_idx(env, lfsck, &rec->lor_fid, it->loi_idx);
