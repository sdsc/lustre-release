						   ...
						   struct lu_buf *buf)
{
	...
	struct dt_object_format 	*dof	= &info->lti_dof;
	/* Here need to use the LOD device, then dt_xattr_set() on the
	 * LOD-object will release the stale the LOV EA cache in RAM. */
	...
	if (lfsck_obj2dev(llr->llr_parent->llo_obj) == dev) {
		parent = llr->llr_parent->llo_obj;
	} else {
		parent = lfsck_object_find_by_dev(env, dev,
