			 ...
			 __u16 gen)
{
	...
	llo->llo_obj = obj;
	/* The gen can be used to check whether some others have
	 * changed the layout during the LFSCK or not. */
	llo->llo_gen = gen;
...
static int lfsck_layout_trans_stop(const struct lu_env *env,
				   ...
				   struct lfsck_component *com,
				   struct thandle *handle, int type, int result)
{
	struct lfsck_layout_master_data *llmd	= com->lc_data;
	struct lfsck_trans_callback	*ltc;
	int				 rc;
	...
	ltc = lfsck_trans_callback_init(com, lfsck_layout_trans_callback, type);
	...
	rc = dt_trans_cb_add(handle, &ltc->ltc_cb);
	if (unlikely(rc != 0)) {
		lfsck_trans_callback_fini(env, ltc);
		rc = dt_trans_stop(env, dev, handle);
	...
	/* XXX: The trans_stop will trigger low layer (OSP) async RPC,
...
stop:
	rc = lfsck_layout_trans_stop(env, dev, com, handle, LLIT_DANGLING, rc);
...
stop:
	rc = lfsck_layout_trans_stop(env, dev, com, handle,
				     LLIT_UNMATCHED_PAIR, rc);
