				   ...
				   int result, bool init)
{
	...
	if (result > 0) {
		...
		if (lo->ll_flags & LF_CRASHED_LASTID) {
			...
			lfsck->li_notify(lfsck->li_notify_data, LNE_LASTID_REBUILT);
			lo->ll_flags &= ~LF_CRASHED_LASTID;
		}
	...
	lfsck->li_notify(lfsck->li_notify_data, LNE_LAYOUT_COMPLETED);
	return rc;
}
...
int lfsck_layout_setup(const struct lu_env *env, struct lfsck_instance *lfsck)
{
	...
	if (lo->ll_flags & LF_CRASHED_LASTID) {
		...
		lfsck->li_notify(lfsck->li_notify_data, LNE_LASTID_REBUILDING);
	}
