...
again:
	...
	if (rc != 0 || unlikely(!ldata_new.ld_leh->leh_reccount))
		GOTO(unlock2,
		     rc = (rc == -ENODATA ? 0 : rc));
					...
					struct lu_attr *la)
{
	...
	if (rc != 0 || unlikely(ldata.ld_leh->leh_overflow_time))
				      ...
				      struct lfsck_namespace *ns)
{
	...
	if (ldata->ld_leh->leh_overflow_time &&
	    cfs_time_before(ldata->ld_leh->leh_overflow_time,
...
out:
	...
	if (count == 0) {
		 ...
		 * so we cannot know whether this linkEA is valid or not.
		 * So keep it there and maybe resolved when next LFSCK run. */
		if (!(ns->ln_flags & LF_INCOMPLETE) && (ldata.ld_leh == NULL ||
		    !ldata.ld_leh->leh_overflow_time)) {
		...
		if (la->la_nlink != 0 && la->la_nlink != count) {
			...
			} else if (likely(!ldata.ld_leh->leh_overflow_time)) {
