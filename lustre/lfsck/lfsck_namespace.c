...
static int lfsck_namespace_add(const struct lu_env *env,
			       struct lfsck_component *com,
				    ...
				    struct dt_object *obj)
{
	...
	if (rc == -EINVAL) {
		rc = lfsck_namespace_add(env, com, fid);
		if (rc == 0)
				     ...
				     struct lfsck_request *lr)
{
	...
	if (lr->lr_event == LE_CREATE_ORPHAN) {
...
stop:
	...
	if (!update && rc == 0)
		lfsck_namespace_add(env, com, lfsck_dto2fid(child));
						...
						struct lfsck_assistant_req *lar)
{
	...
	if (unlikely(fid_is_zero(&lnr->lnr_fid))) {
		if (strcmp(lnr->lnr_name, dotdot) != 0)
			...
			rc = lfsck_namespace_add(env, com, pfid);
...
record:
	...
	ns->ln_mlinked_checked++;
	up_write(&com->lc_sem);
	rc = lfsck_namespace_add(env, com, &lnr->lnr_fid);
