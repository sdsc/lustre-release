...
enum lfsck_namespace_req_types {
	LNRT_LOCAL_NAMEENTRY	= 1,
};
...
lfsck_namespace_assistant_req_init(struct lfsck_instance *lfsck,
				   ...
				   enum lfsck_namespace_req_types type)
{
				    ...
				    struct lu_dirent *ent)
{
	...
	lnr = lfsck_namespace_assistant_req_init(com->lc_lfsck, ent,
						 LNRT_LOCAL_NAMEENTRY);
	if (IS_ERR(lnr)) {
		struct lfsck_namespace *ns = com->lc_file_ram;
						...
						struct lfsck_assistant_req *lar)
{
	...
	if (lnr->lnr_name[0] == '.' &&
	    (lnr->lnr_namelen == 1 ||
	     ...
	     fid_seq_is_dot_lustre(fid_seq(&lnr->lnr_fid))))
		GOTO(out, rc = 0);
			...
			const struct lu_fid *pfid)
{
	...
	rc = dt_declare_xattr_set(env, obj, buf, XATTR_NAME_LINK, fl, th);
	if (rc != 0)
	...
	rc = dt_xattr_set(env, obj, buf, XATTR_NAME_LINK, fl, th, BYPASS_CAPA);
	dt_write_unlock(env, obj);
