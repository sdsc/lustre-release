					 ...
					 const char *type, int *count)
{
	...
	if (!exist) {
		rec->rec_type = lfsck_object_type(orphan);
		rec->rec_fid = cfid;
	...
	while (ldata.ld_lee != NULL) {
		...
		if (!dt_object_exists(parent)) {
			if (!dt_object_remote(parent)) {
				...
				/* The lost remote parent will be recreated. */
				rc = lfsck_namespace_create_parent(env, com,
...
log:
	if (rc <= 0)
		CDEBUG(D_LFSCK, "%s: namespace LFSCK assistant found bad name "
		       ...
		       "in name entry %u, type claimed by child %u. repair it "
		       "by %s: rc = %d\n", lfsck_lfsck2name(lfsck),
		       PFID(lfsck_dto2fid(parent)), PFID(lfsck_dto2fid(child)),
