 ...
 * stored as XATTR_NAME_LFSCK_NAMESPACE EA. In Lustre-2.7, we intorudced
 ...
 * XATTR_NAME_LFSCK_NAMESPACE_V2 EA.
 *
				...
				struct lfsck_component *com)
{
	...
	rc = dt_xattr_get(env, com->lc_obj,
			  ...
			  XATTR_NAME_LFSCK_NAMESPACE_V2, BYPASS_CAPA);
	if (rc == len) {
				 ...
				 struct lfsck_component *com, bool init)
{
	 ...
	 * XATTR_NAME_LFSCK_NAMESPACE EA, then when downgrade to Lustre-2.x,
	 * the old LFSCK will find "invald" XATTR_NAME_LFSCK_NAMESPACE EA,
	...
	if (rc == 0 && init)
		rc = dt_xattr_set(env, obj, &tbuf,
				  XATTR_NAME_LFSCK_NAMESPACE, LU_XATTR_CREATE,
				  handle, BYPASS_CAPA);
#endif
