...
log:
	...
	CDEBUG(D_LFSCK, "%s: namespace LFSCK delete "DFID" for the "
	       "tracing file: rc = %d\n",
...
stop:
	...
	if (!update && !dec && rc == 0)
		lfsck_namespace_trace_add(env, com, cfid, 0);
					   ...
					   struct dt_object *child, __u8 flags)
{
	...
	} else if (flags & LLF_UNCERTAION_LMV) {
		down_write(&com->lc_sem);
				      ...
				      struct lfsck_component *com)
{
	struct lfsck_namespace		*ns	= com->lc_file_ram;
	struct lfsck_assistant_data	*lad	= com->lc_data;
	struct lfsck_lmv		*llmv	= com->lc_lfsck->li_lmv;
	struct lfsck_instance		*lfsck	= com->lc_lfsck;
	struct lfsck_namespace_req	*lnr;
	bool				 wakeup	= false;
	ENTRY;
	...
	/* Generate a dummy request to indicate that this striped
	...
	/* Reserve some buffer for further using. */
	lnr->lnr_size = sizeof(*lnr) + LFSCK_TMPBUF_LEN;
				    ...
				    struct lfsck_component *com)
{
	...
	if (llmv->ll_lmv_master) {
		...
		rc = lfsck_namespace_verify_stripe_slave(env, com, obj, llmv);
	}
				    ...
				    struct dt_object *obj)
{
	...
	if (rc == -EINVAL) {
		rc = lfsck_namespace_trace_add(env, com, fid, 0);
		if (rc == 0)
...
log:
	...
	case LE_SET_LMV_SLAVE: {
		...
		if (rc == 0) {
			...
			ns->ln_striped_shards_repaired++;
			up_write(&com->lc_sem);
			rc = lfsck_namespace_trace_add(env, com, &lr->lr_fid,
...
nodata:
		...
		if (unlikely(rc == -ENOSPC) &&
			...
			rc = lfsck_namespace_trace_add(env, com, &lnr->lnr_fid,
						       LLF_SKIP_NLINK);
			if (rc != 0)
...
record:
	...
	ns->ln_mul_linked_checked++;
	up_write(&com->lc_sem);
	rc = lfsck_namespace_trace_add(env, com, &lnr->lnr_fid, 0);
