				  ...
				  const struct lu_fid *fid, const __u8 flags)
{
	...
	th = dt_trans_create(env, lfsck->li_bottom);
	if (IS_ERR(th))
	...
	rc = dt_declare_insert(env, obj, (const struct dt_rec *)&new,
			       (const struct dt_key *)key, th);
	if (rc != 0)
		GOTO(log, rc);

	rc = dt_trans_start_local(env, lfsck->li_bottom, th);
	if (rc != 0)
		GOTO(log, rc);
...
log:
	if (th != NULL && !IS_ERR(th))
		dt_trans_stop(env, lfsck->li_bottom, th);

	CDEBUG(D_LFSCK, "%s: namespace LFSCK update "DFID" in the tracing"
	       "file for futher double scan, old %c, new %c: rc = %d\n",
	       lfsck_lfsck2name(lfsck), PFID(fid), old, new, rc);
...
static int __lfsck_links_read(const struct lu_env *env, struct dt_object *obj,
 ...
 * \param[in] locked	true if the caller has held the ldlm lock on the obj
 *
...
static int lfsck_namespace_links_remove(const struct lu_env *env,
					struct lfsck_component *com,
					struct dt_object *obj, bool locked)
{
	struct lfsck_instance		*lfsck	= com->lc_lfsck;
 ...
 *
 * \param[out] count	if some other inserted some linkEA entries by race,
 ...
 *
 * \see lfsck_layout_recreate_parent() for orphan name rule/format.
 *
 * \param[in] env	pointer to the thread context
 * \param[in] com	pointer to the lfsck component
 * \param[in] orphan	pointer to the orphan MDT-object
					       ...
					       __u32 type)
{
	...
	const struct lu_fid		*pfid;
	struct lfsck_instance		*lfsck	= com->lc_lfsck;
	struct dt_device		*dev	= lfsck_obj2dt_dev(orphan);
	...
	LASSERT(dt_object_exists(orphan) == 0);
	LASSERT(dt_object_remote(orphan) == 0);
	...
	pfid = lfsck_dto2fid(lfsck->li_lpf_obj);
	/* lfsck->lfsck->li_lpf_obj is attached to lfsck->li_bottom,
	 * the @parent should be attached to @dev that may be different
	 * from lfsck->li_bottom. */
...
again:
	...
	if (linkea_buf.lb_len < ldata_new.ld_leh->leh_len) {
		dt_write_unlock(env, obj);
		dt_trans_stop(env, dev, th);
		lfsck_buf_build(&linkea_buf, ldata_new.ld_buf->lb_buf,
					...
					const struct lu_name *cname)
{
	...
	/* The object has been modified by other(s), cannot replace it. */
	if (la->la_ctime != 0)
		GOTO(log, rc);
...
/**
 * Rebuild the linkEA for the object with the given ldata.
 *
 * \param[in] env	pointer to the thread context
 * \param[in] com	pointer to the lfsck component
 * \param[in] obj	pointer to the dt_object to be handled
 ...
 * \retval		negative error number on failure
 */
static int lfsck_namespace_rebuild_linkea(const struct lu_env *env,
					  struct lfsck_component *com,
					  struct dt_object *obj,
					  struct linkea_data *ldata,
					  bool locked)
...
orphan:
			...
			/* The unque linkEA is invalid, even if the ".." name
			 * entry may be valid, we still cannot know via which
			 * name entry this directory will be referenced. Then
...
orphan2:
		...
		parent = lfsck_object_find(env, lfsck, tfid);
		if (IS_ERR(parent))
			GOTO(out, rc = PTR_ERR(parent));

		/* We trust the unique linkEA entry in spite of whether it
		 * matches the ".." name entry or not. Because even if the
		 * linkEA entry is wrong and the ".." name entry is right,
		 * we still cannot know via which name entry the directory
		 * will be referenced. */
...
lost_parent:
		...
		/* The unique linkEA entry with bad parent will be handled
		 * as orphan. */
		if (unlikely(!dt_try_as_dir(env, parent))) {
			lfsck_object_put(env, parent);
			goto orphan;
		...
		rc = dt_lookup(env, parent, (struct dt_rec *)tfid,
			       ...
			       BYPASS_CAPA);
		if (rc == -ENOENT) {
			/* If the LFSCK is marked as LF_INCOMPLETE, then means
			 * some MDT has ever tried to verify some remote
			 * MDT-object that resides on this MDT, but this MDT
			 * failed to respond such request. So means there may
			 * be some remote name entry on other MDT that
			 * references this object with another name, so we
			 * cannot know whether this linkEA is valid or not.
			 * So keep it there and maybe resolved when next
			 * LFSCK run. */
			if (ns->ln_flags & LF_INCOMPLETE) {
				lfsck_object_put(env, parent);
			...
			if (unlikely(rc == -ESTALE))
				/* It may happen when the remote object has
				 * been removed, but the local MDT does not
				 * aware of that. */
				goto lost_parent;
		...
		if (!lu_fid_eq(tfid, lfsck_dto2fid(child))) {
			...
			rc = lfsck_namespace_replace_cond(env, com, parent,
							  child, tfid, cname);
			lfsck_object_put(env, parent);
			if (rc != 0)
				GOTO(out, rc);
		...
		if (!lu_fid_eq(pfid, lfsck_dto2fid(parent))) {
			...
			rc = lfsck_namespace_repair_unmatched_pairs(env, com,
				child, lfsck_dto2fid(parent), cname, locked);
		}
...
rebuild:
			...
			rc = linkea_add_buf(&ldata_new, cname,
	...
	while (ldata.ld_lee != NULL) {
		...
		LASSERT(!lu_fid_eq(tfid, lfsck_dto2fid(child)));

		type = LNIT_UNMATCHED_PAIRS;
		rc = lfsck_namespace_repair_unmatched_pairs(env, com, child,
					lfsck_dto2fid(parent), cname, locked);
		lfsck_object_put(env, parent);
					   ...
					   struct dt_object *child)
{
	...
	if (S_ISDIR(lfsck_object_type(child))) {
		dt_read_unlock(env, child);
		rc = lfsck_namespace_double_scan_dir(env, com, child);
...
lost_parent:
		...
		if (unlikely(rc == -ESTALE))
			 ...
			 * removed, but the local MDT does not aware of that. */
			goto lost_parent;
				    ...
				    struct dt_object *obj)
{
	...
	if (rc == -EINVAL) {
		rc = lfsck_namespace_update(env, com, fid, 0);
		if (rc == 0)
			rc = lfsck_namespace_links_remove(env, com, obj, false);

		GOTO(out, rc = (rc == -ENOENT ? 0 : rc));
	}

	/* XXX: zero-linkEA object may be orphan, but it also maybe because
	 *	of upgrading. Currently, we cannot record it for double scan.
	 *	Becuase it may cause the LFSCK tracing file to be too large. */
	if (rc == -ENODATA) {
...
out_create:
	...
	case LE_SKIP_NLINK_DECLARE: {
		struct thandle		*th    = lr->lr_handle;
		struct dt_object	*obj   = com->lc_obj;
		struct lu_fid		*key   = &lfsck_env_info(env)->lti_fid3;
		__u8			 flags = 0;
...
static int lfsck_namespace_find_dev_idx(const struct lu_env *env,
					struct lfsck_component *com,
 ...
 * \param[in] parent	pointer to the parent directory
 * \param[in] child	pointer to the object referenced by the name entry
 * \param[in] lnr	pointer to lfsck_namespace_req data, which contains
 *			the name entry's name/type, and other information.
 ...
 * \retval		negative error number on failure
 */
static int lfsck_namespace_repair_dirent(const struct lu_env *env,
					 struct lfsck_component *com,
					 struct dt_object *parent,
					 struct dt_object *child,
					 struct lfsck_namespace_req *lnr,
					 bool update)
{
	...
	/* XXX: We do not know the real file type for the name entry to be
	 *	removed, if it was for sub-directory, the parent's nlink
	 *	should be decreased. But since we do not know, then keep
	 *	the parent's nlink attribute unchanged. */
...
stop:
	...
	if (!update && rc == 0)
		lfsck_namespace_update(env, com, lfsck_dto2fid(child), 0);
...
log:
	if (rc <= 0) {
		...
		CDEBUG(D_LFSCK, "%s: namespace LFSCK assistant found bad name "
		       ...
		       "by %s: rc = %d\n", lfsck_lfsck2name(lfsck),
		       PFID(lfsck_dto2fid(parent)), PFID(lfsck_dto2fid(child)),
		       name, (__u32)lnr->lnr_type, lfsck_object_type(child),
		       update ? "updating" : "removing", rc);
						...
						struct lfsck_assistant_req *lar)
{
	...
	if (unlikely(fid_is_zero(&lnr->lnr_fid))) {
		if (strcmp(lnr->lnr_name, dotdot) != 0)
			...
			rc = lfsck_namespace_update(env, com, pfid, 0);
	...
	idx = lfsck_namespace_find_dev_idx(env, com, &lnr->lnr_fid);
	if (idx < 0)
		GOTO(out, rc = idx);
	...
	if (unlikely(rc == -ENOENT)) {
		...
		if (S_ISDIR(lfsck_object_type(obj))) {
			if (dt_object_remote(obj) == 0) {
				/* For local sub-dir, if there are more than
				 * one linkea entries, or the linkea entry
				 * does not match the name entry, then remove
				 * all and add the correct one. */
				remove = true;
				newdata = true;
			} else {
				/* For remote sub-dir, we cannot make sure
				 * whether the sub-dir back references the
				 * parent via ".." name entry correctly in
				 * the LFSCK first-stage scanning. It may
				 * be that the sub-dir ".." name entry has
				 * no parent FID after file-level backup/
				 * restore and its linkEA may be wrong. So
				 * under such case, we should replace the
				 * linkEA according to current name entry.
				 * But this needs to be done during the
				 * LFSCK second-stage scanning. The LFSCK
				 * will record the name entry for further
				 * possible use cases. */
				remove = false;
				newdata = false;
			}
...
record:
	...
	ns->ln_mul_linked_checked++;
	up_write(&com->lc_sem);
	rc = lfsck_namespace_update(env, com, &lnr->lnr_fid, 0);
					      ...
					      struct lu_dirent *ent)
{
	...
	if (!exist) {
		...
		/* a4. insert child's FID into the LFSCK tracing file. */
	...
	if (!exist) {
		/* b3. set child's ctime as 1 */
		rc = dt_attr_set(env, child, la, th, BYPASS_CAPA);
		if (rc != 0)
			GOTO(stop, rc);

		/* b4. insert child's FID into the LFSCK tracing file. */
		rc = dt_insert(env, obj, (const struct dt_rec *)&flags,
			       (const struct dt_key *)key, th, BYPASS_CAPA, 1);
		if (rc != 0)
			GOTO(stop, rc);
 ...
 * and put them under the backend /lost+found directroy that is invisible
 ...
 * namespace, or to the global visible orphan directroy:
