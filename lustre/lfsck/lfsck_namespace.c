 ...
 * \param[in] parent	pointer to the parent directory
 * \param[in] child	pointer to the object referenced by the name entry
 * \param[in] name	the name of the child under the parent directory
...
stop:
	...
	if (!update && !dec && rc == 0)
		lfsck_namespace_update(env, com, cfid, 0);
...
log:
	if (rc != 0) {
		...
		CDEBUG(D_LFSCK, "%s: namespace LFSCK assistant found bad name "
		       ...
		       name, type, lfsck_object_type(child),
...
again:
		...
		if (!lpf_parent && !lu_fid_eq(pfid, tfid)) {
			linkea_next_entry(&ldata);
			continue;
		}
...
rebuild:
			...
			while (ldata.ld_lee != NULL) {
				...
				if (fid_is_sane(tfid)) {
					parent = lfsck_object_find(env, lfsck,
								   tfid);
					if (IS_ERR(parent)) {
						rc = PTR_ERR(parent);
						if (rc != -ENOENT &&
					...
					rc = lfsck_namespace_repair_dirent(env,
							com, parent, child,
							cname->ln_name, S_IFDIR,
							false, true);
					lfsck_object_put(env, parent);
					if (rc < 0 &&
					    bk->lb_param & LPF_FAILOUT)
