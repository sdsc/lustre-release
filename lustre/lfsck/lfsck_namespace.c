				    ...
				    struct dt_object *obj)
{
	...
	if (!fid_is_sane(pfid)) {
		...
		if ((rc == -ENOENT) ||
			...
			/* zero-linkEA object may be orphan, but it also maybe
			 * because of upgrading. Currently, we cannot record it
			 * for double scan. Because it may cause the LFSCK trace
			 * file to be too large. */
			if (S_ISREG(lfsck_object_type(obj)) &&
			    la->la_nlink > 1 && !lfsck->li_cache_linkea)
				rc = lfsck_namespace_trace_update(env, com,
						fid, LNTF_CHECK_LINKEA, true);
		}
	}
