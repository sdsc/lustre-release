...
struct lfsck_layout {
	...
	/* The known highest target (whcih involved in the LFSCK) index. */
	__u32	ll_max_index;

	/* The size in bytes for ll_bitmap */
	__u32	ll_bitmap_size;

	/* For further using. 256-bytes aligned now. */
	__u64	ll_reserved[12];

	/* For the target (MDT/OST) involved in the LFSCK. */
	__u8	ll_bitmap[0];
};
