...
struct lfsck_instance {
	...
	struct lu_fid		  li_global_root_fid; /* /ROOT or agent ROOT */
...
static void inline lfsck_position_to_cpu(struct lfsck_position *des,
...
static void inline lfsck_position_to_le(struct lfsck_position *des,
					struct lfsck_position *src)
{
...
static inline int lfsck_is_dead_obj(const struct dt_object *obj)
{
	...
	return !!test_bit(LU_OBJECT_ORPHAN, &loh->loh_flags) ||
	       !!test_bit(LU_OBJECT_HEARD_BANSHEE, &loh->loh_flags);
}
