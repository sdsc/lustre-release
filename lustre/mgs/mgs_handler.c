...
static int mgs_setup(struct obd_device *obd, struct lustre_cfg *lcfg)
{
        ...
        obd->u.obt.obt_instance = lsi->lsi_instance;
...
static int mgs_get_lock(struct obd_device *obd, struct ldlm_res_id *res,
...
static int mgs_put_lock(struct lustre_handle *lockh)
{
        ENTRY;
...
void mgs_revoke_lock(struct obd_device *obd, struct fs_db *fsdb, int is_cfg)
{
        struct lustre_handle lockh;
        ...
        rc = mgc_fsname2resid(fsdb->fsdb_name, &res_id, is_cfg);
        LASSERT(rc == 0);

        bit = is_cfg ? FSDB_REVOKING_LOCK : FSDB_REVOKING_IR_LOCK;
...
out_nolock:
        ...
        /* Write NID Table for this target. */
        if (rc == 0) {
                rc = mgs_nidtbl_write(obd, mti);
                if (rc) {
                        LASSERT(!(mti->mti_flags & LDD_F_IR_CAPABLE));
                        CERROR("Write to nidtbl return with %d(ignore)\n", rc);
                }
        }
