...
void mgs_revoke_lock(struct obd_device *obd, struct fs_db *fsdb, int type)
{
        ...
        case CONFIG_T_CONFIG:
                ...
                if (cfs_test_and_set_bit(FSDB_REVOKING_LOCK,
                                         &fsdb->fsdb_flags))
                       rc = -EALREADY;
                break;
        ...
        if (!rc) {
                ...
                if (rc != ELDLM_OK) {
                        CERROR("can't take cfg lock for "LPX64"/"LPX64"(%d)\n",
                               res_id.name[0], res_id.name[1], rc);
...
out_nolock:
        ...
        /* Update IR status for this new target */
        if (rc == 0) {
                rc = mgs_ir_update(obd, mti);
                if (rc) {
                        LASSERT(!(mti->mti_flags & LDD_F_IR_CAPABLE));
                        CERROR("Update IR return with %d(ignore and IR disabled)\n", rc);
                }
        }
