...
/*
 * similar to ost_connect_check_sptlrpc()
 */
...
static int mgs_connect(struct tgt_session_info *tsi)
{
	...
	tgt_counter_incr(tsi->tsi_exp, LPROC_MGS_CONNECT);

	rc = mgs_connect_check_sptlrpc(req);
	/* Make clients reconnecting after a MGS restart happy */
	if (rc < 0) {
...
static int mgs_disconnect(struct tgt_session_info *tsi)
{
	...
	LASSERT(tsi->tsi_exp);
	tgt_counter_incr(tsi->tsi_exp, LPROC_MGS_DISCONNECT);
...
static int mgs_llog_open(struct tgt_session_info *tsi)
{
	...
	if (logname) {
		...
		if (ptr == NULL || len >= sizeof(mgi->mgi_fsname)) {
			LCONSOLE_WARN("%s: Invalid logname received: %s\n",
				      tgt_name(tsi), logname);
		} else {
 ...
 */
#define DEFINE_RPC_HANDLER(base, flags, opc, fn, fmt, version)		\
[opc - base] = {							\
	.th_name	= #opc,						\
...
#define DEF_OBD_HDL(flags, name, fn)					\
	DEFINE_RPC_HANDLER(OBD_FIRST_OPC, flags, name, fn, &RQF_ ## name,\
			   LUSTRE_OBD_VERSION)
...
DEF_MGS_HDL_VAR(0,			MGS_EXCEPTION,	 mgs_exception),
DEF_MGS_HDL    (HABEO_REFERO | MUTABOR,	MGS_SET_INFO,	 mgs_set_info),
DEF_MGS_HDL    (HABEO_REFERO | MUTABOR,	MGS_TARGET_REG,	 mgs_target_reg),
DEF_MGS_HDL_VAR(0,			MGS_TARGET_DEL,	 mgs_target_del),
DEF_MGS_HDL    (0,			MGS_CONFIG_READ, mgs_config_read),
};

static struct tgt_handler mgs_obd_handlers[] = {
DEF_OBD_HDL(0,	OBD_PING,	tgt_obd_ping),
};

static struct tgt_handler mgs_dlm_handlers[] = {
DEF_DLM_HDL(HABEO_CLAVIS,	LDLM_ENQUEUE,	tgt_enqueue),
};

static struct tgt_handler mgs_llog_handlers[] = {
DEF_LLOG_HDL    (0,	LLOG_ORIGIN_HANDLE_CREATE,	mgs_llog_open),
DEF_LLOG_HDL    (0,	LLOG_ORIGIN_HANDLE_NEXT_BLOCK,	tgt_llog_next_block),
DEF_LLOG_HDL    (0,	LLOG_ORIGIN_HANDLE_READ_HEADER,	tgt_llog_read_header),
DEF_LLOG_HDL_VAR(0,	LLOG_ORIGIN_HANDLE_CLOSE,	tgt_llog_close),
DEF_LLOG_HDL    (0,	LLOG_ORIGIN_HANDLE_PREV_BLOCK,	tgt_llog_prev_block),
};

static struct tgt_handler mgs_sec_ctx_handlers[] = {
DEF_SEC_HDL_VAR(0,	SEC_CTX_INIT,		mgs_sec_ctx_handle),
DEF_SEC_HDL_VAR(0,	SEC_CTX_INIT_CONT,	mgs_sec_ctx_handle),
DEF_SEC_HDL_VAR(0,	SEC_CTX_FINI,		mgs_sec_ctx_handle),
};
		     ...
		     struct lu_device_type *ldt, struct lustre_cfg *lcfg)
{
	...
	rc = tgt_register_slice(mgs_common_slice, obd->obd_type,
