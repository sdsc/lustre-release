...
int mgs_fs_setup(const struct lu_env *env, struct mgs_device *mgs)
{
	...
	mgs->mgs_obd->u.obt.obd_nodemap_config_file =
				nm_config_file_register(ncf);
	if (mgs->mgs_obd->u.obt.obd_nodemap_config_file == NULL) {
		lu_object_put(env, &ncf->do_lu);
...
int mgs_fs_cleanup(const struct lu_env *env, struct mgs_device *mgs)
{
	...
	if (mgs->mgs_obd->u.obt.obd_nodemap_config_file) {
		nm_config_file_deregister(
				mgs->mgs_obd->u.obt.obd_nodemap_config_file);
		mgs->mgs_obd->u.obt.obd_nodemap_config_file = NULL;
	}
