...
static void mgs_lcfg_replace_name(char *new_buf, const char *old_buf,
				  const char *new_name, int old_buflen,
				  int new_namelen, int old_namelen)
{
	memcpy(new_buf, new_name, new_namelen);
	memcpy(new_buf + new_namelen, old_buf + old_namelen,
	       old_buflen - old_namelen);
}
				 ...
				 struct llog_rec_hdr *o_rec, void *data)
{
	...
	if (contain_valid_fsname(o_buf, mlfd->mlfd_oldname, o_buflen,
				 o_namelen)) {
		mgs_lcfg_replace_name(n_buf, o_buf, mlfd->mlfd_newname,
				      o_buflen, n_namelen, o_namelen);
			     ...
			     const char *newname)
{
	...
	mgs_lcfg_replace_name(name_buf, mde->mde_name, newname,
			      mde->mde_len, new_namelen, old_namelen);
...
int mgs_lcfg_rename(const struct lu_env *env, struct mgs_device *mgs)
{
	...
	list_for_each_entry_safe(dirent, n, &log_list, mde_list) {
		...
		rc = dt_xattr_get(env, o, &buf, XATTR_USER_RENAME);
		if (rc < 0) {
			if (rc == -ENODATA)
				rc = 0;
			else
		...
		if (rc) {
			/* May because of un-removed old config file. */
