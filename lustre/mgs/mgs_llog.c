...
static void mgs_fsdb_pool_dump(struct fs_db *fsdb)
{
...
mgs_fsdb_pool_get(struct fs_db *fsdb, const char *poolname)
{
	...
	cfs_list_for_each_entry(pool, &fsdb->fsdb_pools,
				mp_fsdb_list) {
		if (strlen(pool->mp_poolname) == strlen(poolname) &&
		    strcmp(pool->mp_poolname, poolname) == 0) {
			return pool;
		}
	}
			    ...
			    struct llog_rec_hdr *rec, void *data)
{
	...
	if (!test_bit(FSDB_OSCNAME18, &fsdb->fsdb_flags) &&
		...
		if (d->skip)
			RETURN(0);
		/* pool new   0:  1:fsname  2:pool_name */
		if (lcfg->lcfg_command == LCFG_POOL_NEW) {
			CERROR("string: %s %s %s %s\n",
...
{
        struct llog_handle *llh = NULL;
	char pool_id_str[6]; /* 16 bit interger */
        int rc;
	...
	if (rc)
		goto out;
	rc = record_base(env, llh, lovname, 0, cmd,
			 poolname, fsname, ostname, pool_id_str);
	if (rc)
		goto out;
