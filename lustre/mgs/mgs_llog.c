...
static int mgs_write_log_param2(const struct lu_env *env, struct mgs_device *mgs,
				struct fs_db *fsdb,
		 ...
		 struct lustre_cfg *lcfg, char *fsname)
{
	...
	rc = mgs_find_or_make_fsdb(env, mgs,
				   lcfg->lcfg_command == LCFG_PARAM2 ?
				   PARAMS_FILENAME : fsname, &fsdb);
	if (rc)
		RETURN(rc);

	if (lcfg->lcfg_command != LCFG_PARAM2 &&
