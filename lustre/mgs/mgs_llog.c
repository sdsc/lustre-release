			      ...
			      struct llog_rec_hdr *rec, void *data)
{
	...
	int cfg_len = REC_DATA_LEN(rec);
		...
		rc = llog_write(env, llh, rec, NULL, 0, rec->lrh_index);
                if (!rc)
                         mml->mml_modified++;
        }
...
{
	/* idx = -1 means append */
	return llog_write(env, llh, &lcr->lcr_hdr, NULL, 0, -1);
}
...
copy_out:
	...
	rc = llog_write(env, mrul->temp_llh, rec, NULL, 0, -1);
