...
out:
        if (rc)
                CERROR("Failed to backup log %s (%d)\n", fsname, rc);
        RETURN(rc);
}

static int mgs_log_is_empty(const struct lu_env *env, struct mgs_device *mgs, char *name);

static int mgs_replace_nids_log(const struct lu_env *env, struct obd_device *mgs, struct fs_db *fsdb,
				char *logname, char *devname, char *nids)
{
	...
	if (rc < 0) {
		CERROR( "Can't make backup for %s\n", logname);
		GOTO(out_free,rc);
	}
...
out_restore:
	if (rc) {
		CERROR("rc: %d, llog should be restored\n", rc);
		rc2 = mgs_backup_llog(env, mgs, backup, logname);
		if (rc2 < 0)
			CERROR( "Can't restore backup %s\n", logname);
	}
		     ...
		     char *devname, char *nids)
{
	...
	cfs_spin_lock(&mgs_obd->obd_dev_lock);
	conn_state = mgs_obd->obd_no_conn;
	mgs_obd->obd_no_conn = 1;
	cfs_spin_unlock(&mgs_obd->obd_dev_lock);
