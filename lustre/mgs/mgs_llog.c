		    ...
		    const char *param)
{
	...
	case LCFG_NODEMAP_ADMIN:
		...
		rc = nodemap_admin(nodemap_name, bool_switch);
		break;
	case LCFG_NODEMAP_TRUSTED:
		bool_switch = simple_strtoul(param, NULL, 10);
		rc = nodemap_trusted(nodemap_name, bool_switch);
		break;
	case LCFG_NODEMAP_SQUASH_UID:
		int_id = simple_strtoul(param, NULL, 10);
		rc = nodemap_squash_uid(nodemap_name, int_id);
		break;
	case LCFG_NODEMAP_SQUASH_GID:
		int_id = simple_strtoul(param, NULL, 10);
		rc = nodemap_squash_gid(nodemap_name, int_id);
		break;
	...
	case LCFG_NODEMAP_ADD_GIDMAP:
		...
		if (cmd == LCFG_NODEMAP_ADD_UIDMAP)
			rc = nodemap_add_idmap(nodemap_name, 0, idmap);
		else
			rc = nodemap_add_idmap(nodemap_name, 1, idmap);
		break;
	...
	case LCFG_NODEMAP_DEL_GIDMAP:
		...
		if (cmd == LCFG_NODEMAP_DEL_UIDMAP)
			rc = nodemap_del_idmap(nodemap_name, 0, idmap);
		else
			rc = nodemap_del_idmap(nodemap_name, 1, idmap);
		break;
	default:
		rc = -EINVAL;
	}
