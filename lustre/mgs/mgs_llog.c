...
{
	return record_base(obd, llh, NULL, nid, LCFG_ADD_UUID,
			   uuid, 0, 0, 0);
}
...
static int record_add_conn(struct obd_device *obd,
			   ...
			   char *uuid)
{
	return record_base(obd, llh, devname, 0, LCFG_ADD_CONN,
			   uuid, 0, 0, 0);
}
 ...
 * \retval 0    success
 */
static int mgs_replace_handler(struct llog_handle *llh,
			       struct llog_rec_hdr *rec,
			       void *data)
{
				...
				char *logname, char *devname, char *nids)
{
	...
	/* open local log */
	rc = llog_create(ctxt, &temp_llh, NULL, temp_log);
	if (rc)
		GOTO(out_free, rc);
	...
	/* open original llog */
	rc = llog_create(ctxt, &orig_llh, NULL, logname);
	if (rc)
		GOTO(out_closel, rc);
...
out_closel:
	...
	if (!rc)
		rc = mgs_replace_with_backup(obd, &obd->u.mgs, temp_log,
					     logname);
...
static int only_mgs_is_running(struct obd_device *mgs_obd)
{
	...
	return (num_devices <= 2) && (mgs_obd->obd_num_exports <= 2);
}
