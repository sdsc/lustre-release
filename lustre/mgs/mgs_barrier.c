				    ...
				    __u32 timeout, __u32 expected)
{
	...
	rc = mgc_fsname2resid(fsdb->fsdb_name, &res_id, CONFIG_T_BARRIER);
	if (rc)
		RETURN(rc);
				     ...
				     const char *name)
{
	...
	c_fsdb = mgs_find_fsdb(mgs, name, false);
	if (likely(c_fsdb)) {
			      ...
			      struct barrier_ctl *bc)
{
	...
	fsdb = mgs_find_fsdb(mgs, name, false);
	if (!fsdb) {
			      ...
			      struct barrier_ctl *bc)
{
	...
	c_fsdb = mgs_find_fsdb(mgs, bc->bc_name, false);
	if (!c_fsdb) {
		mutex_unlock(&mgs->mgs_mutex);
		up_write(&mgs->mgs_barrier_rwsem);
	...
	sprintf(name, "%s.%s", bc->bc_name, BARRIER_FILENAME);
	b_fsdb = mgs_find_fsdb(mgs, name, false);
	if (!b_fsdb) {
		mutex_unlock(&mgs->mgs_mutex);
		up_write(&mgs->mgs_barrier_rwsem);
		mgs_put_fsdb(mgs, c_fsdb);
	...
	case BS_EXPIRED:
	case BS_FAILED:
		b_fsdb->fsdb_barrier_latest_create_time = cfs_time_current_sec();
