 ...
 */
static int nidtbl_read(struct obd_device *unused, struct mgs_nidtbl *tbl,
...
int nidtbl_write(struct obd_device *obd, struct mgs_target_info *mti)
{
        struct fs_db             *fsdb;
...
out:
        ...
        if (rc == 0) {
                CDEBUG(D_MGS, "Try to revoke recover lock of %s\n",
                       fsdb->fsdb_name);
                (void)mgs_revoke_ir_lock(obd, fsdb);
        }

        return rc;
}

void nidtbl_fini_fs(struct obd_device *obd, struct fs_db *fsdb)
{
...
int nidtbl_init_fs(struct obd_device *obd, struct fs_db *fsdb)
{
        struct mgs_nidtbl *tbl = &fsdb->fsdb_nidtbl;
        ...
        if (tbl->mn_version < 0) {
                CERROR("Fatal error on imperative recovery %llu, disabled\n",
                       tbl->mn_version);
                tbl->mn_state = IR_DISABLED;
        }
        return 0;
}
...
int mgs_get_ir_logs(struct ptlrpc_request *req)
{
        ...
        if (fsdb)
                rc = nidtbl_read(obd, &fsdb->fsdb_nidtbl, vers,
                                 type == IR_CLIENT,
                                 buf, LLOG_CHUNK_SIZE);
        cfs_up(&mgs->mgs_sem);
        if (fsdb == NULL)
...
static int lprocfs_ir_set_fsname(struct obd_device *obd,
                                  struct fs_db **dbp,
                                  const char *buf)
{
        struct mgs_obd *mgs = &obd->u.mgs;
        ...
        cfs_down(&mgs->mgs_sem);
        fsdb = mgs_find_fsdb(obd, (char *)buf);
        cfs_up(&mgs->mgs_sem);
        if (fsdb == NULL)
                return -EINVAL;
...
static int lprocfs_ir_set_status(struct obd_device *obd,
                                  struct fs_db **dbp,
                                  const char *buf)
{
        struct fs_db    *fsdb;
...
static int lprocfs_ir_set_timeout(struct obd_device *obd,
                                   struct fs_db **dbp,
                                   const char *buf)
{
        return -EINVAL;
...
static struct lproc_ir_cmd {
        char *name;
        int   namelen;
        int (*handler)(struct obd_device *, struct fs_db **, const char *);
} ir_cmds[] = {
        { "fsname=",  7, lprocfs_ir_set_fsname },
                         ...
                         unsigned long count, void *data)
{
        ...
        kbuf[count - 1] = 0; /* buffer is supposed to end with 0 */
        ptr = kbuf;
