...
int mgs_ir_init_fs(struct obd_device *obd, struct fs_db *fsdb)
{
        ...
        fsdb->fsdb_start_time = cfs_time_current_sec();
        ...
        rc = cfs_kernel_thread(mgs_ir_notify, fsdb, CLONE_VM|CLONE_FILES);
        if (rc > 0) {
...
int mgs_ir_update(struct obd_device *obd, struct mgs_target_info *mti)
{
        ...
        case IR_STARTUP:
                if (cfs_time_before(fsdb->fsdb_start_time + obd_timeout * 5,
...
int mgs_fsc_attach(struct obd_export *exp, char *fsname)
{
        ...
        new_fsc->mfc_ir_capable = !!(exp->exp_connect_flags & OBD_CONNECT_IR);
...
int mgs_fsc_connect(struct obd_export *exp)
{
        struct obd_device      *obd  = exp->exp_obd;
        struct mgs_export_data *data = &exp->u.eu_mgs_data;
