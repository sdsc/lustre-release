...
int test5(void)
{
	...
	for (i = 0; i < 1000; i++) {
		...
		ASSERTF(rc == -EWOULDBLOCK, "rc=%d", rc);
	}
...
int test7(void)
{
	...
	ASSERTF(fd >= 0, "fd=%d", fd);

	/* Ensure it's read-only */
	rc = write(fd, &rc, 1);
	ASSERTF(rc == -1 && errno == EBADF, "rc=%d, errno=%d", rc, errno);
...
static int create_testfile(size_t length)
{
	...
	ASSERTF(fd >= 0, "fd=%d, errno=%d, file=%s", fd, errno, testfile);

	rc = ftruncate(fd, length);
	ASSERTF(rc == 0, "rc=%d, errno=%d, file=%s", rc, errno, testfile);
...
void test101(void)
{
	...
	void test100_progress(struct hsm_copyaction_private *hcp)
	{
		int i;
		int rc;
	...
	helper_progress = test100_progress;
	helper_archiving(length);
}
...
void test109(void)
{
	...
	void test100_progress(struct hsm_copyaction_private *hcp)
	{
		...
		ASSERTF(hca.hca_action == HUA_ARCHIVE,
			"hca_state=%u", hca.hca_action);
		ASSERTF(hca.hca_location.length == 0xffffffffffffffff,
			"length=%llu", hca.hca_location.length);
	}
