...
SIMUL=${SIMUL:-/testsuite/tests/x86_64/rhel5/simul/simul}
IOR=${IOR:-/testsuite/tests/x86_64/rhel5/IOR/src/C/IOR}
MDTEST=${MDTEST:-/usr/bin/mdtest}
...
ha_wait_nodes()
{
	...
	until pdsh -w $nodes -S hostname >/dev/null 2>&1 ||
          [ -e "$ha_stop_file" ] ||
          (($(date +%s) >= end)); do
        ha_sleep 1 >/dev/null
    done
...
ha_main()
{
	...
	ha_on ${ha_clients[0]} " \
		/usr/bin/lfs setstripe $ha_stripe_params $ha_test_dir"
