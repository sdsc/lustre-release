...
test_2 () {
	...
	for num in $(seq $OSTCOUNT); do
		...
		add ost${num} $(mkfs_opts ost${num} $dev) $FSTYPE_OPT --reformat \
			$(ostdevname $num) $(ostvdevname $num) > /dev/null ||
			error "format ost${num} error"
		if [ $fstype == zfs ]; then
			import_zpool ost${num}
			do_facet ost${num} "$ZFS set canmount=on $dev; $ZFS set mountpoint=legacy $dev; $ZFS list $dev"
		fi
