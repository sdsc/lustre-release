...
test_4 () {
    ...
    do_rpc_nodes $(facet_host ost1) run_llverfs $MOUNT -vp "no" || rc=${PIPESTATUS[0]}
    client_df $MOUNT
    [ $rc -ne 0 ] && return $rc
    ...
    if $FULL_MODE; then
        ...
        do_rpc_nodes $(facet_host ost1) run_llverfs $MOUNT -vl "no" || rc=${PIPESTATUS[0]}
        client_df $MOUNT
        [ $rc -ne 0 ] && return $rc
