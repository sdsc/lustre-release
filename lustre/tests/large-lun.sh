...
do_fsck() {
    $RUN_FSCK || return
    local dev
    run_e2fsck $(facet_host $SINGLEMDS) $(mdsdevname ${SINGLEMDS//mds/}) "-y" ||
...
test_2 () {
    ...
    for num in $(seq $OSTCOUNT); do
        ...
        add ost${num} $(mkfs_opts ost${num}) $FSTYPE_OPT --reformat \
            $(ostdevname $num) > /dev/null || error "format ost${num} error"
        ...
        # ensure that the kernel can perform filesystem operations on the
        # complete device without any errors.
...
test_4 () {
    ...
    if $FULL_MODE; then
        ...
        run_llverfs $MOUNT -vl "no" || error "run_llverfs error on lustre"
        client_df $MOUNT
