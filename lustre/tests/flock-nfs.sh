...
test_4() {
	nfs_start || return 1
#define OBD_FAIL_LLITE_FLOCK_UNLOCK_RACE       0x140c
	do_node $lustre_client "$LCTL set_param fail_loc=0x140c"
	do_node $lustre_client \
		"flocks_test 5 set write sleep 10 $LUSTRE_MNT/$LOCKF" &
	sleep 1
...
test_5() {
	...
	do_node $lustre_client \
		"flocks_test 5 set write sleep 10 $LUSTRE_MNT/$LOCKF" &
	sleep 1
#define OBD_FAIL_LLITE_FLOCK_BL_GRANT_RACE     0x140d
	do_node $lustre_client "$LCTL set_param fail_loc=0x140d"
	do_node $NFS_CLIENT "flocks_test 5 set write sleep 5 $LUSTRE_MNT/$LOCKF"
	wait
