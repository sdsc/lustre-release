 ...
 *		modes
 * 2) Punch: punch holes in a non-sparse files at different offsets
 */
			  ...
			  loff_t expected_new_size)
{
	...
	if (st.st_size != expected_new_size) {
		...
		fprintf(stderr, "mode %d, offset %llu, len %llu, "
			"new_size %lu, expected_new_size %lu\n", mode,
			(unsigned long long)offset, (unsigned long long)len,
			st.st_size, expected_new_size);
		rc = -1;
		goto out;
	}
...
int test_punch(int fd)
{
	...
	expected_new_size = st.st_size;
	mode = FALLOC_FL_PUNCH_HOLE | FALLOC_FL_KEEP_SIZE;
	/* Punch at the beginning of the file */
