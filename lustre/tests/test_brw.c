...
int block_debug_check(char *who, void *addr, int size, uint64_t off, uint64_t id)
{
	uint64_t ne_off;
		...
		CERROR("%s: for offset %"PRIu64" off: %#"PRIx64" != %#"PRIx64"\n",
		       who, off, *(uint64_t *)addr, ne_off);
                err = -EINVAL;
        }
        if (memcmp(addr + LPDS, (char *)&id, LPDS)) {
		CERROR("%s: for offset %"PRIu64" id: %#"PRIx64" != %#"PRIx64"\n",
        ...
        addr += size - LPDS - LPDS;
        if (memcmp(addr, (char *)&ne_off, LPDS)) {
		CERROR("%s: for offset %"PRIu64" end off: %#"PRIx64" != %#"PRIx64"\n",
		       who, off, *(uint64_t *)addr, ne_off);
                err = -EINVAL;
        }
        if (memcmp(addr + LPDS, (char *)&id, LPDS)) {
		CERROR("%s: for offset %"PRIu64" end id: %#"PRIx64" != %#"PRIx64"\n",
		       who, off, *(uint64_t *)(addr + LPDS), id);
                err = -EINVAL;
        }
