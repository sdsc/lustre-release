...
mount_after_interval_sub() {
    ...
    if [ $second_mount_rc -eq 0 -a $first_mount_rc -eq 0 ]; then
        ...
        error_noexit \
        "failed to mount on the failover pair $facet,$failover_facet"
...
# Run delay e2fsck on the Lustre server target.
run_delay_e2fsck() {
	local facet=$1
	shift
	local device=$1
...
test_8() {
	...
	do_facet $MMP_MDS "$DEBUGFS -w -R 'ssv free_block_count 0' $MMP_MDSDEV
	...
	do_facet $MMP_OSS "$DEBUGFS -w -R 'ssv free_block_count 0' $MMP_OSTDEV
