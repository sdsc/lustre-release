 ...
 *
 * Adapted from group_lock_test.c by Frank Zago
 *
 * A few portions are extracted from llapi_layout_test.c
 *
 * The purpose of this test is to exercise the new lock ahead ioctl.
 *
...
/* Test single lock ahead request: Valid and invalid */
/*static void test10(void)
{
	int rc;
	int fd;
	...
	* Allocate the ioctl arg *
	count = 1;
	arg = malloc(sizeof(struct llapi_lock_ahead) +
				sizeof(struct lu_extent)*count);

	* Define the lock request *
	arg->lla_version=1;
