...
static const char *maindir = "lock_ahead_test_name_65436563";
...
static void test10(void)
{
	...
	lla = llapi_alloc_lla(count, MODE_WRITE_USER, 0);
...
static void test16(void)
{
	...
	rc = llapi_lock_ahead_one(fd, 0, write_size - 1, MODE_WRITE_USER, 0);
	ASSERTF(rc == 0,
		"cannot lock ahead '%s': %s", mainpath, strerror(errno));
...
static void test20(void)
{
	...
	/* bogus flags, 0x80 */
	lla = llapi_alloc_lla(count, MODE_WRITE_USER, 0x80);
	lla->lla_extents[0].end = 1024 * 1024;
	rc = llapi_lock_ahead(fd, lla);
	ASSERTF(rc == -1 && errno == EINVAL,
	...
	lla = llapi_alloc_lla(count, MODE_WRITE_USER, 0xff);
	lla->lla_extents[0].end = 1024 * 1024;
	rc = llapi_lock_ahead(fd, lla);
	ASSERTF(rc == -1 && errno == EINVAL,
		"unexpected return for bogus flags: %u %d %s",
	...
	lla = llapi_alloc_lla(count, MODE_WRITE_USER, 0xffffffff);
	lla->lla_extents[0].end = 1024 * 1024;
	rc = llapi_lock_ahead(fd, lla);
	ASSERTF(rc == -1 && errno == EINVAL,
		"unexpected return for bogus flags: %u %d %s",
