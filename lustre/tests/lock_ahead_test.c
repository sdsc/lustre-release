...
static struct llapi_lock_ahead *alloc_lla(size_t count)
{
	struct llapi_lock_ahead *lla;
	size_t len;

	len = sizeof(struct llapi_lock_ahead) +
		sizeof(struct lu_extent) * count;

	lla = calloc(1, len);
	ASSERTF(lla != NULL, "cannot allocate %zu bytes", len);

	lla->lla_version = 1;
	lla->lla_lock_mode = READ_USER;
