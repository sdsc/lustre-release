...
pdo_sched() {
			# how long 40-46 take with specific delay
	#usleep 150000 # 93s
	usleep 300000 # 117s
	#usleep 400000 # 136s
	#usleep 500000 # 153s
...
unblock_and_wait() {
	do_facet $SINGLEMDS lctl set_param fail_intr=1
	wait $1
}
...
#define OBD_FAIL_ONCE|OBD_FAIL_MDS_PDO_LOCK    0x145
	...
	PID2=$! ; pdo_sched
	do_facet $SINGLEMDS lctl set_param fail_intr=1
	check_pdo_conflict $PID1 && { unblock_and_wait $PID1; error "mkdir isn't blocked"; }
	wait $PID2 ; [ $? -eq 0 ] || error "mkdir must succeed"
	rm -r $DIR1/*
	return 0
}
...
#define OBD_FAIL_ONCE|OBD_FAIL_MDS_PDO_LOCK    0x145
	...
	PID2=$! ; pdo_sched
	do_facet $SINGLEMDS lctl set_param fail_intr=1
	check_pdo_conflict $PID1 && { unblock_and_wait $PID1; error "create isn't blocked"; }
	wait $PID2 ; [ $? -eq 0 ] || error "create must succeed"
	rm -r $DIR1/*
	return 0
}
...
#define OBD_FAIL_ONCE|OBD_FAIL_MDS_PDO_LOCK    0x145
	...
	PID2=$! ; pdo_sched
	do_facet $SINGLEMDS lctl set_param fail_intr=1
	check_pdo_conflict $PID1 && { unblock_and_wait $PID1; error "link isn't blocked"; }
	wait $PID2 ; [ $? -eq 0 ] || error "link must succeed"
	rm -r $DIR1/*
	return 0
}
...
#define OBD_FAIL_ONCE|OBD_FAIL_MDS_PDO_LOCK    0x145
	...
	PID2=$! ; pdo_sched
	do_facet $SINGLEMDS lctl set_param fail_intr=1
	check_pdo_conflict $PID1 && { unblock_and_wait $PID1; error "unlink isn't blocked"; }
	wait $PID2 ; [ $? -ne 0 ] || error "unlink must fail"
	rm -r $DIR1/*
	return 0
}
...
#define OBD_FAIL_ONCE|OBD_FAIL_MDS_PDO_LOCK    0x145
	...
	PID2=$! ; pdo_sched
	do_facet $SINGLEMDS lctl set_param fail_intr=1
	check_pdo_conflict $PID1 && { unblock_and_wait $PID1; error "rename isn't blocked"; }
	wait $PID2 ; [ $? -eq 0 ] || error "rename must succeed"
	rm -r $DIR1/*
	return 0
}
...
#define OBD_FAIL_ONCE|OBD_FAIL_MDS_PDO_LOCK    0x145
	...
	PID2=$! ; pdo_sched
	do_facet $SINGLEMDS lctl set_param fail_intr=1
	check_pdo_conflict $PID1 && { unblock_and_wait $PID1; error "getattr isn't blocked"; }
	wait $PID2 ; [ $? -ne 0 ] || error "stat must fail"
	rm -r $DIR1/*
	return 0
}
...
#define OBD_FAIL_ONCE|OBD_FAIL_MDS_PDO_LOCK    0x145
	...
	ls -lia $DIR2/ > /dev/null &
	do_facet $SINGLEMDS lctl set_param fail_intr=1
	check_pdo_conflict $PID1 && { unblock_and_wait $PID1; error "readdir isn't blocked"; }
	wait $PID2
	rm -r $DIR1/*
	return 0
}
