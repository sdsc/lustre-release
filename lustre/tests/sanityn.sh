...
nanotime_supported() {
	echo "$(lctl get_param mdc.*-mdc-*.connect_flags | \
...
test_26b() {
	...
	if [ -n "$(nanotime_supported)" ]; then
		if [ "$mt1_ns" != "$mt2_ns" ]; then
			error "not equal nano mtime, client1: "\
			      $mt1_ns", client2: "$mt2_ns"."
		fi
	fi
}
...
test_39a() {
	...
	if [ -n "$(nanotime_supported)" ]; then
		do_node $client1 "touch -m -d "\
			"@$TEST_39_MTIME.$TEST_39_MTIMENS $DIR1/$tfile"
	else
	...
	if [ -n "$(nanotime_supported)" ]; then
		...
		mtime1_ns=`echo $mtime1_ns | awk '{ print $2 }' \
			   | sed 's/^.*\.\(.*\)$/\1/g'`
		[ "$mtime1_ns" = "$TEST_39_MTIMENS" ] ||
			error "nano mtime is not set to past: $mtime1_ns, "\
			      "should be $TEST_39_MTIMENS"
	fi
	...
	if [ -n "$(nanotime_supported)" ]; then
		...
		mtime2_ns=`echo $mtime2_ns | awk '{ print $2 }' \
			   | sed 's/^.*\.\(.*\)$/\1/g'`
	fi
	...
	for (( i=0; i < 2; i++ )) ; do
		...
		if [ -n "$(nanotime_supported)" ]; then
			local mtime3_ns="`do_node $client2 \
					  "stat -c '%y' $DIR1/$tfile-1"`"
			mtime3_ns=`echo $mtime3_ns | awk '{ print $2 }' \
				   | sed 's/^.*\.\(.*\)$/\1/g'`
			[ "$mtime2_ns" = "$mtime3_ns" ] ||
				error "nano mtime ($mtime2_ns) changed "\
				      "(to $mtime3_ns) on rename"
		fi
...
test_39b() {
	...
	for (( i=0; i < 2; i++ )) ; do
		...
		if [ -n "$(nanotime_supported)" ]; then
			...
			mtime4_ns=`echo $mtime4_ns | awk '{ print $2 }' \
				   | sed 's/^.*\.\(.*\)$/\1/g'`
			[ "$mtime3_ns" = "$mtime4_ns" ] ||
				error "different nano mtime on clients: "\
				      "$mtime3_ns, $mtime4_ns"
			[ "$mtime3_ns" = $TEST_39_MTIMENS ] ||
				error "lost nano mtime: $mtime3_ns, should "\
				      "be $TEST_39_MTIMENS"
		fi
...
test_39c() {
	...
	if [ -n "$(nanotime_supported)" ]; then
		...
		mtime2_ns=`echo $mtime2_ns | awk '{ print $2 }' \
			   | sed 's/^.*\.\(.*\)$/\1/g'`
		[ "$mtime1_ns" = "$mtime2_ns" ] ||
			error "create: different nano mtime on clients: "\
			      "$mtime1_ns, $mtime2_ns"
	fi
		...
		if [ -n "$(nanotime_supported)" ]; then
			...
			[ "$mtime3_ns" = "$mtime4_ns" ] ||
				error "different nano mtime on clients: "\
				      "$mtime3_ns, $mtime4_ns"
			[ "$mtime3_ns" != "$mtime2_ns" ] ||
				error "truncate did not update nano mtime: "\
				      "$mtime2_ns, $mtime3_ns"
		fi
