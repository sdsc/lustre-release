...
test_19() { # bug3811
	...
	if ! get_ofd_param $node '' read_cache_enable >/dev/null; then
		echo "not cache-capable obdfilter"
		return 0
	fi

	local MAX=$(get_ofd_param $node '' readcache_max_filesize | \
		    head -n 1)
	set_ofd_param $node '' readcache_max_filesize 4096
	...
	set_ofd_param $node '' readcache_max_filesize $MAX
	rm $DIR1/$tfile
}
