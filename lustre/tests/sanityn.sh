...
test_80b() {
	...
	(
		while true; do
			...
			$LFS mv -M $mdt_idx $migrate_dir1 2&>/dev/null || rc=$?
	...
	while true; do
		...
		touch $migrate_dir2/source_file || rc1=$?
		[ $rc1 -ne 0 -o $rc1 -ne 1 ] ||
			error "touch file failed with $rc1"
		...
		[ -z $(ps auxwww | grep -v grep | grep -q $migrate_pid)] ||
