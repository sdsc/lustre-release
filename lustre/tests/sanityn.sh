...
test_77e() {
	...
	# Add some rules
	tbf_rule_operate ost1 "start\ localhost\ id={0@lo}\ rate=1000"
	local address=$(comma_list "$(host_nids_address $CLIENTS $NETTYPE)")
	local client_nids=$(nids_list $address "\\")
	tbf_rule_operate ost1 "start\ clients\ id={$client_nids}\ rate=100"
	tbf_rule_operate ost1 "start\ others\ id={*.*.*.*@$NETTYPE}\ rate=50"
	nrs_write_read
...
test_77f() {
	...
	# Only operate rules on ost1 since OSTs might run on the same OSS
	# Add some rules
	tbf_rule_operate ost1 "start\ runas\ id={iozone.$RUNAS_ID\ dd.$RUNAS_ID\ tiotest.$RUNAS_ID}\ rate=1000"
	tbf_rule_operate ost1 "start\ iozone_runas\ id={iozone.$RUNAS_ID}\ rate=100"
	tbf_rule_operate ost1 "start\ dd_runas\ id={dd.$RUNAS_ID}\ rate=50"
...
test_77h() {
	...
	tbf_rule_check ost1 "default" \
		"error before inserting any rule"

	tbf_rule_operate ost1 "start\ before\ id={jobid}\ rate=1000"
	tbf_rule_check ost1 "before default" \
		"error when inserting rule \"before\""

	tbf_rule_operate ost1 "start\ after\ id={jobid}\ rate=1000\ rank=default"
	tbf_rule_check ost1 "before after default" \
		"error when inserting rule \"after\""

	tbf_rule_operate ost1 "start\ target\ id={jobid}\ rate=1000\ rank=after"
