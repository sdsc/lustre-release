...
test_33b() {
	...
	echo "COS=0 transactions (avg): $cos0_jbd  time (avg): $cos0_time"
	echo "COS=1 transactions (avg): $cos1_jbd  time (avg): $cos1_time"
	[ "$cos0_jbd" != 0 ] && echo "COS=1 vs COS=0 jbd: \
				      $((((cos1_jbd/cos0_jbd - 1)) * 100 )) %"
	[ "$cos0_time" != 0 ] && echo "COS=1 vs COS=0 time: \
				      $((((cos1_time/cos0_time - 1)) * 100 )) %"

	restore_lustre_params < $param_file
	rm -f $param_file
	return 0
}
run_test 33b "COM: cross create/delete, 2 clients, benchmark under remote dir"
...
test_71() {
	...
	cd $DIR2/$remote_dir || error "exit remote dir"
	rm -rf $DIR1/$remote_dir || error "unlink directory failed"
