...
test_38() { # LU-247, based on the code of test_32b
	remote_ost_nodsh && skip "remote OST with nodsh" && return
	...
	# 3. unaligned "lockless DIRECT_IO" write of 192 bytes in the end of
	#    file B
	# 4. compare A and B
	...
	multiop $DIR1/$tfile oO_CREAT:O_DIRECT:O_RDWR:b4000c ||
		error "first multiop failed"
	multiop $DIR1/$tfile oO_CREAT:O_DIRECT:O_RDWR:z4000b4000c ||
		error "second multiop failed"
	save_lustre_params $HOSTNAME "osc.*.contention_seconds" > $p
	for node in $(osts_nodes); do
		save_lustre_params $node \
	...
	# agressive lockless i/o settings
	for node in $(osts_nodes); do
		do_node $node 'lctl set_param -n ' \
			'ldlm.namespaces.filter-*.max_nolock_bytes 2000000; ' \
			'lctl set_param -n ' \
	...
	for i in $(seq 5); do
		multiop $DIR1/$tfile o:O_DIRECT:O_RDWR:z8000b192c ||
			error "the last multiop failed"
		multiop $DIR2/$tfile o:O_DIRECT:O_RDWR:z8000b192c ||
			error "the last multiop failed"
	done
	[ $(calc_osc_stats lockless_write_bytes) -ne 0 ] ||
		error "lockless i/o was not triggered"
	restore_lustre_params <$p
	log "comparing"
	cmp $DIR1/$tfile $random ||
		error "O_DIRECT+lockless results do not match the original file"
	rm -f $DIR1/$tfile
	rm -f $p
	rm -f $random
}
