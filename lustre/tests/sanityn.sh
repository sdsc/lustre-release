...
test_515a() {
	...
	test_mkdir -p $DIR1/$tdir || error "mkdir $DIR1/$tdir failed"
	mkdir -p $TMP/$tdir || error "mkdir $TMP/$tdir failed"
	$SETSTRIPE -c -1 -S $STRP $DIR1/$tdir || error "setstripe failed"

	$MULTIOP $TMP/$tdir/$tfile oO_CREAT:O_RDWR:w${IOSZ}yc || \
		errro "multiop failed"
	$MULTIOP $TMP/$tdir/$tfile oO_RDWR:z${OFFSET}w${IOSZ}yc || \
	...
	while [ $i -lt $LOOP ]; do
		echo -n "."
		$MULTIOP $DIR1/$tdir/$tfile oO_CREAT:O_RDWR:w${IOSZ}c || \
			errro "multiop failed"
		$MULTIOP $DIR2/$tdir/$tfile oO_RDWR:z${OFFSET}w${IOSZ}c || \
			error "multiop failed"
		cmp $DIR1/$tdir/$tfile $TMP/$tdir/$tfile || \
			error "file contents different"
		cmp $DIR1/$tdir/$tfile $DIR2/$tdir/$tfile || \
			error "file contents different"
		let i=i+1
	done
	echo
...
run_test 515a "writes a part of the same extent with iosvc_enable=0"
...
run_test 515b "writes a part of the same extent with iosvc_enable=1"
...
test_515ca() {
	...
	$MULTIOP $TMP/$tdir/$tfile oO_CREAT:O_RDWR:w${IOSZ}yc || \
		errro "multiop failed"
	$MULTIOP $TMP/$tdir/$tfile oO_RDWR:z${OFFSET}w$((${IOSZ} * 2))yc \
		|| error "multiop failed"
	...
	while [ $i -lt $LOOP ]; do
		...
		$MULTIOP $DIR1/$tdir/$tfile oO_CREAT:O_RDWR:w${IOSZ}c || \
			errro "multiop failed"
		$MULTIOP $DIR2/$tdir/$tfile \
			oO_RDWR:z${OFFSET}w$((${IOSZ} * 2))c || \
			error "multiop failed"
		cmp $DIR1/$tdir/$tfile $TMP/$tdir/$tfile || \
			error "file contents different"
		cmp $DIR1/$tdir/$tfile $DIR2/$tdir/$tfile || \
...
test_515cb() {
	...
	$MULTIOP $TMP/$tdir/$tfile oO_CREAT:O_RDWR:w$((${IOSZ} * 2))yc || \
		errro "multiop failed"
	$MULTIOP $TMP/$tdir/$tfile oO_RDWR:z${OFFSET}w${IOSZ}yc || \
		error "multiop failed"
	...
	while [ $i -lt $LOOP ]; do
		echo -n "."
		$MULTIOP $DIR1/$tdir/$tfile \
			oO_CREAT:O_RDWR:w$((${IOSZ} * 2))c || \
			errro "multiop failed"
		$MULTIOP $DIR2/$tdir/$tfile \
			oO_RDWR:z${OFFSET}w${IOSZ}c || error "multiop failed"
		cmp $DIR1/$tdir/$tfile $TMP/$tdir/$tfile || \
			error "file contents different"
		cmp $DIR1/$tdir/$tfile $DIR2/$tdir/$tfile || \
			error "file contents different"
...
test_515da() {
	...
	$MULTIOP $TMP/$tdir/$tfile oO_CREAT:O_RDWR:w${IOSZ}yc || \
		errro "multiop failed"
	$MULTIOP $TMP/$tdir/$tfile oO_RDWR:O_DIRECT:z${OFFSET}w${IOSZ}yc || \
		error "multiop failed"
	...
	while [ $i -lt $LOOP ]; do
		echo -n "."
		$MULTIOP $DIR1/$tdir/$tfile oO_CREAT:O_RDWR:w${IOSZ}c || \
			errro "multiop failed on $DIR1"
		$MULTIOP $DIR2/$tdir/$tfile \
			oO_RDWR:O_DIRECT:z${OFFSET}w${IOSZ}c || \
			error "multiop failed on $DIR2"
		cmp $DIR1/$tdir/$tfile $TMP/$tdir/$tfile || \
			error "file contents different"
		cmp $DIR1/$tdir/$tfile $DIR2/$tdir/$tfile || \
			error "file contents different"
...
test_515db() {
	...
	$MULTIOP $TMP/$tdir/$tfile \
		oO_CREAT:O_RDWR:O_DIRECT:w${IOSZ}yc || \
		errro "multiop failed"
	$MULTIOP $TMP/$tdir/$tfile oO_RDWR:z${OFFSET}w${IOSZ}yc || \
		error "multiop failed"
	...
	while [ $i -lt $LOOP ]; do
		echo -n "."
		$MULTIOP $DIR1/$tdir/$tfile \
			oO_CREAT:O_RDWR:O_DIRECT:w${IOSZ}c || \
			errro "multiop failed"
		$MULTIOP $DIR2/$tdir/$tfile oO_RDWR:z${OFFSET}w${IOSZ}c || \
			error "multiop failed"
		cmp $DIR1/$tdir/$tfile $TMP/$tdir/$tfile || \
...
test_516a() {
	...
	test_mkdir -p $DIR1/$tdir || error "mkdir $DIR/$tdir failed"
	mkdir -p $TMP/$tdir || error "mkdir $TMP/$tdir failed"
	$SETSTRIPE -c -1 -S $STRP $DIR1/$tdir || error "setstripe failed"
	...
	$MULTIOP $TMP/$tdir/$tfile "oO_CREAT:O_RDWR:O_SYNC:w${IOSZ}c" || \
		error "multiop failed"
	$MULTIOP $TMP/$tdir/$tfile "oO_RDWR:O_APPEND:O_SYNC:w${IOSZ}c" || \
		error "multiop failed"
	...
	while [ $i -lt $LOOP ]; do
		...
		$MULTIOP $DIR1/$tdir/$tfile "oO_CREAT:O_RDWR:w${IOSZ}c" || \
			error "multiop failed"
		$MULTIOP $DIR2/$tdir/$tfile "oO_RDWR:O_APPEND:w${IOSZ}c" || \
			error "multiop failed"
		cmp $DIR1/$tdir/$tfile $TMP/$tdir/$tfile || \
			error "file contents different"
		cmp $DIR1/$tdir/$tfile $DIR2/$tdir/$tfile || \
			error "file contents different"
		rm -f $DIR1/$tdir/$tfile || error "rm failed"
		let i=i+1
	done
	echo
	trap
...
test_516ca() {
	...
	$MULTIOP $TMP/$tdir/$tfile "oO_CREAT:O_RDWR:O_SYNC:w${IOSZ}c" || \
		error "multiop failed"
	$MULTIOP $TMP/$tdir/$tfile \
		"oO_RDWR:O_APPEND:O_SYNC:w$((${IOSZ} * 2))c" || \
		error "multiop failed"
...
test_516cb() {
	...
	$MULTIOP $TMP/$tdir/$tfile \
		"oO_CREAT:O_RDWR:O_SYNC:w$((${IOSZ} * 2))c" || \
		error "multiop failed"
	$MULTIOP $TMP/$tdir/$tfile "oO_RDWR:O_APPEND:O_SYNC:w${IOSZ}c" || \
		error "multiop failed"
...
test_516da() {
	...
	$MULTIOP $TMP/$tdir/$tfile "oO_CREAT:O_RDWR:O_SYNC:w${IOSZ}c" || \
		error "multiop failed"
	$MULTIOP $TMP/$tdir/$tfile "oO_RDWR:O_DIRECT:O_APPEND:w${IOSZ}c" || \
		error "multiop failed"
	...
	while [ $i -lt $LOOP ]; do
		...
		$MULTIOP $DIR1/$tdir/$tfile "oO_CREAT:O_RDWR:w${IOSZ}c" || \
			error "multiop failed"
		$MULTIOP $DIR2/$tdir/$tfile \
			"oO_RDWR:O_DIRECT:O_APPEND:w${IOSZ}c" || \
			error "multiop failed"
		cmp $DIR1/$tdir/$tfile $TMP/$tdir/$tfile || \
			error "file contents different"
		cmp $DIR1/$tdir/$tfile $DIR2/$tdir/$tfile || \
...
test_516db() {
	...
	$MULTIOP $TMP/$tdir/$tfile "oO_CREAT:O_RDWR:O_DIRECT:w${IOSZ}c" || \
		error "multiop failed"
	$MULTIOP $TMP/$tdir/$tfile "oO_RDWR:O_APPEND:O_SYNC:w${IOSZ}c" || \
		error "multiop failed"
...
test_517a() {
	...
	$MULTIOP $TMP/$tdir/$tfile \
		"oO_CREAT:O_RDWR:O_APPEND:O_SYNC:w${IOSZ}c" || \
		error "multiop failed"
	...
	while [ $i -lt $LOOP ]; do
		echo -n "."
		$MULTIOP $DIR1/$tdir/$tfile \
			"oO_CREAT:O_RDWR:O_APPEND:w${IOSZ}c" || \
			error "multiop failed"
		$MULTIOP $DIR2/$tdir/$tfile "oO_RDWR:w${IOSZ}c" || \
			error "multiop failed"
		cmp $DIR1/$tdir/$tfile $TMP/$tdir/$tfile || \
			error "file contents different"
		cmp $DIR1/$tdir/$tfile $DIR2/$tdir/$tfile || \
...
test_517ca() {
	...
	$MULTIOP $TMP/$tdir/$tfile \
		"oO_CREAT:O_RDWR:O_APPEND:O_SYNC:w${IOSZ}c" || \
		error "multiop failed"
	$MULTIOP $TMP/$tdir/$tfile "oO_RDWR:O_SYNC:w$((${IOSZ} * 2))c" || \
		error "multiop failed"
	...
	while [ $i -lt $LOOP ]; do
		...
		$MULTIOP $DIR1/$tdir/$tfile \
			"oO_CREAT:O_RDWR:O_APPEND:w${IOSZ}c" || \
			error "multiop failed"
		$MULTIOP $DIR2/$tdir/$tfile "oO_RDWR:w$((${IOSZ} * 2))c" || \
			error "multiop failed"
		cmp $DIR1/$tdir/$tfile $TMP/$tdir/$tfile || \
			error "file contents different"
		cmp $DIR1/$tdir/$tfile $DIR2/$tdir/$tfile || \
...
test_517cb() {
	...
	$MULTIOP $TMP/$tdir/$tfile \
		"oO_CREAT:O_RDWR:O_APPEND:O_SYNC:w$((${IOSZ} * 2))c" || \
		error "multiop failed"
	$MULTIOP $TMP/$tdir/$tfile "oO_RDWR:O_SYNC:w${IOSZ}c" || \
		error "multiop failed"
	...
	while [ $i -lt $LOOP ]; do
		echo -n "."
		$MULTIOP $DIR1/$tdir/$tfile \
			"oO_CREAT:O_RDWR:O_APPEND:w$((${IOSZ} * 2))c" || \
			error "multiop failed"
		$MULTIOP $DIR2/$tdir/$tfile "oO_RDWR:w${IOSZ}c" || \
			error "multiop failed"
		cmp $DIR1/$tdir/$tfile $TMP/$tdir/$tfile || \
...
test_517da() {
	...
	$MULTIOP $TMP/$tdir/$tfile \
		"oO_CREAT:O_RDWR:O_APPEND:O_SYNC:w${IOSZ}c" || \
		error "multiop failed"
	$MULTIOP $TMP/$tdir/$tfile "oO_RDWR:O_DIRECT:w${IOSZ}c" || \
		error "multiop failed"
	...
	while [ $i -lt $LOOP ]; do
		...
		$MULTIOP $DIR1/$tdir/$tfile \
			"oO_CREAT:O_RDWR:O_APPEND:w${IOSZ}c" || \
			error "multiop failed"
		$MULTIOP $DIR2/$tdir/$tfile "oO_RDWR:O_DIRECT:w${IOSZ}c" || \
			error "multiop failed"
		cmp $DIR1/$tdir/$tfile $TMP/$tdir/$tfile || \
			error "file contents different"
		cmp $DIR1/$tdir/$tfile $DIR2/$tdir/$tfile || \
...
test_517db() {
	...
	$MULTIOP $TMP/$tdir/$tfile \
		"oO_CREAT:O_RDWR:O_APPEND:O_DIRECT:w${IOSZ}c" || \
		error "multiop failed"
	$MULTIOP $TMP/$tdir/$tfile "oO_RDWR:O_SYNC:w${IOSZ}c" || \
		error "multiop failed"
...
test_518a() {
	...
	$MULTIOP $TMP/$tdir/$tfile "oO_CREAT:O_RDWR:w${IOSZ}w${IOSZ}c" || \
		error "multiop failed"
	$MULTIOP $TMP/$tdir/$tfile "oO_RDWR:O_TRUNC:w${IOSZ}c" || \
		error "multiop failed"
	...
	while [ $i -lt $LOOP ]; do
		echo -n "."
		$MULTIOP $DIR1/$tdir/$tfile \
			"oO_CREAT:O_RDWR:w${IOSZ}w${IOSZ}c" || \
			error "multiop failed"
		$MULTIOP $DIR2/$tdir/$tfile "oO_RDWR:O_TRUNC:w${IOSZ}c" || \
			error "multiop failed"
		cmp $DIR1/$tdir/$tfile $TMP/$tdir/$tfile || \
			error "file contents different"
		cmp $DIR1/$tdir/$tfile $DIR2/$tdir/$tfile || \
...
test_518b() {
	...
	$MULTIOP $TMP/$tdir/$tfile \
		"oO_CREAT:O_RDWR:O_SYNC:w${IOSZ}w${IOSZ}c" || \
		error "multiop failed"
	$MULTIOP $TMP/$tdir/$tfile "oO_RDWR:O_TRUNC:O_SYNC:w${IOSZ}c" || \
		error "multiop failed"
...
test_519a() {
	...
	$MULTIOP $TMP/$tdir/$tfile \
	"oO_CREAT:O_RDWR:O_SYNC:w${IOSZ}oO_RDWR:O_APPEND:O_SYNC:w${IOSZ}c" || \
		error "multiop failed"
	$MULTIOP $TMP/$tdir/$tfile \
		"oO_RDWR:O_TRUNC:O_SYNC:w${IOSZ}c" || \
		error "multiop failed"
	...
	while [ $i -lt $LOOP ]; do
		echo -n "."
		$MULTIOP $DIR1/$tdir/$tfile \
		"oO_CREAT:O_RDWR:w${IOSZ}oO_RDWR:O_APPEND:w${IOSZ}c" || \
			error "multiop failed"
		$MULTIOP $DIR2/$tdir/$tfile "oO_RDWR:O_TRUNC:w${IOSZ}c" || \
			error "multiop failed"
		cmp $DIR1/$tdir/$tfile $TMP/$tdir/$tfile || \
...
test_519b() {
	...
	"oO_CREAT:O_RDWR:O_SYNC:w${IOSZ}oO_RDWR:O_APPEND:O_SYNC:w${IOSZ}c" || \
		error "multiop failed"
	$MULTIOP $TMP/$tdir/$tfile \
		"oO_RDWR:O_TRUNC:O_SYNC:w${IOSZ}c" || error "multiop failed"
...
test_520b() {
	...
	$MULTIOP $TMP/$tdir/$tfile \
		"oO_CREAT:O_RDWR:w${IOSZ}coO_RDWR:O_APPEND:w${IOSZ}c" || \
		error "multiop failed"
	$MULTIOP $TMP/$tdir/$tfile "oO_RDWR:z${OFFSET}w$((${IOSZ} * 2))c" || \
		error "multiop failed"
	...
	for FAIL_LOC in ${IOSVC_FAIL[@]}; do
		...
		while [ $i -lt $LOOP ]; do
			...
			$MULTIOP $DIR1/$tdir/$tfile \
				...
				error "multiop failed"
			$MULTIOP $DIR2/$tdir/$tfile \
				"oO_RDWR:O_DIRECT:z${IOSZ}w${IOSZ}c" || \
				error "multiop failed"
			cmp $DIR1/$tdir/$tfile $TMP/$tdir/$tfile || \
				error "file contents different"
			cmp $DIR1/$tdir/$tfile $DIR2/$tdir/$tfile || \
	...
	for FAIL_LOC in ${IOSVC_FAIL[@]}; do
		...
		while [ $i -lt $LOOP ]; do
			...
			$MULTIOP $DIR1/$tdir/$tfile \
				...
				error "multiop failed"
			$MULTIOP $DIR2/$tdir/$tfile \
				"oO_RDWR:O_SYNC:z${OFFSET}w${IOSZ}c" || \
				error "multiop failed"
			cmp $DIR1/$tdir/$tfile $TMP/$tdir/$tfile || \
				error "file contents different"
			cmp $DIR1/$tdir/$tfile $DIR2/$tdir/$tfile || \
	...
	for FAIL_LOC in ${IOSVC_FAIL[@]}; do
		...
		while [ $i -lt $LOOP ]; do
			...
			$MULTIOP $DIR1/$tdir/$tfile \
				...
				error "multiop failed"
			$MULTIOP $DIR2/$tdir/$tfile \
				"oO_RDWR:O_TRUNC:z${OFFSET}w${IOSZ}c" || \
				error "multiop failed"
			cmp $DIR1/$tdir/$tfile $TMP/$tdir/$tfile || \
				error "file contents different"
			cmp $DIR1/$tdir/$tfile $DIR2/$tdir/$tfile || \
...
test_526() {
	...
	for FAIL_LOC in ${IOSVC_FAIL[@]}; do
		...
		while [ $i -lt $LOOP ]; do
			echo -n "."
			$MULTIOP $DIR1/$tdir/$tfile \
				"Ow${IOSZ}z${OFFSET}w${IOSZ}c" || \
				error "multiop failed via $DIR1"
			$MULTIOP $DIR2/$tdir/$tfile "OSMWUc" || \
				error "multiop failed via $DIR2"
			cmp $DIR1/$tdir/$tfile $TMP/$tdir/$tfile || \
				error "file contents different"
			cmp $DIR1/$tdir/$tfile $DIR2/$tdir/$tfile || \
				error "file contents different"
...
test_527() {
	...
	while [ $i -lt $LOOP ]; do
		echo -n "."
		$MULTIOP $DIR1/$tdir/$tfile "Ow${IOSZ}z${OFFSET}w${IOSZ}c" || \
			error "multiop failed"
		$MULTIOP $DIR2/$tdir/$tfile "OSMWUc" || \
			error "multiop failed"
		cmp $DIR1/$tdir/$tfile $TMP/$tdir/$tfile || \
...
test_528() {
	...
	while [ $i -lt $LOOP ]; do
		echo -n "."
		$MULTIOP $DIR1/$tdir/$tfile "Ow${IOSZ}SMWUc" || \
			error "multiop failed"
		$MULTIOP $DIR2/$tdir/$tfile "Ow${IOSZ}z${OFFSET}w${IOSZ}c" || \
			error "multiop failed"
		cmp $DIR1/$tdir/$tfile $TMP/$tdir/$tfile || \
			error "file contents different"
		cmp $DIR1/$tdir/$tfile $DIR2/$tdir/$tfile || \
...
cleanup_test530() {
	remount_client $MOUNT1
	remount_client $MOUNT2
}
...
test_530() {
...

	{
		...
		while [ $n -le 256 ]; do
			echo -n "."
			remount_client $MOUNT1 > /dev/null 2>&1
			let n=n+1
		done
	} &

	{
		...
		while [ $m -le 256 ]; do
			echo -n "."
			remount_client $MOUNT2 > /dev/null 2>&1
			let m=m+1
		done
	} &
...
run_test 530 "mount and umount the same filesystem"
