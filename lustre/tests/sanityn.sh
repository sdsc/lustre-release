...
test_60() {
	...
	chown $TSTUSER $file2 || error "Could not chown $TSTUSER $file2"
...
test_61() {
	...
	# Append data
	echo append >> $file2 || error "Could not append to $file2"
	version2=$($LFS data_version -n $file1)
	[ "$version" == "$version2" ] ||
	    error "data version should not have changed: $version!=$version2"
}
run_test 61 "Verify data_version -n behaviour"
