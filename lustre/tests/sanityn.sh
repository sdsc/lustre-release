...
test_33c() {
	...
	ops_do_cos "mkdir $DIR/$tdir; $LFS mkdir -i 1 $DIR/$tdir/subdir"
	# remote directory unlink
	ops_do_cos "$LFS mkdir -i 1 $DIR/$tdir; rmdir $DIR/$tdir"
	# striped directory create
	ops_do_cos "mkdir $DIR/$tdir; $LFS mkdir -c 2 $DIR/$tdir/subdir"
	# striped directory setattr
	ops_do_cos "$LFS mkdir -c 2 $DIR/$tdir; touch $DIR/$tdir"
	# striped directory unlink
	ops_do_cos "$LFS mkdir -c 2 $DIR/$tdir; rmdir $DIR/$tdir"
	# cross-MDT link
	ops_do_cos "$LFS mkdir -c 2 $DIR/$tdir; \
	...
	# migrate
	ops_do_cos "$LFS mkdir -i 0 $DIR/$tdir; $LFS migrate -m 1 $DIR/$tdir"
	return 0
}
