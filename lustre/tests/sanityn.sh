...
test_77b() { #LU-3266
	...
	do_nodes $oss lctl set_param ost.OSS.*.nrs_crrn_quantum=1
...
orr_trr() {
	...
	do_nodes $oss lctl set_param ost.OSS.*.nrs_"$policy"_quantum=1
	do_nodes $oss lctl set_param ost.OSS.*.nrs_"$policy"_offset_type="physical"
	do_nodes $oss lctl set_param ost.OSS.*.nrs_"$policy"_supported="reads"
	...
	do_nodes $oss lctl set_param ost.OSS.*.nrs_${policy}_quantum=64
	...
	do_nodes $oss lctl set_param ost.OSS.*.nrs_${policy}_offset_type="logical"
