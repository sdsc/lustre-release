...
test_133() {
	...
	dd if=/dev/zero of=$file count=1 oflag=sync &
	pid=$!
	sleep $(( $TIMEOUT / 4 ))

	local osp2dev=$(do_facet $SINGLEMDS "$LCTL get_param -n devices" |
		grep ${ost1_svc}-osc-MDT0000 | awk '{print $1}')
	do_facet $SINGLEMDS $LCTL --device $osp2dev deactivate ||
		error "deactivate failed"
	do_facet $SINGLEMDS $LCTL --device $osp2dev activate ||
		error "activate failed"
	# just enough to connect finish
	sleep 2
	...
	[ $block -eq 0 ] || error "osp_precreate_cleanup_orphans was blocked"
