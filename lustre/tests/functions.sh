...
#        required space       680MB * cbench_IDIRS = ~7 Gb
    ...
    cd $cbench_DIR
    local cmd= \
        "./compilebench -D $testdir -i $cbench_IDIRS -r $cbench_RUNS --makej"
...
run_write_disjoint() {
    ...
    WRITE_DISJOINT=\
        ${WRITE_DISJOINT:-$(which write_disjoint 2> /dev/null || true)}
    # threads per client
...
run_parallel_grouplock() {
    ...
    PARALLEL_GROUPLOCK=\
        ${PARALLEL_GROUPLOCK:-$(which parallel_grouplock 2> /dev/null || true)}
