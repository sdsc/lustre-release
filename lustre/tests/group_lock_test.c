...
static void test10(void)
{
	...
	for (i = 0; i < 1000; i++) {
		rc = ioctl(fd, LL_IOC_GROUP_LOCK, gid);
		ASSERTF(rc == 0,
			"can't lock %s: %d (loop %d)", mainpath, rc, i);
		rc = ioctl(fd, LL_IOC_GROUP_UNLOCK, gid);
		ASSERTF(rc == 0,
			"can't unlock %s: %d (loop %d)", mainpath, rc, i);
	}
	...
	ASSERTF(rc == -1 && errno == EINVAL,
		"lock unexpectedly granted: %s %d %d", mainpath, rc, errno);
	rc = ioctl(fd, LL_IOC_GROUP_UNLOCK, gid);
	ASSERTF(rc == 0, "can't unlock %s: %d", mainpath, rc);
	rc = ioctl(fd, LL_IOC_GROUP_UNLOCK, gid);
...
static void test30(void)
{
	...
	for (gid2 = -50; gid2 < 50; gid2++) {
		...
		if (gid2 == 0)
			ASSERTF(rc == -1 && errno == EINVAL,
				"unexpected value for gid %d: %d %s",
				rc, gid, strerror(errno));
		else
	...
	rc = ioctl(fd1, LL_IOC_GROUP_UNLOCK, gid);
	ASSERTF(rc == 0, "can't unlock %s: %d %s",
		mainpath, rc, strerror(errno));
