...
static void test11(void)
{
	...
	for (gid=1; gid<10000; gid++) {
		...
		case 2: oflags = O_WRONLY | O_APPEND; break;
		case 3: oflags = O_WRONLY | O_CLOEXEC; break;
		case 4: oflags = O_WRONLY | O_DIRECT; break;
		case 5: oflags = O_WRONLY | O_NOATIME; break;
		case 6: oflags = O_WRONLY | O_SYNC; break;
		case 7: oflags = O_RDONLY | O_DIRECT; break;
		case 8: oflags = O_RDONLY | O_RDWR; break;
		}
...
static void test30(void)
{
	...
	ASSERTF(rc == -1 && errno==EINVAL, "unexpected value: %d %d", rc, errno);
	rc = ioctl(fd1, LL_IOC_GROUP_UNLOCK, gid);
	ASSERTF(rc == 0, "can't unlock %s: %d", mainpath, rc);
