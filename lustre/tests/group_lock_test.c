...
static void test10(void)
{
	...
	fd = creat(mainpath, 0);
	ASSERTF(fd >= 0, "errno=%s, file=%s", strerror(errno), mainpath);

	/* Valid command first. */
	gid = 1234;
	rc = ioctl(fd, LL_IOC_GROUP_LOCK, gid);
	ASSERTF(rc == 0, "can't lock %s: %s", mainpath, strerror(errno));
	rc = ioctl(fd, LL_IOC_GROUP_UNLOCK, gid);
	ASSERTF(rc == 0, "can't unlock %s: %s", mainpath, strerror(errno));
	...
	for (i = 0; i < 1000; i++) {
		rc = ioctl(fd, LL_IOC_GROUP_LOCK, gid);
		ASSERTF(rc == 0,
			"can't lock %s: %s (loop %d)", mainpath, strerror(errno), i);
		rc = ioctl(fd, LL_IOC_GROUP_UNLOCK, gid);
		ASSERTF(rc == 0,
			"can't unlock %s: %s (loop %d)", mainpath, strerror(errno), i);
	}
	...
	ASSERTF(rc == -1 && errno == EINVAL,
		"lock unexpectedly granted for %s: %s", mainpath, strerror(errno));
	rc = ioctl(fd, LL_IOC_GROUP_UNLOCK, gid);
	ASSERTF(rc == 0, "can't unlock %s: %s", mainpath, strerror(errno));
	rc = ioctl(fd, LL_IOC_GROUP_UNLOCK, gid);
	ASSERTF(rc == -1 && errno == EINVAL, "unexpected value: %d %s",
		rc, strerror(errno));

	/* 0 is an invalid gid */
	gid = 0;
	rc = ioctl(fd, LL_IOC_GROUP_LOCK, gid);
	ASSERTF(rc == -1 && errno == EINVAL, "unexpected value: %s",
		strerror(errno));
...
static void test11(void)
{
	...
	ASSERTF(rc == sizeof(buf), "write failed: %d %s", rc, strerror(errno));
	close(fd);
	...
	for (gid = 1; gid < 10000; gid++) {
		...
		ASSERTF(fd >= 0, "errno=%s, oflags=%d, gid=%d",
			strerror(errno), oflags, gid);

		rc = ioctl(fd, LL_IOC_GROUP_LOCK, gid);
		ASSERTF(rc == 0, "can't lock %s, oflags=%d, gid=%d: %d %s",
			mainpath, oflags, gid, rc, strerror(errno));
...
static void helper_test20(int fd)
{
	...
	gid = 1234;
	rc = ioctl(fd, LL_IOC_GROUP_LOCK, gid);
	ASSERTF(rc == -1 && errno == ENOTTY, "unexpected value: %d %s",
		rc, strerror(errno));
...
static void test20(void)
{
	...
	ASSERTF(rc == 0, "mkdir failed: %s", strerror(errno));
...
static void test30(void)
{
	...
	ASSERTF(fd1 >= 0, "errno=%s, file=%s", strerror(errno), mainpath);

	/* Open a second time non blocking mode. */
	fd2 = open(mainpath, O_RDWR | O_NONBLOCK);
	ASSERTF(fd2 >= 0, "errno=%s, file=%s", strerror(errno), mainpath);
	...
	ASSERTF(rc == -1 && errno == EINVAL,
		"unexpected value: %d %s", rc, strerror(errno));
	rc = ioctl(fd1, LL_IOC_GROUP_UNLOCK, gid);
	ASSERTF(rc == 0, "can't unlock %s: %s", mainpath, strerror(errno));
