...
test_160() {
	...
	USER_REC1=$($GET_CL_USERS | awk "/$USER/{print \$2}")
	$LFS changelog_clear $MDT0 $USER $(($USER_REC1 + 5))
	USER_REC2=$($GET_CL_USERS | awk "/$USER/{print \$2}")
	...
	if [ $USERS -eq 0 ]; then
		...
		LAST_REC1=$($GET_CL_USERS | head -1 | cut -f3 -d' ')
		echo "verify changelogs are off: $LAST_REC1 == $LAST_REC1"
		[ $LAST_REC1 == $LAST_REC1 ] || error "changelogs not off"
	else
