...
test_505() {
	...
	trap "$LCTL set_param iosvc.iosvc_enable=1" EXIT RETURN
	$LCTL set_param "iosvc.iosvc_enable=1" > /dev/null 2>&1
	...
	while [ $i -le $FSXP ]; do
		$LCTL set_param "iosvc.iosvc_enable=0" > /dev/null 2>&1
		$LCTL set_param "iosvc.iosvc_enable=1" > /dev/null 2>&1
		let i=i+1
	done
...
test_507() {
	...
	rwv -f $DIR/$tdir/$tfile \
		-w -v -n $IOVCNT $IOSZ $(($IOSZ * 2)) $(($IOSZ * 4)) || \
		error "writev failed"

	rwv -f $DIR/$tdir/$tfile \
		-r -v -n $IOVCNT $(($IOSZ * 4)) $(($IOSZ * 2)) $IOSZ || \
		error "readv failed"
...
test_510() {
	...
	echo "Dirty pages not flushed to disk, dirty=$DIRTY,"\
	     "writeback=$WRITEBACK iosvc=$IOSVC"
...
test_512() {
	...
	sync
	BEFORE=$((`$LCTL get_param -n iosvc.workload`))
	dd if=/dev/zero of=$DIR/$tdir/$tfile bs="${IOSZ}K" count=1 || \
		error "dd failed"
	$MULTIOP $DIR/$tdir/$tfile \
		oO_CREAT:O_RDWR:O_APPEND:w$((${IOSZ} * 1024))c || \
		error "multiop failed"
	sync
	$CHECKSTAT -s $(($IOSZ * 1024 * 2)) $DIR/$tdir/$tfile || \
		error "wrong size"
	RESULT=$((`$LCTL get_param -n iosvc.workload` - $BEFORE))
	echo "RESULT: $RESULT"
...
test_515db() {
	...
	$MULTIOP $TMP/$tdir/$tfile oO_CREAT:O_RDWR:O_DIRECT:w${IOSZ}yc || \
		errro "multiop failed"
	$MULTIOP $TMP/$tdir/$tfile oO_RDWR:z${OFFSET}w${IOSZ}yc || \
		error "multiop failed"
...
test_517a() {
	...
	while [ $i -lt $LOOP ]; do
		...
		$MULTIOP $DIR/$tdir/$tfile \
			...
			error "multiop failed"
		$MULTIOP $DIR/$tdir/$tfile "oO_RDWR:w${IOSZ}c" || \
			error "multiop failed"
		cmp $DIR/$tdir/$tfile $TMP/$tdir/$tfile || \
			error "file contents different"
		rm -f $DIR/$tdir/$tfile || error "rm failed"
...
test_517ca() {
	...
	while [ $i -lt $LOOP ]; do
		...
		$MULTIOP $DIR/$tdir/$tfile \
			"oO_CREAT:O_RDWR:O_APPEND:w${IOSZ}c" || \
			error "multiop failed"
		$MULTIOP $DIR/$tdir/$tfile "oO_RDWR:w$((${IOSZ} * 2))c" || \
			error "multiop failed"
		cmp $DIR/$tdir/$tfile $TMP/$tdir/$tfile || \
			error "file contents different"
		rm -f $DIR/$tdir/$tfile || error "rm failed"
...
test_517da() {
	...
	while [ $i -lt $LOOP ]; do
		...
		$MULTIOP $DIR/$tdir/$tfile \
			"oO_CREAT:O_RDWR:O_APPEND:w${IOSZ}c" || \
			error "multiop failed"
		$MULTIOP $DIR/$tdir/$tfile "oO_RDWR:O_DIRECT:w${IOSZ}c" || \
			error "multiop failed"
		cmp $DIR/$tdir/$tfile $TMP/$tdir/$tfile || \
			error "file contents different"
		rm -f $DIR/$tdir/$tfile || error "rm failed"
...
test_518a() {
	...
	while [ $i -lt $LOOP ]; do
		...
		$MULTIOP $DIR/$tdir/$tfile \
			...
			error "multiop failed"
		$MULTIOP $DIR/$tdir/$tfile "oO_RDWR:O_TRUNC:w${IOSZ}c" || \
			error "multiop failed"
		cmp $DIR/$tdir/$tfile $TMP/$tdir/$tfile || \
			error "file contents different"
		rm -f $DIR/$tdir/$tfile || error "rm failed"
