...
test_102b() {
	...
	[ "$stripe_count" -eq $OSTCOUNT ] || \
		error "stripe count $stripe_count != $OSTCOUNT"
	rm -f $DIR/$tfile
}
...
grow_xattr() {
	...
	local orig=$(getfattr -n $xbig --absolute-names --only-values \
		$file 2>/dev/null)
	local osize=$(echo $orig | wc -c)
	...
	local new=$(getfattr -n $xbig --absolute-names --only-values \
		$file 2>/dev/null)
	...
	new=$(getfattr -n $xbig --absolute-names --only-values \
		$file 2>/dev/null)
