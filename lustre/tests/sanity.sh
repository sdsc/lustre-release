...
test_101a() {
	...
	$READS -f $DIR/$tfile -s$((cache_limit * 3192 * 1024)) -b65536 -C \
		-n$nreads -t 180
	...
	if (( $RAMISS > $nreads )); then
		...
		error "misses too much pages ($RAMISS)"
	fi
...
test_101b() {
	...
	do
		...
		RAMISS=$(grep 'generic read pos:' $TMP/$tfile-2.log | wc -l)
		rm -f $TMP/$tfile-[12].log
		if (( $RAMISS > $ITERATION )); then
			$LCTL get_param osc.*-osc*.rpc_stats
			error "misses too much pages ($RAMISS) with size ($BSIZE)"
		fi
...
test_101e() {
	...
	if (( $RAMISS > $count * $size_KB )); then
		$LCTL get_param osc.*-osc*.rpc_stats
		error "misses too much pages ($RAMISS) for small reads"
	fi
