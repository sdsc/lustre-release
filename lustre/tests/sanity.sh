...
test_134b() {
	...
	local low_wm=$(do_facet mds1 $LCTL get_param -n ldlm_watermark_kb_low)
	# disable reclaim temporarily
	do_facet mds1 $LCTL set_param ldlm_watermark_kb_low=0
	...
	if ! ps -p $create_pid  > /dev/null 2>&1; then
		do_facet mds1 $LCTL set_param fail_loc=0
		do_facet mds1 $LCTL set_param fail_val=0
		do_facet mds1 $LCTL set_param ldlm_watermark_kb_low=$low_wm
