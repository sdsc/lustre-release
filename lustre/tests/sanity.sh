...
test_39() {
	...
	if [ -n "$(nanotime_supported)" ]; then
		...
		[ $mtime_ns != "000000000" ] ||
			error "touch doesn't set nanotime"
		mtime_ns=`get_ns_mtime $DIR/${tfile}2`
		[ $mtime_ns != "000000000" ] ||
			error "create doesn't set nanotime"
	fi
...
test_39c() {
	...
	if [ -n "$(nanotime_supported)" ]; then
		...
		[ $mtime2_ns != $TEST_39_MTIMENS ] ||
			error "nano mtime is not updated on write: "\
			      "still $mtime2_ns"
	fi
	...
	for (( i=0; i < 2; i++ )) ; do
		...
		if [ -n "$(nanotime_supported)" ]; then
			...
			[ $mtime_ns = $TEST_39_MTIMENS ] ||
				error "nano mtime($mtime_ns) is not set "\
				      "to $TEST_39_MTIMENS"
		fi
...
test_39e() {
	...
	for (( i=0; i < 2; i++ )) ; do
		...
		if [ -n "$(nanotime_supported)" ]; then
			...
			[ $mtime2_ns = $TEST_39_MTIMENS ] ||
				error "nano mtime($mtime2_ns) is not set "\
				      "to $TEST_39_MTIMENS"
		fi
...
test_39g() {
	...
	for (( i=0; i < 2; i++ )) ; do
		...
		if [ -n "$(nanotime_supported)" ]; then
			local mtime2_ns=`get_ns_mtime $DIR1/$tfile`
			[ $mtime1_ns = $mtime2_ns ] ||
				error "lost nano mtime: $mtime2_ns, "\
				      "should be $mtime1_ns"
		fi
		cancel_lru_locks osc
		if [ $i = 0 ] ; then echo "repeat after cancel_lru_locks"; fi
...
test_39j() {
	...
	for (( i=0; i < 2; i++ )) ; do
		...
		if [ -n "$(nanotime_supported)" ]; then
			local mtime2_ns=`get_ns_mtime $DIR1/$tfile-1`
			[ $mtime2_ns = $mtime1_ns ] ||
				error "nano mtime is lost on close: "\
				      "$mtime2_ns, should be $mtime1_ns"
		fi
		cancel_lru_locks osc
		if [ $i = 0 ] ; then echo "repeat after cancel_lru_locks"; fi
	done
...
test_39k() {
	...
	for (( i=0; i < 2; i++ )) ; do
		...
		[ "$mtime2" = $TEST_39_MTIME ] ||
			error "mtime is lost on close: $mtime2, "\
			      "should be $TEST_39_MTIME"
		if [ -n "$(nanotime_supported)" ]; then
			local mtime2_ns=`get_ns_mtime $DIR1/$tfile`
			[ $mtime2_ns = $TEST_39_MTIMENS ] ||
				error "nano mtime is lost on close: "\
				      "$mtime2_ns, should be $TEST_39_MTIMENS"
		fi
