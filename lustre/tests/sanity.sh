...
exhaust_precreations() {
	...
	test_mkdir -p $DIR/$tdir
	local mdtidx=$(get_mdt_idx "$DIR/$tdir")
...
test_57b() {
	...
	df $dir
	local MDSFREE2=$(do_facet $mfacet \
		lctl get_param -n osd*.*$mdtname.kbytesfree)
...
test_132() { #1028, SOM
	...
	remote_mds_nodsh && skip "remote MDS with nodsh" && return
	mkdir -p $DIR
	local mfacet=mds$(($(get_mdt_idx $DIR) + 1))
	local MOUNTOPT_SAVE=$MOUNTOPT

	dd if=/dev/zero of=$DIR/$tfile count=1 2>/dev/null
	cancel_lru_locks osc

	som1=$(do_facet $mfacet "$LCTL get_param mdt.*.som" |
	...
	som2=$(do_facet $mfacet "$LCTL get_param mdt.*.som" |
	       awk -F= ' {print $2}' | head -n 1)
        if [ $som1 == $som2 ]; then
