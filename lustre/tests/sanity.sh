...
test_401b() {
	local save=$($LCTL get_param jobid_var)
	local tmp=testing

	$LCTL set_param foo=bar jobid_var=$tmp bar=baz &&
		error "no error returned when setting bad parameters"

	local jobid_new=$($LCTL get_param foo jobid_var baz 2> /dev/null) &&
		error "no error returned when getting bad parameters"
	[[ "$jobid_new" == "$tmp" ]] || error "jobid new $jobid_new != $tmp"

	local jobid_old$($LCTL set_param foo=bam jobid_var=$save bar=fog) &&
		error "no error returned when restoring bad parameters"
	[[ "$jobid_old" == "$save" ]] || error "jobid new $jobid_new != $save"
}
