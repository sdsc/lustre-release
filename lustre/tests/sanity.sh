...
test_51b() {
	mkdir -p $DIR/$tdir
	local mdtidx=$(printf "%04x" $($LFS getstripe -M $DIR/$tdir))
	...
	createmany -d $DIR/$tdir/d $NUMTEST &&
		echo $NUMTEST > $DIR/$tdir/fnum ||
		echo "failed" > $DIR/$tdir/fnum
}
...
test_51ba() { # LU-993
	...
	log "nlink before: $(stat -c %h $BASE), created before: $NUMTEST"
