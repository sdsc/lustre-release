...
test_101d() {
	...
	local space=$(($(df -P $DIR | tail -n 1 | awk '{ print $4 }') / 1024))
	[ $space -lt $size ] &&
		skip "Need free space ${size}M, have ${space}M" && return
	...
	local raOFF=$(do_and_time "dd if=$file of=/dev/null bs=1M count=$size")
	...
	local raON=$(do_and_time "dd if=$file of=/dev/null bs=1M count=$size")
	...
	[ $raOFF -le 1 -o $raON -lt $raOFF ] ||
		error "readahead ${raON}s > no-readahead ${raOFF}s ${size}M"
}
...
test_124a() {
	...
	for VALUE in $(lctl get_param ldlm.namespaces.*mdc-*.lru_size); do
		...
		if [ $LRU_SIZE -gt $(default_lru_size) ]; then
			...
			echo "NSDIR=$NSDIR, NS=$(basename $NSDIR)"
			break
		fi
	done

	[ -z "$NSDIR" -o $LRU_SIZE -lt $(default_lru_size) ] &&
		skip "Not enough cached locks created!" && return 0
	echo "LRU=$LRU_SIZE"
