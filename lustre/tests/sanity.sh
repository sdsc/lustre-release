...
test_24o() {
	...
	test_mkdir -p $DIR/$tfile
	rename_many -s random -v -n 10 $DIR/$tfile
}
...
test_24q() {
	[ $PARALLEL == "yes" ] && skip "skip parallel run" && return
	test_mkdir $DIR/$tdir.13a
	test_mkdir $DIR/$tdir.13b
	DIRINO=`ls -lid $DIR/$tdir.13a | awk '{ print $1 }'`
	multiop_bg_pause $DIR/$tdir.13b D_c || return 1
	MULTIPID=$!

	mrename $DIR/$tdir.13a $DIR/$tdir.13b
	$CHECKSTAT -a $DIR/$tdir.13a || error
	$CHECKSTAT -t dir $DIR/$tdir.13b || error
	DIRINO2=`ls -lid $DIR/$tdir.13b | awk '{ print $1 }'`
...
test_24r() { #bug 3789
	test_mkdir $DIR/$tdir.14a
	test_mkdir $DIR/$tdir.14a/b
	mrename $DIR/$tdir.14a $DIR/$tdir.14a/b \
		&& error "rename to subdir worked!"
	$CHECKSTAT -t dir $DIR/$tdir.14a \
		|| error "$DIR/$tdir.14a missing"
	$CHECKSTAT -t dir $DIR/$tdir.14a/b \
		|| error "$DIR/$tdir.14a/b missing"
}
...
test_31b() {
	...
	ln $DIR/$tfile $DIR/$tlink || error
	$MULTIOP $DIR/$tlink Ouc || error
	$CHECKSTAT -t file $DIR/$tfile || error
}
