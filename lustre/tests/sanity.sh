...
test_44a() {
    ...
    local stride=`$LCTL lov_getconfig $DIR | grep default_stripe_size: | \
                      awk '{print $2}'`
    if [ $nstripe -eq 0 -i $nstripe -eq -1 ] ; then
...
test_56u() { # LU-611
	...
	EXPECTED=$(((NUMDIRS + 1) * NUMFILES))
	CMD="$LFIND -ost_index 0 -type f $TDIR"
	NUMS=$($CMD | wc -l)
	[ $NUMS -eq $EXPECTED ] ||
		error "\"$CMD\" wrong: found $NUMS, expected $EXPECTED"

	EXPECTED=$ONESTRIPE
	CMD="$LFIND -ost_index 1 -type f $TDIR"
	NUMS=$($CMD | wc -l)
	[ $NUMS -eq $EXPECTED ] ||
		error "\"$CMD\" wrong: found $NUMS, expected $EXPECTED"
	...
	CMD="$LFIND -ost_index $OSTCOUNT -type f $TDIR"
	NUMS=$($CMD 2>/dev/null | wc -l)
	[ $NUMS -eq $EXPECTED ] ||
		error "\"$CMD\" wrong: found $NUMS, expected $EXPECTED"

	EXPECTED=$(((NUMDIRS + 1) * NUMFILES + ONESTRIPE))
	CMD="$LFIND -ost_index 0,1 -type f $TDIR"
	NUMS=$($CMD | wc -l)
	[ $NUMS -eq $EXPECTED ] ||
		error "\"$CMD\" wrong: found $NUMS, expected $EXPECTED"
}
run_test 56u "check lfs find -ost_index works"
...
test_204a() {
	...
	check_default_stripe_attr --stripe_count
	check_default_stripe_attr --stripe_size
...
test_204d() {
	mkdir -p $DIR/$tdir
	$SETSTRIPE --ost_index 0 $DIR/$tdir
...
test_204e() {
	...
	check_default_stripe_attr --stripe_count --raw
	check_default_stripe_attr --stripe_size --raw
	check_default_stripe_attr --ost_index --raw
