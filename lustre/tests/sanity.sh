...
test_60a() {
	...
	local llog_reader=$(which llog_reader)
	do_facet mgs [ ! -f "$llog_reader" ] &&
		llog_reader=$LUSTRE/utils/llog_reader; \
		do_facet mgs [ ! -f "$llog_reader" ] &&
	...
	for ((i = 0; i < ${#fid_list[@]}; i++)); do
		...
		if [ $rec_type != $rec ]; then
			echo "wrong record type $rec_type, should be $rec"
			pass=false
			break
		fi
		...
		if [ "$rec" == "1064553b" ]; then
			...
			if [ $obj_file != $mntpt/$path ]; then
				echo "wrong obj path $montpt/$path,
					should be $obj_file"
				pass=false
				break
			fi
