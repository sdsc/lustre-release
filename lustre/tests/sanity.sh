...
test_253_fill_ost() {
	...
	if (( lwm <= $((blocks/1024)) )) || [ ! -f $DIR/$tdir/1 ]; then
...
test_253() {
	...
	do_facet $SINGLEMDS $LCTL get_param -n \
		...
		{ skip  "remote MDS doesn\`t support reserved_mb_high" &&
		  return; }
	...
	do_facet mgs $LCTL pool_add $FSNAME.$TESTNAME $ost_name ||
		error "Adding $ost_name to pool fialed"
	...
	wait_update $HOSTNAME "$LCTL get_param -n
			...
			error "Client can\`t see the pool"
	$SETSTRIPE $DIR/$tdir -i $ostidx -c 1 -p $FSNAME.$TESTNAME ||
		error "Setstripe failed"
