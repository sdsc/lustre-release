...
test_220() { #LU-325
	...
	#client doesn`t see ost at pool right after pool_add
	sleep 5
...
test_253() {
	...
	for i in `seq 0 $(($last_id-$next_id))`; do
		dd if=/dev/zero of=$DIR/$tdir/r$i bs=1K count=1 2>/dev/null || \
			break;
	done

	blocks=$($LFS df $MOUNT | grep $ost_name | awk '{ print $4 }')
	if (( new_hwm < $(($blocks/1024)) )); then
		local size_2=$((($blocks/1024)-$new_hwm))
		size_2=$(($size_2+$size_2/10))
		dd if=/dev/zero of=$DIR/$tdir/1 bs=1M seek=$size_1 \
			count=$size_2 oflag=append 2>/dev/null
	fi
	blocks=$($LFS df $MOUNT | grep $ost_name | awk '{ print $4 }')
	echo "OST still has $(($blocks/1024)) mbytes free"
