...
test_184c() {
	...
	local bytes=`stat -c %s $file2`
	local remaining=`stat -c %s $ref1`
	remaining=$((remaining - bytes))
	echo "Copied $bytes bytes before swapping layout..."

	cmp -n $bytes $file1 $ref2 ||
		error "Content mismatch [0, $bytes) of ref2 and file1"
	cmp -n $bytes $file2 $ref1 ||
		error "Content mismatch [0, $bytes) of ref1 and file2"
	cmp -i $bytes:$bytes -n $remaining $file1 $ref1 ||
		error "Content mismatch [$bytes, EOF) of ref1 and file1"
