...
test_60a() {
	...
	fid_list=($(do_facet mgs grep "9_sub" $TMP/$tfile | grep "record" |
		tail -n 4 | awk '{print $NF}'))
	rec_list=($(do_facet mgs grep "9_sub" $TMP/$tfile | grep "record" |
		tail -n 4 | awk '{print $((NF-3))}'))
