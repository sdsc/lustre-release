...
test_116a() { # was previously test_116()
	...
	do_facet $SINGLEMDS lctl get_param -n lod.*-mdtlov.qos_prio_free
	declare -a AVAIL
	free_min_max
	...
	local threshold=$(do_facet $SINGLEMDS \
		lctl get_param -n lod.*MDT0000-mdtlov.qos_threshold_rr)
	threshold=${threshold%%%}
	...
	while [ $i -lt $FILL ]; do
		i=$(($i + 1))
		dd if=/dev/zero of=$DIR/$tdir/$tfile-$i bs=1024 \
			count=200 2>/dev/null
		echo -n .
	done
	...
	[ $MAXC -gt $MINC ] ||
		error_ignore "stripe QOS didn't balance free space"
