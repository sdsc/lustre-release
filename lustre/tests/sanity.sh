...
test_77c() { # bug 10889
	...
	then
		local check_ost=true
		local ost_file
	else
		echo "OSS do not support bulk pages dump upon error"
	fi
	$check_ost && do_facet ost1 $LCTL set_param obdfilter.*-OST*.checksum_dump=1
	...
	osc_file=$(ls /tmp/checksum_dump-osc*)
	...
	orig_cksum=$(dd if=$F77_TMP bs=1 skip=4 count=1048572 2>/dev/null | cksum)
	...
	ost_file=$(do_facet ost1 ls /tmp/checksum_dump-ost*)
...
run_test 77c "checksum error on client write with debug"
