...
test_245a() {
	local res=$(lctl get_param -n mdc.$FSNAME-MDT0000*.import \
		    | grep subtree)

	[ ! -n "$res" ] && skip "Fileset feature is not supported" \
		&& return
	...
	echo foo > $submount/$tfile || \
		error "write $submount/$tfile failed"
