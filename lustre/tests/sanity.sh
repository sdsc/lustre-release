...
test_101c() {
	...
	for osc_rpc_stats in $($LCTL get_param -N osc.*.rpc_stats); do
		local lines=$($LCTL get_param -n $osc_rpc_stats |
				wc | awk '{print $1}')
		if [ $lines -le 20 ]; then
			continue
		fi

		local rpc4k=$($LCTL get_param -n $osc_rpc_stats |
				     awk '$1 == "1:" { print $2; exit; }')
		local rpc8k=$($LCTL get_param -n $osc_rpc_stats |
				     awk '$1 == "2:" { print $2; exit; }')
		local rpc16k=$($LCTL get_param -n $osc_rpc_stats |
				     awk '$1 == "4:" { print $2; exit; }')
		local rpc32k=$($LCTL get_param -n $osc_rpc_stats |
