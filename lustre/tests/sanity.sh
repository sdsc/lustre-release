...
test_240() {
	...
	rm -rf $DIR/$tdir || error "failed to rmdir $DIR/$tdir"
	$LFS mkdir -i 0 $DIR/$tdir || error "failed to mkdir $DIR/$tdir on MDT0"
	$LFS mkdir -i 1 $DIR/$tdir/d0 ||
		error "failed to mkdir $DIR/$tdir/d0 on MDT1"
	...
	echo "stat $DIR/$tdir/d0, should not ASSERT"
	stat $DIR/$tdir/d0 || error "fail to stat $DIR/$tdir/d0"
}
