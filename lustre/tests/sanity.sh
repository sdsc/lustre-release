...
test_311() { # LU-7655 for OST fake write
	...
	# get ost1 size - lustre-OST0000
	local ost1_avail_size=$(do_facet ost1 $LFS df | grep ${ost1_svc} |
				awk '{ print $4 }')
	local blocks=$((ost1_avail_size / 1024)) # by megabytes
	[ $blocks -gt 2000 ] && blocks=2000 # 2G in maximum

	local start_time=$(date +%s.%N)
	dd if=/dev/zero of=$DIR/$tfile bs=1M count=$blocks ||
		error "dd writing error"
	local duration=$(bc <<< "$(date +%s.%N) - $start_time")

	# define OBD_FAIL_OST_FAKE_WRITE                 0x235
	do_facet ost1 $LCTL set_param fail_loc=0x235
