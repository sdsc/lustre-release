...
test_133f() {
	...
	if [ $? = 0 ]; then
		proc_dirs="$proc_dirs /proc/fs/lustre/"
	fi

	VAR=$(find /proc/sys/lustre/ 2>&1)
	if [ $? = 0 ]; then
		proc_dirs="$proc_dirs /proc/sys/lustre/"
	fi

	VAR=$(find /sys/fs/lustre/ 2>&1)
	if [ $? = 0 ]; then
		proc_dirs="$proc_dirs /sys/fs/lustre/"
	fi


	VAR=$(find /proc/sys/lnet/ 2>&1)
	if [ $? = 0 ]; then
		proc_dirs="$proc_dirs /proc/sys/lnet/"
	fi

	VAR=$(find /proc/fs/lnet/ 2>&1)
	if [ $? = 0 ]; then
		proc_dirs="$proc_dirs /proc/fs/lnet/"
	fi

	VAR=$(find /sys/fs/lnet/ 2>&1)
	if [ $? = 0 ]; then
		proc_dirs="$proc_dirs /sys/fs/lnet/"
	fi
...
create_lnet_proc_files() {
	if [ "$1" = "" ] ; then
		error "1st arg is NULL"
	else
		VAR=$(lctl get_param -n $1 2>&1)
		if [ "${VAR/"error"}" = "$VAR" ] ; then
			echo $VAR >$TMP/lnet_$1.out
		else
			error "cannot read /{proc,sys}/{fs,sys}/lnet/$1"
		fi
	fi
...
test_215() { # for bugs 18102, 21079, 21517
	...
	# /{proc,sys}/{fs,sys}/lnet/stats should look as 11 space-separated non-negative numerics
	BR="^$N $N $N $N $N $N $N $N $N $N $N$"
	create_lnet_proc_files "stats"
	check_lnet_proc_stats "stats.out" "/{proc,sys}/{fs,sys}/lnet/stats" "$BR"
	check_lnet_proc_stats "stats.sys" "lnet.stats" "$BR"
	remove_lnet_proc_files "stats"

	# /{proc,sys}/{fs,sys}/lnet/routes should look like this:
	...
	check_lnet_proc_entry "routes.out" "/{proc,sys}/{fs,sys}/lnet/routes" "$BR" "$L1" "$L2"
	check_lnet_proc_entry "routes.sys" "lnet.routes" "$BR" "$L1" "$L2"
	remove_lnet_proc_files "routes"

	# /{proc,sys}/{fs,sys}/lnet/routers should look like this:
	...
	check_lnet_proc_entry "routers.out" "/{proc,sys}/{fs,sys}/lnet/routers" "$BR" "$L1"
	check_lnet_proc_entry "routers.sys" "lnet.routers" "$BR" "$L1"
	remove_lnet_proc_files "routers"

	# /{proc,sys}/{fs,sys}/lnet/peers should look like this:
	...
	check_lnet_proc_entry "peers.out" "/{proc,sys}/{fs,sys}/lnet/peers" "$BR" "$L1"
	check_lnet_proc_entry "peers.sys" "lnet.peers" "$BR" "$L1"
	remove_lnet_proc_files "peers"

	# /{proc,sys}/{fs,sys}/lnet/buffers  should look like this:
	...
	check_lnet_proc_entry "buffers.out" "/{proc,sys}/{fs,sys}/lnet/buffers" "$BR" "$L1"
	check_lnet_proc_entry "buffers.sys" "lnet.buffers" "$BR" "$L1"
	remove_lnet_proc_files "buffers"

	# /{proc,sys}/{fs,sys}/lnet/nis should look like this:
	...
	check_lnet_proc_entry "nis.out" "/{proc,sys}/{fs,sys}/lnet/nis" "$BR" "$L1"
	check_lnet_proc_entry "nis.sys" "lnet.nis" "$BR" "$L1"
	remove_lnet_proc_files "nis"

	# can we successfully write to /{proc,sys}/{fs,sys}/lnet/stats?
	lctl set_param -n stats 0 || error "cannot write to /{proc,sys}/{fs,sys}/lnet/stats"
	sysctl -w lnet.stats=0 || error "cannot write to lnet.stats"
}
run_test 215 "/{proc,sys}/{fs,sys}/lnet exists and has proper content - bugs 18102, 21079, 21517"
...
test_900() {
	[ $PARALLEL == "yes" ] && skip "skip parallel run" && return
        ...
        local proc_dirs=""
        VAR=$(find /proc/fs/lustre/ldlm/namespaces/MGC*/lru_size 2>&1)
        if [ $? = 0 ]; then
                proc_dirs="$proc_dirs /proc/fs/lustre/ldlm/namespaces/MGC*/lru_size"
        fi

        VAR=$(find /proc/sys/lustre/ldlm/namespaces/MGC*/lru_size 2>&1)
        if [ $? = 0 ]; then
                proc_dirs="$proc_dirs /proc/sys/lustre/ldlm/namespaces/MGC*/lru_size"
        fi

        VAR=$(find /sys/fs/lustre/ldlm/namespaces/MGC*/lru_size 2>&1)
        if [ $? = 0 ]; then
                proc_dirs="$proc_dirs /sys/fs/lustre/ldlm/namespaces/MGC*/lru_size"
        fi

        for ls in $proc_dirs; do
                echo "clear" > $ls
        done
