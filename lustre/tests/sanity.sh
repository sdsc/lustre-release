...
get_unused_locks() {
	local mdc_ulocks=0
	local osc_ulocks=0
	mdc_ulocks=$($LCTL get_param -n ldlm.namespaces.*mdc*.lock_unused_count \
		| calc_total)
	osc_ulocks=$($LCTL get_param -n ldlm.namespaces.*osc*.lock_unused_count \
		| calc_total)

	echo $((mdc_ulocks + osc_ulocks))
}
