...
test_51e() {
	...
	test_mkdir -p -c1 $DIR/$tdir	|| error "create $tdir failed"
	test_mkdir -c1 $DIR/$tdir/d0	|| error "create d0 failed"

	local stripe_index=$($LFS getdirstripe -i $DIR/$tdir/d0)
	local NRFILES=65000
	local FREE_INODES=$(mdt_free_inodes $stripe_index)
	[[ $FREE_INODES -lt $((NRFILES + 20)) ]] &&
		skip "not enough inode $FREE_INODES expect $((NRFILES + 20))" &&
		return
