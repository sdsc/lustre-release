...
test_17l() { # LU-279
	...
	for path in "$DIR/$tdir" "$DIR/$tdir/$tfile" "$DIR/$tdir/$tfile.lnk"; do
		# -h to not follow symlinks and -m '' to list all the xattrs for $path
		# grep to remove useless first line containing: '# file: $path'
		for xattr in `getfattr -hm '' $path 2>/dev/null | grep -v '^#'`; do
			getxattr_size_check $path $xattr ||
				error "getxattr_size_check $path $xattr failed"
		done
	done
}
run_test 17l "Ensure getxattr's returned xattr size is consistent ========="
