...
verify_jobstats() {
        ...
        local f
        FACET="$SINGLEMDS"
        for f in do_facet $FACET ls /proc/fs/lustre/mdt/*/job_stats; do
                do_facet $FACET echo "clear all" > $f
        done
        FACET=ost0
        for f in do_facet $FACET ls /proc/fs/lustre/obdfilter/*/job_stats; do
                do_facet $FACET echo "clear all" > $f
        done
}
