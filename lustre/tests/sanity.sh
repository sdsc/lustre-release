...
pool_add_targets() {
	...
	do_facet mgs $LCTL pool_add $FSNAME.$pool $FSNAME-OST[$first-$last/$step]
	wait_update $HOSTNAME \
		"lctl get_param -n lov.$FSNAME-*.pools.$pool | sort -u | tr '\n' ' ' " \
		"$t" || { error_noexit "Add to pool failed"; return 1; }
	local lfscount=$($LFS pool_list $FSNAME.$pool | grep -c "\-OST")
	local addcount=$((($last - $first) / $step + 1))
	[ $lfscount -eq $addcount ] || {
		error_noexit "lfs pool_list bad ost count $lfscount != $addcount"
		return 2
	}
}
...
pool_remove_first_target() {
	...
	local t=$($LCTL get_param -n lov.$FSNAME-*.pools.$pool | head -1)
	do_facet mgs $LCTL pool_remove $FSNAME.$pool $t
	wait_update $HOSTNAME \
		"lctl get_param -n lov.$FSNAME-*.pools.$pool | grep $t" "" || {
...
pool_remove_all_targets() {
	...
	for t in $($LCTL get_param -n lov.$FSNAME-*.pools.$pool | sort -u)
	do
		do_facet mgs $LCTL pool_remove $FSNAME.$pool $t
	done
	wait_update $HOSTNAME \
	...
	$SETSTRIPE -p $pool $file 2>/dev/null && {
		error_noexit "expected failure when creating file with empty pool"
		return 3; }
	return 0
}
...
pool_remove() {
	...
	# setstripe on an empty pool should fail
	$SETSTRIPE -p $pool $file 2>/dev/null && {
		error_noexit "expected failure when creating file with missing pool"
		return 2
	}
