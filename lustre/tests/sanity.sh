...
#	done
	...
	[ "$JOBENV" = "FAKE_JOBID" ] && FAKE_JOBID=id.$testnum.${cmd[0]}.$RANDOM
...
INTERVAL_SAVE=$(do_facet $SINGLEMDS lctl get_param mdt.*.job_cleanup_interval)
cleanup_205() {
	do_facet $SINGLEMDS \
		$LCTL set_param mdt.*.job_cleanup_interval=$INTERVAL_SAVE
...
test_205() { # Job stats
	...
	[ $left -ge 0 ] && sleep $((left + 1))
	cmd="mkdir $DIR/$tdir.2"
	verify_jobstats "$cmd" "$SINGLEMDS"
