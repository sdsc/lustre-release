...
setup_test101bc() {
	...
	for (( i=0; i < OSTCOUNT; i++ )) ; do
		do_facet ost$i $LCTL set_param -n obdfilter.*.read_cache_enable=0
		do_facet ost$i $LCTL set_param -n obdfilter.*.writethrough_cache_enable=0
	done
...
cleanup_test101bc() {
	...
	rm -f $DIR/$tfile
	for (( i=0; i < OSTCOUNT; i++ )) ; do
		do_facet ost$i $LCTL set_param -n obdfilter.*.read_cache_enable=1
		do_facet ost$i $LCTL set_param -n obdfilter.*.writethrough_cache_enable=1
	done
}
