...
zfs_object_blksz() {
	...
	case "${blksz: -1}" in
		k|K) blksz=$((${blksz:0:-1}*1024)) ;;
		m|M) blksz=$((${blksz:0:-1}*1024*1024)) ;;
		*) ;;
	esac
...
test_311() {
	...
	touch $tf
	local oid=$($GETSTRIPE $tf | awk '/obdidx/{getline; print $2}')

	# Get ZFS object id
	local zfs_objid=$(zfs_oid_to_objid ost1 $oid)

	# block size change by sequential write
	local blksz
	...
	# random write
	touch $tf
	oid=$($GETSTRIPE $tf | awk '/obdidx/{getline; print $2}')
...
run_test 311 "make sure ZFS adjusts its block size by write pattern"
