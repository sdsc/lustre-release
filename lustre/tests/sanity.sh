...
test_65k() { # bug11679
	...
	[ $(lustre_version_code $SINGLEMDS) -le $(version_code 2.7.57) ] &&
	...
	for INACTIVE_OSC in $MDS_OSCS; do
		...
		local ostnum=`do_facet $SINGLEMDS lctl get_param -n \
			lov.*md*.target_obd | awk -F: /$ost/'{ print $1 }' | \
			head -n 1`

		for i in $(seq 1000); do
			$SETSTRIPE -i $ostnum -c 1 $DIR/$tdir/$tfile.$ostnum.$i
		done
		local old_iused=$($LFS df -i | grep $ost | awk '{ print $3 }')
		...
		$SETSTRIPE -i $ostnum -c 1 $DIR/$tdir/$tfile.$ostnum.temp
		rm -f $DIR/$tdir/$tfile.$ostnum.temp
		local new_iused
		for i in `seq 20`; do
			new_iused=$($LFS df -i | grep $ost | awk '{ print $3 }')
			[ $new_iused -lt $old_iused ] && break
			sleep 6
		done

		echo "wait $((i * 6)) sec, old Iused $old_iused, new $new_iused"
		# after deactivate and activate, not all orphans are destroyed
		[ $new_iused -lt $old_iused ] ||
			error "activate didn't destroy orphans"
	done
}
...
test_311() {
	[ $(lustre_version_code $SINGLEMDS) -lt $(version_code 2.7.58) ] &&
		skip "lustre < 2.7.58 does not contain LU-4825 fix" && return
	[ $PARALLEL == "yes" ] && skip "skip parallel run" && return

	mkdir -p $DIR/$tdir
	for i in $(seq 1000); do
		$SETSTRIPE -i 0 -c 1 $DIR/$tdir/$tfile.$i
	done
	...
	do_facet $SINGLEMDS "lctl set_param -n \
		osp.*OST0000*MDT0000.max_create_count=0"
	do_facet ost1 "lctl set_param -n obdfilter.*OST0000.degraded=1"

	$SETSTRIPE -i 0 $DIR/$tdir/$tfile || error "setstripe failed"
	local index=$($GETSTRIPE -i $DIR/$tdir/$tfile)
	[ $index -ne 0 ] || error "stripe index $index is 0"

	rm -rf $DIR/$tdir

	do_facet ost1 "lctl set_param -n obdfilter.*OST0000.degraded=0"
	do_facet $SINGLEMDS "lctl set_param -n \
		osp.*OST0000*MDT0000.max_create_count=$max_count"
	do_facet $SINGLEMDS "lctl set_param -n \
		osp.*OST0000*MDT0000.create_count=$count"
	...
	echo "wait $((i * 6)) sec, old Iused $old_iused, Iused $new_iused"
