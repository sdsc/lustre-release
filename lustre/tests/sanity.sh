...
test_56u() { # LU-611
	...
	EXPECTED=$(((NUMDIRS + 1) * NUMFILES))
	CMD="$LFIND -ost-index 0 -type f $TDIR"
	NUMS=$($CMD | wc -l)
	[ $NUMS -eq $EXPECTED ] ||
		error "\"$CMD\" wrong: found $NUMS, expected $EXPECTED"

	EXPECTED=$ONESTRIPE
	CMD="$LFIND -ost-index 1 -type f $TDIR"
	NUMS=$($CMD | wc -l)
	[ $NUMS -eq $EXPECTED ] ||
		error "\"$CMD\" wrong: found $NUMS, expected $EXPECTED"
	...
	CMD="$LFIND -ost-index $OSTCOUNT -type f $TDIR"
	NUMS=$($CMD 2>/dev/null | wc -l)
	[ $NUMS -eq $EXPECTED ] ||
		error "\"$CMD\" wrong: found $NUMS, expected $EXPECTED"

	EXPECTED=$(((NUMDIRS + 1) * NUMFILES + ONESTRIPE))
	CMD="$LFIND -ost-index 0,1 -type f $TDIR"
	NUMS=$($CMD | wc -l)
	[ $NUMS -eq $EXPECTED ] ||
		error "\"$CMD\" wrong: found $NUMS, expected $EXPECTED"
}
run_test 56u "check lfs find -ost-index works"
...
test_204a() {
	mkdir -p $DIR/$tdir
	$SETSTRIPE --stripe-count 0 --stripe-size 0 --ost-index -1 $DIR/$tdir

	check_default_stripe_attr --stripe-count
	check_default_stripe_attr --stripe-size
	check_default_stripe_attr --ost-index
...
test_204d() {
	mkdir -p $DIR/$tdir
	$SETSTRIPE --ost-index 0 $DIR/$tdir
...
test_204e() {
	...
	check_default_stripe_attr --ost-index --raw
