...
test_65k() { # bug11679
	...
	[ $(lustre_version_code $SINGLEMDS) -gt $(version_code 2.7.57) ] &&
	...
	for INACTIVE_OSC in $MDS_OSCS; do
		...
		sleep 10
		local new_iused=$($LFS df -i | grep $ost | awk '{ print $3 }')

		echo "old Iused $old_iused, new Iused $new_iused"
...
test_311() {
	...
	sleep 15
	local new_iused=$($LFS df -i | grep OST0000 | awk '{ print $3 }')
