...
test_27d() {
	mkdir -p $DIR/d27
	$SETSTRIPE -c0 -i-1 -s0 $DIR/d27/fdef || error "setstripe failed"
...
check_seq_oid()
{
        ...
        $GETSTRIPE $1 | while read obdidx oid hex seq; do
                ...
                log "FF stripe:$stripe_num ost:$obdidx oid:$oid/$hex seq:$seq"

				stop ost$ost
                do_facet ost$ost mount -t $FSTYPE $dev $dir $OST_MOUNT_OPTS ||
                        { error "mounting $dev as $FSTYPE failed"; return 3; }

                obj_filename=$(do_facet ost$ost find $dir/O/$seq -name $oid)
                local ff=$(do_facet ost$ost $LL_DECODE_FILTER_FID $obj_filename)
                IFS=$'/= [:]'
                ff=($(echo $ff))
                IFS="$old_ifs"
                ...
                [ ${ff[11]} -ne $lmm_seq ] &&
                        error "FF parent SEQ ${ff[11]} != $lmm_seq"
                # compare lmm_object_id and filter_fid->ff_parent.f_oid
                [ ${ff[12]} -ne $lmm_oid ] &&
                        error "FF parent OID ${ff[12]} != $lmm_oid"
                let stripe=${ff[13]}
                [ $stripe -ne $stripe_num ] &&
                        error "FF stripe $stripe != $stripe_num"
...
test_27z() {
        remote_ost_nodsh && skip "remote OST with nodsh" && return
        mkdir -p $DIR/$tdir
        $SETSTRIPE -c 1 -i 0 -s 1m $DIR/$tdir/$tfile-1 ||
                { error "setstripe -c -1 failed"; return 1; }
		# we need to send an RPC to every object to get parent FID info set
        touch $DIR/$tdir/$tfile-1 || { error "touch $tfile-1 failed"; return 2;}
        $SETSTRIPE -c -1 -i $((OSTCOUNT - 1)) -s 1m $DIR/$tdir/$tfile-2 ||
                { error "setstripe -c 1 failed"; return 3; }
        touch $DIR/$tdir/$tfile-2 || { error "touch $tfile-2 failed"; return 4;}
...
test_56g() {
        ...
        for i in $(seq 1 $NUMFILES) ; do
                ...
                [ $NUMS -eq $EXPECTED ] ||
                        error "lfs find -name \"*$i\" $DIR/${tdir}g wrong: "\
...
test_56q() {
	...
	chgrp $RUNAS_GID $TDIR/file* ||
		error "chown $DIR/${tdir}g/file$i failed"
...
test_56s() { # LU-611
	...
	CMD="$LFIND -stripe_count +0 -type f $TDIR"
	NUMS=$($CMD | wc -l)
	[ $NUMS -eq $EXPECTED ] ||
		error "\"$CMD\" wrong: found $NUMS, expected $EXPECTED"

	EXPECTED=$ONESTRIPE
	CMD="$LFIND -stripe_count 1 -type f $TDIR"
	NUMS=$($CMD | wc -l)
	[ $NUMS -eq $EXPECTED ] ||
		error "\"$CMD\" wrong: found $NUMS, expected $EXPECTED"

	CMD="$LFIND -stripe_count -2 -type f $TDIR"
	NUMS=$($CMD | wc -l)
	[ $NUMS -eq $EXPECTED ] ||
		error "\"$CMD\" wrong: found $NUMS, expected $EXPECTED"

	EXPECTED=0
	CMD="$LFIND -stripe_count $((OSTCOUNT + 1)) -type f $TDIR"
	NUMS=$($CMD | wc -l)
	[ $NUMS -eq $EXPECTED ] ||
		error "\"$CMD\" wrong: found $NUMS, expected $EXPECTED"
}
run_test 56s "check lfs find -stripe_count works"
...
test_56t() { # LU-611
	...
	EXPECTED=$(((NUMDIRS + 1) * NUMFILES))
	CMD="$LFIND -stripe_size 512k -type f $TDIR"
	NUMS=$($CMD | wc -l)
	[ $NUMS -eq $EXPECTED ] ||
		error "\"$CMD\" wrong: found $NUMS, expected $EXPECTED"

	CMD="$LFIND -stripe_size +320k -type f $TDIR"
	NUMS=$($CMD | wc -l)
	[ $NUMS -eq $EXPECTED ] ||
		error "\"$CMD\" wrong: found $NUMS, expected $EXPECTED"

	EXPECTED=$(((NUMDIRS + 1) * NUMFILES + 4))
	CMD="$LFIND -stripe_size +200k -type f $TDIR"
	NUMS=$($CMD | wc -l)
	[ $NUMS -eq $EXPECTED ] ||
		error "\"$CMD\" wrong: found $NUMS, expected $EXPECTED"

	CMD="$LFIND -stripe_size -640k -type f $TDIR"
	NUMS=$($CMD | wc -l)
	[ $NUMS -eq $EXPECTED ] ||
		error "\"$CMD\" wrong: found $NUMS, expected $EXPECTED"

	EXPECTED=4
	CMD="$LFIND -stripe_size 256k -type f $TDIR"
	NUMS=$($CMD | wc -l)
	[ $NUMS -eq $EXPECTED ] ||
		error "\"$CMD\" wrong: found $NUMS, expected $EXPECTED"
	...
	EXPECTED=0
	CMD="$LFIND -stripe_size 1024k -type f $TDIR"
	NUMS=$($CMD | wc -l)
	[ $NUMS -eq $EXPECTED ] ||
		error "\"$CMD\" wrong: found $NUMS, expected $EXPECTED"
}
run_test 56t "check lfs find -stripe_size works"
...
test_56u() { # LU-611
	...
	EXPECTED=$(((NUMDIRS + 1) * NUMFILES))
	CMD="$LFIND -stripe_index 0 -type f $TDIR"
	NUMS=$($CMD | wc -l)
	[ $NUMS -eq $EXPECTED ] ||
		error "\"$CMD\" wrong: found $NUMS, expected $EXPECTED"

	EXPECTED=$ONESTRIPE
	CMD="$LFIND -stripe_index 1 -type f $TDIR"
	NUMS=$($CMD | wc -l)
	[ $NUMS -eq $EXPECTED ] ||
		error "\"$CMD\" wrong: found $NUMS, expected $EXPECTED"
	...
	CMD="$LFIND -stripe_index $OSTCOUNT -type f $TDIR"
	NUMS=$($CMD 2>/dev/null | wc -l)
	[ $NUMS -eq $EXPECTED ] ||
		error "\"$CMD\" wrong: found $NUMS, expected $EXPECTED"

	EXPECTED=$(((NUMDIRS + 1) * NUMFILES + ONESTRIPE))
	CMD="$LFIND -stripe_index 0,1 -type f $TDIR"
	NUMS=$($CMD | wc -l)
	[ $NUMS -eq $EXPECTED ] ||
		error "\"$CMD\" wrong: found $NUMS, expected $EXPECTED"
}
run_test 56u "check lfs find -stripe_index works"
...
test_204a() {
	mkdir -p $DIR/$tdir
	$SETSTRIPE --stripe_count 0 --stripe_size 0 --ost_index -1 $DIR/$tdir

	check_default_stripe_attr count
	check_default_stripe_attr size
	check_default_stripe_attr ost_index
...
test_204b() {
	mkdir -p $DIR/$tdir
	$SETSTRIPE --stripe_count 1 $DIR/$tdir
...
test_204c() {
	mkdir -p $DIR/$tdir
	$SETSTRIPE --stripe_size 65536 $DIR/$tdir
...
test_204d() {
	mkdir -p $DIR/$tdir
	$SETSTRIPE --stripe_index 0 $DIR/$tdir
...
test_204e() {
	...
	check_raw_stripe_attr ost_index
