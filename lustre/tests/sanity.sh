...
test_154c() {
	...
	fid=`lctl list_param -R mdt | grep open_files`
	fid="$fid=`$LFS path2fid $DIR/$tfile`"
	# Open the file         
	exec 7<$DIR/$tfile 
	ofid=`$LCTL get_param $proc_ofile`
	
	exec 7</dev/null
	[ $fid = $ofid ] ||
		error "FID $ofid not found in open files list $fid"

}
run_test 154c "Verify open file fid"
