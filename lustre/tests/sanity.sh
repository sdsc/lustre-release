...
test_404() {
	...
	$SETSTRIPE -c 2 -i 1 -S $((def_stripe_size * 2)) -p $test_pool $MOUNT ||
		error "setstripe failed"
	...
	for i in $(seq 10); do
		...
		[ $count -eq 2 ] || error "$f stripe count $count != 2"
	...
	if [ -z $def_pool ]; then
		$SETSTRIPE -c $def_stripenr -S $def_stripe_size \
			-i $def_stripe_offset $MOUNT || error "setstripe failed"
	else
...
run_test 404 "DNE support fs default striping"
