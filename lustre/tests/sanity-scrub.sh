...
test_0() {
	...
	diff $LUSTRE/tests/test-framework.sh $DIR/$tdir/test-framework.sh > \
		/dev/null || error "(3) File diff failed unexpected!"

	STATUS=`$SHOW_SCRUB | sed -n '3'p | awk '{print $2}'`
	[ "$STATUS" = "completed" ] || \
		error "(4) Expect 'completed', but got '$STATUS'"

	COUNT=`$SHOW_SCRUB | sed -n '16'p | awk '{print $3}'`
        [ $COUNT -eq 1 ] || \
		error "(5) Expect once ran for init, but got '$COUNT'"
}
run_test 0 "Do not auto trigger OI scrub for non-backup/restore case after init"
...
test_1() {
	...
	COUNT=`$SHOW_SCRUB | sed -n '16'p | awk '{print $3}'`
        [ $COUNT -eq 2 ] || \
		error "(4) Expect ran twice, but got '$COUNT'"
