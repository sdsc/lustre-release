...
SCRUB_PATH="/proc/fs/lustre/osd-ldiskfs/${FSNAME}-MDT0000/oi_scrub"
START_SCRUB="do_facet $SINGLEMDS $LCTL scrub_start -M ${FSNAME}-MDT0000 -t OI"
STOP_SCRUB="do_facet $SINGLEMDS $LCTL scrub_stop -M ${FSNAME}-MDT0000"
SHOW_SCRUB="do_facet $SINGLEMDS cat $SCRUB_PATH"
...
test_4() {
	...
	do_facet $SINGLEMDS \
                $LCTL set_param -n mdd.${FSNAME}-MDT0000.auto_scrub 1
	mount_client $MOUNT || error "(5) Fail to start client!"
	diff $LUSTRE/tests/test-framework.sh $DIR/$tdir/test-framework.sh > \
...
test_5() {
	...
	do_facet $SINGLEMDS \
                $LCTL scrub_start -M ${FSNAME}-MDT0000 -t OI -s 100 || \
	...
	do_facet $SINGLEMDS \
                $LCTL scrub_start -M ${FSNAME}-MDT0000 -t OI -s 100 -r || \
	...
	SPEED=`$SHOW_SCRUB | sed -n '23'p | awk '{print $3}'`
	[ $SPEED -gt 144 ] && \
...
#define OBD_FAIL_OSD_SCRUB_FAILURE       0x190
	...
	sleep 2
	do_facet $SINGLEMDS
                $LCTL scrub_start -M ${FSNAME}-MDT0000 -t OI -n on -s 100 || \
...
test_7() {
	...
	POSITION0=`$SHOW_SCRUB | sed -n '16'p | awk '{print $4}'`
...
#define OBD_FAIL_OSD_SCRUB_DELAY         0x191
	do_facet $SINGLEMDS $LCTL set_param fail_loc=0x10000191
	do_facet $SINGLEMDS $LCTL set_param fail_val=20
...
#define OBD_FAIL_OSD_SCRUB_DELAY         0x191
	...
	do_facet $SINGLEMDS
                $LCTL scrub_start -M ${FSNAME}-MDT0000 -t OI -n on || \
		error "(5) Fail to start OI Scrub!"
	mount_client $MOUNT || error "(6) Fail to start client!"
	diff $LUSTRE/tests/test-framework.sh $DIR/$tdir/test-framework.sh > \
		/dev/null || error "(7) File diff failed unexpected!"
