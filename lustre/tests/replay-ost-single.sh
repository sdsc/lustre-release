...
test_2() {
	for i in $(seq 10); do
		echo "tag-$i" > $TDIR/$tfile-$i ||
			error "create $TDIR/$tfile-$i"
	done
	fail ost1
	for i in $(seq 10); do
		grep -q "tag-$i" $TDIR/$tfile-$i ||
			error "grep $TDIR/$tfile-$i"
	done
	rm -f $TDIR/$tfile-*
}
...
test_6() {
	...
	wait_destroy_complete || error "first wait_delete_completed failed"
...
test_7() {
	...
	wait_destroy_complete || error "wait_delete_completed failed"
