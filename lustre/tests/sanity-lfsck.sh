...
test_19a() {
	[ $MDSCOUNT -lt 2 ] &&
		skip "We need at least 2 MDSes for test_19a" && exit 0
	...
	echo "is lost or partly lost. The LFSCK should re-generate the those"
	echo "missed layout EA entries."
	echo "#####"
	...
	[ "$SIZE" != "$SAVED_SIZE" ] || error "(1) Expect incorrect file1 size"

	SIZE=$(ls -il $DIR/$tdir/a2/f2 | awk '{ print $6 }')
	[ "$SIZE" != "$SAVED_SIZE" ] || error "(2) Expect incorrect file2 size"

	echo "Trigger layout LFSCK on all devices to find out orphan OST-objects"
	$START_LAYOUT -A || error "(3) Fail to start LFSCK for layout!"
	sleep 2
	...
	for k in $(seq $MDSCOUNT); do
		local STATUS=$(do_facet mds${k} $LCTL get_param -n \
				...
				awk '/^status/ { print $2 }')
		[ "$STATUS" == "completed" ] ||
		error "(4) MDS${k} Expect 'completed', but got '$STATUS'"
	done
	...
	for k in $(seq $OSTCOUNT); do
		local STATUS=$(do_facet ost${k} $LCTL get_param -n \
				...
				awk '/^status/ { print $2 }')
		[ "$STATUS" == "completed" ] ||
		error "(5) OST${k} Expect 'completed', but got '$STATUS'"
	done
	...
	[ "$SIZE" == "$SAVED_SIZE" ] ||
		error "(7) Expect correct file1 size $SAVED_SIZE, but got $SIZE"

	SIZE=$(ls -il $DIR/$tdir/a2/f2 | awk '{ print $6 }')
	[ "$SIZE" == "$SAVED_SIZE" ] ||
		error "(8) Expect correct file2 size $SAVED_SIZE, but got $SIZE"
}
run_test 19a "Find out orphan OST-object and repair it (1)"

test_19b() {
	[ $MDSCOUNT -lt 2 ] &&
		skip "We need at least 2 MDSes for test_19b" && exit 0

	[ $OSTCOUNT -lt 2 ] &&
		skip "We need at least 2 OSTs for test_19b" && exit 0
	...
	echo "Trigger layout LFSCK on all devices to find out orphan OST-objects"
	$START_LAYOUT -A || error "(1) Fail to start LFSCK for layout!"
	sleep 2
	...
	for k in $(seq $MDSCOUNT); do
		local STATUS=$(do_facet mds${k} $LCTL get_param -n \
				...
				awk '/^status/ { print $2 }')
		[ "$STATUS" == "completed" ] ||
		error "(2) MDS${k} Expect 'completed', but got '$STATUS'"
	done
	...
	for k in $(seq $OSTCOUNT); do
		local STATUS=$(do_facet ost${k} $LCTL get_param -n \
				...
				awk '/^status/ { print $2 }')
		[ "$STATUS" == "completed" ] ||
		error "(3) OST${k} Expect 'completed', but got '$STATUS'"
	done
...
test_19c() {
	...
	echo "created OST-object when repair dangling reference case. Then"
	echo "the LFSCK will exchange the two conflict OST-objects in such"
	echo "MDT-object layout EA."
	echo "#####"
	...
	echo "Inject failure, to make $DIR/$tdir/a1/f2 and $DIR/$tdir/a1/f1"
	echo "to reference the same OST-object. Then drop $DIR/$tdir/a1/f1."
	echo "So $DIR/$tdir/a1/f2 will dangling reference, although its old"
	echo "OST-object still is there."
	echo
	...
	local SIZE=$(ls -il $DIR/$tdir/a1/f2 | awk '{ print $6 }')
	[ "$SIZE" != "$SAVED_SIZE" ] || error "(1) Expect incorrect file2 size"

	echo "Trigger layout LFSCK on all devices to find out orphan OST-objects"
	$START_LAYOUT -A || error "(2) Fail to start LFSCK for layout!"
	sleep 2
	...
	for k in $(seq $MDSCOUNT); do
		local STATUS=$(do_facet mds${k} $LCTL get_param -n \
				...
				awk '/^status/ { print $2 }')
		[ "$STATUS" == "completed" ] ||
		error "(3) MDS${k} Expect 'completed', but got '$STATUS'"
	done
	...
	for k in $(seq $OSTCOUNT); do
		local STATUS=$(do_facet ost${k} $LCTL get_param -n \
				...
				awk '/^status/ { print $2 }')
		[ "$STATUS" == "completed" ] ||
		error "(4) OST${k} Expect 'completed', but got '$STATUS'"
	done
	...
	echo "The file size should be correct after layout LFSCK scanning"
	SIZE=$(ls -il $DIR/$tdir/a1/f2 | awk '{ print $6 }')
	[ "$SIZE" == "$SAVED_SIZE" ] ||
		error "(6) Expect correct file2 size $SAVED_SIZE, but got $SIZE"

	$LFS path2fid $DIR/$tdir/a1/f2
	$LFS getstripe $DIR/$tdir/a1/f2

	echo "There should be some stub under .lustre/lost+found/MDT0000/"
	ls -ail $MOUNT/.lustre/lost+found/MDT0000/E-* ||
