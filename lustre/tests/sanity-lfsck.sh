...
test_18b() {
	...
	do_facet ost1 $LCTL set_param -n \
		obdfilter.${FSNAME}-OST0000.fail_on_inconsistency 1
	cat $DIR/$tdir/f0 &
	cat $DIR/$tdir/f1 || error "(2) Read should not be denied!"
	wait
	...
	repaired=$(do_facet ost1 $LCTL get_param -n \
		...
		awk '/^repaired/ { print $2 }')
	[ $repaired -eq 2 ] ||
		error "(3) Expected 2 repaired, but got $repaired"
}
...
test_19b() {
	...
	do_facet mds1 $LCTL set_param fail_loc=0x1617
	rm -f $DIR/$tdir/a1/f1
	do_facet mds2 $LCTL set_param fail_loc=0x1617
	rm -f $DIR/$tdir/a2/f2
	sync
	sleep 2
	do_facet mds1 $LCTL set_param fail_loc=0
