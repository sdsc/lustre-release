...
[[ $(lustre_version_code $SINGLEMDS) -lt $(version_code 2.6.50) ]] &&
	ALWAYS_EXCEPT="$ALWAYS_EXCEPT 2d 22 23 24 25 26 27"
...
test_23() {
	...
	local repaired=$($SHOW_NAMESPACE |
			 awk '/^multi_referenced_repaired/ { print $2 }')
	[ $repaired -eq 1 ] ||
	...
	cname=$(find $MOUNT/.lustre/lost+found/MDT0000/ -name *-O-*)
	[ ! -z "$cname" ] ||
		error "(11) .lustre/lost+found/MDT0000/ should not be empty"
}
run_test 23 "LFSCK can repair multiple-referenced name entry"
...
test_24() {
	...
	echo "Inject failure stub on MDT0 to simulate the case that"
	echo "the file type stored in the name entry is worng."
...
run_test 24 "LFSCK can repair bad file type in the name entry"
...
test_25a() {
	...
	do_facet $SINGLEMDS $LCTL set_param fail_loc=0x1620
...
run_test 25a "LFSCK can add the missed local name entry back to the namespace"
...
run_test 25b "LFSCK can add the missed remote name entry back to the namespace"
...
run_test 26a "LFSCK can recreate the lost local parent directory as orphan"
...
run_test 26b "LFSCK can recreate the lost remote parent directory as orphan"
...
test_27() {
	...
	#define OBD_FAIL_LFSCK_NO_NAMEENTRY	0x1620
	do_facet mds1 $LCTL set_param fail_loc=0x1620
...
run_test 27 "Skip the failed MDT(s) when handle orphan MDT-objects"
