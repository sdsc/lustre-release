...
test_18f() {
	...
	if [ $MDSCOUNT -ge 2 ]; then
		...
		$LFS setstripe -c 1 -i 1 -s 1M $DIR/$tdir/a3
		dd if=/dev/zero of=$DIR/$tdir/a3/f3 bs=1M count=2
		$LFS mkdir -i 1 $DIR/$tdir/a4
		$LFS setstripe -c 2 -i 1 -s 1M $DIR/$tdir/a4
	...
	local cur_status=$(do_facet ost1 $LCTL get_param -n \
			obdfilter.$(facet_svc ost1).lfsck_layout |
			awk '/^status/ { print $2 }')
	[ "$cur_status" == "partial" ] ||
		error "(3) OST{1} Expect 'partial', but got '$cur_status'"

	cur_status=$(do_facet ost2 $LCTL get_param -n \
		     obdfilter.$(facet_svc ost2).lfsck_layout |
		     awk '/^status/ { print $2 }')
	[ "$cur_status" == "completed" ] ||
		error "(4) OST{2} Expect 'completed', but got '$cur_status'"
	...
	if [ $MDSCOUNT -ge 2 ]; then
		repaired=$(do_facet mds2 $LCTL get_param -n \
			 ...
			 awk '/^repaired_orphan/ { print $2 }')
		[ $repaired -eq 2 ] ||
		error "(6) Expect 2 fixed on mds{2}, but got: $repaired"
	fi
	...
	if [ $MDSCOUNT -ge 2 ]; then
		repaired=$(do_facet mds2 $LCTL get_param -n \
			 ...
			 awk '/^repaired_orphan/ { print $2 }')
		[ $repaired -eq 1 ] ||
		error "(11) Expect 1 fixed on mds{2}, but got: $repaired"
	fi
}
