...
test_3()
{
	...
	local repaired=$($SHOW_NAMESPACE |
			 awk '/^mul_linked_repaired/ { print $2 }')
	[ $repaired -ge 2 ] ||
...
test_22b() {
	...
	echo "fid2ptah should NOT work on the dummy's FID $dummyfid"
	local dummyname=$($LFS fid2path $DIR $dummyfid)
	[ "$dummyname" != "$DIR/$tdir/foo/dummy" ] ||
		error "(4) fid2ptah works unexpectedly."
	...
	echo "fid2ptah should work on the dummy's FID $dummyfid after LFSCK"
	local dummyname=$($LFS fid2path $DIR $dummyfid)
	[ "$dummyname" == "$DIR/$tdir/foo/dummy" ] ||
		error "(8) fid2ptah does not work"
}
...
test_23b() {
	...
	echo "to the name entry_B. But there is something worng for the name"
...
test_23c() {
	...
	sleep 3
	chown 1.1 $DIR/$tdir/d0/foo || error "(8) Fail to chown"
	...
	local repaired=$($SHOW_NAMESPACE |
			 awk '/^dangling_repaired/ { print $2 }')
	[ $repaired -eq 1 ] ||
		error "(10) Fail to repair dangling name entry: $repaired"

	local data=$(cat $DIR/$tdir/d0/foo)
	[ "$data" != "dummy" ] ||
		error "(11) The $DIR/$tdir/d0/foo should not be recovered"
}
