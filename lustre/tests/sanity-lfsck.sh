...
test_20() {
	...
	# 256 blocks on the stripe1
	# 1 block on the stripe2
	dd if=/dev/zero of=$DIR/$tdir/a1/f0 bs=4096 count=513
	dd if=/dev/zero of=$DIR/$tdir/a1/f1 bs=4096 count=513
	dd if=/dev/zero of=$DIR/$tdir/a1/f2 bs=4096 count=513
	dd if=/dev/zero of=$DIR/$tdir/a1/f3 bs=4096 count=513
	...
	[ "$pattern" == "1" ] ||
		error "(5.2) expect the pattern 1, but got $pattern"
	...
	[ $size -eq $((4096 * 513)) ] ||
		error "(5.4) expect the size $((4096 * 513)), but got $size"
	...
	# pattern = (LOV_PATTERN_RAID0 | LOV_PATTERN_F_HOLE)
	[ "$pattern" == "40000001" ] ||
		error "(6.2) expect the pattern 40000001, but got $pattern"
	...
	# stripe0 is dummy
	[ $size -eq $((4096 * (0 + 256 + 1))) ] ||
		error "(6.4) expect the size $((4096 * 257)), but got $size"

	cat $name > /dev/null && error "(6.5) normal read $name should fail"

	$LFS dump --if=$name --of=$DIR/$tdir/dump ||
		error "(6.6) lfs dump $name should work"

	size=$(stat $DIR/$tdir/dump | awk '/Size:/ { print $2 }')
	[ $size -eq $((4096 * (0 + 256 + 1))) ] ||
		error "(6.7) expect dump size $((4096 * 257)), but got $size"

	echo "dummy" >> $name && error "(6.8) write $name should fail"

	chown 1.1 $name && error "(6.9) chown on $name should fail"

	touch $name && error "(6.10) touch $name should fail"

	rm -f $name || error "(6.11) cannot unlink $name"
	...
	# pattern = (LOV_PATTERN_RAID0 | LOV_PATTERN_F_HOLE)
	[ "$pattern" == "40000001" ] ||
		error "(7.2) expect the pattern 40000001, but got $pattern"
	...
	size=$(stat $name | awk '/Size:/ { print $2 }')
	# stripe1 is dummy
	[ $size -eq $((4096 * (256 + 0 + 1))) ] ||
		error "(7.4) expect the size $((4096 * 257)), but got $size"

	cat $name > /dev/null && error "(7.5) normal read $name should fail"

	$LFS dump --if=$name --of=$DIR/$tdir/dump ||
		error "(7.6) lfs dump $name should work"

	size=$(stat $DIR/$tdir/dump | awk '/Size:/ { print $2 }')
	[ $size -eq $((4096 * (256 + 0 + 1))) ] ||
		error "(7.7) expect dump size $((4096 * 257)), but got $size"

	echo "dummy" >> $name && error "(7.8) write $name should fail"

	chown 1.1 $name && error "(7.9) chown on $name should fail"

	touch $name && error "(7.10) touch $name should fail"
	...
	#define LOV_PATTERN_RAID0	0x001
	[ "$pattern" == "1" ] ||
		error "(8.2) expect the pattern 1, but got $pattern"
