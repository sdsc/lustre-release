...
wait_all_targets_blocked()
{
	...
	local count=$(do_facet mds1 \
		      "$LCTL lfsck_query -t $com -M lustre-MDT0000 -w |
		       awk '/^${com}_mdts_${status}/ { print \\\$2 }'")
	[ $count -eq $MDSCOUNT ] || {
		do_facet mds1 "$LCTL lfsck_query -t $com -M lustre-MDT0000"
...
wait_all_targets()
{
	...
	wait_update_facet mds1 "$LCTL lfsck_query -t $com -M lustre-MDT0000 |
		awk '/^${com}_mdts_${status}/ { print \\\$2 }'" \
		"$MDSCOUNT" $LTIME || {
		do_facet mds1 "$LCTL lfsck_query -t $com -M lustre-MDT0000"
