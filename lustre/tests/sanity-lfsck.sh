...
test_18b() {
	...
	mv $MOUNT/.lustre/lost+found/MDT0000/${fid1}-0-R $DIR/$tdir/a1/f1 ||
	error "(5) Fail to move $MOUNT/.lustre/lost+found/MDT0000/${fid1}-0-R"

	if [ $MDSCOUNT -ge 2 ]; then
		local name=$MOUNT/.lustre/lost+found/MDT0001/${fid2}-0-R
...
test_20() {
	...
	#
	# ${fid0}-0-R is the old f0
	#
	local name="$MOUNT/.lustre/lost+found/MDT0000/${fid0}-0-R"
	...
	#
	# ${fid1}-0-R contains the old f1's stripe1 (and stripe2 if OSTs > 2)
	#
	name="$MOUNT/.lustre/lost+found/MDT0000/${fid1}-0-R"
	if [ $OSTCOUNT -gt 2 ]; then
	...
	#
	# ${fid2}-0-R it contains the old f2's stripe0 (and stripe2 if OSTs > 2)
	#
	name="$MOUNT/.lustre/lost+found/MDT0000/${fid2}-0-R"
	if [ $OSTCOUNT -gt 2 ]; then
	...
	#
	# ${fid3}-0-R should contains the old f3's stripe0 and stripe1
	#
	name="$MOUNT/.lustre/lost+found/MDT0000/${fid3}-0-R"
