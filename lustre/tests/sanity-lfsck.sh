...
test_20() {
	...
	if [ $OSTCOUNT -gt 2 ]; then
		[ "$pattern" == "40000001" ] ||
		error "(7.2.1) expect the pattern 40000001, but got $pattern"

		[ $stripes -eq 3 ] ||
		error "(7.3.1) expect the stripe count is 3, but got $stripes"

		[ $size -eq $((4096 * $bcount)) ] ||
		error "(7.4.1) expect the size $((4096 * $bcount)), but got $size"
		...
		echo "dummy" >> $name && error "(7.8.1) write $name should fail"

		chown 1.1 $name && error "(7.9.1) chown on $name should fail"

		touch $name && error "(7.10.1) touch $name should fail"
	else
		[ "$pattern" == "1" ] ||
			error "(7.2.2) expect the pattern 1, but got $pattern"
		...
		chown 1.1 $name || error "(7.9.2) cannot chown on $name"
