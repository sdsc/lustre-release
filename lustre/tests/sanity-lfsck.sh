...
test_1c()
{
	...
	local repaired=$($SHOW_NAMESPACE |
			 awk '/^updated_phase1/ { print $2 }')
	[ $repaired -eq 1 ] ||
		error "(5) Fail to repair missed object: $repaired"

	mount_client $MOUNT || error "(6) Fail to start client!"

	stat $DIR/$tdir/dummy > /dev/null || error "(7) no object".

	do_facet $SINGLEMDS $LCTL set_param fail_loc=0
}
run_test 1c "LFSCK can find out and repair missed object"
...
run_test 3a "LFSCK can find out and repair incorrect object nlink (1)"
...
run_test 3b "LFSCK can find out and repair incorrect object nlink (2)"
...
test_10()
{
	...
	for ((i=2; i<1000; i=$((i+3)))); do
		...
		touch $DIR/$tdir/f${i}
		for ((j=0; j<5; j++)); do
			ln $DIR/$tdir/f${i} $DIR/$tdir/d${i}/f${j}
		done
	done
	...
	unlink $DIR/$tdir/f200 || error "(11) Fail to unlink!"

	rm -rf $DIR/$tdir/d201 || error "(12) Fail to rmdir!"
	...
	stat $DIR/$tdir/d200/f2 > /dev/null || error "(18) Fail to stat!"
}
