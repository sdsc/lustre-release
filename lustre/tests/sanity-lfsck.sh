...
test_0() {
	...
	setupall || error "(15) Fail to startall"
}
...
test_4()
{
	...
	wait_update_facet $SINGLEMDS "$LCTL get_param -n \
		mdd.${MDT_DEV}.lfsck_namespace |
		awk '/^flags/ { print \\\$2 }'" "inconsistent" 3 || {
		$SHOW_NAMESPACE
		return 5
	}
...
test_5()
{
	...
	wait_update_facet $SINGLEMDS "$LCTL get_param -n \
		mdd.${MDT_DEV}.lfsck_namespace |
		awk '/^flags/ { print \\\$2 }'" "inconsistent,upgrade" 3 || {
		$SHOW_NAMESPACE
		return 5
	}
...
test_7a()
{
	...
	mount_client $MOUNT || error "(8) Fail to start client!"
}
...
test_7b()
{
	...
	wait_update_facet $SINGLEMDS "$LCTL get_param -n \
		mdd.${MDT_DEV}.lfsck_namespace |
		awk '/^status/ { print \\\$2 }'" "scanning-phase2" 3 || {
		$SHOW_NAMESPACE
		return 4
	}
...
test_8()
{
	...
	mount_client $MOUNT || error "(26) Fail to start client!"
}
...
test_10()
{
	...
	wait_update_facet $SINGLEMDS "$LCTL get_param -n \
		mdd.${MDT_DEV}.lfsck_namespace |
		awk '/^status/ { print \\\$2 }'" "completed" 5 || {
		$SHOW_NAMESPACE
		return 16
	}
}
...
test_11a() {
	...
	setupall || error "(8) Fail to startall"
}
...
test_11b() {
	...
	setupall || error "(11) Fail to startall"
}
...
test_14() {
	...
	last_id=$(do_facet mds1 lctl get_param -n \
		  osp.$mdtosc_proc.prealloc_last_id)
	next_id=$(do_facet mds1 lctl get_param -n \
		  osp.$mdtosc_proc.prealloc_next_id)
	count=$((last_id - next_id + 1))
