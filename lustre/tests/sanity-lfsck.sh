...
test_29d()
{
	...
	for ((i=0;i<300;i++)) do
		ln $DIR/$tdir/guard/f0 $DIR/$tdir/foo/t${i} ||
			error "(2) Fail to hard link $DIR/$tdir/foo/t${i}"
	done
	...
	for ((i=1;i<201;i++)) do
		rm -f $DIR/$tdir/foo/t${i} ||
			error "(4) Fail to unlink $DIR/$tdir/foo/t${i}"
	done
	...
	echo "sleep 30 seconds to guarantee that the overflow is recognized"
	sleep 30
