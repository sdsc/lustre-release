...
test_23b() {
	...
	local AVAIL=$($LFS df -p $POOL $dir | awk '/summary/ { print $5 }')
	[ $AVAIL -gt $maxfree ] &&
	...
	local etime
	local elapsed
	local maxtime=3000 # if not finished in 3000 sec, terminate it */
	while [ $RC -eq 0 ]; do
		i=$((i + 1))
		...
		elif [ $elapsed -gt $maxtime ]; then
			log "Not finished in $elapsed seconds, terminated"
			RC=1
		fi
	done
