...
check_file_in_pool() {
 	...
 	if [[ -z "$res" ]]; then
 		error "file '$file' not in any pool"
 		return 1
 	fi
 	if [[ "$res" != "$pool" ]]; then
...
check_file_in_osts() {
	...
	local count=$3
	local res=$($LFS getstripe $file | awk '/0x/ { print $2 }')
	local i
...
test_20() {
	...
	lctl mark "===== start mkdir ====="
	mkdir $dir3			# should inherit $POOL from parent $dir1
	lctl mark "===== finish mkdir ====="
	$LFS setstripe -c 1 $dir3	# should inherit $POOL from parent $dir1
	lctl mark "===== start create ====="
	touch $dir3/file3		# should inherit $POOL from $dir3
	lctl mark "===== finish create ====="
	stop_full_debug_logging
	$LFS setstripe -c 1 $dir2/file4	# should inherit $POOL2 from $dir2
	$LFS setstripe -S 1M $dir2/file5 # should inherit $POOL2 from $dir2

	check_file_in_pool $dir2/file2 $POOL2

	if [ $(lustre_version_code mds1) -ge $(version_code 2.8.50) ]; then
