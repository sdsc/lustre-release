...
test_14() {
    ...
    # Create a new filesystem that is small and balanced.
    if [ $OSTSIZE -gt 200000 ]; then
        ( OST_MKFS_OPTS="$OST_MKFS_OPTS --device-size=100000" formatall )
        setupall
    fi
...
test_23() {
    ...
    LOCALE=C $RUNAS dd if=/dev/zero of=$file bs=$BLK_SZ count=$((BUNIT_SZ*2))
    $LFS quota -v -u $RUNAS_ID $dir
    cancel_lru_locks osc
    stat=$(LOCALE=C $RUNAS dd if=/dev/zero of=$file bs=$BLK_SZ \
    ...
    while [ $RC -eq 0 ]; do
        ...
        if [ $RC -eq 1 ]; then
            ...
            [[ $? -ne 0 ]] &&
                error "dd did not fail with ENOSPC; failed with $stat"
        fi
    done
