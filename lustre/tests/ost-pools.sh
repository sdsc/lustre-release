...
sub_test_5() {
	...
	if [[ ! $(grep $SINGLEMDS <<< $LCMD) ]]; then
		...
		mkdir ${DIR}/d1 || error "mkdir d1 failed"
...
test_25() {
	...
	for P in $(seq -f $POOL%g 10); do
		create_pool_nofail $P
		do_facet $SINGLEMDS "lctl pool_add $FSNAME.$P OST0000; sync"
		wait_update $HOSTNAME "lctl get_param -n lov.$FSNAME-*.pools.$P |
			sort -u | tr -d '\n'" "$FSNAME-OST0000_UUID" ||
		...
		rm $POOL_ROOT/$tfile
		if [[ "$STR" == "0" ]]; then
			echo "Creating a file in $P"
			create_file $POOL_ROOT/file-$P $P || break
			check_file_in_pool $POOL_ROOT/file-$P $P || break
		else
