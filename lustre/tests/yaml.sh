...
yml_nodes_file() {
    ...
    if [ -f $logdir/shared ]; then
        do_rpc_nodes $(comma_list $(nodes_list)) \
            "yml_node >> $logdir/node.\\\$(sed 's/\\\\..*//' <<< \\\$(hostname)).yml"
    else
...
yml_test_group() {
    TEST_GROUP=${TEST_GROUP:-"acc-sm-$(short_hostname)"}
    TEST_HOST=${TEST_HOST:-$(short_hostname)}
...
yml_entities() {
    ...
    host=$(short_hostname $(facet_active_host $(facet_active_host mds)))
