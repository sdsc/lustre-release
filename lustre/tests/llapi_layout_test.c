...
void test0(void)
{
	...
	VERIFYF(rc == 0 && count == T0_STRIPE_COUNT, "%lu != %d", count,
		T0_STRIPE_COUNT);
	...
	VERIFYF(rc == 0 && size == T0_STRIPE_SIZE, "%lu != %d", size,
		T0_STRIPE_SIZE);
...
void __test1_helper(llapi_layout_t *layout)
{
	...
	rc = llapi_layout_stripe_count_get(layout, &count);
	VERIFYF(count == T0_STRIPE_COUNT, "%lu != %d", count, T0_STRIPE_COUNT);

	rc = llapi_layout_stripe_size_get(layout, &size);
	VERIFYF(size == T0_STRIPE_SIZE, "%lu != %d", size, T0_STRIPE_SIZE);
	...
	VERIFYF(rc == 0, "errno = %d", errno);
	VERIFYF(ost0 == T0_OST_OFFSET, "%lu != %d", ost0, T0_OST_OFFSET);
	VERIFYF(ost1 != ost0, "%lu == %lu", ost0, ost1);
}
...
void test4(void)
{
	...
	snprintf(cmd, sizeof(cmd), "%s setstripe -p %s -c %d -s %d %s", lfs,
	...
	rc = llapi_layout_stripe_count_get(layout, &count);
	VERIFYF(count == T4_STRIPE_COUNT, "%lu != %d", count, T4_STRIPE_COUNT);

	rc = llapi_layout_stripe_size_get(layout, &size);
	VERIFYF(size == T4_STRIPE_SIZE, "%lu != %d", size, T4_STRIPE_SIZE);
...
void test14(void)
{
	...
	/* NULL path */
	errno = 0;
	rc = llapi_layout_file_create(layout, NULL, 0, 0);
	VERIFYF(rc == -1 && errno == EINVAL, "rc = %d, errno = %d", rc, errno);
...
void test15(void)
{
	...
	errno = 0;
	fd = llapi_layout_file_create(layout, file, 0, 0640);
	...
	errno = 0;
	fd = llapi_layout_file_open(layout, file, 0, 0640);
	...
	VERIFYF(layout != NULL, "errno = %d", errno);
	rc = llapi_layout_stripe_count_get(layout, &count);
	VERIFYF(rc == 0 && count == T15_STRIPE_COUNT, "rc = %d, %lu != %d", rc,
		count, T15_STRIPE_COUNT);
	llapi_layout_free(layout);
}
...
void test16(void)
{
	...
	__get_parent_dir(file, parent, sizeof(parent));

	rc = unlink(file);
	VERIFYF(rc == 0 || errno == ENOENT, "errno = %d", errno);

	dirlayout = llapi_layout_by_path(parent);
	...
	fd = llapi_layout_file_create(filelayout, file, 0, 0640);
	VERIFYF(fd >= 0, "errno = %d", errno);
	...
	VERIFYF(rc == 0, "errno = %d", errno);
	rc = llapi_layout_stripe_count_get(dirlayout, &dcount);
	VERIFYF(rc == 0, "errno = %d", errno);
	VERIFYF(fcount == dcount, "%lu != %lu", fcount, dcount);

	rc = llapi_layout_stripe_size_get(filelayout, &fsize);
	VERIFYF(rc == 0, "errno = %d", errno);
	rc = llapi_layout_stripe_size_get(dirlayout, &dsize);
	VERIFYF(rc == 0, "errno = %d", errno);
	VERIFYF(fsize == dsize, "%lu != %lu", fsize, dsize);
...
void test17(void)
{
	...
	VERIFYF(osts_layout == osts_all, "%lu != %d", osts_layout, osts_all);
...
void test21(void)
{
	...
	fd = llapi_layout_file_create(layout, template, 0640, 0);
...
void test22(void)
{
	...
	fd = llapi_layout_file_create(NULL, file, 0, modein);
	VERIFYF(fd >= 0, "errno = %d", errno);
	...
	VERIFYF(modein == modeout, "%d != %d", modein, modeout);
}
