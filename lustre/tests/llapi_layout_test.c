...
void test1(void)
{
	...
	snprintf(file, sizeof(file), "%s/%s", lustre_dir, T0FILE);
	struct llapi_layout *layout = llapi_layout_by_path(file);
	VERIFYF(layout != NULL, "errno = %d", errno);
...
void test2(void)
{
	...
	struct llapi_layout *layout = llapi_layout_by_fd(fd);
	VERIFYF(layout != NULL, "errno = %d", errno);
...
{
	errno = 0;
	struct llapi_layout *layout = llapi_layout_by_fd(9999);
...
/* llapi_layout_by_path() returns default layout for file with no
 * striping attributes. */
#define T8FILE		"t8"
...
void test8(void)
{
	...
	rc = llapi_layout_stripe_count_get(layout, &count);
	VERIFYF(rc == 0, "errno = %d\n", errno);
	VERIFYF(count == LLAPI_LAYOUT_DEFAULT, "count = %"PRIu64"\n", count);

	rc = llapi_layout_stripe_size_get(layout, &size);
	VERIFYF(rc == 0, "errno = %d\n", errno);
	VERIFYF(size == LLAPI_LAYOUT_DEFAULT, "size = %"PRIu64"\n", size);

	rc = llapi_layout_pattern_get(layout, &pattern);
	VERIFYF(rc == 0, "errno = %d\n", errno);
	VERIFYF(pattern == LLAPI_LAYOUT_DEFAULT, "pattern = %"PRIu64"\n",
		pattern);
...
void test16(void)
{
	...
	deflayout = llapi_layout_expected(lustre_dir);
...
#define T23_DESC	"llapi_layout_by_path fails for non-Lustre file"
void test23(void)
{
	struct llapi_layout *layout;
	char template[PATH_MAX];
...
/* llapi_layout_expected() returns expected layout for file with
 * unspecified layout. */
#define T24FILE		"t24"
#define T24_DESC	"llapi_layout_expected works with existing file"
void test24(void)
{
	int fd;
	int rc;
	...
	snprintf(file, sizeof(file), "%s/%s", lustre_dir, T24FILE);
	...
	layout = llapi_layout_expected(file);
	VERIFYF(layout != NULL, "errno = %d\n", errno);

	rc = llapi_layout_stripe_count_get(layout, &count);
	VERIFYF(rc == 0, "errno = %d\n", errno);
	VERIFYF(count != LLAPI_LAYOUT_DEFAULT, "expected literal value");

	rc = llapi_layout_stripe_size_get(layout, &size);
	VERIFYF(rc == 0, "errno = %d\n", errno);
	VERIFYF(size != LLAPI_LAYOUT_DEFAULT, "expected literal value");

	rc = llapi_layout_pattern_get(layout, &pattern);
	VERIFYF(rc == 0, "errno = %d\n", errno);
	VERIFYF(pattern != LLAPI_LAYOUT_DEFAULT, "expected literal value");

	llapi_layout_free(layout);
}

/* llapi_layout_expected() returns expected layout for directory with
 * unspecified layout. */
#define T25DIR		"d25"
#define T25_DESC	"llapi_layout_expected works with directory"
void test25(void)
{
	int rc;
	struct llapi_layout *layout;
	...
	layout = llapi_layout_expected(dir);
	VERIFYF(layout != NULL, "errno = %d\n", errno);
...
/* llapi_layout_expected() correctly combines specified attributes of parent
 * directory with attributes filesystem root.  */
#define T26DIR		"d26"
#define T26_DESC	"llapi_layout_expected partially specified parent"
...
void test26(void)
{
	...
	rc = llapi_layout_stripe_size_get(layout, &size);
	VERIFYF(rc == 0, "errno = %d\n", errno);
	VERIFYF(size == T26_STRIPE_SIZE, "size = %"PRIu64, size);
...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < NUM_TESTS; i++) {
		...
		if (test(tst->test_fn, tst->test_desc, tst->test_skip, i) != 0)
			rc++;
	}
	return rc;
}
