 ...
 * unspecified layout. */
#define T25FILE		"t25"
#define T25_DESC	"llapi_layout_expected works with existing file"
void test25(void)
{
	int fd;
	int rc;
	...
	snprintf(file, sizeof(file), "%s/%s", lustre_dir, T25FILE);
 ...
 * unspecified layout. */
#define T26DIR		"d26"
#define T26_DESC	"llapi_layout_expected works with directory"
void test26(void)
{
	int rc;
	struct llapi_layout *layout;
...
#define T27DIR		"d27"
#define T27_DESC	"llapi_layout_expected partially specified parent"
#define T27_STRIPE_SIZE	(1048576 * 4)
void test27(void)
{
	int rc;
	struct llapi_layout *layout;
	...
	snprintf(dir, sizeof(dir), "%s/%s", lustre_dir, T27DIR);
	...
	rc = llapi_layout_stripe_size_get(layout, &size);
	VERIFYF(rc == 0, "errno = %d\n", errno);
	VERIFYF(size == T27_STRIPE_SIZE, "size = %"PRIu64, size);
