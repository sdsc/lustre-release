...
void test0(void)
{
	...
	/* create */
	fd = llapi_layout_file_open(file, 0, 0660, layout);
...
void test13(void)
{
	...
	rc = llapi_layout_stripe_count_set(layout, T13_STRIPE_COUNT);
	VERIFYF(rc == 0, "errno = %d", errno);
	fd = llapi_layout_file_open(file, 0, 0644, layout);
	VERIFYF(fd >= 0, "errno = %d", errno);
	rc = close(fd);
	VERIFYF(rc == 0, "errno = %d", errno);
	llapi_layout_free(layout);
...
/* Verify llapi_layout_file_open() returns errors as expected */
#define T14_DESC	"llapi_layout_file_open error handling"
void test14(void)
{
	int rc;
	llapi_layout_t *layout = llapi_layout_alloc();
	...
	/* NULL path */
	errno = 0;
	rc = llapi_layout_file_open(NULL, 0, 0, layout);
	VERIFYF(rc == -1 && errno == EINVAL, "rc = %d, errno = %d", rc, errno);
...
void test16(void)
{
	...
	fd = llapi_layout_file_open(file, 0, 0640, filelayout);
	VERIFYF(fd >= 0, "errno = %d", errno);
...
/* Setting stripe count to LLAPI_LAYOUT_USE_ALL_OSTS uses all available OSTs. */
#define T17FILE		"t17"
#define T17_DESC	"count=LLAPI_LAYOUT_USE_ALL_OSTS uses all available OSTs"
void test17(void)
{
	int rc;
	int fd;
	...
	layout = llapi_layout_alloc();
	VERIFYF(layout != NULL, "errno = %d", errno);
	rc = llapi_layout_stripe_count_set(layout, LLAPI_LAYOUT_USE_ALL_OSTS);
	VERIFYF(rc == 0, "errno = %d", errno);
	fd = llapi_layout_file_open(file, 0, 0640, layout);
	VERIFYF(fd >= 0, "errno = %d", errno);
	rc = close(fd);
...
#define T20FILE		"t20"
#define T20_DESC	"LLAPI_LAYOUT_USE_FS_DEFAULT is honored"
void test20(void)
{
	int		rc;
	int		fd;
	...
	rc = llapi_layout_stripe_size_set(filelayout, LLAPI_LAYOUT_USE_FS_DEFAULT);
	VERIFYF(rc == 0, "rc = %d, errno = %d", rc, errno);

	rc = llapi_layout_stripe_count_set(filelayout, LLAPI_LAYOUT_USE_FS_DEFAULT);
	VERIFYF(rc == 0, "rc = %d, errno = %d", rc, errno);
...
#define T21_DESC	"llapi_layout_file_open fails for non-Lustre file"
void test21(void)
{
	llapi_layout_t *layout;
	...
	fd = llapi_layout_file_open(template, 0, 0640, layout);
...
#define T22FILE		"t22"
#define T22_DESC	"llapi_layout_file_open applied mode correctly"
void test22(void)
{
	int		rc;
	int		fd;
	...
	fd = llapi_layout_file_open(file, 0, modein, NULL);
	VERIFYF(fd >= 0, "errno = %d", errno);
