...
test_99() {
	...
	for ((i = $nrule_old; i < $max; i++)); do
		set_rule $FSNAME o2ib$i any krb5n || error "set rule $i"
	done
	for ((i = $nrule_old; i < $max; i++)); do
		set_rule $FSNAME o2ib$i any || error "remove rule $i"
	done
	...
	for ((i = $nrule_old; i < $max; i++)); do
		set_rule $FSNAME-MDT0000 o2ib$i any krb5i || error "set rule $i"
	done
	for ((i = $nrule_old; i < $max; i++)); do
		set_rule $FSNAME-MDT0000 o2ib$i any || error "remove rule $i"
	done
