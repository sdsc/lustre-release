...
void setup_ladvise_requestlock(struct llapi_lu_ladvise *advice, int mode,
...
static void test10(void)
{
	...
	setup_ladvise_requestlock(&advice, MODE_WRITE_USER, 0, 0,
				  write_size - 1);
...
static void test12(void)
{
	...
	for (i = 0; i < count; i++) {
		setup_ladvise_requestlock(&(advice[i]), MODE_WRITE_USER, 0, 0,
					  write_size - 1);
...
static void test14(void)
{
	...
	for (i = 0; i < num_blocks; i++) {
		...
		advice = malloc(sizeof(struct llapi_lu_ladvise)*count);
		setup_ladvise_requestlock(advice, MODE_WRITE_USER, 0, start,
					  end);
		advice[0].lla_requestlock_result = 98674;
...
/* Use requestlock to verify behavior of ladvise locknoexpand */
static void test16(void)
{
	...
	setup_ladvise_requestlock(advice, MODE_WRITE_USER, 0, start, end);
	...
	ASSERTF(rc == 0, "cannot requestlock '%s' : %s",
		mainpath, strerror(errno));
	ASSERTF(advice[0].lla_requestlock_result == LRL_RESULT_SAME,
		"unexpected extent result for extent: %d",
		advice[0].lla_requestlock_result);
	...
	ASSERTF(rc == 0, "cannot requestlock '%s' : %s",
		mainpath, strerror(errno));
	ASSERTF(advice[0].lla_requestlock_result == LRL_RESULT_DIFFERENT,
		"unexpected extent result for extent %d",
		advice[0].lla_requestlock_result);
...
static void test20(void)
{
	...
	setup_ladvise_requestlock(advice, 0x65464, 0, start, end);
	rc = llapi_ladvise(fd, 0, count, advice);
	ASSERTF(rc == -1 && errno == EINVAL,
	...
	setup_ladvise_requestlock(advice, MODE_WRITE_USER, 0x80, start, end);
	rc = llapi_ladvise(fd, 0, count, advice);
	ASSERTF(rc == -1 && errno == EINVAL,
	...
	setup_ladvise_requestlock(advice, MODE_WRITE_USER, 0xff, start, end);
	rc = llapi_ladvise(fd, 0, count, advice);
	ASSERTF(rc == -1 && errno == EINVAL,
		"unexpected return for bogus flags: %u %d %s",
	...
	setup_ladvise_requestlock(advice, MODE_WRITE_USER, 0xffffffff, start,
				  end);
	rc = llapi_ladvise(fd, 0, count, advice);
	ASSERTF(rc == -1 && errno == EINVAL,
		"unexpected return for bogus flags: %u %d %s",
