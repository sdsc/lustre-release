...
# XXX Once we drop the interoprability with old server (< 2.4), the
#     sanity-qutoa-old.sh should be removed.
if [ $(lustre_version_code $SINGLEMDS) -lt $(version_code 2.4.0) ]; then
	sh $LUSTRE/tests/sanity-quota-old.sh
	exit_status
fi
...
test_6() {
	...
	TESTFILE=$DIR/$tdir/$tfile-$TSTUSR
	$LFS setstripe $TESTFILE -c 1 -i 0
	chown $TSTUSR.$TSTUSR $TESTFILE

	# create file for $TSTUSR2
	TESTFILE2=$DIR/$tdir/$tfile-$TSTUSR2
...
test_20() { # b15754
	LSTR=(2g 1t 4k 3m) # limits strings
	# limits values
	LVAL=($[2*1024*1024] $[1*1024*1024*1024] $[4*1024] $[3*1024*1024])
...
test_23_sub() {
	...
	OST0_UUID=$(ostuuid_from_index 0)
...
test_23() {
	OST0_MIN=$((6 * 1024)) # 6MB, extra space for meta blocks.
	check_whether_skip && return 0
...
test_33() {
	INODES=10 # 10 files
	BLK_CNT=2 # of 2M each
	TOTAL_BLKS=$(($INODES * $BLK_CNT * 1024))
...
test_36() {
	[ $FSTYPE != "ldiskfs" ] && skip "skipping migration test" && return

	# get the mdt0 device name
	local mdt0_node=$(facet_host mds1)
	local mdt0_dev=$(do_node $mdt0_node \
		"lctl get_param -n osd*.*MDT0000.mntdev")
	if [[ $mdt0_dev = *loop* ]]; then
		mdt0_dev=$(do_node $mdt0_node "losetup $mdt0_dev" | \
			sed -e "s/.*(//" -e "s/).*//")
	fi
	...
	do_node $mdt0_node mount -t $MDSFSTYPE $MDS_MOUNT_OPTS $mdt0_dev $mntpt
