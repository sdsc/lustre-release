...
if [ "x$USE_OFD" = "xyes" -a -z "$ONLY" ]; then
...
if [ "${OSTFSTYPE:-$FSTYPE}" = "ldiskfs" -a "x$USE_OFD" = "xyes" ]; then
	for num in `seq $OSTCOUNT`; do
		# not the most efficient way to enable the quota feature on ost
		# but it still allows us to test ofd accounting for now
		device=$(ostdevname $num)
		stop ost$num
		do_facet ost$num "$TUNE2FS -O quota $device"
		[ ${PIPESTATUS[0]} -ne 0] && \
			error "failed to enable quota ext4 feature for ost$num"
