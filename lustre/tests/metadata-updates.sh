...
run_test 1 "create files (mknod), write and  check data, check file attributes"
...
run_test 2 "file(s) attributes modification"
...
run_test 3 "truncate file(s) to 0 size, check new file size"
...
run_test 4 "directory content solidity"
...
test_5()
{
	if [ -f "$WRITE_DISJOINT" ]; then
		...
		mpi_run ${MACHINEFILE_OPTION} $MACHINEFILE \
			...
			$WRITE_DISJOINT -f $WRITE_DISJOINT_FILE -n $NUMLOOPS ||
...
run_test 5 "write_disjoint test"
