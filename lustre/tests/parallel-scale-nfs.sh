...
# don't init_test_env since this file is called
# from parallel-scale-nfsv[3|4].sh
#init_test_env $@
...
client_mnt_opt="-o user_xattr,flock"
[ $NFS_VERSION = "4"] && client_mnt_opt="$client_mnt_opt,32bitapi"
zconf_mount_clients $lustre_client $MOUNT $client_mnt_opt || \
    error "mount lustre on $lustre_client failed"

# setup the nfs
if ! setup_nfs "$NFS_VERSION" "$MOUNT" "$lustre_client" "$CLIENTS"; then
