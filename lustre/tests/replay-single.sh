...
test_119() {
	...
	local hard_timeout=$(do_facet mds1 \
		"lctl get_param -n mdt.*0000.recovery_time_hard")
	...
	#stop mds and restart with shrink recovery_time_hard
	stop mds1
	change_active mds1
	wait_for_facet mds1
	...
	do_facet mds1 $LCTL set_param mdt.*0000.recovery_time_hard=$hard_timeout

	[ -e "$DIR/$tdir/dir_1" ] && error "dir_1 still exists"
...
test_120() {
	...
	for ((i=0;i<20;i++)); do
		[ -e "$DIR/$tdir/dir-$i" ] && error "dir-$i still exists"
		[ -e "$DIR/$tdir/stripe_dir-$i" ] && error "dir-$i still exists"
	done
	true
}
