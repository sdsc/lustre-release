...
test_58b() {
    ...
    large_xattr_enabled && orig="$(generate_string 65536)" || orig="bar"
    ...
    fail $SINGLEMDS
    new=$(getfattr --absolute-names --only-values -n trusted.foo \
          $MOUNT2/$tdir/$tfile)
    [[ "$new" = "$orig" ]] || return 1
    rm -f $DIR/$tdir/$tfile
    rmdir $DIR/$tdir
...
test_58c() { # bug 16570
    ...
    if large_xattr_enabled; then
        orig="$(generate_string 32768)"
        orig1="$(generate_string 65536)"
    else
        orig="bar"
        orig1="bar1"
    fi
    ...
    drop_reint_reply "setfattr -n trusted.foo1 -v $orig1 $DIR/$tdir/$tfile" || \
        return 3
    new=$(getfattr --absolute-names --only-values -n trusted.foo1 \
          $MOUNT2/$tdir/$tfile)
    [[ "$new" = "$orig1" ]] || return 4
    rm -f $DIR/$tdir/$tfile
    rmdir $DIR/$tdir
