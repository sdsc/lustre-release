...
test_70c () {
	...
	local end_run_file=$DIR/run_file_70c
	...
	do_nodesv $clients "set -x; PATH=\$PATH:$LUSTRE/utils:$LUSTRE/tests/ \
MOUNT=$MOUNT \
TESTDIR=$test_dir \
BREAK_ON_ERROR=yes \
END_RUN_FILE=$end_run_file \
LOAD_PID_FILE=tmp_pid_70c \
TESTNAME=test_70c \
...
run_tar.sh" &
	...
	log "Started run_tar load pid=$pids ..."
	...
	do_nodesv $clients "kill -s TERM $pids" || true
}
run_test 70c "run_tar ${MDSCOUNT}mdts recovery; $CLIENTCOUNT clients"
