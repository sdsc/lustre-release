...
test_0c() {
	...
	rm $DIR/$tfile && { error "File exists and it shouldn't"; return 1; }
	return 0
}
...
test_44c() {
	...
	createmany -m $DIR/$tfile-%d 100 ||
		{ error "failed to create directories"; return 1; }
#define OBD_FAIL_TGT_RCVG_FLAG 0x712
	do_facet $SINGLEMDS "lctl set_param fail_loc=0x80000712"
	fail_abort $SINGLEMDS
	unlinkmany $DIR/$tfile-%d 100 &&
		{ error "unliked after fail abort"; return 2; }
	fail $SINGLEMDS
	unlinkmany $DIR/$tfile-%d 100 &&
		{ error "unliked after fail"; return 3; }
	return 0
}
