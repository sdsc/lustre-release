...
namespace_gen_one()
{
	...
	createmany -m ${basedir}/f $count > /dev/null || {
		umount_client $mntpt
	...
	for ((m = 0; m < $count; m++)); do
		createmany -m $basedir/l${m}/lf 5 > /dev/null || {
			umount_client $mntpt
			error_noexit \
	...
	for ((m = 0; m < $count; m++)); do
		...
		createmany -m $basedir/r${m}/rf 4 > /dev/null || {
			umount_client $mntpt
			error_noexit \
	...
	for ((m = 0; m < $count; m++)); do
		...
		createmany -m $basedir/s${m}/sf 4 > /dev/null || {
			umount_client $mntpt
			error_noexit \
