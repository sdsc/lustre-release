...
namespace_gen_one() {
	...
	$LFS setstripe -c 1 -i $idx4 $show_dir || {
		error_noexit "(21) Fail to setstriped on $show_dir"
		return 21
	}
	...
	wait || {
		error_noexit "(22) Fail to gen regular files under $show_dir"
		return 22
	}
	...
	createmany -d $work_dir/d_l_ $count > /dev/null || {
		error_noexit "(23) Fail to gen local sub-dir under $show_dir"
		return 23
	}
	...
	for ((m = 0; m < $count; m++)); do
		createmany -o $work_dir/d_l_${m}/f_l_ 5 > /dev/null || {
			error_noexit \
			"(24) Fail to gen regular under $work_dir/d_l_${m}"
			return 24
		}
	done
	...
	for ((m = 0; m < $count; m++)); do
		ln $work_dir/f_0_${m} $work_dir/f_m_${m} || {
			error_noexit \
			"(25) Fail to hardlink to $work_dir/f_0_${m}"
			return 25
		}
	done
	...
	for ((m = 0; m < $count; m++)); do
		$LFS mkdir -i ${idx3} $work_dir/d_r_${m} || {
			error_noexit \
			"(26) Fail to remote mkdir $work_dir/d_r_${m}"
			return 26
		}
		...
		createmany -o $work_dir/d_r_${m}/f_r_ 4 > /dev/null || {
			error_noexit \
			"(27) Fail to gen regular under $work_dir/d_r_${m}"
			return 27
		}
	done
	...
	for ((m = 0; m < $count; m++)); do
		$LFS setdirstripe -i ${idx1} -c 2 -t all_char \
			$work_dir/d_s_${m} || {
			error_noexit \
			"(28) Fail to make striped-dir $work_dir/d_s_${m}"
			return 28
		}
		...
		createmany -o $work_dir/d_s_${m}/f_s_ 4 > /dev/null || {
			error_noexit \
			"(29) Fail to gen regular under $work_dir/d_s_${m}"
			return 29
		}
	done
}
