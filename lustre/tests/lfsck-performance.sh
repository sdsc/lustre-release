...
MINCOUNT_BACKUP=${MINCOUNT_BACKUP:-8192}
MAXCOUNT_BACKUP=${MAXCOUNT_BACKUP:-32768}
...
test_1() {
	...
	for ((i=$MINCOUNT_BACKUP; i<=$MAXCOUNT_BACKUP; i=$((i * FACTOR)))); do
		local nfiles=$((i - BCOUNT))
...
test_3() {
	...
	[ $MDSSIZE -lt 2000000 ] &&
		skip "MDT device is too small, expect at last 2GB" && exit 0
