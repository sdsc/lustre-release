...
namespace_gen_one()
{
	...
	for ((m = 0; m < $NTHREADS; m++)); do
		...
		createmany -m ${sub_dir}/f_${m}_ $sub_count > /dev/null &
	done
	...
	for ((m = 0; m < $count; m++)); do
		createmany -m $work_dir/d_l_${m}/f_l_ 5 > /dev/null || {
			error_noexit \
	...
	for ((m = 0; m < $count; m++)); do
		...
		createmany -m $work_dir/d_r_${m}/f_r_ 4 > /dev/null || {
			error_noexit \
	...
	for ((m = 0; m < $count; m++)); do
		...
		createmany -m $work_dir/d_s_${m}/f_s_ 4 > /dev/null || {
			error_noexit \
