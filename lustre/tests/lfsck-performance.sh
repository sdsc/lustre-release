...
lfsck_attach() {
	${ECHOCMD} "modprobe obdecho; ${LCTL} <<-EOF
...
namespace_gen_one()
{
	...
	local count=$((UNIT / 100 * 90)) # 90% regular files
	...
	count=$((UNIT /100 * 8)) # 8% local sub-dirs
	...
	for ((m = 0; m < $count; m++)); do
		...
		umount_client $mntpt
		error_noexit "(18) Fail to make striped-dir $basedir/r${m}"
		return 18
	}
	done
...
t7_test()
{
	...
	for ((i = 2; i <= $saved_mdscount; i = $((i + 2)))); do
		...
		for ((j = $MINSUBDIR; j <= $MAXSUBDIR; j = $((j + MINSUBDIR)))); do
			echo "formatall"
			formatall > /dev/null ||
				error "(2) Fail to formatall, subdirs=${j}"
