...
{
	return syscall(SYS_name_to_handle_at, mnt_fd, filename, fh,
		       &mnt_id, flags);
}
...
{
	return syscall(SYS_open_by_handle_at, mnt_fd, fh, mode);
}
...
int main(int argc, char **argv)
{
	...
	int ret, rc = -EINVAL, mnt_id, mnt_fd, fd, i, len, offset;
	...
	/* Get file stats using fd from traditional open */
