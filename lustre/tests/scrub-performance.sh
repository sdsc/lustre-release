...
[ "$MDSFSTYPE" != "ldiskfs" ] && skip "OI scrub performance only for ldiskfs" &&
	exit 0
...
remote_mds && SAVED_LOAD_MODULES_REMOTE=${LOAD_MODULES_REMOTE} &&
...
scrub_attach() {
	if [ ! -z "${SAVED_LOAD_MODULES_REMOTE}" ]; then
		${RCMD} "${LCTL} <<-EOF
...
scrub_cleanup() {
	unload_modules
	[ ! -z "$SAVED_LOAD_MODULES_REMOTE" ] &&
		LOAD_MODULES_REMOTE=${SAVED_LOAD_MODULES_REMOTE}
	formatall
}
