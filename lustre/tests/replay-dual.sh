...
test_26() {
	...
	trap cleanup_26	EXIT
	(
		local tar_dir2=$DIR/$tdir/run_tar2
		while true; do
			test_mkdir -p -c$MDSCOUNT $tar_dir2 || break
			[ $MDSCOUNT -ge 2 ] &&
			$LFS setdirstripe -D -c$MDSCOUNT $tar_dir2 ||
				error "set default dirstripe failed"
			cd $tar_dir2 || break
			tar cf - /etc | tar xf - || error "tar failed"
			cd $DIR/$tdir || break
			rm -rf $tar_dir2 || break
		done
	)&
	tar_26_pid2=$!
	echo "Started tar $tar_26_pid2"
	...
	while [ $elapsed -lt $duration ]; do
		...
		kill -0 $tar_26_pid2 || error "tar $tar_26_pid2 stopped"
