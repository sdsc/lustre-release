...
facet_failover() {
	...
	#
	# We don't write client data synchrnously (to avoid flooding sync
	# writes, when there are many clients connecting), so if the server
	# reboots before the client data reachs disk, the client data will
	# be lost and the client will be evicted after recovery, which is
