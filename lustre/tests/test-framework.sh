...
facet_fsck() {
	...
	case $fstype in
	ldiskfs)
		run_fsck_ldiskfs $(facet_host $facet) $facet_dev $lfsck_opts
		rc=$?
		;;
	zfs)
		run_fsck_zfs $(facet_host $facet) $facet_dev
		rc=$?
		;;
...
run_fsck_all() {
	...
	for facet in $(get_facets MDT); do
