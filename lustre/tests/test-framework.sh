...
if [ -d /etc/modprobe.d ]; then
	OFD_MOD_CONF=/etc/modprobe.d/lustre-tests-ofd.conf
else
	OFD_MOD_CONF=/etc/modprobe.conf
fi

enable_ofd() {
	local nodes=$1
	local node
	local nodes_obdfilter

	for node in ${nodes//,/ }; do
		if do_node $node lsmod | grep -q '^\<obdfilter\>'; then
			nodes_obdfilter+=,$node
		fi
	done
	if [ -n "$nodes_obdfilter" ]; then
		if ! do_nodes ${nodes_obdfilter#,} $LUSTRE_RMMOD; then
			echo "$nodes_obdfilter still using obdfilter.ko"
		fi
	fi

	echo "Updating $OFD_MOD_CONF on $nodes to enable OFD"
	do_nodes $nodes "
		if ! grep 'blacklist obdfilter' $OFD_MOD_CONF &>/dev/null; then
			echo 'blacklist obdfilter' >>$OFD_MOD_CONF;
		fi;
		if ! grep 'alias obdfilter ofd' $OFD_MOD_CONF &>/dev/null; then
			echo 'alias obdfilter ofd' >>$OFD_MOD_CONF;
		fi
	"
}
...
disable_ofd() {
	...
	echo "Updating $OFD_MOD_CONF on $nodes to disable OFD"
	do_nodes $nodes sed -i -e "'/blacklist obdfilter/d'" $OFD_MOD_CONF
	do_nodes $nodes sed -i -e "'/alias obdfilter ofd/d'" $OFD_MOD_CONF
}
...
init_param_vars () {
	...
	if [ $QUOTA_AUTO -ne 0 ] && [ -z "$USE_OFD" ]; then
