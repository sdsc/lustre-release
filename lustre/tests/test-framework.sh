...
export GSS=false
export GSS_SK=false
# SK MDT<>MDT feature does not appear complete
export SK_MDT_INCOMPLETE=1
# flag to trigger if key needs to be generated
export SK_FILE=/tmp/test-framework.key
export SK_NO_KEY=true
...
restore_to_default_flavor()
{
	...
	if [ $GSS_SK ] && [ $SK_MDT_INCOMPLETE ]; then
...
set_flavor_all()
{
	...
	if [ $GSS_SK ] && [ $SK_MDT_INCOMPLETE ] && [ $flavor == "ski" ]; then
		echo "Skipping MDS test of ski until supported by shared key feature"
		set_rule $FSNAME any cli2mdt ski
		set_rule $FSNAME any cli2ost ski
		set_rule $FSNAME any mdt2ost null
		set_rule $FSNAME any mdt2mdt null
		wait_flavor cli2mdt ski
		wait_flavor cli2ost ski
		wait_flavor mdt2ost null
		wait_flavor mdt2mdt null 0
	else
