...
dump_command_output() {
    ...
    if [ -z "$facets" -o -z "$cmd" -o -z "$logname" ]; then
        error_noexit false "Invalid parameters for dump_command_output()"
    fi
    for facet in ${facets//,/ }; do
        node=$(facet_host ${facet})
...
log_zfs_info() {
    ...
    if [ "$(facet_fstype ost1)" = "zfs" ]; then
        dump_file_contents "$(get_facets OST)" "/proc/spl" "${logname}"
        dump_command_output "$(get_facets OST)" "zpool events -v" "${logname}"
    fi

    if [ "$(facet_fstype mds1)" = "zfs" ]; then
        dump_file_contents "$(get_facets MDS)" "/proc/spl" "${logname}"
        dump_command_output "$(get_facets MDS)" "zpool events -v" "${logname}"
    fi
}
...
run_one_logged() {
	...
	local test_log=$LOGDIR/$name
        local zfs_log_name=${TESTSUITE}.test_${1}.zfs_info.$(hostname -s).log
