...
# Calling get_nodes $nodesN returns a specific node.
# For get_node $node this will return all the nodes
# of a specific type
get_nodes() {
    local kind=$(echo ${1}_HOST | sed "s/[0-9]//g")

    [ "${1:0:6}" == "clients" ] && kind="CLIENTS"
    echo $(hostlist_expand "${!kind}")
}
