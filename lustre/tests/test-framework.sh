...
export SK_FILE=${SK_FILE:-main.key}
export SK_DEFAULT_FLAVOR=${SK_DEFAULT_FLAVOR:-ski}
...
init_gss() {
	...
	if [ $GSS_SK ] && [ $SK_NO_KEY ]; then
		...
		echo "Synchronizing Shared Key files..."
		local LGSSC_CONF_FILE="/etc/request-key.d/lgssc.conf"
		local all_nodes_comma=$(comma_list $(all_nodes))
		local list2=$(comma_list $(all_nodes))
		echo "$LGSSC_CONF_LINE" > $LGSSC_CONF_FILE
		do_nodes $all_nodes_comma "rsync -av $HOSTNAME:$LGSSC_CONF_FILE $LGSSC_CONF_FILE >/dev/null 2>&1" 

		# create shared key on all nodes
		mkdir -p $SK_PATH/nodemap
		rm -f $SK_PATH/*.key
		rm -f $SK_PATH/nodemap/*.key
		# for nodemap testing each client may need own key
		NUMCLIENTS=${1:-$CLIENTCOUNT}
		/usr/sbin/lgss_sk -f$FSNAME -w $SK_PATH/$SK_FILE -d /dev/urandom
		for i in $(seq 0 $((NUMCLIENTS-1))); do
			/usr/sbin/lgss_sk -f$FSNAME -n c$i -w $SK_PATH/nodemap/c$i.key -d /dev/urandom;
		done
		# Give non-clients all keys in path, work around client path bug
		do_nodes $all_nodes_comma "rsync -av $HOSTNAME:$SK_PATH/* $SK_PATH >/dev/null 2>&1"

		local all_nodes_nonclient=$(comma_list $(all_server_nodes))
		do_nodes $all_nodes_nonclient "cp -p $SK_PATH/nodemap/* $SK_PATH >/dev/null 2>&1"

		# mount options for servers and clients
		local SK_OPTS="skpath=$SK_PATH"
		MGS_MOUNT_OPTS=$(echo $MGS_MOUNT_OPTS | sed -e 's#-o skpath=[^ ]* ##g');
		MGS_MOUNT_OPTS=$(echo $MGS_MOUNT_OPTS | sed -e 's#,skpath=[^,^ ]*##g');
		MDS_MOUNT_OPTS=$(echo $MDS_MOUNT_OPTS | sed -e 's#-o skpath=[^ ]* ##g');
		MDS_MOUNT_OPTS=$(echo $MDS_MOUNT_OPTS | sed -e 's#,skpath=[^,^ ]*##g');
		OST_MOUNT_OPTS=$(echo $OST_MOUNT_OPTS | sed -e 's#-o skpath=[^ ]* ##g');
		OST_MOUNT_OPTS=$(echo $OST_MOUNT_OPTS | sed -e 's#,skpath=[^,^ ]*##g');
		MGS_MOUNT_OPTS="$MGS_MOUNT_OPTS -o $SK_OPTS"
		MDS_MOUNT_OPTS="$MDS_MOUNT_OPTS -o $SK_OPTS"
		OST_MOUNT_OPTS="$OST_MOUNT_OPTS -o $SK_OPTS"
		MOUNT_OPTS=$(echo $MOUNT_OPTS | sed -e 's#-o skpath=[^ ]* ##g');
		MOUNT_OPTS=$(echo $MOUNT_OPTS | sed -e 's#,skpath=[^,^ ]*##g');
		if [ $MOUNT_OPTS ]; then
...
cleanup_sk() {
	if $GSS_SK; then
		...
		echo "Cleaning up Shared Key on nodes: $list"
		do_nodes $list "rm -f $SK_PATH/*.key"
		SK_NO_KEY=true
	fi
}
...
quota_save_version() {
    ...
    do_facet mgs "lctl set_param -P ${fsname}-MDT*.mdd.quota_type=$spec"
    local varsvc
    local osts=$(get_facets OST)
    for ost in ${osts//,/ }; do
        varsvc=${ost}_svc
        do_facet mgs "lctl set_param -P ${!varsvc}.ost.quota_type=$spec"
    done
}
...
zconf_mount() {
	...
	if [ $GSS_SK ] && [ $SK_NODEMAP_MOUNT ]; then
		# Single client can mount with first nodemap key
		local mountkey=$SK_PATH/nodemap/c0.key
		local prunedopts=$(echo $opts | sed -e "s#skpath=[^,^ ]*#skpath=$mountkey#g")
		do_node $client $MOUNT_CMD $flags $prunedopts $device $mnt || return 1
	else
...
zconf_mount_clients() {
	...
	if [ -n "$FILESET" -a ! -n "$SKIP_FILESET" ]; then
		if [ $GSS_SK ] && [ $SK_NODEMAP_MOUNT ]; then
			# Each client needs mount with own nodemap key
			local i=0
			for nmclient in ${clients//,/ }; do
				local mountkey=$SK_PATH/nodemap/c$i.key
				do_node $nmclient "! grep -q $mnt' ' /proc/mounts || umount $mnt"
				local prunedopts=$(echo $opts | sed -e "s#skpath=[^ ^,]*#skpath=$mountkey#g");
				do_node $nmclient $MOUNT_CMD $flags $prunedopts $MGSNID:/$FSNAME $mnt || return 1
				i=$((i + 1))
			done
		else
			do_nodes $clients "! grep -q $mnt' ' /proc/mounts || umount $mnt"
...
check_and_setup_lustre() {
	...
	if [ "$SHARED_KEY" ]; then
        	echo "GSS_SK: Using GSS shared-key feature"
        	export GSS=true
        	export GSS_SK=true
        	export SEC=$SK_DEFAULT_FLAVOR
        	init_gss
        	set_flavor_all $SEC
	fi
...
get_clients_mount_count () {
	...
	do_nodes $clients cat /proc/mounts | grep lustre | grep -w $MOUNT | wc -l
}
...
count_contexts()
{
    ...
    total_ctx=$(echo "$output" | grep "expire" | grep "key" | grep "hdl" | wc -l)
    echo $total_ctx
}
...
flvr_cnt_cli2mdt()
{
	...
	if [ $GSS_SK ] && [ $flavor != "null" ]; then
		...
		output=$(do_node $c lctl get_param -n mdc.*-MDT*-mdc-*.$PROC_CON 2>/dev/null)
		outcon=$(count_contexts "$output")
...
flvr_cnt_cli2ost()
{
	...
	if [ $GSS_SK ] && [ $flavor != "null" ]; then
		# tmpcnt=min(contexts,flavors) to ensure SK context is on
		output=$(do_node $c lctl get_param -n osc.*OST*-osc-[^M][^D][^T]*.$PROC_CON 2>/dev/null)
		outcon=$(count_contexts "$output")
		if [ "$outcon" -lt "$tmpcnt" ]; then
			tmpcnt=$outcon
		fi
...
flvr_cnt_mdt2mdt()
{
	...
	if [ $GSS_SK ] && [ $flavor != "null" ]; then
		# tmpcnt=min(contexts,flavors) to ensure SK context is on
		output=$(do_facet mds$num lctl get_param -n mdc.*-MDT*-mdc[0-9]*.$PROC_CON 2>/dev/null)
		outcon=$(count_contexts "$output")
		if [ "$outcon" -lt "$tmpcnt" ]; then
			tmpcnt=$outcon
		fi
...
flvr_cnt_mdt2ost()
{
	...
	if [ $GSS_SK ] && [ $flavor != "null" ]; then
		# tmpcnt=min(contexts,flavors) to ensure SK context is on
		output=$(do_facet mds$num lctl get_param -n osc.$mdtosc.$PROC_CON 2>/dev/null)
		outcon=$(count_contexts "$output")
		if [ "$outcon" -lt "$tmpcnt" ]; then
			tmpcnt=$outcon
		fi
...
restore_to_default_flavor()
{
	...
	if [ $GSS_SK ]; then
		echo "Skipping server-to-server tests until supported by shared key feature"
		set_rule $FSNAME any cli2mdt $SK_DEFAULT_FLAVOR
		set_rule $FSNAME any cli2ost $SK_DEFAULT_FLAVOR
		set_rule $FSNAME any mdt2ost null
		set_rule $FSNAME any mdt2mdt null
		wait_flavor cli2mdt $SK_DEFAULT_FLAVOR
		wait_flavor cli2ost $SK_DEFAULT_FLAVOR
		# TODO: future testing when server-to-server is implemented
		# wait_flavor mdt2ost null
		# wait_flavor mdt2mdt null 0
		echo "GSS_SK now at default flavor: $SK_DEFAULT_FLAVOR"
	else
...
set_flavor_all()
{
	...
	if [ $GSS_SK ] && [ $flavor != "null" ]; then
		echo "GSS_SK: Skipping MDS test of $flavor until implemented"
