...
init_param_vars () {
	...
	if [ -n "$(lctl get_param -n mdc.*.connect_flags|grep jobstats)" ]; then
		local jobid_var=`$LCTL get_param -n jobid_var`

		if [ $JOBID_VAR = "existing" ]; then
			echo "keeping jobstats as $jobid_var"
		elif [ $jobid_var != $JOBID_VAR ]; then
			echo "seting jobstats to $JOBID_VAR"

			set_and_check $HOSTNAME "$LCTL get_param -n jobid_var" \
				"$FSNAME.sys.jobid_var" $JOBID_VAR
		fi
	else
		echo "disable jobstats, not supported by server"
	fi
