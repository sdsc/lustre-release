...
enable_ofd() {
	...
	do_nodes $nodes "
		...
		echo Updating \\\$OFD_MOD_CONF on \\\$HOSTNAME to enable OFD;
		if ! grep 'blacklist obdfilter' \\\$OFD_MOD_CONF \
		   &>/dev/null; then
			echo 'blacklist obdfilter' >>\\\$OFD_MOD_CONF;
		fi;
		if ! grep 'alias obdfilter ofd' \\\$OFD_MOD_CONF \
		   &>/dev/null; then
...
disable_ofd() {
	...
	do_nodes $nodes "
		...
		echo Updating \\\$OFD_MOD_CONF on \\\$HOSTNAME to disable OFD;
		sed -e '/blacklist obdfilter/d' \
		    -e '/alias obdfilter ofd/d' -i \\\$OFD_MOD_CONF
	"
}
...
init_param_vars () {
	...
	if [ $QUOTA_AUTO -ne 0 ] && ! $USE_OFD; then
