...
unload_modules() {
    ...
    $LUSTRE_RMMOD $(node_fstypes $HOSTNAME) || return 2
    ...
    if $LOAD_MODULES_REMOTE ; then
        ...
        if [ ! -z $list ]; then
            ...
            for node in ${list//,/ }; do
                do_node $node $LUSTRE_RMMOD $(node_fstypes $node)
            done
...
facet_type() {
    ...
    echo -n $facet | sed -e 's/^fs[0-9]\+//' | sed -e 's/[0-9]\+//' |
        tr '[:lower:]' '[:upper:]'
}
...
csa_add() {
    ...
    if echo "$opts" | grep -q "\([[:space:]]\+\|^\)$opt"; then
        opts=$(echo "$opts" | sed -e "s/\([[:space:]]\+\|^\)$opt[[:space:]]*[^[:space:]]\+/&,$arg/")
    else
        opts+="${opts:+ }$opt $arg"
    fi
    echo -n "$opts"
...
remove_mdt_files() {
    ...
    mount -t $(facet_fstype $SINGLEMDS) $MDS_MOUNT_OPTS $mdtdev $mntpt ||
        return $?
    rc=0;
    for f in $files; do
