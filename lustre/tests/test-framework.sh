...
init_test_env() {
	...
	export CLEAR_DEV_READONLY=true
...
dm_create_dev() {
	...
	if is_dm_dev $facet $dm_dev; then
		# if the existing device was set to "read-only", check whether
		# to clear it or not
		is_dm_flakey_dev $facet $dm_dev && $CLEAR_DEV_READONLY &&
...
stop() {
	...
	if [[ $(facet_fstype $facet) == zfs ]]; then
		# export ZFS storage pool
		export_zpool $facet
	elif dm_flakey_supported $facet && $CLEANUP_DM_DEV ||
	     [[ $facet = fs* ]]; then
...
shutdown_facet() {
	...
	if [[ "$FAILURE_MODE" = HARD ]]; then
		...
		dm_flakey_supported $facet && dm_cleanup_dev $facet
