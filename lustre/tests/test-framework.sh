...
export_dm_dev() {
	...
	local active_dev=${active_facet}_dev
	local dev=${!active_dev}

	if [[ $active_facet = $facet ]]; then
		local failover_dev=${facet}failover_dev
	...
	eval export ${active_dev}_saved=$dev
	eval export ${active_dev}=$dm_dev
}
...
unexport_dm_dev() {
	...
	local saved_dev=${facet}_dev_saved
	if [[ -n ${!saved_dev} ]]; then
		eval export ${facet}_dev=${!saved_dev}
	fi

	saved_dev=${facet}failover_dev_saved
	if [[ -n ${!saved_dev} ]]; then
		eval export ${facet}failover_dev=${!saved_dev}
	fi
}
...
stop() {
	...
	if [[ $(facet_fstype $facet) == zfs ]]; then
		# export ZFS storage pool
		export_zpool $facet
	elif dm_flakey_supported $facet && $CLEANUP_DM_DEV; then
