...
init_gss() {
	...
	if $GSS_SK && $SK_NO_KEY; then
		...
		do_nodes $(comma_list $(all_nodes)) "rsync -av $HOSTNAME:$SK_PATH/* \
					$SK_PATH >/dev/null 2>&1"

		do_nodes $(comma_list $(all_server_nodes)) "cp -p $SK_PATH/nodemap/* \
					$SK_PATH >/dev/null 2>&1"
...
cleanup_sk() {
	if $GSS_SK; then
		...
		stop_gss_daemons
		echo "Cleaning up Shared Key.."
		do_nodes $(comma_list $(all_nodes)) "rm -f $SK_PATH/$FSNAME.key \
...
flvr_cnt_mgc2mgs()
{
    ...
    local output=$(do_facet client lctl get_param -n mgc.*.$PROC_CLI 2>/dev/null)
