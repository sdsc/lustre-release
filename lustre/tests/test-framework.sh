...
init_gss() {
	...
	if ! module_loaded ptlrpc_gss; then
		...
		module_loaded ptlrpc_gss ||
			error_exit "init_gss: GSS=$GSS, but gss/krb5 unsupported"
	fi
	...
	if $GSS_SK && $SK_NO_KEY; then
		...
		do_nodes $all_nodes_comma "rsync -av \
...
cleanup_sk() {
	if $GSS_SK; then
		if $SK_S2S; then
			...
			echo "Nodemap removed for SK; sleeping for 10 sec"
			sleep 10
		fi
		stop_gss_daemons
		local nodes=$(comma_list $(all_nodes))
		echo "Cleaning up Shared Key on nodes: $list"
		do_nodes $nodes "rm -f $SK_PATH/$FSNAME.key \
			$SK_PATH/$FSNAME-*.key $SK_PATH/nodemap/*.key"
		SK_NO_KEY=true
	fi
}
