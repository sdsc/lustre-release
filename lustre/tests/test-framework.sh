...
run_lfsck() {
	do_facet $SINGLEMDS "$LCTL lfsck_start -M $FSNAME-MDT0000 -A -t all"
	...
	for k in $(seq $MDSCOUNT); do
		...
		wait_update_facet mds${k} "$LCTL get_param -n \
			...
			error "MDS${k} is not the expected 'completed'"
	done
	...
	local rep_ost=$(do_nodes $(comma_list $(osts_nodes)) \
			...
			awk '/repaired/ { print $2 }' | calc_sum)
	[ $repaired -eq 0 ] || error "lfsck repaired $rep_mdt+$rep_ost errors"
}
...
check_and_cleanup_lustre() {
	if [ "$LFSCK_ALWAYS" = "yes" -a "$TESTSUITE" != "sanity-lfsck" ]; then
		run_lfsck
	fi
