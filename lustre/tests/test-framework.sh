...
zconf_mount() {
	...
	if [ "x$FILESET" != "FILESET" ];then
		do_node $client $MOUNT_CMD $flags $opts $MGSNID:/$FSNAME $mnt \
			|| return 1
...
zconf_mount_clients() {
	...
	if [ "x$FILESET" != "FILESET" ];then
		do_nodes $clients umount $mnt
		do_nodes $clients $MOUNT_CMD $flags $opts $MGSNID:/$FSNAME $mnt \
			|| return 1
