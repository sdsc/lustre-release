...
add_sk_mntflag() {
	...
	if grep -q skpath <<< "$mt_opts" ; then
		mt_opts=$(echo $mt_opts | sed -e "s#skpath=[^ ,]*#skpath=$SK_PATH#")
	else
		if [ -z "$mt_opts" ]; then
...
init_gss() {
	...
	if $GSS_SK && $SK_NO_KEY; then
		...
		do_nodes $all_nodes_comma "rsync -av $HOSTNAME:$LGSSC_CONF_FILE \
		...
		if $SK_S2S; then
			/usr/sbin/lgss_sk -f$FSNAME -n $SK_S2SNMCLI -w $SK_PATH/$FSNAME-client.key \
				-d /dev/urandom
			/usr/sbin/lgss_sk -f$FSNAME -n $SK_S2SNM -w $SK_PATH/$FSNAME-server.key \
				-d /dev/urandom
		fi
		/usr/sbin/lgss_sk -f$FSNAME -w $SK_PATH/$FSNAME.key \
			-d /dev/urandom
		for i in $(seq 0 $((NUMCLIENTS - 1))); do
			/usr/sbin/lgss_sk -f$FSNAME -n c$i -w \
				$SK_PATH/nodemap/c$i.key -d /dev/urandom;
		done
		# Give non-clients all keys in path, work around client path bug
		do_nodes $all_nodes_comma "rsync -av $HOSTNAME:$SK_PATH/* \
					$SK_PATH >/dev/null 2>&1"

		do_nodes $all_nodes_nonclient "cp -p $SK_PATH/nodemap/* \
					$SK_PATH >/dev/null 2>&1"
...
cleanup_sk() {
	if $GSS_SK; then
		if $SK_S2S; then
			...
			echo "Nodemap removed for SK; sleeping for 10 seconds.."
			sleep 10
		fi
		stop_gss_daemons
		local list=$(comma_list $(all_nodes))
		echo "Cleaning up Shared Key on nodes: $list"
		do_nodes $list "rm -f $SK_PATH/$FSNAME.key $SK_PATH/$FSNAME-*.key $SK_PATH/nodemap/*.key"
		SK_NO_KEY=true
	fi
}
...
zconf_mount() {
	...
	if $GSS_SK && ($SK_UNIQUE_NM || $SK_S2S); then
		...
		local mountkey=$SK_PATH/$FSNAME-client.key
...
setupall() {
	...
	if [ -z "$CLIENTONLY" ]; then
		...
		if $GSS_SK; then
			...
			if $SK_S2S && ! $NM_STARTED; then
				NM_STARTED=true
				# Need to start a nodemap for servers, and one for clients.
				echo "Setting up nodemap config on MGS $(mgs_node)"
        			do_node $(mgs_node) "lctl nodemap_add $SK_S2SNM"
				for servernode in $(all_server_nodes); do
        			...
        			do_node $(mgs_node) "lctl nodemap_add $SK_S2SNMCLI"
				mountcli
				for servernode in $(all_clients_nodes); do
					local nids=$(do_nodes $servernode "$LCTL list_nids")
					for nid in $nids; do
						do_node $(mgs_node) "lctl nodemap_add_range --name \
							$SK_S2SNMCLI --range $nid"
					done
				done
...
# Get all of the client nodes. Borrowed from init_clients_lists()
all_clients_nodes () {
	...
	# for those configs which have SINGLECLIENT set to local client
...
count_contexts()
{
    ...
    total_ctx=$(echo "$output" | grep "expire" | grep "key" |
					grep "hdl" | wc -l)
    echo $total_ctx
}
...
flvr_cnt_cli2mdt()
{
	...
	output=$(do_node $c lctl get_param -n \
				mdc.*-MDT*-mdc-*.$PROC_CLI 2>/dev/null)
...
flvr_cnt_mdt2mdt()
{
	...
	output=$(do_facet mds$num lctl get_param -n \
				mdc.*-MDT*-mdc[0-9]*.$PROC_CLI 2>/dev/null)
	tmpcnt=$(count_flvr "$output" $flavor)
	if $GSS_SK && [ $flavor != "null" ]; then
		# tmpcnt=min(contexts,flavors) to ensure SK context is on
		output=$(do_facet mds$num lctl get_param -n \
				mdc.*-MDT*-mdc[0-9]*.$PROC_CON 2>/dev/null)
		outcon=$(count_contexts "$output")
		if [ "$outcon" -lt "$tmpcnt" ]; then
			tmpcnt=$outcon
		fi
