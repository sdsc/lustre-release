...
export SK_NODEMAP_MOUNT=${SK_NODEMAP_MOUNT:-false}
...
cleanup_sk() {
	if $GSS_SK; then
		...
		do_nodes $list "rm -f $SK_PATH/$FSNAME.key"
		SK_NO_KEY=true
	fi
}
...
restore_to_default_flavor()
{
	...
	if $GSS_SK; then
		echo "Skipping SK server-to-server tests until supported by shared key feature"
