...
wait_delete_completed_mds() {
	...
	for node in $(mdts_nodes); do
		changes=$(do_node $node "lctl get_param -n osp*.*.sync_*" \
			2>/dev/null | awk '{sum=sum+$1} END{print sum}')
	...
	while [ "$WAIT" -ne "$MAX_WAIT" ]; do
		changes=$(do_node $mds2sync "lctl get_param -n osp*.*.sync_*" \
...
wait_destroy_complete () {
	...
	while [ $WAIT -lt $MAX ]; do
		...
		echo "Waiting $WAIT secs for destroys from this client to be" \
		     "done."
		WAIT=$((WAIT + 1))
	done
