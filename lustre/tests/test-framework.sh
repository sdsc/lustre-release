...
start() {
	...
	init_facet_svc $facet
	local mntpt=$(facet_mntpt $facet)
	do_facet ${facet} mkdir -p $mntpt
	mount_facet ${facet}
	RC=$?
	[ $RC -eq 0 ] && init_facet_mntpt ${facet}
...
facet_failover() {
	...
	for facet in $(echo $facets | sed -e "s/,/ /g"); do
		...
		for ((index=0; index<$total; index++)); do
    			for affected_facet in ${affecteds[$index]//,/ }; do
				if [ "$affected_facet" = "$facet" ]; then
					skip=1
				fi
			done
		done
	...
	for ((index=0; index<$total; index++)); do
		...
		local host=$(facet_active_host $facet)
		echo "Failing ${affected[$index]} on $host"
...
facet_host() {
	...
	if [ -z "${!varname}" ]; then
		if [ "${facet:0:3}" == "ost" ]; then
			...
			"${facet:0:3}" == "mds" -o \
			"${facet:0:3}" == "mgs" ]; then
			eval ${facet}_HOST=${mds_HOST}
			eval ${facet}_failover_host=${mds_failover_HOST}
		fi
	fi
	echo -n ${!varname}
}
