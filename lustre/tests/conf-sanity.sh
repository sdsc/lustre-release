...
#   especially when most of them are mapped to something else anyway.
...
test_5a() {	# was test_5
	...
	manual_umount_client || error "manual_mount_client failed"
...
test_24a() {
	...
	mkdir -p $MOUNT2 || error "mkdir -p $MOUNT2 failed"
...
test_41a() { #bug 14134
	...
	mkdir $MOUNT || error "mkdir $MOUNT failed"
	mount_client $MOUNT || error "mount_client $MOUNT failed"
	sleep 5
...
test_50g() {
	...
	$SETSTRIPE -c -1 $DIR/$tfile || error "Unable to lfs setstripe"
...
test_51() {
	...
	$SETSTRIPE -c -1 $MOUNT/d1 || error "$SETSTRIPE -c -1 $MOUNT/d1 failed"
...
test_54b() {
	...
	setup
	run_llverfs $MOUNT -p || error "llverfs failed!"
	cleanup || error "cleanup failed with $?"
}
...
test_70a() {
	...
	mkdir $DIR/$tdir || error "create dir fail"
