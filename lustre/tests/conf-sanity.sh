...
t32_test() {
	...
	if [ $fstype == "ldiskfs" ]; then
		run_e2fsck $node $mdt_dev "-n" || {
			error_noexit "e2fsck on $mdt_dev failed"
			return 1
		}
	fi
	...
	if $r test -f $mdt2_dev; then
		if [ $fstype == "ldiskfs" ]; then
			run_e2fsck $node $mdt2_dev "-n" || {
				error_noexit "e2fsck on $mdt2_dev failed"
				return 1
			}
		fi
		...
		if [ $fstype == "ldiskfs" ]; then
			run_e2fsck $node $fs2mdsdev "-n" || {
				error_noexit "e2fsck on $fs2mdsdev failed"
				return 1
			}
		fi
	...
	if [ $fstype == "ldiskfs" ]; then
		run_e2fsck $node $ost_dev "-n" || {
			error_noexit "e2fsck on $ost_dev failed"
			return 1
		}
	fi
...
# LU-7212 Run LFSCK on the upgraded file system
		if [ $fstype == "ldiskfs" ]; then
			...
			wait_update_facet $SINGLEMDS "$LCTL get_param -n \
				...
				awk '/^status/ { print \\\$2 }'" \
				"completed" 60 || {
				error_noexit "LFSCK namespace not 'completed'"
				return 1
			}
			...
			wait_update_facet $SINGLEMDS "$LCTL get_param -n \
				...
				awk '/^status/ { print \\\$2 }'" \
				"completed" 60 || {
				error_noexit "LFSCK layout not 'completed'"
				return 1
			}
