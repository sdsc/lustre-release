...
check_max_mod_rpcs_in_flight() {
	...
	if [ -z "$tmp" ]; then
		echo "Client does not support multiple modify RPCs in flight"
		return
	fi
	...
	do_facet $facet "$LCTL set_param fail_loc=0x158"
...
run_test 86a "check max_mod_rpcs_in_flight is enforced"
...
run_test 86b "check max_mod_rpcs_in_flight is enforced after update"
...
run_test 86c "check max_mod_rpcs_in_flight update limits"
...
run_test 86d "check one close RPC is allowed above max_mod_rpcs_in_flight"
