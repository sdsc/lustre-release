...
t32_test() {
	...
	if [ "$writeconf" ]; then
		...
		local saved_T32_BLIMIT=$T32_BLIMIT
		local saved_T32_ILIMIT=$T32_ILIMIT
		if [[ $(version_code $img_commit) -lt $(version_code 2.7.0) ]]
		then
		...
		t32_verify_quota $node $fsname $tmp/mnt/lustre || {
			...
			T32_BLIMIT=$saved_T32_BLIMIT
			T32_ILIMIT=$saved_T32_ILIMIT
			return 1
		}
		T32_BLIMIT=$saved_T32_BLIMIT
		T32_ILIMIT=$saved_T32_ILIMIT
