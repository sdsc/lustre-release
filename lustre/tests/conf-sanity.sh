...
test_0() {
	setup
	check_mount || error "check_mount failed with $?"
...
test_5a() {	# was test_5
	...
	stop_mds -f || error "stop_mds failed with $?"
	...
	if grep " $MOUNT " /proc/mounts; then
		...
		grep " $MOUNT " /proc/mounts &&
			echo "test 5: /proc/mounts after second umount" &&
			return 11
	fi
...
test_5c() {
	grep " $MOUNT " /etc/mtab &&
		error false "unexpected entry in mtab before mount" &&
		return 10
...
test_5d() {
	...
	start_mds
	stop_ost -f
	mount_client $MOUNT || "mount_client failed"
	cleanup || error "cleanup failed with $?"
	grep " $MOUNT " /etc/mtab && \
...
test_6() {
	...
	touch $DIR/$tfile || error "touch $tfile filed"
	cleanup || error "cleanup failed with $?"
}
...
test_8() {
	setup
	mount_client $MOUNT2
	check_mount2 || error "check_mount2 failed (45)"
...
test_9() {
	...
	if [ "$CHECK_SUBSYS" ] && [ "$CHECK_SUBSYS" = "mds ost" ]; then
		...
		error "lnet.subsystem_debug: want 'mds ost', " \
		      "got '$CHECK_SUBSYS' (1)"
	fi
	stop_ost || error "stop_ost failed with $?"
}
...
test_17() {
	...
	setup
	check_mount || error "check_mount failed (41)"
	cleanup || error "cleanup failed with $?"
	...
	start_ost
	start_mds && error "start_mds should fail (42)"
	reformat_and_config
}
...
test_19a() {
	start_mds || error "start_mds failed (1)"
	stop_mds -f || error "stop_mds failed (2)"
}
...
test_19b() {
	...
	start_ost || error "start_ost failed (1)"
	stop_ost -f || error "stop_ost failed (2)"
}
...
test_20() {
	...
	start_ost || error "start_ost failed"
	mount_client $MOUNT
	check_mount || error "check_mount failed (43)"
	rm -f $DIR/$tfile
	remount_client ro $MOUNT || error "remount_client failed (44)"
	touch $DIR/$tfile && error "$DIR/$tfile created incorrectly (45)"
	[ -e $DIR/$tfile ] && error "$DIR/$tfile exists incorrectly (46)"
	remount_client rw $MOUNT || error "remount_client failed (47)"
	touch $DIR/$tfile
	[ ! -f $DIR/$tfile ] && error "$DIR/$tfile missing (48)"
	MCNT=$(grep -c $MOUNT /etc/mtab)
	[ "$MCNT" -ne 1 ] && error "$MOUNT in /etc/mtab $MCNT times (49)"
	umount_client $MOUNT
...
test_21c() {
	start_ost || error "start_ost failed"
	start_mds || error "start_mds failed"
	start_ost2 || error "start_ost2 failed"
	wait_osc_import_state mds ost2 FULL
	stop_ost || error "stop_ost failed"
	stop_ost2 || error "stop_oste failed"
	stop_mds || error "stop_mds failed"
...
test_21d() {
	...
	start_mgs || error "start_mgs failed"
	start_ost || error "start_ost failed"
	start_ost2 || error "start_ost2 failed"
	start_mds || error "start_mds failed"
	wait_osc_import_state mds ost2 FULL
	...
	stop_ost || error "stop_ost failed"
	stop_ost2 || error "stop_ost2 failed"
	stop_mds || error "stop_mds failed"
	stop_mgs || error "stop_mgs failed"
	#writeconf to remove all ost2 traces for subsequent tests
	writeconf_or_reformat
	start_mgs || error "second start_mgs failed"
}
...
test_22() {
	...
	mcreate $DIR/$tfile || error "mcreate failed (40)"
	rm -f $DIR/$tfile || error "rm $tfile failed (42)"
	umount_client $MOUNT
	pass
...
test_23a() {	# was test_23
	...
	if [ "$WAIT" -eq "$MAX_WAIT" ]; then
		error "MOUNT_PID $MOUNT_PID and MOUNT_LUSTRE_PID "\
			"$MOUNT_LUSTRE_PID still not killed in $WAIT secs"
		ps -ef | grep mount
	fi
...
test_24a() {
	...
	mount -t lustre $MGSNID:/${FSNAME2} $MOUNT2 || error "mount failed (1)"
	# 1 still works
	check_mount || error "check_mount failed (2)"
	...
	# 2 should work
	sleep 5
	cp /etc/passwd $MOUNT2/b || error "cp failed (3)"
	rm $MOUNT2/b || error "rm failed (4)"
	# 2 is actually mounted
	grep $MOUNT2' ' /proc/mounts > /dev/null || error "grep failed (5)"
	# failover
	...
	cleanup_fs2
	cleanup_nocli || error "cleanup_nocli failed (6)"
}
...
test_24b() {
	...
	cleanup || error "cleanup failed (6)"
}
...
#define OBD_FAIL_MDS_FS_SETUP            0x135
	...
	start_mds || error "start_mds failed (3)"
	stop_mds || error "stop_mds failed (4)"
	unload_modules_conf || error "umload_modules_conf failed with ?"
}
...
test_27a() {
	...
	start_ost || error "start_ost failed (1)"
	start_mds || error "start_mds failed (2)"
	...
	set_conf_param_and_check ost1					      \
	   ...
	   "$FSNAME-OST0000.ost.client_cache_seconds" ||
		error "set_conf_param_and_check ost1 failed (3)"
	cleanup_nocli
}
...
test_28() {
	...
	set_conf_param_and_check client "$TEST" "$PARAM" $FINAL ||
		error "set_conf_param_and_check client failed $FINAL (3)"
	FINAL=$(($FINAL + 1))
	set_conf_param_and_check client "$TEST" "$PARAM" $FINAL ||
		error "set_conf_param_and_check client failed $FINAL (4)"
	umount_client $MOUNT || error "umount_client failed (200)"
	mount_client $MOUNT
	RESULT=$($TEST)
	...
	set_conf_param_and_check client "$TEST" "$PARAM" $ORIG ||
		error "set_conf_param_and_check client failed $ORIG (5)"
	cleanup
}
...
test_29() {
	...
	if [ -z "$RESULT" ]; then
		error "New client not deactivated "\
		      "from start: $(lctl get_param -n $PROC_UUID) (5)"
	else
	...
	set_conf_param_and_check client \
		...
		error "set_conf_param_and_check client failed(6)"
...
test_30a() {
	...
	for i in ${LIST[@]}; do
		set_conf_param_and_check client "$TEST" \
			...
			error "set_conf_param_and_check client failed (3)"
	done
	# make sure client restart still works
	umount_client $MOUNT
	mount_client $MOUNT || error "mount_client failed (4)"
	[ "$($TEST)" -ne "$i" ] &&
	...
	umount_client $MOUNT
	mount_client $MOUNT || error "mount_client failed (6)"
	FINAL=$($TEST)
...
test_30b() {
	...
	# numerical part of it. Hopefully that's not already a failover
	# address for the server.
	OSTNID=$(do_facet ost1 "$LCTL get_param nis" |
		tail -1 | awk '{print $1}')
	...
	umount_client $MOUNT
	mount_client $MOUNT || error "mount_client failed (3)"
...
test_32newtarball() {
	...
	mkdir $tmp/src || error "mkdir $tmp/src failed"
...
test_33a() { # bug 12333, was test_33
	...
	stop fs2ost -f
	stop fs2mds -f
	cleanup_nocli || error "cleanup_nocli failed"
	return $rc
}
...
test_34b() {
	setup
	touch $DIR/$tfile || error "touch $tfile failed (1)"
	stop_mds --force || error "stop_mds failed (2)"
...
test_34c() {
	setup
	touch $DIR/$tfile || error "touch $tfile failed (1)"
	stop_ost --force || error "stop_ost failed (2)"
...
test_35a() { # bug 12459
	...
	stop_mdt 1 || error "stop_mds failed (5)"

	df $MOUNT > /dev/null 2>&1 &
	DFPID=$!
	log "Restarting the MDT: $device"
	start_mdt 1 || error "start_mdt failed (6)"
...
test_35b() { # bug 18674
	...
	do_facet $SINGLEMDS "$LCTL set_param fail_loc=0x80000136" ||
		error "set_param fail_loc=0x80000136 on $SINGLEMDS failed (2)"
	...
	do_facet $SINGLEMDS "$LCTL set_param fail_loc=0" ||
		error "set_param fail_loc=0 on $SINGLEMDS failed (3)"
	...
	[ "$FAILCONN" == "0" ] &&
		error "The client reconnection has not been triggered (4)"
	[ "$FAILCONN" == "2" ] &&
		error "Client tried to reconnect to the failover server while the primary was busy"
...
test_36() { # 12743
	...
	umount -d $MOUNT2
	stop fs3ost -f || error "stop fs3ost failed (200)"
	stop fs2ost -f || error "stop fs2ost failed (201)"
	stop fs2mds -f || error "stop fs2mds failed (202)"
	unload_modules_conf || error "unload_modules_conf failed (203)"
	return $rc
}
...
test_41a() { #bug 14134
	...
	mount_client $MOUNT || error "mount_client $MOUNT failed (1)"
	sleep 5
	...
	umount_client $MOUNT
	stop ost1 -f || error "stop_ost1 failed (201)"
	stop_mds -f || error "stop_mds failed (202)"
	stop_mds -f || error "second stop_mds failed (203)"
	unload_modules_conf || error "unload_modules_conf failed (204)"
	return $rc
}
...
test_41b() {
	...
	echo "blah blah" > $MOUNT/$tfile
	cat $MOUNT/$tfile || error "cat $tfile failed (200)"

	umount_client $MOUNT
	stop_ost || error "stop_ost failed (201)"
	stop_mds -f || error "stop_mds failed (202)"
	stop_mds -f || error "second stop_mds failed (203)"
...
test_46a() {
	...
	start_ost || error "start_mds failed (2)"
	wait_osc_import_state mds ost FULL
	#start_client
	mount_client $MOUNT || error "mount_client $MOUNT failed (3)"
	...
	mount_client $MOUNT2 || error "mount_client $MOUNT2 failed (8)"
	$LFS setstripe -c -1 $MOUNT2 || error "$LFS setstripe failed (9)"
	$LFS getstripe $MOUNT2 || error "$LFS getstripe $MOUNT2 failed (10)"

	echo "ok" > $MOUNT2/$tfile
	$LFS getstripe $MOUNT2/$tfile ||
		error "$LFS getstripe $MOUNT2 failed (10)"
	# fill acl buffer for avoid expand lsm to them
	awk -F : '{if (FNR < 25) { print "u:"$1":rwx" }}' /etc/passwd |
		while read acl; do
		setfacl -m $acl $MOUNT2/$tfile
	done

	# will be deadlock
	stat $MOUNT/$tfile || error "stat $tfile failed (12)"

	cleanup_46a $OSTCOUNT || error "cleanup_46a failed! (13)"
	return 0
}
...
test_48() { # bug 17636
	...
	$LFS setstripe -c -1 $MOUNT || error "$LFS setstripe failed (9)"
	$LFS getstripe $MOUNT || error "$LFS getstripe $MOUNT failed (10)"

	echo "ok" > $MOUNT/$tfile
	$LFS getstripe $MOUNT/$tfile ||
		error "$LFS getstripe $MOUNT/$tfile failed (11)"

	trap cleanup_48 EXIT ERR

	# fill acl buffer for avoid expand lsm to them
	getent passwd | awk -F : '{ print "u:"$1":rwx" }' |  while read acl; do
		setfacl -m $acl $MOUNT/$tfile
	done
...
test_49() { # bug 17710
	...
	reformat
	setup_noconfig
	check_mount || error "check_mount failed (1)"
	...
	umount_client $MOUNT
	stop_ost || error "stop_ost failed (2)"
	stop_mds || error "stop_mds failed (3)"

	LDLM_TIMEOUT=$((LOCAL_TIMEOUT - 1))

	reformat
	setup_noconfig
	check_mount || error "check_mount failed (7)"
...
test_51() {
	...
	mkdir $MOUNT/d1 || error "mkdir $MOUNT/d1 failed"
	...
	local pid=$!
	sleep 2
	start_ost2 || error "start_ost2 failed (2)"
	wait $pid
	stop_ost2 || error "stop_ost2 failed (3)"
	cleanup
	#writeconf to remove all ost2 traces for subsequent tests
	writeconf_or_reformat
}
...
test_52() {
	...
	mkdir $DIR/$tdir || error "Unable to create tdir"
	touch $TMP/modified_first ||
		error "Unable to create temporary file"
	local mtime=$(stat -c %Y $TMP/modified_first)
	do_node $ost1node "mkdir $ost1tmp && touch -m -d @$mtime $ost1tmp/modified_first"

	[ $? -eq 0 ] || error "Unable to create temporary file on $ost1node"
	sleep 1
	...
	[ $? -eq 0 ] || error "Unable to copy files"
	...
	[ $? -eq 0 ] || error "Unable to create $ost1mnt"
	...
	[ $? -eq 0 ] || error "Unable to umount ost1 as ldiskfs"

	start_ost || error "Unable to start OST1 after umount"
	mount_client $MOUNT || error "Unable to mount client at $MOUNT"
	...
	do_node $ost1node "rm -rf $ost1tmp" ||
		error "Unable to delete temporary files on OST1"
	cleanup
}
...
test_68() {
	...
	touch $DIR/$tfile
	do_facet $SINGLEMDS lctl get_param seq.srv*MDT0000.space
...
test_70c() {
	...
	mkdir $DIR/$tdir && error "mkdir $tdir succeed"
