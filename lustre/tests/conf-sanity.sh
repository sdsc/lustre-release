...
test_32newtarball() {
	...
	if [ $FSNAME != t32fs -o \( -z "$MDSDEV" -a -z "$MDSDEV1" \) -o			\
		...
		error "Needs FSNAME=t32fs MDSCOUNT=2 MDSDEV1=<nonexistent_file>"	\
		      "MDSDEV2=<nonexistent_file> (or MDSDEV, in the case of b1_8)"	\
...
t32_test() {
	...
	if [ "$writeconf" ]; then
		...
		# migrate the files/directories to the remote MDT, then move it back
