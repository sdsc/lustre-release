...
recovery_time_min() {
	...
	#obd_timeout/20)
        TO_20=$(($TIMEOUT/20))
        (($CONNECTION_SWITCH_MIN>$TO_20)) && INITIAL_CONNECT_TIMEOUT= \
		$CONNECTION_SWITCH_MIN || INITIAL_CONNECT_TIMEOUT=$TO_20
...
test_79() {
        ...
        echo "start mds service on `facet_active_host $facet`"
        start $facet ${dev} $MDS_MOUNT_OPTS "-o recovery_time_hard=$time_min, \
		recovery_time_soft=$time_min" $@ || return 94
        ...
        createmany -o $DIR1/$tfile-%d 10000&
