...
test_29() {
	...
	if [ -z "$RESULT" ]; then
		error "Client not deactivated: $(lctl get_param -n $PROC_UUID)"
	else
	...
	if [ -z "$RESULT" ]; then
		error "Client start activated: $(lctl get_param -n $PROC_UUID)"
	else
...
test_35b() { # bug 18674
	...
	do_facet mgs "$LCTL conf_param \
		${device}.failover.node=$(h2$NETTYPE $FAKENID)" ||
		error "Set ${device}.failover.node=$(h2$NETTYPE $FAKENID) failed"
