...
test_69() {
	...
	if [ $num_create -gt 0 ]; then
		...
		if [ $ifree -lt 10000 ]; then files=$(( ifree - 50 ));
			else files=10000
		...
		for i in $(seq 1 $j); do
			...
			unlinkmany $DIR/$tdir/$tfile-$i-%d $files ||
	...
	[ $ifree -ge $((ost_max_pre/2 + 1000)) ] &&
		error "OST replacement created too many inodes; $ifree"
	cleanup || error "cleanup failed with $?"
}
