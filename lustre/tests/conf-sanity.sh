...
test_50i() {
	...
	load_modules
	do_facet mdt2 "$TUNEFS --param mdc.active=0 `mdsdevname 2`" ||
	...
	test_mkdir -i 1 $DIR/$tdir/2 || error "mkdir $DIR/$tdir/2 failed"
	# create some file
	createmany -o $DIR/$tdir/2/$tfile-%d 1

	# check MDC import is working
	stat $DIR/$tdir/2/* >/dev/null 2>&1 ||
		error "some OSC imports are still not connected"
