...
# excepted. This test uses OST_INDEX_LIST to format OSTs with a randomly
# assigned index and ensures we can mount such a formatted file system
test_79() { # LU-4665
	[[ $(lustre_version_code $SINGLEMDS) -ge $(version_code 2.6.53) ]] ||
...
run_test 80a "specify OSTs for file (succeed) or directory (fail)"
...
# Test 80b is run to ensure that is the user supplies a pool with a specific
...
run_test 80b "specify OSTs for file with --pool and --ost-list options"
