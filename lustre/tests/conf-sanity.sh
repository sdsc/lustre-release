...
test_36() { # 12743
	...
	add fs2mds $(mkfs_opts mds1) --fsname=${FSNAME2} --reformat \
		$fs2mdsdev $fs2mdsvdev || exit 10
	# XXX after we support non 4K disk blocksize in ldiskfs, specify a different
	# one than the default value here.
	add fs2ost $(mkfs_opts ost1) --mgsnode=$MGSNID --fsname=${FSNAME2} \
...
test_55() {
	...
	do
		add mds1 $(mkfs_opts mds1) --reformat $mdsdev $mdsvdev ||
			exit 10
		add ost1 $(mkfs_opts ost1) --index=$i --reformat $(ostdevname 1) \
			$(ostvdevname 1)
		setup_noconfig
		stopall
		setup_noconfig
