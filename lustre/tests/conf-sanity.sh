...
test_56b() {
	...
	do_facet mds1 "$LCTL set_param mdt.*.enable_remote_dir=1"
	do_facet mds2 "$LCTL set_param mdt.*.enable_remote_dir=1"
	do_facet mds3 "$LCTL set_param mdt.*.enable_remote_dir=1"
	do_facet mds1 "$LCTL set_param mdt.*.enable_remote_dir_gid=-1"
	do_facet mds2 "$LCTL set_param mdt.*.enable_remote_dir_gid=-1"
	do_facet mds3 "$LCTL set_param mdt.*.enable_remote_dir_gid=-1"

	mount_client $MOUNT || error "Unable to mount client"

	echo "This is test file 1!" > $MOUNT/testfile.1 \
		|| error "failed to make test file 1"
	echo "This is test file 2!" > $MOUNT/testfile.2 \
		|| error "failed to make test file 2"
	echo "This is test file 3!" > $MOUNT/testfile.3 \
		|| error "failed to make test file 3"
	rm $MOUNT/testfile.* || error "failed to remove test files"

	$LFS mkdir -c3 $MOUNT/testdir || error "failed to make testdir"
	rm -rf $MOUNT/testdir || error "failed to remove testdir"

	$LFS mkdir -i1000 $MOUNT/remote_dir || \
		error "create remote dir at idx 1000 failed"
	rm -rf $MOUNT/remote_dir || error "failed to remove remote_dir"
