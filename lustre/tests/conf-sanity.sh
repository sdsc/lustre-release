...
wait_osts_up() {
	cmd="$LCTL get_param -n lov.$FSNAME-clilov-*.target_obd | \
	...
	wait_update_facet $SINGLEMDS "eval $cmd" $OSTCOUNT || {
		error "wait_update OSTs up on MDT failed"; exit 0; }
}
...
# Test 82b is run to ensure that is the user supplies a pool with a specific
