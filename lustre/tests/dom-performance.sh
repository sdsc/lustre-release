...
lfs setstripe -P mdt -S ${DOM_SIZE} $DOM
...
run_MDtest() {
	...
	for bsize in 4096 ; do
		run_cmd "mpirun -np $NUM $mdtest \
			-d $TDIR/ -I $FNUM -i 3 -F -z 1 -b 1 -L -u -w $bsize -r $bsize"
	done
	rm -rf $TDIR/*
	return 0
}
...
run_smalliomany() {
	...
	if [ ! -f smalliomany ] ; then
		echo "No smalliomany installed, skipping"
		return 0
	fi
	...
	if [ -f statmany ]; then
		run_cmd "./statmany -s $TDIR/file- $count $((count * 5)) | grep 'total'"
	fi

	for opc in w a r ; do
		run_cmd "./smalliomany -${opc} $TDIR/file- $count 300 | grep 'total'"
	done
...
run_IOR() {
	if ! which IOR > /dev/null 2>&1 ; then
		echo "No IOR installed, skipping"
		return 0
	fi
...
run_dbench() {
	if ! which dbench > /dev/null 2>&1 ; then
		echo "No dbench installed, skipping"
		return 0
	fi
...
run_smallfile() {
	...
	### since smallfile is not install system wide, get it right now
	[ -f master.zip ] || wget https://github.com/bengland2/smallfile/archive/master.zip
	unzip -uo master.zip
	cd ./smallfile-master
