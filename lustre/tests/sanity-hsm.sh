...
copytool_setup() {
	...
	[[ -z "$HSMTOOL_EVENT_FIFO" ]] || {
		...
		ct_test_dir=$(dirname $HSMTOOL_EVENT_FIFO)
		mkdir -p $ct_test_dir
		# Create the fifo and a reader (cat dies when copytool dies)
		mkfifo -m 0644 $HSMTOOL_EVENT_FIFO
		(cat $HSMTOOL_EVENT_FIFO > $ct_test_dir/events) &
	}
...
get_copytool_event_log() {
	...
	do_facet $facet "cat $logfile" ||
		error "could not collect $logfile from $facet"
}
...
test_70() {
	...
	done < get_copytool_event_log $event_log
