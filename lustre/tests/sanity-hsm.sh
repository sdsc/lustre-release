...
test_252() {
    ...
    # Shuffle the policies to have a chance to detect side effects between them
...
test_253() {
    ...
    [ $? -ne 0 ] || \
        error "Successfully set '$priority_ratio' to '$too_low_ratio' although \
no value under '$low_ratio' should be accepted"
    printf "OK\n"

    printf "too much ('$too_high_ratio'): "
    set_hsm_param "$priority_ratio" "$too_high_ratio" 2>/dev/null
    [ $? -ne 0 ] || \
        error "Successfully set '$priority_ratio' to '$too_high_ratio'
although no value over '$high_ratio' should be accepted"
    printf "OK\n"

    printf "not a number ('$bogus_ratio'): "
    set_hsm_param "$priority_ratio" "$bogus_ratio" 2>/dev/null
    [ $? -ne 0 ] || \
        error "Successfully set '$priority_ratio' to '$bogus_value' although \
...
but it is not: '$mask'")
    ...
    [ $? -ne 0 ] || \
        error "Successfully set 'bogus_value' in '$priority_mask' which is \
not a correct value"
    printf "OK\n"
}
...
test_255() {
    ...
    # Set max_requests so that we can easily check for the expected behaviour
    set_hsm_param max_requests $max_requests

    # Increase the loop period
    set_hsm_param loop_period 1

    # Set up the priority_policy
	...
	# At the time this was written, it meant "8 RESTOREs for 2 ARCHIVEs"
	local restore_num=$(grep -o "RESTORE" <<< "${results[@]:0:$max_requests}" |\
		wc -l)
	local restore_num_expect=$((priority_ratio * max_requests / 100))

	[ "$restore_num" -eq $restore_num_expect ] || \
		error "Expected to find '$restore_num_expect' RESTORE HSM requests in
the first batch but instread there was/were '$restore_num'"
	...
	[ $restore_num -eq $restore_num_expect ] || \
		error "Expected to find '$restore_num_expect' RESTORE HSM requests in
the second batch but instread there was/were '$restore_num'"
