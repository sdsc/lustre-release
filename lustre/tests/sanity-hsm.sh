...
mdts_conf_param() {
	...
	for mdtno in $(seq 1 $MDSCOUNT); do
		...
		# $MDT_PREFIX
		if [[ -n $value ]]; then
			do_facet mds${mdtno} $LCTL conf_param $arg \
				$FSNAME-MDT000${idx}.mdt.$key=$value
		else
			do_facet mds${mdtno} $LCTL conf_param $arg \
				$FSNAME-MDT000${idx}.mdt.$key
		fi
	done
}
...
mdts_set_param() {
	...
	for mdtno in $(seq 1 $MDSCOUNT); do
		local idx=$(($mdtno - 1))
		do_facet mds${mdtno} $LCTL set_param $arg \
			mdt.${MDT[$idx]}.$key="$value"
		[[ $? != 0 ]] && rc=1
	done
	return $rc
}
...
set_hsm_param() {
	local param=$1
	local value=$2
	mdts_set_param -n "hsm.$param" "$value"
	return $?
}
...
cdt_clear_mount_state() {
	mdts_conf_param "-d" hsm_control ""
}

cdt_set_mount_state() {
	mdts_conf_param "" hsm_control "$1"
}
...
wait_request_state() {
	...
	local cmd="$LCTL get_param -n ${MDT_PREFIX}${mdtidx}.hsm.agent_actions"
...
run_test 401 "Compound requests splitted and sent to their respective MDTs"
