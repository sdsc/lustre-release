...
export HT=${HT:-"$LUSTRE/utils/lhsmtool_posix"}
...
copytool_setup() {
	...
	if [[ ! -z "$1" ]]; then
		CMD="$HT $HT_VERB --hsm_root $ARC --bandwidth 1 $FSNAME "\
		    "--archive $1"
	else
		CMD="$HT $HT_VERB --hsm_root $ARC --bandwidth 1 $FSNAME"
	fi
...
copytool_remove_backend() {
	local fid=$1
	be=`find $ARC -name $fid`
	echo "Remove from backend: $fid = $be"
	rm -f $be
}

import_file() {
	$HT --archive $AN --hsm_root $ARC --import $1 $2
	return $?
}
...
get_hsm_param() {
	param=$1
	val=$(do_facet $SINGLEMDS $LCTL get_param -n $HSM_PARAM.$param)
	echo $val
}
...
set_test_state() {
	...
	wait_result $mds_HOST "$LCTL get_param -n $MDT_PARAM.hsm_control" \
		$target 10 || fail "cdt state is not $target"
}
...
cdt_check_state() {
	local target=$1
	wait_result $mds_HOST \
		"$LCTL get_param -n $MDT_PARAM.hsm_control" "$target" 20 ||
			fail "cdt state is not $target"
}
...
mds_restart() {
	...
	echo "shutdown of $mds_facet: " `facet_active_host $mds_facet`
	shutdown_facet $mds_facet
	echo "waiting 5s..."
	sleep 5
	echo "restarting $mds_facet"
	mount_facet $mds_facet || fail "Restart of $mds_facet failed"
	echo "waiting end of recovery..."
	wait_result $mds_HOST \
		"$LCTL get_param -n $MDT_PARAM.recovery_status | grep status:" \
		"status: COMPLETE" 20 || fail "mds failed to restart"
}
...
get_hsm_flags() {
	...
	[[ $? == 0 ]] || fail "$LFS hsm_state $f failed (run as $u)"
...
check_hsm_flags() {
	...
	[[ $st == $fl ]] || fail "hsm flags on $f are $st != $fl"
}
...
wait_result() {
	...
	do
		...
		then
			echo "Change after ${WAIT}s from $PREV_RESULT to $RESULT"
			PREV_RESULT=$RESULT
		fi
		[ $WAIT -ge $MAX ] && break
		[ $((WAIT % print)) -eq 0 ] &&
...
wait_request_state()
{
	...
	local request=$2
	local state=$3
	wait_result $mds_HOST "$LCTL get_param -n $HSM_PARAM.agent_actions | grep $fid | \
				grep action=$request | \
				cut -f 13 -d ' ' | cut -f 2 -d =" $state 100 ||
	 fail "request on $fid is not $state"
}
...
get_request_state()
{
	local fid=$1
	local request=$2
	do_facet $SINGLEMDS "$LCTL get_param -n $HSM_PARAM.agent_actions | grep $fid | \
				grep action=$request | cut -f 13 -d ' ' | cut -f 2 -d ="
}
...
get_request_count()
{
	...
	local request=$2
	do_facet $SINGLEMDS "$LCTL get_param -n $HSM_PARAM.agent_actions | grep $fid | \
				grep action=$request | wc -l"
}

wait_all_done()
{
	local timeout=$1
	wait_result $mds_HOST "$LCTL get_param -n $HSM_PARAM.agent_actions | \
		egrep 'WAITING|STARTED' " "" $timeout ||
	fail "requests did not complete"
}
...
my_uuid() {
	$LCTL get_param -n llite.lustre-*.uuid
}
...
test_2() {
	...
	$LFS hsm_set --exists $f || fail "user could not change hsm flags"
	check_hsm_flags $f "0x00000001"

	# chmod do not put the file dirty
	chmod 600 $f || fail "could not chmod test file"
	check_hsm_flags $f "0x00000001"

	# chown do not put the file dirty
	chown $RUNAS_ID $f || fail "could not chown test file"
	check_hsm_flags $f "0x00000001"

	# truncate put the file dirty
	$TRUNCATE $f 1 || fail "could not truncate test file"
	check_hsm_flags $f "0x00000003"

	$LFS hsm_clear --dirty $f || fail "could not clear hsm flags"
	check_hsm_flags $f "0x00000001"
}
...
test_3() {
	...
	cat $f > /dev/null || fail "could not read file"
	check_hsm_flags $f "0x00000001"

	# Open for write without modifying data, does not set dirty
	openfile -f O_WRONLY $f || fail "could not open test file"
	check_hsm_flags $f "0x00000001"

	# Append to a file sets it dirty
	cp -p /etc/passwd $f.append || fail "could not create file"
	$LFS hsm_set --exists $f.append ||
		fail "user could not change hsm flags"
	dd if=/etc/passwd of=$f.append bs=1 count=3 \
	   conv=notrunc oflag=append status=noxfer ||
		fail "could not append to test file"
	check_hsm_flags $f "0x00000003"

	# Modify a file sets it dirty
	cp -p /etc/passwd $f.modify || fail "could not create file"
	$LFS hsm_set --exists $f.modify ||
		fail "user could not change hsm flags"
	dd if=/dev/zero of=$f.modify bs=1 count=3 \
	   conv=notrunc status=noxfer ||
		fail "could not modify test file"
	check_hsm_flags $f "0x00000003"

	# Open O_TRUNC sets dirty
	cp -p /etc/passwd $f.trunc || fail "could not create file"
	$LFS hsm_set --exists $f.trunc ||
		fail "user could not change hsm flags"
	cp /etc/group $f.trunc || fail "could not override a file"
	check_hsm_flags $f "0x00000003"

	# Mmapped a file sets dirty
	cp -p /etc/passwd $f.mmap || fail "could not create file"
	$LFS hsm_set --exists $f.mmap ||
		fail "user could not change hsm flags"
	multiop $f.mmap OSMWUc || fail "could not mmap a file"
	check_hsm_flags $f "0x00000003"
}
...
test_4() {
	...
	[[ -z "$st" ]] || fail "hsm_cancel must not be registered (state=$st)"
}
...
test_10a() {
	...
	mkdir -p $DIR/$tdir/d1
	local f=$DIR/$tdir/hosts
	local fid=$(copy_file /etc/hosts $f)
	$LFS hsm_archive -a $AN $f ||
		fail "hsm_archive failed"
	wait_request_state $fid ARCHIVE SUCCEED

	local AFILE=$(ls $ARC/*/*/*/*/*/*/$fid) || \
		fail "fid $fid not in archive $ARC"
	echo "Verifying content"
	diff $f $AFILE || fail "archived file differs"
	echo -n "Verifying hsm state "
	check_hsm_flags $f "0x0000000b"

	echo "Verifying archive number is $AN"
	local st=$(get_hsm_archive_id $f)
	[[ $st == $AN ]] || fail "Wrong archive number, $st != $AN"
...
test_10b() {
	...
	local fid=$(copy_file /etc/hosts $f)
	$LFS hsm_archive $f || fail "archive request failed"
	wait_request_state $fid ARCHIVE SUCCEED

	$LFS hsm_archive $f || fail "archive of non dirty file failed"
	local cnt=$(get_request_count $fid ARCHIVE)
	[[ "$cnt" == "1" ]] ||
		fail "archive of non dirty file must not make a request"
...
test_10c() {
	...
	$LFS hsm_archive $f && fail "archive a noarchive file must fail"
...
test_12a() {
	...
	check_hsm_state $f "0x00000009"

	diff -q $ARC/$tdir/$tfile $f

	[[ $? -eq 0 ]] || fail "Restored file differs"
...
test_12b() {
	...
	cat $f >> /dev/null || fail "File read failed"
	echo -n "Verifying file state after restore: "
	check_hsm_state $f "0x00000009"
...
test_12c() {
	...
	wait_request_state $fid ARCHIVE SUCCEED
	$LFS hsm_release $f || fail "release $f failed"
...
test_12d() {
	...
	local f=$DIR/$tdir/$tfile
	local fid=$(copy_file /etc/hosts $f)
	$LFS hsm_restore $f || fail "restore of non archived file failed"
	local cnt=$(get_request_count $fid RESTORE)
	[[ "$cnt" == "0" ]] ||
		fail "restore non archived must not make a request"
	$LFS hsm_archive $f ||
		fail "archive request failed"
	wait_request_state $fid ARCHIVE SUCCEED
	$LFS hsm_restore $f ||
		fail "restore of non released file failed"
	local cnt=$(get_request_count $fid RESTORE)
	[[ "$cnt" == "0" ]] ||
		fail "restore a non dirty file must not make a request"

	copytool_cleanup
}
run_test 12d "Restore of a non archived, non released file must work without doing request"
...
test_12e() {
	...
	$LFS hsm_restore $f && fail "restore a dirty file must fail"
...
test_12f() {
	...
	$LFS hsm_archive --archive $AN $f
	wait_request_state $fid ARCHIVE SUCCEED
	$LFS hsm_release $f || fail "release of $f failed"
	$LFS hsm_restore $f
	wait_request_state $fid RESTORE SUCCEED
...
test_12g() {
	...
	[[ $st -eq 0 ]] || fail "Restored file differs"
...
test_12m() {
	...
	local f=$DIR/$tdir/$tfile
	local fid=$(copy_file /etc/passwd $f)
	$LFS hsm_archive $f || fail "archive of $f failed"
	wait_request_state $fid ARCHIVE SUCCEED
...
test_12n() {
	...
	cmp /etc/hosts $f || fail "Restored file differs"
...
test_13() {
	...
	for d in $(seq 1 10); do
		...
		for f in $(seq 1 10); do
			CURR_FILE="$CURR_DIR/file.$f"
			# write file-specific data
			echo "d=$d, f=$f, dir=$CURR_DIR, file=$CURR_FILE" > $CURR_FILE
		done
	done
	# import to Lustre
	import_file "$ARC_SUBDIR" $DIR/$tdir || fail "import failed"
	...
	[ $cnt_diff -eq 0 ] ||
		fail "$cnt_diff imported files differ from read data"
	[ $cnt_ok -eq 100 ] ||
		fail "not enough identical files ($cnt_ok != 100)"
...
test_14() {
	...
	$LFS hsm_set --archived --exists $f || fail "could not force hsm flags"
	...
	$HT --archive $AN --hsm_root="$ARC" --rebind $fid $fid2 $DIR ||
		fail "could not rebind file"

	# restore file and compare md5sum
	local sum2=$(md5sum $f | awk '{print $1}')

	[[ $sum == $sum2 ]] || fail "md5sum mismatch after restore"
...
test_15() {
	...
	for i in `seq 1 $count`; do
		...
		$LFS hsm_archive $f.$i || fail "could not archive file"
	done
	...
	for i in `seq 1 $count`; do
		...
		$LFS hsm_set --archived --exists $f.$i ||
			fail "could not force hsm flags"
		$LFS hsm_release $f.$i || fail "could not release file"
	done
	nl=$(wc -l < $tmpfile)
	[[ $nl == $count ]] || fail "$nl files in list, $count expected"

	echo "rebind list of files"
	$HT --archive $AN --hsm_root="$ARC" --rebind $tmpfile $DIR ||
		fail "could not rebind file list"

	# restore files and compare md5sum
	for i in `seq 1 $count`; do
		local sum2=$(md5sum $f.$i | awk '{print $1}')
		[[ $sum2 == ${sums[$i]} ]] ||
		    fail "md5sum mismatch after restore ($sum2 != ${sums[$i]})"
	done
...
test_20() {
	...
	$LFS hsm_release $f && fail "release should not succeed"

	# For following tests, we must test them with HS_ARCHIVED set
	$LFS hsm_set --exists --archived $f || fail "could not add flag"

	# Could not release a file if no-release is set
	$LFS hsm_set --norelease $f || fail "could not add flag"
	$LFS hsm_release $f && fail "release should not succeed"
	$LFS hsm_clear --norelease $f || fail "could not remove flag"

	# Could not release a file if lost
	$LFS hsm_set --lost $f || fail "could not add flag"
	$LFS hsm_release $f && fail "release should not succeed"
	$LFS hsm_clear --lost $f || fail "could not remove flag"

	# Could not release a file if dirty
	$LFS hsm_set --dirty $f || fail "could not add flag"
	$LFS hsm_release $f && fail "release should not succeed"
	$LFS hsm_clear --dirty $f || fail "could not remove flag"
...
test_23() {
	...
	[ $(stat -c "%Y" $f) -eq "978261179" ] || fail "bad mtime"
	[ $(stat -c "%X" $f) -eq "978261179" ] || fail "bad atime"
...
test_25a() {
	...
	[[ $? == 1 ]] || fail "lost file access should failed (returns $st)"

	copytool_cleanup
}
run_test 25a "Restore lost file (HS_LOST flag) from import (Operation not permitted)"
...
run_test 25b "Restore lost file (HS_LOST flag) after release (Operation not permitted)"
...
test_27a() {
	...
	[[ $? != 0 ]] || fail "Remove of a released file should fail"
...
test_30a() {
	...
	[[ $st == 0 ]] || fail "Failed to exec a released file"
...
test_31a() {
	...
	[[ $err -eq 0 ]] || fail "File size changed during restore"
...
test_33() {
	...
	[ -z $killed ] ||
		fail "Cannot kill process waiting for restore ($killed)"
...
test_52() {
	...
	multiop_bg_pause $f O_c || return 1
	local MULTIPID=$!
	...
	kill -USR1 $MULTIPID
	wait $MULTIPID || fail "multiop close failed"

	check_hsm_state $f "0x0000000b"
...
test_53() {
	...
	$LFS hsm_archive --archive $AN $f || fail "could not archive file"
	wait_request_state $fid ARCHIVE SUCCEED
	check_hsm_state $f "0x00000009"
...
test_55() {
	...
	$TRUNCATE $f 1024 || fail "truncate failed"
	sync
	wait_request_state $fid ARCHIVE FAILED
...
test_57() {
	...
	do_node $CLIENT2 "$LFS hsm_release $f" ||
		fail "hsm_release failed"
	do_node $CLIENT2 "md5sum $f" ||
		fail "hsm_restore failed"
...
test_58() {
	...
	$TRUNCATE $f 0 || fail "truncate failed"
	sync

	local sz=$(stat -c %s $f)
	[[ $sz == 0 ]] || fail "size after truncate is $sz != 0"
	...
	[[ "$state" == "" ]] ||
		fail "truncate 0 trigs a restore, state = $state"
...
test_59() {
	...
	$TRUNCATE $f $sz || fail "truncate failed"
	sync

	local sz1=$(stat -c %s $f)
	[[ $sz1 == $sz ]] || fail "size after truncate is $sz1 != $sz"
	...
	local state=$(get_request_state $fid RESTORE)
	[[ "$state" == "SUCCEED" ]] ||
		fail "truncate $sz does not trig a successfull restore, state = $state"

	cmp $ref $f || fail "file data wrong after truncate"
...
test_90() {
	...
	$LFS hsm_archive --filelist $FILELIST ||
		fail "cannot archive a file list"
	wait_all_done 100
	$LFS hsm_release --filelist $FILELIST ||
		fail "cannot release a file list"
	$LFS hsm_restore --filelist $FILELIST ||
		fail "cannot restore a file list"
	wait_all_done 100
	copytool_cleanup
}
...
double_verify_reset_ham_param() {
	...
	val=$(get_hsm_param $p)
	[[ $val == $val2 ]] ||
		fail "$HSM_PARAM.$p: $val != $val2 should be (2 * $save)"
	...
	if [[ $rc == 0 ]]
	then
		fail "we must not be able to set $HSM_PARAM.$p to 0"
	fi
}
...
test_103() {
	...
	local res=$(do_facet $SINGLEMDS $LCTL get_param $HSM_PARAM.agent_actions) | \
		grep -v CANCELED | grep -v SUCCEED | grep -v FAILED

	[[ -z "$res" ]] || fail "Some request have not been canceled"
...
test_104() {
	...
	local data1=`do_facet $SINGLEMDS $LCTL get_param $HSM_PARAM.agent_actions | \
		grep $fid | cut -f16 -d=`
	cdt_enable

	[[ "$data1" == "$DATAHEX" ]] ||
		fail "Data field in records is ($data1) and not ($DATAHEX)"
...
test_105() {
	...
	local reqcnt1=`do_facet $SINGLEMDS $LCTL get_param $HSM_PARAM.agent_actions | \
		grep WAITING | wc -l`
	cdt_restart
	cdt_disable
	local reqcnt2=`do_facet $SINGLEMDS $LCTL get_param $HSM_PARAM.agent_actions | \
		grep WAITING | wc -l`
	cdt_enable
	cdt_purge
	[[ "$reqcnt1" == "$reqcnt2" ]] ||
		fail "Requests count after shutdown $reqcnt2 != "\
...
test_106() {
	...
	local uuid=`my_uuid`
	local agent=`do_facet $SINGLEMDS $LCTL get_param $HSM_PARAM.agents |\
		grep $uuid`
	copytool_cleanup
	[[ ! -z "$agent" ]] || fail "My uuid $uuid not found in agent list"
	local agent=`do_facet $SINGLEMDS $LCTL get_param $HSM_PARAM.agents |\
		grep $uuid`
	[[ -z "$agent" ]] ||
		fail "My uuid $uuid still found in agent list, after copytool shutdown"
	copytool_setup
	local agent=`do_facet $SINGLEMDS $LCTL get_param $HSM_PARAM.agents |\
		grep $uuid`
	copytool_cleanup
	[[ ! -z "$agent" ]] ||
		fail "My uuid $uuid not found in agent list after copytool restart"
...
test_111a() {
	...
	# Test result
	[[ $st == 0 ]] || fail "Restore does not failed"

	copytool_cleanup
}
run_test 111a "No retry policy (import case), restore will fail (No such file or directory)"
...
run_test 111b "No retry policy (release case), restore will fail (No such file or directory)"
...
test_112() {
	...
	# Test result
	[[ ! -z "$res" ]] || fail "action is $l which is not an ARCHIVE"
...
test_220() {
	...
	local target=0x0
	[[ $flags == $target ]] || fail "Changelog flag is $flags not $target"
...
test_223a() {
	...
	local target=0xfd
	[[ $flags == $target ]] ||
		fail "Changelog flag is $flags not $target"
...
test_250() {
	...
	while [[ $cnt != 0 || $wt != 0 ]]; do
		sleep 1
		cnt=$(do_facet $SINGLEMDS "$LCTL get_param $HSM_PARAM.agent_actions" | \
			grep STARTED | grep -v CANCEL | wc -l)
		[[ $cnt -le $maxrequest ]] ||
			fail "$cnt > $maxrequest too many started requests"
		wt=$(do_facet $SINGLEMDS "$LCTL get_param $HSM_PARAM.agent_actions" | \
			grep WAITING | wc -l)
