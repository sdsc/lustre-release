...
test_33() {
	...
	cdt_purge
	wait_for_grace_delay
	# Also raise grace_period significantly so the Canceled
	# Restore action will stay enough long avail.
	local old_grace=$(get_hsm_param grace_period)
	set_hsm_param grace_period 100
	...
	# restore orig grace_period.
	set_hsm_param grace_period $old_grace
