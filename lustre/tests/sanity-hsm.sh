...
copy_file() {
	...
	rm -f $f
	cp $1 $f || fail "cannot copy $1 to $f"
	path2fid $f || fail "cannot get fid on $f"
}

make_small() {
        local file2=${1/$DIR/$DIR2}
        dd if=/dev/urandom of=$file2 count=2 bs=1M conv=fsync ||
		fail "cannot create $file2"
        path2fid $1 || fail "cannot get fid on $1"
}
...
test_25a() {
	...
	[[ $st == 1 ]] || fail "lost file access should failed (returns $st)"
...
test_52() {
	...
	multiop_bg_pause $f O_c || fail "multiop failed"
	local MULTIPID=$!
