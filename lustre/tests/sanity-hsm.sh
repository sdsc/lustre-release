...
helper_test_113() {
	...
	skip "HSM_MIGRATE is not supported" && return

	# test needs a running copytool
	copytool_setup

	# lfs hsm_migrate file onto OST 0 if it is on OST 1, or onto
	...
	cmd="$LFS hsm_migrate $optcmd $newobdidx $file"
	echo $cmd
	...
	[[ $newobdidx -ne $realobdix ]] &&
		error "unexpected new OST (was=$obdidx, wanted=$newobdidx, got=$realobdix)"
	[[ "$oldmd5" != "$newmd5" ]] &&
...
run_test 113a "HSM_MIGRATE test with -i"

test_113b() {
	helper_test_113 "-o"
}
run_test 113b "HSM_MIGRATE test with -o"
