...
static void cleanup()
{
	...
	ASSERTF((rc > 0 && rc < sizeof(cmd)), "invalid name");
	system(cmd);
}
...
static void helper_fid2path(const char *filename)
{
	...
	ASSERTF(rc == 0, "rc=%d, fid=%s, name=%s", rc, fidstr, filename);
...
static void test10(void)
{
	...
	rc = unlink(mainpath);
	ASSERTF(rc == 0, "can't unlink %s", mainpath);
	...
	ASSERTF(rc == 0, "rc=%d", rc);
	rc = mknod(mainpath, 0 | S_IFBLK, statbuf.st_rdev);
	ASSERTF(rc == 0, "rc=%d", rc);
	helper_fid2path(mainpath);
	rc = unlink(mainpath);
	ASSERTF(rc == 0, "can't unlink %s", mainpath);
	...
	/* Against a socket. */
	ASSERTF(rc == 0, "rc=%d", rc);
	rc = mknod(mainpath, 0 | S_IFSOCK, (dev_t)0);
	ASSERTF(rc == 0, "rc=%d", rc);
	helper_fid2path(mainpath);
	rc = unlink(mainpath);
	ASSERTF(rc == 0, "can't unlink %s", mainpath);
...
static void test20()
{
	...
	ASSERTF((rc > 0 && rc < sizeof(testpath)), "invalid name for testpath");

	rc = mkdir(testpath, S_IRWXU);
	ASSERTF(rc == 0, "cannot create dir %s: %d", testpath, rc);
	 ...
	 * reasonably long mountpath, we should have created about
	 * 2000. */
...
static void test30(void)
{
	...
	ASSERTF(fd >= 0, "fd=%d, errno=%d, file=%s",
		fd, errno, links[0].filename);
	close(fd);
	...
	do {
		...
		rc = llapi_fid2path(links[0].filename, fidstr, buf,
				    sizeof(buf), &recno, &linkno);
		ASSERTF(rc == 0, "rc=%d, fid=%s", rc, fidstr);
...
static void help_test40(void)
{
	...
	ASSERTF(rc == 0, "rc=%d", rc);
	ASSERTF(strcmp(buf, maindir) == 0, "strings are different");
...
static void test42(void)
{
	...
	fd = creat(link0, 0);
	ASSERTF(fd >= 0, "fd=%d, errno=%d, file=%s", fd, errno, link0);
	close(fd);
