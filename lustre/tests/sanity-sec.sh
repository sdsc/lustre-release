...
test_23() {
	...
	nodemap_acl_test $unmapped_fs ${clients[0]} ${clients[1]} ||
		error "acl count (1)"

	# setfacl on trusted cluster to mapped user, verify it's seen
	nodemap_acl_test $mapped_fs ${clients[0]} ${clients[1]} &&
		error "acl count (2)"

	# setfacl on mapped cluster to mapped user, verify it's seen
	nodemap_acl_test $mapped_c1 ${clients[1]} ${clients[0]} &&
		error "acl count (3)"

	# setfacl on mapped cluster to unmapped user, verify error
	nodemap_acl_test $unmapped_fs ${clients[1]} ${clients[0]} 1 ||
		error "acl count (4)"
	...
	nodemap_acl_test $unmapped_c1 ${clients[1]} ${clients[0]} ||
		error "acl count (6)"
