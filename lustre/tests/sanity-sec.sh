...
squash_id() {
	...
	cmd[0]="$LCTL nodemap_modify --parameter squash_uid"
	cmd[1]="$LCTL nodemap_modify --parameter squash_gid"
...
test_13() {
	...
	for ((i = 0; i < NODEMAP_COUNT; i++)); do
		for ((j = 0; j < NODEMAP_RANGE_COUNT; j++)); do
			for ((k = 1; k < 253; k++)); do
				if ! test_nid $SN.$i.$j.$k ${HN}_${i}; then
					rc=$((rc + 1))
				fi
			done
		done
...
test_14() {
	...
	for ((i = 0; i < NODEMAP_COUNT; i++)); do
		for ((j = 0; j < NODEMAP_RANGE_COUNT; j++)); do
			for ((k = 1; k < 253; k++)); do
				if ! test_nid $SN.$i.$j.$k default; then
					rc=$((rc + 1))
				fi
			done
		done
