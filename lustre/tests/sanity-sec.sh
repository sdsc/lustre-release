...
nodemap_acl_test() {
	...
	[ "$setfacl_error" == "1" ] && echo "WARNING: unable to setfacl in nodemap_acl_test"
...
test_23() {
	...
	do_facet mgs $LCTL set_param nodemap.c0.admin_nodemap=1
	do_facet mgs $LCTL set_param nodemap.c0.trusted_nodemap=1
	do_facet ost0 $LCTL set_param nodemap.c0.admin_nodemap=1
	do_facet ost0 $LCTL set_param nodemap.c0.trusted_nodemap=1

	do_facet mgs $LCTL set_param nodemap.c1.admin_nodemap=0
	do_facet mgs $LCTL set_param nodemap.c1.trusted_nodemap=0
	...
	nodemap_acl_test $unmapped_fs ${clients[0]} ${clients[1]} || error "acl count (1)"

	# setfacl on trusted cluster to mapped user, verify it's seen
	nodemap_acl_test $mapped_fs ${clients[0]} ${clients[1]} && error "acl count (2)"
	
	# setfacl on mapped cluster to mapped user, verify it's seen
	nodemap_acl_test $mapped_c1 ${clients[1]} ${clients[0]} && error "acl count (3)"
	
	# setfacl on mapped cluster to unmapped user, verify error
	nodemap_acl_test $unmapped_fs ${clients[1]} ${clients[0]} 1 || error "acl count (4)"

	# 2 mapped clusters
	do_facet mgs $LCTL set_param nodemap.c0.admin_nodemap=0
	do_facet mgs $LCTL set_param nodemap.c0.trusted_nodemap=0
	...
	nodemap_acl_test $mapped_c1 ${clients[1]} ${clients[0]} && error "acl count (5)"

	# setfacl to mapped user on c1, not mapped to c0, verify not seen
	nodemap_acl_test $unmapped_c1 ${clients[1]} ${clients[0]} || error "acl count (6)"
