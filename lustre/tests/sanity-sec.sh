...
test_idmap() {
	...
	# XXX debugging
	do_facet mgs lctl get_param nodemap.*.idmap
	do_facet mgs lctl get_param nodemap.*.ranges
...
do_fops() {
	...
	local res="$c $w $r $d $q"
	local expected=$(get_fops_expected $key)
...
nodemap_acl_test_setup() {
	...
	do_node ${clients_arr[0]} rm -rf $DIR/$tdir
	do_node ${clients_arr[0]} find $DIR/ -ls
	do_node ${clients_arr[0]} mkdir -p $DIR/$tdir
