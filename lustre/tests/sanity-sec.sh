...
[ -z "$USER0" ] && U=runas && P="/etc/passwd" &&
	skip "no '$U:x:$RUNAS_ID:$RUNAS_GID:$U:/home/$U:/bin/bash' in $P" &&
	exit 0
[ -z "$USER2" ] && U=runas2 && P="/etc/passwd" &&
	skip "no '$U:x:$RUNAS_ID2:$RUNAS_GID2:$U:/home/$U:/bin/bash' $P" &&
	exit 0

IDBASE=${IDBASE:-$(id -u quota_usr 2> /dev/null || echo 60000)}
