...
wait_nm_sync() {
	...
	local is_active=$(lctl get_param -n nodemap.active)
	[ "$is_active" == "0" ] && return

	local max_retries="20"
	local is_sync
	local out1=$(do_facet mgs $LCTL get_param nodemap.${client}.${key})
	local out2
	...
	[ $is_sync == "0" ] && error "mgs and oss ${key} mismatch, $i attempts"
...
test_fops_chmod_dir() {
	...
	[ "$num_clients" == "1" ] && new_admin_cli_i=0

	if [ "$new_admin_cli_i" != "" ]; then
	...
	do_node $test_fops_admin_client chmod $mode $DIR/$tdir || return 1

	# restore admin for single client
	...
	for mds_i in -1 0 1 2; do
		...
		for client in $clients; do
			...
			for u in ${client_user_list[$cli_i]}; do
				...
				# check quota
				test_fops_chmod_dir $cli_i 777 \
					$DIR/$tdir || error cannot chmod $key
