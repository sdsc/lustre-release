...
do_fops_quota_test() {
	...
	for node in $(all_server_nodes); do
		echo - $node
		do_node $(host_nids_address $node $NETTYPE) lctl mark BEFORE QUOTA
		do_node $(host_nids_address $node $NETTYPE) 'find /proc/fs/lustre/nodemap \
			-type f -perm /444 | xargs grep . '
	done
	...
	for node in $(all_server_nodes); do
		echo - $node
		do_node $(host_nids_address $node $NETTYPE) lctl mark AFTER QUOTA
		do_node $(host_nids_address $node $NETTYPE) 'find /proc/fs/lustre/nodemap \
			-type f -perm /444 | xargs grep . '
	done
