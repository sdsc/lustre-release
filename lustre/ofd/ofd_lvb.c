 ...
 *
 * This file contains methods for OFD LVB (Lock Value Block) operations.
 *
 * LVB is a special opaque (to LDLM) data that associated with an LDLM lock
 * and transferred from client to server and back.
 *
 * Currently LVBs are used by:
 *  - OSC-OST to maintain current object size/times
 ...
 *
 * This function initializes new LVB data for the given LDLM resource if
 * it is not initialized yet. New LVB is filled with attributes of the object
 ...
 *
 * This function updates a LVB data for the given LDLM resource from
 * the \a request and from disk.
 *
 * This is be called in two ways:
 ...
 * \param[in] increase_only	don't allow values to move backwards
 *
 * \retval		0 on successful setup
 * \retval		negative value on error
 */
