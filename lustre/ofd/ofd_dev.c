				    ...
				    __u64 start, __u64 end)
{
	...
	if (oa->o_valid & OBD_MD_FLHANDLE) {
		...
		if (lock != NULL) {
			...
			if (lock->l_granted_mode == LCK_PW &&
				...
				locks = ofd_prolong_one_lock(tsi, lock,
							     &extent, timeout);
				LDLM_LOCK_PUT(lock);
				RETURN(locks);
			}
	...
	list_for_each_entry(lock, &exp->exp_bl_list, l_exp_list) {
		...
		locks = ofd_prolong_one_lock(tsi, lock, &extent, timeout);
	}
...
static int ofd_rw_hpreq_check(struct ptlrpc_request *req)
{
	...
	locks = ofd_prolong_extent_locks(tsi, start, end);
...
static int ofd_punch_hpreq_check(struct ptlrpc_request *req)
{
	...
	locks = ofd_prolong_extent_locks(tsi, oa->o_size, oa->o_blocks);
...
void ofd_hp_brw(struct tgt_session_info *tsi)
{
	struct niobuf_remote	*rnb;
	struct obd_ioobj	*ioo;
...
void ofd_hp_punch(struct tgt_session_info *tsi)
{
