 ...
 * new locked_region structure and locks it with extent lock, then adds to
 * the list of all such regions.
 *
 ...
 *
 * The locking of sparce areas will cause dirty data to be flushed back from
 ...
 *
 * Every time a BRW request arrives for the object covered by
 * the lock, extend the timeout on that lock. The RPC should
 ...
 *
 * Check if \a lock is the same as the lock from request \a req by comparing
 * the resource name, mode, and extent as one taken from the request.
 *
 ...
 *
 * This function checks if the incoming request is a candidate for
 * high-priority processing.
 *
 * \param[in] tsi	target session environment for this request
 *
 * \retval		rq_ops is set to high-priority operation table
 *			if request is a candidate for that else rq_ops is NULL
 */
