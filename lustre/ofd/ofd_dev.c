 ...
 *
 * This file contains OSD API methods for OFD, request handlers and
 * supplemental functions to set OFD up and clean it up as well.
 *
 ...
 *
 * \param[in] char	parameters to check
 *
 ...
 * pass them through to the lower level.
 *
 * \param[in] env	execution environment
 ...
 *
 * This function allocates OFD part of compound object and
 ...
 * creating until problem is fixed by LFSCK.
 *
 * \param[in] env	execution environment
 ...
 * serving user requests. In OFD it starts lfsk check routines and initialize
 * recovery.
 *
 * \param[in] env	execution environment
 ...
 * device is in full state and consistent. An OFD calculates grant space upon
 * this event.
 *
 * \param[in] env	execution environment
 * \param[in] dev	lu_device of OFD device
...
/**
 * Links OFD procfs entries to the corresponding OSD entries.
 *
 * IO statistic and controls were moved to the OSD, the same entries
 * are kept in OFD for compatibility reasons and made as symlinks.
 *
 * \param[in] ofd	OFD device
 */
 ...
 * It also connects to the master server and get own FID sequence (SEQ)
 * range to this particular OFD.
 *
 * \param[in] env	execution environment
 * \param[in] ofd	OFD device
 *
 ...
 *
 * This function returns a list of extents which desribes how a file's
 ...
 *
 * This is supplimental function for lock_zero_regions(). It allocates
 * new locked_region structure and lock it with extent lock, then add to the
 * list of all such regions.
 *
 ...
 *
 * The locking of sparce areas should cause dirty data flushed back from
 * clients. This is used during the FIEMAP getting to make sure there is no
 * unaccounted cached data on clients.
 *
 ...
 *
 * This function goes through list of locked regions, unlock and free
 * them one-by-one.
 *
 * \param[in] ns	LDLM namespace
 * \param[in] locked	list head of regions list
 ...
 * by its FID, gets attributes from storage and pack result to the reply.
 *
 * \param[in] tsi	target session environment for this request
 *
 * \retval		0 if successful
 ...
 *
 * This is OFD-specific part of request handling. It finds the OFD object
 * by its FID, sets attributes from request and pack result to the reply.
 *
 * \param[in] tsi	target session environment for this request
 *
 * \retval		0 if successful
 ...
 *
 * This is OFD-specific part of request handling. It is main purpose to
 ...
 * get the requested data from bottom storage and pack reply.
 *
 * \param[in] tsi	target session environment for this request
 *
 * \retval		0 if successful
 ...
 * prolong their timeout. Requests with priority might lead to faster
 * release of these locks and timeout prolongation prevents lock
 * timeout for request service time.
 *
 * \param[in] tsi	target session environment for this request
 ...
 * It is used to give priority to RPCs that might lead to fastest release
 * of that lock when a lock is contended.
 *
 ...
 * so that client has more room to send lock cancel RPC.
 *
 ...
 *
 * This function checks if the incoming request is capable for
 * high-priority processing.
 *
 * \param[in] tsi	target session environment for this request
 *
 * \retval		rq_ops is set to high-priority operation table
 *			if request is capable for that else rq_ops is NULL
 */
...
/**
 * Assign high priority operations to the OST_PUNCH request.
 *
 * This function checks if the incoming request is capable for
 * high-priority processing.
 *
 ...
 * specify handlers for them. The tgt_request_handler()
...
/**
 * Initialize OFD device according with parameters in the config log \a cfg.
 *
 * This is main starting point of OFD initialization. It fills all OFD
