...
static int ofd_ladvise_hdl(struct tgt_session_info *tsi)
{
	...
	rc = ofd_auth_capa(exp, &info->fti_fid, ostid_seq(&body->oa.o_oi),
			   BYPASS_CAPA, CAPA_OPC_OSS_READ);
	if (rc)
		RETURN(rc);
	...
	case LU_LADVISE_WILLNEED:
		...
		remote_nb.rnb_len = ladvise->lla_end - ladvise->lla_start;
		if (remote_nb.rnb_len == 0)
			GOTO(out, rc = 0);

		rc = tgt_brw_lock(exp->exp_obd->obd_namespace, &tsi->tsi_resid,
				  &ioo, &remote_nb, &lockh, LCK_PR);
		if (rc != 0)
			GOTO(out, rc);
