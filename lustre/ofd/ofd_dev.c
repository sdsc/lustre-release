 ...
 */
static inline void ofd_drop_id(struct obd_export *exp, struct obdo *oa)
{
...
static int ofd_getattr_hdl(struct tgt_session_info *tsi)
{
	...
	curr_version = dt_version_get(tsi->tsi_env, ofd_object_child(fo));
...
static int ofd_setattr_hdl(struct tgt_session_info *tsi)
{
	...
	if (body->oa.o_valid & OBD_MD_FLSIZE &&
		...
		CERROR("%s: setattr from %s is trying to truncate object "DFID
		       ...
		       PFID(lu_object_fid(tsi->tsi_corpus)), mdsinum);
		RETURN(-EPERM);
	}
	...
	ost_fid_build_resid(lu_object_fid(&fo->ofo_obj.do_lu),
			    &fti->fti_resid);
	res = ldlm_resource_get(ofd->ofd_namespace, NULL, &fti->fti_resid,
				LDLM_EXTENT, 0);
	if (res != NULL) {
...
TGT_RPC_HANDLER(OST_FIRST_OPC,
		HABEO_REFERO,		OST_SET_INFO,	ofd_set_info_hdl,
