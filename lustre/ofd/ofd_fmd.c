 ...
 *
 * This file provides functions to handle FMD (Filter Modification Data).
 * The FMD is responsible for file attributes to be applied in XID order,
 * so older requests can't re-write newer attributes.
 *
 * FMD is organized as per-client list and identified by FID of object,
 * each FMD stores FID of object and the highest received XID of modification
 * request for this object.
 *
 * FMD can expire if there is no updates for a long time to keep the list
 * reasonably small.
 *
...
/**
 * Drop FMD reference and free it if reference is last.
 *
 ...
 *
 * The \a keep FMD is not to be expired in any case, this
 * parameter is used by ofd_fmd_find_nolock() to don't expire just
 * found FMD.
 *
 * \param[in] exp	OBD export
 ...
 * Expire FMD entries.
 *
 * This is just wrapper to call ofd_fmd_expire_nolock() under required lock.
 *
 * \param[in] exp	OBD export
 */
 ...
 *
 * Caller must hold filter_export_data::fed_lock and take FMD reference by
 * itself.
 *
 * \param[in] exp	OBD export
 ...
 * or if passed FID is zero (which will only cause old entries to expire).
 ...
 *
 * NB: this function is used only if DO_FMD_DROP is defined. It is not now, so
 * FMD drop is not happens and FMD are dropped just being expired.
 *
 * \param[in] exp	OBD export
