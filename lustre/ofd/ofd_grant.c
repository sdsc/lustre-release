 ...
 * Each client nodes is granted an initial amount of reserved space at connect
 ...
 *
 * \param[in] obd  - OBD device for which grant accounting should be verified
 ...
 * \param[in] exp - export used to print client info in debug messages
...
/**
 * Processing grant information from obdo structure packed in incoming BRW
 *
 ...
 *
 * Client nodes can explicitely release grant space (i.e. process called grant
 ...
 * \param[in,out] oa - incoming obdo sent by the client
 * \param[in] left_space - remaining free space with space already granted taken
 *			   out
 */
 ...
 *
 * \retval space (in bytes) that will be consumed to write the network buffer
 */
 ...
 * \param[in] want - how much grant space the client would like to have
 * \param[in] left - remaining free space with granted space taken out
 * \param[in] conservative - if set to true, the server should be cautious and
 *			     bound how much space is granted back to the client.
 *			     Otherwise, the server should try hard to satisfy
 *			     the client request.
 *
 ...
 *
 * New non-readonly connection gets an initial grant allocation equals to
 ...
 * This function consumes enough space to create \nr objects and allocates more
 * grant space to the self export for future precreation requests, if possible.
 *
 * \param[in] env - LU environment provided by the caller
