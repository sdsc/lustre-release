 ...
 *
 * This file contains ODB API methods for OFD which are used for export
 * handling, configuration purposes and recovery. Several methods are used by
 * ECHO client only since it still uses OBD API. Such methods have _echo_
 * prefix in name.
 *
...
/**
 * Initialize OFD per-export statistic.
 *
 * This function sets up procfs entries for various OFD export counters.
 *
...
/**
 * Re-initialize export upon client re-connection.
 *
 * This function parse connection data again and resets export statistic.
 *
 * \param[in] env	execution environment
 * \param[in] exp	OBD export
 ...
 *
 * This function finalizes the client connection. The client export is
...
/**
 * Get bottom file system statistic.
 *
 * Helper function for ofd_statfs(), also used by grant code.
 *
 * \param[in]  env	  execution environment
 * \param[in]  exp	  OFD device
 ...
 * It is called from several place by using the OBD API as well as
 ...
 *
 * This function is only used by ECHO client when it is ran on top of OFD,
 ...
 *
 * Supplimental function to destroy object by FID, it is used by request
 ...
 * \param[in] orphan	set if orphaned object is being destroyed
 *
 * \retval		0 if successful
 * \retval		negative value on error
 */
 ...
 *
 * This is supplemental function for ofd_iocontrol() to returns object
 ...
 * from a cleanup function upon forceful device cleanup. For OFD there is no
 * special actions, it invokes just target_recovery_cleanup().
 *
 * \param[in] obd	OBD device of OFD
 ...
 * It controls FMD (oFd Modification Data) expiration each time PING is
 * received.
 *
 * \param[in] env	execution environment
 ...
 *
 * This function checks the OFD device health - ability to response on
 * incoming requests. There are two health_check methods:
 * - get statfs from bottom device. It checks just responsiveness of
 *   bottom device
 * - do write attempt on bottom device to check it is fully operational and
 *   is not stuck. This is expensive method and require special configuration
 *   option --enable-health-write while making Lustre, it is turned off
 *   by default.
 *
