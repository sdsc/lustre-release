		...
		struct obd_info *oinfo, struct obd_trans_info *oti)
{
	...
	fo = ofd_object_find_exists(env, ofd, &info->fti_fid);
	if (IS_ERR(fo)) {
		...
		void *capa)
{
	...
	CDEBUG(D_HA, "%s: Destroy object "DOSTID"\n", ofd_name(ofd),
	       POSTID(&oa->o_oi));
	...
	rc = ofd_destroy_by_fid(env, ofd, &info->fti_fid, 0);
	if (rc == -ENOENT) {
		...
		CERROR("%s: error destroying object "DFID": %d\n",
		       ofd_obd(ofd)->obd_name, PFID(&info->fti_fid), rc);
		GOTO(out, rc);
	}
