 ...
 * distributed by MDS to client upon open, and will be packed into subsequent
 * requests to MDS and OSS. Capability key is shared by MDS and OSS, which is
 * used to sign and authenticate capability (HMAC algorithm).
 *
 ...
 *
 * A new capability key is received from MDS, because MDS only uses the
 ...
 *
 * OFD authenticate the capability packed in the request. Firstly, it will
 ...
 * \param[in] opc  - opcode the called requested
