			     ...
			     struct dt_txn_commit_cb *cb, int err)
{
	...
	ossc = container_of0(cb, struct ofd_soft_sync_callback, ossc_cb);

	CDEBUG(D_INODE, "export %p soft sync count is reset\n", ossc->ossc_exp);
	cfs_atomic_set(&ossc->ossc_exp->exp_filter_data.fed_soft_sync_count, 0);
...
retry:
	...
	if (th->th_sync == 0) {
		for (i = 0; i < niocount; i++) {
			...
			if (!soft_sync && lnb[i].lnb_flags & OBD_BRW_SOFT_SYNC)
				soft_sync = true;
		}
	}
...
out_stop:
	...
	if (!th->th_sync && soft_sync && !trigger_sync &&
	    (cfs_atomic_inc_return(&fed->fed_soft_sync_count) ==
			ofd->ofd_soft_sync_limit)) {
		/* if there are ofd_soft_sync_limit number of consecutive
		 * SOFT_SYNC RPCs, trigger a commit, and reset
		 * fed_soft_sync_count in commit callback. */
		ofd_soft_sync_cb_add(th, exp);
		trigger_sync = true;
	} else if (!soft_sync) {
		/* reset fed_soft_sync_count upon non-SOFT_SYNC RPC */
		cfs_atomic_set(&fed->fed_soft_sync_count, 0);
	}
