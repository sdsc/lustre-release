 ...
 * also LFSCK routines to check parent FID (PFID) consistency.
 *
...
/**
 * Verify single object for PFID consistency.
 *
...
/**
 * Verification thread to check PFID consistency.
 *
 ...
 *
 * Checks the parent FID is sane and start extended
 ...
 * \retval		-EPERM if PFID was verified by still insane
 */
 ...
 *
 * This function parses remote buffers from client to local buffers
 ...
 *
 * This function parses remote buffers from client to local buffers
 * and prepare the latter. If there is recovery in progress and required
...
/**
 * Prepare Bulk IO requests processing.
 *
...
/**
 * Put local buffers for read bulk IO.
 *
 * \param[in] env	execution environment
 * \param[in] ofd	OFD device
 ...
 *
 * Helper function for ofd_commitrw_write() to change object attributes
 * and write parent FID into extended attributes when needed.
 *
 * \param[in] env	execution environment
 * \param[in] ofd	OFD device
 ...
 *
 * This function adds callback to be called upon when commin is done.
 *
 * \param[in] th	transaction handle
 * \param[in] exp	OBD export of client
 *
 ...
 *
 * This function finalizes write IO processing by writting data to the disk.
 ...
 *
 * This is pair function to the ofd_preprw(). It finishes bulk IO
