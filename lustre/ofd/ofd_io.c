...
static int ofd_inconsistency_verification_main(void *args)
{
	...
	while (1) {
		...
		while (!list_empty(&ofd->ofd_inconsistency_list)) {
			...
			lr->lr_seq = fo->ofo_header.loh_fid.f_seq;
			lr->lr_oid = fo->ofo_header.loh_fid.f_oid;
			lr->lr_ver = fo->ofo_header.loh_fid.f_ver;
			lr->lr_fid = oii->oii_pfid;
			lr->lr_fid2.f_seq = pfid->f_seq;
			lr->lr_fid2.f_oid = pfid->f_oid;
			lr->lr_fid2.f_ver = pfid->f_ver;
			...
			case LPVS_INIT:
				if (rc < 0)
					CDEBUG(D_LFSCK, "%s: fail to verify "
					       "local stored PFID xattr for "
					       DFID", client given PFID "DFID
			...
			case LPVS_INCONSISTENT:
				...
				if (rc < 0)
					CDEBUG(D_LFSCK, "%s: fail to verify "
					       "client given PFID for "DFID
					       ", client given PFID "DFID
					       ", local stored PFID "DFID
					       ", rc = %d\n",
					       ofd_obd(ofd)->obd_name,
					       PFID(&fo->ofo_header.loh_fid),
					...
					CDEBUG(D_LFSCK, "%s: both client given "
					       "PFID and local stored PFID are "
					       "invalid for the OST-object "
					       DFID", client given PFID "DFID
					       ", local stored PFID "DFID"\n",
					       ofd_obd(ofd)->obd_name,
					       PFID(&fo->ofo_header.loh_fid),
			...
			case LPVS_INCONSISTENT_TOFIX:
				...
				if (rc == 0) {
					...
					CDEBUG(D_LFSCK, "%s: fixed the staled "
					       "OST PFID xattr for "DFID
					       ", with client given PFID "DFID
					       ", old stored PFID "DFID"\n",
					       ofd_obd(ofd)->obd_name,
					       PFID(&fo->ofo_header.loh_fid),
					       PFID(&oii->oii_pfid), PFID(pfid));
				} else {
					CDEBUG(D_LFSCK, "%s: fail to fix the "
					       "OST PFID xattr for "DFID
					       ", client given PFID "DFID
					       ", local stored PFID "DFID
					       ", rc = %d\n",
				       ...
				       struct ofd_object *fo, struct obdo *oa)
{
	...
	oii->oii_pfid.f_ver = oa->o_stripe_idx;
