...
static int ofd_inconsistency_verification_main(void *args)
{
	...
	while (!list_empty(&ofd->ofd_inconsistency_list)) {
		...
		list_del_init(&oii->oii_list);
		fo = oii->oii_obj;
		/* When the thread to exit, the RPC service threads should have
		 * stopped already, so clear the ofo_pfid_checking without lock
		 * (dt_write_lock) is safe. */
		fo->ofo_pfid_checking = 0;
		spin_unlock(&ofd->ofd_inconsistency_lock);
				       ...
				       struct ofd_object *fo, struct obdo *oa)
{
	...
	oii->oii_pfid.f_ostidx = oa->o_stripe_idx;
