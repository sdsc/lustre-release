...
static int ofd_inconsistency_self_repair_main(void *args)
{
	...
	while (1) {
		...
		while (!list_empty(&ofd_inconsistency_list)) {
			oii = list_entry(ofd_inconsistency_list.next,
					 struct ofd_inconsistency_item,
					 oii_list);
		...
		l_wait_event(thread->t_ctl_waitq,
			     !list_empty(&ofd_inconsistency_list) ||
...
static void ofd_add_inconsistency_item(struct ofd_object *fo, struct obdo *oa)
{
	...
	spin_lock(&ofd_inconsistency_lock);
	if (list_empty(&ofd_inconsistency_list))
		wakeup = true;
	list_add_tail(&oii->oii_list, &ofd_inconsistency_list);
