...
struct ofd_seq *ofd_seq_get(struct ofd_device *ofd, obd_seq seq)
{
	...
	/* Use a linked list of oseq for now, since there are expected
	 * to be very few of them (~2-3 * mdt_count) for a filesystem.
	 * If this is not the case in the future, a hash could be used. */
				   ...
				   struct ofd_seq *new_seq)
{
	...
	cfs_list_for_each_entry(os, &ofd->ofd_seq_list, os_list) {
		if (os->os_seq == new_seq->os_seq) {
			...
			cfs_write_unlock(&ofd->ofd_seq_list_lock);
			/* The seq has been added */
			ofd_seq_put(env, new_seq);
			ofd_seq_destroy(env, new_seq);
			return os;
		}
	}
...
obd_id ofd_seq_last_oid(struct ofd_seq *oseq)
{
	...
	cfs_spin_lock(&oseq->os_last_oid_lock);
	id = oseq->os_last_oid;
	cfs_spin_unlock(&oseq->os_last_oid_lock);
...
void ofd_seqs_fini(const struct lu_env *env, struct ofd_device *ofd)
{
	...
	while (!cfs_list_empty(&dispose)) {
		oseq = container_of0(dispose.next,
				   struct ofd_seq, os_list);
			     ...
			     obd_seq seq)
{
	...
	cfs_mutex_init(&oseq->os_create_lock);
	cfs_spin_lock_init(&oseq->os_last_oid_lock);
	oseq->os_seq = seq;

	/* Set refc to be 2, one for oseq itself, the other
	 * one will be balanced by seq_put after ofd_seq_load */
	cfs_atomic_set(&oseq->os_refc, 2);
	...
	if (info->fti_attr.la_size == 0) {
		...
		CERROR("%s: corrupted size "LPU64" LAST_ID of seq "LPX64"\n",
			ofd_name(ofd),
			(unsigned long long)info->fti_attr.la_size, seq);
