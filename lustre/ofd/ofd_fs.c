...
/**
 * Helper to restrict precreate batch count by its upper limit.
 *
...
/**
 * Helper function to put the ofd_seq structure.
 *
...
/**
 * Helper to get last object ID for the given sequence.
 *
...
/**
 * Function sets new last object ID for the given sequence.
 *
			   ...
			   struct ofd_seq *oseq)
{
	...
	rc = dt_declare_record_write(env, obj, &info->fti_buf, info->fti_off, th);
	if (rc < 0)
		GOTO(out, rc);
	rc = dt_trans_start_local(env, ofd->ofd_osd, th);
	if (rc < 0)
		GOTO(out, rc);
	rc = dt_record_write(env, obj, &info->fti_buf, &info->fti_off, th);
	if (rc < 0)
		GOTO(out, rc);
...
/**
 * Gets LWP exports from LWP connection for local FLDB server and SEQ client.
 *
...
/**
 *
 * Initializes SEQ and FLD service on OFD.
 *
