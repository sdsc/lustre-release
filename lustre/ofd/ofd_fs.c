...
/**
 * Generic function to write fixed size records on disk
 *
 * This function is a helper which hides all needed work with transactions
 * inside and uses OSD API functions to call a lower layer, OSD usually.
 *
 * \param[in]     env	execution environment
 * \param[in]     ofd	OFD device
 * \param[in]     dt	dt_object to write in
 * \param[in]     buf	buffer with data and size
 * \param[in,out] off	offset to start from, after successful write it
 *			contains the new offset.
 *
 * \retval		0 on successful write of data in \a buf
 * \retval		negative error if write was failed
 */
...
/**
 * Helper to check and restrict batch count by its upper limit -
 * the ofd_device::ofd_precreate_batch value.
 *
 * \param[in] ofd	OFD device
 ...
 */
static inline int ofd_precreate_batch(struct ofd_device *ofd, int batch)
{
	int count;
...
/**
 * Helper to get ofd_seq structure by given \a seq.
 * It finds appropriate structure in the list of them and
 * take reference on it.
 *
 * \param[in] ofd	OFD device
 * \param[in] seq	sequence number, FID sequence usually
 *
 * \retval		pointer to the proper ofd_seq structure
 * \retval		NULL if ofd_seq is not found
 */
static inline struct ofd_seq *ofd_seq_get(struct ofd_device *ofd, obd_seq seq)
{
	struct ofd_seq *oseq;
...
/**
 * Helper function to drop reference on ofd_seq structure and
 * free it if reference is last one.
 *
 * \param[in] env	execution environment
 * \param[in] oseq	ofd_seq structure to put
 */
static inline void ofd_seq_put(const struct lu_env *env, struct ofd_seq *oseq)
{
...
/**
 * Function adds a new ofd_seq for the given OFD device.
 *
 * First it looks for existent ofd_seq for the same sequence and
 * then adds the \a new_seq if not found.
 *
 * \param[in] env	execution environment
 * \param[in] ofd	OFD device
 ...
 */
static inline struct ofd_seq *ofd_seq_add(const struct lu_env *env,
...
/**
 * Helper to get last OID for the given sequence.
 *
 * \param[in] ofd_seq	OFD sequence structure
 *
 * \retval		the last OID for this sequence
 */
static inline obd_id ofd_seq_last_oid(struct ofd_seq *oseq)
{
	obd_id id;
...
/**
 * Function sets new last OID for the given sequence.
 *
 * \param[in] oseq	OFD sequence
 * \param[in] id	the new OID to set
 */
static inline void ofd_seq_last_oid_set(struct ofd_seq *oseq, obd_id id)
{
	spin_lock(&oseq->os_last_oid_lock);
 ...
 *
 * This function writes the sequence data to the disk. This is just
 * an OID of the latest used object FID. Each one is stored in per-sequence
 * file.
 *
 * \param[in] env	execution environment
 * \param[in] ofd	OFD device
...
/**
 * Deregisters LWP items for FLDB and SEQ client on OFD
 *
 * \param[in]     ofd	OFD device
 */
 ...
 *
 * This function goes through the list of ofd_seq structures and
 * frees them.
 *
 * \param[in] env	execution environment
 * \param[in] ofd	OFD device
...
/**
 * Stops FLDB and SEQ services on OFD.
 *
 * \param[in] env	execution environment
 * \param[in] ofd	OFD device
 *
 ...
 */
/**
 * Get the ofd_seq structure and load it with data from disk
 *
...
/**
 * This function initializes local FLDB server.
 *
 * \param[in] env	execution environment
...
/**
 * This function gets LWP exports from LWP connection for local
 * FLDB server and SEQ client.
 *
 * \param[in] ofd	OFD device
 *
...
/**
 *
 * This function initializes SEQ and FLD service on OFD.
 *
 * \param[in] env	execution environment
 * \param[in] ofd	OFD device
 *
 ...
 *
 * This function sets up service files for OFD, it is health_check
 * file for now only.
 *
 * \param[in] env	execution environment
 * \param[in] ofd	OFD device
 ...
 *
 * This function syncs all data and closes service files for OFD.
 *
 * \param[in] env	execution environment
 * \param[in] ofd	OFD device
 */
