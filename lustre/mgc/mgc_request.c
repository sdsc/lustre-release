	...
	struct config_llog_instance *cfg, struct super_block *sb)
{
	...
	cld = do_config_log_add(obd, PARAMS_FILENAME, CONFIG_T_PARAMS, &lcfg, sb);
                          ...
                          struct super_block *sb)
{
	...
	if (IS_ERR(params_cld)) {
		CERROR("can't create params log\n");
		GOTO(out_err1, rc = PTR_ERR(params_cld));
	}
	...
	while (1) {
		...
		cfs_list_for_each_entry(cld, &config_llog_list,
			...
			spin_unlock(&config_list_lock);
                        ...
                        cld->cld_lostlock = 0;
			if (likely(!stopped)) {
				/*requeue params (param2) llog here*/
				do_requeue(cld);
				if (cld->cld_params)
					do_requeue(cld->cld_params);
			}
...
static int mgc_process_config(struct obd_device *obd, obd_count len, void *buf)
{
		...
		if (rc == 0 && cld->cld_params != NULL) {
			...
			if (rc == -ENOENT) {
				CDEBUG(D_MGC, "There is no params config file yet\n");
				rc = 0;
			}
			/* params log is optional */
			if (rc)
				CERROR("Cannot process params llog %d\n", rc);
		}
                config_log_put(cld);
