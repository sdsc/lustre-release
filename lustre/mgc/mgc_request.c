...
static int mgc_requeue_thread(void *data)
{
	int rc = 0;
	bool first = true;
	cfs_time_t check_non_nm_time = 0;
	ENTRY;
	...
	while (1) {
		...
		int to;
		bool nodemap_active = nodemap_is_active();
		bool is_nodemap_pass = nodemap_active &&
				       cfs_time_current() < check_non_nm_time;
		...
		/* For non-nodemap locks or if nodemap is disabled, wait a few
		   seconds to allow the server who caused the lock revocation
		   to finish its setup, plus some random so everyone doesn't
		   try to reconnect at once. */
		to = msecs_to_jiffies(MGC_TIMEOUT_MIN_SECONDS * MSEC_PER_SEC);
		/* rand is centi-seconds */
		to += msecs_to_jiffies(rand * MSEC_PER_SEC / 100);

		/* if nodemap is disabled, use a timer */
		if (!nodemap_active) {
			lwi = LWI_TIMEOUT(to, NULL, NULL);
			l_wait_event(rq_waitq,
				     rq_state & (RQ_STOP | RQ_PRECLEANUP),
				     &lwi);
		}

		/*
		 * if the nodemap subsystem is active, we want to check for
		 * those locks more often than others, so we don't sleep and
		 * check non-nodemap locks only after a certain time has
		 * passed.
		 */
		if (nodemap_active && !is_nodemap_pass)
			check_non_nm_time = cfs_time_current() + to;
		...
		list_for_each_entry(cld, &config_llog_list,
			...
			if (!cld->cld_lostlock)
				continue;
			if (is_nodemap_pass && !cld_is_nodemap(cld))
				continue;
