                                  ...
                                  void *data, int datalen)
{
        ...
        while (datalen > 0) {
                ...
                if (entry->mne_nid_type != 0) /* mbz, no need to swap now */
                        break;
                ...
                *strrchr(obdname, '-') = 0;
                buflen = strlen(obdname);
                buflen += sprintf(obdname + buflen, "-%s%04x",
                                  is_ost ? "OST" : "MDT", entry->mne_index);

                cname = is_ost ? "osc" : "mdc",
                buflen += sprintf(obdname + buflen, "-%s-%s",
                                  cname, cfg->cfg_instance);
