...
/* fake functions are in mdt_hsm_cdt_client.c, will be removed with LU-3343 */
...
int mdt_cdt_add_hal(struct mdt_thread_info *mti,
		    struct hsm_action_list *hal, struct obd_uuid *uuid)
{
	return 0;
}
int mdt_cdt_update_request_state(struct mdt_thread_info *mti,
 ...
 * \retval -ve failure
 */
int mdt_hsm_agent_register(struct mdt_thread_info *mti, struct obd_uuid *uuid,
			   int count, __u32 *archive_id)
{
	struct coordinator	*cdt = &mti->mti_mdt->mdt_coordinator;
	cfs_list_t		*pos, *tmp;
	struct hsm_agent	*ha = NULL;
	int			 rc;
	ENTRY;
	...
	ha->ha_uuid = *uuid;
	ha->ha_archive_cnt = count;
	if (ha->ha_archive_cnt == 0) {
		ha->ha_archive_id = NULL;
	} else {
		rc = ha->ha_archive_cnt * sizeof(*ha->ha_archive_id);
		OBD_ALLOC(ha->ha_archive_id, rc);
		if (!ha->ha_archive_id)
			GOTO(free, rc = -ENOMEM);
		memcpy(ha->ha_archive_id, archive_id, rc);
	}
	cfs_atomic_set(&ha->ha_requests, 0);
	cfs_atomic_set(&ha->ha_success, 0);
	cfs_atomic_set(&ha->ha_failure, 0);
				...
				struct obd_uuid *uuid, __u32 archive_mask)
{
	...
	if (count != 0) {
		sz = count * sizeof(*archive_id);
		OBD_ALLOC(archive_id, sz);
		if (!archive_id)
			RETURN(-ENOMEM);
		...
		for (i = 0; i < sizeof(archive_mask) * 8; i++) {
			if ((1 << i) & archive_mask) {
				archive_id[count] = i + 1;
				count++;
			}
		}
	}

	rc = mdt_hsm_agent_register(mti, uuid, count, archive_id);

	if (sz)
		OBD_FREE(archive_id, sz);
 ...
 * \retval -ve failure
 */
int mdt_hsm_agent_unregister(struct mdt_thread_info *mti, struct obd_uuid *uuid)
{
	struct coordinator	*cdt = &mti->mti_mdt->mdt_coordinator;
	cfs_list_t		*pos, *tmp;
	struct hsm_agent	*ha = NULL;
...
out:
	...
	CDEBUG(D_HSM, "agent %s unregistered: %d\n", obd_uuid2str(uuid), rc);
	...
	cfs_list_for_each_safe(pos, tmp, &cdt->cdt_agents) {
		...
		if (memcmp(&ha->ha_uuid, uuid, sizeof(*uuid)) == 0) {
			if ((succ_rq == 0) && (fail_rq == 0) && (new_rq == 0)) {
				...
				cfs_atomic_add(succ_rq, &ha->ha_success);
				cfs_atomic_add(fail_rq, &ha->ha_failure);
				cfs_atomic_add(new_rq, &ha->ha_requests);
				cfs_atomic_sub(succ_rq, &ha->ha_requests);
				cfs_atomic_sub(fail_rq, &ha->ha_requests);
			}
			GOTO(out, rc = 0);
		}
			    ...
			    struct obd_uuid *uuid)
{
	...
	cfs_list_for_each_safe(pos, tmp, &cdt->cdt_agents) {
		...
		if ((load == -1) ||
		    (load > cfs_atomic_read(&ha->ha_requests))) {
			load = cfs_atomic_read(&ha->ha_requests);
			*uuid = ha->ha_uuid;
			rc = 0;
		}
		if (cfs_atomic_read(&ha->ha_requests) == 0)
			break;
	}
	up_read(&cdt->cdt_agent_lock);
...
int mdt_hsm_agent_send(struct mdt_thread_info *mti,
		       struct hsm_action_list *hal, int purge)
{
	...
	for (i = 0 ; i < hal->hal_count ; i++) {
		if (hai->hai_action != HSMA_CANCEL) {
			...
			obj = mdt_cdt_get_md_hsm(mti, &hai->hai_fid, &hsm,
						 NULL);
			...
			if (IS_ERR(obj))
				GOTO(free, rc = PTR_ERR(obj));

			if (!mdt_cdt_check_action_compat(hai,
							 hal->hal_archive_id,
							 hal->hal_flags,
							 &hsm)) {
	 ...
	 */
	if (!purge) {
		/* set registered even if failure because we may have
		 * partial work done */
		registered = 1;
		rc = mdt_cdt_add_hal(mti, hal, &uuid);
		if (rc)
			GOTO(free, rc);
	}
	...
	if (exp == NULL) {
		...
		CERROR("%s: agent uuid (%s) not found, unregistering\n",
		       mdt_obd_name(mdt), obd_uuid2str(&uuid));
	...
	if (rc)
		CERROR("%s: Cannot send request to agent (%s), err=%d\n",
			       ...
			       struct hsm_progress_kernel *pgs)
{
	...
	rc = mdt_cdt_update_request_state(mti, pgs, 1);
	RETURN(rc);
}
 ...
 */
static void *mdt_hsm_agent_proc_start(struct seq_file *s, loff_t *p)
{
...
static int mdt_hsm_agent_proc_show(struct seq_file *s, void *v)
{
	...
	seq_printf(s, ") r=%d s=%d f=%d\n",
		   cfs_atomic_read(&ha->ha_requests),
		   cfs_atomic_read(&ha->ha_success),
		   cfs_atomic_read(&ha->ha_failure));
	RETURN(0);
}
