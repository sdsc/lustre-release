...
int mdt_hsm_check_action_compat(struct hsm_action_item *hai, int hal_an,
...
int mdt_hsm_agent_register(struct mdt_thread_info *mti,
			   ...
			   __u32 *archive_id, int nr_archives)
{
	...
	cfs_list_for_each_safe(pos, tmp, &cdt->cdt_agents) {
		...
		if (obd_uuid_equals(&ha->ha_uuid, uuid) == 0) {
				...
				const struct obd_uuid *uuid, __u32 archive_mask)
{
	...
	rc = mdt_hsm_agent_register(mti, uuid, archive_id, nr_archives);
		       ...
		       struct hsm_action_list *hal, bool purge)
{
	...
	for (i = 0; i < hal->hal_count; i++, hai_next(hai)) {
		if (hai->hai_action != HSMA_CANCEL) {
			...
			if (!mdt_hsm_check_action_compat(hai,
							 hal->hal_archive_id,
							 hal->hal_flags,
							 &hsm)) {
