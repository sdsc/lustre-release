...
int out_tx_end(const struct lu_env *env, struct thandle_exec_args *th)
{
	...
	rc = out_trans_start(env, th->ta_dev, th->ta_handle);
	if (unlikely(rc))
		GOTO(stop, rc);
...
static struct dt_object *out_object_find(const struct lu_env *env,
					 struct dt_device *dt,
					 struct lu_fid *fid)
{
	struct lu_object	*lu_obj;
	struct lu_object	*dt_lu_obj;

	lu_obj = lu_object_find(env, &dt->dd_lu_dev, fid, NULL);
	if (IS_ERR(lu_obj))
		return (struct dt_object *)lu_obj;

	dt_lu_obj = lu_object_locate(lu_obj->lo_header, dt->dd_lu_dev.ld_type);
	if (IS_ERR(dt_lu_obj)) {
		lu_object_put(env, lu_obj);
		return ERR_PTR(-ENOENT);
	}

	return lu2dt_obj(dt_lu_obj);
}
...
int out_handle(struct out_thread_info *info)
{
	...
	for (i = 0; i < count; i++) {
		...
		dt_obj = out_object_find(env, dt, &update->u_fid);
		if (IS_ERR(dt_obj))
