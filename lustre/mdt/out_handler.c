...
static int __out_tx_create(struct mdt_thread_info *, struct lu_object *,
...
static int __out_tx_attr_set(struct mdt_thread_info *, struct lu_object *,
...
static int __out_tx_xattr_set(struct mdt_thread_info *, struct lu_object *,
			      const struct lu_buf *, const char *name,
			      int flags, struct thandle_exec_args *,
			      struct update_reply *, int, char *, int);
#define out_tx_xattr_set(info, obj, buf, name, fl, th, reply, idx)	\
	__out_tx_xattr_set(info, obj, buf, name, fl, th, reply, idx,	\
			   __FILE__, __LINE__)

static int __out_tx_ref_add(struct mdt_thread_info *, struct lu_object *,
			    struct thandle_exec_args *, struct update_reply *,
			    int, char *, int);
#define out_tx_ref_add(info, obj, th, reply, idx) \
	__out_tx_ref_add(info, obj, th, reply, idx, __FILE__, __LINE__)

static int __out_tx_ref_del(struct mdt_thread_info *, struct lu_object *,
			    struct thandle_exec_args *, struct update_reply *,
			    int, char *, int);
#define out_tx_ref_del(info, obj, th, reply, idx) \
	__out_tx_ref_del(info, obj, th, reply, idx, __FILE__, __LINE__)

static int __out_tx_index_insert(struct mdt_thread_info *, struct lu_object *,
				 char *, struct lu_fid *,
...
static int __out_tx_index_delete(struct mdt_thread_info *, struct lu_object *,
			  ...
			  struct thandle_exec_args *th, int err)
{
	...
	for (i = 0; i < th->ta_argno; i++) {
		if (th->ta_args[i].object != NULL) {
			lu_object_put(env, th->ta_args[i].object);
...
static void out_object_put(struct mdt_thread_info *info,
			   struct dt_object *dt_obj)
{
		       ...
		       struct tx_arg *arg)
{
	...
	lu_attr_to_obdo(obdo, &arg->u.create.attr);
	lustre_set_wire_obdo(obdo, obdo);
	obdo_cpu_to_le(obdo, obdo);
	update_insert_reply(arg->reply, obdo, sizeof(*obdo), arg->index, rc);

	return rc;
}

static int __out_tx_create(struct mdt_thread_info *info, struct lu_object *obj,
			   ...
			   int index, char *file, int line)
{
	...
	th->ta_err = dt_declare_create(info->mti_env, dt_obj, attr, NULL, dof,
				       th->ta_handle);
	if (th->ta_err != 0)
		return th->ta_err;
...
static int out_create(struct mdt_thread_info *info)
{
	...
	if (S_ISDIR(attr->la_mode)) {
		...
		fid1 = update_param_buf(update, 1, &size);
		if (fid1 == NULL || size != sizeof(*fid1)) {
	...
	rc = out_tx_create(info, obj, attr, fid1, dof, &info->mti_handle,
				...
				struct thandle *th, struct tx_arg *arg)
{
	CERROR("%s: attr set undo "DFID" unimplemented yet!: rc = %d\n",
	       mdt_obd_name(info->mti_mdt),
	       PFID(&arg->object->lo_header->loh_fid), -ENOTSUPP);
...
static int out_attr_get(struct mdt_thread_info *info)
{
	...
	dt_obj = out_object_find(info, fid);
	if (IS_ERR(dt_obj))
		RETURN(PTR_ERR(dt_obj));

	dt_read_lock(info->mti_env, dt_obj, MOR_TGT_CHILD);
	...
	lu_attr_to_obdo(obdo, la);
	obdo_cpu_to_le(obdo, obdo);
	lustre_set_wire_obdo(obdo, obdo);

out_unlock:
	dt_read_unlock(info->mti_env, dt_obj);
	out_object_put(info, dt_obj);
...
static int out_index_lookup(struct mdt_thread_info *info)
{
	...
	dt_read_lock(info->mti_env, dt_obj, MOR_TGT_CHILD);
	if (!dt_try_as_dir(env, dt_obj))
		GOTO(out_unlock_put, rc = -ENOTDIR);
...
static int __out_tx_xattr_set(struct mdt_thread_info *info,
			      struct lu_object *obj, const struct lu_buf *buf,
				    ...
				    struct thandle *th, struct tx_arg *arg)
{
	...
	if (dt_try_as_dir(info->mti_env, dt_obj) == 0) {
		CERROR("%s: "DFID" is not directory: rc = %d\n",
		       mdt_obd_name(info->mti_mdt),
		       PFID(lu_object_fid(arg->object)), -ENOTDIR);
		return -ENOTDIR;
	}
...
static int out_index_insert(struct mdt_thread_info *info)
{
	...
	if (!fid_is_sane(fid1)) {
		CERROR("%s: invalid FID "DFID": rc = %d\n",
		       mdt_obd_name(info->mti_mdt), PFID(fid1),
		       -EPROTO);
		RETURN(-EPROTO);
	}
	...
	rc = out_tx_index_insert(info, obj, name, fid1, &info->mti_handle,
				 info->mti_u.update.mti_update_reply,
				 info->mti_u.update.mti_update_reply_index);
	if (rc != 0)
		GOTO(out_put, rc);
 ...
 * dis-assemplied into object updates in master MDD layer, so out
