	...
	*ibits = 0;
		...
		*ibits = MDS_INODELOCK_OPEN;
		...
		if (open_flags & MDS_OPEN_LOCK) {
			if (open_flags & FMODE_WRITE)
				lm = LCK_CW;
			/* If the file is released then we can't deny
			 * write because we must restore (write) it to
			 * access it. */
	...
	if (rc == 0 && acq_lease) {
		...
		CDEBUG(D_INODE, "acq_lease "DFID": openers: %d, want: %d\n",
			PFID(mdt_object_fid(obj)),
			atomic_read(&obj->mot_open_count), opencnt);

		if (atomic_read(&obj->mot_open_count) > opencnt)
			GOTO(out, rc = -EBUSY);
	}
	GOTO(out, rc);
...
int mdt_reint_open(struct mdt_thread_info *info, struct mdt_lock_handle *lhc)
{
	...
	if (!req_is_replay(req)) {
		...
		if (rc != 0)
			GOTO(out_unlock_child, result = rc);
	...
	if (rc) {
        ...
        EXIT;
out_unlock_child:
	/* Because of mot_open_sem we should match calls to
	 * mdt_object_open_lock() and mdt_object_open_unlock(), except
	 * that neither function modifies mot_open_sem during
	 * replay. So this is OK, at least with respect to
	 * mot_open_sem, probably. */
