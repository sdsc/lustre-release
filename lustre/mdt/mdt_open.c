                          ...
                          struct mdt_lock_handle *lhc)
{
        ...
        if (mdt_get_disposition(ldlm_rep, DISP_OPEN_CREATE)) {
                 ...
                 */
                parent = mdt_object_find(env, mdt, rr->rr_fid1, 1);
                if (IS_ERR(parent)) {
                        rc = PTR_ERR(parent);
                ...
                child = mdt_object_find(env, mdt, rr->rr_fid2, 0);
                if (IS_ERR(child)) {
                        rc = PTR_ERR(child);
                           ...
                           struct ldlm_reply *rep)
{
        ...
        o = mdt_object_find(info->mti_env, info->mti_mdt, rr->rr_fid2, 1);
        if (IS_ERR(o))
                RETURN(PTR_ERR(o));
                                ...
                                struct mdt_lock_handle *lhc)
{
        ...
        o = mdt_object_find(env, mdt, rr->rr_fid2, 1);
        if (IS_ERR(o)) {
                if (PTR_ERR(o) == -ENOENT) {
        ...
        if (ma->ma_valid & MA_PFID) {
                parent = mdt_object_find(env, mdt, &ma->ma_pfid, 1);
                if (IS_ERR(parent)) {
                        CDEBUG(D_INODE, "Fail to find parent "DFID
                               " for anonymous created %ld, try to"
                               " use system default.\n",
                          ...
                          struct ldlm_reply *rep, __u32 flags)
{
        ...
        o = mdt_object_find(info->mti_env, info->mti_mdt, fid, 0);
        if (IS_ERR(o))
                RETURN(rc = PTR_ERR(o));
...
int mdt_reint_open(struct mdt_thread_info *info, struct mdt_lock_handle *lhc)
{
        ...
        child = mdt_object_find(info->mti_env, mdt, child_fid, 0);
        if (IS_ERR(child))
