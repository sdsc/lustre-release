				  ...
				  struct mdt_object *o, struct md_attr *ma)
{
	...
	rc = mo_permission(info->mti_env, NULL, mdt_object_child(o), NULL,
			   MAY_WRITE);
	if (rc < 0)
		GOTO(put, rc);
	...
	rc = mdt_object_lock(info, o, lh1, MDS_INODELOCK_LAYOUT |
			     MDS_INODELOCK_XATTR, MDT_LOCAL_LOCK);
	if (rc < 0)
...
unlock1:
	mdt_object_unlock(info, o, lh1, 1);
...
int mdt_mfd_close(struct mdt_thread_info *info, struct mdt_file_data *mfd)
{
	...
	if (ma->ma_attr_flags & MDS_CLOSE_LAYOUT_SWAP) {
		...
		if (rc < 0) {
			CDEBUG(D_INODE, "%s: File " DFID
			       " swap layouts failed: %d\n",
			       mdt_obd_name(info->mti_mdt),
			       PFID(mdt_object_fid(o)), rc);
