			    ...
			    struct mdt_object *obj, __u64 ioepoch, bool enable)
{
	...
	if (enable) {
		...
		attrs->som_ioepoch = cpu_to_le64(ioepoch);
		attrs->som_mountid = cpu_to_le64(obd->u.obt.obt_mount_count);
		if ((la->la_valid & LA_SIZE) != 0)
			attrs->som_size = cpu_to_le64(la->la_size);
		if ((la->la_valid & LA_BLOCKS) != 0)
			attrs->som_blocks = cpu_to_le64(la->la_blocks);
