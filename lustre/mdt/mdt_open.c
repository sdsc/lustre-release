...
mdt_object_open_unlock(struct mdt_thread_info *info, struct mdt_object *obj,
		       ...
		       enum oo_lock_status oo_lk, int rc);
...
static int mdt_object_open_lock(struct mdt_thread_info *info,
				struct mdt_object *obj,
				struct mdt_lock_handle *lhc,
				__u64 *ibits, enum oo_lock_status *oo_lk)
{
	struct md_attr	*ma = &info->mti_attr;
...
out:
	...
	return 0;
failed:
	mdt_object_open_unlock(info, obj, lhc, *ibits, *oo_lk, rc);
	return rc;
}
			 ...
			 struct mdt_lock_handle *lhc)
{
        ...
        struct mdt_object       *o;
        int                      rc;
	enum oo_lock_status	 oo_lk = OO_NO_LOCK;
	__u64			 ibits = 0;
	ENTRY;
	...
	rc = mdt_object_open_lock(info, o, lhc, &ibits, &oo_lk);
	if (rc != 0)
		GOTO(out, rc);
	...
	mdt_object_open_unlock(info, o, lhc, ibits, oo_lk, rc);
	EXIT;
out:
...
int mdt_reint_open(struct mdt_thread_info *info, struct mdt_lock_handle *lhc)
{
	...
	rc = mdt_object_open_lock(info, child, lhc, &ibits, &oo_lk);
	if (rc != 0)
		GOTO(out_child, result = rc);
	/* Try to open it now. */
	...
	EXIT;
out_child_unlock:
	mdt_object_open_unlock(info, child, lhc, ibits, oo_lk, result);
out_child:
