                        ...
                        struct mdt_object *o, __u64 flags, int created)
{
		...
		atomic_inc(&o->mot_opencount);
		if (flags & MDS_OPEN_LEASE)
			atomic_inc(&o->mot_leasecount);
				...
				__u64 *ibits)
{
	...
	*ibits = 0;
	...
	if (acq_lease) {
		...
		if (atomic_read(&obj->mot_leasecount) > 0) {
		...
		if (!(open_flags & MDS_OPEN_LOCK)) {
			CWARN("Request lease for file:"DFID ", but open lock "
		...
		*ibits = MDS_INODELOCK_OPEN;
		...
		} else if (atomic_read(&obj->mot_leasecount) > 0) {
			if (open_flags & FMODE_WRITE)
				lm = LCK_CW;
			else
				lm = LCK_CR;
		...
		CDEBUG(D_INODE, "normal open: lease count: %d, lm: %d\n",
			atomic_read(&obj->mot_opencount), lm);
	}
	...
	/* Check if there is any other opening handle after acquiring
	 * open lock, this is to revoke caching open locks.
	 * XXX: Now only exclusive open is supported. Need to check the
	 * type of open for generic lease support. */
	if (acq_lease && !req_is_replay(mdt_info_req(info))) {
		struct mdt_file_data *mfd;
		int opencnt = 0;
		...
		mfd = mdt_handle2mfd(info, info->mti_rr.rr_handle);
		if (mfd != NULL)
			++opencnt;

		CDEBUG(D_INODE, "acq_lease: current openers: %d, want: %d\n",
			atomic_read(&obj->mot_opencount), opencnt);

		if (atomic_read(&obj->mot_opencount) > opencnt)
			GOTO(out, rc = -EBUSY);
	}
	rc = 0;
...
int mdt_mfd_close(struct mdt_thread_info *info, struct mdt_file_data *mfd)
{
	...
	if ((ma->ma_attr_flags & MDS_DATA_MODIFIED) ||
		...
		if (mode & MDS_OPEN_LEASE) {
			LASSERT(atomic_read(&o->mot_leasecount) > 0);
			atomic_dec(&o->mot_leasecount);
		}
		LASSERT(atomic_read(&o->mot_opencount) > 0);
		atomic_dec(&o->mot_opencount);
