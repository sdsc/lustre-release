                                ...
                                struct mdt_lock_handle *lhc)
{
        ...
        if (md_should_create(flags)) {
                if (!lu_fid_eq(rr->rr_fid1, rr->rr_fid2)) {
                        p = mdt_object_find(env, mdt, rr->rr_fid1);
                        if (IS_ERR(p)) {
                                LCONSOLE_WARN("Fail to find parent "DFID" for "
                                              "anonymous created %ld, try to "
                                              "use server-side parent.\n",
                                              PFID(rr->rr_fid1), PTR_ERR(p));
                                p = NULL;
                        }
                }
                if (p == NULL)
        ...
        if (ma->ma_valid & MA_PFID) {
                p = mdt_object_find(env, mdt, &ma->ma_pfid);
                if (IS_ERR(p)) {
                        LCONSOLE_WARN("Fail to find parent "DFID" for "
                                      "anonymous created %ld, try to "
                                      "use system default.\n",
                                      PFID(&ma->ma_pfid), PTR_ERR(p));
                        p = NULL;
                }
        }
