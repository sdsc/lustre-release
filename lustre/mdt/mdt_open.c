                                     ...
                                     const struct lustre_handle *handle)
{
	...
	if (mfd == NULL &&
	    (req_is_replay(req) ||
	      ...
	      * @handle contains open handle in that case. In recovery, open
	      * REQ will be replayed and the lease REQ may be resent which will
	      * bring a stale open handle, so we need to fix it up here. */
					  ...
					  struct md_attr *attr, int mode)
{
	...
	spec->sp_cr_flags = MDS_CREATE_VOLATILE | mode;
	...
	if (rc == 0) {
		...
		if (rc < 0) {
			int rc2;
			rc2 = mdo_unlink(env, mdt_object_child(local_root),
					 mdt_object_child(obj), lname,
					 &info->mti_attr, 0);
			if (rc2 < 0)
				CERROR("%s: Unlink volatile "DFID" error %d\n",
					mdt_obd_name(mdt), PFID(fid), rc2);
		}
	}
	EXIT;
			   ...
			   struct md_attr *ma)
{
	...
	/* Why do you want to release an empty file? */
	if (!(ma->ma_valid & MA_LOV))
		GOTO(out, rc = -ENODATA);
	...
	rc = le32_to_cpu(ma->ma_lmm->lmm_pattern) | LOV_PATTERN_F_RELEASED;
	ma->ma_lmm->lmm_pattern = cpu_to_le32(rc);
