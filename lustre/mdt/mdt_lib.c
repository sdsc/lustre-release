                           ...
                           const struct md_attr *ma)
{
	...
	/* For last unlink on a file, HSM attrs have been fetched.
	 * If an archived file and raolu policy has been enabled, send a
	 * remove request to archive.
	 * CDT must be running.
	 */
	if ((ma->ma_valid & MA_HSM) && (ma->ma_hsm.mh_flags & HS_EXISTS) &&
	    (cdt->mdt_coordinator.cdt_state != CDT_STOPPED) &&
	    (cdt->mdt_coordinator.cdt_raolu == true)) {
