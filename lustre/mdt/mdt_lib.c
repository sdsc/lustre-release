			  ...
			  struct mdt_body *body)
{
	...
	/*
	printk("ucred: %s nodemap: %s\n", obd_export_nid2str(info->mti_exp),
					  nodemap->nm_name);
	printk("ucred: %u %u\n", body->uid, body->gid);
	*/
...
static int old_init_ucred_reint(struct mdt_thread_info *info)
{
	...
	/*
	printk("ucred_reint: %s nodemap: %s\n",
		obd_export_nid2str(info->mti_exp), nodemap->nm_name);
	printk("ucred_reint: %u %u\n", uc->uc_fsuid, uc->uc_fsgid);
	*/
