...
int mdt_unpack_name(struct req_capsule *pill,
		    ...
		    struct lu_name *name)
{
	name->ln_name = req_capsule_client_get(pill, field);
	name->ln_namelen = req_capsule_get_size(pill, field, RCL_CLIENT) - 1;

	if (!lu_name_is_valid(name)) {
		name->ln_name = NULL;
		name->ln_namelen = 0;
...
static int mdt_create_unpack(struct mdt_thread_info *info)
{
	...
	rc = mdt_unpack_name(pill, &RMF_NAME, &rr->rr_name);
	if (rc < 0)
		RETURN(rc);
...
static int mdt_rename_unpack(struct mdt_thread_info *info)
{
	...
	rc = mdt_unpack_name(pill, &RMF_SYMTGT, &rr->rr_tgt_name);
	if (rc < 0)
		RETURN(rc);
...
static int mdt_open_unpack(struct mdt_thread_info *info)
{
	...
	mdt_unpack_name(pill, &RMF_NAME, &rr->rr_name);
