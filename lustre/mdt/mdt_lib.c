...
static int mdt_setattr_unpack_rec(struct mdt_thread_info *info)
{
	...
	if (rec->sa_bias & MDS_EXCLUSIVE_CLOSE)
		ma->ma_attr_flags |= MDS_EXCLUSIVE_CLOSE;
	else
		ma->ma_attr_flags &= ~MDS_EXCLUSIVE_CLOSE;
...
static int mdt_extended_close_unpack(struct mdt_thread_info *info)
{
	struct md_attr          *ma = &info->mti_attr;
	struct req_capsule      *pill = info->mti_pill;
	int			 flags = ma->ma_attr_flags;
	ENTRY;

	if (!(flags & MDS_HSM_RELEASE) && !(flags & MDS_EXCLUSIVE_CLOSE))
		RETURN(0);

	req_capsule_extend(pill, &RQF_MDS_EXTENDED_CLOSE);
...
int mdt_close_unpack(struct mdt_thread_info *info)
{
	...
	rc = mdt_extended_close_unpack(info);
	if (rc)
		RETURN(rc);
