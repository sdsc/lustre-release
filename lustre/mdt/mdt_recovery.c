...
static void mdt_steal_ack_locks(struct ptlrpc_request *req)
{
	...
	spin_lock(&exp->exp_lock);
		...
		/* do not drop reply data until real REP-ACK */
		oldrep->rs_xid = 0;
...
__u32 mdt_req_from_lcd(struct ptlrpc_request *req)
{
	...
	if (lustre_msg_get_opc(req->rq_reqmsg) != MDS_CLOSE &&
		...
		pre_versions[0] = lrd->lrd_pre_versions[0];
		pre_versions[1] = lrd->lrd_pre_versions[1];
		pre_versions[2] = lrd->lrd_pre_versions[2];
		pre_versions[3] = lrd->lrd_pre_versions[3];
...
{
	mdt_req_from_lcd(mdt_info_req(mti));
	return;
}
