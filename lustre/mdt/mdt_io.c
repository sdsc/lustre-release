		   ...
		   struct ldlm_resource *res)
{
	...
	mode = ldlm_lock_match(ns, LDLM_FL_BLOCK_GRANTED, &res->lr_name,
			       LDLM_IBITS, &policy, LCK_PW | LCK_EX,
			       &lockh, 0);
			    ...
			    const struct lu_fid *fid)
{
	...
	mode = ldlm_lock_match(mdt->mdt_namespace, LDLM_FL_BLOCK_GRANTED,
			       res_id, LDLM_IBITS, policy, LCK_PW, &lockh, 0);
			 ...
			 struct lustre_handle *lh)
{
	...
	CDEBUG(D_INFO, "File size "LPU64", reply size %d/%d/%d\n", mbo->mbo_size,
	...
	CDEBUG(D_INFO, "Read %i (wanted %u) bytes from "LPU64"\n", copied,
	       len, offset);
	if (copied < len)
