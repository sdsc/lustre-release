		   ...
		   struct ldlm_resource *res)
{
	...
	/* don't consider locks from disconnected exports */
	if (lock->l_export->exp_failed || lock->l_export->exp_disconnected ||
	    lock->l_export->exp_imp_reverse->imp_state == LUSTRE_IMP_CLOSED)
		GOTO(out, rc = 0);
