				   ...
				   size_t count, loff_t *off)
{
	...
	if (mdt->mdt_opts.mo_evict_everywhere) {
...
static int mdt_evict_everywhere_seq_show(struct seq_file *m, void *data)
{
	struct obd_device *obd = m->private;
	struct mdt_device *mdt = mdt_dev(obd->obd_lu_dev);

	return seq_printf(m, "%u\n", mdt->mdt_opts.mo_evict_everywhere);
}

static ssize_t
mdt_evict_everywhere_seq_write(struct file *file, const char __user *buffer,
			       size_t count, loff_t *off)
{
	struct seq_file   *m = file->private_data;
	struct obd_device *obd = m->private;
	...
	if (rc)
		return rc;
	mdt->mdt_opts.mo_evict_everywhere = !!val;
	return count;
}
LPROC_SEQ_FOPS(mdt_evict_everywhere);
...
static struct lprocfs_vars lprocfs_mdt_obd_vars[] = {
	...
	{ .name =	"evict_everywhere",
	  .fops =	&mdt_evict_everywhere_fops		},
	{ .name =	"hash_stats",
	  .fops =	&mdt_hash_fops				},
	{ .name =	"sec_level",
