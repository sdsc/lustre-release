                                       ...
                                       unsigned long count, void *data)
{
        ...
        OBD_ALLOC(tmpbuf, BUFLEN);
        if (tmpbuf == NULL)
                return -ENOMEM;

        /*
         * OBD_ALLOC() will zero tmpbuf, but we only copy BUFLEN - 1
         * bytes into tmpbuf, to ensure that the string is NUL-terminated.
         * UUID_MAX should include a trailing NUL already.
         */
        if (cfs_copy_from_user(tmpbuf, buffer,
                              ...
                              unsigned long count, void *data)
{
        ...
        /*
         * OBD_ALLOC() will zero tmpbuf, but we only copy UUID_MAX - 1
         * bytes into tmpbuf, to ensure that the string is NUL-terminated.
         * UUID_MAX should include a trailing NUL already.
         */
        if (cfs_copy_from_user(tmpbuf, buffer,
