                                    ...
                                    unsigned long count, void *data)
{
        struct obd_device *obd = data;
        struct mdt_device *mdt = mdt_dev(obd->obd_lu_dev);
        struct identity_downcall_param idp = { { 0 } };
        int rc = 0;
        ...
        if (idp.u.idp_2x->idd_nperms > N_PERMS_MAX ||
            count < sizeof(struct identity_downcall_data)) {
                ...
                if (count < offsetof(struct mds_grp_downcall_data,
                                     mgd_groups[idp.u.idp_18->mgd_ngroups])) {
                        CERROR("%s: identity downcall failed for incompete "
                               "data\n", obd->obd_name);
                        GOTO(out, rc = -EINVAL);
                }
        } else {
                ...
                if (count < offsetof(struct identity_downcall_data,
                                     idd_groups[idp.u.idp_2x->idd_ngroups])) {
                        CERROR("%s: identity downcall failed for incompete "
                               "data\n", obd->obd_name);
                        GOTO(out, rc = -EINVAL);
                }
