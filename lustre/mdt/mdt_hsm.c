...
int mdt_hsm_state_set(struct tgt_session_info *tsi)
{
	...
	if ((hss->hss_setmask | hss->hss_clearmask) & ~HSM_FLAGS_MASK) {
		CDEBUG(D_HSM, "Incompatible masks provided (set "LPX64
		       ", clear "LPX64") vs supported set ("LPX64").\n",
	...
	if (((hss->hss_setmask | hss->hss_clearmask) & ~HSM_USER_MASK) &&
		...
		CDEBUG(D_HSM, "Incompatible masks provided (set "LPX64
		       ", clear "LPX64") vs unprivileged set ("LPX64").\n",
	...
	if (hss->hss_valid & HSS_ARCHIVE_ID) {
		...
		if (hss->hss_archive_id > MAX_ARCHIVE_CNT) {
			CDEBUG(D_HSM, "archive id %u exceeds maximum %u.\n",
