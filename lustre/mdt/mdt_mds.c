...
DEF_MDT_HNDL_F(HABEO_REFERO,		GETSTATUS,	mdt_getstatus),
DEF_MDT_HNDL_F(HABEO_CORPUS,		GETATTR,	mdt_getattr),
DEF_MDT_HNDL_F(HABEO_CORPUS|HABEO_REFERO,	GETATTR_NAME,
							mdt_getattr_name),
DEF_MDT_HNDL_F(HABEO_CORPUS,		GETXATTR,	mdt_getxattr),
DEF_MDT_HNDL_F(HABEO_REFERO,		STATFS,		mdt_statfs),
DEF_MDT_HNDL_F(MUTABOR,			REINT,		mdt_reint),
DEF_MDT_HNDL_F(HABEO_CORPUS,		CLOSE,		mdt_close),
DEF_MDT_HNDL_F(HABEO_CORPUS,		DONE_WRITING,	mdt_done_writing),
DEF_MDT_HNDL_F(HABEO_REFERO,		PIN,		mdt_pin),
DEF_MDT_HNDL_0(0,			SYNC,		mdt_sync),
DEF_MDT_HNDL_F(HABEO_CORPUS|HABEO_REFERO,	IS_SUBDIR,
							mdt_is_subdir),
#ifdef HAVE_QUOTA_SUPPORT
...
struct mdt_handler mdt_readpage_ops[] = {
	DEF_MDT_HNDL_F(0,			CONNECT,  mdt_connect),
...
struct mdt_opc_slice mdt_readpage_handlers[] = {
	{
		.mos_opc_start	= MDS_GETATTR,
		.mos_opc_end	= MDS_LAST_OPC,
...
static int mds_dtss_handle(struct ptlrpc_request *req)
{
	return mdt_handle_common(req, mdt_seq_handlers);
}
...
static int mds_start_ptlrpc_service(struct mds_device *m)
{
	...
	conf = (typeof(conf)) {
		...
		.psc_thr		= {
			.tc_thr_name		= LUSTRE_MDT_NAME "_mdsc",
	...
	conf = (typeof(conf)) {
		...
		.psc_thr		= {
			.tc_thr_name		= LUSTRE_MDT_NAME "_mdss",
			.tc_nthrs_init		= MDS_OTHR_NTHRS_INIT,
			.tc_nthrs_max		= MDS_OTHR_NTHRS_MAX,
	...
	conf = (typeof(conf)) {
		...
		.psc_thr		= {
			.tc_thr_name		= LUSTRE_MDT_NAME "_dtss",
			.tc_nthrs_init		= MDS_OTHR_NTHRS_INIT,
			.tc_nthrs_max		= MDS_OTHR_NTHRS_MAX,
			.tc_ctx_tags		= LCT_MD_THREAD | LCT_DT_THREAD
		},
		.psc_ops		= {
			.so_req_handler		= mds_dtss_handle,
			.so_req_printer		= target_print_req,
			.so_hpreq_handler	= NULL,
		},
	};
