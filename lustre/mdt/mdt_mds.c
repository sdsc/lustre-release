...
#define DEF_TARGET_HDL DEF_MDT_HDL
#define out_connect mdt_connect
#define out_disconnect mdt_disconnect
static struct target_handler out_ops[] = {
	DEF_TARGET_HDL(0,		MDS_CONNECT,	out_connect),
	DEF_TARGET_HDL(0,		MDS_DISCONNECT,	out_disconnect),
	DEF_TARGET_HDL(MUTABOR,		MDS_OBJ_UPDATE,	out_handle),
};

static struct mdt_opc_slice obj_update_handlers[] = {
	{
		.mos_opc_start = MDS_GETATTR,
		.mos_opc_end   = MDS_LAST_OPC,
		.mos_hs        = out_ops
...
{
	return mdt_handle_common(req, obj_update_handlers);
}
