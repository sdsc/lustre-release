		     ...
		     llog_cb_t cb, void *data)
{
	...
	if (lctxt->loc_handle == NULL) {
		rc = llog_open(env, lctxt, &lctxt->loc_handle, NULL,
			       HSM_ACTIONS, LLOG_OPEN_EXISTS);
		if (rc)
			GOTO(out, rc);

		rc = llog_init_handle(env, lctxt->loc_handle,
				      LLOG_F_IS_CAT, NULL);
		if (rc)
			GOTO(out, rc);
	}
			 ...
			 __u64 flags, struct hsm_action_item *hai)
{
	...
	larr->arr_hdr.lrh_type = AGENT_REC;
