...
static struct lu_kmem_descr hsm_cdt_caches[] = {
	{
...
/**
 * Find object by FID and fill in MD attributes
					  ...
					  struct mdt_lock_handle *lh)
{
	...
	/* find object by FID and lock it to get/set MD flags */
	obj = mdt_object_find_lock(mti, fid, lh, ibits);
	if (IS_ERR(obj))
		RETURN(obj);
			      ...
			      void *data)
{
	...
	case ARS_WAITING: {
		...
		if (found == -1) {
			 ...
			 * enough */
			dscb->request[empty_slot].hal_sz =
				     sizeof(*dscb->request[empty_slot].hal) +
				     cfs_size_round(MTI_NAME_MAXLEN+1) +
				     2 * cfs_size_round(larr->arr_hai.hai_len);
			OBD_ALLOC(hal, dscb->request[empty_slot].hal_sz);
			if (!hal) {
			...
			hal->hal_count = 0;
			dscb->request[empty_slot].hal_used_sz = hal_size(hal);
			dscb->request[empty_slot].hal = hal;
			dscb->request_cnt++;
			found = empty_slot;
			hai = hai_zero(hal);
		} else {
...
int mdt_hsm_cdt_init(struct mdt_device *mdt)
{
	...
	if (cdt_mti->mti_exp == NULL)
		GOTO(free, rc = -ENOMEM);

	uc = mdt_ucred(cdt_mti);
	hsm_init_ucred(uc);

	rc = lu_kmem_init(hsm_cdt_caches);
	if (rc)
		GOTO(free, rc);
				     ...
				     enum agent_req_status *status)
{
	...
	*status = ARS_WAITING;
	 ...
	 * hsm_get_md_attr()::mo_attr_get() got it from disk and
	 * it is still valid
	 */
	if (pgs->hpk_errval != 0) {
	...
	if (ma->ma_valid & MA_HSM)
		rc = mdt_hsm_attr_set(mti, obj, &ma->ma_hsm);
