			       ...
			       unsigned long count, void *data)
{
	...
	rc = 0;
	if (strncmp(kernbuf, CDT_ENABLE_CMD, strlen(CDT_ENABLE_CMD)) == 0) {
		if (cdt->cdt_state == CDT_DISABLE) {
			cdt->cdt_state = CDT_RUNNING;
			mdt_hsm_cdt_wakeup(mdt);
		} else {
	...
	} else if (strncmp(kernbuf, CDT_STOP_CMD, strlen(CDT_STOP_CMD)) == 0) {
		if ((cdt->cdt_state == CDT_STOPPING) ||
		    (cdt->cdt_state == CDT_STOPPED)) {
			CERROR("%s: Coordinator already stopped\n",
			       mdt_obd_name(mdt));
	...
	} else if (strncmp(kernbuf, CDT_DISABLE_CMD,
	...
	} else if (strncmp(kernbuf, CDT_PURGE_CMD, strlen(CDT_PURGE_CMD)) == 0) {
		rc = hsm_cancel_all_actions(mdt);
	} else if (strncmp(kernbuf, CDT_HELP_CMD, strlen(CDT_HELP_CMD)) == 0) {
		usage = 1;
	} else {
		usage = 1;
		rc = -EINVAL;
