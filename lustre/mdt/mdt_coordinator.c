...
static int mdt_coordinator(void *data)
{
	...
	const unsigned int	 wait_event_time = 1;
	unsigned long		 next_housekeeping = 0;
	int			 rc = 0;
	ENTRY;
	...
	OBD_ALLOC(hsd.request, hsd.max_requests * sizeof(*hsd.request));
	if (!hsd.request)
		GOTO(out, rc = -ENOMEM);
	...
	while (1) {
		...
		lwi = LWI_TIMEOUT(cfs_time_seconds(wait_event_time),
				  NULL, NULL);
		...
		if (!list_empty(&cdt->cdt_deferred_hals))
			mdt_hsm_process_deferred_hals(mti);

		/* If no event, and no housekeeping to do, continue to
		 * wait. */
		if (next_housekeeping <= get_seconds()) {
			next_housekeeping = get_seconds() + cdt->cdt_loop_period;
		...
		if (hsd.max_requests != cdt->cdt_max_requests) {
			 ...
			 */
			OBD_FREE(hsd.request,
				 hsd.max_requests * sizeof(*hsd.request));
			hsd.max_requests = cdt->cdt_max_requests;
			OBD_ALLOC(hsd.request,
				  hsd.max_requests * sizeof(*hsd.request));
			if (!hsd.request) {
				rc = -ENOMEM;
				break;
		...
		for (i = 0; i < hsd.max_requests; i++) {
			 ...
			 */
			hal = kuc_alloc(request->hal_used_sz,
			...
			memcpy(hal, request->hal, request->hal_used_sz);
			 ...
			 */
			hai = hai_first(hal);
			for (j = 0; j < request->hal->hal_count; j++) {
			...
			kuc_free(hal, request->hal_used_sz);
...
out:
	if (hsd.request)
		OBD_FREE(hsd.request, hsd.max_requests * sizeof(*hsd.request));
	...
	mdt_hsm_free_deferred_hals(&cdt->cdt_deferred_hals);
...
int mdt_hsm_cdt_init(struct mdt_device *mdt)
{
	...
	spin_lock_init(&cdt->cdt_deferred_hals_lock);
