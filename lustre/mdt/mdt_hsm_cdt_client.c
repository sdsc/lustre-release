			       ...
			       struct hsm_action_list *hal)
{
	...
	rc = cdt_llog_process(env, mdt, hsm_find_compatible_cb, &dccb);
 ...
 * \retval boolean
 */
static bool hsm_check_action_needed(struct hsm_action_item *hai, int hal_an,
				   __u64 rq_flags, struct md_hsm *hsm)
{
	...
	case HSMA_RESTORE:
		 ...
		 * mdt_hsm_check_action_compat() will return an error
		 */
			...
			struct hsm_action_list *hal, __u64 *compound_id)
{
	...
	*compound_id = cfs_atomic_inc_return(&cdt->cdt_compound_id);
	...
	for (i = 0; i < hal->hal_count; i++, hai = hai_next(hai)) {
		...
		/* if file has no hsm flags it cannot be restored
		 * this is the case when we set file size when we import a file
		 */
		if (is_restore && (mh.mh_flags == 0))
			GOTO(out, rc = -EBUSY);

		/* Check if an action is needed, compare request
		 * and HSM flags status */
		if (!hsm_check_action_needed(hai, archive_id, flags, &mh))
			continue;

		/* Check if file request is compatible with HSM flags status
		 * and stop at first incompatible
		 */
		if (!mdt_hsm_check_action_compat(hai, archive_id, flags, &mh))
			GOTO(out, rc = -EPERM);
		...
		if (hai->hai_action == HSMA_RESTORE) {
			...
			if (IS_ERR(child)) {
				rc = PTR_ERR(child);
				CERROR("%s: cannot not take layout lock for "
				       DFID" rc=%d\n", mdt_obd_name(mdt),
				       PFID(&crh->crh_fid), rc);
				OBD_FREE_PTR(crh);
				GOTO(out, rc);
