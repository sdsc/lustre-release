				      ...
				      cfs_list_t *lock_list)
{
	...
	if (S_ISDIR(lu_object_attr(&obj->mot_obj.mo_lu)))
		RETURN(0);
	...
	for (count = 0; count < ldata.ld_leh->leh_reccount; count++) {
		...
		if (rc != 0) {
			CERROR("%s: can not lock object"DFID": rc =%d\n",
			     ...
			     struct mdt_lock_handle *lhc)
{
	...
	/* 2: find & lock the migrated object. */
	lname = mdt_name(info->mti_env, (char *)rr->rr_name, rr->rr_namelen);
	...
	/* 4: For migration, create the remote migrate object first. */
	*new_fid = *rr->rr_fid2;
	*rr->rr_fid2 = *rr->rr_fid1;
	mnew = mdt_object_find(info->mti_env, info->mti_mdt, new_fid);
	if (IS_ERR(mnew))
		GOTO(out_unlock_list, rc = PTR_ERR(mnew));
	...
	rc = mdo_rename(info->mti_env, mdt_object_child(msrcdir),
			mdt_object_child(msrcdir), old_fid, &slname,
			    ...
			    struct mdt_lock_handle *lhc)
{
	...
	DEBUG_REQ(D_INODE, req, "migrate "DFID"/%s to "DFID"",
