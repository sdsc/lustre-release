						 ...
						 struct mdt_object *child)
{
	...
	if (!mdt_is_dne_client(exp)) {
		CERROR("%s: Client %s with export %s tries to create remote"
		       " directory "DFID" : rc = %d.\n",
		       mdt_obd_name(mdt), obd_uuid2str(&exp->exp_client_uuid),
		       obd_export_nid2str(exp), PFID(mdt_object_fid(child)),
		       -EPERM);
		return -EPERM;
	}
	...
	if ((S_ISDIR(attr->la_mode) && spec->u.sp_ea.eadata != NULL &&
		...
		CERROR("%s: Client %s with export %s tries to create striped"
		       " directory "DFID" : rc = %d.\n",
		       mdt_obd_name(mdt), obd_uuid2str(&exp->exp_client_uuid),
		       obd_export_nid2str(exp), PFID(mdt_object_fid(child)),
		       -EPERM);
