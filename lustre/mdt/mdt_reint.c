                             ...
                             struct lu_fid *fid, int idx)
{
        ...
        if (rc == 0) {
                ...
                child = mdt_object_find(info->mti_env, info->mti_mdt, fid, 0);
...
static int mdt_md_create(struct mdt_thread_info *info)
{
        ...
        child = mdt_object_find(info->mti_env, mdt, rr->rr_fid2, 0);
        if (likely(!IS_ERR(child))) {
...
static int mdt_md_mkobj(struct mdt_thread_info *info)
{
        ...
        o = mdt_object_find(info->mti_env, mdt, info->mti_rr.rr_fid2, 0);
        if (!IS_ERR(o)) {
                             ...
                             struct mdt_lock_handle *lhc)
{
        ...
        repbody = req_capsule_server_get(info->mti_pill, &RMF_MDT_BODY);
        mo = mdt_object_find(info->mti_env, info->mti_mdt, rr->rr_fid1, 1);
        if (IS_ERR(mo))
                            ...
                            struct mdt_lock_handle *lhc)
{
        ...
        mc = mdt_object_find(info->mti_env, info->mti_mdt, child_fid, 1);
        if (IS_ERR(mc))
                          ...
                          struct mdt_lock_handle *lhc)
{
        ...
        ms = mdt_object_find(info->mti_env, info->mti_mdt, rr->rr_fid1, 1);
        if (IS_ERR(ms))
...
static int mdt_rename_sanity(struct mdt_thread_info *info, struct lu_fid *fid)
{
        ...
        do {
                ...
                dst = mdt_object_find(info->mti_env, info->mti_mdt, &dst_fid,1);
                if (!IS_ERR(dst)) {
                            ...
                            struct mdt_lock_handle *lhc)
{
        ...
        mold = mdt_object_find(info->mti_env, info->mti_mdt, old_fid, 1);
        if (IS_ERR(mold))
        ...
        if (rc == 0) {
                ...
                mnew = mdt_object_find(info->mti_env, info->mti_mdt, new_fid,0);
                if (IS_ERR(mnew))
