...
static int mdt_md_create(struct mdt_thread_info *info)
{
	...
	/*
	 * if this is mkdir and SoC is enabled, cancel any conflicting lock
	 * on parent (even from the same client) to eliminate dependency
	 * between previous operations on parent and this mkdir, because this
	 * mkdir may be striped directory, which can be recovered from update
	 * log.
	 */
	if (mdt_soc_is_enabled(mdt) && S_ISDIR(ma->ma_attr.la_mode) &&
	    !mdt_object_remote(parent))
		rc = mdt_object_local_lock(info, parent, lh,
					   MDS_INODELOCK_UPDATE,
					   LDLM_FL_COS_INCOMPAT |
					   LDLM_FL_STRICT_COS, false);
	else
			   ...
			   struct ldlm_enqueue_info *einfo)
{
	...
	einfo->ei_cb_cb = mdt_compatible_ast;
...
relock:
	...
	if (rc != 0)
		GOTO(out_unlock, rc);
	if (mdt_modify_remote_check(info))
		GOTO(relock, rc);
...
relock:
	...
	if (mdt_object_remote(mp) &&
	    !lustre_handle_is_used(&child_lh->mlh_rreg_lh)) {
...
relock:
	rc = mdt_object_lock(info, mp, lhp, MDS_INODELOCK_UPDATE);
	if (rc != 0) {
		mdt_object_put(info->mti_env, mp);
		RETURN(rc);
	}
				   ...
				   struct mdt_object **tgtp)
{
	...
	if (*tgtp != NULL) {
		tgt = *tgtp;
	} else if (lu_fid_eq(fid_src, fid_tgt)) {
		tgt = src;
...
relock:
	...
	if (reverse) {
		...
		if (rc == 0 && mdt_modify_remote_check(info)) {
		...
		if (tgt != src) {
			rc = mdt_object_lock_save(info, tgt, lh_tgt, 1);
			if (rc == 0 && mdt_modify_remote_check(info)) {
				mdt_object_unlock(info, tgt, lh_tgt, 1);
				GOTO(relock, rc);
			}
