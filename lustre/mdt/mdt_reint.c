			   ...
			   struct ldlm_enqueue_info *einfo)
{
	...
	if (!lu_fid_eq(s0_fid, mdt_object_fid(obj))) {
		...
		*s0_objp = mdt_object_find_lock(mti, s0_fid, s0_lh, ibits);
		if (IS_ERR(*s0_objp))
			...
			struct md_attr *ma)
{
	...
	rc = mdt_reint_object_lock(info, mo, lh, lockpart, lock_slaves);
	if (rc != 0)
		RETURN(rc);
	...
	if (lock_slaves) {
		s0_lh = &info->mti_lh[MDT_LH_LOCAL];
		mdt_lock_reg_init(s0_lh, LCK_PW);
		rc = mdt_lock_slaves(info, mo, LCK_PW, lockpart, s0_fid, s0_lh,
				     &s0_obj, einfo);
		if (rc != 0)
			GOTO(out_unlock, rc);
	}
			    ...
			    struct mdt_lock_handle *lhc)
{
	...
	cos_incompat = (mdt_object_remote(mp) || lock_slaves);
				     ...
				     struct mdt_lock_handle *lhc)
{
	...
	mold = mdt_object_find(info->mti_env, info->mti_mdt, old_fid);
	if (IS_ERR(mold))
		GOTO(out_put_tgtdir, rc = PTR_ERR(mold));
	 ...
	 * lookup with version checking */
	fid_zero(new_fid);
	rc2 = mdt_lookup_version_check(info, mtgtdir, &rr->rr_tgt_name, new_fid,
				      3);
	if (rc2 == 0) {
	...
	if (rc2 == 0) {
		...
		GOTO(out_unlock_parents, rc = rc2);
	} else {
		lh_oldp = &info->mti_lh[MDT_LH_OLD];
		mdt_lock_reg_init(lh_oldp, LCK_EX);
		lock_ibits = MDS_INODELOCK_LOOKUP | MDS_INODELOCK_XATTR;
