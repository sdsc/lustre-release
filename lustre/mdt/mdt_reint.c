			     ...
			     __u64 ibits)
{
	...
	union lmv_mds_md		*lmv = ma->ma_lmv;
	ENTRY;

	if (!(ma->ma_valid & MA_LMV))
		RETURN(0);
	/* Note: lmv has been swapped before see mdt_lock_slaves */
	if (((struct lmv_mds_md_common *)lmv)->lmv_stripe_count <= 1)
		RETURN(0);
			   ...
			   ldlm_mode_t mode, __u64 ibits)
{
	...
	ma->ma_need = MA_LMV;
	ma->ma_valid = 0;
	ma->ma_lmv = (union lmv_mds_md *)mti->mti_xattr_buf;
				      ...
				      cfs_list_t *lock_list)
{
	...
	for (count = 0; count < ldata.ld_leh->leh_reccount; count++) {
		...
		if (mdt_object_remote(mdt_pobj)) {
			...
			mdt_lock_pdo_init(&mll->mll_lh, LCK_PW, name.ln_name,
					  name.ln_namelen);
			     ...
			     struct mdt_lock_handle *lhc)
{
	...
	if ((ma->ma_valid & MA_LMV)) {
		...
		lmm1 = (struct lmv_mds_md_v1 *)ma->ma_lmv;
		lmm_common = (struct lmv_mds_md_common *)ma->ma_lmv;
		if (le32_to_cpu(lmm_common->lmv_magic) != LMV_MAGIC_MIGRATE) {
		...
		fid_le_to_cpu(&lmm1->lmv_data[1], &lmm1->lmv_data[1]);
		if (!fid_is_sane(&lmm1->lmv_data[1]))
