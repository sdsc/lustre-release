...
static int mdt_md_create(struct mdt_thread_info *info)
{
	...
	/*
	 * if this is mkdir and COS is enabled, cancel any conflicting lock
	 ...
	 * mkdir may be striped directory, which can be recovered from mirrored
	 * MDS.
	 */
	if (mdt_cos_is_enabled(mdt) && S_ISDIR(ma->ma_attr.la_mode))
		rc = mdt_object_lock_strict(info, parent, lh,
	...
	child = mdt_object_new(info->mti_env, mdt, rr->rr_fid2);
		...
		if (rc == 0) {
			/*
			 * if child is directory and COS is enabled, save child
			 ...
			 * which can be recovered from mirrored MDS, but 1nd
