					  ...
					  void *args)
{
	...
	data = (struct req_interval_data *)args;
...
static void mdt_cdt_free_request_tree(struct cdt_req_progress *crp)
{
	...
	for (i = 0 ; i <= crp->crp_max / NODE_VECTOR_SZ ; i++)
		OBD_FREE(crp->crp_node[i],
			 NODE_VECTOR_SZ * sizeof(struct interval_node));

	/* free main vector */
	OBD_FREE(crp->crp_node,
		 sizeof(struct interval_node *) *
			       ...
			       struct hsm_extent *extent)
{
	...
	if (crp->crp_cnt >= crp->crp_max) {
		...
		OBD_ALLOC(v, NODE_VECTOR_SZ * sizeof(struct interval_node));
		if (v == NULL)
			GOTO(out, rc = -ENOMEM);
		...
		if (crp->crp_max == 0)
			...
			osz = sizeof(struct interval_node *) *
			      (crp->crp_max / NODE_VECTOR_SZ + 1);

		nsz = osz + sizeof(struct interval_node *);
		/* increase main vector size */
		OBD_ALLOC(new_vv, nsz);
		if (new_vv == NULL) {
			OBD_FREE(v, NODE_VECTOR_SZ);
			GOTO(out, rc = -ENOMEM);
		}
...
int mdt_cdt_add_request(struct coordinator *cdt, struct cdt_agent_req *new_car)
{
	...
	cfs_list_for_each_safe(pos, tmp, &cdt->cdt_requests) {
		...
		car = cfs_list_entry(pos, struct cdt_agent_req,
				     car_request_list);
		if (car->car_hai->hai_cookie == new_car->car_hai->hai_cookie) {
	...
	mdt_hsm_agent_statistics(cdt, 0, 0, 1, &new_car->car_uuid);
...
struct cdt_agent_req *mdt_cdt_find_request(struct coordinator *cdt,
					   __u64 cookie, struct lu_fid *fid)
{
	cfs_list_t		*pos, *tmp;
	struct cdt_agent_req	*car;
	ENTRY;
					     ...
					     struct hsm_progress_kernel *pgs)
{
	...
	if (pgs->hpk_flags & HP_FLAG_COMPLETED) {
		if (pgs->hpk_errval != 0)
			mdt_hsm_agent_statistics(cdt, 0, 1, 0, &car->car_uuid);
		else
			mdt_hsm_agent_statistics(cdt, 1, 0, 0, &car->car_uuid);
	}
	RETURN(car);
}
