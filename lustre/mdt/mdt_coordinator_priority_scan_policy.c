...
struct spo_priority_data {
	...
	int			 priority_mask;	/** cf. "batch" function */
	int			 ratio;		/** cf. "batch" function */
	int			 full_mask;	/** cf. "batch" function */
				 ...
				 struct llog_agent_req_rec *larr)
{
	...
	if (request->hal == NULL) {
		CERROR("Cannot allocate memory (%zu o) for compound "LPX64"\n",
		       hal_sz, larr->arr_compound_id);
		return -ENOMEM;
	}
...
#define oriented_for(i, start, end) \
			  ...
			  struct llog_handle *llh, struct llog_rec_hdr *hdr)
{
	...
	if (rc)
		CERROR("%s: cannot cleanup timed out request: "DFID" for cookie "LPX64" action=%s\n",
						 ...
						 struct hsm_action_list *hal)
{
	...
	if (cookies == NULL) {
		CERROR("%s: Cannot allocate memory (%d o) for cookies vector "LPX64"\n",
		       mdt_obd_name(mti->mti_mdt), sz, hal->hal_compound_id);
		return -ENOMEM;
	}
...
static int spo_priority_process_llog(struct coordinator *cdt)
{
	...
	for (i = 0; i < spd->max_requests; i++) {
		...
		rc = spo_priority_process_waiting_requests(mti, hal);
		if (rc < 0)
			break;
	}
...
static int spo_priority_ratio_seq_show(struct seq_file *m, void *v)
{
	...
	down_read(&spd->rwsem);
	seq_printf(m, "ratio\t%i%%/%i%%\t(high/low)\n", spd->ratio,
		   100 - spd->ratio);
	up_read(&spd->rwsem);
					    ...
					    size_t count, loff_t *off)
{
	...
	/* Not 100 nor 0 to avoid complete starvation on a type of request */
