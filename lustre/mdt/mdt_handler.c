				...
				struct obd_connect_data *data)
{
	...
	if (data->ocd_connect_flags & OBD_CONNECT_MAX_IN_FLIGHT) {
		spin_lock(&exp->exp_lock);
		*exp_connect_flags_ptr(exp) |= OBD_CONNECT_MAX_IN_FLIGHT;
		spin_unlock(&exp->exp_lock);
		/* XXX: how do we really control this in the runtime? */
		data->ocd_maxinflight = 6;
		exp->exp_connect_data.ocd_maxinflight = data->ocd_maxinflight;
	}
