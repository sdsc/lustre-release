...
#if 0
	/* Disable capainfo here, since initializng mdd thread info in mdt
	 * violates layer, and also after moving ptlrpc service threads to
 	 * mds layer, MDD layer might not be setup yet at this moment.
	 */
...
static int mds_start_ptlrpc_service(struct mds_device *m)
{
	...
	conf = (typeof(conf)) {
		...
		.psc_thr		= {
			...
			.tc_nthrs_default	= MDS_NTHRS_DEFAULT,
	...
	conf = (typeof(conf)) {
		...
		.psc_thr		= {
			...
			.tc_nthrs_default	= MDS_RDPG_NTHRS_DEFAULT,
	...
	conf = (typeof(conf)) {
		...
		.psc_thr		= {
			...
			.tc_nthrs_default	= MDS_SETA_NTHRS_DEFAULT,
	...
	m->mds_dtss_service = ptlrpc_register_service(&conf, procfs_entry);
	if (IS_ERR(m->mds_dtss_service)) {
		rc = PTR_ERR(m->mds_dtss_service);
...
CFS_MODULE_PARM(mdt_num_threads, "ul", ulong, 0444,
                "number of mdt service threads to start (deprecated)");
