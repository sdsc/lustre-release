...
struct mdt_object *mdt_object_find_in_cache(const struct lu_env *env,
					    struct mdt_device *d,
					    const struct lu_fid *f)
{
	struct lu_object_conf conf = { .loc_flags = LOC_F_NOALLOC };
	struct lu_object *o;
	struct mdt_object *m;
	ENTRY;
			    ...
			    void *data, int flag)
{
	...
	case LDLM_CB_CANCELING: {
		...
		if (ibits & (MDS_INODELOCK_XATTR | MDS_INODELOCK_UPDATE)) {
			...
			/* BFL doesn't have a real object, return directly */
			if (lu_fid_eq(&fid, &LUSTRE_BFL_FID))
				RETURN(0);
			...
			mo = mdt_object_find_in_cache(&env, mdt, &fid);
			if (IS_ERR(mo)) {
				CDEBUG(D_DLMTRACE,
				       "object "DFID" is not found: %ld.\n",
				       PFID(&fid), PTR_ERR(mo));
				lu_env_fini(&env);
				RETURN(PTR_ERR(mo));
			}
