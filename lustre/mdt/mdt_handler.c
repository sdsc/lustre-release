...
/**
 * Steal cross-MDT lock from ptlrpc_reply_state. Cross-MDT lock will be saved
 * into ptlrpc_reply_state in mdt_save_remote_lock(), which will be released
 * upon transaction commit, but this lock might be revoked while local
 * transaction not committed yet, and it needs to be released immediately
 * because the MDT where the resource resides shouldn't will commit transaction
 * there, so when BAST is received, steal such lock from ptlrpc_reply_state.
 *
 * \param lock cross-MDT lock
 * \param lockh lock handle of the cross-MDT lock
 * \param rs   ptlrpc_reply_state where lock is saved
 * \retval 1 lock is stolen and released
 ...
 * else if lock->l_ast_data is NULL, which means current transaction not
 * stopped and this lock not saved yet, and the lock released in
 * mdt_save_remote_lock() later.
 *
 * \param lock	the lock which blocks a request or cancelling lock
 * \param desc	unused
 * \param data	unused
			    ...
			    void *data, int flag)
{
	...
	if (lock->l_readers == 0 && lock->l_writers == 0) {
		...
		if (rc < 0)
		   ...
		   mdt_steal_cross_mdt_lock(lock, &lockh, &mode, rs)) {
		lock->l_ast_data = NULL;
		unlock_res_and_lock(lock);
		...
		/*
		 * if rs is NULL, there are two cases:
		 * 1. current transaction is not stopped and lock is not saved
		 *    in ptlrpc_reply_state yet, however CBPENDING was set for
		 *    this lock before entering this function, and later
		 *    mdt_save_remote_lock() will know, and won't save it.
		 * 2. lock has been stolen from ptlrpc_reply_state in another
		 *    BAST, nothing needs to be done here.
		 */
	}
			...
			__u64 dlmflags)
{
	...
	LDLM_DEBUG(lock, "dlmflags "LPX64, dlmflags);

	/* ignore existed COS lock if MDT COS is not enabled and request lock
	 * doesn't check COS compatibility */
	if (!mdt_cos_is_enabled(mdt) && mdt_soc_is_enabled(mdt) &&
	    lock->l_req_mode == LCK_COS && !(dlmflags & LDLM_FL_COS_INCOMPAT))
		RETURN(true);
	rc = lockmode_compat(lock->l_req_mode, req_mode);
	RETURN(rc);
}
				 ...
				 __u64 dlmflags, bool nonblock)
{
	...
	if (lustre_handle_is_used(&lh->mlh_pdo_lh)) {
		...
		if (lh->mlh_pdo_mode == LCK_CW || lh->mlh_pdo_mode == LCK_EX) {
	...
	if (lustre_handle_is_used(&lh->mlh_reg_lh)) {
		if (lh->mlh_reg_mode == LCK_PW || lh->mlh_reg_mode == LCK_EX) {
			mdt_fid_unlock(&lh->mlh_reg_lh, lh->mlh_reg_mode);
			dlmflags |= LDLM_FL_COS_INCOMPAT;
		} else {
			locked++;
		}
			 ...
			 struct mdt_lock_handle *lh, __u64 ibits, bool nonblock)
{
	...
	if (!mdt_object_remote(o))
		return mdt_object_local_lock(info, o, lh, ibits, 0, nonblock);
	...
	if (ibits & MDS_INODELOCK_LOOKUP) {
		if (lh->mlh_reg_mode == LCK_EX || lh->mlh_reg_mode == LCK_PW)
			dlmflags |= LDLM_FL_COS_INCOMPAT;
	...
	if (ibits & MDS_INODELOCK_UPDATE) {
		...
		if (rc != ELDLM_OK) {
			...
			RETURN(rc);
		} else if (lh->mlh_rreg_mode == LCK_EX ||
			   lh->mlh_rreg_mode == LCK_PW) {
...
static inline void mdt_enable_soc(const struct lu_env *env,
				  struct mdt_device *mdt)
{
...
/**
 * Check SoC (Sync on Cancel) status, if it's enabled, return true, else
 * get MDT count, if it's more than 1, enable SoC, and return true.
 */
static void mdt_check_set_soc(const struct lu_env *env, struct mdt_device *mdt)
{
	int mdt_count = 0;
			  ...
			  enum ldlm_mode mode, int decref)
{
	...
	if (lustre_handle_is_used(h)) {
		if (decref || !info->mti_has_trans ||
			...
			/* check whether MDT was added dynamically, if so enable
			 * Sync on Cancel, this is checked here in case this MDT
			 * never modify remote MDT object. */
			mdt_check_set_soc(info->mti_env, mdt);
			...
			if (likely(req != NULL)) {
				...
				ptlrpc_save_lock(req, NULL, h, mode, cos,
						 false);
			} else {
				mdt_fid_unlock(h, mode);
			}
...
/**
 * Save cross-MDT lock within request object when Sync-on-Cancel (SOC) enabled
 *
 ...
 * LCK_COS because this is client lock, and transaction commit won't happen on
				 ...
				 int decref)
{
	...
	mdt_enable_soc(info->mti_env, mdt);
	...
	if (lustre_handle_is_used(h)) {
		if (decref || !info->mti_has_trans ||
			...
			ptlrpc_save_lock(req, lock, h, mode, 1, false);
			LDLM_LOCK_PUT(lock);
		}
		h->cookie = 0ull;
	}
