		       ...
		       const struct lu_name *lname)
{
	...
	if (lu_name_is_valid(lname)) {
		...
		if (unlikely(lh->mlh_pdo_hash == 0))
			lh->mlh_pdo_hash = 1;
		/* limit pdo hash size to 255, so that on DNE system, there are
		 * at most 255 COS hash locks for a directory, this may lead to
		 * some hash collision, however if hash is evenly distributed
		 * it's fine. */
		lh->mlh_pdo_hash &= 0xff;
	} else {
		lh->mlh_pdo_hash = 0;
	}
}
                     ...
                     void *data, int flag)
{
	...
	if (lock->l_req_mode & (LCK_PW | LCK_EX) &&
		...
		else if (mdt_soc_is_enabled(mdt) &&
		 ...
		 const __u64 *client_cookie, bool cos_match)
{
	...
	if (cos_match) {
		...
		match = ldlm_lock_match(ns,
					...
					LDLM_FL_LOCAL_ONLY |
					LDLM_FL_COS_INCOMPAT, res_id,
					LDLM_IBITS, policy, LCK_COS,
