...
int mdt_getstatus(struct tgt_session_info *tsi)
{
	...
	if (req_capsule_get_size(info->mti_pill, &RMF_NAME, RCL_CLIENT) > 0) {
		...
		rc = mdt_lookup_fileset(info, fileset, &repbody->fid1);
		if (rc < 0)
			GOTO(out, rc = err_serious(rc));
	} else {
...
struct path_lookup_info {
	...
	struct lu_fid		*pli_root_fid;	/**< root fid, this may be
						     fileset fid */
			    ...
			    struct path_lookup_info *pli)
{
	...
	*ptr = 0;
	...
	while (!lu_fid_eq(pli->pli_root_fid,
...
static int mdt_path(struct mdt_thread_info *info, struct mdt_object *obj,
		    ...
		    struct lu_fid *fid, struct lu_fid *root_fid)
{
	struct mdt_device	*mdt = info->mti_mdt;
	struct path_lookup_info	*pli;
	int			tries = 3;
			...
			struct getinfo_fid2path *fp)
{
	...
	rc = mdt_path(info, obj, fp->gf_path, fp->gf_pathlen, &fp->gf_recno,
		      &fp->gf_linkno, &fp->gf_fid, root_fid);
