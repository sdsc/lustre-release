...
static int mdt_statfs(struct tgt_session_info *tsi)
{
	...
	rc = tgt_statfs_internal(tsi->tsi_env, &mdt->mdt_lut, osfs,
				 cfs_time_shift_64(-OBD_STATFS_CACHE_SECONDS), NULL);
	if (unlikely(rc))
		GOTO(out, rc);
	...
	tgt_grant_sanity_check(mdt->mdt_lu_dev.ld_obd, __FUNCTION__);
...
static int mdt_obd_disconnect(struct obd_export *exp)
{
	...
	if (!(exp->exp_flags & OBD_OPT_FORCE))
		tgt_grant_sanity_check(exp->exp_obd, __FUNCTION__);
