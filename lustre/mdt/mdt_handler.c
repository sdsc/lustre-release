			    ...
			    __u64 flags)
{
        ...
        *lockp = new_lock;
        ...
        cfs_hash_add(new_lock->l_export->exp_lock_hash,
                     ...
                     &new_lock->l_exp_hash);
	DEBUG_REQ(D_DLMTRACE, req, "added lock %p in hash with rhandle "LPX64
		  " unhashed %d", new_lock, new_lock->l_remote_handle.cookie,
                                    ...
                                    struct mdt_lock_handle *lh)
{
	...
	DEBUG_REQ(D_DLMTRACE, req, "lock %p found in hash with rhandle "LPX64,
		  lock, remote_hdl.cookie);
        if (lock) {
                if (lock != new_lock) {
