...
{
	struct lu_env		env;
	struct mdt_device	*mdt = (struct mdt_device *)data;
...
static int mdt_seq_init_cli(const struct lu_env *env, struct mdt_device *mdt)
{
	...
	snprintf(pre, MAX_OBD_NAME + 5, "ctl-%s", mdt_obd_name(mdt));
	rc = seq_client_init(ss->ss_client_seq, NULL, LUSTRE_SEQ_METADATA, pre,
			     ss->ss_node_id == 0 ?  ss->ss_control_seq : NULL);
	OBD_FREE(pre, MAX_OBD_NAME + 5);
	if (rc) {
		OBD_FREE_PTR(ss->ss_client_seq);
		ss->ss_client_seq = NULL;
		RETURN(rc);
