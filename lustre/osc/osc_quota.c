...
static inline int md_quota_flag(int qtype, u32 *flag)
{
	switch (qtype) {
	case USRQUOTA:
...
static inline int fl_quota_flag(int qtype, u32 *flag)
{
	switch (qtype) {
	case USRQUOTA:
		    ...
		    u64 valid, u32 flags)
{
	...
	for (type = 0; type < MAXQUOTAS; type++) {
		...
		rc = md_quota_flag(type, &qtype_flag);
		if (rc)
			break;
		...
		oqi = cfs_hash_lookup(cli->cl_quota_hash[type], &qid[type]);
		rc = fl_quota_flag(type, &qtype_flag);
		if (rc)
			break;
