                          ...
                          struct cl_page *page, void *cbdata)
{
        ...
        if (oap->oap_cmd & OBD_BRW_WRITE &&
            !cfs_list_empty(&oap->oap_pending_item))
                CL_PAGE_DEBUG(D_ERROR, env, page, "exists %lu/%s.\n",
                                start, current->comm);
...
#ifdef __linux__
        {
                ...
                if (PageLocked(vmpage))
                        CDEBUG(D_CACHE, "page %p index %lu locked for %d.\n",
                               ops, page->cp_index, (oap->oap_cmd & OBD_BRW_RWMASK));
        }
#endif
...
static void osc_io_end(const struct lu_env *env, const struct cl_io_slice *slice)
{
        struct osc_io *oio = cl2osc_io(env, slice);
