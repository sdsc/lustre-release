			  ...
			  struct osc_page *ops , void *cbdata)
{
	...
	struct osc_async_page *oap;
	struct osc_punch_range range = *(struct osc_punch_range *)cbdata;
			    ...
			    struct osc_io *oio, __u64 start, __u64 end)
{
	...
	struct osc_punch_range range;
	int     start_partial, end_partial;
	pgoff_t trunc_start, trunc_end;

	clob = oio->oi_cl.cis_obj;

	trunc_start	= cl_index(clob, start);
	start_partial	= cl_offset(clob, trunc_start) < start;

	trunc_end	= cl_index(clob, end);
	end_partial	= cl_offset(clob, trunc_end) < end;
	...
	osc_page_gang_lookup(env, io, cl2osc(clob),
			     trunc_start + start_partial,
			     trunc_end + end_partial,
                                ...
                                const struct cl_io_slice *slice)
{
	...
	loff_t			 start	= size, end = OBD_OBJECT_EOF;
	...
	if (result == 0 && oio->oi_lockless == 0) {
		...
		if (io_is_trunc || io_is_punch)
			...
			result = osc_prealloc_base(osc_export(cl2osc(obj)),
						   &oinfo, osc_async_upcall,
						   cbargs, PTLRPCD_SET);
		else
