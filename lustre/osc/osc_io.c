				...
				cl_commit_cbt cb)
{
	...
	struct osc_page *opg;
	pgoff_t last_index = CL_PAGE_EOF;
	int result = 0;
	ENTRY;
	...
	while (qin->pl_nr > 0) {
		...
		last_index = opg->ops_cl.cpl_page->cp_index;
	...
	if (last_index != CL_PAGE_EOF)
		osc_page_touch_at(env, osc2cl(osc), last_index,
				  result == 0 ? to : PAGE_SIZE);
