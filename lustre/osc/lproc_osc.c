                               ...
                               int *eof, void *data)
{
        struct obd_device *dev = data;
        struct client_obd *cli = &dev->u.cli;
        int shift = CFS_PAGE_SHIFT >> 10;
        int rc;

        rc = snprintf(page, count,
                      "lru total: %dM, free: %dK, in lru: %dK, busy %dK.\n",
                      cli->cl_lru_total >> (20 - CFS_PAGE_SHIFT),
                      cfs_atomic_read(&cli->cl_lru_left) << shift,
                      cfs_atomic_read(&cli->cl_lru_in_list) << shift,
                               ...
                               unsigned long count, void *data)
{
        ...
        if (pages_number < 0 ||
            pages_number < (cli->cl_dirty_max << (21 - CFS_PAGE_SHIFT)) ||
            pages_number > cfs_num_physpages / 2) /* 1/2 of RAM */
                return -ERANGE;

        client_obd_list_lock(&cli->cl_lru_list_lock);
        if (pages_number < cli->cl_lru_total)
