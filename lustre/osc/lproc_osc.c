...
static ssize_t
osc_max_rpcs_in_flight_seq_write(struct file *file, const char *buffer,
				 size_t count, loff_t *off)
{
	struct obd_device *dev =
		((struct seq_file *)file->private_data)->private;
				       ...
				       size_t count, loff_t *off)
{
	...
	mult = 1 << (20 - PAGE_CACHE_SHIFT);
	buffer = lprocfs_find_named_value(buffer, "used_mb:",
					  (unsigned long *)&count);
	rc = lprocfs_write_frac_helper(buffer, count, &pages_number, mult);
	if (rc)
		return rc;
...
static ssize_t
osc_cur_grant_bytes_seq_write(struct file *file, const char *buffer,
			      size_t count, loff_t *off)
{
...
static ssize_t
osc_grant_shrink_interval_seq_write(struct file *file, const char *buffer,
				    size_t count, loff_t *off)
{
	struct obd_device *obd =
		((struct seq_file *)file->private_data)->private;
...
static ssize_t
osc_checksum_type_seq_write(struct file *file, const char *buffer,
			    size_t count, loff_t *off)
{
	struct obd_device *obd =
		((struct seq_file *)file->private_data)->private;
...
static ssize_t
osc_resend_count_seq_write(struct file *file, const char *buffer,
			   size_t count, loff_t *off)
{
	struct obd_device *obd =
		((struct seq_file *)file->private_data)->private;
...
static ssize_t
osc_contention_seconds_seq_write(struct file *file, const char *buffer,
				 size_t count, loff_t *off)
{
	struct obd_device *obd =
		((struct seq_file *)file->private_data)->private;
...
static ssize_t
osc_lockless_truncate_seq_write(struct file *file, const char *buffer,
				size_t count, loff_t *off)
{
	struct obd_device *obd =
		((struct seq_file *)file->private_data)->private;
...
{
	return lprocfs_rd_obd_max_pages_per_rpc(m, m->private);
}

static ssize_t
osc_obd_max_pages_per_rpc_seq_write(struct file *file, const char *buffer,
				    size_t count, loff_t *off)
{
	struct obd_device *dev =
		((struct seq_file *)file->private_data)->private;
...
static ssize_t
osc_rpc_stats_seq_write(struct file *file, const char *buf,
