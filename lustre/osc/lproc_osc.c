...
static int osc_rd_cache_mb(char *page, char **start, off_t off, int count,
			   int *eof, void *data)
{
	struct obd_device *dev = data;
	struct client_obd *cli = &dev->u.cli;
	...
	rc = snprintf(page, count,
		      "lru pages: in lru: %dP, busy %dP, waiters: %d.\n",
...
static int osc_wr_cache_mb(struct file *file, const char *buffer,
			   unsigned long count, void *data)
{
	struct obd_device *dev = data;
	struct client_obd *cli = &dev->u.cli;
...
static struct lprocfs_vars lprocfs_osc_obd_vars[] = {
	...
	{ "osc_cache_mb",    osc_rd_cache_mb,     osc_wr_cache_mb, 0 },
