...
#define OSC_DUMP_GRANT(lvl, cli, fmt, args...) do {			\
	...
	CDEBUG(lvl, "%s: grant { dirty: %ld/%ld dirty_pages: %ld/%llu "	\
	       "unstable_pages: %ld/%llu dropped: %ld avail: %ld, "	\
	       "reserved: %ld, flight: %d } lru {in list: %ld, "	\
	       "left: %ld, waiters: %ld }" fmt,				\
	       __tmp->cl_import->imp_obd->obd_name,			\
	       __tmp->cl_dirty, __tmp->cl_dirty_max,			\
	       atomic64_read(&obd_dirty_pages), obd_max_dirty_pages,	\
	       atomic64_read(&obd_unstable_pages), obd_max_dirty_pages,	\
	       __tmp->cl_lost_grant, __tmp->cl_avail_grant,		\
	       __tmp->cl_reserved_grant, __tmp->cl_w_in_flight,		\
	       atomic64_read(&__tmp->cl_lru_in_list),			\
	       atomic64_read(&__tmp->cl_lru_busy),			\
	       atomic64_read(&__tmp->cl_lru_shrinkers), ##args);	\
} while (0)
