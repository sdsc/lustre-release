...
struct extent_rpc_data {
	...
	unsigned		erd_page_count;
	unsigned		erd_max_pages;
	unsigned		erd_max_chunks;
};
				    ...
				    struct extent_rpc_data *data)
{
	...
	if (data->erd_max_chunks < chunk_count)
		RETURN(0);
...
static inline unsigned osc_max_write_chunks(const struct client_obd *cli)
{
	 ...
	 * is set to 32MB for a safety net. Ideally OST should tell the client
	 * what the biggest transaction size is, but it's good enough for now.
	 *
	 * This will not affect ldiskfs because the chunk size in ldiskfs is
	 * 4K, and the maximum RPC size is 16MB.
	 */
	return (32 * 1024 * 1024) >> cli->cl_chunkbits;
}
