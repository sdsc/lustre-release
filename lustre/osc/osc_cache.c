...
#define OSC_DUMP_GRANT(lvl, cli, fmt, args...) do {			\
	...
	CDEBUG(lvl, "%s: grant { dirty: %ld/%ld dirty_pages: %lld/%lu "	\
	       "unstable_pages: %lld/%ld dropped: %ld avail: %ld, "	\
	       "reserved: %ld, flight: %d } lru {in list: %lld, "	\
	       "left: %lld, waiters: %d }" fmt,				\
	       ...
	       (unsigned long long) atomic64_read(&__tmp->cl_lru_in_list), \
	       (unsigned long long) atomic64_read(&__tmp->cl_lru_busy),	\
