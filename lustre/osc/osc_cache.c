			    ...
			    struct osc_extent *victim)
{
	...
	if (cur->oe_end - cur->oe_start + 1 +
	    victim->oe_end - victim->oe_start + 1 > cli->cl_max_extent_size)
		return -ERANGE;
...
restart:
	...
	while (ext != NULL) {
		...
		/* check whether maximum extent limit will be hit */
		if (ext_chk_end - ext_chk_start + 1) {
			ext = next_extent(ext);
			continue;
		}
...
static int osc_extent_expand(struct osc_extent *ext, pgoff_t index, int *grants)
{
	...
	if (end_index - ext->oe_start + 1 > cli->cl_max_extent_size)
		GOTO(out, rc = -ERANGE);
...
#define OSC_DUMP_GRANT(lvl, cli, fmt, args...) do {			\
	...
	CDEBUG(lvl, "%s: grant { dirty: %ld/%ld dirty_pages: %ld/%lu "	\
	       "dropped: %ld avail: %ld, dirty_grant: %ld reserved: %ld, " \
	       "flight: %d } lru {in list: %ld, left: %ld, waiters: %d }" fmt, \
