...
static void osc_class_queue_reclaim(struct client_obd *cli,
				  struct osc_class *class)
{
	if (class->oc_reclaim_time == 0) {
		LASSERT(class->oc_dirty_pages < class->oc_max_pages);
		class->oc_reclaim_time = ktime_to_ns(ktime_get()) +
				    ...
				    int *returned)
{
	...
	if (*returned ||
		...
		osc_class_queue_reclaim(cli, class);
	} else {
		...
		if (returned_npages > 0) {
			*returned = 1;
			...
			osc_class_queue_reclaim(cli, top_class);
					  ...
					  bool remove)
{
	...
	class->oc_request_check_time = ktime_to_ns(ktime_get());
	if (remove) {
...
osc_class_init(struct client_obd *cli, const char *jobid)
{
	...
	class->oc_request_check_time = 0;
	class->oc_request_nsecs = 0;
	class->oc_in_request_heap = false;
