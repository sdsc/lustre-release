...
static int osc_extent_wait(const struct lu_env *env, struct osc_extent *ext, int state);
                                 ...
                                 struct osc_extent *ext)
{
        ...
        cfs_list_for_each_entry(oap, &ext->oe_pages, oap_pending_item) {
                ...
                do {
                        ...
                        /* to avoid deadlock, vvp_page_make_ready() returns
                         * -EAGAIN if it can't grab page lock. Mostly this
                         * is due to ll_writepage() already held page lock
                         * and try to flush this page(remember no process is
                         * is using this page).
                         * In that case, osc_flush_async_page() may have
                         * already set ASYNC_READY for us. */
                } while (rc == -EAGAIN);
