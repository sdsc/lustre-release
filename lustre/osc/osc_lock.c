...
static int osc_lock_match_group(const struct lu_env *env, struct osc_lock *oscl,
				struct lustre_handle *handle)
{
	...
	if (unlikely(ldlm_lock_addref_try(handle, LCK_GROUP) < 0))
		RETURN(-ENOLCK);

	dlmlock = ldlm_handle2lock(handle);
	LASSERT(dlmlock != NULL);

	lock_res_and_lock(dlmlock);
	if (ldlm_is_failed(dlmlock) || ldlm_is_cbpending(dlmlock))
		rc = -ENOLCK;
	unlock_res_and_lock(dlmlock);
	if (rc < 0)
		GOTO(out, rc);

	osc_lock_granted(env, oscl, handle, false);

out:
	ldlm_lock_decref(handle, LCK_GROUP);
	LDLM_LOCK_PUT(dlmlock);
	RETURN(rc);
}
	...
	list_for_each_entry(tmp_oscl, &obj->oo_ol_list,
		...
		if (descr->cld_mode == CLM_GROUP) {
			if (need->cld_mode == CLM_GROUP &&
			    need->cld_gid == descr->cld_gid) {
				spin_unlock(&obj->oo_ol_spin);

				/* If this is a group lock, it must reuse the
				 * existing lock */
				rc = osc_lock_match_group(env, oscl,
							 &tmp_oscl->ols_handle);
				spin_lock(&obj->oo_ol_spin);
			}
			break;
			    ...
			    struct cl_io *unused, struct cl_sync_io *anchor)
{
	...
	if (oscl->ols_state == OLS_GRANTED) /* group lock */
		RETURN(0);
