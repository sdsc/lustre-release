				...
				__u32 client_cksum)
{
	...
	snprintf(dbgcksum_file_name, sizeof(dbgcksum_file_name),
		 "/tmp/checksum_dump-osc-"DFID":[%llu-%llu]-%x-%x",
				...
				struct osc_brw_async_args *aa)
{
	...
	CERROR("original client csum %x (type %x), server csum %x (type %x), "
	       "client csum now %x\n", client_cksum,
	       cksum_type_unpack(aa->aa_oa->o_flags), server_cksum, cksum_type,
	       new_cksum);
	return 1;
}
				...
				struct osc_brw_async_args *aa, int rc)
{
	...
	/* XXX just in case it is not copied from previous request and to
	 * allow for cksum check on Server side upon resend */
	if (aa->aa_oa->o_cksum)
		new_aa->aa_oa->o_cksum = aa->aa_oa->o_cksum;
