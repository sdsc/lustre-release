                                ...
                                long writing_bytes)
{
	...
	if (unlikely(cli->cl_dirty_pages - cli->cl_dirty_transit >
		...
		if (OCD_HAS_FLAG(&cli->cl_import->imp_connect_data,
			 ...
			 * grant space */
			nrextents = (nrpages + cli->cl_max_extent_size - 1)  /
				     cli->cl_max_extent_size;
			oa->o_undirty += nrextents * cli->cl_extent_tax;
		}
        }
...
static void osc_init_grant(struct client_obd *cli, struct obd_connect_data *ocd)
{
	...
	if (OCD_HAS_FLAG(ocd, GRANT_PARAM)) {
		...
		cli->cl_extent_tax = ocd->ocd_ext_tax_kb << 10;
		/* determine the appropriate chunk size used by osc_extent. */
		cli->cl_chunkbits = max_t(int, PAGE_CACHE_SHIFT,
					  ocd->ocd_blockbits);
		/* determine maximum extent size, in #pages */
		size = ocd->ocd_max_ext_blks << ocd->ocd_blockbits;
		cli->cl_max_extent_size = size >> PAGE_CACHE_SHIFT;
		if (cli->cl_max_extent_size == 0)
			cli->cl_max_extent_size = 1;
	} else {
		cli->cl_extent_tax = 0;
		cli->cl_chunkbits = PAGE_CACHE_SHIFT;
		cli->cl_max_extent_size = DT_MAX_BRW_PAGES;
	}
	client_obd_list_unlock(&cli->cl_loi_list_lock);

	CDEBUG(D_CACHE, "%s, setting cl_avail_grant: %ld cl_lost_grant: %ld."
		"chunk bits: %d cl_max_extent_size: %d\n",
		cli->cl_import->imp_obd->obd_name,
