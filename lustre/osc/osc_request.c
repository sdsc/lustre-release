		     ...
		     struct ptlrpc_request **reqp, int resend)
{
	...
	if ((cmd & OBD_BRW_WRITE) != 0) {
		opc = OST_WRITE;
		req = ptlrpc_request_alloc(cli->cl_import, &RQF_OST_BRW_WRITE);
		if (req == NULL)
...
static int __init osc_init(void)
{
	 ...
	 * cl_max_rpcs_in_flight. The request count can never be enough as
	 * long as the OSC number keeps on growing. And there isn't much solid
	 * evidence showing that a relatively small request pool will cause
