                         ...
                         struct ptlrpc_request *req, void *data, int rc)
{
        ...
        if (req->rq_fake == 1) {
                ...
                /* FIXME: Because the way ptlrpcd handles new ptlrpc_requests,
                 * and fake request handler is called immediately, there exists
                 * a race that ptlrpcd threads don't refill their env with
                 * osc_key, while fake request is being handled. As a workaround
                 * refilled it here. - Jinshan
                 */
                       ...
                       int count, obd_flag brw_flags, enum async_flags async_flags)
{
        ...
        loi_list_maint(cli, loi);
        if (!osc_max_rpc_in_flight(cli, loi) &&
            /* Alarm's refcount being one means the alarm is not in ptlrpc
             * list so that we can submit it.
             * This is ugly, but it lacks a decent way to check if an RPC
             * is currently in ptlrpcd list. - Jinshan
             */
            cfs_atomic_read(&cli->cl_alarm->rq_refcount) == 1 &&
            lop_makes_rpc(cli, &loi->loi_write_lop, OBD_BRW_WRITE)) {
