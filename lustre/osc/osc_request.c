                                ...
                                long writing_bytes)
{
	...
	if (unlikely(cli->cl_dirty - cli->cl_dirty_transit >
		...
		oa->o_undirty = 0;
			    ...
			    (__u64)(obd_max_dirty_pages + 1))) {
		/* The atomic_read() allowing the atomic_inc() are
		 * not covered by a lock thus they may safely race and trip
		 * this CERROR() unless we add in a small fudge factor (+1). */
		CERROR("%s: dirty %llu + %llu - %llu > system dirty_max %llu\n",
		       cli->cl_import->imp_obd->obd_name,
		       (__u64)atomic64_read(&obd_unstable_pages),
		       (__u64)atomic64_read(&obd_dirty_pages),
		       (__u64)atomic64_read(&obd_dirty_transit_pages),
		       obd_max_dirty_pages);
		oa->o_undirty = 0;
                              ...
                              void *val, struct ptlrpc_request_set *set)
{
	...
	if (KEY_IS(KEY_CACHE_LRU_SHRINK)) {
		struct client_obd *cli = &obd->u.cli;
		__u64 nr = atomic64_read(&cli->cl_lru_in_list) >> 1;
		__u64 target = *(int *)val;
