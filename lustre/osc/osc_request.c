                              ...
                              void *val, struct ptlrpc_request_set *set)
{
	...
	if (KEY_IS(KEY_LRU_SET)) {
		...
		cfs_atomic_inc(&cli->cl_lru->ccl_refc);
	...
	if (KEY_IS(KEY_LRU_SHRINK)) {
		...
		nr = osc_lru_shrink(cli, nr > target ? target : nr);
		*(int *)val = target - nr;
		RETURN(0);
	}
...
int osc_cleanup(struct obd_device *obd)
{
	...
	/* lru cleanup */
	if (cli->cl_lru != NULL) {
		LASSERT(cfs_atomic_read(&cli->cl_lru->ccl_refc) > 0);
		cfs_spin_lock(&cli->cl_lru->ccl_lock);
		cfs_list_del_init(&cli->cl_lru_osc);
		cfs_spin_unlock(&cli->cl_lru->ccl_lock);
		cli->cl_lru_left = NULL;
		cfs_atomic_dec(&cli->cl_lru->ccl_refc);
		cli->cl_lru = NULL;
	}
