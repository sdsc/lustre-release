                 ...
                 int cmd, struct loi_oap_pages *lop)
{
        ...
        cfs_list_for_each_entry_safe(oap, tmp, &lop->lop_pending,
                ...
                /* To make sure there is no gap in the page vec we're going
                 * to send. The page list of lop_pending is not sorted, that
                 * means gap may appear after the function sort_brw_pages()
                 * is called, even though it looks like okay here.
                 * Unfortunately, there is no easy way to fix it. Upper logic
                 * has to make sure this. -jay
                 */
                if (ending_offset > max_off) {
                        LASSERT((max_off & ~CFS_PAGE_MASK) == 0);
                        max_off = ending_offset;
                } else if (oap->oap_page_off + oap->oap_count < CFS_PAGE_SIZE) {
                        CERROR("Uncontiguous: max_off:"LPU64"off:%d, cnt:%d\n",
                               max_off, oap->oap_page_off, oap->oap_count);
                        LASSERT(0); /* */
                }
