                       ...
                       int count, obd_flag brw_flags, enum async_flags async_flags)
{
        ...
        if (!osc_max_rpc_in_flight(cli, loi) &&
            lop_makes_rpc(cli, &loi->loi_write_lop, OBD_BRW_WRITE)) {
                LASSERT(cli->cl_alarm != NULL);
                rc = ptlrpc_run_work(cli->cl_alarm);

                CDEBUG(D_OTHER, "Firing alarm for client obd %p/%d.\n",
                       cli, rc);
        }
        client_obd_list_unlock(&cli->cl_loi_list_lock);
...
int osc_setup(struct obd_device *obd, struct lustre_cfg *lcfg)
{
        ...
        if (rc == 0) {
                void *handler;
                handler = ptlrpc_alloc_work(cli->cl_import,
                                            brw_queue_work, cli);
                if (!IS_ERR(handler))
                        cli->cl_alarm = handler;
                else
                        rc = PTR_ERR(handler);
        }
...
static int osc_precleanup(struct obd_device *obd, enum obd_cleanup_stage stage)
{
        ...
        case OBD_CLEANUP_EXPORTS: {
                ...
                obd_zombie_barrier();
                if (cli->cl_alarm) {
                        ptlrpc_destroy_work(cli->cl_alarm);
                        cli->cl_alarm = NULL;
                }
