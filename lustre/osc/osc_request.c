                                ...
                                long writing_bytes)
{
		...
		long max_in_flight = (cli->cl_max_pages_per_rpc << PAGE_CACHE_SHIFT)*
                              ...
                              struct brw_page **pga)
{
        ...
        while (page_count-- > 0) {
		ptr = kmap(pga[i]->pg) + (pga[i]->off & ~PAGE_CACHE_MASK);
				   ...
				   cksum_type_t cksum_type)
{
	...
	while (nob > 0 && pg_count > 0) {
		...
		if (i == 0 && opc == OST_READ &&
			...
			int off = pga[i]->off & ~PAGE_CACHE_MASK;
		...
		LL_CDEBUG_PAGE(D_PAGE, pga[i]->pg, "off %d checksum %x\n",
			       (int)(pga[i]->off & ~PAGE_CACHE_MASK), cksum);
                                ...
                                int resend)
{
	...
	req->rq_no_retry_einprogress = 1;
		...
		int poff = pg->off & ~PAGE_CACHE_MASK;
...
static obd_count max_unfragmented_pages(struct brw_page **pg, obd_count pages)
{
        ...
        LASSERT (pages > 0);
	offset = pg[i]->off & ~PAGE_CACHE_MASK;
                     ...
                     struct ptlrpc_request_set *rqset, int async, int agl)
{
	...
	policy->l_extent.start -= policy->l_extent.start & ~PAGE_CACHE_MASK;
	policy->l_extent.end |= ~PAGE_CACHE_MASK;
                         ...
                         void *localdata)
{
        ...
        if (data != NULL && (data->ocd_connect_flags & OBD_CONNECT_GRANT)) {
				...
				2 * cli->cl_max_pages_per_rpc << PAGE_CACHE_SHIFT;
