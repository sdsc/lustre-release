                                ...
                                int resend)
{
	...
	unsigned long integrity_fl = 0;
	unsigned ichunk_size = 1 << ocd->ocd_ichunk_size;
	...
	if (cli->cl_checksum)
		integrity_fl = cli->cl_cksum_type & OBD_CKSUM_T10AB;
	...
	osc_integrity_prep_pill(pill, opc, page_count, ichunk_size,
				integrity_fl);
	...
	desc = ptlrpc_prep_bulk_imp(req, page_count +
				    osc_integrity_bulk(page_count, ichunk_size,
							integrity_fl),
	...
	rc = osc_integrity_prep_brw(req, cmd, integrity_fl, ichunk_size, pga,
				    page_count, body, &integrity, desc,
				    &requested_nob);
	...
	aa->aa_integrity_sz = osc_integrity_size(page_count, ichunk_size,
						 integrity_fl);
...
static int osc_brw_fini_request(struct ptlrpc_request *req, int rc)
{
	...
	if (cmd == OBD_BRW_WRITE) {
        ...
        if (rc < aa->aa_requested_nob)
		handle_short_read(rc - aa->aa_integrity_sz, aa->aa_page_count,
				  aa->aa_ppga);

        if (body->oa.o_valid & OBD_MD_FLCKSUM && !aa->aa_integrity) {
...
out:
	irc = osc_integrity_fini_brw(req, cmd, 1 << ocd->ocd_ichunk_size,
				     aa->aa_ppga,
				     (iosz > 0) ? ((iosz + CFS_PAGE_SIZE - 1) >>
						    CFS_PAGE_SHIFT) : 0,
