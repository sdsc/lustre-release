                         ...
                         struct ptlrpc_request *req, void *data, int rc)
{
	...
	if (lustre_msg_get_opc(req->rq_reqmsg) == OST_WRITE) {
		cli->cl_w_in_flight--;
		/**
		 * We don't want to cleanup the binary heap unless very
		 * necessary.
		 */
 ...
 * \retval 1 e1 < e2
 */
static int osc_class_heap_cmp(struct cfs_binheap_node *e1,
			      struct cfs_binheap_node *e2)
{
	struct osc_class *class1;
	struct osc_class *class2;

	class1 = container_of(e1, struct osc_class, oc_node);
	class2 = container_of(e2, struct osc_class, oc_node);

	if (class1->oc_dirty_pages < class2->oc_dirty_pages)
		return 1;
	else if (class1->oc_dirty_pages > class2->oc_dirty_pages)
		return 0;

	return 1;
}

static struct cfs_binheap_ops osc_class_heap_ops = {
	.hop_enter	= NULL,
	.hop_exit	= NULL,
	.hop_compare	= osc_class_heap_cmp,
};
...
int osc_setup(struct obd_device *obd, struct lustre_cfg *lcfg)
{
	...
	cli->cl_class_heap = cfs_binheap_create(&osc_class_heap_ops,
						CBH_FLAG_ATOMIC_GROW,
						4096, NULL, NULL, 0);
	if (!cli->cl_class_heap)
...
out_free_assign_heap:
	cfs_binheap_destroy(cli->cl_class_heap);
out_ptlrpcd:
	ptlrpcd_decref();
	RETURN(rc);
}
...
int osc_cleanup(struct obd_device *obd)
{
	...
	cfs_hash_putref(cli->cl_class_hash);
	LASSERT(cfs_binheap_is_empty(cli->cl_class_heap));
	cfs_binheap_destroy(cli->cl_class_heap);
