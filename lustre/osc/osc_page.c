...
static int osc_cache_too_much(struct client_obd *cli)
{
	...
	long pages = atomic64_read(&cli->cl_lru_in_list);
		   ...
		   long target, bool force)
{
	...
	client_obd_list_lock(&cli->cl_lru_list_lock);
	maxscan = min(target << 1, (long) atomic64_read(&cli->cl_lru_in_list));
...
long osc_lru_reclaim(struct client_obd *cli)
{
	...
	CDEBUG(D_CACHE, "%s: cli %p no free slots, pages: %lld, busy: %lld.\n",
	       cli->cl_import->imp_obd->obd_name, cli,
	       (unsigned long long) atomic64_read(&cli->cl_lru_in_list),
	       (unsigned long long) atomic64_read(&cli->cl_lru_busy));
	...
	while (--max_scans > 0 && !list_empty(&cache->ccc_lru)) {
		...
		CDEBUG(D_CACHE, "%s: cli %p LRU pages: %lld, busy: %lld.\n",
		       cli->cl_import->imp_obd->obd_name, cli,
		       (unsigned long long) atomic64_read(&cli->cl_lru_in_list),
		       (unsigned long long) atomic64_read(&cli->cl_lru_busy));
