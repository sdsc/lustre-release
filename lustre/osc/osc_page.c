...
static int osc_cache_too_much(struct client_obd *cli)
{
	...
	if (atomic_read(cli->cl_lru_left) < cache->ccc_lru_max >> 2) {
		if (pages >= budget)
...
{
	/* Check if the cl_page is referenced by somebody else */
	...
	if (cli->cl_check_unstable) {
		...
		/* If unstable account is turned on, bulk transfer may hold
		 * one refcount for recovery so we need to check vmpage
		 * refcount as well; otherwise, even we can destroy cl_page
		 * but the corresponding vmpage can't be reused.
...
int osc_lru_reclaim(struct client_obd *cli, unsigned int npages)
{
	...
	CDEBUG(D_CACHE, "%s: start to reclaim %d pages from LRU\n",
	       cli_name(cli), npages);
	rc = osc_lru_shrink(env, cli, npages, true);
	if (rc >= npages) {
		CDEBUG(D_CACHE, "%s: reclaimed %d/%d pages from LRU\n",
	...
	CDEBUG(D_CACHE, "%s: cli %p no free slots, pages: %d/%d, want: %d.\n",
		cli_name(cli), cli, atomic_read(&cli->cl_lru_in_list),
		atomic_read(&cli->cl_lru_busy), npages);
			   ...
			   struct osc_page *opg)
{
	 ...
	 * Only in the case that the LRU slots are in extreme shortage,
	 * it should have reserved enough slots for an IO. */
