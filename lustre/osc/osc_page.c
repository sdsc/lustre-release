...
static int osc_lru_reclaim(struct client_obd *cli)
{
	...
	while (--max_scans > 0 && !cfs_list_empty(&cache->ccc_lru)) {
		victim = cfs_list_entry(cache->ccc_lru.next, struct client_obd,
					cl_lru_osc);
		...
		if (cfs_atomic_read(&victim->cl_lru_in_list) > 0) {
			...
			rc = osc_lru_shrink(victim, max_to_shrink(victim));
			spin_lock(&cache->ccc_lru_lock);
			if (rc != 0)
				break;
		}
	...
	CDEBUG(D_CACHE, "%s: Freed %d pages from other cli: %p.\n",
