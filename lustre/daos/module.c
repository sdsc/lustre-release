...
daos_wi_noop_run(struct cfs_workitem *wi)
{
	...
	OBD_FREE_PTR(wnp);
	return 1; /* tell scheduler this workitem is gone */
}
...
daos_noop(struct daos_usr_noop *unp)
{
	...
	rc = daos_event_dispatch(unp->unp_ev,
				 NULL, NULL, NULL, &wnp->wnp_event);
	if (rc != 0)
		return rc;
	...
	cfs_wi_schedule(the_daos.dm_scheds[current->pid % the_daos.dm_schedn],
			&wnp->wnp_wi);
	return 0;
}
...
#if DAOS_TEST
	if (the_daos.dm_scheds != NULL) {
		int	i;

		for (i = 0; i < the_daos.dm_schedn; i++) {
			if (the_daos.dm_scheds[i] != NULL)
				cfs_wi_sched_destroy(the_daos.dm_scheds[i]);
		}

		OBD_FREE(the_daos.dm_scheds,
			 the_daos.dm_schedn * sizeof(*the_daos.dm_scheds));
		the_daos.dm_scheds = NULL;
	}
#endif
	daos_hhash_finalize();
}
...
daos_initialize(void)
{
	...
	if (rc != 0)
		goto failed;
#if DAOS_TEST
	the_daos.dm_schedn = cfs_cpt_weight(cfs_cpt_table, CFS_CPT_ANY);
	while (the_daos.dm_schedn > 16)
		the_daos.dm_schedn >>= 1;

	OBD_ALLOC(the_daos.dm_scheds,
		  the_daos.dm_schedn * sizeof(*the_daos.dm_scheds));
	if (the_daos.dm_scheds == NULL)
		goto failed;
	...
	do {
		...
		for (i = 0; i < the_daos.dm_schedn; i++) {
			sprintf(name, "daos.%d", i);
			rc = cfs_wi_sched_create(name, cfs_cpt_table,
						 CFS_CPT_ANY, 1,
						 &the_daos.dm_scheds[i]);
			if (rc != 0)
				goto failed;
		}
	} while (0);
