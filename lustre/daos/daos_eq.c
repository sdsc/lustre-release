...
daos_eq_free(struct daos_hlink *hlink)
{
	...
	LASSERT(daos_hlink_empty(&eq->eq_hlink));
 ...
 * This is kernel handler of DAOS API daos_eq_create()
 * Please check lustre/include/daos/daos_api.h for details
 */
int
daos_eq_create(struct daos_dev_env *denv, struct daos_usr_eq_create *eqc)
{
	...
	/* eq::eq_cookie is increased each time an event is added and the
	 * value is used as an unique ID & hash key for the event inside
	 * the per-eq hash, zero is reserved for invalid event ID */
	eq->eq_cookie = 1;
	...
	if (rc != 0) {
		...
		eqd.eqd_handle = eqh;
		eqd.eqd_force = 1;
		rc = daos_eq_destroy(denv, &eqd);
		if (rc != 0)
			CERROR("Failed to cleanup not-returned EQ: %d\n", rc);
		rc = -EFAULT;
	}
failed:
	daos_eq_putref(eq);
 ...
 * This is kernel handler of DAOS API daos_eq_destroy()
 * Please check lustre/include/daos/daos_api.h for details
 */
int
daos_eq_destroy(struct daos_dev_env *denv, struct daos_usr_eq_destroy *eqd)
{
	...
	if (!list_empty(&eq->eq_flink))
		list_del_init(&eq->eq_flink); /* remove from dev open list */
	spin_unlock(&denv->den_lock);
	...
	while (!list_empty(&zombies)) {
		 ...
		 * release RPC reference taken by this event */
 ...
 * This is kernel handler of DAOS API daos_eq_poll()
 * Please check lustre/include/daos/daos_api.h for details
 */
int
 ...
 * This is kernel handler of DAOS API daos_eq_query()
 * Please check lustre/include/daos/daos_api.h for details
 */
int
daos_eq_query(struct daos_dev_env *denv, struct daos_usr_eq_query *eqq)
{
	...
	spin_lock(&eq->eq_lock);
	if (eq->eq_finalizing || eq->eq_error != 0) {
	...
	if (daos_eq_query_buf_alloc(&head, rc) < rc)
		GOTO(out, rc = -ENOMEM);
...
static int
daos_parent_event_can_complete(struct daos_kevent *ev)
{
	LASSERT(ev->ev_childn_comp <= ev->ev_childn);

	if (ev->ev_status == DAOS_EVS_COMPLETE || /* already completed */
	    ev->ev_status == DAOS_EVS_NONE) /* not dispatched at all */
		return 0;
 ...
 * This is kernel handler of DAOS API daos_event_init()
 * Please check lustre/include/daos/daos_api.h for details
 */
int
...
daos_event_complete_cb(struct daos_kevent *ev, int unlinked)
{
	...
	/* NB: called w/o EQ lock because completion callback might
	 * need to copy data to userspace */
	LASSERT(ev->ev_status == DAOS_EVS_COMPLETE);
	...
	list_for_each_entry(tmp, &ev->ev_children, ev_link) {
		 ...
		 * a very long list again, with holding lock in hot path. */
		tmp->ev_status = DAOS_EVS_NONE;
	}

	if (ev->ev_ops != NULL && ev->ev_ops->op_complete!= NULL) {
		rc2 = ev->ev_ops->op_complete(ev->ev_kparam, ev->ev_uparam,
					      ev->ev_error, unlinked);
		if (rc2 != 0 && rc == 0)
			rc = rc2;
	}
...
/**
 * Finalize an event.
 * This is kernel handler of DAOS API daos_event_fini()
 * Please check lustre/include/daos/daos_api.h for details
 */
int
daos_event_fini(struct daos_dev_env *denv, struct daos_usr_ev_fini *evf)
{
 ...
 again:
	...
	case DAOS_EVS_NONE:
		...
		if (daos_parent_event_can_complete(parent)) {
...
daos_event_abort_one(struct daos_kevent *ev, int unlinked)
{
	...
	if (ev->ev_ops != NULL && ev->ev_ops->op_abort != NULL)
		ev->ev_ops->op_abort(ev->ev_kparam, ev->ev_uparam, unlinked);
}
 ...
 * This is kernel handler of DAOS API daos_event_abort()
 * Please check lustre/include/daos/daos_api.h for details
 */
int
 ...
 * This is kernel handler of DAOS API daos_event_next()
 * Please check lustre/include/daos/daos_api.h for details
 */
int
...
/**
 * Dispatch an event to an asynchronous operaiton, it will take a reference
 * cound on the specified event, the reference can only be released by calling
 * \a daos_event_complete().
 *
 * \param eid [IN]	event ID
 * \param uparam [IN]	user parameters for completion callback, for example,
 * 			userspace buffers to store results.
 * \param kparam [IN]	kernel parameter for completion callback, for example,
 * 			pointer to RPC
 * \param ops [IN]	callbacks for event "abort" and event "completion"
 * \param evpp [OUT]	returned event
 *
 * \return		0 or error code
 */
int
daos_event_dispatch(daos_event_id_t eid, void *uparam, void *kparam,
		    daos_kevent_ops_t *ops, struct daos_kevent **evpp)
{
	...
	if (eq->eq_finalizing || eq->eq_error) {
		...
		rc = eq->eq_finalizing ? -ENOENT : eq->eq_error;
		goto out;
	}
...
/**
 * Complete an event, it will release reference count taken by
 * daos_event_dispatch() and call completion callback if event is still
 * alive (not unlinked by user).
 *
 * \param ev		pointer to event
 * \param error		error code of asynchronous operation, it will be
 * 			copied to userspace as returned value of operaiton.
