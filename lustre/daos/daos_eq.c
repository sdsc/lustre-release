...
{
	struct daos_hlink *hlink = daos_hlink_lookup(eqh.cookie);
...
#define DAOS_EQ_HASH_BITS	7
#define DAOS_EQ_HASH_BITS_MAX	14

extern cfs_hash_ops_t		daos_event_hops;

static int
daos_eq_create(struct daos_usr_param *uparam)
{
	struct daos_usr_eq_create *eqc = daos_usr_param_body(uparam);
	struct daos_eq		  *eq;
	daos_handle_t		   eqh;
	int			   rc;
...
static int
daos_eq_destroy(struct daos_usr_param *uparam)
{
	struct daos_usr_eq_destroy *eqd = daos_usr_param_body(uparam);
	struct daos_eq		   *eq;
	struct daos_kevent	   *ev;
	...
	if (eq == NULL) {
		CDEBUG(D_INFO, "Can't find EQ by "LPU64"\n",
...
daos_event_copy_out(struct daos_kevent *ev, daos_event_t __user **uevent_pp)
{
	...
	if (put_user(ev->ev_error, &ev->ev_status) != 0)
		return -EFAULT;
...
static int
daos_eq_poll(struct daos_usr_param *uparam)
{
	struct daos_usr_eq_poll *eqp = daos_usr_param_body(uparam);
	struct daos_eq		*eq;
	wait_queue_t		 wait;
	int			 rc = 0;
	int			 count = 0;
...
static int
daos_eq_query(struct daos_usr_param *uparam)
{
	struct daos_usr_eq_query *eqq = daos_usr_param_body(uparam);
	struct daos_eq		 *eq;
	struct daos_kevent	 *ev;
...
{
	ev->ev_id = eq->eq_cookie++;
	if (unlikely(eq->eq_cookie == 0))
		eq->eq_cookie = 1; /* zero is reserved for NUL */

	ev->ev_ref = 1;
	cfs_hash_add(eq->eq_hash, &ev->ev_id, &ev->ev_hnode);
}
			 ...
			 struct daos_kevent *ev, int unlink_children)
{
	...
	if (ev->ev_id == 0) /* already unlinked */
		return;

	cfs_hash_del(eq->eq_hash, &ev->ev_id, &ev->ev_hnode);
	LASSERT(ev->ev_ref > 0);
	ev->ev_ref--;
	ev->ev_id = 0; /* mark it as unlinked */
	...
	list_for_each_entry(tmp, &ev->ev_children, ev_link) {
		...
		cfs_hash_del(eq->eq_hash, &tmp->ev_id, &tmp->ev_hnode);
...
daos_event_poll_wait_locked(struct daos_eq *eq, struct daos_kevent *ev)
{
	...
	if (master->ev_status != DAOS_EVS_COMPLETE)
		return 0; /* it's an inflight event */
...
int
daos_event_init(struct daos_usr_param *uparam)
{
	struct daos_usr_ev_init *evi = daos_usr_param_body(uparam);
	struct daos_kevent	*parent = NULL;
	struct daos_kevent	*ev;
	struct daos_eq		*eq;
	int			rc;

	if (evi->evi_uevent == NULL) {
		CDEBUG(D_INFO, "Userspace event is NULL\n");
		return -EINVAL;
	}

	if (evi->evi_eqh.cookie == 0)
		evi->evi_eqh = evi->evi_parent.ev_eqh;

	if (evi->evi_parent.ev_eqh.cookie != 0 &&
	    evi->evi_parent.ev_eqh.cookie != evi->evi_eqh.cookie) {
		CDEBUG(D_INFO, "Child event should be on the same EQ with "
			       "parent "LPU64"/"LPU64"\n",
			       evi->evi_parent.ev_eqh.cookie,
			       evi->evi_eqh.cookie);
		return -EINVAL;
	}
	...
	if (evi->evi_parent.ev_id != 0) {
		parent = cfs_hash_lookup(eq->eq_hash, &evi->evi_parent.ev_id);
		if (parent == NULL) {
	...
	rc = copy_to_user(&evi->evi_upriv->ev_id,
			  &ev->ev_id, sizeof(ev->ev_id));
	if (rc != 0) {
...
static int
daos_event_fini(struct daos_usr_param *uparam)
{
	struct daos_usr_ev_fini *evf = daos_usr_param_body(uparam);
	struct daos_kevent	*parent;
	struct daos_kevent	*ev;
	struct daos_eq		*eq;
	int			rc = 0;

	eq = daos_eq_lookup(evf->evf_id.ev_eqh);
	if (eq == NULL) {
		CDEBUG(D_INFO, "Unknown EQ cookie "LPU64"\n",
		       evf->evf_id.ev_eqh.cookie);
		return -ENOENT;
	}

	spin_lock(&eq->eq_lock);
 again:
	ev = cfs_hash_lookup(eq->eq_hash, &evf->evf_id.ev_id);
	if (ev == NULL) {
		spin_unlock(&eq->eq_lock);
...
static int
daos_event_abort(struct daos_usr_param *uparam)
{
	struct daos_usr_ev_abort *eva = daos_usr_param_body(uparam);
	struct daos_kevent	 *ev;
	struct daos_eq		 *eq;

	eq = daos_eq_lookup(eva->eva_id.ev_eqh);
	if (eq == NULL)
		return -ENOENT;

	spin_lock(&eq->eq_lock);
	ev = cfs_hash_lookup(eq->eq_hash, &eva->eva_id.ev_id);
...
static int
daos_event_next(struct daos_usr_param *uparam)
{
	struct daos_usr_ev_next *evn = daos_usr_param_body(uparam);
	struct daos_kevent	*parent;
	...
	if (evn->evn_cursor.ev_eqh.cookie != 0 &&
	    evn->evn_cursor.ev_eqh.cookie != evn->evn_parent.ev_eqh.cookie)
		return -EINVAL;

	eq = daos_eq_lookup(evn->evn_parent.ev_eqh);
	if (eq == NULL)
		return -ENOENT;

	spin_lock(&eq->eq_lock);
	parent = cfs_hash_lookup(eq->eq_hash, &evn->evn_parent.ev_id);
	if (parent == NULL)
		GOTO(out, rc = -ENOENT);

	cur = cfs_hash_lookup(eq->eq_hash, &evn->evn_cursor.ev_id);
	if (cur == NULL)
		GOTO(out, rc = -ENOENT);
...
out:
	spin_unlock(&eq->eq_lock);
	if (ucur != NULL && copy_to_user(evn->evn_next, &ucur, sizeof(ucur)))
		rc = -EFAULT;
		    ...
		    void *kparam, daos_kevent_ops_t *ops)
{
	...
	ev = cfs_hash_lookup(eq->eq_hash, &eid.ev_id);
	if (ev == NULL) {
		CDEBUG(D_INFO, "Can't find event "LPU64" from EQ\n", eid.ev_id);
		goto out;
	}

	if (!daos_event_is_idle(ev) || !list_empty(&ev->ev_children) != 0) {
		CDEBUG(D_INFO, "Even is not idle(%d) or it has %d children\n",
		       !daos_event_is_idle(ev), !list_empty(&ev->ev_children));
		ev = NULL;
		goto out;
	}
...
daos_event_complete(struct daos_kevent *ev, int error)
{
	...
	if (parent != NULL) {
		...
		/* not unlinked, increase completion counter on parent */
		if (ev != NULL)
...
long
daos_eq_ioctl(unsigned int cmd, struct daos_usr_param *uparam)
{
	...
	case DAOS_IOC_EQ_CREATE:
		return daos_eq_create(uparam);

	case DAOS_IOC_EQ_DESTROY:
		return daos_eq_destroy(uparam);

	case DAOS_IOC_EQ_POLL:
		return daos_eq_poll(uparam);

	case DAOS_IOC_EQ_QUERY:
		return daos_eq_query(uparam);

	case DAOS_IOC_EV_INIT:
		return daos_event_init(uparam);

	case DAOS_IOC_EV_FINI:
		return daos_event_fini(uparam);

	case DAOS_IOC_EV_ABORT:
		return daos_event_abort(uparam);

	case DAOS_IOC_EV_NEXT:
		return daos_event_next(uparam);
	}
}
