...
static inline struct update_request *
...
void out_destroy_update_req(struct update_request *update)
{
	...
	LASSERT(cfs_list_empty(&update->ur_callback_list));
...
struct update_request *out_create_update_req(struct dt_device *dt)
{
	...
	CFS_INIT_LIST_HEAD(&update->ur_callback_list);
			...
			struct ptlrpc_request **reqp)
{
	...
	/* XXX: Check the comment for using MDC_REPLY_PORTAL as OST-side
	 *	OUT RPC reply portal in ost_setup(). It is some hack. We
...
int out_insert_update(const struct lu_env *env, struct update_request *update,
		      int op, struct lu_fid *fid, int count,
		      int *lens, char **bufs)
{
