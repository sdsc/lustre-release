...
static ssize_t update_buffer_resize(struct update_buffer *ubuf, size_t new_size)
{
	...
	CDEBUG(D_INFO, "resize update from %lu to %lu\n", ubuf->ub_req_size,
	       new_size);
		       ...
		       int params_count, __u16 *param_sizes, __u64 batchid)
{
	...
	for (i = 0; i < params_count; i++)
		update_length += cfs_size_round(param_sizes[i] + sizeof(*param));

	if (unlikely(cfs_size_round(ureq_size + update_length) >
		     ubuf->ub_req_size)) {
		size_t new_size = ubuf->ub_req_size;

		/* enlarge object update request size */
		while (new_size <
		       cfs_size_round(ureq_size + update_length))
