...
/**
 * This function packs update into the update_buffer header, which will be
 * either sent to the remote MDT or stored in the update log. The maxim
 * update buffer size is 1M for now.
 *
...
/**
 * This function packs update into the update_buffer, pack update header
 * and then insert all parameters.
 *
 * param[in] env:	execution environment
 * param[in] ubuf:	update bufer which it will pack the update in.
 * param[in] op:	update operation.
...
/**
 * The following functions insert different update into the update_buffer
 *
 * param[in] env:	execution environment.
...
int out_xattr_del_insert(const struct lu_env *env, const struct lu_fid *fid,
			 const char *name, struct update_buffer *ubuf,
			 __u64 batchid)
{
	int	size = strlen(name) + 1;

	return out_insert_update(env, ubuf, OUT_XATTR_DEL, fid,
				 1, &size, &name, batchid);
}
EXPORT_SYMBOL(out_xattr_del_insert);
			    ...
			    struct update_buffer *ubuf, __u64 batchid)
{
	...
	return out_insert_update(env, ubuf, OUT_INDEX_INSERT, fid,
				 ARRAY_SIZE(sizes), sizes, bufs, batchid);
}
