		...
		if (arg->u.write.buf.lb_len >= LLOG_MIN_REC_SIZE && arg->u.write.pos > (32768 + LLOG_MIN_REC_SIZE)) {
			...
			if (lur->lur_hdr.lrh_type == UPDATE_REC || lur->lur_hdr.lrh_type == LLOG_PAD_MAGIC) {
				...
				if (rc < 0) {
					...
					dt_attr_get(env, dt_obj, &attr);
					CERROR("read record "DFID" tail_pos %llu rc %d index %d size %d\n",
						PFID(lu_object_fid(&dt_obj->do_lu)), tail_pos, rc, lur->lur_hdr.lrh_index, (int)attr.la_size);
					LBUG();
				} else {
				...
				CDEBUG(D_HA, "write record "DFID" pos %llu index %u mdt_index %u length %d\n",
				       PFID(lu_object_fid(&dt_obj->do_lu)), arg->u.write.pos, lur->lur_hdr.lrh_index, lur->lur_hdr.lrh_id, (int)arg->u.write.buf.lb_len);
			}
		}
