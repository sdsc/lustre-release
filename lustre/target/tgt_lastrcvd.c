...
static int tgt_bitmap_chunk_alloc(struct lu_target *lut, int chunk)
{
	...
	OBD_ALLOC(bm, BITS_TO_LONGS(LUT_REPLY_SLOTS_PER_CHUNK));
	if (bm == NULL)
		return -ENOMEM;
...
repeat:
	...
	if (tgt_is_multimodrpcs_client(exp)) {
		 ...
		 * Lustre versions to mount a target with reply_data file */
		spin_lock(&tgt->lut_client_bitmap_lock);
		tgt->lut_num_clients++;
		if (tgt->lut_num_clients == 1) {
		...
		spin_unlock(&tgt->lut_client_bitmap_lock);
				 ...
				 unsigned long last_size)
{
	...
	/* record generation for next new client */
	atomic_set(&tgt->lut_client_generation, generation + 1);
