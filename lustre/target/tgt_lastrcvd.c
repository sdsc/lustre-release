 ...
 * These are common function for server recovery-related functionality
 *
...
int tgt_client_data_update(const struct lu_env *env, struct obd_export *exp)
{
	...
	rc = tgt_client_data_write(env, tg, ted->ted_lcd, &tti->tti_off, th);
	EXIT;
out:
...
int tgt_server_data_read(const struct lu_env *env, struct lu_target *tg)
{
	...
	tti->tti_off = 0;
	tti_buf_lsd(tti);
	rc = dt_record_read(env, tg->lut_last_rcvd, &tti->tti_buf,
			    &tti->tti_off);
	if (rc == 0)
			   ...
			   int sync)
{
	...
	rc = tgt_server_data_write(env, tg, th);
out:
	dt_trans_stop(env, tg->lut_bottom, th);
...
void tgt_boot_epoch_update(struct lu_target *lut)
{
	...
	cfs_spin_unlock(&lut->lut_obd->obd_recovery_task_lock);
	/** update server epoch */
	tgt_server_data_update(&env, lut, 1);
	lu_env_fini(&env);
}
...
int tgt_client_del(const struct lu_env *env, struct obd_export *exp)
{
	...
	rc = tgt_server_data_update(env, tg, 0);
	if (rc != 0) {
