		     ...
		     void *cookie)
{
	...
	if (tgt_is_multimodrpcs_client(tsi->tsi_exp)) {
		/*
		 * Use maximum possible file offset for dt_declare_record_write()
		 * to ensure ZFS will reserve enough credits for a write anywhere
		 * in this file, since we don't know where in the file the write
		 * will be because a replay slot has not been assigned.  This
		 * should be replaced by dmu_tx_hold_append() when available.
		 */
