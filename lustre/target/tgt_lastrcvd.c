			  ...
			  unsigned long last_size)
{
	...
	for (cl_idx = 0; off < last_size; cl_idx++) {
		...
		last_transno = max(lcd->lcd_last_transno,
		...
		*ted->ted_lcd = *lcd;
		...
		spin_lock(&tgt->lut_translock);
		tgt->lut_last_transno = max(last_transno, tgt->lut_last_transno);
		spin_unlock(&tgt->lut_translock);
	}
...
static struct server_compat_data tgt_scd[] = {
	...
	[LDD_F_SV_TYPE_OST] = {
		...
		.incompat = OBD_INCOMPAT_OST | OBD_INCOMPAT_COMMON_LR |
			    OBD_INCOMPAT_GROUPS | OBD_INCOMPAT_FID,
		.rocinit = 0,
		     ...
		     void *cookie)
{
	...
	/* ECHO client case */
	if (tgt_ses_req(tsi) == NULL || tsi->tsi_exp == NULL)
		return 0;
