...
void update_records_dump(struct update_records *records, unsigned int mask)
{
	...
	for (i = 0; i < ops->uo_count; i++) {
		CDEBUG(mask, "%d: "DFID" %s: p1:%d p2:%d p3:%d\n",
		       i, PFID(&ops->uo_op[i].uop_fid),
		       update_op_str(ops->uo_op[i].uop_type),
		       (int)ops->uo_op[i].uop_params_off[0],
		       (int)ops->uo_op[i].uop_params_off[1],
		       (int)ops->uo_op[i].uop_params_off[2]);
	}
	return;
}
...
int update_records_update_pack(const struct lu_env *env,
			       ...
			       int *max_op_size,
			       struct update_params *params,
			       int *max_param_size, int params_count,
			       const char **params_bufs,
			       int *params_size)
{
	struct update_op	*op;
	int			params_length = 0;
	...
	if (unlikely(*max_param_size < params_length)) {
		CDEBUG(D_INFO, "max_param_size %d params size %d\n",
		       *max_param_size, params_length);
		*max_param_size = params_length;
		return -E2BIG;
	}

	LASSERTF(params_count <= MAX_UPDATE_PARAMS, "params_count is %d\n",
		 params_count);

	op = update_ops_get_op(ops, ops->uo_count);
	op->uop_fid = *fid;
	...
	for (i = 0; i < params_count; i++) {
		...
		CDEBUG(D_INFO, "pack %s params %d:%d\n",
...
int update_records_attr_set_pack(const struct lu_env *env,
				 struct update_ops *ops, int *max_ops_size,
				 struct update_params *params,
				 int *max_param_size,
				 const struct lu_fid *fid,
				 const struct lu_attr *attr)
