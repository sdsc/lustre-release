			 ...
			 unsigned int mask)
{
	...
	CDEBUG(mask, "master transno = "LPU64" batchid = "LPU64" flags = %x"
	       ...
	       records->ur_batchid, records->ur_flags, ops->uops_count,
	...
	for (i = 0; i < ops->uops_count; i++) {
		...
		for (j = 0;  j < op->uop_params_count; j++) {
			...
			LASSERT(param != NULL);
			CDEBUG(mask, "param = %p %dth offset = %hu size = %hu\n",
			       param, j, op->uop_params_off[j],
			       param->oup_len);
		}
				      ...
				      size_t *params_size)
{
	...
	if (unlikely(*max_op_size < update_op_size(params_count))) {
		CDEBUG(D_INFO, "max_op_size = %zu update_op = %zu\n",
		       *max_op_size, update_op_size(params_count));
		*max_op_size = update_op_size(params_count);
		return -E2BIG;
	}
	...
	for (i = 0; i < params_count; i++) {
		...
		CDEBUG(D_INFO, "%s %uth param offset = %d size = %zu\n",
		       update_op_str(op_type), i, index, params_size[i]);

		op->uop_params_off[i] = index;
	}

	CDEBUG(D_INFO, "%huth "DFID" %s params_count = %d\n",
	       ops->uops_count, PFID(fid),
	       update_op_str(op_type), params_count);
		      ...
		      size_t new_op_size, size_t new_param_size)
{
	...
	if (new_op_size > (tur->tur_update_records_size - ops_size -
			   sizeof(*tur->tur_update_records))) { 

		extend_size = 0;
		while (extend_size < new_op_size)
			extend_size += UPDATE_RECORDS_BUFFER_SIZE;
	...
	if (new_param_size > (tur->tur_update_params_size -
			      params_size)) {

		extend_size = 0;
		while (extend_size < new_param_size)
			extend_size += UPDATE_PARAMS_BUFFER_SIZE;
...
int merge_params_updates_buf(const struct lu_env *env, struct thandle *th)
{
	...
	params_size = update_params_size(tur->tur_update_params);
	ops_size = update_ops_size(&tur->tur_update_records->ur_ops);
	if (sizeof(*tur->tur_update_records) + ops_size + params_size >
	    tur->tur_update_records_size) {
		int rc;
