			     ...
			     struct tx_arg *arg)
{
	...
	CDEBUG(D_INFO, "%s: write buf %p len %ld "DFID" pos "LPU64": rc = %d\n",
	       dt_obd_name(th->th_dev), arg->u.write.buf.lb_buf,
	       arg->u.write.buf.lb_len, PFID(lu_object_fid(&dt_obj->do_lu)),
	       arg->u.write.pos, rc);
