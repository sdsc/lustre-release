...
int out_tx_end(const struct lu_env *env, struct thandle_exec_args *ta)
{
	...
	tsi->tsi_reply_fail_id = OBD_FAIL_OBJECT_UPDATE_NET_REP;
stop:
...
static int out_create(struct tgt_session_info *tsi)
{
	...
	if (S_ISDIR(attr->la_mode) && update->ou_lens[1] > 0) {
		int size;
...
out:
	object_update_result_insert(reply, lbuf->lb_buf, lbuf->lb_len, 0, rc);
	RETURN(rc);
}
...
TGT_UPDATE_HDL(MUTABOR,	OBJECT_UPDATE,	out_handle),
};
