	...
	if (unlikely(rc != 0 &&
		...
		if (rc == -ENOSPC) {
			...
			lfsck_pack_rfa(lr, lu_object_fid(&dt_obj->do_lu),
				       th, LE_SKIP_NLINK, LFSCK_TYPE_NAMESPACE);
			tgt_lfsck_in_notify(env,
				tgt_ses_info(env)->tsi_tgt->lut_bottom, lr);
		}
			      ...
			      int index, const char *file, int line)
{
	...
	if (strcmp(name, XATTR_NAME_LINK) == 0) {
		...
		lfsck_pack_rfa(lr, lu_object_fid(&dt_obj->do_lu), ta->ta_handle,
				    ...
				    struct thandle *th, struct tx_arg *arg)
{
	...
	if (unlikely(dt_object_exists(dt_obj) == 0))
		RETURN(-ESTALE);
