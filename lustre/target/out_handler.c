...
static int out_attr_get(struct tgt_session_info *tsi)
{
	...
	if (unlikely(update->ou_result_size << 3 < sizeof(*obdo)))
		return -EPROTO;
...
static int out_xattr_get(struct tgt_session_info *tsi)
{
	...
	if (unlikely(tti->tti_u.update.tti_update->ou_result_size == 0))
		return -EPROTO;
...
static int out_index_lookup(struct tgt_session_info *tsi)
{
	...
	if (unlikely(update->ou_result_size << 3 < sizeof(tti->tti_fid1)))
		return -EPROTO;
...
int out_handle(struct tgt_session_info *tsi)
{
	...
	for (i = 0; i < update_buf_count; i++) {
		...
		for (j = 0; j < our->ourq_count; j++) {
			...
			reply_size += update->ou_result_size << 3;
		}
 	}
