			     ...
			     struct llog_cookie *cookie)
{
	...
	LASSERT(records->ur_ops.uops_count > 1);
...
static bool top_check_write_updates(struct top_thandle *top_th)
{
	...
	if (tur->tur_update_records == NULL ||
	    tur->tur_update_records->ur_ops.uops_count <= 1)
		return false;
...
stop_master_trans:
	...
	if (master_st != NULL && !master_st->st_committed) {
		LASSERT(master_st->st_sub_th != NULL);
		...
		} else if (tur->tur_update_records->ur_master_transno == 0) {
...
static int distribute_txn_commit_thread(void *_arg)
{
	...
	while (distribute_txn_commit_thread_running(lut)) {
		...
		CDEBUG(D_INFO, "%s: batchid: "LPU64" committed batchid "
	...
	list_for_each_entry_safe(tmt, tmp, &list, tmt_commit_list) {
		...
		list_for_each_entry(st, &tmt->tmt_sub_trans_list, st_list) {
			if (!st->st_committed) {
				LASSERT(atomic_read(&tmt->tmt_refcount) > 1);
				top_multiple_thandle_put(tmt);
			}
		}
		top_multiple_thandle_put(tmt);
