		   ...
		   struct thandle *th)
{
	...
	if (top_th->tt_update_records != NULL && !top_th->tt_master_committed) {
		struct thandle_update_records *tur;
...
stop_master_trans:
	...
	if (!top_th->tt_master_committed) {
		...
		dt_txn_hook_commit(top_th->tt_child);
		sub_trans_commit_cb(top_th->tt_child);
...
stop_other_trans:
	...
	/* If it is NOT multiple node transaction, the top transaction will
	 * be destroyed here. */
	top_thandle_put(top_th);
