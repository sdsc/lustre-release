...
void top_thandle_dump(unsigned int mask, struct top_thandle *top_th)
{
	...
	CDEBUG(mask, "top_handle %p ref %d child %p update_records %p"
	       ...
	       top_th->tt_update_records, top_th->tt_child->th_committed,
	...
	list_for_each_entry(st, &top_th->tt_sub_trans_list, st_list) {
		CDEBUG(mask, "st %p obd %s committed %d\n", st,
		       ...
		       st->st_sub_th ? st->st_sub_th->th_committed : 0);
		   ...
		   struct thandle *th)
{
	...
	/* Step 1: Preparing the update logs for distribute transaction. */
	if (top_th->tt_update_records != NULL &&
	    !top_th->tt_child->th_committed) {
		struct thandle_update_records *tur;

		tur = top_th->tt_update_records;

		/* Merge the parameters and updates into one buffer */
		rc = prepare_writing_updates(env, top_th->tt_update_records);
		if (rc < 0) {
...
stop_master_trans:
	...
	if (top_th->tt_update_records != NULL && th->th_result == 0) {
...
stop_other_trans:
	...
	list_for_each_entry(st, &top_th->tt_sub_trans_list, st_list) {
		/* Released when sub thandle is committed */
		if (top_th->tt_multiple_node)
			top_thandle_get(top_th);
		...
		if (likely(!st->st_sub_th->th_committed)) {
			/* Release it when this sub thandle is committed. */
			if (top_th->tt_multiple_node)
				top_thandle_get(top_th);
	 ...
	 * top_trans_committed_cb directly */
	if (all_committed && top_th->tt_multiple_node)
...
void sub_trans_commit_cb(struct thandle *sub_th)
{
	...
	if (th_abort) {
		 ...
		 * Note: we need two put here, one for holding master sub th,
		 * one for holding top_th see (top_trans_stop()). */
		top_thandle_put(top_th);
		top_thandle_put(top_th);
		RETURN_EXIT;
	}
	...
	top_thandle_put(top_th);
	if (all_committed)
		/* The final put will be done after the update log is being
		 * cancelled */
		top_trans_committed_cb(&top_th->tt_super);
