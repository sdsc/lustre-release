				      ...
				      __u32 mask)
{
	...
	CDEBUG(mask, "%s tmt %p refcount %d committed %d\n",
	...
	list_for_each_entry(st, &tmt->tmt_sub_trans_list, st_list) {
		CDEBUG(mask, "st %p obd %s committed %d stop %d sub_th %p "
		       " cookie "DOSTID"\n",
				   ...
				   struct thandle_update_records *tur)
{
	...
	tur->tur_update_records->ur_batchid = tmt->tmt_batchid;
...
static void top_trans_committed_cb(struct top_multiple_thandle *tmt)
{
	...
	tmt->tmt_committed = 1;
	lut = dt2lu_dev(tmt->tmt_master_sub_dt)->ld_site->ls_target;
				       ...
				       struct dt_device *dt_dev)
{
	...
	list_add(&st->st_list, &tmt->tmt_sub_trans_list);
	return st;
}
				    ...
				    struct top_multiple_thandle *tmt)
{
	...
	st->st_dcb.dcb_func = sub_trans_commit_cb;
	st->st_dcb.dcb_data = tmt;
	INIT_LIST_HEAD(&st->st_dcb.dcb_linkage);
	dt_trans_cb_add(st->st_sub_th, &st->st_dcb);
}
...
top_trans_create(const struct lu_env *env, struct dt_device *master_dev)
{
	...
	parent_th->th_top = &top_th->tt_super;
 ...
 *
 */
void distribute_txn_assign_batchid(struct top_multiple_thandle *tmt)
{
	struct target_distribute_txn_data *tdtd;
	struct dt_device *dt = tmt->tmt_master_sub_dt;

	LASSERT(dt != NULL);
	tdtd = dt2lu_dev(dt)->ld_site->ls_target->lut_tdtd;
	spin_lock(&tdtd->tdtd_batchid_lock);
	tmt->tmt_batchid = tdtd->tdtd_batchid++;
 ...
 */
static void distribute_txn_insert_by_batchid(struct top_multiple_thandle *new)
{
		   ...
		   struct thandle *th)
{
	...
	if (tur != NULL && tur->tur_update_records != NULL &&
	    !master_st->st_committed && th->th_result == 0) {
		/* Merge the parameters and updates into one buffer */
		rc = prepare_writing_updates(env, tmt, tur);
		if (rc < 0) {
...
stop_master_trans:
	...
	if (master_st->st_sub_th != NULL) {
		top_multiple_thandle_get(tmt);
		rc = dt_trans_stop(env, master_dev, master_st->st_sub_th);
		if (rc < 0)
			top_multiple_thandle_put(tmt);

		/* Update master transno after master stop callback */
		if (tur->tur_update_records->ur_master_transno == 0)
			tur->tur_update_records->ur_master_transno =
							th->th_transno;
	}

	/* Insert the distribute txn into the list */
	...
	if (tur != NULL && tur->tur_update_records != NULL &&
		...
		update_records_dump(tur->tur_update_records, D_HA);
		list_for_each_entry(st, &tmt->tmt_sub_trans_list, st_list) {
			if (st->st_committed || st == master_st)
				continue;
...
stop_other_trans:
	...
	list_for_each_entry(st, &tmt->tmt_sub_trans_list, st_list) {
		...
		rc = dt_trans_stop(env, st->st_sub_th->th_dev,
				   st->st_sub_th);
		if (rc < 0)
			/* balance for create_sub_thandle */
			st->st_sub_th = NULL;
				...
				struct top_thandle *top_th)
{
	...
	INIT_LIST_HEAD(&tmt->tmt_sub_trans_list);
				...
				const struct dt_object *sub_obj)
{
	...
	/* If the sub device (OSP for OST) does not need a thandle to track
	 * updates, then return tt_child */
	if (sub_th == NULL)
		RETURN(top_th->tt_child);
	...
	if (top_th->tt_multiple_thandle == NULL) {
		...
		tmt->tmt_master_sub_dt = top_th->tt_child->th_dev;
...
void top_multiple_thandle_destroy(struct top_multiple_thandle *tmt)
{
	...
	LASSERT(tmt->tmt_magic == TOP_THANDLE_MAGIC);
	list_for_each_entry_safe(st, tmp, &tmt->tmt_sub_trans_list,
				 st_list) {
		list_del(&st->st_list);
		OBD_FREE_PTR(st);
	}
...
static inline int
distribute_txn_commit_thread_running(struct target_distribute_txn_data *tdtd)
{
	return tdtd->tdtd_commit_thread.t_flags & SVC_RUNNING;
}

static inline int
distribute_txn_commit_thread_stopped(struct target_distribute_txn_data *tdtd)
{
	return tdtd->tdtd_commit_thread.t_flags & SVC_STOPPED;
}
					 ...
					 struct top_multiple_thandle *tmt)
{
	...
	list_for_each_entry(st, &tmt->tmt_sub_trans_list, st_list) {
		...
		CDEBUG(D_HA, "%s: cancel update log "DOSTID": rc = %d\n",
		       obd->obd_name, POSTID(&cookie->lgc_lgl.lgl_oi), rc);
	}
			      ...
			      __u64 batchid)
{
	...
	if (rc < 0)
		GOTO(stop, rc);
	rc = dt_trans_start(env, tdtd->tdtd_lut->lut_bottom, th);
	if (rc < 0)
		GOTO(stop, rc);
...
static int distribute_txn_commit_thread(void *_arg)
{
	...
	struct ptlrpc_thread	*thread = &tdtd->tdtd_commit_thread;
	...
	while (distribute_txn_commit_thread_running(tdtd)) {
		spin_lock(&tdtd->tdtd_batchid_lock);
		list_for_each_entry_safe(tmt, tmp, &tdtd->tdtd_list,
					 tmt_commit_list) {
			if (tmt->tmt_committed == 0)
				break;
			LASSERT(tmt->tmt_batchid >= batchid);
		...
		l_wait_event(tdtd->tdtd_commit_thread_waitq,
			     !distribute_txn_commit_thread_running(tdtd) ||
			     committed < tdtd->tdtd_committed_batchid ||
			...
			__u32 index)
{
	...
	tdtd->tdtd_batchid = lut->lut_last_transno;

	init_waitqueue_head(&tdtd->tdtd_commit_thread_waitq);
	init_waitqueue_head(&tdtd->tdtd_commit_thread.t_ctl_waitq);
	...
	l_wait_event(tdtd->tdtd_commit_thread.t_ctl_waitq,
		     distribute_txn_commit_thread_running(tdtd) ||
		     distribute_txn_commit_thread_stopped(tdtd), &lwi);
	RETURN(0);
}
...
{
	/* Stop cancel thread */
	if (!distribute_txn_commit_thread_running(tdtd))
		return;

	spin_lock(&tdtd->tdtd_batchid_lock);
	tdtd->tdtd_commit_thread.t_flags = SVC_STOPPING;
	spin_unlock(&tdtd->tdtd_batchid_lock);
	wake_up(&tdtd->tdtd_commit_thread_waitq);
	wait_event(tdtd->tdtd_commit_thread.t_ctl_waitq,
		   tdtd->tdtd_commit_thread.t_flags & SVC_STOPPED);
