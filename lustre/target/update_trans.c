			     ...
			     struct sub_thandle *sub_th)
{
	...
	do {
		...
		rec_len = (unsigned long)next - (unsigned long)src;
		/* If its size > llog chunk_size, then Write current chunk to
		 * the update llog. Note: it also needs to leave some space
		 * for pad rec. (see llog_osd_pad()) */
		if (cfs_size_round(lur->lur_hdr.lrh_len + rec_len) +
