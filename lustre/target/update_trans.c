				      ...
				      __u32 mask)
{
	...
	LASSERTF(tmt->tmt_magic == TOP_THANDLE_MAGIC, "magic is %x", tmt->tmt_magic);
					 ...
					 struct thandle *sub_th, int err)
{
	...
	LASSERTF(atomic_read(&tmt->tmt_refcount) > 1, "refcount is %d\n", atomic_read(&tmt->tmt_refcount));
	top_multiple_thandle_dump(tmt, D_INFO);
...
stop_master_trans:
	...
	if (master_st != NULL && master_st->st_sub_th != NULL) {
		...
		if (!master_st->st_started && !list_empty(&tmt->tmt_commit_list))
