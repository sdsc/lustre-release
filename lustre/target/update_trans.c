				     ...
				     struct thandle *sub_th)
{
	...
	record->lur_hdr.lrh_len = LLOG_CHUNK_SIZE;
				   ...
				   struct top_multiple_thandle *tmt)
{
	...
	if (update_size + params_size >
	    tur->tur_update_records_buf_size) {
		int rc;

		rc = tur_update_records_extend(tur, update_size + params_size);
		if (rc < 0)
			return rc;
	...
	update_records_dump(&lur->lur_update_rec, D_INFO);
		   ...
		   struct thandle *th)
{
	...
	if (master_st != NULL && master_st->st_sub_th != NULL &&
		...
		lur = tur->tur_update_records;
		update_records_dump(&lur->lur_update_rec, D_HA);
...
stop_master_trans:
	...
	if (write_updates) {
		struct llog_update_record *lur = tur->tur_update_records;
