			       ...
			       cksum_type_t server_cksum_type)
{
	...
	cksum_type = cksum_type_unpack(oa->o_valid & OBD_MD_FLFLAGS ?
				       oa->o_flags : 0);
	new_cksum = tgt_checksum_bulk(desc, OST_READ, server_cksum_type);
	...
	LCONSOLE_ERROR_MSG(0x132, "BAD READ CHECKSUM: %s: from %s inode "DFID
		...
		((__u64)BD_GET_KIOV(desc, bd_iov_count - 1).kiov_page->index
			<< PAGE_SHIFT) +
			BD_GET_KIOV(desc, bd_iov_count - 1).kiov_len - 1);
