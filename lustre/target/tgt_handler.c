 ...
 * Unified target BAST
 *
 * Ensure data and metadata are synced to disk when lock is canceled
 * (if requested). If it's extent lock, normally sync obj is enough, but if it's
 * cross-MDT lock, because remote object version is not set, a filesystem sync
 * is needed.
 *
