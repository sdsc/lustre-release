			       ...
			       cksum_type_t cksum_type)
{
	...
	for (i = 0; i < desc->bd_iov_count; i++) {
		...
		if (i == 0 && opc == OST_WRITE &&
			...
			int off = desc->bd_u.bd_kiov.bd_vec[i].kiov_offset &
			  ~CFS_PAGE_MASK;
			int len = desc->bd_u.bd_kiov.bd_vec[i].kiov_len;
			struct page *np = tgt_page_to_corrupt;
			char *ptr =
			  kmap(desc->bd_u.bd_kiov.bd_vec[i].kiov_page) + off;
			...
			if (np) {
				...
				kunmap(np);
				desc->bd_u.bd_kiov.bd_vec[i].kiov_page = np;
			} else {
		...
		cfs_crypto_hash_update_page(hdesc,
				  desc->bd_u.bd_kiov.bd_vec[i].kiov_page,
				  desc->bd_u.bd_kiov.bd_vec[i].kiov_offset &
					~CFS_PAGE_MASK,
				  desc->bd_u.bd_kiov.bd_vec[i].kiov_len);
...
int tgt_brw_read(struct tgt_session_info *tsi)
{
	...
	desc = ptlrpc_prep_bulk_exp(req, npages, ioobj_max_brw_get(ioo),
				    BULK_PUT_SOURCE | BULK_OP_ACTIVE,
				    OST_BULK_PORTAL);
	if (desc == NULL)
