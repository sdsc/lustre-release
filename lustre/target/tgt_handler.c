...
static int check_read_checksum(struct tgt_session_info *tsi,
			       struct ptlrpc_bulk_desc *desc, struct obdo *oa,
			       ...
			       cksum_type_t server_cksum_type)
{
	...
	cksum_type = cksum_type_unpack(oa->o_valid & OBD_MD_FLFLAGS ?
				       oa->o_flags : 0);
	new_cksum = tgt_checksum_bulk(tsi->tsi_tgt, desc, OST_READ,
				      server_cksum_type);
...
int tgt_brw_read(struct tgt_session_info *tsi)
{
	...
	if (body->oa.o_valid & OBD_MD_FLCKSUM) {
		...
		if (body->oa.o_cksum)
			check_read_checksum(tsi, desc, &body->oa, &req->rq_peer,
					    body->oa.o_cksum,
