		 ...
		 struct lustre_handle *lh, int mode)
{
	...
	if (nrbufs == 0 || !(nb->rnb_flags & OBD_BRW_SRVLOCK))
		RETURN(0);

	for (i = 1; i < nrbufs; i++)
		if (!(nb->rnb_flags & OBD_BRW_SRVLOCK))
			RETURN(-EFAULT);

	RETURN(tgt_extent_lock(ns, res_id, nb->rnb_offset,
			       nb->rnb_offset + nb->rnb_len - 1,
			       lh, mode, &flags));
}
		    ...
		    struct lustre_handle *lh, int mode)
{
	...
	LASSERT(mode == LCK_PR || mode == LCK_PW);
	LASSERT((obj->ioo_bufcnt > 0 && (niob->rnb_flags & OBD_BRW_SRVLOCK)) ==
...
int tgt_brw_read(struct tgt_session_info *tsi)
{
	...
	for (i = 0; i < npages; i++) {
		...
		if (page_rc != 0) { /* some data! */
			LASSERT(local_nb->lnb_page != NULL);
			ptlrpc_prep_bulk_page_nopin(desc, local_nb->lnb_page,
						    local_nb[i].lnb_page_offset,
						    page_rc);
		}

		if (page_rc != local_nb->lnb_len) { /* short read */
			/* All subsequent pages should be 0 */
			while (++i < npages)
				LASSERT(local_nb->lnb_rc == 0);
			break;
		}
	}
...
int tgt_brw_write(struct tgt_session_info *tsi)
{
	...
	if ((remote_nb->rnb_flags & OBD_BRW_MEMALLOC) &&
...
skip_transfer:
	...
	if (rc == 0) {
		...
		for (i = j = 0; i < niocount; i++) {
			int len = remote_nb->rnb_len;

			nob += len;
			rcs[i] = 0;
			do {
				LASSERT(j < npages);
				if (local_nb->lnb_rc < 0)
					rcs[i] = local_nb->lnb_rc;
				len -= local_nb->lnb_len;
				j++;
			} while (len > 0);
			LASSERT(len == 0);
		}
