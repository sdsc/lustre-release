...
static int tgt_ost_body_unpack(struct tgt_session_info *tsi, __u32 flags)
{
	...
	body->oa.o_uid = nodemap->map(nodemap, NODEMAP_UID,
				      NODEMAP_CLIENT_TO_FS, body->oa.o_uid);
	body->oa.o_gid = nodemap->map(nodemap, NODEMAP_GID,
				      NODEMAP_CLIENT_TO_FS, body->oa.o_gid);
