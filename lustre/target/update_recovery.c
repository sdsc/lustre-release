			...
			struct distribute_txn_replay_req *new)
{
	...
	list_for_each_entry_reverse(iter, &tdtd->tdtd_replay_list, dtrq_list) {
		...
		if (iter->dtrq_updates->ur_master_transno ==
		    new->dtrq_updates->ur_master_transno &&
		    iter->dtrq_updates->ur_batchid >
		    new->dtrq_updates->ur_batchid)
			continue;
				     ...
				     __u32 mdt_index)
{
	...
	update_records_dump(record, D_HA);
				  ...
				  struct thandle *th)
{
	...
	wobdo = update_params_get_param_buf(params,
					   op->uop_params_off[0],
					   &size);
	if (wobdo == NULL)
		RETURN(-EIO);
	...
	if (LLOG_REC_HDR_NEEDS_SWABBING(&uti->uti_update_rec->ur_hdr))
				 ...
				 struct distribute_txn_replay_req *dtrq)
{
	...
	update_env_info(env)->uti_update_rec = records;
