 ...
 */
static struct update_recovery_header *
update_recovery_header_create_insert(struct update_recovery_data *urd,
				     struct update_records *record)
{
	...
	if (urh == NULL) {
		update_recovery_header_insert(urd, new);
					   ...
					   __u32 index)
{
	...
	spin_unlock(&urd->urd_list_lock);
	if (urh == NULL) {
		urh = update_recovery_header_create_insert(urd, record);
		if (IS_ERR(urh))
			RETURN(PTR_ERR(urh));
					...
					struct thandle *sub_th)
{
	...
	/* Check if the sub thandle is already committed */
	if (sub_thandle_is_committed(top_th, sub_th))
		RETURN(0);

	/* Get mdt index of the update */
	...
	/* Lookup the update recovery in the header list */
	ur = update_recovery_lookup(urh, mdt_index);
	if (ur == NULL) {
		...
		/* If the object is on the master MDT of the transaction,
		 * the update log might already been destoryed, see recovery
		 * description in update_recovery.c. So compare the transno
		 * in the update log with the last committed transno on the
		 * master MDT. If transno(in update log) > last committed
		 * transno, then replay the update, otherwise only mark the
		 * transaction to been committed. */
		if (mdt_index == seq_site->ss_node_id &&
		    urh->urh_updates->ur_batchid != 0 &&
		    urh->urh_updates->ur_batchid <=
	...
	/* Mark the sub thandle is committed */
	if (top_th->tt_child == sub_th) {
		top_th->tt_child->th_committed = 1;
		top_th->tt_child_committed = 1;
		top_th->tt_master_cookie = ur->ur_llog_cookie;
	} else {
		st = lookup_sub_thandle(&top_th->tt_super, sub_th->th_dev);
		LASSERT(st != NULL);
