					  ...
					  struct thandle *th)
{
	...
	for (i = 0; i < ops->uops_count; i++, op = update_op_next_op(op)) {
		...
		/* Note: we need lock all of objects of the operation even
		 * if the update for the object has been committed, so make
		 * sure no other process interfer redoing the operation. */
