 ...
 *
 * Create recovery update header and put it the update recovery list.
 *
 * \param[in] urd	update_recovery_data where update recovery list is.
 * \param[in] record    update records to be added.
 *
...
/**
 * Check if the update needs to be replayed
 *
 * Check whether the updates needs to be replay on the target. It will
 * check if the updates has been committed on disk by checking whether
 * the update recovery exists in the list of update_recovery_header.
 *
 * \param[in] env	execution environment
 * \param[in] dt	the dt device of the recovery, usually it is LOD.
 * \param[in] urh	the update recovery header which holds the
 *                      updates
 * \param[in] fid       the fid of the update, which is used to find out
 *                      which MDT the updates will be replayed.
 *
					...
					struct thandle *th, bool declare)
{
	...
	fid = update_params_get_param_buf(params,
					  op->uop_params_off[1], &size);
	if (fid == NULL)
		RETURN(-EIO);
	if (size != sizeof(*fid))
	...
	if (declare)
		rc = dt_declare_insert(env, dt_obj,
				       (const struct dt_rec *)&uti->uti_fid,
					...
					struct list_head *lock_list)
{
	...
	list_for_each_entry_safe(urlh, tmp, lock_list, urlh_list) {
		ldlm_lock_decref(&urlh->urlh_lh, urlh->urlh_mode);
					  ...
					  struct thandle *th)
{
	...
	struct update_params	*params = &records->ur_params;
	unsigned int		i;
	int			rc = 0;
	...
	for (i = 0; i < ops->uops_count; i++) {
		...
		sub_dt_dev = lu2dt_dev(sub_dt_obj->do_lu.lo_dev);
		...
		/* Check if it needs to replay updates on the MDT.*/
		rc = update_recovery_is_needed(env, urd->urd_dt, urh, fid, &ur);
		if (rc < 0) {
			lu_object_put(env, &dt_obj->do_lu);
			GOTO(out, rc);
		}

		/* If it does not need replay, just mark the committed flag and
		 * remember the update record cookie. */
		if (rc == 0) {
			st = lookup_sub_thandle(th, sub_dt_dev);
			LASSERT(st != NULL);
			LASSERT(st->st_update != NULL);
			LASSERT(ur != NULL);
			st->st_committed = 1;
			st->st_update->stu_cookie = ur->ur_llog_cookie;
			lu_object_put(env, &dt_obj->do_lu);
			continue;
		}
			   ...
			   struct update_recovery_header *urh)
{
	...
	tur->tur_update_params = update_records_get_params(records);
	...
	for (i = 0; i < ops->uops_count; i++) {
		...
		st = lookup_sub_thandle(th, sub_dt_dev);
		LASSERT(st != NULL);
		/* skip if it is already committed */
		if (st->st_committed) {
			lu_object_put(env, &dt_obj->do_lu);
			continue;
		}
