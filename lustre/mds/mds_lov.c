                             ...
                             obd_id idx)
{
        ...
        if (data[off] < 2) {
                ...
                rc = obd_get_info(NULL, lov_exp, sizeof(KEY_LAST_ID), KEY_LAST_ID,
                                  &size, &lastid, NULL);
                if (rc)
                        GOTO(out, rc);
                               ...
                               struct obd_uuid *uuid)
{
        ...
        rc = obd_get_info(NULL, mds->mds_lov_exp, sizeof(KEY_LOVDESC), KEY_LOVDESC,
                          &valsize, ld, NULL);
        if (rc)
                GOTO(out, rc);
...
static int mds_propagate_capa_keys(struct mds_obd *mds, struct obd_uuid *uuid)
{
        ...
        for (i = 0; i < 2; i++) {
                ...
                info.capa = key;
                rc = obd_set_info_async(NULL, mds->mds_lov_exp, sizeof(KEY_CAPA_KEY),
