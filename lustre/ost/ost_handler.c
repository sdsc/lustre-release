...
static struct ost_thread_local_cache *ost_tls_get(struct ptlrpc_request *r)
{
        ...
        if (unlikely(tls == NULL)) {
                ...
                if (tls != NULL) {
			tls->olc_temporary = 1;
			if (unlikely(ost_integrity_alloc(&tls->olc_integrity))){
                                OBD_FREE_PTR(tls);
                                tls = NULL;
                        }
...
static void ost_tls_put(struct ptlrpc_request *r)
{
	...
	if (unlikely(tls->olc_temporary)) {
		ost_integrity_free(&tls->olc_integrity);
                OBD_FREE_PTR(tls);
...
static int ost_brw_read(struct ptlrpc_request *req, struct obd_trans_info *oti)
{
	...
	integrity = &tls->olc_integrity;
	rc = ost_integrity_init(integrity, (body->oa.o_valid & OBD_MD_FLCKSUM) ?
				body->oa.o_flags : 0);
	if (rc < 0) {
		CERROR("%s: bad integrity data %llx\n", exp->exp_obd->obd_name,
			(long long)body->oa.o_flags);
		GOTO(out, rc);
	}
	...
	if (integrity->type != 0) {
		...
		if (rc < 0) {
			CERROR("%s: integrity prep failed with %d\n",
	...
	if (integrity->type == OBD_CKSUM_T10B)
        ...
        if (body->oa.o_valid & OBD_MD_FLCKSUM && !integrity->type) {
                cksum_type_t cksum_type =
...
static int ost_brw_write(struct ptlrpc_request *req, struct obd_trans_info *oti)
{
	...
	integrity = &tls->olc_integrity;
	ost_integrity_init(integrity, body->oa.o_valid & OBD_MD_FLCKSUM ?
			   body->oa.o_flags : 0);
	if (rc < 0) {
		CERROR("%s: bad integrity data %llx\n",
...
skip_transfer:
        if (client_cksum != 0 && rc == 0 && !integrity->type) {
...
static int ost_setup(struct obd_device *obd, struct lustre_cfg* lcfg)
{
	...
	if (oss_large_buffers)
		extra_buf = PTLRPC_MAX_BRW_SIZE >> (9 - 1);
	...
	svc_conf = (typeof(svc_conf)) {
		...
		.psc_buf		= {
			.bc_nbufs		= OST_NBUFS,
			.bc_buf_size		= OST_IO_BUFSIZE + extra_buf,
			.bc_req_max_size	= OST_IO_MAXREQSIZE + extra_buf,
			.bc_rep_max_size	= OST_IO_MAXREPSIZE + extra_buf,
