		       ...
		       cfs_list_t *locked)
{
	...
	rc = ost_lock_get(exp, oa, begin, end, &region->lh, LCK_PR, 0);
	if (rc)
		return rc;
			     ...
			     cfs_list_t *locked)
{
	...
	if (begin < OBD_OBJECT_EOF) {
		CDEBUG(D_OTHER, "ost lock [%llu,%llu]\n",
		       begin, OBD_OBJECT_EOF);
		rc = lock_region(exp, oa, begin, OBD_OBJECT_EOF, locked);
	}
...
static int ost_get_info(struct obd_export *exp, struct ptlrpc_request *req)
{
	...
	/* LU-3219: to prevent data corruption due unflashed data
	 * we lock here ranges, that have no data. Probably, this ranges
	 * have unflashed data on clients. Also we call obd_get_info
	 * once again after this rages locked. */
	if (KEY_IS(KEY_FIEMAP)) {
