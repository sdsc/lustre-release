		...
		if (i == 0 && opc == OST_WRITE &&
			...
			if (np) {
				char *ptr = kmap_atomic(local_nb[i].page,
							KM_USER0);
				char *ptr2 = cfs_page_address(np);

				memcpy(ptr2 + off, ptr + off, len);
				memcpy(ptr2 + off, "bad3", min(4, len));
				kunmap_atomic(ptr, KM_USER0);
				cfs_crypto_hash_update_page(hdesc, np, off, len);
				continue;
			} else {
...
static int ost_brw_read(struct ptlrpc_request *req, struct obd_trans_info *oti)
{
	...
	int no_reply = 0, npages_read, fail = 0;
        struct ost_thread_local_cache *tls;
	unsigned char fail_backup[4];
        ENTRY;
	...
	if (unlikely(OBD_FAIL_CHECK(OBD_FAIL_OST_CHECKSUM_SEND))) {
		int off = local_nb[0].lnb_page_offset & ~CFS_PAGE_MASK;
		int len = local_nb[0].len;
		char *ptr = kmap_atomic(local_nb[0].page,
					KM_USER0);
		memcpy(fail_backup, ptr + off, min(4,len));
	...
	if (rc == 0) {
		if (body->oa.o_flags & OBD_FL_SHORT_IO) {
			...
			short_io_size = req_capsule_get_size(&req->rq_pill,
							     &RMF_SHORT_IO,
							     RCL_SERVER);
			rc = ost_pages2shortio(local_nb, npages_read,
	...
	if (unlikely(fail == 0x01)) {
		int off = local_nb[0].lnb_page_offset & ~CFS_PAGE_MASK;
		int len = local_nb[0].len;
		char *ptr = kmap_atomic(local_nb[0].page,
					KM_USER0);
		memcpy(ptr + off, fail_backup, min(4, len));
		kunmap_atomic(ptr, KM_USER0);
	}
