                       ...
                       struct obd_trans_info *oti)
{
        ...
        req->rq_status = obd_destroy(req->rq_svc_thread->t_env, exp, &body->oa,
                                     NULL, oti, NULL, capa);
        RETURN(0);
}
...
static int ost_statfs(struct ptlrpc_request *req)
{
        ...
        req->rq_status = obd_statfs(req->rq_svc_thread->t_env, req->rq_export, osfs,
                      ...
                      struct obd_trans_info *oti)
{
        ...
        req->rq_status = obd_create(req->rq_svc_thread->t_env, exp, &repbody->oa,
                                    NULL, oti);
...
static int ost_sync(struct obd_export *exp, struct ptlrpc_request *req)
{
        ...
        req->rq_status = obd_sync(req->rq_svc_thread->t_env, exp, oinfo,
                                  body->oa.o_size, body->oa.o_blocks, NULL);
        OBD_FREE_PTR(oinfo);
                       ...
                       struct obd_trans_info *oti)
{
        ...
        req->rq_status = obd_setattr(req->rq_svc_thread->t_env, exp, oinfo, oti);
...
static int ost_brw_read(struct ptlrpc_request *req, struct obd_trans_info *oti)
{
        ...
        rc = obd_preprw(req->rq_svc_thread->t_env, OBD_BRW_READ, exp, &body->oa,
                        1, ioo, remote_nb, &npages, local_nb, oti, capa);
        if (rc != 0)
                GOTO(out_lock, rc);
...
out_commitrw:
        ...
        rc = obd_commitrw(req->rq_svc_thread->t_env, OBD_BRW_READ, exp, &body->oa,
                          1, ioo, remote_nb, npages, local_nb, oti, rc);
...
static int ost_brw_write(struct ptlrpc_request *req, struct obd_trans_info *oti)
{
        ...
        npages = OST_THREAD_POOL_SIZE;
        rc = obd_preprw(req->rq_svc_thread->t_env, OBD_BRW_WRITE, exp, &body->oa,
                        objcount, ioo, remote_nb, &npages, local_nb, oti, capa);
        if (rc != 0)
                GOTO(out_lock, rc);
