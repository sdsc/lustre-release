...
static int ldiskfs_osd_skip_empty_group_seq_show(struct seq_file *m, void *data)
{
	struct osd_device *dev = osd_dt_dev((struct dt_device *)m->private);
	...
	return seq_printf(m, "%d\n", !!dev->od_skip_empty_group);
}

static ssize_t
ldiskfs_osd_skip_empty_group_seq_write(struct file *file, const char *buffer,
				       size_t count, loff_t *off)
{
	struct seq_file	  *m = file->private_data;
	struct dt_device  *dt = m->private;
	...
	dev->od_skip_empty_group = !!val;
	return count;
}
LPROC_SEQ_FOPS(ldiskfs_osd_skip_empty_group);
...
struct lprocfs_seq_vars lprocfs_osd_obd_vars[] = {
	...
	{ .name	=	"skip_empty_group",
	  .fops	=	&ldiskfs_osd_skip_empty_group_fops	},
	{ NULL }
};
