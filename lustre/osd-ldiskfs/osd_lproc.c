...
static int ldiskfs_osd_index_in_idif_seq_show(struct seq_file *m, void *data)
{
	...
	LASSERT(dev != NULL);
	if (unlikely(dev->od_mnt == NULL ||
		     dev->od_dt_dev.dd_lu_dev.ld_site->ls_tgt == NULL))
		return -EINPROGRESS;
				    ...
				    size_t count, loff_t *off)
{
	...
	if (dev->od_index_in_idif) {
		...
		LCONSOLE_WARN("OST-index in IDIF has been enabled, "
...
struct lprocfs_vars lprocfs_osd_obd_vars[] = {
	...
	{ .name	=	"index_in_idif",
	  .fops	=	&ldiskfs_osd_index_in_idif_fops	},
	{ NULL }
};
