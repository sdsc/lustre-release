...
iam_load_idle_blocks(struct iam_container *c, iam_ptr_t blk)
{
	...
	if (le16_to_cpu(head->iih_magic) != IAM_IDLE_HEADER_MAGIC) {
		CERROR("%.16s: invalid idle block head, blk = %u, magic = %d\n",
		       name, blk, le16_to_cpu(head->iih_magic));
		brelse(bh);
		c->ic_idle_failed = 1;
...
static int iam_format_guess(struct iam_container *c)
{
	...
	if (result == 0) {
		...
		limit = (struct dx_countlimit *)(c->ic_root_bh->b_data +
						 c->ic_descr->id_root_gap);
		cfs_down(&c->ic_idle_sem);
		bh = iam_load_idle_blocks(c, le32_to_cpu(limit->idle_blocks));
...
iam_new_node(handle_t *h, struct iam_container *c, __u32 *b, int *e)
{
	...
	if (c->ic_idle_bh != NULL) {
		...
		*b = le32_to_cpu(limit->idle_blocks);
		limit->idle_blocks = head->iih_next;
		*e = ldiskfs_journal_dirty_metadata(h, c->ic_root_bh);
		if (*e != 0) {
			limit->idle_blocks = cpu_to_le32(*b);
			goto fail;
		}

		bh = c->ic_idle_bh;
		idle = iam_load_idle_blocks(c, le32_to_cpu(limit->idle_blocks));
...
iam_recycle_leaf(handle_t *h, struct iam_path *p, struct iam_leaf *l)
{
	...
	if (lh == NULL) {
		CERROR("%.16s: No memory to recycle idle blocks\n", name);
		return -ENOMEM;
	}
	...
	if (unlikely(c->ic_idle_failed)) {
		cfs_up(&c->ic_idle_sem);
		CERROR("%.16s: idle blocks failed, will lose the blk %u\n",
		       name, frame->leaf);
		return 0;
	}
	...
	if (c->ic_idle_bh == NULL) {
		...
		limit->idle_blocks = cpu_to_le32(frame->leaf);
	...
	if (count == iam_idle_blocks_limit(inode)) {
		...
		head->iih_magic = cpu_to_le16(IAM_IDLE_HEADER_MAGIC);
		head->iih_count = 0;
		head->iih_next = limit->idle_blocks;
		rc = iam_txn_dirty(h, p, l->il_bh);
		if (rc != 0)
			goto unlock;
