		   ...
		   int insert)
{
	...
	if (scrub->os_partial_scan) {
		struct osd_scrub_bad_oimap_slot *slot;
		struct osd_scrub_bad_oimap_slot *tmp;
		__u64 time;
		__u64 count = 0;
		int i;

		/* Calculate whether need to scan the whole device or not.
		 *
		 * We maintain a small array that contains
		 * SCRUB_BAD_OIMAP_ARRAY_SIZE time-slots for the speed
		 * statistics of the auto-detected bad OI mappings, and
		 * each time-slot covers 2^SCRUB_BAD_OIMAP_SLOT_SHIFT
		 * seconds. If at some time-point, the summary of all
		 * the auto-detected bad OI mappings in these time-slots
		 * exceeds the threshold (osd_device::od_full_scrub_speed),
		 * then need to scan the whole device with full speed. */
		time = cfs_time_current_sec() >> SCRUB_BAD_OIMAP_SLOT_SHIFT;
		slot = &scrub->os_osbos[time & SCRUB_BAD_OIMAP_ARRAY_MASK];
		for (i = 0; i < SCRUB_BAD_OIMAP_ARRAY_SIZE; i++) {
			tmp = &scrub->os_osbos[i];
			if ((slot->osbos_time != time) &&
			    (tmp->osbos_time +
			     SCRUB_BAD_OIMAP_ARRAY_SIZE <= time))
				tmp->osbos_count = 0;
			else
				count += tmp->osbos_count;
		}

		slot->osbos_time = time;
		slot->osbos_count++;

		if (++count > dev->od_full_scrub_speed)
