...
iget:
	...
	bag = &osd_dt_obj(osd_fid2oi(dev, fid)->oi_dir)->oo_dir->od_container;
...
static int osd_otable_it_rec(const struct lu_env *env, const struct dt_it *di,
			     struct lu_dirent *ent, __u32 attr)
{
	struct osd_otable_it    *it  = (struct osd_otable_it *)di;
	struct osd_otable_cache *ooc = &it->ooi_cache;

	*(struct lu_fid *)ent = ooc->ooc_cache[ooc->ooc_consumer_idx].oic_fid;
	return 0;
}
