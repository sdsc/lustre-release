		       ...
		       struct osd_idmap_cache *oic, int val)
{
	...
	if (val == SCRUB_NEXT_FOO || val == SCRUB_NEXT_FOO_OLD) {
		...
		if (val == SCRUB_NEXT_FOO_OLD && scrub->os_convert_ff) {
			...
			struct super_block *sb, bool scrub)
{
	...
	if (rc == 0) {
		...
		if (dev->od_is_md) {
			...
			rc = osd_get_idif(info, inode,
					  &info->oti_obj_dentry, fid);
			if (rc == 0) {
				if (scrub)
			   ...
			   loff_t offset, __u64 ino, unsigned d_type)
{
	...
	if (rc != -ENODATA) {
		CWARN("%s: cannot get_lma for '%.*s': rc = %d\n",
		      osd_name(dev), namelen, name, rc);
		GOTO(put, rc);
	}
	...
	if (rc != 0) {
		CWARN("%s: cannot get_idif for '%.*s': rc = %d\n",
		      osd_name(dev), namelen, name, rc);
		GOTO(put, rc);
	}
