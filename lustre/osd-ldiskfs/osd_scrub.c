		   ...
		   int insert)
{
	...
	if (scrub->os_partial_scan) {
		...
		now = cfs_time_current_sec() >> SCRUB_BAD_OIMAP_SLOT_SHIFT;
		slot = &scrub->os_osbos[now & SCRUB_BAD_OIMAP_ARRAY_MASK];
		for (i = 0; i < SCRUB_BAD_OIMAP_ARRAY_SIZE; i++) {
			tmp = &scrub->os_osbos[i];
			if ((tmp->osbos_time +
				SCRUB_BAD_OIMAP_ARRAY_SIZE <= now) &&
			    (tmp->osbos_count > 0)) {
				scrub->os_osbos_total -= tmp->osbos_count;
				tmp->osbos_count = 0;
			}
		}
		...
		if (++scrub->os_osbos_total > dev->od_full_scrub_speed)
