...
/**
 * \retval SCRUB_NEXT_FOO_OLD: FID-on-OST
 * \retval 0: FID-on-MDT
 */
			   ...
			   loff_t offset, __u64 ino, unsigned d_type)
{
	...
	if (rc == SCRUB_NEXT_FOO || rc == SCRUB_NEXT_FOO_OLD) {
		rc = osd_obj_map_rename(info, dev, parent, child, fid);
		if (rc == 0) {
			...
			if (inode->i_size == 0 && inode->i_mode & S_ISUID &&
				...
				/* If the OST-object has never been modified,
				 * then remove it directly. */
				mutex_lock_nested(&dir->i_mutex,
						  I_MUTEX_PARENT);
				ll_vfs_unlink(dir, child, dev->od_mnt);
