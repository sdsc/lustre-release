			...
			struct super_block *sb, bool scrub)
{
	...
	if (ldiskfs_test_inode_state(inode, LDISKFS_STATE_LUSTRE_NOOI)) {
		iput(inode);
		return SCRUB_NEXT_CONTINUE;
	}
...
static const struct osd_lf_map osd_lf_maps[] = {
	...
	/* MDT device and OST device have the named file LAST_RCVD, but with
	 * different FIDs, since they cannot exist on the same device, just
	 * use MDT_LAST_RECV_OID for the default FID.
	 *
	 * If the LAST_RCVD on OST without FID-in-LMA, then it is upgraded
	 * from old server. Under such case, assigning the MDT_LAST_RECV_OID
	 * to the LAST_RCVD is harmless. */

	/* last_rcvd for MDT */
	{ LAST_RCVD, { FID_SEQ_LOCAL_FILE, MDT_LAST_RECV_OID, 0 },
		OLF_SHOW_NAME, NULL, NULL },

	/* last_rcvd for OST */
	{ LAST_RCVD, { FID_SEQ_LOCAL_FILE, OFD_LAST_RECV_OID, 0 },
		OLF_SHOW_NAME, NULL, NULL },
	...
	/* LOGS, upgrade from 1.8 device */
	{ "LOGS", { 0, 0, 0 }, OLF_SCAN_SUBITEMS,
		osd_ios_general_scan, osd_ios_varfid_fill },

	/* OBJECTS, upgrade from 1.8 device */
	{ "OBJECTS", { 0, 0, 0 }, OLF_SCAN_SUBITEMS,
		osd_ios_general_scan, osd_ios_varfid_fill },
