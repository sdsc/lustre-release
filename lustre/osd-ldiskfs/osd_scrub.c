...
static int osd_scrub_next(struct osd_thread_info *info, struct osd_device *dev,
			  struct osd_iit_param *param,
			  struct osd_idmap_cache **oic, bool noslot)
{
	struct osd_scrub     *scrub  = &dev->od_scrub;
	struct ptlrpc_thread *thread = &scrub->os_thread;
	struct lu_fid	     *fid;
	...
	if (noslot || scrub->os_partial_scan)
		   ...
		   int insert)
{
	...
	if (scrub->os_partial_scan) {
		struct osd_scrub_bad_mappings_slot *slot;
		struct osd_scrub_bad_mappings_slot *tmp;
		__u64 time;
		__u64 count = 0;
		int pos;
		int i;

		time = cfs_time_current_sec() >> SCRUB_BAD_MAPPINGS_SLOT_SHIFT;
		pos = time & SCRUB_BAD_MAPPINGS_ARRAY_MASK;
		slot = &scrub->os_osbms[pos];
		for (i = 0; i < SCRUB_BAD_MAPPINGS_ARRAY_SIZE; i++) {
			tmp = &scrub->os_osbms[i];
			if ((slot->osbms_time != time) &&
			    (tmp->osbms_time +
			     SCRUB_BAD_MAPPINGS_ARRAY_SIZE <= time))
				tmp->osbms_count = 0;
			else
				count += tmp->osbms_count;
		}

		slot->osbms_time = time;
		slot->osbms_count++;
		count++;
