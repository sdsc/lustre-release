...
#define SCRUB_NEXT_EXIT 	3
#define SCRUB_NEXT_WAIT 	4
#define SCRUB_NEXT_CRASH	5
#define SCRUB_NEXT_FATAL	6
...
static int
osd_iit_next(struct super_block *sb, ldiskfs_group_t bg, __u32 gbase,
	     __u32 *offset, __u32 *pos, struct buffer_head *bitmap)
{
	*offset = ldiskfs_find_next_bit(bitmap->b_data,
					LDISKFS_INODES_PER_GROUP(sb), *offset);
	if (*offset >= LDISKFS_INODES_PER_GROUP(sb)) {
		*pos = 1 + (bg + 1) * LDISKFS_INODES_PER_GROUP(sb);
...
static int
osd_iit_iget(struct osd_thread_info *info, struct osd_device *dev,
...
osd_scrub_next(struct osd_thread_info *info, struct osd_device *dev,
	       struct super_block *sb, ldiskfs_group_t bg, __u32 gbase,
	       __u32 *offset, struct buffer_head *bitmap,
	       struct osd_idmap_cache **oic, int noslot)
{
	...
	rc = osd_iit_next(sb, bg, gbase, offset,
			  &scrub->os_pos_current, bitmap);
	if (rc != 0)
		return rc;
...
static int
osd_preload_next(struct osd_thread_info *info, struct osd_device *dev,
		 struct super_block *sb, ldiskfs_group_t bg, __u32 gbase,
		 __u32 *offset, struct buffer_head *bitmap)
{
	...
	rc = osd_iit_next(sb, bg, gbase, offset, &ooc->ooc_pos_preload, bitmap);
	if (rc != 0)
		return rc;
	...
	rc = osd_iit_iget(info, dev,
			  ...
			  ooc->ooc_pos_preload, sb, &inode);
	/* If succeed, it needs to move forware; otherwise up layer LFSCK may
...
static int
osd_scrub_exec(struct osd_thread_info *info, struct osd_device *dev,
	       struct super_block *sb, __u32 gbase, __u32 *offset,
	       struct osd_idmap_cache *oic, int rc, int *noslot)
{
...
static int
osd_preload_exec(struct osd_device *dev, int rc)
{
	struct osd_otable_cache	*ooc = &dev->od_otable_it->ooi_cache;
			       ...
			       struct osd_device *dev, __u32 max, int preload)
{
	...
	limit = le32_to_cpu(LDISKFS_SB(sb)->s_es->s_inodes_count);

again:
	while (*pos <= limit && *count < max) {
		struct buffer_head *bitmap;
		struct osd_idmap_cache *oic = NULL;
		ldiskfs_group_t bg = (*pos - 1) / LDISKFS_INODES_PER_GROUP(sb);
		__u32 offset = (*pos - 1) % LDISKFS_INODES_PER_GROUP(sb);
		...
		if (bitmap == NULL) {
			CERROR("%.16s: fail to read bitmap for %u, "
			       ...
			       LDISKFS_SB(sb)->s_es->s_volume_name, (__u32)bg);
			RETURN(-EIO);
		}

		while (offset < LDISKFS_INODES_PER_GROUP(sb) && *count < max) {
			if (preload == 0)
				rc = osd_scrub_next(info, dev, sb, bg,
						    gbase, &offset, bitmap,
						    &oic, noslot);
			else
				rc = osd_preload_next(info, dev, sb, bg,
						      gbase, &offset, bitmap);
			switch (rc) {
			...
			if (preload == 0)
				rc = osd_scrub_exec(info, dev, sb, gbase,
						    &offset, oic, rc, &noslot);
			else
				rc = osd_preload_exec(dev, rc);
			if (rc != 0) {
				brelse(bitmap);
				RETURN(rc);
			}
