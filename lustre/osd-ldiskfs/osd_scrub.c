				  ...
				  struct scrub_file *src)
{
	...
	des->sf_run_time	= le16_to_cpu(src->sf_run_time);
				 ...
				 struct scrub_file *src)
{
	...
	des->sf_run_time	= cpu_to_le16(src->sf_run_time);
...
void osd_scrub_file_reset(struct osd_scrub *scrub, __u8 *uuid, __u16 flags)
{
	...
	CDEBUG(D_LFSCK, "Reset OI scrub file, flags = 0x%x\n", flags);
	memcpy(sf->sf_uuid, uuid, 16);
	sf->sf_status = SS_INIT;
	sf->sf_flags |= flags;
	sf->sf_param = 0;
...
osd_scrub_check_update(const struct lu_env *env, struct osd_device *dev,
		       struct osd_idmap_cache *oic, int rc)
{
	struct osd_scrub	     *scrub  = &dev->od_scrub;
	struct scrub_file	     *sf     = &scrub->os_file;
	...
	if (scrub->os_in_prior)
		oii = cfs_list_entry(oic, struct osd_inconsistent_item, oii_cache);
...
iget:
		inode = osd_iget(info, dev, lid, fid, 0);
		if (IS_ERR(inode)) {
			rc = PTR_ERR(inode);
			/* Some one removed the inode. */
			if (rc == -ENOENT || rc == -ESTALE)
				rc = 0;
			GOTO(out, rc);
		}

		/* XXX: prevent the inode to be unlinked during OI scrub. */
		...
		if (unlikely(!(sf->sf_flags & SF_RECREATED)))
			sf->sf_flags |= SF_RECREATED;
	} else if (osd_id_eq(lid, lid2)) {
			GOTO(out, rc = 0);
	}

	if (unlikely(!(sf->sf_flags & SF_INCONSISTENT)))
	       ...
	       __u32 *offset, struct osd_idmap_cache **oic)
{
	...
	*offset = ldiskfs_find_next_bit(bitmap->b_data,
	...
	inode = osd_iget(info, dev, lid, fid, OSD_IF_GEN_LID | OSD_IF_RET_FID);
	if (IS_ERR(inode)) {
		rc = PTR_ERR(inode);
...
static int osd_scrub_main(void *args)
{
	...
	while (scrub->os_pos_current <= max) {
		...
		while (offset < LDISKFS_INODES_PER_GROUP(sb)) {
			...
			rc = osd_scrub_check_update(&env, dev, oic, rc);
			if (rc != 0) {
				brelse(bitmap);
				GOTO(post, rc);
			}
...
next:
			...
			if (dev->od_otable_it != NULL) {
				...
				if (it->ooi_waiting && ooc->ooc_pos_preload <
		...
		while (offset < LDISKFS_INODES_PER_GROUP(sb) &&
			...
			if (IS_ERR(inode)) {
				...
				CERROR("%.16s: fail to read inode, group = %u, "
				       ...
				       LDISKFS_SB(sb)->s_es->s_volume_name, bg,
...
out:
	...
	if (ooc->ooc_pos_preload > max) {
		CDEBUG(D_LFSCK, "OSD pre-loaded all: pid = %d\n",
		       cfs_curproc_pid());
...
int osd_scrub_dump(struct osd_device *dev, char *buf, int len)
{
	...
	rc = snprintf(buf, len,
		      ...
		      "OI files: %d\n"
		      "Status: %s\n",
		      (int)sf->sf_version, (int)sf->sf_oi_count,
