		       ...
		       struct osd_idmap_cache *oic, int val)
{
	...
	rc = osd_oi_lookup(info, dev, fid, lid2,
		(val == SCRUB_NEXT_FOO ||
		 val == SCRUB_NEXT_FOO_OLD) ? OI_KNOWN_ON_OST : 0);
	if (rc != 0) {
		if (rc != -ENOENT)
			GOTO(out, rc);
			...
			struct super_block *sb, bool scrub)
{
	...
	if (rc == 0) {
		...
		if (lma->lma_compat & LMAC_FID_ON_OST && scrub) {
		...
		if (rc == 0) {
			...
			/* XXX: If the object has neither LMA nor "fid" xattr,
			 *	then it maybe for old 1.8 MDT-object upgrading,
			 *	or maybe the object is crashed. For the former
			 *	case, we can generate IGIF; for the later case,
			 *	we do not know how to re-generate the FID. */
			   ...
			   loff_t offset, __u64 ino, unsigned d_type)
{
	...
	if (IS_ERR(child)) {
		CWARN("%.16s: fail to lookup child %.*s, rc = %d\n",
		      LDISKFS_SB(osd_sb(dev))->s_es->s_volume_name,
		      namelen, name, (int)PTR_ERR(child));
		RETURN(0);
	}
	...
	if (S_ISDIR(inode->i_mode)) {
		rc = osd_ios_new_item(dev, child, osd_ios_general_scan,
				      osd_ios_lf_fill);
		if (rc != 0)
			CWARN("%.16s: fail to add child %.*s, rc = %d\n",
			      LDISKFS_SB(osd_sb(dev))->s_es->s_volume_name,
			      namelen, name, rc);
		GOTO(put, rc);
	}
	...
	if (rc != -ENODATA) {
		CWARN("%.16s: fail to get_lma for %.*s, rc = %d\n",
		      LDISKFS_SB(osd_sb(dev))->s_es->s_volume_name,
		      namelen, name, rc);
		GOTO(put, rc);
	}
	...
	if (rc != 0) {
		CWARN("%.16s: fail to get_idif for %.*s, rc = %d\n",
		      LDISKFS_SB(osd_sb(dev))->s_es->s_volume_name,
		      namelen, name, rc);
		GOTO(put, rc);
	}
...
rename:
	...
	if (rc == 0) {
		...
		CWARN("%.16s: fail to rename for %.*s, "DFID", rc = %d\n",
		      LDISKFS_SB(osd_sb(dev))->s_es->s_volume_name,
		      namelen, name, PFID(fid), rc);
	}
