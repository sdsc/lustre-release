...
static CFS_LIST_HEAD(osd_scrub_ls_list);
...
static const struct osd_scrub_ls_map oslm_root[] = {
	...
	/* ROOT */
	{ { FID_SEQ_LOCAL_FILE, MDD_ROOT_INDEX_OID, 0 }, 0, 4, "ROOT",
		osd_scrub_ROOT_scan, NULL },
	 ...
	 * with different FIDs, so need some special processing. */
#if 0
	/* last_rcvd for MDT */
				     ...
				     const struct osd_inode_id *id, int ops)
{
	...
	if (ops == DTO_INDEX_UPDATE) {
		...
		if (rc == -EEXIST) {
			rc = 1;
			/* XXX: There are trouble things when andding OI
...
static int osd_scrub_prep(struct osd_device *dev)
{
	...
	if (sf->sf_flags & (SF_RECREATED | SF_INCONSISTENT | SF_UPGRADED))
		scrub->os_full_speed = 1;
			...
			struct super_block *sb, bool scrub)
{
	...
	if (LDISKFS_I(inode)->i_state_flags & I_LUSTRE_NOOI) {
		iput(inode);
		return SCRUB_NEXT_CONTINUE;
	}

	if (scrub && LDISKFS_I(inode)->i_state_flags & I_LUSTRE_NOSCRUB) {
...
static int
osd_scrub_new_osli(struct dentry *dentry, scandir_t scandir, filldir_t filldir)
{
	...
	cfs_list_add_tail(&item->list, &osd_scrub_ls_list);
	return 0;
}
		   ...
		   struct inode *inode, const struct lu_fid *fid)
{
	...
	if (rc == -ENODATA) {
		if (fid == NULL || fid_is_zero(fid))
			lu_igif_build(&lma->lma_self_fid, inode->i_ino,
				      inode->i_generation);
		else
			lma->lma_self_fid = *fid;
		mutex_lock(&inode->i_mutex);
		rc = __osd_ea_fid_set(info, dev, inode, &lma->lma_self_fid,
				      false);
		mutex_unlock(&inode->i_mutex);
		if (rc != 0)
			RETURN(rc);
	...
	rc = __osd_oi_lookup(info, dev, &lma->lma_self_fid, id2);
	if (rc != 0) {
		if (rc != -ENOENT)
			RETURN(rc);

		rc = osd_scrub_refresh_mapping(info, dev, &lma->lma_self_fid,
					       id, DTO_INDEX_INSERT);
		RETURN(rc);
	}
	...
	rc = osd_scrub_refresh_mapping(info, dev, &lma->lma_self_fid, id,
				       DTO_INDEX_UPDATE);
				 ...
				 loff_t offset, __u64 ino, unsigned d_type)
{
	...
	child = ll_lookup_one_len(name, fill_buf->dentry, namelen);
	if (IS_ERR(child))
	...
	rc = osd_scrub_new_osli(child, osd_scrub_general_scan,
			       ...
			       loff_t offset, __u64 ino, unsigned d_type)
{
	...
	rc = osd_scrub_new_osli(child, map->scandir, map->filldir);
	if (rc != 0)
		dput(child);
		 ...
		 struct dentry *dentry, filldir_t filldir)
{
	...
	for (i = 0; i < MAX_OBJID_GROUP && rc == 0; i++) {
		parent = dev->od_ost_map->groups[i].groot;
		child = ll_lookup_one_len("LAST_ID", parent, strlen("LAST_ID"));
		if (IS_ERR(child)) {
			rc = PTR_ERR(child);
			if (rc == -ENOENT)
				rc = 0;
		    ...
		    struct dentry *dentry, filldir_t filldir)
{
	...
	child = ll_lookup_one_len(dot_lustre_name, dentry,
				  strlen(dot_lustre_name));
	if (IS_ERR(child)) {
		rc = PTR_ERR(child);
		if (rc == -ENOENT) {
			/* It is 1.8 MDT device, or broken 2.x MDT device.
		 	 * Mark it as SF_UPGRADED since cannot distinguish.
		 	 * The .lustre will be created later by MDD caller. */
			if (!(sf->sf_flags & SF_UPGRADED)) {
				osd_scrub_file_reset(scrub,
					LDISKFS_SB(osd_sb(dev))->s_es->s_uuid,
					SF_UPGRADED);
				rc = osd_scrub_file_store(scrub);
		...
		rc = osd_scrub_scan_one(info, dev, child->d_inode,
					&LU_DOT_LUSTRE_FID);
		dput(child);
	}
			       ...
			       struct osd_device *dev)
{
	...
	while (1) {
		...
		if (cfs_list_empty(&osd_scrub_ls_list))
			break;

		item = cfs_list_entry(osd_scrub_ls_list.next,
	...
	while (!cfs_list_empty(&osd_scrub_ls_list)) {
		item = cfs_list_entry(osd_scrub_ls_list.next,
				      struct osd_scrub_ls_item, list);
		cfs_list_del_init(&item->list);
		dput(item->dentry);
...
int osd_scrub_setup(const struct lu_env *env, struct osd_device *dev)
{
	...
	LDISKFS_I(scrub->os_inode)->i_state_flags |= I_LUSTRE_NOOI;
	...
	if (rc == 0 && !dev->od_noscrub &&
	    ((sf->sf_status == SS_PAUSED) ||
	     (sf->sf_status == SS_CRASHED &&
	      sf->sf_flags & (SF_RECREATED | SF_INCONSISTENT | SF_UPGRADED |
			      SF_AUTO)) ||
	     (sf->sf_status == SS_INIT &&
	      sf->sf_flags & (SF_RECREATED | SF_INCONSISTENT | SF_UPGRADED))))
...
/**
 * Used to notify otable iteration to be paused.
 */
