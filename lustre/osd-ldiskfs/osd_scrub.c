				     ...
				     int ops, enum oi_check_flags flags)
{
	...
	/* DTO_INDEX_INSERT is enough for other two ops: delete/update,
	 * but save stack. */
				  ...
				  struct scrub_file *src)
{
	...
	des->sf_flags2		= le16_to_cpu(src->sf_flags2);
				 ...
				 struct scrub_file *src)
{
	...
	des->sf_flags2		= cpu_to_le16(src->sf_flags2);
	memcpy(des->sf_oi_bitmap, src->sf_oi_bitmap, SCRUB_OI_BITMAP_SIZE);
}
		     ...
		     struct inode *inode, const struct lu_fid *fid)
{
	...
	/* 4) generate new XATTR_NAME_FID with the saved parent FID and add it*/
	if (size > 0)
		rc = __osd_xattr_set(info, inode, XATTR_NAME_FID, ff, size,
				     XATTR_CREATE);
		...
		if (unlikely(fid_is_last_id(fid))) {
			if (scrub) {
				if (lma->lma_compat & LMAC_FID_ON_OST)
					...
					/* For Lustre-2.4 LAST_ID file, it has
					 * FID-in-LMA, but no LMAC_FID_ON_OST.
					 * Then the OI scrub will update it. */
					rc = SCRUB_NEXT_OSTOBJ_OLD;
			}
			/* For up layer iteration, LAST_ID is a visible object
			 * to be checked and repaired, so return it directly. */
			return rc;
		}
		     ...
		     struct dentry *dentry, filldir_t filldir)
{
	...
	if (unlikely(sf->sf_flags2 & SF2_NEW)) {
		sf->sf_flags2 &= ~SF2_NEW;
		rc = osd_scrub_file_store(scrub);
		if (rc != 0)
			RETURN(rc);
...
int osd_scrub_setup(const struct lu_env *env, struct osd_device *dev)
{
	...
	if (rc == -ENOENT) {
		...
		/* If the "/O" does not exist when mount, neither for the file
		 * "/OI_scrub", then it is quite probably that the device is
		 * new formatted one, under such case, mark it as SF2_NEW.
		 *
		 * For the rare case that "/O" and "OI_scrub" both lost on an
		 * old device, it can be found and handled later. */
		if (dev->od_maybe_new)
			sf->sf_flags2 = SF2_NEW;
		dirty = 1;
	} else if (rc != 0) {
	...
	if (rc == 0) {
		if (sf->sf_flags & SF_UPGRADE ||
		    !(sf->sf_flags2 & SF2_NEW || sf->sf_success_count > 0)) {
			dev->od_igif_inoi = 0;
			dev->od_check_ff = 1;
		} else {
			dev->od_igif_inoi = 1;
