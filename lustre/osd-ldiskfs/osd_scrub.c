...
{
        return !cfs_list_empty(&dev->od_unmatched_wait_list);
}
...
static int osd_scrub_main(void *arg)
{
        ...
        if (start < LDISKFS_FIRST_INO(osi->osi_sb))
                ...
                osi->osi_current_position = start + 1;
                            ...
                            struct lustre_capa *capa)
{
        ...
        rc = osd_oi_lookup(osd_oti_get(env), &dev->od_oi,
                           (const struct lu_fid *)key,
                            ...
                            int ignore_quota)
{
        ...
        struct dt_object    *idx    = dev->od_oi.oi_dir;
        struct inode        *inode;
        int                  rc;
        ENTRY;
        ...
        if (rc == -ENOENT) {
                ...
                /* Maybe race with osd_inode_remove(). */
                                    ...
                                    struct lustre_capa *capa)
{
        ...
        if (rc < 0) {
                ...
                cfs_spin_lock(&dev->od_osi_lock);
                dev->od_osi = NULL;
                cfs_list_splice_init(&dev->od_unmatched_wait_list, &wait_list);
                cfs_spin_unlock(&dev->od_osi_lock);
                osi_put(osi);
...
static int osd_scrub_next(const struct lu_env *env, struct dt_it *di)
{
        ...
        while (1) {
                ...
                if (scrub_has_urgent(dev)) {
                        osi->osi_next_oui =
                                cfs_list_entry(dev->od_unmatched_wait_list.next,
                        ...
                        if (osg->osg_completed) {
                                ...
                                if (scrub_has_urgent(dev)) {
                                        osi->osi_next_oui = cfs_list_entry(
                                               dev->od_unmatched_wait_list.next,
                                               struct osd_unmatched_item,
                                               oui_list);
...
static int osd_scrub_key_size(const struct lu_env *env, const struct dt_it *di)
{
        ...
        if (osi->osi_next_oui != NULL)
                return sizeof(osi->osi_next_oui->oui_id.oii_ino);

        return sizeof(osi->osi_current_key);
}

static int osd_scrub_rec(const struct lu_env *env,
                         const struct dt_it *di, void *buf, __u32 attr)
{
        struct osd_scrub_it   *osi   = (struct osd_scrub_it *)di;
        struct osd_device     *dev   = osi->osi_dev;
        struct dt_scrub_rec   *dsr   = (struct dt_scrub_rec *)buf;
        ...
        /* XXX: add more linkea process in the future. */
...
static int osd_scrub_rec_size(const struct lu_env *env, const struct dt_it *di,
                              int len, int *size)
{
        switch (len) {
        case 2:
                /* size[1] = sizeof(linkea) */
                /* fall through */
        case 1:
                size[0] = sizeof(struct osd_inode_id);
                return 0;
        default:
                return -EINVAL;
        }
                   ...
                   struct osd_unmatched_item *oui)
{
        ...
        if (cfs_list_empty(&dev->od_unmatched_wait_list))
                osi = osi_get(dev->od_osi);

        cfs_list_add_tail(&oui->oui_list, &dev->od_unmatched_wait_list);
        cfs_spin_unlock(&dev->od_osi_lock);
