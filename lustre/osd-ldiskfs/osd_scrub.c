...
static void osd_scrub_post(struct osd_scrub *scrub, int result)
{
        ...
        if (result > 0) {
                ...
                sf->sf_flags &= ~(SF_NONSCRUBBED | SF_INCONSISTENT | SF_AUTO);
...
int osd_scrub_start(struct osd_device *dev)
{
        ...
        /* od_muext: prevent curcurrent start/stop */
...
int osd_scrub_setup(const struct lu_env *env, struct osd_device *dev)
{
        ...
        if (get_mount_flags(dev->od_mount->lmi_sb) & LMD_FLG_SCRUBBED &&
            sf->sf_flags & SF_NONSCRUBBED) {
                sf->sf_flags &= ~SF_NONSCRUBBED;
                dirty = 1;
        }
        ...
        if (rc == 0 && !scrub->os_noauto_scrub &&
            ((sf->sf_status == SS_CRASHED && (sf->sf_flags & SF_INCONSISTENT ||
                                              sf->sf_flags & SF_AUTO)) ||
             (sf->sf_status == SS_INIT && (sf->sf_flags & SF_NONSCRUBBED ||
                                           sf->sf_flags & SF_INCONSISTENT))))
...
int osd_scrub_dump(struct osd_device *dev, char *buf, int len)
{
        ...
        rc = snprintf(buf, len,
                      ...
                      "Version: %d\n"
                      "Status: %s\n",
                      (int)sf->sf_version, scrub_status_names[sf->sf_status]);
        if (rc <= 0)
                goto out;
