...
/*
 * This file contains implementation of T10 routines for the OSD layer.
 * At this layer we do the following:
 *
 * 1. First, we need to verify if the underlying device supports T10 DIX
 *    and learn the integrity sector size: osd_setup_integrity()
 *
 * 2. If we are performing a read request from the OSS r/o cache, we
 *    need to verify if the data are correct, so we calculate their
 *    T10 CRCs and check they are the same as attached to the page.
 *    If the page has been partially truncated, it can be uptodate,
 *    but will have no integrity data. If data correctness cannot
 *    be verified, an error is reported to the caller, who is
 *    expected to re-read the data from disk: osd_check_integrity()
 *
 * 3. After a successful read or write, integrity data from integrity
 *    buffer needs to be attached to the corresponding r/o cache,
 *    so that it can be reused by later reads: osd_pull_integrity()
 *
 * 4. If a client is not T10-capable, but the OSD is, we will
 *    calculate integrity data to have partial DIX/DIF integrity and
 *    protected r/o cache for T10-capable client requests:
 *    osd_reconstruct_integrity()
 *
 * 5. The corresponding entries of thread's integrity array are
 *    attached to the block I/O request by osd_attach_integrity.
 */
...
#define MAX_ITAGS_PER_PAGE (CFS_PAGE_SIZE >> 9)
...
{
	struct osd_integrity *integrity = &osd->od_integrity;
	...
	if (bi && !strcmp(bi->name, "T10-DIF-TYPE1-CRC")) {
		struct osd_integrity_t10 *t10 = &integrity->od_t10;

		LASSERT(bi->sector_size != 0);

		t10->t10_chunks = CFS_PAGE_SIZE / bi->sector_size;
		t10->t10_chunks_bits = ffz(~t10->t10_chunks);
		integrity->od_type = OSD_INTEGRITY_T10;
	} else {
		integrity->od_type = OSD_INTEGRITY_NONE;
	}
			...
			struct integrity *integrity, int n)
{
	...
	if ((integrity->type & OBD_CKSUM_T10AB) == 0 && integrity->type != 0)
		return -EINVAL;
	...
	if (unlikely(!PagePrivate2(page))) {
		/* With certain race conditions MM can kick the metadata
		 * without further page release. Same with partial page
		 * truncate. */
	...
	for (i = 0; i < int_chunks; i++, tpl++) {
		if (crc[i] != cpu_to_be16(crc_t10dif(data + i*chunk, chunk))) {
			CERROR("cache integrity check failed "
			       "inode=%lu index=%lu\n offset=%d",
			       page->mapping->host->i_ino, page->index,
			       i * chunk);
		...
		/* Is it needed for INTEGRITY_NONE requests? */
		tpl->guard_tag = crc[i];
	}
	kunmap_atomic(data, KM_USER0);
...
/**
 * Calculate integrity for requests from non-T10-compliant clients.
 *
 * \param osd      : the osd device
				...
				struct integrity *integrity, int n)
{
	...
	struct obd_integrity_dif_tuple **int_buf = iobuf->dr_integrity;
	int buf_pos = iobuf->dr_npages;

	if (integrity != NULL &&
	    osd->od_integrity.od_type == OSD_INTEGRITY_T10) {
		int int_chunk = osd->od_integrity.od_t10.t10_chunks;
		int_buf[buf_pos] = &integrity->oi.t10.oi_tpl[n*int_chunk];
	} else {
		int_buf[buf_pos] = NULL;
	}
}
