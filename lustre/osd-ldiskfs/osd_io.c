...
/*
 * This function copy from RHEL6.4 fs/ext4/extents.c
 * The ext4_ext_walk_space was removed since RHEL6.5, Lustre need it
 * map blocks. Although can implement it via ext4_map_blocks. But may
 * hurt performance. Especially if the file is very fragmentary.
 * We tend to keep minimum ldiskfs patches usually. So we just need
 * export some functions if place this part in Lustre, It is less than
 * add a patch bring back ext4_ext_walk_space in ldiskfs. And this part
 * also can be used after 3.7 kernel server support.
 */
		       ...
		       unsigned long num, struct bpointers *data)
{
	...
	while (block < last && block != EXT_MAX_BLOCKS) {
		...
		if (unlikely(path[depth].p_hdr == NULL)) {
			up_read(&LDISKFS_I(inode)->i_data_sem);
			CERROR("ino=%lu, path[%d].p_hdr == NULL",
			       inode->i_ino, depth);
			err = -EIO;
			break;
		}
		...
		if (unlikely(cbex.ec_len == 0)) {
			CERROR("ino=%lu, cbex.ec_len == 0", inode->i_ino);
			err = -EIO;
			break;
		}
