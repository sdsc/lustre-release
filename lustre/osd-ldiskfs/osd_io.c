			    ...
			    int rw, int line, int pages)
{
	...
	CDEBUG(D_OTHER, "realloc %lu for %u (%u) pages in %p\n",
	       pages * sizeof(iobuf->dr_pages[0]), i, pages, current);
	pages = i;
	blocks = pages * (CFS_PAGE_SIZE >> osd_sb(d)->s_blocksize_bits);
	iobuf->dr_max_pages = 0;
	CDEBUG(D_OTHER, "realloc %lu for %u blocks in %p\n",
	       blocks * sizeof(iobuf->dr_blocks[0]), blocks, current);
