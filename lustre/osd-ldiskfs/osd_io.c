			...
			struct niobuf_local *lnb, int npages)
{
	...
	for (i = 0; i < npages; i++) {
		if (lnb->lnb_page == NULL)
			continue;
		LASSERT(PageLocked(lnb->lnb_page));
		unlock_page(lnb->lnb_page);
		page_cache_release(lnb->lnb_page);
                          ...
                          struct niobuf_local *lnb, int npages)
{
	...
	for (i = 0; i < npages; i++) {
		 ...
		 */
		ClearPageUptodate(lnb->lnb_page);

		if (lnb->lnb_len == PAGE_CACHE_SIZE)
			continue;

		if (maxidx >= lnb->lnb_page->index) {
			osd_iobuf_add_page(iobuf, lnb->lnb_page);
		} else {
			long off;
			char *p = kmap(lnb->lnb_page);

			off = lnb[i].lnb_page_offset;
			if (off)
				memset(p, 0, off);
			off = (lnb[i].lnb_page_offset + lnb->lnb_len) &
				~PAGE_CACHE_MASK;
			if (off)
                                    ...
                                    struct thandle *handle)
{
	...
	for (i = 0; i < npages; i++) {
		if (i && lnb[i].lnb_file_offset !=
		    lnb[i - 1].lnb_file_offset + lnb->lnb_len)
			extents++;
	...
	lnb->lnb_flags &= ~(OBD_BRW_OVER_USRQUOTA | OBD_BRW_OVER_GRPQUOTA);
                            ...
                            struct thandle *thandle)
{
	...
	ll_vfs_dq_init(inode);
		...
		if (lnb->lnb_rc) { /* ENOSPC, network RPC error, etc. */
		...
		LASSERT(PageLocked(lnb->lnb_page));
		LASSERT(!PageWriteback(lnb->lnb_page));

		if (lnb[i].lnb_file_offset + lnb->lnb_len > isize)
			isize = lnb[i].lnb_file_offset + lnb->lnb_len;
		 ...
		 * page cache.
		 */
		LASSERT(!PageDirty(lnb->lnb_page));

		SetPageUptodate(lnb->lnb_page);
                         ...
                         struct niobuf_local *lnb, int npages)
{
	...
	for (i = 0; i < npages; i++) {
		...
		if (i_size_read(inode) <
		    lnb[i].lnb_file_offset + lnb->lnb_len - 1)
			lnb->lnb_rc = i_size_read(inode) -
