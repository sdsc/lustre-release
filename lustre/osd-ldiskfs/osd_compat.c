			   ...
			   struct osd_object *obj, struct osd_thandle *oh)
{
	...
	rc = osd_ldiskfs_add_entry(oh->ot_handle, agent,
				obj->oo_inode, NULL);
	parent->d_inode->i_nlink++;
	mark_inode_dirty(parent->d_inode);
	mutex_unlock(&parent->d_inode->i_mutex);
	if (rc != 0) {
		CERROR(""DFID" add agent error: rc = %d\n",
...
int osd_ost_init(struct osd_device *dev)
{
	...
	rwlock_init(&dev->od_ost_map->oom_seq_list_lock);
	sema_init(&dev->od_ost_map->oom_dir_init_sem, 1);
...
static void osd_ost_fini(struct osd_device *osd)
{
	...
	write_lock(&map->oom_seq_list_lock);
	cfs_list_for_each_entry_safe(osd_seq, tmp,
				     &map->oom_seq_list,
				     oos_seq_list) {
		osd_seq_free(map, osd_seq);
	}
	write_unlock(&map->oom_seq_list_lock);
	if (map->oom_root)
		dput(map->oom_root);
	OBD_FREE_PTR(map);
	osd->od_ost_map = NULL;
...
struct osd_obj_seq *osd_seq_find(struct osd_obj_map *map, obd_seq seq)
{
	...
	read_lock(&map->oom_seq_list_lock);
	osd_seq = osd_seq_find_locked(map, seq);
	read_unlock(&map->oom_seq_list_lock);
	return osd_seq;
}
...
static struct osd_obj_seq *osd_seq_load(struct osd_device *osd, obd_seq seq)
{
	...
	down(&map->oom_dir_init_sem);
	rc = osd_seq_load_locked(osd, osd_seq);
	up(&map->oom_dir_init_sem);
