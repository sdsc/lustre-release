                                ...
                                struct thandle *handle)
{
	...
	if (attr->la_valid & LA_UID || attr->la_valid & LA_GID) {
		...
		allocated = obj->oo_inode->i_dquot[USRQUOTA] != NULL;
		...
		qi->lqi_space      = 1;
		rc = osd_declare_qid(env, oh, qi, allocated, enforce, NULL);
		...
		allocated = obj->oo_inode->i_dquot[GRPQUOTA] != NULL;
                                 ...
                                 int fl, struct thandle *handle)
{
	...
	/* xattr set may involve inode quota change */
                                 ...
                                 struct thandle *handle)
{
	...
	osd_trans_declare_op(env, oh, OSD_OT_XATTR_SET,
			     osd_dto_credits_noquota[DTO_XATTR_SET]);
	/* xattr del may involve inode quota change */
	oh->ot_credits += LDISKFS_MAXQUOTAS_INIT_BLOCKS(sb);
