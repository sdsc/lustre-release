			   ...
			   const struct lu_object_conf *conf)
{
	...
	if (result == 0 && obj->oo_inode != NULL) {
		...
		rc = osd_get_lma(oti, obj->oo_inode,
			...
			struct thandle *handle)
{
	...
	if (attr->la_flags & LUSTRE_LMA_FL_MASKS) {
		...
		rc1 = osd_get_lma(info, inode, &info->oti_obj_dentry, lma);
		if (rc1 != 0)
			GOTO(out, rc);
		...
		lustre_lma_swab(lma);
		rc1 = __osd_xattr_set(info, inode, XATTR_NAME_LMA,
                              ...
                              struct thandle *th)
{
	...
	if (S_ISDIR(inode->i_mode)) {
		LASSERTF(osd_inode_unlinked(inode) || inode->i_nlink == 1 ||
			inode->i_nlink == 2, "inode->i_ino %lu nlink %d\n",
			      ...
			      struct dt_object *dt, struct thandle *th)
{
	...
	CDEBUG(D_INFO, DFID" increase nlink %d ino %lu\n",
	       PFID(lu_object_fid(&dt->do_lu)), inode->i_nlink, inode->i_ino);
	/*
			      ...
			      struct thandle *th)
{
	...
	CDEBUG(D_INFO, DFID" decrease nlink %d ino %lu\n",
	       PFID(lu_object_fid(&dt->do_lu)), inode->i_nlink, inode->i_ino);
