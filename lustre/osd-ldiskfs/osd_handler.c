                     ...
                     struct osd_device *o, struct lustre_cfg *cfg)
{
	...
	if (opts == NULL || strstr(opts, "bigendian_extents") == NULL) {
		printk(KERN_ERR "ldiskfs: extents feature is not guaranteed to "
	...
	*options = '\0';
	if (opts != NULL) {
		...
		for (rc = 0; sout[rc]; rc++) {
			char *op;
			str = strstr(options, sout[rc]);
			if (str == NULL)
				continue;
			op = str + strlen(sout[rc]);
			if (*op == ',')
				op++;
			memmove(str, op, strlen(op) + 1);
		}
	} else
	...
	if (ldiskfs_blocks_count(LDISKFS_SB(osd_sb(o))->s_es) > (8ULL << 32) &&
	    force_over_128tb == 0) {
		printk(KERN_ERR "LDISKFS does not support filesystems "
