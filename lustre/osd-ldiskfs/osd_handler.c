		       ...
		       struct lu_buf *buf)
{
	...
	vallen = oxe->oxe_len - oxe->oxe_namelen - 1;
			...
			const struct lu_buf *buf)
{
	...
	size_t len = sizeof(*oxe) + namelen + 1;
	...
	if (buf != NULL) {
		memcpy(oxe->oxe_buf + namelen + 1, buf->lb_buf, buf->lb_len);
		oxe->oxe_exist = true;
	} else {
		oxe->oxe_exist = false;
	}
			 ...
			 struct lu_buf *buf, const char *name)
{
	...
	if (strcmp(name, XATTR_NAME_LOV) == 0 ||
	    strcmp(name, XATTR_NAME_DEFAULT_LMV))
		cache_xattr = true;
	...
	if (cache_xattr) {
		if (rc == -ENOENT || rc == -ENODATA)
			osd_oxc_add(obj, name, NULL);
		else if (rc == 0)
			osd_oxc_add(obj, name, buf);
	}
