...
static void osd_object_delete(const struct lu_env *env, struct lu_object *l)
{
		...
		if (osd_obj2dev(obj)->od_quota_inited && obj->oo_pool_valid) {
			qsd = qsd_hash_lookup(osd_obj2dev(obj)->od_slaves_hash,
					      obj->oo_pool_id);
			if (IS_ERR(qsd)) {
...
static void osd_quota_fini(const struct lu_env *env, struct osd_device *osd)
{
	down_write(&osd->od_quota_sem);
	qsd_all_fini(env, &osd->od_quota_slaves, osd->od_slaves_hash);
	up_write(&osd->od_quota_sem);
}

static int osd_quota_start(const struct lu_env *env, struct osd_device *osd)
{
	int rc;

	down_read(&osd->od_quota_sem);
	rc = qsd_all_start(env, &osd->od_quota_slaves);
	up_read(&osd->od_quota_sem);
	return rc;
}

static int osd_quota_prepare(const struct lu_env *env, struct osd_device *osd,
			      struct dt_object *root)
{
	int rc;

	down_read(&osd->od_quota_sem);
	rc = qsd_all_prepare(env, &osd->od_quota_slaves, root);
	up_read(&osd->od_quota_sem);
	return rc;
}
					 ...
					 struct lu_device *d)
{
	...
	osd_shutdown(env, o);
	if (o->od_quota_inited) {
		LASSERT(cfs_list_empty(&o->od_quota_slaves));
		cfs_hash_putref(o->od_slaves_hash);
	}
...
static int osd_pool_new(const struct lu_env *env, struct osd_device *osd,
			char *pool_name, int pool_id)
{
	int rc;
	struct qsd_instance *qsd;

	qsd = qsd_init(env, osd->od_svname, &osd->od_dt_dev,
		       osd->od_proc_entry, pool_name, pool_id);
	if (IS_ERR(qsd)) {
		rc = PTR_ERR(qsd);
		return rc;
	}

	rc = qsd_hash_add_unique(osd->od_slaves_hash, qsd);
	if (rc) {
		qsd_putref(env, qsd);
		rc = -EEXIST;
		return rc;
	...
	down_write(&osd->od_quota_sem);
	qsd_list_add(qsd, &osd->od_quota_slaves);
	up_write(&osd->od_quota_sem);
			    ...
			    struct lustre_cfg *cfg)
{
	...
	o->od_slaves_hash = cfs_hash_create("POOL_HASH",
					     HASH_POOLS_CUR_BITS,
					     HASH_POOLS_MAX_BITS,
					     ...
					     CFS_HASH_DEFAULT);
	if (!o->od_slaves_hash)
		GOTO(out_procfs, rc = -ENOMEM);

	CFS_INIT_LIST_HEAD(&o->od_quota_slaves);
	init_rwsem(&o->od_quota_sem);

	rc = osd_pool_new(env, o, NULL, 0);
	if (rc)
		GOTO(out_hash, rc);

	o->od_quota_slave = qsd_hash_lookup(o->od_slaves_hash, 0);
                                 ...
                                 struct lu_device *d)
{
	...
	if (osd->od_quota_inited) {
		rc = osd_quota_start(env, osd);
		if (rc)
			RETURN(rc);
	}
...
int osd_obd_pool_new(struct obd_device *obd, char *poolname, int pool_id)
{
	...
	rc = osd_pool_new(env, osd, poolname, pool_id);
...
int osd_obd_pool_del(struct obd_device *obd, char *poolname, int pool_id)
{
	...
	qsd = qsd_hash_del_key(osd->od_slaves_hash, pool_id);
	if (IS_ERR(qsd))
		GOTO(out_fini, rc = PTR_ERR(qsd));
	down_write(&osd->od_quota_sem);
	qsd_list_unlink(qsd);
	up_write(&osd->od_quota_sem);
                       ...
                       struct lu_device *dev)
{
	...
	/* TODO: check empty */
	rc = osd_quota_prepare(env, osd, qsd_root);
