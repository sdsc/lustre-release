                          ...
                          struct lu_context_key *key)
{
	...
	kref_init(&info->oti_iobuf->dr_refs);
...
{
	struct osd_iobuf *buf = container_of(kref, struct osd_iobuf, dr_refs);

	lu_buf_free(&buf->dr_pg_buf);
	lu_buf_free(&buf->dr_bl_buf);
	OBD_FREE_PTR(buf);
}
                         ...
                         struct lu_context_key *key, void* data)
{
	...
	kref_put(&info->oti_iobuf->dr_refs, osd_iobuf_release);
	OBD_FREE_PTR(info);
}
