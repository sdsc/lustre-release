                              ...
                              struct thandle *th)
{
	 ...
	 * applied only to local files */
	if (inode->i_nlink == 0) {
		LASSERT(!fid_is_norm(lu_object_fid(&dt->do_lu)));
		/* just skip this ref_del call */
		CERROR("%s: invalid nlink counter for local file "DFID", "
		       "can be result of upgrade from older version\n",
