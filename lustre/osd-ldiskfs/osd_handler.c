...
#define OSD_QID_ALLOCATED       0x100
#define OSD_QID_TYPE(type)      (type & ~OSD_QID_ALLOCATED)
                            ...
                            struct inode *inode)
{
        ...
        if (inode && inode->i_dquot[type]->dq_off)
                allocated = 1;

        for (i = 0; i < oh->ot_id_cnt; i++) {
                if (oh->ot_id_array[i].ug_id == id &&
                    OSD_QID_TYPE(oh->ot_id_array[i].ug_type) == type) {
                        if (allocated)
                                oh->ot_id_array[i].ug_type |= OSD_QID_ALLOCATED;
                        return;
                }
        }
