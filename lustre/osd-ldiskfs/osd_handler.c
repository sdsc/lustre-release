...
again:
	...
	if (lma != NULL) {
			...
			*fid = lma->lma_self_fid;
			...
			if (rc == 0)
				*attr |= LUDA_REPAIR;
				...
				CDEBUG(D_LFSCK, "%s: fail to update FID "
		...
		} else { /* !fid_is_sane(fid), trust the FID-in-LMA. */
			if (*attr & LUDA_VERIFY_DRYRUN) {
				*fid = lma->lma_self_fid;
				*attr |= LUDA_REPAIR;
			...
			*fid = lma->lma_self_fid;
			...
			if (rc == 0)
				*attr |= LUDA_REPAIR;
				...
				CDEBUG(D_LFSCK, "%s: fail to append FID "
	...
	} else { /* lma is NULL, trust the FID-in-dirent if it is valid. */
		if (*attr & LUDA_VERIFY_DRYRUN) {
			if (fid_is_sane(fid)) {
				*attr |= LUDA_REPAIR;
