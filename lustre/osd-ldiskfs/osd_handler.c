...
out:
        if (result == -EREMCHG && !dev->od_fl_noauto_scrub &&
            ++scrub_once == 1) {
                CDEBUG(D_SCRUB, "Trigger OI scrub by RPC for "DFID"\n",
                       PFID(fid));
                result = osd_scrub_start(env, dev, 1);
                   ...
                   __u32 ino, struct lu_fid *fid)
{
        ...
        inode = osd_iget(info, osd_dt_dev(dt), id, fid, OSD_IF_RET_FID);
        if (IS_ERR(inode))
                                ...
                                struct dt_rec *dtrec, __u32 attr)
{
        ...
        if (!fid_is_sane(fid))
                rc = osd_ea_fid_get(env, osd_obj2dt(obj),
                                    it->oie_dirent->oied_ino, fid);
                     ...
                     struct osd_device *o, struct lustre_cfg *cfg)
{
        ...
        if (get_mount_flags(lmi->lmi_sb) & LMD_FLG_NOAUTO_SCRUB)
                o->od_fl_noauto_scrub = 1;
                                          ...
                                          struct lustre_cfg *cfg)
{
        ...
        if (o != NULL) {
                ...
                if (result == 0) {
                        ...
                        cfs_spin_lock_init(&o->od_otable_it_lock);
                        cfs_mutex_init(&o->od_otable_it_mutex);
