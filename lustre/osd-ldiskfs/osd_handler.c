...
check_oi:
	if (rc != 0) {
		...
		if (rc == 0) {
			if (!IS_ERR(inode) && ldiskfs_test_inode_state(inode,
		      ...
		      struct thandle *th)
{
		...
		ldiskfs_clear_inode_state(inode, LDISKFS_STATE_LUSTRE_DEAD);
                obj->oo_inode = inode;
                result = 0;
        } else {
                if (obj->oo_hl_head != NULL) {
						  ...
						  struct thandle *th)
{
	...
	ldiskfs_clear_inode_state(local, LDISKFS_STATE_LUSTRE_DEAD);
