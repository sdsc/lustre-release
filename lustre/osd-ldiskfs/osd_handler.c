                    ...
                    struct thandle *th)
{
	...
	if (unlikely(osd_param_is_not_sane(dev, th))) {
		...
		CWARN("  create: %u/%u, delete: %u/%u, destroy: %u/%u\n",
		      oh->ot_declare_ops[OSD_OT_CREATE],
		      oh->ot_declare_ops_cred[OSD_OT_CREATE],
		      oh->ot_declare_ops[OSD_OT_DELETE],
		      oh->ot_declare_ops_cred[OSD_OT_DELETE],
		      oh->ot_declare_ops[OSD_OT_DESTROY],
		      oh->ot_declare_ops_cred[OSD_OT_DESTROY]);
		CWARN("  attr_set: %u/%u, xattr_set: %u/%u\n",
		      oh->ot_declare_ops[OSD_OT_ATTR_SET],
		      oh->ot_declare_ops_cred[OSD_OT_ATTR_SET],
		      oh->ot_declare_ops[OSD_OT_XATTR_SET],
		      oh->ot_declare_ops_cred[OSD_OT_XATTR_SET]);
		CWARN("  write: %u/%u, punch: %u/%u, quota %u/%u\n",
		      oh->ot_declare_ops[OSD_OT_WRITE],
		      oh->ot_declare_ops_cred[OSD_OT_WRITE],
		      oh->ot_declare_ops[OSD_OT_PUNCH],
		      oh->ot_declare_ops_cred[OSD_OT_PUNCH],
		      oh->ot_declare_ops[OSD_OT_QUOTA],
		      oh->ot_declare_ops_cred[OSD_OT_QUOTA]);
		CWARN("  insert: %u/%u, delete: %u/%u\n",
		      oh->ot_declare_ops[OSD_OT_INSERT],
		      oh->ot_declare_ops_cred[OSD_OT_INSERT],
		      oh->ot_declare_ops[OSD_OT_DESTROY],
		      oh->ot_declare_ops_cred[OSD_OT_DESTROY]);
		CWARN("  ref_add: %u/%u, ref_del: %u/%u\n",
		      oh->ot_declare_ops[OSD_OT_REF_ADD],
		      oh->ot_declare_ops_cred[OSD_OT_REF_ADD],
		      oh->ot_declare_ops[OSD_OT_REF_DEL],
		      oh->ot_declare_ops_cred[OSD_OT_REF_DEL]);
                                ...
                                struct thandle *handle)
{
	...
	osd_trans_declare_op(oh, OSD_OT_ATTR_SET,
                        ...
                        struct lustre_capa *capa)
{
	...
	osd_trans_exec_op(handle, OSD_OT_ATTR_SET);
                      ...
                      struct thandle *th)
{
		...
		if (OBD_FAIL_CHECK(OBD_FAIL_FID_NLINK))
			inode->i_nlink = 0;
        } else {
                if (obj->oo_hl_head != NULL) {
                        ldiskfs_htree_lock_head_free(obj->oo_hl_head);
				     ...
				     struct thandle *handle)
{
	...
	if (attr && S_ISDIR(attr->la_mode)) {
		...
		osd_trans_declare_op(oh, OSD_OT_INSERT, 0);
	}
                             ...
                             struct thandle *th)
{
	...
	osd_trans_exec_op(th, OSD_OT_CREATE);
	osd_trans_declare_rb(th, OSD_OT_REF_ADD);
				      ...
				      struct thandle *th)
{
	...
	osd_trans_declare_op(oh, OSD_OT_DELETE,
			     osd_dto_credits_noquota[DTO_OBJECT_DELETE]);
	/* Recycle idle OI leaf may cause additional three OI blocks
	 * to be changed. */
	osd_trans_declare_op(oh, OSD_OT_DESTROY,
                              ...
                              struct thandle *th)
{
	...
	osd_trans_exec_op(th, OSD_OT_DESTROY);
                                      ...
                                      struct thandle *handle)
{
	...
	osd_trans_declare_op(oh, OSD_OT_REF_ADD,
			     osd_dto_credits_noquota[DTO_ATTR_SET_BASE]);
                              ...
                              struct dt_object *dt, struct thandle *th)
{
	...
	osd_trans_exec_op(th, OSD_OT_REF_ADD);
                                      ...
                                      struct thandle *handle)
{
	...
	osd_trans_declare_op(oh, OSD_OT_REF_DEL,
			     osd_dto_credits_noquota[DTO_ATTR_SET_BASE]);
                              ...
                              struct thandle *th)
{
	...
	osd_trans_exec_op(th, OSD_OT_REF_DEL);
                                 ...
                                 int fl, struct thandle *handle)
{
	...
	osd_trans_declare_op(oh, OSD_OT_XATTR_SET,
                         ...
                         struct thandle *handle, struct lustre_capa *capa)
{
	...
	osd_trans_exec_op(handle, OSD_OT_XATTR_SET);
                                ...
                                struct lustre_capa *capa)
{
	...
	osd_trans_exec_op(handle, OSD_OT_DELETE);
                                ...
                                struct lustre_capa *capa, int ignore_quota)
{
	...
	osd_trans_exec_op(th, OSD_OT_INSERT);
                            ...
                            struct htree_lock *hlock, struct thandle *th)
{
	...
	if (!fid_is_client_mdt_visible((const struct lu_fid *)fid) ||
	    pobj->oo_inode == osd_sb(osd_obj2dev(pobj))->s_root->d_inode)
		ldp->edp_magic = 0;
	else
                              ...
                              struct thandle *th)
{
		...
		if (!fid_is_client_mdt_visible(
					(const struct lu_fid *)dot_dot_fid))
			dot_dot_ldp->edp_magic = 0;
		else
                             ...
                             struct dt_rec *rec, const struct dt_key *key)
{
	...
	ENTRY;
		...
		if (OBD_FAIL_CHECK(OBD_FAIL_FID_LOOKUP)) {
			...
			inode = osd_iget(oti, dev, &oic->oic_lid, true);
			if (IS_ERR(inode)) {
				fid_zero(&oic->oic_fid);
...
static inline void osd_it_append_attrs(struct lu_dirent *ent, int len,
				       __u16 type, __u64 clue)
{
	int align;
	...
	} else if (ent->lde_attrs & LUDA_VERIFY) {
		__u64 *p;

		align = sizeof(__u64) - 1;
		len = (len + align) & ~align;
		p = (__u64 *)(ent->lde_name + len);
		/* Use the opaque clue without swab. */
		*p = clue;
	}
...
osd_it_pack_dirent(struct lu_dirent *ent, struct lu_fid *fid, __u64 offset,
		   char *name, __u16 namelen, __u16 type, __u32 attr,__u64 clue)
{
	...
	osd_it_append_attrs(ent, namelen, type, clue);
}
...
osd_dirent_check_repair(const struct lu_env *env, struct osd_object *obj,
			...
			struct osd_inode_id *id, __u64 *clue, __u32 *attr)
{
	struct osd_thread_info     *info	= osd_oti_get(env);
	struct lustre_mdt_attrs    *lma		= &info->oti_mdt_attrs;
	struct osd_device	   *dev		= osd_obj2dev(obj);
...
again:
	...
	inode = osd_iget(info, dev, id, false);
	if (IS_ERR(inode)) {
		rc = PTR_ERR(inode);
	...
	/* We locked one name entry, trust it, so the inode should be there. */
	if (unlikely(inode->i_nlink == 0)) {
		/* Do not repair under dryrun mode. */
		if (*attr & LUDA_VERIFY_DRYRUN) {
			*attr |= LUDA_REPAIR | LUDA_LOST_FOUND;
			GOTO(out_inode, rc = 0);
		}
		...
		dirty = true;
		/* The object nlink count may be not 1, MDD layer LFSCK will
		 * try to correct it. */
		inode->i_nlink = 1;
	...
	rc = __osd_xattr_get(inode, &info->oti_obj_dentry, XATTR_NAME_CLUE,
			     (void *)clue, sizeof(__u64));
	if (rc != 0)
		*clue = 0;
...
out_inode:
	if (*attr & LUDA_LOST_FOUND && fid_is_sane(fid)) {
		struct dt_object *dt;
		struct osd_object *target;

		dt = dt_locate(env, &dev->od_dt_dev, fid);
		if (!IS_ERR(dt)) {
			target = osd_dt_obj(dt);
			if (target->oo_inode != inode)
				set_bit(LU_OBJECT_HEARD_BANSHEE,
					&dt->do_lu.lo_header->loh_flags);
			osd_object_put(env, target);
		}
	}
				...
				struct dt_rec *dtrec, __u32 attr)
{
	...
	if (attr & LUDA_VERIFY) {
		...
		rc = osd_dirent_check_repair(env, obj, it, fid,
					     &oic->oic_lid, &clue, &attr);
	} else {
		if (!fid_is_sane(fid)) {
			if (OBD_FAIL_CHECK(OBD_FAIL_FID_LOOKUP))
				RETURN(-ENOENT);
...
pack:
	osd_it_pack_dirent(lde, fid, it->oie_dirent->oied_off,
			   ...
			   it->oie_dirent->oied_namelen,
			   it->oie_dirent->oied_type, attr, clue);
	oic->oic_fid = *fid;
	if (!(attr & LUDA_VERIFY) &&
                       ...
                       struct lu_device *dev)
{
	...
	LDISKFS_SB(osd_sb(osd))->s_mount_state |= LDISKFS_ORPHAN_FS;
