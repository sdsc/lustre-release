                                ...
                                struct thandle *th)
{
	...
	if (result == 0 && fid_seq(fid) >= FID_SEQ_START) {
		LASSERTF(!fid_is_igif(fid), DFID, PFID(fid));
                            ...
                            struct htree_lock *hlock, struct thandle *th)
{
	...
	/* XXX: add FID-in-direntry only for ROOT for now, that should be just
	 * extended for all named files when LU-2462 is fixed */
	if (fid_is_norm((struct lu_fid *)fid) &&
