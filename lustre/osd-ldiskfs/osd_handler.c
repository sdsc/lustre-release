...
/**
 * Read FID from the inode LMA, and verify/return the FID according to @flags.
 *
 * OSD_IF_VERIFY : verify whether the FID is equal to the given @fid.
 * OSD_IF_RET_FID: return the FID for normal fid, otherwise, IGIF will be used.
 */
static int osd_iget_lma(struct osd_thread_info *info, struct inode *inode,
		...
		if (lma->lma_incompat & ~cpu_to_le32(LMA_INCOMPAT_SUPP)) {
			...
			if (flags & OSD_IF_VERIFY) {
				if (!lu_fid_eq(fid, &lma->lma_self_fid)) {
					CDEBUG(D_LFSCK, "inconsistent obj: "
					       DFID", %lu, "DFID"\n",
		       ...
		       enum osd_iget_flags flags)
{
	...
	if (IS_ERR(inode)) {
		...
		* we can have dead inode here. */
		...
		if (flags != 0) {
			rc = osd_iget_lma(info, inode, id, fid, flags);
			if (rc != 0) {
				iput(inode);
				inode = ERR_PTR(rc);
			}
