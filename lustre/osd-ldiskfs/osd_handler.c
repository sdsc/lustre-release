...
lookup:
	...
	if (result == -ENOENT) {
		if (!fid_is_idif(fid) &&
		    ((sf->sf_flags & SF_UPGRADED && fid_is_igif(fid)) ||
...
/**
 * \retval 1	inconsistency is found, may be fixed, depends on dryrun or not
 * \retval 0	in consistency, without fixing
			...
			struct osd_inode_id *id, __u64 *clue)
{
	...
	credits = osd_dto_credits_noquota[DTO_INDEX_DELETE] +
		  osd_dto_credits_noquota[DTO_INDEX_INSERT] + 2;
	jh = ldiskfs_journal_start_sb(sb, credits);
	if (IS_ERR(jh)) {
		rc = PTR_ERR(jh);
		CERROR("%.16s: fail to start trans for dirent check_repair: "
		       "credits = %d, name = %.*s, rc = %d\n",
		       devname, credits, ent->oied_namelen, ent->oied_name, rc);
		RETURN(rc);
	}
