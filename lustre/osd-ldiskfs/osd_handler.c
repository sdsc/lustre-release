...
static int osd_iget_lma(struct inode *inode, struct dentry *dentry,
		       ...
		       struct osd_inode_id *id)
{
	...
	char	     *name  = dev->od_dt_dev.dd_lu_dev.ld_obd->obd_name;

	inode = ldiskfs_iget(osd_sb(dev), id->oii_ino);
	if (IS_ERR(inode)) {
		CWARN("%s: no inode: ino = %u, rc = %ld\n",
		      name, id->oii_ino, PTR_ERR(inode));
	} else if (id->oii_gen != OSD_OII_NOGEN &&
		   inode->i_generation != id->oii_gen) {
		CWARN("%s: unmatched inode: ino = %u, gen0 = %u, gen1 = %u\n",
		      name, id->oii_ino, id->oii_gen, inode->i_generation);
		iput(inode);
		inode = ERR_PTR(-ESTALE);
	} else if (inode->i_nlink == 0) {
		/* due to parallel readdir and unlink,
		* we can have dead inode here. */
		CWARN("%s: stale inode: ino = %u\n", name, id->oii_ino);
...
struct inode *osd_iget_plus(struct osd_thread_info *info,
			    struct osd_device *dev, struct osd_inode_id *id,
			    struct lu_fid *fid, enum osd_iget_plus_flags flags)
{
	...
	if (flags & OSD_IPF_GEN_LID) {
		osd_id_gen(id, inode->i_ino, inode->i_generation);
		if (flags == OSD_IPF_GEN_LID)
			return inode;
	}

	rc = osd_iget_lma(inode, &info->oti_obj_dentry, lma);
	if (rc == -ENODATA && flags & OSD_IPF_RET_FID) {
                              ...
                              struct thandle *th)
{
	...
	/* XXX: parallel control for OI scrub. For most of cases, there is no
			  ...
			  struct osd_inode_id *id)
{
	struct osd_thread_info   *info  = osd_oti_get(env);
	struct inode		 *inode;
	enum osd_iget_plus_flags  flags = OSD_IPF_GEN_LID;
	ENTRY;

	osd_id_gen(id, ino, OSD_OII_NOGEN);
	inode = osd_iget_plus(info, osd_obj2dev(obj), id, fid,
			      flags | OSD_IPF_RET_FID);
	if (IS_ERR(inode))
