...
static int
osd_ldiskfs_filterfid(void *buf, const char *name, int namelen, loff_t offset,
		      __u64 ino, unsigned d_type)
{
	...
	if (IS_ERR(inode)) {
		...
		return (rc == -ENOENT || rc == -ESTALE) ? 1 : 0;
	}
...
#else
	rc = fops->readdir(filp, &oclb, osd_ldiskfs_filterfid);
#endif
...
start:
				...
				LCONSOLE_WARN("%.16s: trigger %s OI scrub by "
					      "RPC for "DFID", rc = %d [1]\n",
					      osd_name(dev),
					      flags & SS_AUTO_FULL ?
						"full" : "partial",
...
trigger:
	...
	if (!dev->od_noscrub && ++once == 1) {
		rc = osd_scrub_start(dev, SS_AUTO_PARTIAL | SS_CLEAR_DRYRUN |
				     SS_CLEAR_FAILOUT);
		CDEBUG(D_LFSCK | D_CONSOLE, "%.16s: trigger partial OI scrub "
		       "by RPC for "DFID", rc = %d [2]\n",
		       LDISKFS_SB(osd_sb(dev))->s_es->s_volume_name,
		       PFID(fid), rc);
		if (rc == 0 || rc == -EALREADY)
			goto again;
