 ...
 * and the item's name. If the system is resotred from MDT file level backup,
 ...
 *
 * 1) The slave MDT-object and the MDT-object are on different MDTs. It is
 *    relative easy. Be as one of remote MDT-objects, the slave MDT-object
 ...
 *
 * 2) The slave MDT-object and the MDT-object reside on the same MDT. Under
 *    such case, during lookup the master MDT-object, we will lookup the slave
 *    MDT-object via readdir against the master MDT-object, because the slave
 *    MDT-objects information are stored as sub-directories with the name
...
iget:
	/* The OI mapping was found in cache or OI, look it up by ino#. */
...
trigger:
			/* Couldn't find the FID the client wanted, might be
			 * something wrong with the filesystem that will need
			 * LFSCK to verify or repair. */
...
start:
				...
				CDEBUG(D_LFSCK | D_CONSOLE | D_WARNING,
				       "%.16s: trigger %s OI scrub for RPC "
				       "lookup of missing FID"DFID": rc = %d\n",
				       osd_name(dev), flags & SS_AUTO_FULL ?
