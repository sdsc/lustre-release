...
static const struct dt_object_operations      osd_obj_lfsck_it_ops;
			   ...
			   const struct lu_object_conf *conf)
{
	...
	if (result == 0) {
		...
		} else if (fid_is_lfsck_it(&l->lo_header->loh_fid)) {
			obj->oo_dt.do_ops = &osd_obj_lfsck_it_ops;
                         ...
                         const struct dt_index_features *feat)
{
        ...
        if (osd_object_is_root(obj)) {
                ...
                if (S_ISDIR(obj->oo_inode->i_mode))
                        ...
                        result = -ENOTDIR;
                ea_dir = 1;
	} else if (fid_is_lfsck_it(&dt->do_lu.lo_header->loh_fid)) {
...
static int osd_lfsck_it_attr_get(const struct lu_env *env,
				 struct dt_object *dt,
				 struct lu_attr *attr,
				 struct lustre_capa *capa)
{
...
static const struct dt_object_operations osd_obj_lfsck_it_ops = {
	.do_attr_get	= osd_lfsck_it_attr_get,
	.do_index_try	= osd_index_try,
};
