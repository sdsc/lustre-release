            ...
            struct dt_device *dev, char *name, struct dt_object **objp)
{
        ...
        if (rc != 0) {
                ...
                CERROR("Wrong index %s on %s: %d\n",
                       name, dev->dd_lu_dev.ld_obd->obd_name, rc);
                return rc;
        }
                  ...
                  struct osd_oi *oi_table, unsigned oi_count, int try_all)
{
        ...
        for (i = 0; i < (oi_count != 0 ? oi_count : OSD_OI_FID_NR_MAX); i++) {
                ...
                CERROR("Can't open %s of total %d on %s: %d\n",
                       name, oi_count, dev->dd_lu_dev.ld_obd->obd_name, rc);
                break;
        }
                ...
                struct md_device *mdev)
{
        ...
        if (rc != -ENOENT) {
                CERROR("Can't open %s on %s: %d\n",
                       OSD_OI_NAME_BASE, dev->dd_lu_dev.ld_obd->obd_name, rc);
                goto out;
        }
