...
#define OSD_OI_NAME           "oi"
#define OSD_OI_16_NAME        "oi.16"

static int osd_oi_index_create(struct osd_thread_info *info,
                               struct dt_device *dev,
                               struct md_device *mdev, int oi_num)
{
        ...
        for (i = 0; i < oi_num; ++i) {
                ...
                sprintf(name, "%s.%d", OSD_OI_NAME, i);
...
/**
 * Open OI(Object Index) containers.
 * If \a oi_num is zero, which means caller doesn't know how many OIs there
 * will be, this function can either return 0 for new filesystem, or number
 * of OIs on existed filesystem.
 *
 * If \a oi_num is non-zero, which means caller does know number of OIs on
 * filesystem, this function should return the exactly same number on
 * success, or error code in failure.
 *
 * \param  oi_num  Number of expected OI containers
 *
...
osd_oi_open(struct osd_thread_info *info,
            struct dt_device *dev, struct osd_oi *oi_table, unsigned oi_num)
{
        ...
        /* NB: oi_num != 0 means that we have already created/known all OIs
         * and have known exact number of OIs. */
        LASSERT(oi_num <= OSD_OI_FID_NR_MAX);

        for (i = 0; i < (oi_num != 0 ? oi_num : OSD_OI_FID_NR_MAX); i++) {
                ...
                if (IS_ERR(obj)) {
                        rc = PTR_ERR(obj);
                        /* it's new filesystem and probably no OI at all? */
                        if (oi_num == 0 && rc == -ENOENT)
                                continue;

                        CERROR("Can't open %s of total %d: %d\n",
                               name, oi_num, rc);
                        goto failed;
                }
                /* successfully opened an OI container */

                if (single_oi) { /* already found o.16 from old version */
                        CERROR("Unexpected %s for single OI mode\n", name);
                        goto failed;
                }

                if (count == 0 && strcmp(name, OSD_OI_16_NAME) == 0) {
                        single_oi = 1;
                        CDEBUG(D_INFO, "Open single OI for old versions\n");

                } else if (count != i) {
                        CERROR("Noncontinuous OIs, %s fid "DFID"\n",
                               name, PFID(&info->oti_fid));
                        goto failed;
                }
                ...
                oi_table[count++].oi_dir = obj;
        }

        return count;
 failed:
        CERROR("Failed to open all OIs\n");
                ...
                struct md_device *mdev)
{
        ...
        rc = osd_oi_open(info, dev, oi, 0);
        if (rc != 0)
                goto out;

        /* create OI objects */
        rc = osd_oi_index_create(info, dev, mdev, osd_oi_num);
        if (rc != 0)
                goto out;

        rc = osd_oi_open(info, dev, oi, osd_oi_num);
...
void osd_oi_fini(struct osd_thread_info *info, struct osd_oi **oi_table)
{
        ...
        for (i = 0; i < OSD_OI_FID_NR_MAX; i++) {
                if (oi[i].oi_dir != NULL) {
                        lu_object_put(info->oti_env, &oi[i].oi_dir->do_lu);
                        oi[i].oi_dir = NULL;
                }
        }
