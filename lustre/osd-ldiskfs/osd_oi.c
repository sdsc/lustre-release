				   ...
				   struct dt_index_features *feat)
{
	...
	if (bh) {
		...
		brelse(bh);
		inode = osd_iget(info, osd, id, true);
		if (!IS_ERR(inode)) {
			iput(inode);
                       ...
                       char *name, struct osd_oi **oi_slot, bool create)
{
	...
	ldiskfs_set_inode_state(inode, LDISKFS_STATE_LUSTRE_NOOI);
        OBD_ALLOC_PTR(oi);
        if (oi == NULL)
...
{
	if (fid_is_idif(fid))
	       return osd_compat_objid_lookup(info, osd, fid, id);
		  ...
		  struct thandle *th)
{
	...
	/* XXX: no OI mapping for OST object. Need more handle for DNE case. */
	if (fid_is_idif(fid))
	...
	if (rc != 0) {
		...
		inode = osd_iget(info, osd, oi_id, true);
		if (IS_ERR(inode)) {
			rc = PTR_ERR(inode);
...
update:
	...
	seq = fid_seq(fid);
	if (unlikely(seq == FID_SEQ_LOCAL_FILE))
		rc = osd_compat_spec_insert(info, osd, fid, id, th);
	else if (unlikely(seq == FID_SEQ_LLOG))
		/* Old 2.x/1.8 servers need that. */
		rc = osd_compat_objid_insert(info, osd, fid, id, th);
	return rc;
}
		  ...
		  struct thandle *th)
{
	...
	if (fid_seq(fid) == FID_SEQ_LLOG)
		osd_compat_objid_delete(info, osd, fid, th);
