...
struct osd_obj_seq {
	...
	spinlock_t	 oos_last_id_lock; /* For non-sleepable
					    * oos_last_id changes. */
	struct semaphore oos_last_id_sem;  /* For sleepable contention
					    * when update LAST_ID file. */
...
int osd_obj_map_update(struct osd_thread_info *info, struct osd_device *osd,
		       const struct lu_fid *fid, const struct osd_inode_id *id,
		       struct thandle *th);
int osd_obj_map_rename(struct osd_thread_info *info, struct osd_device *osd,
		       struct dentry *src_parent, struct dentry *src_child,
