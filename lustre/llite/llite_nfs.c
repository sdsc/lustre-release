                                   ...
                                   loff_t hash, u64 ino, unsigned type)
{
        ...
        if (ino == lgd->lgd_inode->i_ino) {
                /* It is hack to access lde_fid for comparison with lgd_fid.
                 * So the input 'name' must be the member of 'lu_dirent'. */
                lde = container_of0(name, struct lu_dirent, lde_name);
                fid_le_to_cpu(&fid, &lde->lde_fid);
                if (likely(lu_fid_eq(&fid,
                                     &ll_i2info(lgd->lgd_inode)->lli_fid))) {
