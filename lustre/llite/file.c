                             ...
                             struct ptlrpc_request **request)
{
	...
	if (LOV_MAGIC != cpu_to_le32(LOV_MAGIC))
		lustre_swab_lov_user_md(lmm, LL_TO_HOST,
...
static int ll_file_getstripe(struct inode *inode, unsigned long arg)
{
	struct lu_env	*env;
	int		refcheck;
	int		rc = -ENODATA;
			    ...
			    unsigned long arg)
{
	...
	if (rc == 0) {
		...
		rc = ll_file_getstripe(inode, arg);
	}
 ...
 *
 * @param flags whether do sync on the OST side;
 *		0: no sync
...
ll_file_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case LL_IOC_LOV_GETSTRIPE:
		RETURN(ll_file_getstripe(inode, arg));
