...
static int ll_md_close(struct obd_export *md_exp, struct inode *inode,
		       struct file *file)
{
	struct inode *dinode = file->f_path.dentry->d_inode;
	...
	if (dinode != NULL) { /* Can this ever be false? */
                int lockmode;
		...
		if (!md_lock_match(md_exp, flags, ll_inode2fid(d_inode),
                                   LDLM_IBITS, &policy, lockmode,
                                   &lockh)) {
			rc = ll_md_real_close(d_inode,
                                              fd->fd_omode);
                }
	} else {
		CERROR("released file has negative dentry: file = %p, "
		       "dentry = %p, name = %s\n", file, file->f_path.dentry,
