				      ...
				      struct file_lock *file_lock)
{
	...
	rc = md_enqueue_async(sbi->ll_md_exp, &einfo, &flock, NULL, op_data,
			      ll_flock_upcall, 0);
...
static int ll_flock_upcall(void *cookie, int err)
{
	...
	struct mdc_enqueue_args *mea = cookie;
...
int ll_file_flock(struct file *file, int cmd, struct file_lock *file_lock)
{
	...
	if (file_lock->fl_lmops && file_lock->fl_lmops->lm_grant &&
		...
		rc = md_enqueue_async(sbi->ll_md_exp, &einfo, &flock, NULL,
				      op_data, ll_flock_upcall, flags);
		if (rc) {
			fput(file);
