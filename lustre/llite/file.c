                ...
                struct file *file)
{
	...
	if (fd->fd_lease_och != NULL) {
		...
		rc = ll_lease_close(fd->fd_lease_och, &lease_broken);
                       ...
                       struct lookup_intent *it, struct obd_client_handle *och)
{
	...
	och->och_lockh.cookie = it->d.lustre.it_lock_handle;
	och->och_inode = ll_info2i(lli);
...
struct obd_client_handle *ll_lease_open(struct inode *inode, struct file *file,
					mode_t mode)
{
	struct lookup_intent it = { .it_op = IT_OPEN, .it_flags = mode };
	struct ll_sb_info *sbi = ll_i2sbi(inode);
	struct md_op_data *op_data;
	struct ptlrpc_request *req;
	...
	if (mode != FMODE_WRITE && mode != FMODE_READ)
	...
	if (file != NULL) {
		...
		if (!(mode & file->f_mode) || (file->f_mode & FMODE_EXEC))
	...
	op_data = ll_prep_md_op_data(NULL, inode, inode, NULL, 0, mode,
					LUSTRE_OPC_ANY, NULL);
	if (IS_ERR(op_data))
	...
	it.it_flags = mode | MDS_OPEN_LOCK | MDS_OPEN_BY_FID | MDS_OPEN_LEASE;
	rc = md_intent_lock(sbi->ll_md_exp, op_data, NULL, 0, &it, 0, &req,
				ll_md_blocking_lease_ast, LDLM_FL_NO_LRU);
	ll_finish_md_op_data(op_data);
	if (req != NULL) {
		ptlrpc_req_finished(req);
	...
	if (it.d.lustre.it_lock_mode == 0 ||
		...
		CERROR("ERROR: lease granted but no open lock, %d/%Lu.\n",
...
out_close:
	...
	/* cancel open lock */
	if (it.d.lustre.it_lock_mode != 0) {
		ldlm_lock_decref_and_cancel(&och->och_lockh,
 ...
 */
int ll_lease_close(struct obd_client_handle *och, bool *lease_broken)
{
	struct inode *inode = och->och_inode;
	struct ldlm_lock *lock;
	int rc;
	ENTRY;

	*lease_broken = true;
	lock = ldlm_handle2lock(&och->och_lockh);
	if (lock != NULL) {
		lock_res_and_lock(lock);
		if (!(lock->l_flags & LDLM_FL_CANCEL))
			*lease_broken = false;
		unlock_res_and_lock(lock);
		LDLM_DEBUG(lock, "lease lock cancelled? %d\n", *lease_broken);
		ldlm_lock_put(lock);
	}

	if (!*lease_broken)
		ldlm_cli_cancel(&och->och_lockh, LCF_ASYNC);
...
long ll_file_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case LL_IOC_SET_LEASE: {
		...
		if (arg == F_UNLCK) {
			...
			if (och != NULL) {
				...
				rc = ll_lease_close(och, &lease_broken);
		...
		CDEBUG(D_INODE, "Get lease with mode %d\n", mode);

		/* apply for lease */
		och = ll_lease_open(file->f_dentry->d_inode, file, mode);
		if (IS_ERR(och))
			RETURN(PTR_ERR(och));
		...
		if (och != NULL) {
			...
			ll_lease_close(och, &lease_broken);
			rc = -EBUSY;
		}
		RETURN(rc);
	}
	case LL_IOC_GET_LEASE: {
		...
		mutex_lock(&lli->lli_och_mutex);
		if (fd->fd_lease_och != NULL)
			lock = ldlm_handle2lock(&fd->fd_lease_och->och_lockh);
		mutex_unlock(&lli->lli_och_mutex);
		...
		if (lock != NULL) {
			...
			if (!ldlm_is_cancel(lock))
				rc = file->f_mode & (FMODE_READ|FMODE_WRITE);
			unlock_res_and_lock(lock);
			ldlm_lock_put(lock);
		}
		RETURN(rc);
