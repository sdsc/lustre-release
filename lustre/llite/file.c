...
int ll_flock_upcall(void *cookie, int err);
int
...
static void ll_file_flock_async_cb(struct ldlm_flock_info *args, int err)
{
	...
	if (!(args->fa_flags & FA_FL_CANCELED)) {
		if (err == 0) {
			err = ll_file_flock_lock(file, flc);
			if (err) {
				CDEBUG(D_ERROR, "ll_file_flock_lock failed "
						"err=%d file=%p %llu-%llu "
						"flags=%x\n",
				       err, file, flc->fl_start, flc->fl_end,
				       flc->fl_flags);
...
int ll_file_flock(struct file *file, int cmd, struct file_lock *file_lock)
{
	...
	CDEBUG(D_DLMTRACE, "inode=%lu, pid=%u, owner="LPX64", flags=%#llx, "
	...
	if (file_lock->fl_lmops && file_lock->fl_lmops->fl_grant &&
		...
		if (rc == 0 && file_lock->fl_type != F_UNLCK &&
			...
			if (rc2) {
				einfo.ei_mode = LCK_NL;
				md_enqueue(sbi->ll_md_exp, &einfo, NULL, op_data,
					   &lockh, &flock, 0, NULL /* req */,
					   flags);
				rc = rc2;
			}
		}
		if (rc && file_lock->fl_type == F_UNLCK &&
			...
			if (obd && obd->obd_force) {
				...
				CDEBUG(D_ERROR, "unlock failed (%d). Reacquire "
						"kerlnel lock.\n", rc);
				locks_init_lock(&flock);
				locks_copy_lock(&flock, file_lock);
				flock.fl_type = F_RDLCK;
				ll_file_flock_lock(file, &flock);
			}
		}
	}
out:
