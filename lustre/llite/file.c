...
restart:
	...
	if (cl_io_rw_init(env, io, iot, *ppos, count) == 0) {
		...
		case IO_NORMAL:
			vio->vui_iov = local_iov;
			vio->vui_nrsegs = nrsegs;
...
out:
	...
	if ((result == 0 || rc == -ENODATA) && io->ci_need_restart) {
	...
	/**
	 * Restart non-error IO for short read/write due to possible layout lock
	 * revocation.
	 */
	if (rc == 0 && result > 0 && io->u.ci_wr.wr.crw_count > 0 &&
	    (iot == CIT_WRITE ||
