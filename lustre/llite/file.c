...
static int ll_file_getstripe(struct inode *inode, void __user *arg)
{
	struct lu_env	*env;
	int		refcheck;
	int		rc = -ENODATA;
	...
	rc = CL_OBJECT_OP(env, ll_i2info(inode)->lli_clob, getstripe, arg);
	cl_env_put(env, &refcheck);
	RETURN(rc);
}
			    ...
			    unsigned long arg)
{
	...
	if (rc == 0) {
		...
		rc = ll_file_getstripe(inode, (void __user *)arg);
	}
...
ll_file_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case LL_IOC_LOV_GETSTRIPE:
		RETURN(ll_file_getstripe(inode, (void __user *)arg));
