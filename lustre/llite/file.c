				   ...
				   int is_parallel, int *perr)
{
	...
	CDEBUG(D_VFSTRACE, "file: %s, generic %s pos: %lld, count: %zu\n",
...
restart:
	...
	vio = vvp_env_io(env);
	if (!cl_io_rw_init(env, io, iot, *ppos, count)) {
	...
	if ((rc == 0 || rc == -ENODATA) && count > 0 && io->ci_need_restart) {
		CDEBUG(D_VFSTRACE, "file: %s, restart %s pos: %lld, "
	...
	CDEBUG(D_VFSTRACE, "file: %s, finish %s pos: %lld, count: %zu, "
		"result: %zd, rc: %d\n", file_dentry(file)->d_name.name,
				  ...
				  loff_t *ppos, size_t count)
{
	...
	CDEBUG(D_VFSTRACE, "file: %s, type: %d, pos: %lld, count: %zu, "
		"chunks: %zu, csize: %zu\n", file_dentry(file)->d_name.name,
...
out:
	...
	CDEBUG(D_VFSTRACE, "file: %s, type: %d, pos: %lld, count: %zu, "
		"result: %zd, rc: %d\n", file_dentry(file)->d_name.name, iot,
		*ppos, count, result, rc);
