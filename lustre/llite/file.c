 ...
 * the CLOSE rpc.
 */
static void ll_prepare_close(struct inode *inode, struct inode *inode2,
...
out:
	...
	ll_prep_md_op_data(op_data, inode, inode2, NULL,
			   0, 0, LUSTRE_OPC_ANY, NULL);
        EXIT;
}

/**
 * Perform a close, possibly with an intent.
 * The meaning of "data" depends on the value of "intent".
 *
 * If intent is MDS_HSM_RELEASE, data is a pointer to the data version.
 * If intent is MDS_CLOSE_LAYOUT_SWAP, data is a pointer to the inode to
 * swap layouts with.
 */
				     ...
				     const void *data)
{
	...
	case MDS_CLOSE_LAYOUT_SWAP:
		LASSERT(data != NULL);
		ll_prepare_close(inode, (struct inode *)data, op_data, och);
		op_data->op_bias |= MDS_CLOSE_LAYOUT_SWAP;
		op_data->op_data_version = 0;
		op_data->op_lease_handle = och->och_lease_handle;
		op_data->op_fid2 = ll_i2info((struct inode *)data)->lli_fid;
		break;

	case MDS_HSM_RELEASE:
		LASSERT(data != NULL);
		ll_prepare_close(inode, NULL, op_data, och);
	...
	if (rc == 0 &&
		...
		if (!(body->valid & OBD_MD_CLOSE_INTENT_EXECED))
			rc = -EBUSY;
	}
...
static int ll_close_layout_swap(struct obd_client_handle *och,
				struct inode *inode, struct inode *inode2)
{
	int	rc;
	ENTRY;

	CDEBUG(D_INODE, "%s: intent close of file "DFID".\n",
	       ll_get_fsname(inode->i_sb, NULL, 0),
