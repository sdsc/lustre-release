				     ...
				     __u64 *data_version)
{
	...
	if (data_version != NULL) {
		/* XXX: too bad - pass in data_version assumes release */
		op_data->op_bias |= MDS_HSM_RELEASE;
		op_data->op_data_version = *data_version;
		op_data->op_lease_handle = och->och_lockh;
		op_data->op_attr.ia_valid |= ATTR_SIZE | ATTR_BLOCKS;
	}
			...
			bool *lease_broken)
{
	...
	*lease_broken = true;
	...
	if (lock != NULL) {
		...
		*lease_broken = ldlm_is_cancel(lock);
		unlock_res_and_lock(lock);
		ldlm_lock_put(lock);
	}

	CDEBUG(D_INODE, "lease for "DFID" broken? %d\n",
		PFID(&ll_i2info(inode)->lli_fid), *lease_broken);
