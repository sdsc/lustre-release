                     ...
                     __u64 ioepoch, int sync)
{
	...
	rc = cl_object_getattr(env, ll_i2info(inode)->lli_clob, &oinfo, NULL);
	if (rc == 0) {
...
static int ll_ioctl_fiemap(struct inode *inode,
...
int ll_data_version(struct inode *inode, __u64 *data_version, int flags)
{
	...
	rc = cl_object_dataversion(env, ll_i2info(inode)->lli_clob,
				   data_version, flags);
	cl_env_put(env, &refcheck);
	RETURN(rc);
}
