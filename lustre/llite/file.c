...
void ll_dom_lvb_update(const struct lu_env *env, struct inode *inode)
{
	...
	if (attr->cat_size > attr->cat_kms) {
		CDEBUG(D_INODE, "update from attr, setting rss="LPU64", kms="
...
static inline int ll_dom_readpage(void *data, struct page *page)
{
	...
	if (lnb->lnb_len < PAGE_CACHE_SIZE)
		memset(kaddr + lnb->lnb_len, 0,
		       PAGE_CACHE_SIZE - lnb->lnb_len);
			...
			struct lookup_intent *it)
{
	...
	CDEBUG(D_INFO, "Get data buffer along with open, len %i, i_size "
	       LPU64"\n", rnb->rnb_len, i_size_read(inode));


	lnb.lnb_file_offset = rnb->rnb_offset;
	start = lnb.lnb_file_offset / PAGE_CACHE_SIZE;
	index = 0;
	LASSERT(lnb.lnb_file_offset % PAGE_CACHE_SIZE == 0);
	lnb.lnb_page_offset = 0;
	do {
		struct cl_page *clp;

		lnb.lnb_data = data + (index << PAGE_CACHE_SHIFT);
		lnb.lnb_len = rnb->rnb_len - (index << PAGE_CACHE_SHIFT);
		if (lnb.lnb_len > PAGE_CACHE_SIZE)
			lnb.lnb_len = PAGE_CACHE_SIZE;
		...
		index++;
	} while (rnb->rnb_len > (index << PAGE_CACHE_SHIFT));
	rc = 0;
	EXIT;
out_env:
