				...
				unsigned long nr_segs, loff_t pos)
{
	...
	OBD_ALLOC(to, sizeof(*to));
	if (to == NULL) {
		result = -ENOMEM;
		goto out;
	}
#ifdef HAVE_NEW_ITER_INIT
	iov_iter_init(to, READ, local_iov, nr_segs, count);
#else
	iov_iter_init(to, local_iov, nr_segs, count, 0);
#endif

	result = ll_file_read_iter(iocb, to);

	if (to != NULL)
		OBD_FREE(to, sizeof(*to));
out:
	if (nr_segs > 1)
				 ...
				 unsigned long nr_segs, loff_t pos)
{
	...
	OBD_ALLOC(from, sizeof(*from));
	if (from == NULL) {
		result = -ENOMEM;
		goto out;
	}
#ifdef HAVE_NEW_ITER_INIT
	iov_iter_init(from, WRITE, local_iov, nr_segs, count);
#else
	iov_iter_init(from, local_iov, nr_segs, count, 0);
#endif

	result = ll_file_write_iter(iocb, from);

	if (from != NULL)
		OBD_FREE(from, sizeof(*from));
out:
	if (nr_segs > 1)
		OBD_FREE(local_iov, sizeof(*iov) * nr_segs);
