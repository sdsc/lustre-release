...
static int ll_get_fid_by_name(struct inode *parent, char *name,
	       ...
	       char *name, int namelen)
{
	...
	if (dchild != NULL) {
		if (dchild->d_inode != NULL) {
			CDEBUG(D_INFO, "clear inode "DFID"%p cache.\n",
			       PFID(ll_inode2fid(dchild->d_inode)),
			       dchild->d_inode);
			if (S_ISREG(dchild->d_inode->i_mode))
				rc = cl_sync_file_range(dchild->d_inode, 0,
							OBD_OBJECT_EOF,
							CL_FSYNC_ALL, 0);
			truncate_inode_pages(&dchild->d_inode->i_data, 0);
			cl_inode_fini(dchild->d_inode);
		}
		dput(dchild);
	}
out_free:
...
static int ll_merge_md_attr(struct inode *inode)
{
	...
	LASSERT(ll_i2info(inode)->lli_lmv_md != NULL);
	rc = md_merge_attr(ll_i2mdexp(inode), ll_i2info(inode)->lli_lmv_md,
			   &attr);
	if (rc != 0)
		RETURN(rc);
			   ...
			   __u64 ibits)
{
	...
	if (!S_ISREG(inode->i_mode)) {
		if (S_ISDIR(inode->i_mode) &&
		    ll_i2info(inode)->lli_lmv_md != NULL) {
