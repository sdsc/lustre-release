		       ...
		       struct file *file)
{
	...
	if (file->f_path.dentry->d_inode) { /* Can this ever be false? */
                int lockmode;
		...
		mutex_unlock(&lli->lli_och_mutex);
                ...
                if (!md_lock_match(md_exp, flags, ll_inode2fid(inode),
                                   ...
                                   &lockh)) {
			rc = ll_md_real_close(file->f_path.dentry->d_inode,
                                              fd->fd_omode);
                }
	} else {
...
out:
	...
	if ((result == 0 || result == -ENODATA) && io->ci_need_restart) {
        ...
        if (iot == CIT_READ) {
                if (result >= 0)
			ll_stats_ops_tally(ll_i2sbi(file->f_path.dentry->d_inode),
