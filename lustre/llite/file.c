...
#ifdef HAVE_KUID_KGID_T
	if ((inode->i_mode & S_ISUID) && uid_eq(inode->i_uid, current_fsuid()))
		op_data->op_fsuid = from_kuid(&init_user_ns, current_uid());

	if ((inode->i_mode & S_ISGID) && gid_eq(inode->i_gid, current_fsgid()))
		op_data->op_fsgid = from_kgid(&init_user_ns, current_gid());
#else
