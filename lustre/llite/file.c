			   ...
			   __u64 ibits)
{
	...
	if (!S_ISREG(inode->i_mode)) {
		 ...
		 */
		/* search for a cached layout lock, will be always the case
		 * except in case of running restore */
		mode = ll_take_md_lock(inode, MDS_INODELOCK_LAYOUT, &lockh, 0);
		if (mode != 0) {
			/* hit cached lock, so we are sure there is no running
			 * restore we can safely glimpse */
