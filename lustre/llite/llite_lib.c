...
static struct ll_sb_info *ll_init_sbi(void)
{
	...
	atomic64_set(&sbi->ll_cache.ccc_lru_left, lru_page_max);
	spin_lock_init(&sbi->ll_cache.ccc_lru_lock);
	INIT_LIST_HEAD(&sbi->ll_cache.ccc_lru);

	atomic64_set(&sbi->ll_cache.ccc_unstable_nr, 0);
...
void ll_put_super(struct super_block *sb)
{
	...
	if (force == 0) {
		...
		rc = l_wait_event(sbi->ll_cache.ccc_unstable_waitq,
			atomic64_read(&sbi->ll_cache.ccc_unstable_nr) == 0,
			&lwi);
	}

	ccc_count = atomic64_read(&sbi->ll_cache.ccc_unstable_nr);
	if (force == 0 && rc != -EINTR)
		LASSERTF(ccc_count == 0, "count: %llu\n", ccc_count);
