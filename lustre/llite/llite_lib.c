...
static int ll_init_lsm_md(struct inode *inode, struct lustre_md *md)
{
	...
	for (i = 0; i < lsm->lsm_md_stripe_count; i++) {
		...
		if (lsm->lsm_md_hash_type == LMV_HASH_TYPE_MIGRATION && i == 0)
...
static void ll_update_lsm_md(struct inode *inode, struct lustre_md *md)
{
	...
	if (!lsm_md_eq(lli->lli_lsm_md, lsm)) {
		...
		CERROR("%s: inode "DFID":	"DFID"mismatch\n"
		       ...
		       PFID(&old_lsm->lsm_md_master_fid),
		       lsm, old_lsm, lsm->lsm_md_magic, old_lsm->lsm_md_magic,
		...
		for (idx = 0; idx < old_lsm->lsm_md_stripe_count; idx++) {
			CERROR("%s: FID in old lsm idx %d, old: "DFID"\n",
			       ll_get_fsname(inode->i_sb, NULL, 0), idx,
			       PFID(&old_lsm->lsm_md_oinfo[idx].lmo_fid));
		}

		for (idx = 0; idx < lsm->lsm_md_stripe_count; idx++) {
			CERROR("%s: FID in new lsm idx %d, new: "DFID"\n",
			       ll_get_fsname(inode->i_sb, NULL, 0), idx,
