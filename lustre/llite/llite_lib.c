...
void ll_update_inode(struct inode *inode, struct lustre_md *md)
{
		...
		if (body->atime > inode->i_atime.tv_sec) {
			inode->i_atime.tv_sec = body->atime;
			inode->i_atime.tv_nsec = body->atime_ns;
		} else if (body->atime == inode->i_atime.tv_sec &&
			body->atime_ns > inode->i_atime.tv_nsec) {
			inode->i_atime.tv_nsec = body->atime_ns;
		}
		...
		if (body->mtime > inode->i_mtime.tv_sec) {
		...
		} else if (body->mtime == inode->i_mtime.tv_sec &&
			   body->mtime_ns > inode->i_mtime.tv_nsec) {
			CDEBUG(D_INODE, "setting ino %lu mtime from %lu.%09lu "
			       "to "LPU64".%09u\n", inode->i_ino,
			       inode->i_mtime.tv_sec, inode->i_mtime.tv_nsec,
			       body->mtime, body->mtime_ns);
	...
	if (body->valid & OBD_MD_FLCTIME) {
		if (body->ctime > inode->i_ctime.tv_sec) {
			inode->i_ctime.tv_sec = body->ctime;
			inode->i_ctime.tv_nsec = body->ctime_ns;
		} else if (body->ctime == inode->i_ctime.tv_sec &&
			body->ctime_ns > inode->i_ctime.tv_nsec) {
			inode->i_ctime.tv_nsec = body->ctime_ns;
		}
