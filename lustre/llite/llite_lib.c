...
static struct ll_sb_info *ll_init_sbi(void)
{
	...
	/* initialize lru data */
	OBD_ALLOC(sbi->ll_cache, sizeof(*sbi->ll_cache));
	if (sbi->ll_cache == NULL) {
		OBD_FREE(sbi, sizeof(*sbi));
		RETURN(NULL);
	}
	cfs_atomic_set(&sbi->ll_cache->ccc_users, 1);
...
void ll_free_sbi(struct super_block *sb)
{
	...
	if (sbi != NULL) {
		...
		spin_unlock(&ll_sb_lock);
		if (cfs_atomic_dec_and_test(&sbi->ll_cache->ccc_users))
			OBD_FREE(sbi->ll_cache, sizeof(*sbi->ll_cache));
		OBD_FREE(sbi, sizeof(*sbi));
	}
	EXIT;
}
