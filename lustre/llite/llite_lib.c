		  ...
		  struct super_block *sb, struct lookup_intent *it)
{
	...
	/* Handling piggyback layout lock, layout lock can be piggybacked by
	 * getattr and open request.
	 * The lsm can apply to inode only it comes with a layout lock
	 * otherwise correct layout may be overwritten, for example:
	 * 1. proc1: mdt returns a lsm w./o granting layout
	 * 2. layout was changed by othe clients
	 * 3. proc2: refresh layout and layout lock granted
	 * 4. proc1: apply wrong layout */
	if (it != NULL && it->d.lustre.it_lock_mode != 0) {
		...
		if (lock->l_policy_data.l_inodebits.bits&MDS_INODELOCK_LAYOUT) {
			...
			conf.coc_lock = lock;
			conf.u.coc_md = &md;
			/* FIXME: this may cause cascading problem as we're
			 * holding layout lock and configure layout. */
