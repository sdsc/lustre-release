...
static int ll_init_lsm_md(struct inode *inode, struct lustre_md *md)
{
	...
	for (i = 0; i < lsm->lsm_md_stripe_count; i++) {
		...
		if (i == 0) {
			...
			md->lm_slave_fid = fid;
			md->lmv = NULL;
			lsm->lsm_md_oinfo[i].lmo_root = ll_iget(inode->i_sb,
...
static void ll_update_lsm_md(struct inode *inode, struct lustre_md *md)
{
	...
	if (lsm == NULL) {
		if (lli->lli_lmv_md == NULL) {
			...
			CERROR("%s:"DFID" used to have layout,"
			       ...
			       lli->lli_lmv_md->lsm_md_stripe_count,
			       lli->lli_lmv_md->lsm_md_master_mdt_index,
			       lli->lli_lmv_md->lsm_md_hash_type,
			       lli->lli_lmv_md->lsm_md_layout_version,
	...
	if (lli->lli_lmv_md->lsm_md_magic != lsm->lsm_md_magic ||
	    lli->lli_lmv_md->lsm_md_stripe_count != lsm->lsm_md_stripe_count ||
	    lli->lli_lmv_md->lsm_md_master_mdt_index !=
						lsm->lsm_md_master_mdt_index ||
	    lli->lli_lmv_md->lsm_md_hash_type != lsm->lsm_md_hash_type ||
	    lli->lli_lmv_md->lsm_md_layout_version !=
		...
		CERROR("inode %p %lu mismatch\n"
		       ...
		       lsm->lsm_md_master_mdt_index,
		       lli->lli_lmv_md->lsm_md_master_mdt_index,
		       lsm->lsm_md_hash_type, lli->lli_lmv_md->lsm_md_hash_type,
		       lsm->lsm_md_layout_version,
		       lli->lli_lmv_md->lsm_md_layout_version,
	...
	for (idx = 0; idx < lli->lli_lmv_md->lsm_md_stripe_count; idx++) {
		if (!lu_fid_eq(&lli->lli_lmv_md->lsm_md_oinfo[idx].lmo_fid,
			       &lsm->lsm_md_oinfo[idx].lmo_fid)) {
			CERROR("FID in lsm mismatch idx %d, old:"DFID
			       "new:"DFID"\n", idx,
				PFID(&lli->lli_lmv_md->lsm_md_oinfo[idx].lmo_fid),
