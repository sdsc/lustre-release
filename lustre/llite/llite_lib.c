...
int ll_lock_ahead(struct file *file, struct llapi_lock_ahead __user *arg)
{
	...
	/* Lock ahead v1 supports only non-blocking operation;
	 * lock ahead v2 hopes to fix this */
	flags |= CEF_NONBLOCK;

	/* Copy array of extents */
	len = count*sizeof(struct lu_extent);
	...
	for(i=0; i < count; i++) {
		...
		/* -ECANCELED indicates a matching lock was already present,
		 * which we save for userspace above, but is not really an
		 * error for lock ahead */
