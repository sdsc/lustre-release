                                    ...
                                    struct vfsmount *mnt)
{
        ...
        else if (sbi->ll_flags & LL_SBI_PACKAGED_XATTR) {
                op_data->op_valid |= OBD_MD_PACKAGED_XATTR;
...
void ll_clear_inode(struct inode *inode)
{
        ...
        if (sbi->ll_flags & LL_SBI_RMT_CLIENT) {
                ...
                if (!S_ISDIR(inode->i_mode))
                        LASSERT(lli->lli_def_acl == NULL);
                else if (lli->lli_def_acl) {
...
void ll_update_inode(struct inode *inode, struct lustre_md *md)
{
        ...
        if (sbi->ll_flags & LL_SBI_RMT_CLIENT) {
                ...
                if (!S_ISDIR(inode->i_mode)) {
                        LASSERT(lli->lli_def_acl == NULL);
                        if (unlikely(md->def_acl &&
                                     md->def_acl != DUMMY_ACL)) {
                                posix_acl_release(md->def_acl);
                                md->def_acl = NULL;
                        }
                } else if (lli->lli_pxt_valid & PXT_DEFACL) {
                        if (lli->lli_def_acl &&
...
#ifdef CONFIG_FS_POSIX_ACL
        else if (body->valid & OBD_MD_FLACL && lli->lli_pxt_valid & PXT_ACL) {
                cfs_spin_lock(&lli->lli_lock);
                if (lli->lli_posix_acl && lli->lli_posix_acl != DUMMY_ACL)
                        posix_acl_release(lli->lli_posix_acl);
                lli->lli_posix_acl = md->posix_acl;
                cfs_spin_unlock(&lli->lli_lock);
        }
