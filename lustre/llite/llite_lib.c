...
void ll_umount_begin(struct super_block *sb)
{
        ...
        if (ioc_data) {
                obd_iocontrol(NULL, IOC_OSC_SET_ACTIVE, sbi->ll_md_exp,
                              sizeof ioc_data, ioc_data, NULL);

                obd_iocontrol(NULL, IOC_OSC_SET_ACTIVE, sbi->ll_dt_exp,
                              sizeof ioc_data, ioc_data, NULL);
...
int ll_obd_statfs(struct inode *inode, void *arg)
{
        ...
        rc = obd_iocontrol(NULL, IOC_OBD_STATFS, exp, len, buf, NULL);
        if (rc)
                GOTO(out_statfs, rc);
out_statfs:
        if (buf)
