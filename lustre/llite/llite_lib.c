 ...
 * \param[in] o_req	pointer to the original open request
 * \param[in] it	the intention for the original request
 */
void ll_open_cleanup(struct super_block *sb, struct ptlrpc_request *o_req,
		     struct lookup_intent *it)
{
	struct mdt_body			*body;
	struct md_op_data		*op_data;
	struct ptlrpc_request		*c_req;
	...
	OBD_ALLOC_PTR(op_data);
	if (op_data == NULL) {
		CWARN("%s: cannot allocate RAM to release open handle for "
		      DFID"\n",
...
cleanup:
	if (rc != 0 && it != NULL && it->it_op & IT_OPEN)
		ll_open_cleanup(sb != NULL ? sb : (*inode)->i_sb, req, it);
