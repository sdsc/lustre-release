...
static void ll_clear_lsm_md(struct inode *inode)
{
	struct ll_inode_info *lli = ll_i2info(inode);

	LASSERT(S_ISDIR(inode->i_mode));

	if (lli->lli_lmv_md != NULL) {
		lmv_free_memmd(lli->lli_lmv_md);
		lli->lli_lmv_md = NULL;
	}
}
...
static int ll_init_lsm_md(struct inode *inode, struct lustre_md *md)
{
	...
	for (i = 0; i < lsm->lsm_count; i++) {
		...
		LASSERT(lsm->lsm_oinfo[i].lmo_root == NULL);
		if (i == 0) {
			lsm->lsm_oinfo[i].lmo_root = inode;
		} else {
			...
			md->lm_slave_fid = fid;
			md->lmv = NULL;
			lsm->lsm_oinfo[i].lmo_root = ll_iget(inode->i_sb,
					   cl_fid_build_ino(fid, sbi->ll_flags &
							 LL_SBI_32BIT_API), md);
			md->lmv = lsm;
...
static void ll_update_lsm_md(struct inode *inode, struct lustre_md *md)
{
	...
	if (lsm == NULL) {
		if (lli->lli_lmv_md == NULL) {
			RETURN_EXIT;
		} else if (lli->lli_lmv_md->lsm_md_magic == LMV_MAGIC_MIGRATE) {
			...
			CERROR("%s:"DFID" used to have layout,"
			       ...
			       PFID(ll_inode2fid(inode)),
			       lli->lli_lmv_md->lsm_md_magic,
			       lli->lli_lmv_md->lsm_count,
			       lli->lli_lmv_md->lsm_master,
			       lli->lli_lmv_md->lsm_hash_type,
			       lli->lli_lmv_md->lsm_layout_version,
			       lli->lli_lmv_md->lsm_md_pool_name);
			RETURN_EXIT;
		}
	}
	...
	if (lli->lli_lmv_md->lsm_md_magic != lsm->lsm_md_magic ||
	    lli->lli_lmv_md->lsm_count != lsm->lsm_count ||
	    lli->lli_lmv_md->lsm_master != lsm->lsm_master ||
	    lli->lli_lmv_md->lsm_hash_type != lsm->lsm_hash_type ||
	    lli->lli_lmv_md->lsm_layout_version != lsm->lsm_layout_version ||
	    strncmp(lli->lli_lmv_md->lsm_md_pool_name, lsm->lsm_md_pool_name,
		    strlen(lsm->lsm_md_pool_name))) {
		CERROR("inode %p %lu mismatch\n"
		       "    new(%p)     vs     lli_lmv_md(%p):\n"
		       "    magic:      %x                   %x\n"
		       "    count:      %x                   %x\n"
		       "    master:     %x                   %x\n"
		       "    hash_type:  %x                   %x\n"
		       "    layout:     %x                   %x\n"
		       "    pool:       %s                   %s\n",
		       inode, inode->i_ino, lsm, lli->lli_lmv_md,
		       lsm->lsm_md_magic, lli->lli_lmv_md->lsm_md_magic,
		       lsm->lsm_count, lli->lli_lmv_md->lsm_count,
		       lsm->lsm_master, lli->lli_lmv_md->lsm_master,
		       lsm->lsm_hash_type, lli->lli_lmv_md->lsm_hash_type,
	...
	for (idx = 0; idx < lli->lli_lmv_md->lsm_count; idx++) {
		if (memcmp(&lli->lli_lmv_md->lsm_oinfo[idx].lmo_fid,
			   &lsm->lsm_oinfo[idx].lmo_fid,
			   sizeof(lsm->lsm_oinfo[idx].lmo_fid))) {
			CERROR("FID in lsm mismatch idx %d, old:"DFID
			       "new:"DFID"\n", idx,
				PFID(&lli->lli_lmv_md->lsm_oinfo[idx].lmo_fid),
				PFID(&lsm->lsm_oinfo[idx].lmo_fid));
			LBUG();
		}
	}

	md_update_lsm_md(ll_i2mdexp(inode), ll_i2info(inode)->lli_lmv_md,
