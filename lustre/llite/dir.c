...
int ll_dir_read(struct inode *inode, __u64 *__pos, struct md_op_data *op_data,
		struct dir_context *ctx)
{
#else
int ll_dir_read(struct inode *inode, __u64 *_pos, struct md_op_data *op_data,
				...
				if (pos == MDS_DIR_END_OFF) {
					...
					ll_release_page(inode, page,
					    le32_to_cpu(dp->ldp_flags) &
							LDF_COLLIDE);
					next = pos;
					page = ll_get_dir_page(inode, op_data,
							       pos, &chain);
				} else {
					/*
