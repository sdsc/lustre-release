...
static void ll_agl_trigger(struct inode *child, struct ll_statahead_info *sai)
{
        ...
        if (unlikely(index < sai->sai_i_next_stated)) {
                clli->lli_agl_idx = 0;
                iput(child);
                RETURN_EXIT;
        }
        ...
        if (likely(rc == 0)) {
                ...
                if (sai->sai_agl_i_min == 0 ||
                    unlikely(sai->sai_agl_i_min > index)) {
                        sai->sai_agl_i_min = index;
...
static struct ll_statahead_info *ll_sai_alloc(void)
{
        ...
        /* XXX: currently, can support (2 ^ 32 - 1) items per directory. */
...
out:
        ...
        if (sai->sai_agl_valid) {
                ...
                LASSERT(thread_is_running(agl_thread));
