...
/*
 * If the given index is behind of statahead window more than SA_OLD_ENTRY_MAX,
 * then it is old.
 */
static inline int is_old_entry(struct ll_statahead_info *sai, int index)
{
        return (sai->sai_max + index + SA_OLD_ENTRY_MAX < sai->sai_index);
}
...
static inline int has_old_agl(struct ll_statahead_info *sai)
{
        ...
        return (sai->sai_agl_min != 0 &&
                is_old_entry(sai, sai->sai_agl_min + 1));
}
                  ...
                  const char *name, int len)
{
        ...
        entry_size = sizeof(struct ll_sai_entry) + (len & ~3) + 4;
...
ll_sai_entry_fini(struct ll_statahead_info *sai, struct ll_sai_entry *entry)
{
        ...
        cfs_list_for_each_entry_safe(pos, next, &sai->sai_entries_sent,
                                     se_list) {
                if (is_old_entry(sai, pos->se_index))
...
static void do_statahead_interpret(struct ll_statahead_info *sai)
{
        ...
        if (agl_should_run(sai, child) &&
            ll_agl_entry_to_prep(sai, child, entry->se_index))
...
out:
        ...
        rc = ll_sai_entry_to_stated(sai, entry, rc < 0 ? rc : SA_ENTRY_SUCC);
...
static int ll_agl_thread(void *arg)
{
        ...
        while (thread_is_running(thread)) {
                ...
                cfs_spin_lock(&plli->lli_agl_lock);
                while ((!agl_prep_empty(sai) || has_old_agl(sai)) &&
                       thread_is_running(thread)) {
                        if (has_old_agl(sai)) {
                                clli = agl_first_sent_entry(sai);
                                do_agl_cleanup_one(clli, plli, sai, 0);
