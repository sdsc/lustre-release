...
static inline int sa_should_run(struct ll_statahead_info *sai)
{
        if (ll_i2sbi(sai->sai_inode)->ll_sa_max > 0)
                return thread_is_running(&sai->sai_thread);
        return 0;
}
...
static inline int agl_should_run(struct ll_statahead_info *sai,
                                 struct inode *inode)
{
        if (sai->sai_agl_valid && inode != NULL && S_ISREG(inode->i_mode) &&
            ll_i2info(inode)->lli_smd != NULL) {
                struct ll_sb_info *sbi = ll_i2sbi(inode);

                if (sbi->ll_sa_max > 0 && sbi->ll_agl_enabled)
                        return thread_is_running(&sai->sai_agl_thread);
        }
        return 0;
}
 ...
 */
static void ll_agl_entry_to_prep(struct ll_statahead_info *sai,
                            ...
                            int entry_name_len)
{
        ...
        if (!dentry) {
                ...
                if (rc == 1 && agl_should_run(sai, dentry->d_inode)) {
                        ll_agl_entry_to_prep(sai, dentry->d_inode,
                                             se->se_index);
                        cfs_waitq_signal(&sai->sai_agl_thread.t_ctl_waitq);
                }
        }
...
static int ll_agl_thread(void *arg)
{
        ...
        while (1) {
                ...
                cfs_spin_lock(&plli->lli_agl_lock);
                while (thread_is_running(thread) && !agl_prep_empty(sai)) {
        ...
        cfs_spin_lock(&plli->lli_agl_lock);
        /* prevent others add new entries into sai_agl_e_prep */
