...
static int do_statahead_interpret(struct ll_statahead_info *sai)
{
        ...
        /* statahead is fid based, client should know the right MDS */
 ...
 */
static int do_sa_lookup(struct inode *dir, struct dentry *dentry, struct lu_fid *fid)
{
        struct md_enqueue_info   *minfo;
        struct ldlm_enqueue_info *einfo;
        ...
        rc = sa_args_init(dir, dentry, &minfo, &einfo, fid, capas);
        if (rc)
                RETURN(rc);
...
static int do_sa_revalidate(struct inode *dir, struct dentry *dentry)
{
        ...
        rc = sa_args_init(dir, dentry, &minfo, &einfo, NULL, capas);
        if (rc)
                RETURN(rc);
...
static int ll_statahead_one(struct dentry *parent, struct lu_fid *fid,
                            const char *entry_name, int entry_name_len)
{
        ...
        if (!dentry) {
                ...
                if (dentry)
                        rc = do_sa_lookup(dir, dentry, fid);
                else
                        GOTO(out, rc = -ENOMEM);
        } else {
...
keep_de:
                        ...
                        rc = ll_statahead_one(parent, &fid, name, namelen);
                        if (rc < 0) {
                                ll_put_page(page);
                                GOTO(out, rc);
                        }
...
int do_statahead_enter(struct inode *dir, struct dentry **dentryp, int lookup)
{
        ...
        if (sai) {
                ...
                if (sbi->ll_sa_speedup && !sa_received_empty(sai))
                        do_statahead_interpret(sai);
