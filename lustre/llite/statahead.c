                                ...
                                struct inode *inode, int index)
{
        ...
        if (clli->lli_agl_index == 0) {
                clli->lli_agl_index = index;
...
/* Do NOT forget to drop inode refcount when into sai_agl_prep. */
static void ll_agl_trigger(struct inode *inode, struct ll_statahead_info *sai)
{
        ...
        if (lli->lli_glimpse_time != 0 &&
            cfs_time_before(cfs_time_current(),
                            cfs_time_add(lli->lli_glimpse_time,
                                         cfs_time_seconds(1)))) {
                                   ...
                                   struct ll_sa_entry *target)
{
        ...
        rc = md_revalidate_lock(ll_i2mdexp(dir), it, &md.body->fid1, NULL);
        if (rc != 1)
                GOTO(out_md, rc = -EAGAIN);

        rc = ll_prep_inode(&child, req, dir->i_sb, &md);
        if (rc)
                GOTO(out_md, rc);
