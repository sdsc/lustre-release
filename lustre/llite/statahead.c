...
ll_sai_unplug(struct ll_statahead_info *sai, struct ll_sa_entry *entry)
{
	...
	if (hit) {
		...
		if (sa_low_hit(sai) && thread_is_running(thread)) {
			...
			ll_stop_statahead(sai->sai_dentry->d_inode);
			RETURN_EXIT;
		}
	}
		       ...
		       int only_unplug)
{
	...
	sai = ll_sai_alloc((*dentryp)->d_parent);
	if (sai == NULL)
		GOTO(out, rc = -ENOMEM);
	...
	rc = PTR_ERR(kthread_run(ll_statahead_thread, (*dentryp)->d_parent,
...
out:
	/* clear lli_opendir_key to not statahead for this dir again. */
	spin_lock(&lli->lli_sa_lock);
	lli->lli_opendir_key = NULL;
	lli->lli_opendir_pid = 0;
	spin_unlock(&lli->lli_sa_lock);
