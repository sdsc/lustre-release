...
{
        return cfs_list_entry(sai->sai_agl_prep.next,
...
{
        return cfs_list_entry(sai->sai_agl_sent.next,
                              struct ll_inode_info, lli_agl_list);
}
 ...
 */
static inline int is_omitted_entry(struct ll_statahead_info *sai, int index)
{
        return (sai->sai_max + index + SA_OMITTED_ENTRY_MAX < sai->sai_index);
}

static inline int has_omitted_agl(struct ll_statahead_info *sai)
{
        /* AGL maybe fall behind statahead with one entry */
        return (sai->sai_agl_min != 0 &&
                is_omitted_entry(sai, sai->sai_agl_min + 1));
}
 ...
 * lock. It is caller's duty to release the init refcount on the entry, so it is
...
/*
 * Insert inode into the list of sai_agl_prep.
 * Return 1 if it is the first entry in sai_agl_prep.
 */
                                ...
                                struct inode *inode, int index)
{
        ...
        if (likely(clli->lli_agl_idx == 0)) {
                ...
                if (agl_prep_empty(sai))
                        rc = 1;
                cfs_list_add_tail(&clli->lli_agl_list, &sai->sai_agl_prep);
...
/*
 * Held parent lli_agl_lock already.
 */
static void do_agl_cleanup_one(struct ll_inode_info *clli,
                               struct ll_inode_info *plli,
                               struct ll_statahead_info *sai, int wait)
{
        ENTRY;
        ...
        if (agl_sent_empty(sai))
                ...
                sai->sai_agl_min = agl_first_sent_entry(sai)->lli_agl_idx;
        cfs_spin_unlock(&plli->lli_agl_lock);
...
static struct ll_statahead_info *ll_sai_alloc(void)
{
        ...
        CFS_INIT_LIST_HEAD(&sai->sai_agl_prep);
        CFS_INIT_LIST_HEAD(&sai->sai_agl_sent);
...
static void ll_sai_put(struct ll_statahead_info *sai)
{
        ...
        if (cfs_atomic_dec_and_lock(&sai->sai_refcount, &plli->lli_sa_lock)) {
                ...
                /* I am the unique thread to operate the sai_agl_prep now */
...
static void ll_agl_trigger(struct inode *child, struct ll_statahead_info *sai)
{
        ...
        if (is_omitted_entry(sai, index + 1)) {
                clli->lli_agl_idx = 0;
                /* drop refcount when into sai_agl_prep */
                iput(child);
                RETURN_EXIT;
        }
        ...
        if (rc == 0) {
                cfs_spin_lock(&plli->lli_agl_lock);
                if (sai->sai_agl_min == 0 ||
                    unlikely(sai->sai_agl_min > index)) {
                        sai->sai_agl_min = index;
                        pos = &sai->sai_agl_sent;
                } else {
...
int do_glimpse_size(struct inode *inode)
{
        ...
        cfs_spin_lock(&clli->lli_agl_lock);
        if (likely(clli->lli_agl_parent != NULL)) {
...
static void do_statahead_interpret(struct ll_statahead_info *sai)
{
        ...
        if (agl_should_run(sai, child)) {
                rc = ll_agl_entry_to_prep(sai, child, entry->se_index);
                if (rc)
                                  ...
                                  struct md_enqueue_info *minfo, int rc)
{
        ...
        if (unlikely(lli->lli_sai == NULL ||
                ...
                entry = ll_sa_entry_get_byindex(sai,
                        ...
                        struct obd_capa **pcapa)
{
        ...
        minfo->mi_cbdata = (void *)(long)entry->se_index;
                            ...
                            int entry_name_len)
{
        ...
        entry = ll_sa_entry_alloc(sai, sai->sai_index, entry_name, entry_name_len);
        if (IS_ERR(entry))
        ...
        if (!dentry) {
                ...
                if (rc == 1 && agl_should_run(sai, dentry->d_inode)) {
                        rc1 = ll_agl_entry_to_prep(sai, dentry->d_inode,
                                                   entry->se_index);
                        if (rc1)
                             cfs_waitq_signal(&sai->sai_agl_thread.t_ctl_waitq);
                }
...
static int ll_agl_thread(void *arg)
{
        ...
        while (1) {
                ...
                cfs_spin_lock(&plli->lli_agl_lock);
                while ((!agl_prep_empty(sai) || has_omitted_agl(sai)) &&
                       thread_is_running(thread)) {
                        if (has_omitted_agl(sai)) {
...
static int ll_statahead_thread(void *arg)
{
        ...
        if (sbi->ll_agl_enabled) {
                CDEBUG(D_READA, "start agl thread: [pid %d]\n", cfs_curproc_pid());
...
out:
        ...
        if (!sa_received_empty(sai)) {
                ...
                /* To release the possible ibits lock held by received entries. */
                while (!sa_received_empty(sai))
                        do_statahead_interpret(sai);
                       ...
                       int only_unplug)
{
        ...
        if (sai) {
                ...
                if (!ll_sa_entry_stated(entry)) {
                        if (sai->sai_in_readpage && !sa_received_empty(sai))
                                do_statahead_interpret(sai);
