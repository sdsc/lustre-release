                  ...
                  const char *name, int len)
{
	...
	/*
	 * Only directory scanner can remove entry, and it only removes stated
	 * entries, that means ptlrpcd can access un-stated entries without
	 * refcount safely (but it can't access it after moving it to stated
	 * list), so sa_entry doesn't need refcount at all.
	 */
...
out:
	...
	l_wait_event(thread->t_ctl_waitq, sai->sai_sent == sai->sai_replied,
		     &lwi);
                       ...
                       int only_unplug)
{
	...
	if (sai != NULL) {
		...
		if (lli->lli_opendir_key == NULL) {
			/*
			 * statahead is stopped, we should not access statahead
			 * data any longer.
			 */
			spin_unlock(&lli->lli_sa_lock);
			RETURN(-EAGAIN);
		...
		if (unlikely(thread_is_stopped(thread) &&
			...
			/*
			 * statahead thread has quit, and no remaining stated
			 * entries.
			 */
			spin_unlock(&lli->lli_sa_lock);
                        RETURN(-EAGAIN);
