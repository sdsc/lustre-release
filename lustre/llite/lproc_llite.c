                               ...
                               int count, int *eof, void *data)
{
	...
	*eof = 1;
	...
	unused_mb = cfs_atomic_long_read(&cache->ccc_lru_left) >> shift;
                               ...
                               unsigned long count, void *data)
{
	...
	if (diff >= 0) {
		cfs_atomic_long_add(diff, &cache->ccc_lru_left);
		GOTO(out, rc = 0);
	}
	...
	while (diff > 0) {
		...
		do {
			...
			ov = cfs_atomic_long_read(&cache->ccc_lru_left);
			if (ov == 0)
				break;

			nv = ov > diff ? ov - diff : 0;
			rc = cfs_atomic_long_cmpxchg(&cache->ccc_lru_left,
						     ov, nv);
			if (likely(ov == rc)) {
				diff -= ov - nv;
				nrpages += ov - nv;
...
out:
	if (rc >= 0) {
		...
		cfs_atomic_long_add(nrpages, &cache->ccc_lru_left);
	}
	return rc;
}
