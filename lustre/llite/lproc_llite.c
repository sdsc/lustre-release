                               ...
                               int count, int *eof, void *data)
{
	...
	*eof = 1;
	return snprintf(page, count, "llite lru [ users: %d, total %luM, "
			"left: %dP, reclaim count: %u ].\n",
                               ...
                               unsigned long count, void *data)
{
	...
	/* difficult - reduce lru slots, but it has enough free slots. */
	diff = -diff;
	do {
	...
	/* more difficult - have to ask oscs to free lru slots. */
	diff -= rc;
	while (diff > 0) {
