...
const struct file_operations ll_rw_extents_stats_fops;
const struct file_operations ll_rw_extents_stats_pp_fops;
const struct file_operations ll_rw_offset_stats_fops;
...
static int ll_max_readahead_whole_mb_seq_show(struct seq_file *m, void *v)
{
	struct super_block *sb = m->private;
	struct ll_sb_info *sbi = ll_s2sbi(sb);
	long pages_number;
...
static ssize_t
ll_max_readahead_whole_mb_seq_write(struct file *file, const char *buffer,
				    size_t count, loff_t *off)
{
	struct super_block *sb =
		((struct seq_file *)file->private_data)->private;
...
LPROC_SEQ_FOPS(ll_max_readahead_whole_mb);
					  ...
					  size_t count, loff_t *off)
{
	...
	mult = 1 << (20 - PAGE_CACHE_SHIFT);
	buffer = lprocfs_find_named_value(buffer, "max_cached_mb:",
					  (unsigned long *)&count);
	rc = lprocfs_write_frac_helper(buffer, count, &pages_number, mult);
	if (rc)
		RETURN(rc);
...
static struct lprocfs_vars lprocfs_llite_obd_vars[] = {
	...
	{ "max_read_ahead_whole_mb",	&ll_max_readahead_whole_mb_fops, 0 },
