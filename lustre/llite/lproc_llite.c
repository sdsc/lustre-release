					...
					size_t count, loff_t *off)
{
	...
	sbi->ll_xattr_cache_enabled = (unsigned int)val;
			      ...
			      size_t count, loff_t *off)
{
	...
	pages_number = (unsigned long)val;
			  ...
			  void *data, enum stats_track_type type)
{
	...
	if (pid > INT_MAX || pid < INT_MIN)
        ...
        ll_s2sbi(sb)->ll_stats_track_id = (int)pid;
        if (pid == 0)
					  ...
					  size_t count, loff_t *off)
{
	...
	if (val < 0 || val > ULONG_MAX)
        ...
        if (val >= 0 && val <= LL_SA_RPC_MAX)
                sbi->ll_sa_max = (unsigned long)val;
        else
                CERROR("Bad statahead_max value %lu. Valid values are in the "
                       "range [0, %d]\n", (unsigned long)val, LL_SA_RPC_MAX);
