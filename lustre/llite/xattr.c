...
static ssize_t ll_getxattr_lov(struct inode *inode, void *buf, size_t buf_size)
{
	...
	if (S_ISREG(inode->i_mode)) {
		...
		layout.cl_buf.lb_buf = buf;
		layout.cl_buf.lb_len = buf_size;
		rc = cl_object_layout_get(env, obj, &layout);
		if (rc == 0) {
			rc = layout.cl_size;
			if (rc == 0)
				rc = -ENODATA;
			else if (buf_size > 0 && rc > buf_size)
				rc = -ERANGE;
		}
		...
		if (rc > 0) {
			...
			/* This function can be used by getxattr().
			 * Do not return layout gen for getxattr() since
			 * otherwise it would confuse tar --xattr by
			 * recognizing layout gen as stripe offset when
			 * the file is restored. See LU-2809. */
