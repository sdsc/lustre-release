...
int get_hsm_state(struct inode *inode)
{
	...
	/* else fallthrough. hus_states remains 0 */
out:
                ...
                const void *value, size_t size, int flags)
{
	...
	if ((strncmp(name, XATTR_TRUSTED_PREFIX,
		...
		/* LU-6214 Avoid anyone directly setting the RELEASED flag. */
		if (lump != NULL &&
			...
			if (!(state & HS_ARCHIVED)) {
				...
				lump->lmm_pattern ^ LOV_PATTERN_F_RELEASED);
				/* Here the state is: real file is not archived but
 				 * user is requesting to set the RELEASED flag so we
 				 * mask off the released flag from the request */
				lump->lmm_pattern ^= LOV_PATTERN_F_RELEASED;
			}
		}
