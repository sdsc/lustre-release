...
static ssize_t ll_getxattr_lov(struct inode *inode, const char *name, void *buf,
			       size_t buf_size)
{
	ssize_t rc;
	...
	if (S_ISREG(inode->i_mode)) {
		...
		rc = cl_object_xattr_get(env, obj, name, buf, buf_size);
		    ...
		    size_t buf_size)
{
	...
	if (strcmp(name, XATTR_LUSTRE_LOV) == 0 ||
		...
		return ll_getxattr_lov(inode, name, buf, buf_size);
	else
...
ssize_t ll_listxattr(struct dentry *dentry, char *buf, size_t buf_size)
{
	...
	rc2 = ll_getxattr_lov(inode, XATTR_LUSTRE_LOV, NULL, 0);
	if (rc2 == -ENODATA)
		RETURN(rc);
