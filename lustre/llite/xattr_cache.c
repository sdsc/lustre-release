...
struct ll_xattr_entry {
	cfs_list_t	xe_list;    /* protected with lli_xattrs_list_rwsem */
 ...
 * fill in @xdata->xld_buffer or only calculate buffer size
 * in @xdata->xld_tail if xld_buffer is NULL.
 *
 * \retval 0 no error occured
 */
static int ll_xattr_cache_list(cfs_list_t *cache,
			       struct ll_xattr_list_data *xdata)
{
	struct ll_xattr_entry *xattr, *tmp;

	ENTRY;

	cfs_list_for_each_entry_safe(xattr, tmp, cache, xe_list) {
		...
		if (xdata->xld_buffer) {
			...
			if (xdata->xld_size < 0)
				break;
			memcpy(&xdata->xld_buffer[xdata->xld_tail],
			...
			__u64 valid)
{
	...
	if (valid & OBD_MD_FLXATTR) {
		...
		rc = ll_xattr_cache_list(&lli->lli_xattrs, &xdata);
	}
