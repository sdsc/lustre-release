...
static int ll_xattr_cache_fini(struct ll_inode_info *lli)
{
	...
	if (!(lli->lli_flags & LLIF_XATTR_CACHE))
		RETURN(0);
				  ...
				  struct ptlrpc_request **req)
{
	...
	if (rc < 0) {
		CERROR("md_intent_lock failed with %d\n", rc);
		mutex_unlock(&lli->lli_xattrs_enq_lock);
		RETURN(rc);
	}
...
static int ll_xattr_cache_refill(struct inode *inode, struct lookup_intent *oit)
{
	...
	/* Matched but no cache? The lock has been forcedly cancelled by refill. */
	if (req == NULL) {
	...
	for (i = 0; i < body->max_mdsize; i++) {
		...
		if (rc < 0) {
			/* terrible, need to destroy the cache and
			 * cancel the cached lock */
			__u64 *ilh = &oit->d.lustre.it_lock_handle;
			ll_xattr_cache_fini(lli);
			up_write(&lli->lli_xattrs_list_rwsem);

			ldlm_lock_decref_and_cancel((struct lustre_handle *)ilh,
 ...
 * or
 * remove the xattr @name (@valid has OBD_MD_FLXATTRLS set) from @inode.
