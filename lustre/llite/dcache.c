...
static int d_revalidate_statahead(struct dentry *dentry, unsigned int flags)
{
...
int ll_revalidate_nd(struct dentry *dentry, unsigned int flags)
{
	...
	rc = d_revalidate_statahead(dentry, flags);
	RETURN(rc);
}
#else
int ll_revalidate_nd(struct dentry *dentry, struct nameidata *nd)
{
	...
	/*
	 * Cached negative dentry is invalid if open/create is demanded, return
	 * 0 to let lookup to handle this.
	 */
	if ((nd->flags & (LOOKUP_OPEN | LOOKUP_CREATE)) &&
	    dentry->d_inode == NULL)
		RETURN(0);

	rc = d_revalidate_statahead(dentry, nd->flags);
	RETURN(rc);
}
#endif
