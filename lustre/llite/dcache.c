...
/*
 * Mark dentry INVALID, and drop dentry if possible. Don't really prune dentry
 * here, let dcache to reclaim it later.
 */
void ll_drop_dentry(struct dentry *dentry)
{
        spin_lock(&dentry->d_lock);
        /*
         * disconected dentry can not be found without lookup, no need to unhash
         * or mark invalid.
         */
        if (!(dentry->d_flags & (DCACHE_DISCONNECTED | DCACHE_LUSTRE_INVALID)))
        {
...
revalidate_finish:
         ...
         * dentry */
        ll_lock_dcache(de->d_inode);
        if (d_unhashed(de))
                d_rehash_cond(de, 0);
        ll_unlock_dcache(de->d_inode);
