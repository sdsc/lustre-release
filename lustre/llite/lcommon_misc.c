...
int cl_init_ea_size(struct obd_export *md_exp, struct obd_export *dt_exp)
{
	...
	val_size = sizeof(max_cookiesize);
	rc = obd_get_info(NULL, dt_exp, sizeof(KEY_MAX_COOKIESIZE),
			  KEY_MAX_COOKIESIZE, &val_size, &max_cookiesize);
	if (rc != 0)
		RETURN(rc);
	...
	CDEBUG(D_HA,
	       ...
	       def_easize, max_easize, max_cookiesize);

	rc = md_init_ea_size(md_exp, max_easize, def_easize, max_cookiesize, 0);
	RETURN(rc);
}
