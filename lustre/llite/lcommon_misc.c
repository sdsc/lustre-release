 ...
 */
int cl_lock_ahead(struct file *file, struct llapi_lu_ladvise *ladvise)
{
	...
	CDEBUG(D_VFSTRACE, "Lock ahead request: file=%.*s, inode=%p, mode=%s "
	       ...
	       user_lockname[ladvise->lla_value1], (__u64) start, (__u64) end);
...
again:
		...
		if (result > 0) {
			...
			cl_mode = cl_mode_user_to_kernel(ladvise->lla_value1);
			if (cl_mode < 0) {
				cl_io_fini(env, io);
				cl_env_put(env, &refcheck);
				RETURN(cl_mode);
	...
	if (result == -ECANCELED)
		result = LA_MATCHED_DIFFERENT;
	else if (result == -EEXIST)
		result = LA_MATCHED_SAME;
