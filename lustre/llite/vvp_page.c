                               ...
                               const struct cl_page_slice *slice)
{
        ...
        if (trylock_page(vmpage)) {
                if (clear_page_dirty_for_io(vmpage)) {
                        ...
                        /* osc_flush_async_page() has already brought into
                         * ready state. See osc_flush_async_page() for detals.*/
                        result = -EALREADY;
                } else {
                        CL_PAGE_DEBUG(D_ERROR, env, pg, "truncated?\n");
                        LASSERT(0);
                }
                unlock_page(vmpage);
        } else {
                /* I don't know if it's good to wait in ptlrpcd thread context.
                 * However, since we have known that there is no active process
                 * using this page, this means it should be locked for a short
                 * while so this may be okay. */
                wait_on_page_locked(vmpage);

                /* never ever try to lock page again here because it may
                 * cause deadlock. Say:
                 * proc 1: flush page - set PG_writeback and unlock page
                 * proc 2: lock page and wait for PG_writeback bit
                 * this proc - ptlrpcd: can never get the page lock because
                 * nobody can clear PG_writeback for us. */
                result = -EAGAIN;
        }
        RETURN(result);
}
