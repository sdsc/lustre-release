                           ...
                           struct ll_dio_pages *pv)
{
        ...
        for (i = 0; i < page_count; i++) {
                ...
                if (clp->cp_type == CPT_CACHEABLE) {
                        cfs_page_t *vmpage = cl_page_vmpage(clp);
                        cfs_page_t *src_page;
                        cfs_page_t *dst_page;
                        void       *src;
                        void       *dst;
                         ...
                         struct page **pagep, void **fsdata)
{
	 ...
	 * XXX: grab_cache_page_write_begin() may be different with
	 * grab_cache_page_nowait(), be careful. */
	vmpage = grab_cache_page_nowait(mapping, index);
	if (unlikely(vmpage == NULL)) {
		struct cl_object *clob = ll_i2info(mapping->host)->lli_clob;

		lcc = ll_cl_init(file, NULL, 0);
		if (IS_ERR(lcc))
                        ...
                        struct page *vmpage, void *fsdata)
{
	...
	if (copied > 0 && !PageDirty(vmpage)) {
		struct cl_page_list *plist = &cio->u.write.cui_queue;

		lcc->lcc_page = NULL; /* page queued */
