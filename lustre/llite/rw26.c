...
#ifdef HAVE_DIRECTIO_ITER
static ssize_t
ll_direct_IO_317_seg(const struct lu_env *env, struct cl_io *io,
		     int rw, struct inode *inode,
		     struct address_space *mapping,
...
#ifdef HAVE_DIRECTIO_ITER
static ssize_t
ll_direct_IO_317(struct kiocb *iocb, struct iov_iter *iter,
		 loff_t file_offset)
{
	struct lu_env *env;
	struct cl_io *io;
	...
	while (iov_iter_count(iter)) {
		...
		if (likely(result > 0)) {
			...
			result = ll_direct_IO_317_seg(env, io,
						      iov_iter_rw(iter),
...
#ifdef HAVE_DIRECTIO_ITER
	.orig_aops.direct_IO		= ll_direct_IO_317,
#else
