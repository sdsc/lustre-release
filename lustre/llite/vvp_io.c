...
#ifindef HAVE_FILE_OPERATIONS_READ_WRITE_ITER
/**
 * lustre_file_aio_write - write data to a file
 *
 ...
 *
 * Duplicate/change of__generic file_aio_write. We need to take out i_mutex
 * to safely change SUID bits
 *
 * This function does *not* take care of syncing data in case of O_SYNC write.
 * A caller has to handle it. This is mainly due to the fact that we want to
 * avoid syncing under i_mutex.
 */
ssize_t lustre_file_aio_write(struct kiocb *iocb, const struct iovec *iov,
...
#else
		result = lustre_file_aio_write(vio->vui_iocb,
					       vio->vui_iter->iov,
					       vio->vui_iter->nr_segs,
					       &vio->vui_iocb->ki_pos);
#endif
