                                  ...
                                  const struct cl_io_slice *ios)
{
	...
	cio->u.write.cui_to = CFS_PAGE_SIZE;
			      ...
			      struct cl_page_list *plist, int from, int to)
{
	...
	/* Sync write pages one by one for out of quota error.
	 * We can't write them in batch because if the IO runs into
	 * error we won't know which part of write has failed. */
	while (plist->pl_nr > 0) {
		bool need_clip = false;
		int clip_to = CFS_PAGE_SIZE;

		if (from != 0 && bytes == 0) /* first page */
			need_clip = true;
		if (plist->pl_nr == 1 && to != CFS_PAGE_SIZE) {
			need_clip = true;
			clip_to = to;
		}
		...
		if (need_clip) {
			cl_page_clip(env, page, from, clip_to);
			LASSERTF(clip_to > from, "page(%p): %d -> %d\n",
				 page, from, clip_to);
		}
		rc = ll_page_sync_io(env, io, page, CIT_WRITE);
		if (need_clip)
			cl_page_clip(env, page, 0, CFS_PAGE_SIZE);

		if (rc < 0) {
			cl_page_list_add(plist, page); /* add it back */
			break;
		}
...
int vvp_io_write_commit(const struct lu_env *env, struct cl_io *io)
{
	...
	if (npages > 0) {
		...
		bytes = npages << CFS_PAGE_SHIFT;

		/* first page */
		bytes -= cio->u.write.cui_from;
		if (queue->pl_nr == 0) /* last page */
			bytes -= CFS_PAGE_SIZE - cio->u.write.cui_to;
	...
	cl_page_list_discard(env, io, queue);
	cl_page_list_disown(env, io, queue);
                              ...
                              const struct cl_io_slice *ios)
{
	...
	if (result > 0) {
		...
		if (cio->u.write.cui_written > 0) {
			...
			CDEBUG(D_VFSTRACE, "write: nob %ld, result: %ld\n",
				io->ci_nob, result);
		}
	}
	if (result > 0) {
