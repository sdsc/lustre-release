...
static int vvp_io_kernel_fault(struct vvp_fault_io *cfio)
{
        ...
        int mkwrite_flags = FAULT_FLAG_WRITE|FAULT_FLAG_MKWRITE;

        if ((vmf->flags & mkwrite_flags) == mkwrite_flags) {
                LASSERT(vmf->page != NULL);
                cfio->ft_vmpage = vmf->page;
                page_cache_get(cfio->ft_vmpage);
                              ...
                              const struct cl_io_slice *ios)
{
        ...
        result = vvp_io_kernel_fault(&vio->u.fault);
        if (result != 0)
                return result;
