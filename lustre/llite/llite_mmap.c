...
restart:
        ...
        if (result == 0) {
                ...
                /* the page must be unlocked otherwise it will definitly cause
                 * deadlock. Says:
                 * 1. this process holds page lock and grab cl_lock mutex to
                 *    unuse cl_lock;
                 * 2. ldlm_bl_callback holds cl_lock mutex to call
                 *    cl_lock_page_out().
                 * -jay
                 */
                LASSERT(vmpage != NULL && !PageLocked(vmpage));
