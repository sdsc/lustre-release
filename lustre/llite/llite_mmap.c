                            ...
                            bool *retry)
{
        ...
        io = ll_fault_io_init(vma, &env,  &nest, vmf->pgoff, NULL);
        if (IS_ERR(io))
        ...
        if (vio->u.fault.ft_vmpage) {
                LASSERT(vmpage == vio->u.fault.ft_vmpage);
                page_cache_release(vmpage);
        }
        ...
        if (result == 0 || result == -ENODATA) {
                ...
                if (vmpage->mapping == NULL) {
                        ...
                        /* truncate it if the cl_lock is being revoked. */
                        truncate_complete_page(vmpage->mapping, vmpage);
                        LASSERT(vmpage->mapping == NULL);
                        unlock_page(vmpage);
                } else if (!PageDirty(vmpage)) {
                        /* race, the page has been cleaned by ptlrpcd after
                         * it was unlocked, it has to be added into dirty
                         * cache again otherwise this write will be leaked.
                         */
                        unlock_page(vmpage);
...
static int ll_page_mkwrite(struct vm_area_struct *vma, struct vm_fault *vmf)
{
        ...
        do {
                retry = false;
                result = ll_page_mkwrite0(vma, vmf, &retry);
