...
int ll_writepages(struct address_space *mapping, struct writeback_control *wbc)
{
	...
	if (sbi->ll_forced_umount)
		/* if the mountpoint is umounted forcibly, MDCs and OSCs have
		 * already been invalidated so that we should ignore layout.
		 * Otherwise refreshing layout in vvp_io_init() will fail, this
		 * causes failure on discarding dirty pages and then LBUG on
		 * truncate_inode_page() later on. */
		ignore_layout = 1;
