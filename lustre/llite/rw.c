...
unsigned long ras_increase_step(struct inode *inode)
{
	...
	rw_size = cl_object_ra_size(clob);

	if (rw_size == 0 || rw_size > PTLRPC_MAX_BRW_SIZE >> PAGE_SHIFT) {
		CERROR("wrong value returned for RW size (%lu),"
		       " falling back to 1 MiB\n", rw_size);
out_default:
		rw_size = PTLRPC_MAX_BRW_SIZE >> PAGE_SHIFT;
	}
