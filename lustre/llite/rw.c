...
/**
 * Initiates read of a page with given index.
 *
 * \retval -ve: page was not added to \a queue for error;
 * \retval   0: page was already uptodate so it will be skipped
 *              from being added, Or page was added into \a queue
 *              for read.
 */
					 ...
					 unsigned long page_index)
{
	 ...
	 * size, otherwise, there will be too many works which won't do
	 * any real prefetch.
	 */
			...
			loff_t pos, size_t count)
{
	...
	rc |= ll_readahead_detect_stride(ras, pos, count,
					 rc == 1 ? NULL : work);
	spin_unlock(&ras->lrs_lock);
