...
static unsigned long ras_get_increase_step(struct inode *inode)
{
	...
	ra_size = cl_object_ra_size(clob);

	if (ra_size == 0 || ra_size > PTLRPC_MAX_BRW_SIZE >> PAGE_SHIFT) {
		CERROR("wrong value returned for RA size (%lu),"
		       " falling back to 1 MiB\n", ra_size);
out_default:
		ra_size = PTLRPC_MAX_BRW_SIZE >> PAGE_SHIFT;
	}
				...
				struct ll_ra_info *ra)
{
	 ...
	 */
	if (stride_io_mode(ras))
		ras_stride_increase_window(ras, ra, ras_get_increase_step(inode));
	else
