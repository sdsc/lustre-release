...
out:
	if (ret < min) {
		...
		cfs_atomic_add(min - ret, &ra->ra_cur_pages);
		ret = min;
	}
        RETURN(ret);
}
		 ...
		 bool hit)
{
        ...
        unsigned long ra_end, len, min = 0;
        struct inode *inode;
        struct ll_ra_read *bead;
	...
	if (!hit && bead != NULL && bead->lrr_start + bead->lrr_count > start)
		min = bead->lrr_start + bead->lrr_count - start;

        reserved = ll_ra_count_get(ll_i2sbi(inode), ria, len, min);
        if (reserved < len)
