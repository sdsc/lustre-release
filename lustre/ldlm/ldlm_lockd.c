...
static void ldlm_lock_reorder_req(struct ldlm_lock *lock)
{
        ...
        cfs_list_for_each_entry(req, &lock->l_export->exp_hp_rpcs,
                 ...
                 * processing.
		 * We evaluate ptlrpc_request_reorderable() without holding
		 * svcpt->scp_req_lock, and then redo the checks with the lock
		 * held once we need to obtain a reliable result.
		 */
