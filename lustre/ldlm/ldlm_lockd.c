...
static int
ldlm_lock_ast_deadline(struct ldlm_lock *lock, struct ptlrpc_request *req)
{
	int lck_dl = ldlm_bl_timeout(lock);
	int req_dl = ptlrpc_deadline_estimate(req);

	return max(lck_dl, req_dl);
}
...
static void ldlm_update_resend(struct ptlrpc_request *req, void *data)
{
	...
	ldlm_refresh_waiting_lock(lock, ldlm_lock_ast_deadline(lock, req));
}
                             ...
                             void *data, int flag)
{
	...
	if (instant_cancel) {
		...
		int deadline = ldlm_lock_ast_deadline(lock, req);

		LASSERT(lock->l_granted_mode == lock->l_req_mode);

		DEBUG_REQ(D_RPCTRACE, req,
			  "waiting lock deadline %d, bl_timeout %d, "
			  "rq_timeout %lu, net_latency: %d\n",
			  deadline, ldlm_bl_timeout(lock),
			  req->rq_deadline, ptlrpc_at_get_net_latency(req));
...
existing_lock:
	...
	lock->l_flags |= flags & LDLM_FL_INHERIT_MASK;
		...
		dlm_rep->lock_flags |= ldlm_flags_to_wire(LDLM_FL_AST_SENT);
				...
				int sec = cfs_time_current_sec();
				int req_dl = req->rq_deadline - sec;
				int lck_dl = ldlm_bl_timeout(lock);
				int net_la = AT_OFF ? 0 :
					at_get(&req->rq_export->
						     exp_imp_reverse->
				...
				DEBUG_REQ(D_RPCTRACE, req,
					...
					"cur %d, req_dl %d, lck_dl %d "
					"net_latency: %d",
					req->rq_arrival_time.tv_sec,
					req->rq_deadline, sec, req_dl, lck_dl,
					net_la);

				req_dl += net_la + 10;
				lck_dl = max(req_dl, lck_dl);

				ldlm_add_waiting_lock(lock, lck_dl);
			}
                }
        }
