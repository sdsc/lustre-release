...
#ifdef HAVE_SERVER_SUPPORT
/* Timeout for the Blocking cycle (BL AST RPC, data flush, lock cancel RPC).
 * Used for lock callback timeout and AST re-send period. */
                         ...
                         const struct ldlm_callback_suite *cbs)
{
	...
	ldlm_request_cancel(req, dlm_req, LDLM_ENQUEUE_CANCEL_OFF, ATF_SKIP);
			...
			int first, enum at_flags flags)
{
	...
	if (count == 1 && dlm_req->lock_handle[0].cookie == 0)
		...
		if ((flags & ATF_STATS) && ldlm_is_ast_sent(lock)) {
...
int ldlm_handle_cancel(struct ptlrpc_request *req)
{
	...
	if (!ldlm_request_cancel(req, dlm_req, 0, ATF_STATS))
