...
static unsigned int
ldlm_lock_ast_timeout(struct ldlm_lock *lock)
{
...
static void ldlm_update_resend(struct ptlrpc_request *req, void *data)
{
	...
	ldlm_refresh_waiting_lock(lock, ldlm_lock_ast_timeout(lock));
}
                             ...
                             void *data, int flag)
{
	...
	if (instant_cancel) {
		...
		unsigned int timeout = ldlm_lock_ast_timeout(lock);
...
existing_lock:
	...
	/* refresh it again, service AT may have changed request deadline */
	lock->u.srv.sl_enq_deadline = req->rq_deadline;
		...
		dlm_rep->lock_flags |= ldlm_flags_to_wire(LDLM_FL_AST_SENT);
				...
				unsigned int tmo = ldlm_lock_ast_timeout(lock);

				if (list_empty(&lock->l_pending_chain))
					ldlm_add_waiting_lock(lock, tmo);
				else
					ldlm_refresh_waiting_lock(lock, tmo);
			}
                }
        }
