...
static ldlm_mode_t lock_matches(struct ldlm_lock *lock,
				lock_match_data_t *data)
{
...
static unsigned int itree_overlap_cb(struct interval_node *in, void *args)
{
	...
	lock_match_data_t *data = args;
	struct ldlm_lock *lock;
	ldlm_mode_t mode;

	list_for_each_entry(lock, &node->li_group, l_sl_policy) {
		mode = lock_matches(lock, data);
		if ((int)mode < 0) {
				      ...
				      lock_match_data_t *data)
{
	...
	for (; tree_ix < LCK_MODE_NUM; tree_ix++) {
		struct ldlm_interval_tree *tree = &res->lr_itree[tree_ix];
