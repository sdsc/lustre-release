...
/**
 * Removes ldlm lock \param lock from lru. Assumes lru is already locked.
 */
...
/**
 * Removes ldlm lock \param lock from lru. Obtains the lru lock first.
 */
...
/**
 * Adds ldlm lock \param lock to namespace LRU. Assumes LRU is already locked.
 */
...
/**
 * Adds ldlm lock \param lock to namespace LRU. Obtains necessary LRU locks
 * first.
 */
...
/**
 * Moves ldlm lock \param lock that is already in namespace LRU to the tail of
...
/**
 * Destroy a locked lock helper.
 *
 * Used by ldlm_lock_destroy and ldlm_lock_destroy_nolock
 * Must be called under lock and resource lock.
 *
 * Does not actually free the lock data, rather marks the lock as
 ...
 * and removes the lock from lru list.  Actual lock freeing would occur
 * when last lock reference goes away.
 *
...
/**
 * Destroys a ldlm lock \param lock. Performs necessary locking first.
 */
...
/**
 * Destroys a ldlm lock \param lock that is already locked.
 */
...
/**
 * Moves ldlm lock \param lock to another resource.
...
/**
 * Fills in handle for ldlm lock \param lock into supplied \param lockh
 ...
 *
 * if \param flags: atomically get the lock and set the flags.
 ...
 * lock descriptor \param desc structure.
 */
...
/**
 * Add specified r/w reference to ldlm lock with handle \param lockh.
 * r/w reference type is determined by \param mode
...
/**
 * Helper function.
 * Add specified r/w reference to ldlm lock \param lock.
 * r/w reference type is determined by \param mode
...
/**
 * Attempts to add r/w reference to a lock with handle \param lockh, and fails
 * if lock is already LDLM_FL_CBPENDING or destroyed.
 *
...
/**
 * Add specified r/w reference to ldlm lock \param lock.
 * Locks ldlm lock and Calls ldlm_lock_addref_internal to do the work.
...
/**
 * Removes r/w reference for ldlm lock \param lock.
...
/**
 * Decrease r/w refcount for ldlm lock with handle \param lockh
 */
...
/**
 * Decrease r/w refcount for ldlm lock with handle \param lockh and mark it for
 * subsequent cancellation once r/w crefcount drops to zero instead of putting
 * into LRU.
 *
...
/**
 * Send a single a glimpse AST from a work list.
 *
 * Called from ldlm_run_ast_work().  Invokes glimpse callback and
 * releases the lock reference taken in ofd_intent_policy() or
 ...
 *
 * Typicaly is called after some resource locks are cancelled to see
 * if anything could be granted as a result of that cancellation.
 */
...
/**
 * Helper function to call blocking AST for ldlm lock \param lock in a
 * "cancelling" mode.
 */
...
/**
 * Remove skiplist-enabled ldlm lock \param req from granted list
 */
...
/**
 * Attempts to cancel ldlm lock \param lock that has no r/w references.
 */
...
/**
 * Print lock with lock handle \param lockh description into debug log.
 *
