 ...
 *
 * There are two special rules for COS lock if LDLM_FL_COS_INCOMPAT is not set:
 * 1. if COS is enabled, and the dependent transactions are from the same client
 *    (lock client_cookie equals), these two locks are compatible.
 * 2. if COS is disabled, COS lock is compatible with any lock. IOW, on DNE
 *    system, local transactions won't trigger Commit-on-Sharing.
 * Except these two cases, LDLM lock compatiblity matrix is in effect.
 *
