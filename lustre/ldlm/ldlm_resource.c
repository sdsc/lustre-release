				   ...
				   loff_t *off)
{
	...
	if (wm_low) {
		if (ldlm_lock_limit != 0 && watermark > ldlm_lock_limit) {
			CERROR("reclaim_threshold must be smaller than "
			       "lock_limit.\n");
			return -EINVAL;
		}
		...
		*data = watermark;
		...
		if (ldlm_reclaim_threshold != 0 &&
			...
			CERROR("lock_limit must be greater than "
			       "reclaim_threshold.\n");
			return -EINVAL;
		}
