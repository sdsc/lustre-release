...
#ifdef LPROCFS
static ssize_t ldlm_dump_ns_seq_write(struct file *file, const char *buffer,
...
LPROC_SEQ_FOPS_WO(ldlm_dump_ns);
...
static int ldlm_ns_resources_seq_show(struct seq_file *m, void *v)
{
...
LPROC_SEQ_FOPS_RO(ldlm_ns_resources);

static int ldlm_ns_locks_seq_show(struct seq_file *m, void *v)
{
	struct ldlm_namespace *ns = m->private;
        __u64                  locks;
...
LPROC_SEQ_FOPS_RO(ldlm_ns_locks);

static int ldlm_lru_size_seq_show(struct seq_file *m, void *v)
{
	struct ldlm_namespace *ns = m->private;
...
static ssize_t ldlm_lru_size_seq_write(struct file *file, const char *buffer,
				       size_t count, loff_t *off)
{
...
LPROC_SEQ_FOPS(ldlm_lru_size);

static int ldlm_elc_seq_show(struct seq_file *m, void *v)
{
	struct ldlm_namespace *ns = m->private;
...
static ssize_t ldlm_elc_seq_write(struct file *file, const char *buffer,
				  size_t count, loff_t *off)
{
	struct ldlm_namespace *ns =
		((struct seq_file *)file->private_data)->private;
...
int ldlm_namespace_proc_register(struct ldlm_namespace *ns)
{
	...
	LDLM_NS_ADD_VAR("resource_count", ns, &ldlm_ns_resources_fops);
	LDLM_NS_ADD_VAR("lock_count", ns, &ldlm_ns_locks_fops);

        if (ns_is_client(ns)) {
		LDLM_NS_ADD_VAR("lock_unused_count", &ns->ns_nr_unused,
				&ldlm_uint_fops);
		LDLM_NS_ADD_VAR("lru_size", ns, &ldlm_lru_size_fops);
		LDLM_NS_ADD_VAR("lru_max_age", &ns->ns_max_age,
				&ldlm_rw_uint_fops);
		LDLM_NS_ADD_VAR("early_lock_cancel", ns, &ldlm_elc_fops);
        } else {
