 ...
 * flock() system call, also called bsd locks). Limitation is only the process
 * that requested a lock can release it, so if you share a file descriptor
 ...
 *
 * This function has a horrible O(n^2) double list iteration that really
 * uses a lot of CPU time on a highly-contended resource.
 */
static int
