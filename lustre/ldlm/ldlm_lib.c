...
/**
 * update a flags for import during reconnect process
 */
 ...
 *
 * \param exp export to handle recoonect process
				...
				struct ptlrpc_request *req)
{
	...
	if (rc != 0) {
		/* it is safe to still in RECOVERY phase as we are not able to
		 * setup correct security flavor so requests are not able to be
		 * delivered correctly */
		return rc;
	}
...
int target_handle_connect(struct ptlrpc_request *req)
{
	...
	class_incref(target, __FUNCTION__, current);
