...
#ifdef __KERNEL__
static void
target_start_recovery_timer(struct obd_device *obd, struct ptlrpc_request *req,
                            int new_client);
#else
static inline void
target_start_recovery_timer(struct obd_device *obd, struct ptlrpc_request *req,
...
no_export:
        ...
        if (rc == 0 && target->obd_recovering)
                target_start_recovery_timer(target, req, !export);
...
/* Reset the recovery window with a new timeout, if the timeout has already
 * started.
 * The caller must have updated obd->obd_recovery_timeout.
 */
void target_reset_recovery_timer(struct obd_device *obd)
{
        ...
        cfs_spin_unlock(&obd->obd_recovery_task_lock);
        CDEBUG(D_HA, "%s: reset recovery timer to %u seconds\n",
               obd->obd_name, (unsigned)obd->obd_recovery_timeout);
}
EXPORT_SYMBOL(target_reset_recovery_timer);

static void start_recovery_timer(struct obd_device *obd)
{
        cfs_spin_lock(&obd->obd_recovery_task_lock);
        if (cfs_timer_is_armed(&obd->obd_recovery_timer)) {
                cfs_spin_unlock(&obd->obd_recovery_task_lock);
                            ...
                            int new_client)
{
        ...
        if (service_time > obd->obd_recovery_timeout && !new_client)
                obd->obd_recovery_timeout = service_time;

        start_recovery_timer(obd);
}

/* extend recovery window to have extra @duration seconds at least. */
static void extend_recovery_timer(struct obd_device *obd, int duration)
{
        ...
        cfs_spin_lock(&obd->obd_recovery_task_lock);
        if (!obd->obd_recovering || obd->obd_abort_recovery ||
            cfs_timer_is_armed(&obd->obd_recovery_timer)) {
                cfs_spin_unlock(&obd->obd_recovery_task_lock);
                return;
        }
        ...
        if (duration > left)
                cfs_timer_arm(&obd->obd_recovery_timer,
                              cfs_time_shift(duration));
        cfs_spin_unlock(&obd->obd_recovery_task_lock);
        CDEBUG(D_HA, "%s: recovery timer will expire in %u seconds\n",
               obd->obd_name, (unsigned)duration);
}
                               ...
                               svc_handler_t handler)
{
        ...
        if (!exp_finished(req->rq_export)) {
                 ...
                 */
                if (!AT_OFF)
                        to = lustre_msg_get_timeout(req->rq_reqmsg), 1;
