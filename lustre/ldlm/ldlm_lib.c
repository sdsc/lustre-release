...
int client_obd_setup(struct obd_device *obddev, struct lustre_cfg *lcfg)
{
	...
	if (cli->cl_dirty_max >> PAGE_CACHE_SHIFT > get_num_physpages() / 8)
		cli->cl_dirty_max = get_num_physpages() << (PAGE_CACHE_SHIFT - 3);
	...
	} else if (get_num_physpages() >> (20 - PAGE_CACHE_SHIFT) <= 128 /* MB */) {
		cli->cl_max_rpcs_in_flight = 2;
	} else if (get_num_physpages() >> (20 - PAGE_CACHE_SHIFT) <= 256 /* MB */) {
		cli->cl_max_rpcs_in_flight = 3;
	} else if (get_num_physpages() >> (20 - PAGE_CACHE_SHIFT) <= 512 /* MB */) {
