...
static void target_finish_recovery(struct lu_target *lut)
{
	...
	if (lut->lut_tdtd != NULL)
		LCONSOLE_WARN("%s update count is %d execute_count %d\n",
			      obd->obd_name,
			      (int)lut->lut_tdtd->tdtd_update_count,
			      (int)lut->lut_tdtd->tdtd_execute_count);
                                  ...
                                  struct obd_device *obd)
{
	 ...
	 * it will be handled */
	update_replay = is_req_replayed_by_update(req);
	spin_lock(&obd->obd_recovery_task_lock);
	if (transno < obd->obd_next_recovery_transno && !update_replay) {
		/* Processing the queue right now, don't re-add. */
		LASSERT(list_empty(&req->rq_list));
		spin_unlock(&obd->obd_recovery_task_lock);
		RETURN(1);
...
void target_send_reply(struct ptlrpc_request *req, int rc, int fail_id)
{
	...
	if (rs->rs_transno <= exp->exp_last_committed ||
		...
		CDEBUG(D_INFO, "Schedule reply immediately\n");
