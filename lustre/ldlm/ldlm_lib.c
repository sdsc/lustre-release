...
static inline const char *bulk2type(struct ptlrpc_bulk_desc *desc)
{
	return ptlrpc_is_bulk_get_sink(desc->bd_type) ? "GET" : "PUT";
}
                   ...
                   struct l_wait_info *lwi)
{
	...
	if (exp->exp_failed ||
		...
		if (ptlrpc_is_bulk_put_sink(desc->bd_type))
	...
	if (rc == -ETIMEDOUT) {
		...
		rc = -ETIMEDOUT;
	} else if (ptlrpc_is_bulk_get_sink(desc->bd_type)) {
