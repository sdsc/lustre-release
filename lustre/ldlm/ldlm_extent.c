					    ...
					    void *args)
{
	...
	/* If the end of this lock is the highest in the tree, then we can
	 * stop after checking this lock. */
	if (lock->l_policy_data.l_extent.end == arg->root->in_max_high)
		RETURN(INTERVAL_ITER_STOP);
	else
		RETURN(INTERVAL_ITER_CONT);
}
