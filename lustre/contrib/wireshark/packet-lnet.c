...
static int
dissect_csum(tvbuff_t *tvb, proto_tree *tree, int offset)
{
	guint32 csum;
...
static int
dissect_req_cookie(tvbuff_t *tvb, proto_tree *tree, int offset)
{
	guint32 req;
	...
	if (!req)
		...
		proto_tree_add_item(tree, hf_lnet_ksm_zc_req_cookie, tvb,
...
static int
dissect_ack_cookie(tvbuff_t *tvb, proto_tree *tree, int offset)
{
	guint32 ack;
	...
	if (!ack)
		...
		proto_tree_add_item(tree, hf_lnet_ksm_zc_ack_cookie, tvb,
				    offset, 8, little_endian);
...
dissect_ksock_msg_noop(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
	...
	offset = 0;
	proto_tree_add_item(tree, hf_lnet_ksm_type, tvb, offset, 4,
			    little_endian);
	offset += 4;
...
static int
dissect_ksock_msg(tvbuff_t *tvb, proto_tree *tree, int offset)
{
	proto_tree_add_item(tree, hf_lnet_ksm_type, tvb, offset, 4,
			    little_endian);
	offset += 4;
...
static int
dissect_dest_nid(tvbuff_t *tvb, proto_tree *tree, int offset)
{
	proto_tree_add_item(tree, hf_lnet_dest_nid_addr, tvb, offset, 4,
			    little_endian);
	offset += 4;
	proto_tree_add_item(tree, hf_lnet_dest_nid_interface, tvb, offset, 2,
			    little_endian);
	offset += 2;
	proto_tree_add_item(tree, hf_lnet_dest_nid_lnet_type, tvb, offset, 2,
			    little_endian);
	offset += 2;
	return offset;
}
...
static int
dissect_src_nid(tvbuff_t *tvb, proto_tree *tree, int offset)
{
	proto_tree_add_item(tree, hf_lnet_src_nid_addr, tvb, offset, 4,
			    little_endian);
	offset += 4;
	proto_tree_add_item(tree, hf_lnet_src_nid_interface, tvb, offset, 2,
			    little_endian);
	offset += 2;
	proto_tree_add_item(tree, hf_lnet_src_nid_lnet_type, tvb, offset, 2,
			    little_endian);
	offset += 2;
	return offset;
}
...
static int
dissect_lnet_put(tvbuff_t *tvb, proto_tree *tree, int offset,
		 packet_info *pinfo _U_)
{
	...
	proto_tree_add_item(tree, hf_dst_wmd_interface, tvb, offset, 8,
			    little_endian);
	offset += 8;
	...
	offset += 8;
	proto_tree_add_item(tree, hf_hdr_data, tvb, offset, 8, little_endian);
	offset += 8;
	...
	offset += 4;
	proto_tree_add_item(tree, hf_offset, tvb, offset, 4, little_endian);
	offset += 4;
	return offset ;
}

static int
dissect_lnet_get(tvbuff_t *tvb, proto_tree *tree, int offset,
		 packet_info *pinfo _U_)
{
...
dissect_lnet_hello(tvbuff_t *tvb, proto_tree *tree, int offset)
{
	...
	proto_tree_add_item(tree, hf_hello_incarnation, tvb, offset, 8,
			    little_endian);
	offset += 8;
	proto_tree_add_item(tree, hf_hello_type, tvb, offset, 4, little_endian);
	offset += 4;
	return offset;
}

static int
dissect_lnet_ack(tvbuff_t *tvb, proto_tree *tree, int offset,
		 packet_info *pinfo _U_)
{
	...
	proto_tree_add_item(tree, hf_match_bits, tvb, offset, 8, little_endian);
	offset += 8;
	proto_tree_add_item(tree, hf_mlength, tvb, offset, 4, little_endian);
	offset += 4;
	return offset ;
}
...
static guint
get_lnet_message_len(packet_info  __attribute__((__unused__)) *pinfo,
...
static guint
get_noop_message_len(packet_info  __attribute__((__unused__)) *pinfo,
		     tvbuff_t *tvb, int offset)
{
	return 24;
}
...
dissect_lnet(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
	...
	case KSOCK_MSG_LNET:
		tcp_dissect_pdus(tvb, pinfo, tree, TRUE,
				 LNET_HEADER_LEN, get_lnet_message_len,
...
*get_lnet_conv(packet_info *pinfo , GHashTable *lnet_hash_table,
	       guint64 match_bits)
{
	conversation_t     *conversation;
	lnet_request_key_t  request_key;
	lnet_request_key_t *new_request_key;
	...
	request_val = (struct lnet_request_val *) g_hash_table_lookup(
			lnet_hash_table, &request_key);
	if (!request_val) {
		...
		*new_request_key = request_key;
		...
		g_hash_table_insert(lnet_hash_table, new_request_key,
				    request_val);
	}
	return request_val;
}
...
dissect_lnet_message(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
	...
	if (check_col(pinfo->cinfo, COL_INFO)) {
		...
		col_add_fstr(pinfo->cinfo, COL_INFO, "LNET_%s",
			     (msg_type < sizeof(lnet_msg_type_t) /
			     sizeof(value_string)) ?
			     lnet_msg_type_t[msg_type].strptr : "Unknown") ;
	}
	...
	if (tree) {
		...
		proto_tree *lnet_nid_src_tree = NULL;	/* subtree for the
							   nids */
		proto_tree *lnet_nid_dest_tree = NULL;
		proto_item *ti_src_nid;
		...
		ti = proto_tree_add_item(tree, proto_lnet, tvb, 0, -1, FALSE);
			/* principal node */
		/*	ti=proto_tree_add_protocol_format(tree, proto_lnet,
							  tvb, 0, -1, "Lnet");
		*/

		lnet_tree = proto_item_add_subtree(ti, ett_lnet); /* add the
								     subtree */
		...
		/* pid */
		proto_tree_add_item(lnet_tree, hf_lnet_src_pid, tvb, offset, 4,
				    little_endian);
		offset += 4;
		proto_tree_add_item(lnet_tree, hf_lnet_dest_pid, tvb, offset, 4,
				    little_endian);
		offset += 4;
		...
		proto_item_append_text(ti, " %s", (msg_type <
				       sizeof(lnet_msg_type_t) /
				       sizeof(value_string)) ?
				       lnet_msg_type_t[msg_type].strptr :
				       "Unknown");
		proto_tree_add_item(lnet_tree, hf_lnet_msg_type, tvb, offset, 4,
				    little_endian);
		offset += 4;
		...
		switch (msg_type) {
		case LNET_MSG_ACK:
			offset = dissect_lnet_ack(tvb, lnet_tree, offset,
						  pinfo);
			match = tvb_get_letoh64(tvb, 72);
			break;
		case LNET_MSG_PUT:
			offset = dissect_lnet_put(tvb, lnet_tree, offset,
						  pinfo);
			match = tvb_get_letoh64(tvb, 72);
			break;
		case LNET_MSG_GET:
			offset = dissect_lnet_get(tvb, lnet_tree, offset,
						  pinfo);
			match = tvb_get_letoh64(tvb, 72);
			break;
		case LNET_MSG_REPLY:
		...
		conversation_val = get_lnet_conv(pinfo , lnet_request_hash,
						 match);
		/*	proto_tree_add_text(tree, tvb, 0 , 0,
				"match = %" G_GINT64_MODIFIER "u parent = %d",
		...
		if (payload_length > 0) {
			...
			next_tvb = tvb_new_subset(tvb, offset, payload_length,
						  payload_length);
			if (msg_type == LNET_MSG_PUT)
				dissector_try_port(subdissector_table,
						   tvb_get_letohl(tvb,
...
proto_register_lnet(void)
{
	static hf_register_info hf[] = {
		{ &hf_lnet_ksm_type,
			{ "Type of socklnd message", "lnet.ksm_type",
			  FT_UINT32, BASE_HEX, VALS(ksm_type_t), 0x0, "",
			  HFILL} },
		{ &hf_lnet_ksm_csum,
			{ "Checksum", "lnet.ksm_csum", FT_UINT32, BASE_DEC,
			  NULL, 0x0, "", HFILL} },
		{ &hf_lnet_ksm_zc_req_cookie,
			{ "Ack required", "lnet.ksm_zc_req_cookie", FT_UINT64,
			  BASE_HEX, NULL, 0x0, "", HFILL} },
		{ &hf_lnet_ksm_zc_ack_cookie,
			{ "Ack", "lnet.ksm_zc_ack_cookie", FT_UINT64, BASE_HEX,
			  NULL, 0x0, "", HFILL} },
		{ &hf_lnet_ib_magic,
		...
		{ &hf_lnet_src_nid,
			{ "Src nid", "lnet.src_nid", FT_UINT64, BASE_HEX,
			  NULL, 0x0, "src nid", HFILL} },
		{ &hf_lnet_src_nid_addr,
			{ "Src nid", "lnet.src_nid_addr", FT_IPv4,
			  BASE_NONE, NULL, 0x0, "", HFILL} },
		{ &hf_lnet_src_nid_lnet_type,
			{ "lnd network type", "lnet.src_nid_type",
			  FT_UINT16, BASE_DEC, VALS(lndnames), 0x0, "",
			  HFILL} },
		{ &hf_lnet_src_nid_interface,
			{ "lnd network interface", "lnet.src_nid_net_interface",
			  FT_UINT16, BASE_DEC, NULL, 0x0, NULL, HFILL} },

		{ &hf_lnet_dest_nid,
			{ "Dest nid", "lnet.dest_nid", FT_UINT64, BASE_HEX,
			  NULL, 0x0, "", HFILL} },

		{ &hf_lnet_dest_nid_addr,
			{ "Destination nid", "lnet.dest_nid_addr", FT_IPv4,
			  BASE_NONE, NULL, 0x0, "", HFILL} },
		{ &hf_lnet_dest_nid_lnet_type,
			{ "lnd network type", "lnet.dest_nid_type", FT_UINT16,
			  BASE_DEC, VALS(lndnames), 0x0, "", HFILL} },
		{ &hf_lnet_dest_nid_interface,
			{ "lnd network interface",
			  "lnet.dest_nid_net_interface", FT_UINT16 , BASE_DEC,
			  NULL, 0x0, NULL, HFILL} },

		{ &hf_lnet_dest_pid,
			{ "Dest pid", "lnet.dest_pid", FT_UINT32, BASE_DEC_HEX,
			  NULL, 0x0, "dest pid", HFILL} },
		{ &hf_lnet_src_pid,
			{ "Src pid", "lnet.src_pid", FT_UINT32, BASE_DEC_HEX,
			  NULL, 0x0, "src nid", HFILL} },

		{ &hf_lnet_msg_type,
			{ "Message type", "lnet.msg_type", FT_UINT32, BASE_DEC,
			  VALS(lnet_msg_type_t), 0x0,
			  "msg type", HFILL} },
		{ &hf_lnet_payload_length,
			{ "Payload length", "lnet.payload_length", FT_UINT32,
			  BASE_DEC, NULL, 0x0, "", HFILL} },
		{ &hf_lnet_payload,
			{ "Payload", "lnet.payload", FT_NONE, BASE_NONE, NULL,
			  0x0, "", HFILL} },

		{&hf_dst_wmd,
			{ "DST MD index ", "lnet.msg_dst_cookie", FT_BYTES,
			  BASE_NONE, NULL, 0x0, "", HFILL} },
		{ &hf_dst_wmd_interface,
			{ "DST MD index interface",
			  "lnet.msg_dst_inteface_cookie", FT_UINT64,
			  BASE_HEX_DEC, NULL, 0x0, "", HFILL} },
		{ &hf_dst_wmd_object,
			{ "DST MD index object", "lnet.msg_dst_object_cookie",
			  FT_UINT64, BASE_HEX_DEC, NULL, 0x0, "", HFILL} },
		{ &hf_match_bits,
			{ "Match bits", "lnet.msg_dst_match_bits", FT_UINT64,
			  BASE_HEX_DEC, NULL, 0x0, "", HFILL} },
		{ &hf_mlength,
			{ "Message length", "lnet.msg_length", FT_UINT32,
			  BASE_DEC, NULL, 0x0, "", HFILL} },


		/* Put */
		{ &hf_hdr_data,
			{ "hdr data", "lnet.msg_hdr_data", FT_UINT64,
			  BASE_HEX_DEC, NULL, 0x0, "", HFILL} },
		{ &hf_ptl_index,
			{ "ptl index", "lnet.ptl_index", FT_UINT32, BASE_DEC,
			  VALS(portal_indices), 0x0, "", HFILL} },
		{ &hf_offset,
			{ "offset", "lnet.offset", FT_UINT32, BASE_DEC, NULL,
			  0x0, "", HFILL} },

		/* Get*/
		{ &hf_src_offset,
			{ "src offset", "lnet.src_offset", FT_UINT32,
			  BASE_DEC, NULL, 0x0, "", HFILL} },
		{ &hf_sink_length,
			{ "sink length", "lnet.sink_length", FT_UINT32,
			  BASE_DEC, NULL, 0x0, "", HFILL} },

		/* Hello */
		{ &hf_hello_incarnation,
			{ "hello incarnation ", "lnet.hello_incarnation",
			  FT_UINT64, BASE_HEX_DEC, NULL, 0x0, "", HFILL} },
		{ &hf_hello_type,
			{ "hello type", "lnet.hello_type", FT_UINT32, BASE_DEC,
			  NULL, 0x0, "", HFILL} },

		{ &hf_lnet_msg_header,
			{ "ptl header", "lnet.ptl_header", FT_NONE, BASE_NONE,
			  NULL, 0x0, "", HFILL} },

		{ &hf_lnet_msg_filler,
			{ "msg filler (padding)", "lnet.ptl_filler", FT_NONE,
			  BASE_NONE, NULL, 0x0, "", HFILL} },
	...
	lnet_module = prefs_register_protocol(proto_lnet,
					      proto_reg_handoff_lnet);
	...
	subdissector_table = register_dissector_table("lnet.ptl_index",
						      "lnet portal index",
						      FT_UINT32 , BASE_DEC);
