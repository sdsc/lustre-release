...
static int dissect_csum(tvbuff_t * tvb, proto_tree *tree, int offset)
{
	...
	if (!csum)
		...
		if (ib_packet)
			proto_tree_add_item(tree, hf_lnet_ib_csum, tvb, offset, 4, little_endian);
		else
...
static int dissect_lnet_put(tvbuff_t * tvb, proto_tree *tree, int offset, packet_info *pinfo _U_)
{
	...
	if (check_col(pinfo->cinfo, COL_INFO)) 
		col_append_sep_str(pinfo->cinfo, COL_INFO, ", ", val_to_str(tvb_get_letohl(tvb,offset), portal_indices, "Unknown")); /* add some nice value  */
	proto_item_append_text(tree, ", %s" , val_to_str(tvb_get_letohl(tvb,offset), portal_indices, "Unknown")); /* print ptl_index */
...
dissect_lnet_message(tvbuff_t * tvb, packet_info *pinfo, proto_tree *tree)
{
	...
	if (tree) {
		...
		proto_tree_add_item(lnet_tree, hf_lnet_msg_filler, tvb, offset, msg_filler_length, little_endian);
		...
		if (payload_length>0)
		{
			...
			/* display of payload */ 
			proto_tree_add_item(lnet_tree,hf_lnet_payload, tvb, offset, payload_length,
					    little_endian); 
