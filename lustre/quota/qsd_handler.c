				 ...
				 void *arg, int ret)
{
	...
	if (rc == 0 && lvb != 0) {
		if (lvb->l_quota_id.lvb_id_qunit != 0)
			qsd_set_qunit(lqe, lvb->l_quota_id.lvb_id_qunit);
		if (lvb->l_quota_id.lvb_id_flags & LDLM_GL_QUOTA_EDQUOT)
			lqe->lqe_edquot = true;
		else
	...
	if (req_is_preacq(reqbody) && ret == -EDQUOT)
...
again:
	...
	case QSD_ADJ: {
		...
		if (lqe->lqe_qunit != 0 && granted < usage + lqe->lqe_qtune) {
	      ...
	      enum qsd_ops op)
{
	...
	if (req_is_acq(qbody) || req_is_preacq(qbody)) {
		...
		if (rc) {
			memset(&qti->qti_lockh, 0, sizeof(qti->qti_lockh));
			if (req_is_preacq(qbody)) {
				if (req_has_rep(qbody))
		...
		} else if (req_is_acq(qbody) && qbody->qb_count == 0) {
			 ...
			 int *flags)
{
	...
	if (rc == 0) {
		...
		LQUOTA_ERROR(lqe, "Acquire quota failed:%d", rc);
		 ...
		 int *flags)
{
	...
	if ((qsd->qsd_tgt_type == LDD_F_SV_TYPE_OST && !qi->lqi_is_blk) ||
	    (qsd->qsd_tgt_type == LDD_F_SV_TYPE_MDT && qi->lqi_is_blk))
		RETURN(0);
