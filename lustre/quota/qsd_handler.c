 ...
 * flight for the lquota entry \lqe. Otherwise, EBUSY is returned.
 */
...
/* helper function calculating how time a service thread should be waiting for
 * quota space */
static inline cfs_duration_t qsd_wait_timeout(void)
{
	return cfs_time_seconds(min_t(int, at_max / 2, obd_timeout / 2));
}
 ...
 * needed for a given quota ID. If a non-null \qbody is passed, then the
 * \qbody structure (qb_count/flags/usage) is filled with appropriate data
 ...
 *
 * \retval true  - space adjustment is required and \qbody is filled, if not
 *                 NULL
...
/*
 * Try to consume local quota space if enough.
 *
 ...
 *         -EAGAIN : need to acquire space from master
 */
 ...
 *         -EBUSY       : already a quota request in flight
...
/*
 * Acquire \space of quota space in order to complete an operation.
 ...
 *
 * \retval 1    - exit from l_wait_event and real return value in \ret
 *         0    - continue waiting
 */
static int qsd_acquire(const struct lu_env *env, struct lquota_entry *lqe,
		       long long space, int *ret)
{
	int rc;
	ENTRY;

	LQUOTA_DEBUG(lqe, "need to acquire:"LPD64, space);

	if (lqe2qqi(lqe)->qqi_qsd->qsd_stopping) {
		*ret = -EINPROGRESS;
		RETURN(1); /* exit from l_wait_event */
	}
			 ...
			 int *flags)
{
	...
	lwi = LWI_TIMEOUT(qsd_wait_timeout(), NULL, NULL);
