...
static int qsd_pending_updates(struct qsd_qtype_info *qqi)
{
	struct qsd_instance	*qsd = qqi->qqi_qsd;
	...
	cfs_list_for_each_entry(upd, &qsd->qsd_upd_list, qur_link) {
		if (upd->qur_qqi == qqi) {
			cfs_read_unlock(&qsd->qsd_lock);
			CERROR("%s: pending %s updates for type:%d.\n",
			       qsd->qsd_svname,
	...
	cfs_list_for_each_entry(lqe, &qsd->qsd_adjust_list, lqe_link) {
		if (lqe2qqi(lqe) == qqi) {
			cfs_spin_unlock(&qsd->qsd_adjust_lock);
			CERROR("%s: pending adjust for type:%d.\n",
	...
	cfs_hash_for_each_safe(qqi->qqi_site->lqs_hash, qsd_entry_iter_cb,
			       &dqacq);
	if (dqacq) {
		CERROR("%s: pending dqacq for type:%d.\n", qsd->qsd_svname,
		       qqi->qqi_qtype);
		return 1;
	}
