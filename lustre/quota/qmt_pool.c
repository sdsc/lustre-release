...
static void qpi_fini(const struct lu_env *, struct qmt_pool_info *);
		    ...
		    struct dt_object *root)
{
	...
	cfs_list_for_each(l, &set->qs_list) {
		qpi = cfs_list_entry(l, struct qmt_pool_info, qpi_linkage);
		rc = qpi_prepare(env, qpi, root);
		if (rc)
			break;
	}
	up_write(&set->qs_sem);
...
void qpi_set_fini(const struct lu_env *env, struct quota_set *set)
{
	...
	cfs_list_for_each_safe(l, tmp, &set->qs_list) {
		...
		cfs_hash_del(set->qs_hash, &qpi->qpi_key,
			     &qpi->qpi_hash);
		/* remove from list */
		cfs_list_del_init(&qpi->qpi_linkage);
...
int qpi_set_add(struct quota_set *set, struct qmt_pool_info *qpi)
{
	...
	down_write(&set->qs_sem);
	cfs_list_add(&qpi->qpi_linkage, &set->qs_list);
	up_write(&set->qs_sem);
...
/*
 * Allocate a new qmt_pool_info structure
 *
 ...
 */
static struct qmt_pool_info *qpi_init(const struct lu_env *env,
				      struct qmt_device *qmt,
				      const char *pool_name, int pool_id,
				      int pool_type)
{
 ...
 */
static void qpi_fini(const struct lu_env *env, struct qmt_pool_info *pool)
{
	int	qtype;
	ENTRY;
					     ...
					     int pool_id, int pool_type)
{
	...
	pool = qpi_set_lookup(&qmt->qmt_pool_set, key);
	if (pool == NULL) {
...
int qmt_pool_init(const struct lu_env *env, struct qmt_device *qmt)
{
	...
	for (res = LQUOTA_FIRST_RES; res < LQUOTA_LAST_RES; res++) {
		struct qmt_pool_info *qpi;
		qpi = qpi_init(env, qmt, NULL, 0, res);
		if (IS_ERR(qpi)) {
			CERROR("failed to init qpi %d (%d)\n", res, rc);
			rc = PTR_ERR(qpi);
			break;
		}
		rc = qpi_set_add(&qmt->qmt_pool_set, qpi);
		if (rc) {
			CERROR("failed to add qpi %d to qmt hash\n", res);
			qpi_putref(env, qpi);
			break;
		}
	}
		     ...
		     struct dt_object *qmt_root)
{
	...
	rc = qpi_set_prepare(env, &qmt->qmt_pool_set, qmt_root);
...
int qmt_pool_new(struct obd_device *obd, char *poolname, int pool_id)
{
	...
	int			 res, rc = 0;
	struct lu_env		*env;
	struct dt_object	*qmt_root = NULL;
	ENTRY;
	...
	if (qmt->qmt_pool_set.qs_started) {
		qmt_root = lquota_disk_dir_find_create(env, qmt->qmt_child,
						       NULL, QMT_DIR);
		if (IS_ERR(qmt_root)) {
			rc = PTR_ERR(qmt_root);
			CERROR("failed to create master quota directory"
			       " (%d)\n", rc);
			lu_env_fini(env);
			OBD_FREE_PTR(env);
			RETURN(rc);
	...
	for (res = LQUOTA_FIRST_RES; res < LQUOTA_LAST_RES; res++) {
		struct qmt_pool_info *qpi;
		qpi = qpi_init(env, qmt, poolname, pool_id, res);
		if (IS_ERR(qpi)) {
			CERROR("failed to init qpi %d (%d)\n", res, rc);
			rc = PTR_ERR(qpi);
			break;
		...
		if (qmt->qmt_pool_set.qs_started) {
			/*
			 * XXX Maybe it is better to remove qpi_set_prepare and
			 * prepare/start all QPIs individually here?
			 */
		...
		rc = qpi_set_add(&qmt->qmt_pool_set, qpi);
		if (rc) {
			CERROR("failed to add pool to qmt hash (%d)\n", rc);
			qpi_putref(env, qpi);
			break;
		}
	}
...
int qmt_pool_del(struct obd_device *obd, char *poolname, int pool_id)
{
	...
	for (res = LQUOTA_FIRST_RES; res < LQUOTA_LAST_RES; res++) {
		pool = qpi_set_del(&qmt->qmt_pool_set, pool_id + (res << 16));
		if (pool == NULL) {
			rc = -ENOENT;
			break;
		}
