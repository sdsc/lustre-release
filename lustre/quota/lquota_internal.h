 ...
 * of 0x%x-%s,glb_fid,slv_uuid, which mean 2(0x) + 8(fid) + 1(-) + 40(UUID_MAX)
 * which means 51 chars + '\0' */
#define LQUOTA_NAME_MAX 52

/* reserved OID in FID_SEQ_QUOTA for local objects */
enum quota_local_oid {
	QUOTA_USR_OID = 1UL, /* slave index copy for user quota */
	QUOTA_GRP_OID = 2UL, /* slave index copy for group quota */
	/* all OID after this are allocated dynamically by the QMT */
	QUOTA_GENERATED_OID = 100UL,
};
