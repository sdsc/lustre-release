...
static inline __u32 qtype2local_oid(int qtype)
{
	switch (qtype) {
	case USRQUOTA:
				  ...
				  struct dt_device *dev, int type)
{
	...
	lu_local_obj_fid(&qti->qti_fid, qtype2local_oid(type));
...
static inline int qtype2lqtype(int qtype)
{
	switch (qtype) {
	case USRQUOTA:
