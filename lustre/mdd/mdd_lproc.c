...
static int lprocfs_rd_scrub_async_commit(char *page, char **start, off_t off,
                                         int count, int *eof, void *data)
{
        struct mdd_device *mdd = data;
        char *str;
        ...
        LASSERT(mdd != NULL);
        if (mdd->mdd_scrub_header_mem.sh_param.sp_flags & SPF_ASYNC)
                str = "yes";
        else
                str = "no";
...
static int lprocfs_wr_scrub_async_commit(struct file *file, const char *buffer,
                                         unsigned long count, void *data)
{
        struct mdd_device *mdd = data;
        struct scrub_param *sp_mem = &mdd->mdd_scrub_header_mem.sh_param;
        ...
        if (strcmp(dummy, "yes") == 0) {
                val = 1;
        } else if (strcmp(dummy, "no") == 0) {
                val = 0;
        } else {
                CWARN("Invalid value for scrub async commit.\n"
                      "The valid value should be: yes|no\n");
                return -EINVAL;
        }

        cfs_down(&mdd->mdd_scrub_header_sem);
        if (sp_mem->sp_flags & SPF_ASYNC)
                old = 1;
        else
                old = 0;
...
static struct lprocfs_vars lprocfs_mdd_obd_vars[] = {
        ...
        { "scrub_async_commit", lprocfs_rd_scrub_async_commit,
                                lprocfs_wr_scrub_async_commit, 0 },
