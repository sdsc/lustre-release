...
static int mdd_declare_lfsck_namespace_reset(const struct lu_env *env,
					      struct lfsck_component *com,
					      struct thandle *handle)
{
	int rc;

	rc = dt_declare_punch(env, com->lc_obj, 0, OBD_OBJECT_EOF, handle);
	if (rc != 0)
		return rc;

	rc = dt_declare_xattr_set(env, com->lc_obj,
				  mdd_buf_get(env, com->lc_file_disk,
					      com->lc_file_size),
				  XATTR_NAME_LFSCK_NAMESPACE, 0, handle);
	return rc;
}
				     ...
				     struct lfsck_component *com, bool init)
{
	...
	if (IS_ERR(handle)) {
		rc = PTR_ERR(handle);
		CERROR("%.16s: fail to create trans for reset "
		       "lfsck_namespace: %d\n,", mdd_lfsck2name(lfsck), rc);
		GOTO(out, rc);
	}

	rc = mdd_declare_lfsck_namespace_reset(env, com, handle);
	if (rc != 0) {
		CERROR("%.16s: fail to declare trans for reset "
		       "lfsck_namespace: %d\n,", mdd_lfsck2name(lfsck), rc);
		GOTO(stop, rc);
	}
	...
	rc = dt_trans_start_local(env, mdd->mdd_bottom, handle);
	if (rc != 0) {
		CERROR("%.16s: fail to start trans for reset "
		       "lfsck_namespace: %d\n,", mdd_lfsck2name(lfsck), rc);
		GOTO(stop, rc);
	}

	rc = dt_punch(env, com->lc_obj, 0, OBD_OBJECT_EOF, handle, BYPASS_CAPA);
	if (rc != 0) {
		CERROR("%.16s: fail to punch for reset "
		       "lfsck_namespace: %d\n,", mdd_lfsck2name(lfsck), rc);
		GOTO(stop, rc);
	}
	...
	rc = dt_xattr_set(env, com->lc_obj,
			  mdd_buf_get(env, com->lc_file_disk,
				      com->lc_file_size),
			  XATTR_NAME_LFSCK_NAMESPACE, LU_XATTR_REPLACE,
			  handle, BYPASS_CAPA);
	if (rc != 0)
		CERROR("%.16s: fail to store lfsck_namespace, len = %d, "
		       "rc = %d\n",
		       mdd_lfsck2name(lfsck), com->lc_file_size, rc);
