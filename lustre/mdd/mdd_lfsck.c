				      ...
				      __u8 *flags, bool force)
{
	...
	fid_cpu_to_be(key, fid);
	rc = dt_insert(env, obj,
		       (const struct dt_rec *)flags, (const struct dt_key *)key,
		       handle, BYPASS_CAPA, 1);
	if (unlikely(rc == -EEXIST) && force)
		rc = dt_update(env, obj, (const struct dt_rec *)flags,
			       (const struct dt_key *)key, handle,
			       BYPASS_CAPA, 1);
				 ...
				 const struct lu_name *cname)
{
	...
	rc = dt_insert(env, mdd_object_child(parent),
		       (const struct dt_rec*)cfid, (const struct dt_key *)cname,
		       handle, BYPASS_CAPA, 1);
	if (rc != 0)
		GOTO(unlock, rc);
