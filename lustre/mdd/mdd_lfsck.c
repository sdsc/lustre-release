					      ...
					      struct thandle *handle)
{
	...
	rc = mdo_declare_index_delete(env, p, name, handle);
	if (rc != 0)
		return rc;
				      ...
				      struct mdd_device *mdd)
{
	...
	obj = llo_store_resolve(env, &mdd->mdd_md_dev, mdd->mdd_child, "", fid);
	if (IS_ERR(obj))
		GOTO(out, rc = PTR_ERR(obj));

	parent = md2mdd_obj(obj);
	...
	handle = mdd_trans_create(env, mdd);
	if (IS_ERR(handle))
		GOTO(out_child, rc = PTR_ERR(handle));
	...
	rc = __mdd_index_delete_only(env, parent, lfsck_namespace_name, handle,
				     BYPASS_CAPA);
	if (rc != 0)
		GOTO(stop, rc);

	rc = mdo_ref_del(env, child, handle);
	if (rc != 0) {
		__mdd_index_insert_only(env, parent, mdo2fid(child),
					lfsck_namespace_name, handle,
					BYPASS_CAPA);
		GOTO(stop, rc);
	}

	rc = mdd_la_get(env, child, &ma->ma_attr, BYPASS_CAPA);
	if (rc != 0)
		GOTO(stop, rc);

	rc = mdd_finish_unlink(env, child, ma, handle);
				      ...
				      __u8 *flags, bool force)
{
	...
	if (rc == 0) {
		...
		*flags | LLF_RECORD_ONDISK;
		if ((*flags & ~LLF_RECORD_ONDISK) == (tf & ~LLF_RECORD_ONDISK))
			RETURN(0);
...
static void
mdd_lfsck_linkea_unpin_all(const struct lu_env *env, struct mdd_object *obj)
{
	struct lfsck_linkea_header *llh  = &obj->mod_llh;
	struct lfsck_linkea_entry  *lle;
				     ...
				     struct lfsck_component *com, bool init)
{
	...
	lu_object_put(env, &com->lc_obj->do_lu);
	com->lc_obj = NULL;
	rc = mdd_lfsck_namespace_unlink(env, mdd);
	if (rc != 0)
		GOTO(out, rc);
...
record:
	LASSERT(count > 0);
	LASSERTF(count >= llh->llh_count, "count = %d, llh->llh_count = %d\n",
		 count, llh->llh_count);
	...
	if (count == 1) {
		...
		rc = mdd_lfsck_namespace_update(env, com, cfid, &llh->llh_flags,
						true);
		if (rc != 0)
			GOTO(unlock, rc);
	...
	if (llh->llh_flags & LLF_VERIFIED) {
		...
		if (count == la->la_nlink) {
			mdd_lfsck_linkea_unpin_all(env, child);
	...
	if (!(llh->llh_flags & LLF_RECORD_ONDISK)) {
		if (count != la->la_nlink)
			llh->llh_flags = LLF_UNMATCH_NLINKS;
					   ...
					   struct lfsck_component *com)
{
	...
	do {
		...
		rc = mdd_lfsck_namespace_double_scan_one(env, com, target,
							 flags);
		mdd_lfsck_linkea_unpin_all(env, target);
		...
		*fid = ent->lde_fid;
		...
		if ((rc < 0 && rc != -ENODATA) ||
			...
			if (!lu_fid_eq(fid, &ent->lde_fid)) {
				CDEBUG(D_LFSCK, "Re-created %s, [old: "DFID
				       "], [new: "DFID"]\n", ent->lde_name,
				       PFID(&ent->lde_fid), PFID(fid));
				goto unlock;
			}
		}
...
fini_oit:
	...
	lu_site_purge_pinned(&env, site, ~0);
