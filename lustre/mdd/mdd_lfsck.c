...
mdd_lfsck_component_find(struct md_lfsck *lfsck, __u16 type, cfs_list_t *list)
{
	struct lfsck_component *com;
					...
					struct lfsck_component *com)
{
	...
	if (!cfs_list_empty(&com->lc_link))
		cfs_list_del(&com->lc_link);
	if (!cfs_list_empty(&com->lc_link_dir))
		cfs_list_del(&com->lc_link_dir);
			  ...
			  const char *prefix)
{
	...
	if (fid_is_zero(&pos->lp_dir_parent)) {
		if (pos->lp_oit_cookie == 0)
			rc = snprintf(*buf, *len, "N/A, N/A, N/A\n");
		else
			rc = snprintf(*buf, *len, LPX64", N/A, N/A\n",
				      pos->lp_oit_cookie);
	} else {
		rc = snprintf(*buf, *len, LPX64", "DFID", "LPX64"\n",
			      pos->lp_oit_cookie, PFID(&pos->lp_dir_parent),
			      pos->lp_dir_cookie);
	}
	if (rc <= 0)
		return -ENOSPC;
				   ...
				   struct md_lfsck *lfsck)
{
	...
	rc = dt_read(env, lfsck->ml_bookmark_obj,
		     mdd_buf_get(env, &lfsck->ml_bookmark_disk, len), &pos);
	if (rc == len) {
		...
		CERROR("%.16s: fail to load lfsck_bookmark, expected = %d, "
		       "rc = %d\n", mdd_lfsck2name(lfsck), len, rc);
		if (rc > 0)
			rc = -EFAULT;
	} else {
		/* return -ENOENT for empty lfsck_bookmark. */
		rc = -ENODATA;
	}
	return rc;
}
				    ...
				    struct md_lfsck *lfsck)
{
	...
	rc = obj->do_body_ops->dbo_write(env, obj,
			mdd_buf_get(env, &lfsck->ml_bookmark_disk, len),
			&pos, handle, BYPASS_CAPA, 1);
	if (rc != len) {
...
{
	des->ln_magic = le64_to_cpu(src->ln_magic);
	...
	des->ln_ents_added = le64_to_cpu(src->ln_ents_added);
...
static void inline mdd_lfsck_namespace_to_le(struct lfsck_namespace *des,
					     struct lfsck_namespace *src)
{
	des->ln_magic = cpu_to_le64(src->ln_magic);
	...
	des->ln_ents_added = cpu_to_le64(src->ln_ents_added);
				    ...
				    struct lfsck_component *com)
{
	...
	if (rc == len) {
		...
		if (ns->ln_magic != LFSCK_BOOKMARK_MAGIC) {
				     ...
				     struct lfsck_component *com, bool init)
{
	...
	rc = dt_xattr_set(env, obj,
			  ...
			  handle, BYPASS_CAPA);
	if (rc != len) {
		CERROR("%.16s: fail to store lfsck_namespace, expected = %d, "
		       "rc = %d\n", mdd_lfsck2name(lfsck), len, rc);
		if (rc >= 0)
			rc = -EFAULT;
	} else {
				      ...
				      __u8 *flags)
{
	...
	rc = obj->do_index_ops->dio_lookup(env, obj, (struct dt_rec *)flags,
...
static int mdd_lfsck_namespace_insert(const struct lu_env *env,
				      struct lfsck_component *com,
				      const struct lu_fid *fid,
				      __u8 *flags, bool force)
{
	...
	fid_cpu_to_be(key, fid);
	rc = obj->do_index_ops->dio_insert(env, obj,
					   (const struct dt_rec *)flags,
					   (const struct dt_key *)key,
					   handle, BYPASS_CAPA, 1);
	if (unlikely(rc == -EEXIST) && force)
		rc = obj->do_index_ops->dio_update(env, obj,
					   (const struct dt_rec *)flags,
					   (const struct dt_key *)key,
					   handle, BYPASS_CAPA, 1);
				      ...
				      const struct lu_fid *fid)
{
	...
	fid_cpu_to_be(key, fid);
	rc = obj->do_index_ops->dio_delete(env, obj, (const struct dt_key *)key,
					   handle, BYPASS_CAPA);
				      ...
				      __u8 *flags)
{
	...
	rc = dt_declare_update(env, obj, (const struct dt_rec *)flags,
			       (const struct dt_key *)fid, handle);
	if (rc != 0)
		goto out;
...
static struct lfsck_linkea_entry *
mdd_lfsck_linkea_find(struct mdd_object *obj, const struct lu_fid *pfid,
		      const struct lu_name *cname)
{
	struct lfsck_linkea_entry *lle;

	cfs_list_for_each_entry(lle, &obj->mod_llh.llh_list, lle_link) {
		if (!lu_fid_eq(pfid, &lle->lle_fid))
			continue;

		if (cname->ln_namelen != lle->lle_namelen)
			continue;

		if (!strncmp(cname->ln_name, lle->lle_name, lle->lle_namelen))
			return lle;
	}
	return NULL;
}

static int mdd_lfsck_linkea_pin(struct mdd_object *obj,
				const struct lu_fid *pfid,
				const struct lu_name *cname)
{
	struct lu_object_header    *loh  = mdd2lu_obj(obj)->lo_header;
	struct lfsck_linkea_header *llh  = &obj->mod_llh;
	struct lfsck_linkea_entry  *lle;
	int			    size = sizeof(*lle) + cname->ln_namelen;

	OBD_ALLOC(lle, size);
	if (lle == NULL) {
		CWARN("Not enough memory when mdd_lfsck_linkea_add() for: "
		      "parent "DFID", child %.*s, size %d\n",
		      PFID(pfid), cname->ln_namelen, cname->ln_name, size);
		return -ENOMEM;
	}

	CFS_INIT_LIST_HEAD(&lle->lle_link);
	lle->lle_fid = *pfid;
	lle->lle_namelen = cname->ln_namelen;
	memcpy(lle->lle_name, cname->ln_name, cname->ln_namelen);

	cfs_list_add_tail(&lle->lle_link, &llh->llh_list);
	llh->llh_count++;
	if (llh->llh_flags & LLF_VERIFIED) {
		lu_object_set_used(loh);
		lu_object_clear_pinned(loh);
	} else {
		lu_object_set_pinned(loh);
	}
	return 0;
}

static void mdd_lfsck_linkea_unpin(struct mdd_object *obj,
				   struct lfsck_linkea_entry *lle)
{
	struct lfsck_linkea_header *llh  = &obj->mod_llh;
	int			    size = sizeof(*lle) + lle->lle_namelen;
...
static void mdd_lfsck_linkea_unpin_all(struct mdd_object *obj)
{
	struct lu_object_header    *loh  = mdd2lu_obj(obj)->lo_header;
	struct lfsck_linkea_header *llh  = &obj->mod_llh;
	struct lfsck_linkea_entry  *lle;
	...
	cfs_list_for_each_entry_safe(lle, next, &llh->llh_list, lle_link) {
		cfs_list_del(&lle->lle_link);
		size = sizeof(*lle) + lle->lle_namelen;
		OBD_FREE(lle, size);
	}
	llh->llh_count = 0;
	lu_object_clear_pinned(loh);
	lu_object_clear_used(loh);
}

static int mdd_lfsck_linkea_destroy(const struct lu_env *env,
				    struct mdd_object *obj)
{
	...
	rc = mdo_xattr_del(env, obj, XATTR_NAME_LINK, handle, BYPASS_CAPA);
...
static int mdd_lfsck_add_links(const struct lu_env *env, struct mdd_object *obj,
...
static int mdd_lfsck_store_links(const struct lu_env *env,
				 struct mdd_object *obj,
				 struct mdd_link_data *ldata)
{
	struct mdd_device *mdd    = mdd_obj2mdd_dev(obj);
...
static int mdd_lfsck_update_nlink(const struct lu_env *env,
				  struct mdd_object *obj, int nlink)
{
	...
	rc = mdd_declare_attr_set(env, mdd, obj, la, handle);
	if (rc != 0)
		GOTO(out, rc);
	...
	rc = mdd_attr_set_changelog(env, &obj->mod_obj, handle, LA_NLINK);
...
/**
 * \retval +ve	inserted
 * \retval 0	no need to insert
 * \retval -ve	error cases
 */
static int mdd_lfsck_insert_name(const struct lu_env *env,
				 struct mdd_object *parent,
				 struct mdd_object *child,
				 const struct lu_name *cname)
{
	struct dt_object    *next   = mdd_object_child(parent);
	struct mdd_device   *mdd    = mdd_obj2mdd_dev(parent);
	const struct lu_fid *cfid   = mdo2fid(child);
	struct thandle	    *handle;
	int		     rc;
	ENTRY;
	...
	rc = mdo_declare_index_insert(env, parent, cfid, cname->ln_name,
				      handle);
	if (rc != 0)
		GOTO(out, rc);

	rc = mdd_declare_changelog_store(env, mdd, cname, handle);
	if (rc != 0)
		GOTO(out, rc);
	...
	mdd_write_lock(env, parent, MOR_TGT_CHILD);
	if (unlikely(mdd_is_dead_obj(parent)))
		GOTO(unlock, rc = 0);

	rc = next->do_index_ops->dio_insert(env, next,
					    (const struct dt_rec*)cfid,
					    (const struct dt_key *)cname,
					    handle, BYPASS_CAPA, 1);
	if (rc != 0)
		GOTO(unlock, rc);

	rc = mdd_changelog_ns_store(env, mdd, CL_HARDLINK, 0, child, parent,
				    cname, handle);

	GOTO(unlock, rc = (rc == 0 ? 1 : rc));

unlock:
	mdd_write_unlock(env, parent);
out:
	mdd_trans_stop(env, mdd, rc, handle);
	return rc;
}
					       ...
					       __u8 flags)
{
	...
	if (rc != 0) {
		if ((bk->lb_param & LPF_DRYRUN) && (flags & LLF_VERIFIED) &&
	...
	while (count-- > 0) {
		...
		lle = mdd_lfsck_linkea_find(child, pfid, cname);
		if (lle != NULL) {
		...
		if (mdd_object_exists(parent) <= 0)
			goto shrink;

		mdd_read_lock(env, parent, MOR_TGT_CHILD);
		if (unlikely(mdd_is_dead_obj(parent))) {
			mdd_read_unlock(env, parent);
			goto shrink;
		}

		if (!S_ISDIR(mdd_object_type(parent))) {
			mdd_read_unlock(env, parent);
			goto shrink;
		}
		...
		if (rc == 0) {
			...
			/* For the case of there is linkea entry, but without
			 * name entry pointing to the object, and the object
			 * links count is not less than the count of the name
			 * entries, then we should add the name entry back
			 * to namespace. */
			rc = mdd_lfsck_insert_name(env, parent, child, cname);
			if (rc == 1) {
				ldata.ml_lee = (struct link_ea_entry *)
				       ((char *)ldata.ml_lee + ldata.ml_reclen);
				ns->ln_ents_added++;
		...
		if (lu_fid_eq(cfid, lu_object_fid(mdd2lu_obj(child)))) {
			ldata.ml_lee = (struct link_ea_entry *)
				       ((char *)ldata.ml_lee + ldata.ml_reclen);
			continue;
		}
...
shrink:
	...
	if (update) {
		rc = mdd_lfsck_store_links(env, child, &ldata);
		if (rc != 0)
			RETURN(rc);
	...
	rc = mdd_lfsck_update_nlink(env, child, ldata.ml_leh->leh_reccount);
	if (rc == 0) {
		ns->ln_objs_nlink_repaired++;
		rc = 1;
	}
				     ...
				     struct lfsck_component *com, bool init)
{
	...
	lu_object_put(env, &com->lc_obj->do_lu);
	com->lc_obj = NULL;
	rc = llo_store_unlink(env, &mdd->mdd_md_dev, mdd->mdd_child, "",
			      lfsck_namespace_name);
	if (rc != 0)
		GOTO(out, rc);

	lu_local_obj_fid(fid, LFSCK_NAMESPACE_OID);
	mdo = llo_store_create_index(env, &mdd->mdd_md_dev, mdd->mdd_child, "",
	...
	dto = dt_store_open(env, mdd->mdd_child, "", lfsck_namespace_name, fid);
	if (IS_ERR(dto))
					...
					struct lu_dirent *ent)
{
	...
	struct lfsck_namespace	   *ns	     =
				(struct lfsck_namespace *)com->lc_file_ram;
	struct lfsck_linkea_header *llh	     = &child->mod_llh;
	struct lfsck_linkea_entry  *lle;
	struct mdd_link_data	    ldata    = { 0 };
	...
	rc = mdd_links_read(env, child, &ldata);
	if (rc == 0) {
		rc = mdd_links_find(env, child, &ldata, cname, pfid);
		if (rc == 0) {
			count = ldata.ml_leh->leh_reccount;
...
unmatch:
			...
			if (S_ISDIR(mdd_object_type(child))) {
				rc = mdd_lfsck_linkea_destroy(env, child);
				if (rc != 0)
					GOTO(unlock, rc);
...
add:
		rc = mdd_lfsck_add_links(env, child, &ldata, pfid, cname);
		if (rc != 0)
			GOTO(unlock, rc);
...
record:
	...
	if (count == 1) {
		...
		llh->llh_flags = LLF_VERIFIED | LLF_UNMATCH_NLINKS;
		rc = mdd_lfsck_namespace_insert(env, com, cfid,
						&llh->llh_flags, true);
		if (rc != 0)
			GOTO(unlock, rc);
	...
	if (count == llh->llh_count + 1 &&
	    !(llh->llh_flags & LLF_VERIFIED)) {
			changed = true;
		llh->llh_flags |= LLF_VERIFIED;
	}

	if (llh->llh_flags & LLF_VERIFIED) {
		if (count > la->la_nlink) {
			rc = mdd_lfsck_update_nlink(env, child, count);
			if (rc != 0)
				GOTO(unlock, rc);
		...
		if (count == la->la_nlink) {
			mdd_lfsck_linkea_unpin_all(child);
			mdd_lfsck_namespace_delete(env, com, cfid);
			llh->llh_flags &= ~LLF_UNMATCH_NLINKS;
			GOTO(unlock, rc = 0);
		}

		if (!(llh->llh_flags & LLF_UNMATCH_NLINKS)) {
			llh->llh_flags |= LLF_UNMATCH_NLINKS;
			changed = true;
		}
	...
	/* Add child into the lfsck_namespace when the first entry verified. */
	if (llh->llh_count == 0) {
		if (la->la_nlink > 1)
			ns->ln_mlinked_checked++;
		...
		if (rc != 0) {
			...
			/* The pinned child may be unpinned before the LFSCK
			 * finished because of memory pressure.
			 *
			 * That means n_nlink_checked may be not accurate. */
			rc = mdd_lfsck_namespace_lookup(env, com, cfid,
							&llh->llh_flags);
			if (rc != 0)
				GOTO(unlock, rc);
		}
...
pin:
	rc = mdd_lfsck_linkea_pin(child, pfid, cname);
			 ...
			 char *buf, int len)
{
	...
	if (ns->ln_status == LS_SCANNING_PHASE1) {
		...
		rc = snprintf(buf, len,
			      ...
			      "updated_phase1 (or to be): "LPU64"\n"
			      "updated_phase2 (or to be): "LPU64"\n"
			      "failed_phase1: "LPU64"\n"
			      "failed_phase2: "LPU64"\n"
			      "dirs: "LPU64"\n"
			      "M-linked: "LPU64"\n"
					   ...
					   struct lfsck_component *com)
{
	...
	if (rc == 0) {
		rc = iops->next(env, di);
		if (rc > 0)
			/* The end of the lfsck_namespace file. */
			GOTO(fini, rc);
	} else if (rc > 0) {
		rc = 0;
	}
	...
	do {
		...
		down_write(&target->mod_lfsck_rwsem);
		if (unlikely(mdd_is_dead_obj(target)))
			goto unlock;
		...
		rc = mdd_lfsck_namespace_double_scan_one(env, com, target,
							 flags);
		mdd_lfsck_linkea_unpin_all(target);

unlock:
		up_write(&target->mod_lfsck_rwsem);
...
checkpoint:
		...
		if ((rc == 0) || ((rc > 0) && !(bk->lb_param & LPF_DRYRUN))) {
			...
			mdd_lfsck_namespace_update(env, com, &fid, &flags);
		}
...
fini:
	...
	if (rc > 0) {
		...
		if (!(bk->lb_param & LPF_DRYRUN))
			ns->ln_flags &= ~(LF_INCONSISTENT | LF_UPGRADED);
				     ...
				     struct md_lfsck *lfsck)
{
	...
	obj = dt_store_open(env, mdd->mdd_child, "", lfsck_namespace_name,
...
static void mdd_lfsck_unpack_ent(struct lu_dirent *ent, __u64 *clue)
{
	...
	if (ent->lde_attrs & LUDA_VERIFY) {
		...
		clue = (__u64 *)(ent->lde_name + len);
		*clue = le64_to_cpu(*clue);
	}
}
...
unlock:
	mdd_write_unlock(env, obj);
	if (rc < 0)
		mdd_lfsck_fail(env, lfsck, true, false);
	return rc;
}
				...
				struct md_lfsck *lfsck)
{
	...
	do {
		...
		lfsck->ml_new_scanned++;
		rc = iops->rec(env, di, (struct dt_rec *)ent, 0);
		if (rc != 0) {
		...
		if (rc < 0 || eclue != le64_to_cpu(iclue)) {
			const struct lu_name *cname;
...
static int mdd_lfsck_main(void *args)
{
	...
	struct lu_site		*site	  =
					md2lu_dev(&mdd->mdd_md_dev)->ld_site;
	struct dt_it		*oit_di;
	int			 rc;
	ENTRY;
	...
	site->ls_allow_pinned = 1;
	mdd->mdd_lfsck_namespace = 1;
...
fini_oit:
	...
	mdd->mdd_lfsck_namespace = 0;
	site->ls_allow_pinned = 0;
	lu_site_purge_pinned(&env, site);
			  ...
			  const struct lu_name *cname)
{
	...
	lle = mdd_lfsck_linkea_find(obj, pfid, cname);
	if (lle != NULL)
		return;

	mdd_lfsck_linkea_pin(obj, pfid, cname);
}
			  ...
			  const struct lu_name *cname)
{
	...
	mdd_lfsck_linkea_unpin(obj, lle);
}
		   ...
		   __u16 type, char *buf, int len)
{
	...
	spin_lock(&lfsck->ml_lock);
	com = mdd_lfsck_component_find(lfsck, type, &lfsck->ml_list_scan);
	if (com == NULL)
		    ...
		    struct lfsck_start *start)
{
	...
	if (start->ls_active != 0) {
		...
		while (start->ls_active != 0) {
			if (i & start->ls_active) {
				com = mdd_lfsck_component_find(lfsck, i,
							&lfsck->ml_list_idle);
				if (com != NULL) {
...
int mdd_lfsck_setup(const struct lu_env *env, struct mdd_device *mdd)
{
	...
	/* More initialization for other LFSCK components in the future. */

	if (!cfs_list_empty(&lfsck->ml_list_scan))
		rc = mdd_lfsck_start(env, lfsck, NULL);
