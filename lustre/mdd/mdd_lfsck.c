...
static int mdd_lfsck_main(void *args)
{
        ...
        struct ptlrpc_thread   *thread = &lfsck->ml_thread;
        struct dt_object       *obj    = lfsck->ml_obj;
        ...
        if (rc != 0) {
                CERROR("LFSCK: fail to init env: rc = %d\n", rc);
                GOTO(noenv, rc);
        }
        ...
        if (IS_ERR(di)) {
                rc = PTR_ERR(di);
                CERROR("LFSCK: fail to init iteration: rc = %d\n", rc);
                GOTO(fini_env, rc);
        }

        CDEBUG(D_OTHER, "LFSCK: flags = 0x%x, pid = %d\n",
        ...
        CDEBUG(D_OTHER, "LFSCK: iteration start: pos = %s\n",
...
out:
        CDEBUG(D_OTHER, "LFSCK: iteration stop: pos = %s, rc = %d\n",
                    ...
                    struct lfsck_start *start)
{
        ...
        if (rc < 0)
                CERROR("Cannot start LFSCK thread: rc %d\n", rc);
        else
                l_wait_event(thread->t_ctl_waitq,
                             thread_is_running(thread) ||
...
const char mdd_lfsck_name[] = "lfsck";
...
int mdd_lfsck_setup(const struct lu_env *env, struct mdd_device *mdd)
{
        ...
        obj = dt_store_open(env, mdd->mdd_child, "", mdd_lfsck_name,
...
void mdd_lfsck_cleanup(const struct lu_env *env, struct mdd_device *mdd)
{
        ...
        if (lfsck->ml_obj != NULL) {
                mdd_lfsck_stop(env, lfsck);
                lu_object_put(env, &lfsck->ml_obj->do_lu);
                lfsck->ml_obj = NULL;
        }
}
