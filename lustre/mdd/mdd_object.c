...
mdd_object_find(const struct lu_env *env, struct mdd_device *d,
                const struct lu_fid *f, const struct lu_object_conf *c)
{
        struct mdd_object *m;
        struct lu_object *o;
        int once = 0, rc;
...
again:
        o = lu_object_find_slice(env, mdd2lu_dev(d), f, c);
        if (IS_ERR(o)) {
                rc = PTR_ERR(o);
                if (rc == -EREMCHG && !d->mdd_noscrub && ++once == 1) {
                        ...
                        const char *path, struct lu_fid *fid)
{
        ...
        struct lu_object_conf *c = &info->mti_conf;
        ...
        *f = mdd->mdd_root_fid;
        ...
        while(1) {
                ...
                *name = '\0';
                ...
                obj = mdd_object_find(env, mdd, f, c);
                if (obj == NULL)
                        GOTO(out, rc = -EREMOTE);
                if (IS_ERR(obj))
                ...
                c->loc_flags = 0;
                rc = mdd_lookup(env, &obj->mod_obj, lname, f, c, NULL);
