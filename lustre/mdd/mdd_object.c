...
again:
	...
	if ((mdd_obj->mod_flags & ORPHAN_OBJ) != 0) {
		...
		if (rc != 0) {
			CERROR("%s (OSD %s): object "DFID" can not be deleted "
			       "from orphan list, maybe cause OST objects can "
			       "not be destroyed (err: %d).\n",
			       lu_dev_name(mdd2lu_dev(mdd)),
			       lu_dev_name(&mdd->mdd_bottom->dd_lu_dev),
			       PFID(mdd_object_fid(mdd_obj)), rc);
	...
	if (rc != 0) {
		CERROR("%s (OSD %s): Error when prepare to delete object "DFID
		       ", which will cause OST objects can not be destroyed. "
		       "(err = %d). \n",
		       lu_dev_name(mdd2lu_dev(mdd)),
		       lu_dev_name(&mdd->mdd_bottom->dd_lu_dev),
		       PFID(mdd_object_fid(mdd_obj)), rc);
