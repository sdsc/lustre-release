			    ...
			    struct md_object *obj2, __u64 flags)
{
	...
	if (snd_buf) {
		...
		snd_fl  = LU_XATTR_REPLACE;
	} else {
		snd_lmm = NULL;
		snd_gen = 0;
		snd_fl  = LU_XATTR_CREATE;
	}
	...
	/* prepare transaction */
	if (snd_buf)
		rc = mdd_declare_xattr_set(env, mdd, fst_o, snd_buf,
					   XATTR_NAME_LOV, fst_fl, handle);
	else
		rc = mdd_declare_xattr_del(env, mdd, fst_o, XATTR_NAME_LOV,
					   handle);
	if (rc)
		GOTO(stop, rc);
	...
	if (snd_buf)
		rc = mdo_xattr_set(env, fst_o, snd_buf, XATTR_NAME_LOV,
				   fst_fl, handle, mdd_object_capa(env, fst_o));
	else
		rc = mdo_xattr_del(env, fst_o, XATTR_NAME_LOV, handle,
	...
	if (fst_buf)
		rc = mdo_xattr_set(env, snd_o, fst_buf, XATTR_NAME_LOV,
				   snd_fl, handle, mdd_object_capa(env, snd_o));
	else
