			    ...
			    struct md_object *obj2, __u64 flags)
{
	...
	int			 rc;
	__u16			 fst_gen, snd_gen, old_fst_gen;
	ENTRY;
	...
	/* save the orignal generation number in le32 format
	...
	if (fst_lmm)
		fst_lmm->lmm_layout_gen = snd_gen;

	if (snd_lmm)
		snd_lmm->lmm_layout_gen = fst_gen;
	...
	if (rc) {
		...
		if (fst_lmm)
			fst_lmm->lmm_layout_gen = old_fst_gen;
