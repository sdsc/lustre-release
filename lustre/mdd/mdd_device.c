				    ...
				    struct mdd_device *mdd, __u32 id)
{
	struct mdd_changelog_user_purge mcup = {
		...
		.mcup_found = false,
		.mcup_usercount = 0,
		.mcup_minrec = CHANGELOG_UNREGISTER,
	};
	struct llog_ctxt *ctxt;
	int rc;
	...
	if ((rc == 0) && mcup.mcup_found) {
		CDEBUG(D_IOCTL, "%s: Purging changelog entries for user %d record="LPU64"\n",
				  ...
				  void *data)
{
	...
	if ((rec->cur_endrec > 0) && (rec->cur_endrec < mcuc->mcuc_minrec))
	...
	if (rec->cur_endrec > mcuc->mcuc_endrec) {
		CWARN("Request %llu out of range: %llu\n",
	...
	CDEBUG(D_IOCTL, "Rewriting changelog user %u endrec to "LPU64"\n",
