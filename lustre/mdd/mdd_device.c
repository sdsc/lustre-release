                         ...
                         unsigned int cmd, int len, void *karg)
{
	...
	case OBD_IOC_START_LFSCK: {
		...
		rc = mdd_lfsck_start(env, &mdd->mdd_lfsck, start);
		RETURN(rc);
	}
...
static struct lu_local_obj_desc llod_lfsck_key = {
	.llod_name      = mdd_lfsck_name,
	.llod_oid       = MDD_LFSCK_OID,
	.llod_is_index  = 0,
};
...
static int __init mdd_mod_init(void)
{
	...
	llo_local_obj_register(&llod_lfsck_key);
...
static void __exit mdd_mod_exit(void)
{
	...
	llo_local_obj_unregister(&llod_lfsck_key);
