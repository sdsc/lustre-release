 ...
 */
static int mdd_get_info(const struct lu_env *env, struct md_device *md,
			__u32 keylen, void *key, __u32 *vallen, void *val)
{
	...
	if (KEY_IS(KEY_ROOT)) {
		*((struct lu_fid *) val) = mdd->mdd_root_fid;
		*vallen = sizeof(mdd->mdd_root_fid);
	} else if (KEY_IS(KEY_CHANGELOG_INDEX)) {
		spin_lock(&mdd->mdd_cl.mc_lock);
		*((__u64 *)val) = mdd->mdd_cl.mc_index;
		spin_unlock(&mdd->mdd_cl.mc_lock);
	} else {
		CERROR("%s: Not supported key %s: rc = %d\n",
		       mdd2obd_dev(mdd)->obd_name, (char *)key, -EOPNOTSUPP);
		rc = -EOPNOTSUPP;
	}
                         ...
                         struct lu_context_key *key, void *data)
{
	...
	lu_large_buf_free(&info->mti_link_buf);
