...
#define CHANGELOG_UNREGISTER -1ULL
				    ...
				    struct mdd_device *mdd, __u32 id)
{
	...
	CDEBUG(D_IOCTL, "%s: Purge request: id=%d\n",
	       mdd_obj_dev_name(mdd->mdd_dot_lustre), id);
	...
	if ((rc == 0) && (mcup.mcup_found)) {
		CDEBUG(D_IOCTL, "%s: Purging changelog entries for user %d\n",
		       mdd_obj_dev_name(mdd->mdd_dot_lustre), id);
		rc = mdd_changelog_llog_cancel(env, mdd, CHANGELOG_UNREGISTER);
	} else {
		CWARN("%s: No changelog for user %u; rc=%d\n",
		      mdd_obj_dev_name(mdd->mdd_dot_lustre), id, rc);
		rc = -ENOENT;
	}
				  ...
				  void *data)
{
	...
	mcuc->mcuc_minrec = mcuc->mcuc_endrec;
	mcuc->mcuc_flush = 1;
			       ...
			       __u64 endrec)
{
	...
	CDEBUG(D_IOCTL, "%s: Purge request: id=%d, endrec=%lld\n",
	       mdd_obj_dev_name(mdd->mdd_dot_lustre), id, endrec);
	...
	if (start_rec < endrec) {
		CWARN("%s: Could not clear changelog, requested address "\
		      "out of range\n", mdd_obj_dev_name(mdd->mdd_dot_lustre));
		RETURN(-EINVAL);
	}
	...
	if (rc < 0) {
		CWARN("%s: Failure to clear the changelog for user %d: %d\n",
		      mdd_obj_dev_name(mdd->mdd_dot_lustre), id, rc);
	} else if (mcuc.mcuc_flush == 1) {
		CDEBUG(D_IOCTL, "%s: Purging changelog entries up to "LPU64
		       "\n", mdd_obj_dev_name(mdd->mdd_dot_lustre),
		       mcuc.mcuc_minrec);
		rc = mdd_changelog_llog_cancel(env, mdd, mcuc.mcuc_minrec);
	} else if (mcuc.mcuc_flush == 0) {
