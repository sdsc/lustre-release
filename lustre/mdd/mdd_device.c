				   ...
				   struct mdd_device *mdd)
{
	...
	if (rc == 0) {
		 ...
		 * IGIF, LOCAL FID or Normal FID, if this is the first
		 * mount from the previous version */
		if (!lu_fid_eq(&fid, &pfid) && !fid_is_igif(&pfid) &&
		    !fid_is_local_file(&pfid) && !fid_is_norm(&pfid)) {
			CERROR("%s: Got an invalid root FID "DFID"\n",
