                      ...
                      struct md_op_spec *spec)
{
	...
	if (!fid_is_norm(f) && fid_seq(f) != FID_SEQ_SPECIAL) {
                       ...
                       struct lu_device *cdev)
{
	...
	fid.f_oid = FID_OID_SPECIAL_ROOT;
	fid.f_ver = 0;
	...
	mdd->mdd_root_fid = *lu_object_fid(&lroot->do_lu);
	...
	mdd->mdd_orphans = dto;
	if (!dt_try_as_dir(env, mdd->mdd_orphans)) {
...
out_root:
	lu_object_put_nocache(env, &lroot->do_lu);
out_put:
