                       ...
                       struct lu_device *cdev)
{
	...
	if (rc != 0) {
		CERROR("%s: cannot initialize '%s': rc = %d\n",
		       mdd2obd_dev(mdd)->obd_name, ".lustre/fid", rc);
		GOTO(out, rc);
	}
...
static struct lu_local_obj_desc llod_mdd_root = {
	...
	.llod_fid	= LU_LOCAL_OBJ_FID(MDD_ROOT_INDEX_OID),
	.llod_is_index	= 1,
	.llod_feat	= &dt_directory_features,
};
...
static struct lu_local_obj_desc llod_dot_lustre = {
	.llod_dir	= mdd_root_dir_name,
...
static struct lu_local_obj_desc llod_obf = {
	.llod_dir	= "ROOT/.lustre", /* Sorry. */
...
static int __init mdd_mod_init(void)
{
	...
	LASSERT(lu_fid_eq(&LU_OBF_FID, &llod_obf.llod_fid));
	...
	llo_local_obj_register(&llod_dot_lustre);
	llo_local_obj_register(&llod_obf);
...
static void __exit mdd_mod_exit(void)
{
	...
	llo_local_obj_unregister(&llod_obf);
	llo_local_obj_unregister(&llod_dot_lustre);
