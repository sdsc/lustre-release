...
int mdd_object_kill(const struct lu_env *env, struct mdd_object *obj,
                    struct md_attr *ma, struct thandle *);
...
struct thandle* mdd_trans_create(const struct lu_env *env, struct mdd_device *mdd);
int mdd_trans_start(const struct lu_env *, struct mdd_device *, struct thandle *);
...
static inline int mdo_declare_attr_set(const struct lu_env *env, struct mdd_object *obj,
                             ...
                             struct thandle *handle)
{
        ...
        if (mdd_object_exists(obj)) {
                ...
                if (dt_try_as_dir(env, next))
                        rc = dt_declare_insert(env, next, (struct dt_rec*) fid,
                             ...
                             const char *name, struct thandle *handle)
{
        ...
        return dt_declare_delete(env, next, (const struct dt_key *) name, handle);
}
