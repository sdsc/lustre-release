...
static inline void mdd_scrub_print(__u32 level, struct dt_key *key,
                                   int keysize, int rc, char *str)
{
        switch (keysize) {
        case 16:
                CDEBUG(level, "MDD scrub failed to %s at: "LPU64"/"LPU64"\n",
                       str, *(__u64 *)key, *((__u64 *)key + 1));
                break;
        case 8:
                          ...
                          struct scrub_header *header_disk)
{
        ...
        mdd_scrub_write_txn_param_build(env, mdd, sizeof(*header_disk));
...
static int mdd_scrub_main(void *arg)
{
        ...
        const struct dt_it_ops   *iops        = &dio->dio_it;
        ...
        start_time = cfs_time_current_sec();
        ...
        if (key_is_zero(&msi->msi_position))
                ...
                header_mem->sh_time_initial_start = start_time;
        header_mem->sh_time_latest_start = start_time;
        ...
        while (1) {
                ...
                if (rc != 0) {
                        if (rc < 0) {
                                ...
                                key = iops->key(&env, it);
                                mdd_scrub_convert_key(&msi->msi_position, key,
                                                      key_size);
                                /* It is unnecessary to process this object, skip. */
                        }
                        goto check;
                }
                ...
                if (param->sp_flags & SPF_DRYRUN) {
                        ...
                        rc = dio->dio_lookup(&env, dto,
                                        (struct dt_rec * )&lid,
                                        (const struct dt_key *)&dsr.dsr_fid,
                                        NULL, BYPASS_CAPA);
                        if (rc == 0) {
                                if (memcmp(&lid, &dsr.dsr_fid, key_size)) {
                                        dinfo->sid_items_unmatched++;
                                        if (unlikely(key_is_zero(
...
update:
                mdd_scrub_txn_param_build(&env, mdd,
                                          MDD_TXN_SCRUB_UPDATE_REC_OP);
                if (param->sp_flags & SPF_ASYNC)
                        tp->tp_flags |= TPF_SCRUB_ASYNC;
                handle = mdd_trans_start(&env, mdd);
                if (IS_ERR(handle)) {
                        rc = PTR_ERR(handle);
                        estr = "start trans";
...
check:
                if (rc < 0) {
                        ...
                        (*failed)++;
                        if (dsr.dsr_valid & DSV_LOCAL_ID)
                                key = (struct dt_key *)&dsr.dsr_lid;
                        else
                                key = iops->key(&env, it);
                        mdd_scrub_convert_key(&msi->msi_position, key, key_size);
                        if (param->sp_flags & SPF_FAILOUT)
                                mdd_scrub_print(D_ERROR, key, key_size, rc, estr);
                        else
                                mdd_scrub_print(D_SCRUB, key, key_size, rc, estr);
                }
        ...
        *checkpoint_position = msi->msi_position;
        if (rc > 0) {
                if (param->sp_flags & SPF_DRYRUN) {
                        ...
                        header_mem->sh_time_last_complete = *checkpoint_time;
...
static int mdd_scrub_store_fid(const struct lu_env *env, struct mdd_device *mdd)
{
        ...
        mdd_scrub_txn_param_build(env, mdd, MDD_TXN_SCRUB_STORE_FID_OP);
        handle = mdd_trans_start(env, mdd);
        if (IS_ERR(handle)) {
                rc = PTR_ERR(handle);
        ...
        rc = dio->dio_delete(env, root, (struct dt_key *)mdd_scrub_name,
                             handle, BYPASS_CAPA);
        if (rc != 0) {
        ...
        rc = dio->dio_insert(env, root, (const struct dt_rec *)fid,
                             (const struct dt_key *)mdd_scrub_name,
                             handle, BYPASS_CAPA, 1);
        if (rc != 0) {
                   ...
                   int restored)
{
        ...
        if (trigger_scrub != 0 && m->mdd_noscrub == 0) {
                ...
                if (rc != 0)
                        /* Allow MDT to mount up even OI Scrub auto trigger
                         * failure. */
                    ...
                    struct start_scrub_param *param, int reset)
{
        ...
        if (param == NULL) {
                ...
                if (header->sh_current_status == SS_RESTORED || reset != 0)
                        sp->sp_flags &= ~SPF_DRYRUN;

                mdd_init_scrub_param(sp);
                if (reset == 0)
                        /* 'reset == 0' means it is triggered by auto detect
                         * mechanism when MDT mounts up, but not by parallel
                         * PRC. Under such case, try to update OI file ASAP. */
                        sp->sp_flags |= SPF_ASYNC;
        } else {
                ...
                if (param->ssp_valid & SSP_ASYNC) {
                        if (param->ssp_param.sp_flags & SPF_ASYNC)
                                sp->sp_flags |= SPF_ASYNC;
                        else
                                sp->sp_flags &= ~SPF_ASYNC;
                }
                ...
                if (param->ssp_valid & SSP_LINKEA) {
                        if (param->ssp_param.sp_flags & SPF_LINKEA)
                                sp->sp_flags |= SPF_LINKEA;
                        else
                                sp->sp_flags &= ~SPF_LINKEA;
                }
                cfs_up(&mdd->mdd_scrub_header_sem);
        ...
        if (sp->sp_flags & SPF_LINKEA)
                dsp->dsp_flags |= DSF_LINKEA;
