				     ...
				     struct dt_allocation_hint *hint)
{
	...
	rc = mdd_declare_create_internal(env, p, c, attr, handle, spec, hint);
	if (rc)
		GOTO(out, rc);
			      ...
			      struct dt_allocation_hint *hint)
{
	...
	rc = mdd_declare_create(env, mdd, p, c, attr, handle, spec,
 ...
 **/
static int mdd_create(const struct lu_env *env, struct mdd_object *pobj,
		      struct mdd_object *son, struct lu_attr *attr,
		      struct md_op_spec *spec, struct lu_buf *acl_buf,
		      struct lu_buf *def_acl_buf,
		      struct dt_allocation_hint *hint, struct thandle *handle)
{
	const struct lu_buf *buf;
	int rc;

	mdd_write_lock(env, son, MOR_TGT_CHILD);
	LASSERT(!mdd_object_exists(son));
	rc = mdo_create(env, son, attr, hint, &mdd_env_info(env)->mti_dof,
			handle);
	if (rc)
		GOTO(unlock, rc);
		      ...
		      struct md_op_spec *spec, struct md_attr *ma)
{
	...
	rc = mdd_create(env, mdd_pobj, son, attr, spec, &acl_buf,
				      ...
				      struct thandle *handle)
{
	...
	rc = mdd_declare_create_internal(env, mdd_pobj, mdd_tobj, la,
						handle, spec, NULL);
	if (rc != 0)
		return rc;
			      ...
			      struct lu_attr *la)
{
	...
	rc = mdd_create(env, mdd_pobj, mdd_tobj, la, spec, NULL, NULL,
			hint, handle);
	if (rc != 0)
		GOTO(stop_trans, rc);
