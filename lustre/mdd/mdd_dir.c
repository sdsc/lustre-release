...
out_stop:
	...
	if (rc == 0) {
		 ...
		 * fail, because local transaction will mostly succeed at this
		 * stage. And there is no easy way to rollback all of previous
		 * operation, so let's remove the object from namespace, and
		 * LFSCK should handle the orphan object. XXX */
