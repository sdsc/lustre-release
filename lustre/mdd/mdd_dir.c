			     ...
			     const struct lu_name *child_name)
{
	...
	for (count = 0; count < ldata.ld_leh->leh_reccount; count++) {
		...
		if (!(mdd_sobj->mod_flags & MIGRATING_OBJ) &&
		    lmm_buf.lb_buf != NULL) {
			/* If the file is mulitple link regular file, since
			 * these links will be updated in several transaction,
			 * and right now we do not allow different MDT objects
			 * point to the same OST objects, so we will delete
			 * the layout right now. */
			rc = mdo_xattr_del(env, mdd_sobj,
					   XATTR_NAME_LOV, handle,
					   mdd_object_capa(env, mdd_sobj));
			if (rc != 0)
				GOTO(stop_trans, rc);
			mdd_sobj->mod_flags |= MIGRATING_OBJ;
		}
...
stop_trans:
		mdd_trans_stop(env, mdd, rc, handle);
		if (OBD_FAIL_CHECK(OBD_FAIL_MIGRATE_LINKEA))
			rc = -EINTR;
out_put:
				   ...
				   struct md_attr *ma)
{
	...
	if (S_ISREG(so_attr->la_mode) && so_attr->la_nlink == 1) {
		rc = mdo_xattr_del(env, mdd_sobj, XATTR_NAME_LOV, handle,
				   mdd_object_capa(env, mdd_sobj));
		/* If the file is mulitple link regular file, the LOVEA
		 * has been deleted in mdd_update_linkea */
		if (rc != 0 && rc != -ENODATA)
			GOTO(stop_trans, rc);
	}
	...
	if (is_dir) {
		...
		if (so_attr->la_nlink > 0) {
			...
			CDEBUG(D_INFO, "%s: "DFID" nlink %d to 1\n",
			       ...
			       so_attr->la_nlink);
			so_attr->la_nlink = 1;
			rc = mdd_attr_set_internal(env, mdd_sobj, so_attr,
						   handle, 0);
			if (rc != 0) {
				mdd_write_unlock(env, mdd_sobj);
				GOTO(stop_trans, rc);
	...
	rc = mdd_attr_set_internal(env, mdd_pobj, la, handle, 0);
	if (rc != 0)
		GOTO(stop_trans, rc);
		       ...
		       struct md_object *tobj, struct md_attr *ma)
{
	...
	rc = mdd_rename_sanity_check(env, mdd_pobj, pattr, mdd_pobj, pattr,
				     mdd_sobj, so_attr, NULL, NULL);
	if (rc != 0)
		GOTO(put, rc);
	 ...
	 * step 4 will update the FID to the new file/directory on the
	 * remote MDT.
	 *
	...
	/* step 3: migrate entries to the orphan object */
	if (S_ISDIR(lu_object_attr(&mdd_sobj->mod_obj.mo_lu))) {
		...
		/* step 4: migrate linkea to the orphan object */
	...
	/* step 5: update name entry to the new object */
