...
static int mdd_declare_links_add(const struct lu_env *env,
				 struct mdd_object *mdd_obj,
				 struct thandle *handle,
				 struct linkea_data *ldata, bool create)
{
	int	rc;
	int	ea_len;
	void	*linkea;
			    ...
			    struct linkea_data *data)
{
	...
	rc = mdd_declare_links_add(env, c, handle, data, false);
	if (rc != 0)
		return rc;
			      ...
			      struct dt_allocation_hint *hint)
{
	...
	if (unlikely(spec->sp_cr_flags & MDS_OPEN_VOLATILE)) {
		...
		rc = mdd_declare_links_add(env, c, handle, ldata, true);
		if (rc)
			return rc;
			      ...
			      struct thandle *handle)
{
	...
	rc = mdd_declare_links_add(env, mdd_sobj, handle, ldata, false);
	if (rc)
		return rc;
					   ...
					   struct thandle *handle)
{
	...
	rc = mdd_declare_links_add(env, mdd_tobj, handle, NULL, false);
	if (rc != 0)
		return rc;
