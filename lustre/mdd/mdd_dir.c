...
out:
	if (rc != 0) {
		if (newlname == NULL)
			CDEBUG_LIMIT(D_ERROR, "link_ea add failed %d "DFID"\n",
				     rc, PFID(mdd_object_fid(mdd_obj)));
		else if (oldpfid == NULL)
			CDEBUG_LIMIT(D_ERROR, "link_ea add '%.*s' failed %d "
				     DFID"\n", newlname->ln_namelen,
				     newlname->ln_name, rc,
				     PFID(mdd_object_fid(mdd_obj)));
		else if (newpfid == NULL)
			CDEBUG_LIMIT(D_ERROR, "link_ea del '%.*s' failed %d "
				     DFID"\n", oldlname->ln_namelen,
				     oldlname->ln_name, rc,
				     PFID(mdd_object_fid(mdd_obj)));
		else
			CDEBUG_LIMIT(D_ERROR, "link_ea rename '%.*s'->'%.*s' "
				     "failed %d "DFID"\n", oldlname->ln_namelen,
				     oldlname->ln_name, newlname->ln_namelen,
				     newlname->ln_name, rc,
				     PFID(mdd_object_fid(mdd_obj)));
	}
