				...
				bool declare, __u32 flags)
{
	...
	/* la_extra_flags will be stored in LMA in osd_attr_set() */
	attr->la_valid = LA_FLAGS;
	attr->la_extra_flags = flags;
				     ...
				     struct thandle *handle)
{
	...
	rc = mdd_mark_dead_object(env, obj, handle, true, LUSTRE_LMA_DEAD_FL);
	if (rc != 0)
		return rc;
