			      ...
			      struct mdd_object *mdd_tobj)
{
	...
	while (rem > 0) {
		...
		if (strncmp(XATTR_NAME_LOV, xname,
		    ...
		    strncmp(XATTR_NAME_LMV, xname,
		            sizeof(XATTR_NAME_LMV) - 1) == 0)
			goto next;
			      ...
			      struct lu_attr *la)
{
	...
	mgr_ea = (struct lmv_mds_md *)info->mti_xattr_buf;
	mgr_ea->lmv_magic = cpu_to_le32(LMV_MAGIC_MIGRATE);
	mgr_ea->lmv_count = 2;
	mgr_ea->lmv_master = mdd_seq_site(mdd)->ss_node_id;
	mgr_ea->lmv_data[0] = *lu_object_fid(mdd2lu_obj(mdd_sobj));
	mgr_ea->lmv_data[1] = *lu_object_fid(mdd2lu_obj(mdd_tobj));
	fid_cpu_to_le(&mgr_ea->lmv_data[0], &mgr_ea->lmv_data[0]);
	fid_cpu_to_le(&mgr_ea->lmv_data[1], &mgr_ea->lmv_data[1]);
