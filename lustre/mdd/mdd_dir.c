                      ...
                      struct md_attr *ma)
{
        ...
        rc = mdd_declare_unlink(env, mdd, mdd_pobj, mdd_cobj, lname, ma, handle);
        if (rc)
                GOTO(stop, rc);
                           ...
                           const struct md_attr *ma)
{
        ...
        /* XXX: we never get here in w/o DNE */
        LBUG();
                           ...
                           struct md_attr *ma)
{
        ...
        rc = mdd_declare_create_data(env, mdd, son, handle);
        if (rc)
                GOTO(stop, rc);
                      ...
                      struct md_attr* ma)
{
        ...
        rc = mdd_declare_create(env, mdd, mdd_pobj, son, lname, ma,handle,spec);
        if (rc)
                GOTO(out_stop, rc);
                              ...
                              struct thandle *handle)
{
        ...
        if (S_ISDIR(mdd_object_type(mdd_sobj))) {
                /* source child can be directory, counted by source dir's nlink */
                ...
                /* new target child can be directory, counted by target dir's nlink */
        ...
        /* new name */
        rc = mdo_declare_index_insert(env, mdd_tpobj, mdo2fid(mdd_tobj),
                        tname->ln_name, handle);
        if (rc)
                return rc;

        if (mdd_tobj && mdd_object_exists(mdd_tobj)) {
                /* target child can be directory, counted by target dir's nlink */
