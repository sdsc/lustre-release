				      ...
				      int declare)
{
	...
	for (count = 0; count < ldata->ld_leh->leh_reccount; count++) {
		...
		CDEBUG(D_INFO, "%s: update "DFID" with %*s:"DFID" %d\n",
		       mdd2obd_dev(mdd)->obd_name, PFID(mdd_object_fid(pobj)),
		       lname.ln_namelen, lname.ln_name,
		       PFID(mdd_object_fid(mdd_tobj)), lname.ln_namelen);
			      ...
			      struct lu_attr *la)
{
	...
	const struct lu_buf	*buf;
	struct thandle		*handle;
	struct lmv_mds_md_common *mgr_ea_common;
	struct lmv_mds_md_v1	*mgr_ea_v1;
	int			mgr_easize;
	int			rc;
	ENTRY;
	...
	mgr_ea_common = (struct lmv_mds_md_common *)info->mti_xattr_buf;
	mgr_ea_v1 = (struct lmv_mds_md_v1 *)info->mti_xattr_buf;
	mgr_ea_common->lmv_magic = cpu_to_le32(LMV_MAGIC_MIGRATE);
	mgr_ea_common->lmv_stripe_count = cpu_to_le32(2);
	mgr_ea_common->lmv_master_mdt_index = mdd_seq_site(mdd)->ss_node_id;
	mgr_ea_v1->lmv_data[0] = *lu_object_fid(mdd2lu_obj(mdd_sobj));
	mgr_ea_v1->lmv_data[1] = *lu_object_fid(mdd2lu_obj(mdd_tobj));
	fid_cpu_to_le(&mgr_ea_v1->lmv_data[0], &mgr_ea_v1->lmv_data[0]);
	fid_cpu_to_le(&mgr_ea_v1->lmv_data[1], &mgr_ea_v1->lmv_data[1]);
	...
	mgr_easize = lmv_mds_md_size(2, LMV_MAGIC_MIGRATE);
	buf = mdd_buf_get_const(env, mgr_ea_v1, mgr_easize);
