...
static int mdd_migrate_xattr_internal(const struct lu_env *env,
				      struct mdd_object *mdd_sobj,
				      struct mdd_object *mdd_tobj,
				      struct thandle *handle,
				      bool declare)
{
	...
	list_xsize = mdd_xattr_list(env, &mdd_sobj->mod_obj, &LU_BUF_NULL);
	...
	rc = mdd_xattr_list(env, &mdd_sobj->mod_obj, &list_xbuf);
	if (rc < 0)
		return rc;
	rc = 0;
	rem = list_xsize;
	...
	while (rem > 0) {
		...
		if (declare) {
			rc = mdo_declare_xattr_set(env, mdd_tobj, &xbuf, xname,
						   0, handle);
			if (rc != 0)
				GOTO(out, rc);
		} else {
			rc = mdo_xattr_set(env, mdd_tobj, &xbuf, xname, 0,
					   handle,
					   mdd_object_capa(env, mdd_sobj));
			if (rc != 0)
				GOTO(out, rc);
...
static int mdd_declare_migrate_xattr(const struct lu_env *env,
				     struct mdd_object *mdd_sobj,
				     struct mdd_object *mdd_tobj,
				     struct thandle *handle)
{
	return mdd_migrate_xattr_internal(env, mdd_sobj, mdd_tobj, handle,
					  true);
}

static int mdd_migrate_xattr(const struct lu_env *env,
			     struct mdd_object *mdd_sobj,
			     struct mdd_object *mdd_tobj,
			     struct thandle *handle)
{
				      ...
				      struct thandle *handle)
{
	...
	rc = mdd_declare_migrate_xattr(env, mdd_sobj, mdd_tobj, handle);
	if (rc != 0)
		return rc;
			      ...
			      struct lu_attr *la)
{
	...
	rc = mdd_migrate_xattr(env, mdd_sobj, mdd_tobj, handle);
	if (rc != 0)
		GOTO(stop_trans, rc);
stop_trans:
	if (handle != NULL)
			       ...
			       struct mdd_object *mdd_tobj)
{
	...
	do {
		...
		rc = __mdd_index_insert(env, mdd_tobj, &ent->lde_fid, name, 0,
		       ...
		       struct md_object *tobj, struct md_attr *ma)
{
	 ...
	 * 2. For directory, migrate the entries to the new MDT and update
	 * linkEA of each children. (In multiple transactions).
	 * 3. Finally, update the name/FID to point to the new file/directory
	 * in a separate transaction.
	 *
	 * So if 1 and 2 succeed, but 3 failed, it will left some orphans in
	 * the filesystem, but the filesystem is still consistency. */

	/* step 1: create orphan object on the remote MDT */
	rc = mdd_migrate_create(env, mdd_pobj, mdd_sobj, mdd_tobj, so_attr);
	if (rc != 0)
		GOTO(unlock, rc);
