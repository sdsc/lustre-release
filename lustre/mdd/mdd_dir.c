				...
				bool declare, __u32 flags)
{
	...
	/* If the object is not orphan, we do not need set
	 * flags to OSD, mdd_finish_unlink()->mdo_destroy()->
	 * osd_object_destroy() will mark the object is destoryed */
	if (!(flags & LUSTRE_ORPHAN_FL))
		return 0;
				     ...
				     struct thandle *handle)
{
	...
	rc = mdd_mark_dead_object(env, obj, handle, true, LUSTRE_ORPHAN_FL);
	if (rc != 0)
		return rc;
