			    ...
			    struct linkea_data *data)
{
	...
	dir->dir_type = mdd_object_type(c);
			      ...
			      struct thandle *handle)
{
	...
	dir->dir_type = mdd_object_type(mdd_sobj);
				      ...
				      int declare)
{
	...
	for (count = 0; count < ldata->ld_leh->leh_reccount; count++) {
		...
		if (declare) {
			...
			dir->dir_type = mdd_object_type(mdd_tobj);
			       ...
			       struct mdd_object *mdd_tobj)
{
	...
	do {
		...
		if (likely(!target_exist)) {
			...
			dir->dir_type = mdd_object_type(child);
