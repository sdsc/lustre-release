...
again:
	...
	for (count = 0; count < ldata->ld_leh->leh_reccount; count++) {
		...
		if (declare) {
			 ...
			 * snprintf */
			snprintf(tmp_name, lname.ln_namelen, "%s",
				 lname.ln_name);
				    ...
				    struct lu_attr *sattr)
{
	...
	source_idx = mdd->mdd_md_dev.md_lu_dev.ld_site->ld_seq_site->ss_node_id;
	...
	for (count = 0; count < ldata->ld_leh->leh_reccount; count++) {
		...
		rc = mdd_fld_lookup(env, mdd, &fid, &parent_idx);
		if (rc != 0)
			RETURN(rc);

		/* Only migrate the object if all of its parents are not all
		 * current MDT. */
