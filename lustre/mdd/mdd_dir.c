...
static int __mdd_lookup(const struct lu_env *, struct md_object *,
			const struct lu_attr *, const struct lu_name *,
			struct lu_fid*, int);
                    ...
                    struct lu_fid *fid, int mask)
{
        ...
        if (unlikely(dlh == NULL))
                return -ENOMEM;
        rc = __mdd_lookup(env, pobj, pattr, lname, fid, mask);
...
{
	struct lu_attr *pattr = mdd_env_pattr(env);
        int rc;
        ENTRY;
	...
	if (rc != 0)
        ...
        rc = __mdd_lookup_locked(env, pobj, pattr, lname, fid, MAY_EXEC);
        RETURN(rc);
}
		   ...
		   struct lu_fid *fid)
{
	...
	rc = mdd_la_get(env, obj, pattr, BYPASS_CAPA);
	if (rc != 0)
		RETURN(rc);

        return __mdd_lookup_locked(env, &obj->mod_obj, pattr, &lname_dotdot,
				   fid, 0);
}
				 ...
				 struct mdd_object *src_obj)
{
        ...
        if (tgt_obj) {
                rc = mdd_may_create(env, tgt_obj, mdd_env_tattr(env), NULL, 1,
				    0);
                if (rc)
                        RETURN(rc);
        }

        rc = __mdd_may_link(env, src_obj, mdd_env_cattr(env));
                    ...
                    struct md_attr *ma)
{
	...
	rc = mdd_la_get(env, mdd_tobj, mdd_env_cattr(env), BYPASS_CAPA);
	if (rc != 0)
		RETURN(rc);

	rc = mdd_la_get(env, mdd_tobj, mdd_env_tattr(env), BYPASS_CAPA);
	if (rc != 0)
		RETURN(rc);
	...
	if (rc != 0) {
        ...
        la->la_valid = LA_CTIME | LA_MTIME;
	rc = mdd_time_check_set(env, mdd_tobj, mdd_env_tattr(env), la, handle);
        if (rc)
                GOTO(out_unlock, rc);

        la->la_valid = LA_CTIME;
        rc = mdd_time_check_set(env, mdd_sobj, mdd_env_cattr(env), la, handle);
	if (rc == 0) {
			    ...
			    struct mdd_object *cobj)
{
	...
	rc = mdd_may_delete(env, pobj, mdd_env_pattr(env), cobj,
			    mdd_env_cattr(env), NULL, 1, 1);
...
{
        const char *name = lname->ln_name;
	struct lu_attr    *cattr = mdd_env_cattr(env);
        struct lu_attr    *la = &mdd_env_info(env)->mti_la_for_fix;
	...
	rc = mdd_la_get(env, mdd_pobj, mdd_env_pattr(env), BYPASS_CAPA);
	if (rc)
		RETURN(rc);
	...
	rc = mdd_unlink_sanity_check(env, mdd_pobj, mdd_cobj);
	if (rc)
		GOTO(cleanup, rc);
	...
	la->la_valid = LA_CTIME | LA_MTIME;
	rc = mdd_time_check_set(env, mdd_pobj, mdd_env_pattr(env), la, handle);
	if (rc)
		GOTO(cleanup, rc);
	...
	if (cattr->la_nlink > 0 || mdd_cobj->mod_count > 0) {
                ...
                la->la_valid = LA_CTIME;
                rc = mdd_time_check_set(env, mdd_cobj, cattr, la, handle);
                if (rc)
                        GOTO(cleanup, rc);
        }
                           ...
                           struct md_attr *ma)
{
	...
	struct lu_attr    *attr = mdd_env_cattr(env);
        int                rc;
        ENTRY;
                                   ...
                                   struct md_op_spec *spec)
{
	...
	if (spec->sp_cr_lookup) {
                 ...
                 * _index_insert.
                 */
                rc = __mdd_lookup_locked(env, pobj, pattr, lname, fid,
                                         MAY_WRITE | MAY_EXEC);
                if (rc != -ENOENT)
                        RETURN(rc ? : -EEXIST);
        } else {
		      ...
		      struct md_op_spec *spec, struct md_attr* ma)
{
	...
	rc = mdd_create_sanity_check(env, pobj, lname, attr, spec);
        if (rc)
                RETURN(rc);
	...
	*la = *attr;
	la->la_valid = LA_CTIME | LA_MTIME;
	rc = mdd_time_check_set(env, mdd_pobj, pattr, la, handle);
	if (rc)
		GOTO(cleanup, rc);
                                   ...
                                   struct mdd_object *tobj)
{
	...
	rc = mdd_may_delete(env, src_pobj, mdd_env_pattr(env), sobj,
			    mdd_env_cattr(env), NULL, 1, 0);
	if (rc)
		RETURN(rc);
	...
	if (!tobj)
		rc = mdd_may_create(env, tgt_pobj, mdd_env_tpattr(env), NULL,
				    (src_pobj != tgt_pobj), 0);
	else
		rc = mdd_may_delete(env, tgt_pobj, mdd_env_tpattr(env), tobj,
				    mdd_env_tattr(env), mdd_env_cattr(env),
				    (src_pobj != tgt_pobj), 1);

	if (!rc && !tobj && (src_pobj != tgt_pobj) &&
	    S_ISDIR(mdd_env_cattr(env)->la_mode))
		rc = __mdd_may_link(env, tgt_pobj, mdd_env_tpattr(env));
                      ...
                      struct md_attr *ma)
{
	...
	struct lu_attr *tattr = mdd_env_tattr(env);
	...
	rc = mdd_la_get(env, mdd_sobj, mdd_env_cattr(env), BYPASS_CAPA);
	if (rc)
		GOTO(out_pending, rc);

	rc = mdd_la_get(env, mdd_spobj, mdd_env_pattr(env), BYPASS_CAPA);
	if (rc)
		GOTO(out_pending, rc);
	...
	rc = mdd_la_get(env, mdd_tpobj, mdd_env_tpattr(env), BYPASS_CAPA);
	if (rc)
		GOTO(out_pending, rc);
	...
	is_dir = S_ISDIR(mdd_env_cattr(env)->la_mode);
        ...
        if (mdd_sobj) {
                la->la_valid = LA_CTIME;
		rc = mdd_time_check_set(env, mdd_sobj, mdd_env_cattr(env), la,
					handle);
                if (rc)
                        GOTO(fixup_tpobj, rc);
        }
		...
		la->la_valid = LA_CTIME;
		rc = mdd_time_check_set(env, mdd_tobj, tattr, la, handle);
		if (rc != 0) {
		...
		if (mdd_env_cattr(env)->la_nlink == 0)
			cl_flags |= CLF_RENAME_LAST;
        }

        la->la_valid = LA_CTIME | LA_MTIME;
	rc = mdd_time_check_set(env, mdd_spobj, mdd_env_pattr(env), la, handle);
        if (rc)
                GOTO(fixup_tpobj, rc);

        if (mdd_spobj != mdd_tpobj) {
                la->la_valid = LA_CTIME | LA_MTIME;
		rc = mdd_time_check_set(env, mdd_tpobj, mdd_env_tpattr(env), la,
					handle);
        }
