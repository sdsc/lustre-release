				      ...
				      struct thandle *handle)
{
	...
	la_copy->la_flags = LUSTRE_IMMUTABLE_FL;
	mdd_flags_xlate(mdd_sobj, la_copy->la_flags);
	rc = mdo_declare_attr_set(env, mdd_sobj, la_copy, handle);
			      ...
			      struct lu_attr *la)
{
	...
	struct lu_attr		*la_copy = &info->mti_la_for_fix;
	int			mgr_easize;
	int			rc;
	ENTRY;
	...
	if (S_ISLNK(la->la_mode)) {
		buf = lu_buf_check_and_alloc(
				...
				la->la_size + 1);
		rc = mdd_readlink(env, &mdd_sobj->mod_obj,
				  (struct lu_buf *)buf);
		if (rc != 0)
			RETURN(rc);
		spec->u.sp_symname = buf->lb_buf;
	} else{
	...
	la_copy->la_flags = LUSTRE_IMMUTABLE_FL;
	mdd_flags_xlate(mdd_sobj, la_copy->la_flags);
	rc = mdo_attr_set(env, mdd_sobj, la_copy, handle,
			  mdd_object_capa(env, mdd_sobj));
stop_trans:
	if (handle != NULL)
		mdd_trans_stop(env, mdd, rc, handle);
				    ...
				    struct lu_attr *sattr)
{
	...
	mgr_easize = lmv_mds_md_size(2, LMV_MAGIC_MIGRATE);
	mgr_buf = lu_buf_check_and_alloc(&mdd_env_info(env)->mti_big_buf,
					 mgr_easize);
