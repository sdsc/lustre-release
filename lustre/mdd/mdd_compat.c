 ...
 *
 * Given only ZFS is affected where ./.. are not stored, we need to:
 *  - delete ./.. from /ROOT and it`s subdirectories
				   ...
				   struct mdd_object *o)
{
	...
	/* remove . and .. */
	/* do not insert them back - not stored in ZFS */
	...
	if (rc)
		GOTO(out, rc);
	/* ignore non-existing ./.. - we stored them on a disk on the
...
static int mdd_fix_childs(const struct lu_env *env,
			  struct mdd_device *mdd,
			  struct dt_object *o)
{
...
int mdd_compat_fixes(const struct lu_env *env, struct mdd_device *mdd)
{
	...
	if (fid_is_igif(&mdd->mdd_root_fid)) {
		/* IGIF is valid */
		RETURN(0);
	}

	/*
	 * FID is supposed to be IGIF or in FID_SEQ_ROOT for:
	 *  - new ldiskfs fs
	 *  - new zfs fs
	 *  - old zfs fs, by now processed with osd_convert_root_to_new_seq()
	 */
	if (fid_seq(&mdd->mdd_root_fid) != FID_SEQ_ROOT) {
		CERROR("%s: wrong seq "LPX64" is used for /ROOT\n",
		       mdd2obd_dev(mdd)->obd_name,
		       fid_seq(&mdd->mdd_root_fid));
		RETURN(-EINVAL);
	}
	...
	if (strcmp(mdd->mdd_bottom->dd_lu_dev.ld_type->ldt_name,
		...
		CERROR("%s: "DFID" is used on ldiskfs?!\n",
		       PFID(&mdd->mdd_root_fid), mdd2obd_dev(mdd)->obd_name);
		RETURN(-ENOTSUPP);
	}
	...
	rc = mdd_fix_childs(env, mdd, o);
	if (rc)
		GOTO(out, rc);

	/* update LMA on /ROOT. made for simplicity in MDD,
	 * not in osd-zfs. correct LMA will imply the whole
	 * directory have been coverted successfully */
