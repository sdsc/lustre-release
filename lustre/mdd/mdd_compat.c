				   ...
				   struct mdd_object *o)
{
	...
	rc = dt_delete(env, mdd_object_child(o), dot, th, LC_BYPASS_CAPA);
	if (rc == -ENOENT)
		rc = 0;
	if (rc)
		GOTO(out, rc);
	rc = dt_delete(env, mdd_object_child(o), dotdot, th, LC_BYPASS_CAPA);
	if (rc == -ENOENT)
		rc = 0;
	if (rc)
		GOTO(out, rc);
			      ...
			      const struct lu_name *name)
{
	...
	rc = mdo_attr_get(env, o, la, LC_BYPASS_CAPA);
	if (rc)
		GOTO(out, rc);
			   ...
			   struct mdd_object *o)
{
	...
	if (rc)
		GOTO(out, rc);
	rc = mdo_xattr_set(env, o, &buf, XATTR_NAME_LMA, 0, th, LC_BYPASS_CAPA);
out:
	dt_trans_stop(env, mdd->mdd_child, th);
	RETURN(rc);
}
			    ...
			    struct dt_object *o)
{
	...
	it = iops->init(env, o, LUDA_64BITHASH, LC_BYPASS_CAPA);
	if (IS_ERR(it)) {
		rc = PTR_ERR(it);
...
int mdd_compat_fixes(const struct lu_env *env, struct mdd_device *mdd)
{
	...
	buf.lb_buf = lma;
	rc = mdo_xattr_get(env, root, &buf, XATTR_NAME_LMA, LC_BYPASS_CAPA);
	if (rc < 0 && rc != -ENODATA) {
