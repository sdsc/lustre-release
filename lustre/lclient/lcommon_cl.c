...
void *ccc_key_init(const struct lu_context *ctx, struct lu_context_key *key)
{
	...
	OBD_SLAB_ALLOC_PTR_GFP(info, ccc_thread_kmem, CFS_ALLOC_NOFS);
	if (info == NULL)
		info = ERR_PTR(-ENOMEM);
	return info;
}
                                  ...
                                  struct lu_context_key *key)
{
	...
	OBD_SLAB_ALLOC_PTR_GFP(session, ccc_session_kmem, CFS_ALLOC_NOFS);
	if (session == NULL)
		 ...
		 struct cl_req *req)
{
	...
	OBD_SLAB_ALLOC_PTR_GFP(vrq, ccc_req_kmem, CFS_ALLOC_NOFS);
	if (vrq != NULL) {
				   ...
				   const struct lu_object_operations *luops)
{
	...
	OBD_SLAB_ALLOC_PTR_GFP(vob, ccc_object_kmem, CFS_ALLOC_NOFS);
	if (vob != NULL) {
		  ...
		  const struct cl_lock_operations *lkops)
{
	...
	OBD_SLAB_ALLOC_PTR_GFP(clk, ccc_lock_kmem, CFS_ALLOC_NOFS);
	if (clk != NULL) {
