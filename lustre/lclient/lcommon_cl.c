...
struct lov_stripe_md *__lsm_get(struct inode *inode, int io)
{
        ...
        if (S_ISREG(cl_inode_mode(inode)) && !io)
                ...
                struct cl_io *io)
{
        ...
        LASSERT(io->ci_lsm == NULL && io->ci_ll == NULL);

        /* grab reference on the layout lock for read/write/truncate and just
         * a reference on the lsm for the other operations if ci_lsm_needed
         * is set */
        if (io->ci_type == CIT_READ || io->ci_type == CIT_WRITE ||
            io->ci_type == CIT_FAULT || cl_io_is_trunc(io)) {
                io->ci_lsm = lsm_get_io(inode);
                io->ci_ll  = &cl_i2info(inode)->lli_ll;
        } else if (io->ci_lsm_needed) {
...
int ccc_io_iter_init(const struct lu_env *env, const struct cl_io_slice *ios)
{
        ...
        io->ci_lsm = lsm_get_io(inode);
        io->ci_ll  = &cl_i2info(inode)->lli_ll;
        if (layout_gen != io->ci_lsm->lsm_layout_gen)
