...
/* In userspace (liblustre.h), these definitions in their normal form (e.g.
 * #define i_mtime i_stbuf.st_mtime, where st_mtime is #defined as
 * st_mtim.tv_sec by bits/stat.h) prevent us from frobbing the struct timespec
 * contained within the stat structure of the userland inode structures. To
 * maintain a consistent-looking interface in both kernel and userland, we
 * define i_xtime to reference the struct timespec of the stat structure in
 * userland to emulate the i_xtime struct timespecs in a real Linux VFS inode
 * structure.
 */
#undef i_mtime
#undef i_atime
