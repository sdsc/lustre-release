                ...
                if (rc > 0) {
                        ...
                        if (rc == 0) {
                                ...
                                *descr = whole_file;
                                ...
                                if (!IS_ERR(lock)) {
                                        rc = cl_wait_cancel(env, lock, 0, 0);
                                        if (rc == CLO_WAIT) {
                                                caa->caa_env = env;
                                                caa->caa_lock = lock;
...
int cl_agl_fini(struct inode *inode, struct cl_agl_args *caa)
{
        ...
        rc = cl_wait_cancel(env, lock, 0, 0);
        if (rc == CLO_WAIT) {
                rc = cl_wait_cancel(env, lock, 1, 0);
                if (rc == 0)
                        rc = +1;
        }
        if (rc >= 0) {
        ...
        cl_io_fini(env, caa->caa_io);
        cl_env_put(env, &caa->caa_refcheck);
        cl_env_decover(env);
        RETURN(rc);
}

int cl_agl_cleanup(struct cl_agl_args *caa)
{
        struct lu_env  *env = caa->caa_env;
        struct cl_lock *lock = caa->caa_lock;
        int rc;
        ...
        cl_env_recover(env);
        rc = cl_wait_cancel(env, lock, 0, 1);
        if (rc == 0)
                cl_unuse(env, lock);
        cl_lock_release(env, lock, caa->caa_scope, caa->caa_source);
        cl_io_fini(env, caa->caa_io);
