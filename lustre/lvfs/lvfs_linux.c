...
int lvfs_check_io_health(struct obd_device *obd, struct file *file)
{
	...
	OBD_ALLOC(write_page, PAGE_CACHE_SIZE);
        if (!write_page)
                RETURN(-ENOMEM);

	rc = fsfilt_write_record(obd, file, write_page, PAGE_CACHE_SIZE, &offset, 1);

	OBD_FREE(write_page, PAGE_CACHE_SIZE);
