...
{
	struct cfg_interop_param *param_ptr = NULL;
	char			 *value = NULL;
	int			  name_len = 0;
	...
	param_ptr = ptr;
	while (param_ptr->old_param != NULL) {
		if (strncmp(param, param_ptr->old_param, name_len) == 0 &&
		    name_len == strlen(param_ptr->old_param))
			RETURN(param_ptr);
		param_ptr++;
	}
