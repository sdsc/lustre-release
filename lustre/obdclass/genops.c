...
unsigned int class_en_enable = 1;
EXPORT_SYMBOL(class_en_enable);
CFS_MODULE_PARM(class_en_enable, "i", int, 0600,
...
{
	do_fail_export(exp, class_en_enable, NULL);
}
...
int obd_export_evict_by_nid(struct obd_device *obd, const char *nid)
{
	...
	do {
		...
		LCONSOLE_WARN("%s: evicting %s (at %s) by administrative "
			      ...
			      obd_export_nid2str(doomed_exp));
		do_fail_export(doomed_exp, class_en_enable, NULL);
