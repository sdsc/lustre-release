...
int obd_set_max_mod_rpcs_in_flight(struct client_obd *cli, int max)
{
	...
	if (max >= cli->cl_max_rpcs_in_flight) {
		CERROR("%s: can't set max_mod_rpcs_in_flight to a value (%d) "
			   ...
			   struct lookup_intent *it)
{
	...
	do {
		spin_lock(&cli->cl_mod_rpcs_lock);
		max = cli->cl_max_mod_rpcs_in_flight;
		/* check if a slot is available (see comment above) */
		if (cli->cl_mod_rpcs_in_flight < max ||
		    (close_req && cli->cl_close_rpcs_in_flight == 0) ||
		    (!close_req && cli->cl_mod_rpcs_in_flight ==
				   cli->cl_close_rpcs_in_flight)) {
			...
			test_and_set_bit(i, cli->cl_mod_tag_bitmap);
			spin_unlock(&cli->cl_mod_rpcs_lock);
			tag = i + 1;
			return tag;
		}
			  ...
			  struct lookup_intent *it, __u16 tag)
{
	...
	test_and_clear_bit(tag - 1, cli->cl_mod_tag_bitmap);
	spin_unlock(&cli->cl_mod_rpcs_lock);
	wake_up(&cli->cl_mod_rpcs_waitq);
}
