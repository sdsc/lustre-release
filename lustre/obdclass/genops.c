...
struct obd_device **class_setup_dev_array(const char *typename, int *len)
{
        int rc, tmp;
        size_t size;
...
EXPORT_SYMBOL(class_setup_dev_array);
...
EXPORT_SYMBOL(class_cleanup_dev_array);
...
void class_fail_export(struct obd_export *exp)
{
        ...
        mgc_array = class_setup_dev_array(LUSTRE_MGC_NAME, &len);
        if (mgc_array) {
                for (i = 0; i < len; i++) {
                        ...
                        /* export object will be evicted after mgc_notify_evict.
                         * So I have to identify an exact mgc object somehow */

                        /* Maybe I can use client's uuid to identify mgc object
                         */
                ...
                class_cleanup_dev_array(mgc_array, len);
                mgc_array = NULL;
        } else {
                do_fail_export(exp);
        }
