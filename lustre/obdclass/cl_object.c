...
static struct lu_env *cl_env_obtain(void *debug)
{
	...
	clc  = &cl_env_caches[cfs_get_cpu()];
	list = &clc->cl_env_cache_list;

	LASSERT(!spin_is_locked(&clc->cl_env_cache_lock));
	spin_lock(&clc->cl_env_cache_lock);
	...
	if (clc->cl_env_cache_nr > 0) {
		...
		spin_unlock(&clc->cl_env_cache_lock);
		cfs_put_cpu();
 ...
 */
void cl_env_cache_purge(void *info)
{
	CFS_LIST_HEAD(dying);
	struct cl_env_cache *clc;
	struct cl_env       *cle;
...
void cl_env_cache_purge_all(void)
{
	ENTRY;
	cfs_on_each_cpu(cl_env_cache_purge, NULL, 1);
	EXIT;
}
...
int cl_global_init(void)
{
	...
	cfs_for_each_possible_cpu(i) {
		...
		spin_lock_init(&cl_env_caches[i].cl_env_cache_lock);
	}
