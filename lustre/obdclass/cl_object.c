...
*/
	...
	for (i = 0; i < CS_NR; i++)
		nob += snprintf(page + nob, count - nob,
				"%8u", cfs_atomic_read(&cs->cs_stats[i]));
        return nob;
}
...
int cl_site_init(struct cl_site *s, struct cl_device *d)
{
		...
		s->cs_pages = lprocfs_alloc_stats(CS_NR, 0);
		if (s->cs_pages != NULL) {
			for (i = 0; i < CS_NR; i++)
				lprocfs_counter_init(s->cs_pages, i,
...
int cl_site_stats_print(const struct cl_site *site, char *page, int count)
{
	...
	/* collect percpu counter for pages. */
	cache_stats_init(&stats, "pages");
	for (i = 0; i < CS_NR; i++) {
		cfs_atomic_set(&stats.cs_stats[i],
			lprocfs_stats_collector(site->cs_pages, i,
 ...
 */
        ...
        nob += cache_stats_print(&stats, page + nob, count - nob, 1);
        nob += snprintf(page + nob, count - nob, " [");
