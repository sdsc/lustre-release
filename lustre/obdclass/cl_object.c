...
void cl_object_kill(const struct lu_env *env, struct cl_object *obj)
{
        ...
        if (cfs_atomic_read(&loh->loh_ref) > 1)
                LU_OBJECT_DEBUG(D_WARNING, env, &obj->co_lu,
                                "kill an object with refcount %d\n",
                                cfs_atomic_read(&loh->loh_ref));

        cfs_might_sleep();

        bkt = lu_site_bkt_from_fid(site, &loh->loh_fid);
        l_wait_event(bkt->lsb_marche_funebre,
                     cfs_atomic_read(&loh->loh_ref) == 1,
                     &lwi);
                                ...
                                cfs_hlist_node_t *hnode, void *data)
{
        ...
        if (obj)
                ...
                obj = cfs_list_entry(hdr->loh_layers.next, struct lu_object,
                                     lo_linkage);
        lu_object_put(arg->lsp_env, obj);
