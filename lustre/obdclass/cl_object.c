...
static void __cl_env_percpu_get(struct cl_env **cle, int *cpu)
{
	struct cl_env *__cle;
	int __cpu;
	...
	__cle = cl_env_percpu[__cpu];
	cl_env_percpu[__cpu] = NULL;

	cfs_local_irq_enable();
	cfs_put_cpu();

	/* save cpu number cle was pulled from, this is needed to ensure
	 * cle is put back in the same index in cl_env_percpu_put */
	*cpu = __cpu;
	*cle = __cle;
}


void cl_env_percpu_get(struct lu_env **env, int *cpu)
{
	struct cl_env *__cle;
	int rc, __cpu;

	LASSERT(cpu != NULL && env != NULL);

	__cl_env_percpu_get(&__cle, &__cpu);
	...
	*cpu = __cpu;
	...
	cl_env_init0(__cle, __builtin_return_address(0));
	lu_context_enter(&(*env)->le_ctx);
	lu_context_enter(&__cle->ce_ses);
	cl_env_attach(__cle);
}
