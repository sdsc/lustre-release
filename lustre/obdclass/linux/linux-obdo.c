...
void obdo_refresh_inode(struct inode *dst, struct obdo *src, obd_flag valid)
{
	...
	if (valid & OBD_MD_FLATIME && src->o_atime >= dst->i_atime.tv_sec) {
		if (src->o_atime == dst->i_atime.tv_sec &&
		    src->o_atime_ns > dst->i_atime.tv_nsec) {
			dst->i_atime.tv_nsec = src->o_atime_ns;
		} else if (src->o_atime > dst->i_atime.tv_sec) {
			dst->i_atime.tv_sec = src->o_atime;
			dst->i_atime.tv_nsec = src->o_atime_ns;
		}
	}
	if (valid & OBD_MD_FLMTIME && src->o_mtime >= dst->i_mtime.tv_sec) {
		if (src->o_mtime == dst->i_mtime.tv_sec &&
		    src->o_mtime_ns > dst->i_mtime.tv_nsec) {
			dst->i_mtime.tv_nsec = src->o_mtime_ns;
		} else if (src->o_mtime > dst->i_mtime.tv_sec) {
			dst->i_mtime.tv_sec = src->o_mtime;
			dst->i_mtime.tv_nsec = src->o_mtime_ns;
		}
	}
	if (valid & OBD_MD_FLCTIME && src->o_ctime >= dst->i_ctime.tv_sec) {
		if (src->o_ctime == dst->i_ctime.tv_sec &&
		    src->o_ctime_ns > dst->i_ctime.tv_nsec) {
			dst->i_ctime.tv_nsec = src->o_ctime_ns;
		} else if (src->o_ctime > dst->i_ctime.tv_sec) {
