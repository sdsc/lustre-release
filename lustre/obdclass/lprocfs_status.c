...
static void
obd_connect_seq_flags2str(struct seq_file *m, __u64 flags, char *sep)
{
	__u64 mask = 1;
	int i;
	bool first = true;
...
static int lprocfs_stats_seq_open(struct inode *inode, struct file *file)
{
	...
	seq->private = CFS_PDE_DATA(inode);
...
{
	struct exp_hash_cb_data *data = cb_data;
	struct obd_export       *exp = cfs_hash_object(hs, hnode);
...
int lprocfs_rd_nid_stats_clear(struct seq_file *m, void *data)
{
	return seq_printf(m, "%s\n",
                        "Write into this file to clear all nid stats and "
                        "stale nid entries");
}
EXPORT_SYMBOL(lprocfs_rd_nid_stats_clear);

static int lprocfs_wr_nid_stats_clear_cb(void *obj, void *data)
{
...
int lprocfs_wr_nid_stats_clear(struct file *file, const char *buffer,
			       unsigned long count, void *data)
{
	struct obd_device *obd = data;
        ...
        cfs_hash_cond_del(obd->obd_nid_stats_hash,
			  lprocfs_wr_nid_stats_clear_cb, &free_list);
...
EXPORT_SYMBOL(lprocfs_wr_nid_stats_clear);
...
int
lprocfs_read_frac_helper(char *buffer, unsigned long count, long val, int mult)
{
...
int lprocfs_rd_obd_hash(struct seq_file *m, void *v)
{
...
EXPORT_SYMBOL(lprocfs_rd_obd_hash);
...
int lprocfs_rd_obd_recovery_status(char *page, char **start, off_t off,
...
EXPORT_SYMBOL(lprocfs_rd_obd_recovery_status);

int lprocfs_rd_obd_ir_factor(struct seq_file *m, void *v)
{
	struct obd_device *obd = m->private;
	LASSERT(obd != NULL);
...
EXPORT_SYMBOL(lprocfs_rd_obd_ir_factor);

int lprocfs_wr_obd_ir_factor(struct file *file, const char *buffer,
			     unsigned long count, void *data)
{
	struct obd_device *obd = data;
	int val, rc;
...
EXPORT_SYMBOL(lprocfs_wr_obd_ir_factor);

int lprocfs_rd_obd_recovery_time_soft(struct seq_file *m, void *v)
{
	struct obd_device *obd = m->private;
	LASSERT(obd != NULL);
...
EXPORT_SYMBOL(lprocfs_rd_obd_recovery_time_soft);

int lprocfs_wr_obd_recovery_time_soft(struct file *file, const char *buffer,
				      unsigned long count, void *data)
{
	struct obd_device *obd = data;
	int val, rc;
...
EXPORT_SYMBOL(lprocfs_wr_obd_recovery_time_soft);

int lprocfs_rd_obd_recovery_time_hard(struct seq_file *m, void *v)
{
	struct obd_device *obd = m->private;
	LASSERT(obd != NULL);
...
EXPORT_SYMBOL(lprocfs_rd_obd_recovery_time_hard);

int lprocfs_wr_obd_recovery_time_hard(struct file *file, const char *buffer,
				      unsigned long count, void *data)
{
	struct obd_device *obd = data;
	int val, rc;
...
EXPORT_SYMBOL(lprocfs_wr_obd_recovery_time_hard);

int lprocfs_rd_obd_max_pages_per_rpc(struct seq_file *m, void *data)
{
        struct obd_device *dev = data;
...
EXPORT_SYMBOL(lprocfs_rd_obd_max_pages_per_rpc);

/*
int lprocfs_rd_target_instance(struct seq_file *m, void *data)
{
       struct obd_device *obd = data;
...
EXPORT_SYMBOL(lprocfs_rd_target_instance);
*/
