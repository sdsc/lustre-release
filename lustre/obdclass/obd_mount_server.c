...
static int server_sb2mti(struct super_block *sb, struct mgs_target_info *mti)
{
	...
	strncpy(mti->mti_fsname, ldd->ldd_fsname, sizeof(mti->mti_fsname));
	strncpy(mti->mti_svname, ldd->ldd_svname, sizeof(mti->mti_svname));
...
int server_register_target(struct super_block *sb)
{
	...
	rc = obd_set_info_async(NULL, mgc->u.cli.cl_mgc_mgsexp,
				...
				KEY_REGISTER_TARGET, sizeof(*mti), mti, NULL);
	if (rc) {
	...
	if (lsi->lsi_ldd->ldd_flags & LDD_F_SV_TYPE_MDT) {
		...
		if (!obd) {
			rc = lustre_start_simple(LUSTRE_MDS_OBDNAME,
				/* FIXME pre-rename, should eventually be
				* LUSTRE_MDS_NAME */
				LUSTRE_MDT_NAME,
...
int server_fill_super(struct super_block *sb)
{
	...
	if (!(lsi->lsi_lmd->lmd_flags & LMD_FLG_NOSVC) &&
		...
		/* FIXME overmount client here, or can we just start a client
		 * log and client_fill_super on this sb?
		 * We need to make sure server_put_super gets called too -
		 * ll_put_super calls lustre_common_put_super; check there for
		 * LSI_SERVER flag, call s_p_s if so. Probably should start
		 * client from new thread so we can return. Client will not
		 * finish until all servers are connected.
		 * Note - MGS-only server does NOT get a client, since there is
		 * no lustre fs associated - the MGS is for all lustre fs's */
