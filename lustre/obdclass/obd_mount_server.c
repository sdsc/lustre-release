...
static int server_start_mgs(struct super_block *sb)
{
	...
	if (!rc) {
		 ...
		 * (see b=17758), but server_put_mount should be called here,
		 * for there is no MGS device with name=LUSTRE_MGS_OBDNAME,
		 * then the mount won't be unreigstered and lsi won't be freed
		 * in mgs_device_fini */
		server_put_mount(LUSTRE_MGS_OBDNAME, mnt);
	}
