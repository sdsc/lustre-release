...
static inline int tgt_is_mdt0(char *tgtname)
{
	__u32 idx;
	int   type;
...
static inline int is_mdc_for_mdt0(char *devname)
{
	char   *ptr;
...
/**
 * Stop the osp(fsname-MDT0000-osp-OSTxxxx) or (fsname-MDT0000-osp-MDT0000) for
 * an OST target.
 **/
...
{
	PRINT_CMD(D_MOUNT, "mti %s\n", title);
	PRINT_CMD(D_MOUNT, "server: %s\n", mti->mti_svname);
	PRINT_CMD(D_MOUNT, "fs:     %s\n", mti->mti_fsname);
	PRINT_CMD(D_MOUNT, "uuid:   %s\n", mti->mti_uuid);
	PRINT_CMD(D_MOUNT, "ver: %d  flags: %#x\n",
