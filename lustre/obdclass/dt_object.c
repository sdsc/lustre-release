			       ...
			       struct dt_it *it, __u32 attr, void *arg)
{
	...
	do {
		...
		if (rc != -ESTALE) {
			...
			/* XXX sigh we do not have rec_size it API, but right
			 * now only lu_dirent will iterate the entry, so we
			 * will use lu_dirent to calculate the recsize */
			if (ii->ii_flags & II_FL_VARREC)
				recsize = lu_dirent_calc_size(keysize, attr);
			else
