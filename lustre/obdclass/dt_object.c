			       ...
			       struct dt_it *it, __u32 attr, void *arg)
{
	...
	LASSERTF(keysize == ii->ii_keysize,
		 "keysize %d, expected size %d, not matched!\n",
		 keysize, ii->ii_keysize);

	if (keysize > sizeof(ii->ii_hash_start))
		bigkey = true;
	...
	do {
		...
		if (!(ii->ii_flags & II_FL_NOHASH) && !bigkey) {
		  ...
		  struct idx_info *ii, const struct lu_rdpg *rdpg)
{
	...
	ii->ii_flags &= (II_FL_NOHASH | IT_FL_VIRTUAL);
