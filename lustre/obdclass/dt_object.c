            ...
            struct lu_buf *buf, loff_t *pos)
{
        ...
        return dt->do_body_ops->dbo_read(env, dt, buf, pos, LC_BYPASS_CAPA);
}
                   ...
                   struct lu_buf *buf, loff_t *pos)
{
        ...
        rc = dt->do_body_ops->dbo_read(env, dt, buf, pos, LC_BYPASS_CAPA);
                    ...
                    const struct lu_buf *buf, loff_t *pos, struct thandle *th)
{
        ...
        rc = dt->do_body_ops->dbo_write(env, dt, buf, pos, th, LC_BYPASS_CAPA, 1);
        if (rc == buf->lb_len)
                rc = 0;
        else if (rc >= 0)
                rc = -EFAULT;
                    ...
                    dt_obj_version_t version, struct thandle *th)
{
        ...
        rc = dt_xattr_set(env, o, &vbuf, xname, 0, th, LC_BYPASS_CAPA);
        if (rc < 0)
...
dt_obj_version_t dt_version_get(const struct lu_env *env, struct dt_object *o)
{
        ...
        vbuf.lb_buf = &version;
        vbuf.lb_len = sizeof(version);
        rc = dt_xattr_get(env, o, &vbuf, xname, LC_BYPASS_CAPA);
		  ...
		  void *arg)
{
	...
	LASSERT(iops != NULL);
	it = iops->init(env, obj, rdpg->rp_attrs, LC_BYPASS_CAPA);
	if (IS_ERR(it))
		RETURN(PTR_ERR(it));
