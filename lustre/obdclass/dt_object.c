                      ...
                      struct dt_device *dev, struct thandle *th)
{
	...
	list_for_each_entry(cb, &dev->dd_txn_callbacks, dtc_linkage) {
		struct thandle *dtc_th;

		if (cb->dtc_txn_start == NULL ||
		    !(cb->dtc_tag & env->le_ctx.lc_tags))
			continue;

		if (cb->dtc_flags & DT_TXN_CALLBACK_TOP) {
			dtc_th = th->th_top;
			dtc_th->th_result = th->th_result;
		} else {
			dtc_th = th;
		}
...
void dt_txn_hook_commit(struct thandle *th)
{
	...
	list_for_each_entry(cb, &th->th_dev->dd_txn_callbacks,
		...
		if (cb->dtc_txn_commit)
			cb->dtc_txn_commit(dtc_th, cb->dtc_cookie);
	}
}
...
static int updates_write(const struct lu_env *env, struct thandle *th)
{
	...
	list_for_each_entry_safe(lst, tmp, &top_th->tt_sub_trans_list, st_list) {
