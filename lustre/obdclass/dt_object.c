...
{
	ldata->ld_buf = lu_large_buf_alloc(buf, CFS_PAGE_SIZE);
		...
		struct linkea_data *ldata)
{
	...
	if (rc == -ERANGE) {
		...
		if (rc < 0)
			return rc;
		ldata->ld_buf = lu_large_buf_alloc(ldata->ld_buf, rc);
		if (ldata->ld_buf->lb_buf == NULL)
			return -ENOMEM;
		rc = dt_xattr_get(env, dt_obj, ldata->ld_buf,
				  XATTR_NAME_LINK, BYPASS_CAPA);
		   ...
		   const struct lu_fid *pfid)
{
	...
	if (ldata->ld_leh->leh_len + ldata->ld_reclen >
	    ldata->ld_buf->lb_len) {
		if (lu_large_buf_grow(ldata->ld_buf, ldata->ld_leh->leh_len +
						     ldata->ld_reclen) < 0)
			return -ENOMEM;
	}
