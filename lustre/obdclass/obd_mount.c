...
/**
 * MDP (Metadata Device Proxy) is used by slaves (Non-MDT0 targets)
 * to manage the connection to MDT0, each filesystem on the slave
 * node will have at most one MDP device.
 *
 * The OST slave will communicate with MDT0 by the connection
 * established by MDP to get quota and fid sequence.
 *
 **/
...
/**
 * Retrieve MDT nids from the client log, then start the MDP device.
...
int lustre_disconnect_osp(char *name)
{
	struct obd_device	*osp;
	char			*ospname;
	struct lustre_cfg	*lcfg = NULL;
	...
	rc = tgt_name2ospname(name, ospname);
	if (rc != 0)
		GOTO(out, rc);

	osp = class_name2obd(ospname);
        if (osp == NULL) {
                CERROR(" Can not find MDP %s\n", ospname);
                GOTO(out, rc = -ENOENT);
        }
...
EXPORT_SYMBOL(lustre_disconnect_osp);
...
static int lustre_stop_osp(struct super_block *sb)
{
	...
	if (rc != 0) {
		CERROR("Can not end config log %s\n", ospname);
		GOTO(cleanup, rc);
	}
...
static void server_put_super(struct super_block *sb)
{
	...
	if (IS_OST(lsi) || IS_MDT(lsi)) {
		if (lustre_stop_osp(sb) < 0)
			CERROR("%s: Can not stop MDP!", tmpname);
	}
