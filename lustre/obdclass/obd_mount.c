...
static int lustre_start_mgc(struct super_block *sb)
{
        ...
        if (obd && !obd->obd_stopping) {
                ...
                if (lmd_is_client(lsi->lsi_lmd)) {
                        ...
                        rc = obd_get_info(obd->obd_self_export,
                                          ...
                                          &vallen, data, NULL);
                        LASSERT(rc == 0);
                        has_ir = !!(data->ocd_connect_flags & OBD_CONNECT_IR);
        ...
        if (lmd_is_client(lsi->lsi_lmd) &&
            lsi->lsi_lmd->lmd_flags & LMD_FLG_NOIR)
                data->ocd_connect_flags &= ~OBD_CONNECT_IR;
...
int server_register_target(struct super_block *sb)
{
        ...
        writeconf = !!(ldd->ldd_flags & (LDD_F_NEED_INDEX |
                                         LDD_F_UPDATE | LDD_F_UPGRADE14));
        /* Register the target */
...
struct lustre_sb_info *lustre_init_lsi(struct super_block *sb)
{
        ...
        lsi->lsi_lmd->lmd_recovery_ir_factor = OBD_IR_FACTOR_DEFAULT;
        s2lsi_nocast(sb) = lsi;
...
void server_calc_timeout(struct lustre_sb_info *lsi, struct obd_device *obd)
{
        ...
        lmd = lsi->lsi_lmd;
        if (lmd) {
                factor = lmd->lmd_recovery_ir_factor;
        ...
        if (has_ir) {
                ...
                new_soft = soft / factor;
                new_hard = hard / factor;

                /* make sure the timeout is not too short */
                new_soft = max_t(int, min, new_soft);
                new_hard = max_t(int, new_soft, new_hard);

                LCONSOLE_INFO("%s: Imperative Recovery enabled, recovery "
                              "window is shrunk from (%d/%d) to (%d/%d)\n",
        ...
        while (*s1) {
                ...
                if (strncmp(s1, "abort_recov", 11) == 0) {
                        ...
                        lmd->lmd_recovery_time_hard = max_t(int,
                                simple_strtoul(s1 + 19, NULL, 10), time_min);
                        clear++;
                } else if (strncmp(s1, "ir_factor=", 10) == 0) {
                        lmd->lmd_recovery_ir_factor = simple_strtoul(s1 + 10,
                                                                     NULL, 10);
                        clear++;
