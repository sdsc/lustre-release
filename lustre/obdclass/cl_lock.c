...
int cl_lock_state_wait(const struct lu_env *env, struct cl_lock *lock)
{
        ...
        cfs_sigset_t blocked, non_fatal;
        int result;
        ...
        if (result == 0) {
                 ...
                 * LU-305 */
                cfs_sigfillset(&non_fatal);
                cfs_sigdelset(&non_fatal, SIGKILL);
                cfs_sigdelset(&non_fatal, SIGINT);
                cfs_sigdelset(&non_fatal, SIGTERM);
                cfs_sigdelset(&non_fatal, SIGQUIT);
                cfs_sigdelset(&non_fatal, SIGALRM);
                blocked = cfs_block_sigs(non_fatal);
