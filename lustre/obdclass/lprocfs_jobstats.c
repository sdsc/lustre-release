 ...
 */
static int job_cleanup_iter_callback(cfs_hash_t *hs, cfs_hash_bd_t *bd,
...
static void lprocfs_job_cleanup(struct obd_job_stats *stats, int before)
{
	...
	if (likely(before >= 0)) {
		if (likely(now < stats->ojs_last_cleanup +
				 stats->ojs_cleanup_interval / 2))
			return;
			   ...
			   cntr_init_callback init_fn)
{
	...
	if (strcmp(obd->obd_type->typ_name, LUSTRE_MDT_NAME) != 0 &&
		...
		CERROR("%s: invalid device type %s for job stats\n",
		       obd->obd_name);
		RETURN(-EINVAL);
	}
