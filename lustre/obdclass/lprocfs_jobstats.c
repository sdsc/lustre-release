...
static int lprocfs_jobstats_seq_show(struct seq_file *p, void *v)
{
        ...
        for (i = 0; i < s->ls_num; i++) {
                ...
                snprintf(lc_name, sizeof(lc_name), "%s:", cntr->lc_name);
                seq_printf(p, "  %-15s ", lc_name);
                seq_printf(p, "{ samples: %15"LPF64"u", ret.lc_count);
                if (cntr->lc_units[0] != '\0')
                        seq_printf(p, ", unit: %s", cntr->lc_units);

                if (cntr->lc_config & LPROCFS_CNTR_AVGMINMAX) {
                        seq_printf(p, ", min:%9"LPF64"u, max:%9"LPF64"u,"
                                   " sum:%16"LPF64"u",
