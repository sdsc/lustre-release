...
void lustre_swab_update_ops(struct update_ops *uops)
{
	...
	__swab32s(&uops->uops_count);
	__swab32s(&uops->uops_padding);
	for (i = 0; i < uops->uops_count; i++) {
		lustre_swab_lu_fid(&uops->uops_op[i].uop_fid);
		__swab16s(&uops->uops_op[i].uop_type);
		__swab16s(&uops->uops_op[i].uop_params_count);
		for (j = 0; j < uops->uops_op[i].uop_params_count; j++)
...
void lustre_swab_llog_rec(struct llog_rec_hdr *rec)
{
	...
	case UPDATE_REC:
	{
		...
		__swab32s(&record->ur_master_index);
		__swab32s(&record->ur_flags);
		__swab64s(&record->ur_batchid);
		__swab64s(&record->ur_cookie);
