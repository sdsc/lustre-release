...
int lustre_get_jobid(char *jobid)
{
	...
	if (rc) {
		if (rc == -EOVERFLOW) {
			 ...
			 * variable length strings instead of fixed numbers),
			 * it might make sense to keep the unique parts for
			 * jobid, instead of just returning an error.  That
			 * would mean allocating a larger temp buffer, and
			 * truncating the string at some separator to fit
			 * into the specified jobid_len. */
			static int printed;
