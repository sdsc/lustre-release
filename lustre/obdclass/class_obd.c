...
int lustre_get_jobid(char *jobid)
{
	...
	memset(tmp_jobid, 0, LUSTRE_JOBID_SIZE);
	...
	if (strcmp(obd_jobid_var, JOBSTATS_NODELOCAL) == 0) {
		memcpy(&tmp_jobid[0], obd_jobid_node, LUSTRE_JOBID_SIZE);
		GOTO(out, rc = 0);
	}

	/* Use process name + fsuid as jobid */
	if (strcmp(obd_jobid_var, JOBSTATS_PROCNAME_UID) == 0) {
		snprintf(&tmp_jobid[0], LUSTRE_JOBID_SIZE, "%s.%u",
			 current_comm(),
			 from_kuid(&init_user_ns, current_fsuid()));
		GOTO(out, rc = 0);
	}

	rc = cfs_get_environ(obd_jobid_var, &tmp_jobid[0], &jobid_len);
	if (rc) {
		if (rc == -EOVERFLOW) {
