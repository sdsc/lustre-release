...
int lustre_get_jobid(char *jobid)
{
        ...
        /* Use process id as jobid, for testing in non-job
         * environment. */
        if (!memcmp(obd_jobid_var, JOBSTATS_PROCESSID,
                    strlen(JOBSTATS_PROCESSID))) {
        ...
        if (ret) {
                CDEBUG((ret != -ENOENT && ret != -EINVAL) ? D_OTHER : D_ERROR,
