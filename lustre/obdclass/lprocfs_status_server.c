...
int lprocfs_exp_print_replydata_seq(cfs_hash_t *hs, cfs_hash_bd_t *bd,
				    struct hlist_node *hnode, void *cb_data)

{
	...
	seq_printf(m, "reply cnt: %d\n"
		      "reply hwm: %d\n"
		      "reply released by xid: %d\n"
		      "reply released by tag: %d\n",
		   ted->ted_reply_cnt,
		   ted->ted_reply_hwm,
		   ted->ted_release_xid,
		   ted->ted_release_tag);
...
int lprocfs_exp_setup(struct obd_export *exp, lnet_nid_t *nid)
{
	...
	if (IS_ERR(entry)) {
		CWARN("Error adding the reply_data file\n");
		rc = PTR_ERR(entry);
		GOTO(destroy_new_ns, rc);
	}
