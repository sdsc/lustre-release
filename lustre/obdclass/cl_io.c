...
/**
 * Cancel list of pages that have been submitted by cl_io_submit_rw.
 */
static int page_io_cancel(const struct lu_env *env, struct cl_sync_io *unused,
			  void *data)
{
	struct cl_page_list *queue = data;
	struct cl_page *page;
	int result = 0;
		    ...
		    long timeout, cl_sync_io_cancel_f cancel, void *data)
{
	...
	rc = l_wait_event(anchor->csi_waitq,
			  atomic_read(&anchor->csi_sync_nr) == 0, &lwi);
	if (rc < 0) {
		int rc2 = -ENOSYS;

		if (cancel != NULL)
			rc2 = cancel(env, anchor, data);

		CERROR("SYNC IO failed with error: %d, try to cancel "
		       "%d remaining entities: %d\n",
		       rc, atomic_read(&anchor->csi_sync_nr), rc2);
