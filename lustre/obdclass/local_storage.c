				      ...
				      struct dt_object_format *dof)
{
	...
	struct dt_insert_rec	*dir	= &dti->dti_dir;
	...
	dir->dir_fid = fid;
	dir->dir_mode = dto->do_lu.lo_header->loh_attr;
	rc = dt_declare_insert(env, parent, (const struct dt_rec *)dir,
	...
	if (dti->dti_dof.dof_type == DFT_DIR) {
		...
		rc = dt_insert(env, dto, (const struct dt_rec *)dir,
			       (const struct dt_key *)".", th, BYPASS_CAPA, 1);
		if (rc != 0)
			GOTO(destroy, rc);

		dt_ref_add(env, dto, th);
		dir->dir_fid = lu_object_fid(&parent->do_lu);
		rc = dt_insert(env, dto, (const struct dt_rec *)dir,
			       (const struct dt_key *)"..", th, BYPASS_CAPA, 1);
		if (rc != 0)
			GOTO(destroy, rc);
	...
	rc = dt_insert(env, parent, (const struct dt_rec *)dir,
