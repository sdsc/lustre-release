			  ...
			  enum llog_open_flag flags)
{
	...
	if (logid != NULL) {
		...
		if (IS_ERR(dchild)) {
			rc = PTR_ERR(dchild);
			CERROR("error looking up logfile "LPX64":0x%x: "
	...
	if (!(flags & LLOG_OPEN_NEW) && handle->lgh_file == NULL)
		GOTO(out, rc = -ENOENT);
out:
	RETURN(rc);
}
...
static int llog_lvfs_create(const struct lu_env *env, struct llog_handle *handle,
			    struct thandle *th)
{
	struct llog_ctxt	*ctxt = handle->lgh_ctxt;
        struct obd_device *obd;
