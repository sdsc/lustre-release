...
static inline int scrub_type_dump(char **buf, int *len, __u16 type)
{
        ...
        switch (type) {
        case ST_OI_SCRUB:
                rc = snprintf(*buf, *len, "LFSCK: OI scrub\n");
                break;
        case ST_LAYOUT_SCRUB:
                rc = snprintf(*buf, *len, "LFSCK: layout scrub\n");
                break;
        case ST_DNE_SCRUB:
                rc = snprintf(*buf, *len, "LFSCK: DNE scrub\n");
                break;
        default:
                rc = snprintf(*buf, *len, "LFSCK: unknown scrub 0x%x\n", type);
                break;
        }
        if (rc <= 0)
                return -ENOSPC;
...
static inline int scrub_bits_dump(char **buf, int *len, int *count,
                                  __u16 flags, const char *names[],
                                  char *estr, int newline)
{
        int save = *len;
        int rc;
        int i;
        ...
        for (i = 0; flags != 0; i++) {
                ...
                if (flags & bit) {
                        if (names[i] != NULL)
                                rc = snprintf(*buf, *len, "%s%s",
                                              *count == 0 ? " " : ", ",
                                              names[i]);
                        else
                                rc = snprintf(*buf, *len, "%s%s 0x%x",
                                              *count == 0 ? " " : ", ",
                                              estr, bit);
                        if (rc <= 0)
                                return -ENOSPC;
...
static inline int scrub_param_method_dump(char **buf, int *len, __u16 method)
{
        ...
        case SM_NAMESPACE:
                ...
                break;
        default:
                rc = snprintf(*buf, *len, "Method: unknown method 0x%x\n",
                              method);
                break;
        }
        if (rc <= 0)
                                        ...
                                        cfs_duration_t duration, int dryrun)
{
        ...
        if ((sud->sud_status == SS_SCANNING) &&
            ((dryrun != 0 && sud->sud_flags & SUF_DRYRUN) ||
             (dryrun == 0 && !(sud->sud_flags & SUF_DRYRUN)))) {
                checked = ss->ss_items_checked + new_checked;
                speed = checked;
...
static int do_scrub_unit_dump(char *buf, int len, struct scrub_exec_head *seh,
                              ...
                              __u64 new_checked, cfs_duration_t duration,
                              const char *flags[])
{
        ...
        rc = scrub_bits_dump(&buf, &len, &count, sud->sud_flags,
                             scrub_unit_flags_names, "unknown desc flag", 0);
        if (rc < 0)
                return rc;

        rc = scrub_bits_dump(&buf, &len, &count, su->su_flags,
                             flags, "unknown su flag", 1);
        if (rc < 0)
                return rc;
        ...
        count = 0;
        rc = scrub_bits_dump(&buf, &len, &count,
                             sh->sh_param_flags & ~SPF_DRYRUN,
                             scrub_param_flags_names, "unknown param flag", 1);
        if (rc < 0)
                return rc;

        rc = snprintf(buf, len, "REPAIR STATISTICS:\n");
        if (rc <= 0)
                return rc;
        buf += rc;
        len -= rc;

        rc = scrub_statistics_dump(&buf, &len, sud, &su->su_repair, new_checked,
                                   duration, 0);
        if (rc < 0)
                return rc;

        rc = snprintf(buf, len, "DRYRUN STATISTICS:\n");
        if (rc <= 0)
                return rc;
        buf += rc;
        len -= rc;

        rc = scrub_statistics_dump(&buf, &len, sud, &su->su_dryrun, new_checked,
                                   duration, 1);
        if (rc < 0)
                return rc;
                    ...
                    __u16 type, const char *flags[])
{
        ...
        if (sud->sud_status == SS_SCANNING) {
                cfs_list_for_each_entry(seu, &seh->seh_units, seu_list) {
                        su = &seu->seu_unit;
                        if (su->su_type == type) {
                                new_checked = seu->seu_new_checked;
...
dump:
        rc = do_scrub_unit_dump(buf, len, seh, sud, su, new_checked, duration,
                                flags);
