 ...
 */
/*
 * Copyright (c) 2012 Intel Incorporation
...
int lustre_buf2som(void *buf, int rc, struct md_som_data *msd)
{
	...
	/* copy SOM attributes */
	msd->msd_compat   = le32_to_cpu(attrs->som_compat);
	msd->msd_incompat = le32_to_cpu(attrs->som_incompat);
	msd->msd_ioepoch  = le64_to_cpu(attrs->som_ioepoch);
	msd->msd_size     = le64_to_cpu(attrs->som_size);
	msd->msd_blocks   = le64_to_cpu(attrs->som_blocks);
	msd->msd_mountid  = le64_to_cpu(attrs->som_mountid);
...
int lustre_buf2hsm(void *buf, int rc, struct md_hsm *mh)
{
	...
	/* copy HSM attributes */
	mh->mh_compat   = le32_to_cpu(attrs->hsm_compat);
	mh->mh_flags    = le32_to_cpu(attrs->hsm_flags);
	mh->mh_arch_id  = le64_to_cpu(attrs->hsm_arch_id);
	mh->mh_arch_ver = le64_to_cpu(attrs->hsm_arch_ver);
