	     ...
	     void *buf, struct thandle *th)
{
	...
	if (lgh->lgh_logops->lop_add == NULL) {
		CERROR("ctxt %p ctxt loc_ops %p obd %s after init for id "
			LPX64"/"LPX64 ":%x lgh %p lgh_ops %p\n",
			lgh->lgh_ctxt, lgh->lgh_ctxt->loc_logops, lgh->lgh_ctxt->loc_obd->obd_name,
			lgh->lgh_id.lgl_oid, lgh->lgh_id.lgl_oseq,
			lgh->lgh_id.lgl_ogen, lgh, lgh->lgh_logops);
		LBUG();
		RETURN(-EOPNOTSUPP);
	}
