...
out:
	...
	if (unlikely(rc == -EIO && loghandle->lgh_obj != NULL)) {
		if (dt_object_remote(loghandle->lgh_obj)) {
			 ...
			 * corrupted. To be able to finally release the log
			 * we discard any remaining bits in the header */
			CERROR("%s: Local llog "DFID" found corrupted\n",
			       loghandle->lgh_ctxt->loc_obd->obd_name,
			       PFID(lu_object_fid(&loghandle->lgh_obj->do_lu)));
			while (index <= last_index) {
