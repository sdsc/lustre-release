			...
			loff_t *off, int len, int index, struct thandle *th)
{
	...
	/**
	 * Allocate full length to avoid two separate writes for header
	 * and tail. Such 2-steps scheme needs extra protection and complex
	 * error handling.
	 */
	OBD_ALLOC(rec, len);
	if (rec == NULL)
 ...
 * \param[in] rec	llog record header. This is a real header
 * of the full llog record to write. This is the beginning of buffer
 * to write, the length of buffer is stored in \a rec::lrh_len
 *
 * \param[in] idx	index of the llog record. If \a idx == -1 then
 * this is append case, otherwise \a idx is the index of record to modify
 *
 * \param[in] th	current transaction handle
 *
 ...
 * It is used for further cancel of this llog record.
 *
 * \param[in]  cookiecount	the number of cookies to return. Not used now,
 * the amount of cookie to return is always 1.
 *
 * \param[in]  idx		index of the llog record. If \a idx == -1 then
 * this is append case, otherwise \a idx is the index of record to modify
 *
			      ...
			      int idx, struct thandle *th)
{
	 ...
	 * The common cases are llog header update (idx == 0) or llog record
	 * modification during llog process. During modification we don't need
	 * extra header update because the bitmap and record count are not
	 * changed. The record header and tail remains the same too.
	 */
	if (idx != -1) {
	...
	/**
	 * NB: the caller should make sure only 1 process access the lgh_last_idx,
	 * e.g. append should be exclusive. Otherwise it might hit the assert.
	 */
...
out:
	...
	if (rc) {
		...
		llh->llh_count--;
		/* return tail index back only if it wasn't changed
		 * by someone else */
		if (likely(llh->llh_tail.lrt_index == index))
 ...
 * \param[in]	  len		required len to read, it is LLOG_CHUNK_SIZE
 * usually.
 *
 ...
 *
 * \retval		0 on successful open, llog_handle::lgh_obj contains
 * the dt_object of the llog.
 * \retval		negative value on error
 */
 ...
 * \param[in]	ctxt_idx	the llog index, it defines the purpose of this
 * llog. Every new llog type have to use own index.
 ...
 *
 * This function reads the CATALOGS file which contains the list of llog
 ...
 * function returns just number of catalog IDs in the file.
 * \param[in]	fid		LLOG_CATALOGS_OID for CATALOG object
 *
 * \retval		0 on successful read of catalog IDs
 * \retval		negative value on error
 * \retval		positive value which is number of records in the file
 * 			if \a idarray is NULL
 */
 ...
 *
 * This function writes the CATALOG file which contains the list of llog
