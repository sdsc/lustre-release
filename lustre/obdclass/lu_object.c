...
int lu_device_type_init(struct lu_device_type *ldt)
{
	...
	cfs_atomic_set(&ldt->ldt_device_nr, 0);
...
{
	if (cfs_atomic_inc_return(&t->ldt_device_nr) == 1 &&
...
void lu_device_fini(struct lu_device *d)
{
	...
	LASSERT(cfs_atomic_read(&t->ldt_device_nr) > 0);

	if (cfs_atomic_dec_and_test(&t->ldt_device_nr) &&
