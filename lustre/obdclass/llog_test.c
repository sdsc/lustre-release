...
static int llog_test_9(const struct lu_env *env, struct obd_device *obd)
{
	 ...
	 * its catalog entry to be freed for next forced wrap in 9e */
	...
	if (cath->lgh_last_idx > cath->lgh_hdr->llh_cat_idx) {
		CERROR("9e: catalog failed to wrap around\n");
		GOTO(out, rc = -EINVAL);
	}
	...
	if (la.la_size != cat_max_size) {
		CERROR("9e: catalog size has changed after it has wrap around,"
		       " current size = "LPU64", expected size = "LPU64"\n",
		       la.la_size, cat_max_size);
		GOTO(out, rc = -EINVAL);
	}
	CWARN("9e: catalog successfully wrap around, last_idx %d, first %d\n",
