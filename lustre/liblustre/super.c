...
void llu_update_inode(struct inode *inode, struct lustre_md *md)
{
	...
	if (body->valid & OBD_MD_FLATIME) {
		if (body->atime >= st->st_atim.tv_sec) {
			if ((body->atime_ns > st->st_atim.tv_nsec &&
			     body->atime == st->st_atim.tv_sec) ||
			    body->atime > st->st_atim.tv_sec) {
	...
	if (body->valid & OBD_MD_FLMTIME) {
		if (body->mtime >= st->st_mtim.tv_sec) {
			if ((body->mtime_ns > st->st_mtim.tv_nsec &&
			     body->mtime == st->st_mtim.tv_sec) ||
			    body->mtime > st->st_mtim.tv_sec) {
	...
	if (body->valid & OBD_MD_FLCTIME) {
		if (body->ctime >= st->st_ctim.tv_sec) {
			if ((body->ctime_ns > st->st_ctim.tv_nsec &&
			     body->ctime == st->st_ctim.tv_sec) ||
			    body->ctime > st->st_ctim.tv_sec) {
...
void obdo_to_inode(struct inode *dst, struct obdo *src, obd_flag valid)
{
	...
	if (valid & OBD_MD_FLCTIME && src->o_ctime >= st->st_ctim.tv_sec) {
		if (src->o_ctime == st->st_ctim.tv_sec &&
		    src->o_ctime_ns > st->st_ctim.tv_nsec) {
			st->st_ctim.tv_nsec = src->o_ctime_ns;
		} else if (src->o_ctime > st->st_ctim.tv_sec) {
