...
void llu_update_inode(struct inode *inode, struct lustre_md *md)
{
	...
	if (body->valid & OBD_MD_FLATIME) {
		if (ll_nanotime_after(body->atime, body->atime_ns,
				      st->st_atim.tv_sec,
				      st->st_atim.tv_nsec)) {
	...
	if (body->valid & OBD_MD_FLMTIME) {
		if (ll_nanotime_after(body->mtime, body->mtime_ns,
				      st->st_mtim.tv_sec,
				      st->st_mtim.tv_nsec)) {
	...
	if (body->valid & OBD_MD_FLCTIME) {
		if (ll_nanotime_after(body->ctime, body->ctime_ns,
				      st->st_ctim.tv_sec,
				      st->st_ctim.tv_nsec)) {
...
void obdo_to_inode(struct inode *dst, struct obdo *src, obd_flag valid)
{
	...
	if (valid & OBD_MD_FLCTIME &&
	    ll_nanotime_after(src->o_ctime, src->o_ctime_ns,
			      st->st_ctim.tv_sec, st->st_ctim.tv_nsec)) {
