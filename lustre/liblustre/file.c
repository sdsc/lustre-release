                        ...
                        obd_flag valid)
{
	...
	if (valid & OBD_MD_FLATIME && src->o_atime >= st->st_atim.tv_sec) {
		if (src->o_atime == st->st_atim.tv_sec &&
		    src->o_atime_ns > st->st_atim.tv_nsec) {
			st->st_atim.tv_nsec = src->o_atime_ns;
		} else if (src->o_atime > st->st_atim.tv_sec) {
			st->st_atim.tv_sec = src->o_atime;
			st->st_atim.tv_nsec = src->o_atime_ns;
		}
	}
	if (valid & OBD_MD_FLMTIME && src->o_mtime >= st->st_mtim.tv_sec) {
		if (src->o_mtime == st->st_mtim.tv_sec &&
		    src->o_mtime_ns > st->st_mtim.tv_nsec) {
			st->st_mtim.tv_nsec = src->o_mtime_ns;
		} else if (src->o_mtime > st->st_mtim.tv_sec) {
                           ...
                           struct lustre_handle *fh)
{
	...
	op_data->op_attr.ia_atime = st->st_atim;
	op_data->op_attr.ia_mtime = st->st_mtim;
	op_data->op_attr.ia_ctime = st->st_ctim;
