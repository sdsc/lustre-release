...
void llu_layout_lock_put(struct inode *inode);
...
/*
 * Companion of llu_lsm_get. Release a reference on a lsm, last ref frees it.
 * lsmp may be different from ll_i2info(inode)->lli_smd in this case inode is
 * used to get export for lsm methods
 *
 * \param struct inode *inode entry inode
