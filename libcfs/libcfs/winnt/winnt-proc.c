...
proc_file_read(struct file * file, const char * buf, size_t nbytes, loff_t *ppos)
{
    ...
    if (!(page = (char *) kmalloc(PAGE_CACHE_SIZE, 0)))
        return -ENOMEM;
...
lustre_open_file(char * filename)
{
    ...
    fh = kmalloc(sizeof(file_t), __GFP_ZERO);
    if (!fh) {
        return NULL;
    }
...
Eoverflow:
	m->op->stop(m, p);
	kfree(m->buf);
	m->buf = kmalloc(m->size <<= 1, CFS_ALLOC_KERNEL | __GFP_ZERO);
		...
		int psize)
{
	...
	private = kmalloc(psize, CFS_ALLOC_KERNEL | __GFP_ZERO);
	if (private == NULL)
		goto out;
