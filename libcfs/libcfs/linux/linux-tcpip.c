...
libcfs_sock_write (struct socket *sock, void *buffer, int nob, int timeout)
{
	int		rc;
	mm_segment_t	oldmm = get_fs();
	long		jiffies_left = msecs_to_jiffies(timeout * MSEC_PER_SEC);
	unsigned long	then;
	struct timeval	tv;
	...
	for (;;) {
		...
		if (timeout != 0) {
			...
			tv = (struct timeval) {
				.tv_sec = jiffies_to_msecs(jiffies_left) /
							   MSEC_PER_SEC,
...
libcfs_sock_read (struct socket *sock, void *buffer, int nob, int timeout)
{
	...
	for (;;) {
		...
		tv = (struct timeval) {
			.tv_sec = jiffies_to_msecs(jiffies_left) / MSEC_PER_SEC,
			.tv_usec = ((jiffies_left %
				    msecs_to_jiffies(MSEC_PER_SEC)) * 1000000) /
				    msecs_to_jiffies(MSEC_PER_SEC)
		};
                set_fs(KERNEL_DS);
