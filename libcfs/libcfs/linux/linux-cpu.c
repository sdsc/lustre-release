...
/*
 * Update the maximum NUMA distance based on the distance between the
 * from node and all nodes in the to_mask. When a node is added the
 * previous maximum cannot decrease, so this can be used to only check
 * against the new node.
 */
static unsigned
cfs_cpt_distance_update(int from, nodemask_t *to_mask, unsigned maximum)
{
	unsigned distance;
	int to;
...
static void cfs_cpt_add_node(struct cfs_cpt_table *cptab, int cpt, int node)
{
	...
	if (!node_isset(node, *cptab->ctb_nodemask)) {
		...
		cptab->ctb_distance = cfs_cpt_distance_update(node,
							cptab->ctb_nodemask,
							cptab->ctb_distance);
	}
	...
	if (!node_isset(node, *cptab->ctb_parts[cpt].cpt_nodemask)) {
		...
		for (cpt2 = 0; cpt2 < cptab->ctb_nparts; cpt2++) {
			...
			part2->cpt_distance[cpt] =
				cfs_cpt_distance_update(node,
						part2->cpt_nodemask,
						part2->cpt_distance[cpt]);
		}
	}
...
static void cfs_cpt_del_node(struct cfs_cpt_table *cptab, int cpt, int node)
{
	...
	if (cpu >= nr_cpu_ids && node_isset(node,  *part->cpt_nodemask)) {
		...
		for (cpt2 = 0; cpt2 < cptab->ctb_nparts; cpt2++) {
			...
			part2->cpt_distance[cpt] =
				cfs_cpt_distance_calculate(part->cpt_nodemask,
						part2->cpt_nodemask);
		}
	}
...
cfs_cpt_set_cpumask(struct cfs_cpt_table *cptab, int cpt, cpumask_t *mask)
{
	...
	for_each_cpu(i, mask) {
		cfs_cpt_add_cpu(cptab, cpt, i);
		cfs_cpt_add_node(cptab, cpt, cpu_to_node(i));
	}
