...
static unsigned int cfs_alloc_flags_to_gfp(unsigned int flags)
{
	...
	if ((flags & CFS_ALLOC_ATOMIC) != 0)
		...
		mflags |= __GFP_WAIT;
	if ((flags & CFS_ALLOC_NOWARN) != 0)
		mflags |= __GFP_NOWARN;
	if ((flags & CFS_ALLOC_IO) != 0)
		mflags |= __GFP_IO;
	if ((flags & CFS_ALLOC_FS) != 0)
		mflags |= __GFP_FS;
	if ((flags & CFS_ALLOC_HIGH) != 0)
		mflags |= __GFP_HIGHMEM;
	/* NB: __GFP_ZERO can't be accepted by general allocators of kernel
	 * versions before 2.6.22, which means we only pass __GFP_ZERO to
	 * general allocators for kernel 2.6.22 or later, and we can pass
	 * it to page allocators for all kernel versions. */
...
void *
cfs_malloc(size_t nr_bytes, unsigned int flags)
{
...
void *
cfs_vmalloc(size_t nr_bytes, unsigned int flags)
{
	void	*ptr;
...
EXPORT_SYMBOL(cfs_vmalloc);

void *
cfs_cpt_vmalloc(struct cfs_cpt_table *cptab, int cpt,
		size_t nr_bytes, unsigned int flags)
{
	void	*ptr;
...
EXPORT_SYMBOL(cfs_vfree);

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,22))
static unsigned int cfs_page_alloc_flags_to_gfp(unsigned int flags)
{
	return cfs_alloc_flags_to_gfp(flags);
}
#else
static unsigned int cfs_page_alloc_flags_to_gfp(unsigned int flags)
{
	unsigned int mflags = cfs_alloc_flags_to_gfp(flags);

	return (flags & CFS_ALLOC_ZERO) == 0 ? mflags : (mflags | __GFP_ZERO);
}
#endif
...
cfs_page_t *cfs_page_alloc(unsigned int flags)
{
         ...
         */
	return alloc_page(cfs_page_alloc_flags_to_gfp(flags));
}
EXPORT_SYMBOL(cfs_page_alloc);

cfs_page_t *cfs_page_cpt_alloc(struct cfs_cpt_table *cptab,
			       int cpt, unsigned int flags)
{
	return alloc_pages_node(cfs_cpt_spread_node(cptab, cpt),
				cfs_page_alloc_flags_to_gfp(flags), 0);
}
EXPORT_SYMBOL(cfs_page_cpt_alloc);

void cfs_page_free(cfs_page_t *page)
{
        __free_page(page);
}
EXPORT_SYMBOL(cfs_page_free);
...
void *
cfs_mem_cache_cpt_alloc(cfs_mem_cache_t *cachep,
			struct cfs_cpt_table *cptab, int cpt,
			size_t nr_bytes, unsigned int flags)
{
	void	*ptr;
