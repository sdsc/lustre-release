...
void
cfs_error(int level, int _rc, const char *fmt, ...)
{
	va_list args;
	int tmp_errno = errno;
...
void
cfs_printf(int level, const char *fmt, ...)
{
	va_list args;
	int tmp_errno = errno;
...
static int
cfs_find_file(char *file, const char *param_path)
{
	char pattern[PATH_MAX];

	/* First see if this is in sysfs. */
	snprintf(pattern, PATH_MAX, CFS_GLOB_SYSFS_PATTERN,
		 param_path);
	pattern[PATH_MAX - 1] = '\0';
	if (cfs_first_match(pattern, file) == 0)
		return 0;
	...
	snprintf(pattern, PATH_MAX,
		 ...
		 param_path);
	pattern[PATH_MAX - 1] = '\0';
	return cfs_first_match(pattern, file);
}

int
cfs_get_param(const char *param_path, char *result,
	      unsigned int result_size, int use_concatenation)
{
	char file[PATH_MAX];
	...
	rc = cfs_find_file(file, param_path);
	if (rc)
		return rc;
