...
cfs_get_param_paths(glob_t *paths, const char *pattern, ...)
{
	...
	size_t len = path_len + strlen(pattern);
	char buf[PATH_MAX];
	va_list args;
	int rc;

	fprintf(stderr, "cfs_get_param_paths length %u\n", len);
	...
	fprintf(stderr, "vsnprintf return %d\n", rc);

	len = strncat(path + path_len, buf, sizeof(path) - path_len);
	if (len >= sizeof(path) - path_len) {
		errno = E2BIG;
		return -1;
	}

	rc = glob(path, GLOB_BRACE | GLOB_MARK, NULL, paths);
	if (rc == GLOB_NOMATCH) {
		errno = ENODEV;
		return -1;
	}
