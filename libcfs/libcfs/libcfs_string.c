 ...
 */
int
cfs_range_expr_list_match(__u32 value, cfs_list_t *expr_list)
{
...
CFS_EXPORT_SYMBOL(cfs_range_expr_list_match);
 ...
 */
int
cfs_num_exprs_parse(char *str, int len, unsigned min, unsigned max,
                    struct cfs_num_exprs **elpp)
{
        struct cfs_num_exprs     *expr_list;
        struct cfs_range_expr    *expr;
        struct cfs_lstr           src;
        int                       rc;
...
CFS_EXPORT_SYMBOL(cfs_num_exprs_parse);
 ...
 */
int
cfs_num_exprs_match(__u32 value, struct cfs_num_exprs *exprs)
{
        return cfs_range_expr_list_match(value, &exprs->el_exprs);
}
CFS_EXPORT_SYMBOL(cfs_num_exprs_match);
 ...
 */
void
cfs_num_exprs_list_free(cfs_list_t *list)
{
        struct cfs_num_exprs *exprs;

        while (!cfs_list_empty(list)) {
                exprs = cfs_list_entry(list->next,
                                       struct cfs_num_exprs, el_link);
                cfs_list_del(&exprs->el_link);
                cfs_range_expr_list_free(&exprs->el_exprs);
                LIBCFS_FREE(exprs, sizeof(*exprs));
        }
}
CFS_EXPORT_SYMBOL(cfs_num_exprs_list_free);
...
cfs_ip_addr_parse(char *str, int len, cfs_list_t *list)
{
        ...
        while (src.ls_str != NULL) {
                ...
                rc = cfs_num_exprs_parse(res.ls_str,
                                         res.ls_len, 0, 255, &exprs);
                if (rc != 0)
                        goto out;

                cfs_list_add_tail(&exprs->el_link, list);
                i++;
        }
...
cfs_ip_addr_match(__u32 addr, cfs_list_t *list)
{
        ...
        cfs_list_for_each_entry_reverse(exprs, list, el_link) {
                if (cfs_range_expr_list_match(addr & 0xff, &exprs->el_exprs))
                        return 0;
                addr >>= 8;
                i++;
        }
...
CFS_EXPORT_SYMBOL(cfs_ip_addr_match);
...
CFS_EXPORT_SYMBOL(cfs_ip_addr_free);
