...
bool cfs_nidrange_is_contiguous(struct list_head *nidlist)
{
	...
	list_for_each_entry(nr, nidlist, nr_link) {
		return nr->nr_netstrfns->nf_is_contiguous(nidlist);
	}
 ...
 * \param	*max_nid
 */
void cfs_nidrange_min_max(struct list_head *nidlist, char *min_nid,
			  char *max_nid)
{
	struct nidrange		*nr;
	struct netstrfns	*nf = NULL;
	...
	snprintf(min_nid, LNET_NIDSTR_SIZE, "%s@%s%d", min_addr_str, lndname,
		 netnum);
	snprintf(max_nid, LNET_NIDSTR_SIZE, "%s@%s%d", max_addr_str, lndname,
		 netnum);
}
	...
	list_for_each_entry(nr, nidlist, nr_link) {
		list_for_each_entry(ar, &nr->nr_addrranges, ar_link) {
			...
			tmp_ip_addr = ((min_ip[0] << 24) | (min_ip[1] << 16) |
				       (min_ip[2] << 8) | min_ip[3]);
			if ((tmp_ip_addr < min_ip_addr) || (min_ip_addr == 0))
				min_ip_addr = tmp_ip_addr;
			tmp_ip_addr = ((max_ip[0] << 24) | (max_ip[1] << 16) |
				       (max_ip[2] << 8) | max_ip[3]);
			if (tmp_ip_addr > max_ip_addr)
				max_ip_addr = tmp_ip_addr;
		}
	}
	*max_nid = max_ip_addr;
	*min_nid = min_ip_addr;
