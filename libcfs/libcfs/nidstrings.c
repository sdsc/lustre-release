...
struct addrrange {
        ...
        cfs_list_t ar_link;
        /**
         * List head for cfs_num_expr_list::rel_list.
         */
...
libcfs_num_parse(char *str, int len, cfs_list_t *list)
{
        ...
        rc = cfs_num_expr_parse(str, len, 0, MAX_NUMERIC_VALUE, &el);
        if (rc == 0)
...
free_addrranges(cfs_list_t *list)
{
        ...
        cfs_list_for_each_safe(pos, next, list) {
                ...
                cfs_num_expr_list_free(&ar->ar_numaddr_ranges);
                cfs_list_del(pos);
...
{
        return cfs_num_addr_match(addr, numaddr, 32);
}
...
{
        return cfs_num_addr_match(addr, numaddr, 8);
}
