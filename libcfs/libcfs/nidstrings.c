...
struct addrrange {
        ...
        cfs_list_t ar_link;
        /**
         * List head for cfs_num_exprs::rel_list.
         */
...
libcfs_num_parse(char *str, int len, cfs_list_t *list)
{
        ...
        rc = cfs_num_exprs_parse(str, len, 0, MAX_NUMERIC_VALUE, &exprs);
        if (rc == 0)
...
{
        struct cfs_lstr addrrange, net, tmp;
        struct nidrange *nr;
...
free_addrranges(cfs_list_t *list)
{
        while (!cfs_list_empty(list)) {
                ...
                cfs_num_exprs_list_free(&ar->ar_numaddr_ranges);
...
libcfs_num_match(__u32 addr, cfs_list_t *numaddr)
{
        ...
        exprs = cfs_list_entry(numaddr->next, struct cfs_num_exprs, el_link);

        return cfs_num_exprs_match(addr, exprs);
}
