...
#define CBH_ALLOC(ptr, h, cptid)					\
do {									\
	if ((h)->cbh_flags & CBH_FLAG_ATOMIC_GROW)			\
		LIBCFS_CPT_ALLOC_GFP((ptr), h->cbh_cptab, cptid, CBH_NOB,\
				     CFS_ALLOC_ATOMIC);			\
	else								\
		LIBCFS_CPT_ALLOC((ptr), h->cbh_cptab, cptid, CBH_NOB);	\
} while (0)

#define CBH_FREE(ptr)	LIBCFS_FREE(ptr, CBH_NOB)

static int
cfs_binheap_grow(cfs_binheap_t *h, int cptid)
{
	...
	if (hwm == 0) {
		...
		CBH_ALLOC(h->cbh_elements1, h, cptid);
	...
	if (hwm < CBH_SIZE * CBH_SIZE) {
		...
		CBH_ALLOC(frag2, h, cptid);
		if (frag2 == NULL)
			return -ENOMEM;

		if (hwm == 0) {
			/* first use of double indirect */
			CBH_ALLOC(h->cbh_elements2, h, cptid);
	...
	if (hwm == 0) {
		...
		CBH_ALLOC(h->cbh_elements3, h, cptid);
...
cfs_binheap_insert(cfs_binheap_t *h, cfs_binheap_node_t *e)
{
	...
	if (new_idx == h->cbh_hwm) {
		rc = cfs_binheap_grow(h, cfs_cpt_current(h->cbh_cptab, 0));
		if (rc != 0)
			return rc;
	}
