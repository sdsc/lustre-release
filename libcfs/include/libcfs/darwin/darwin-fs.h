...
int	kern_file_size(file_t *fp, off_t	*size);
#define filp_size(fp)			\
	({					\
		off_t		__size;		\
...
file_t *kern_file_open(const char *name, int flags, int mode);
int kern_file_close(file_t *fp);
int kern_file_read(file_t *fp, void *buf, size_t nbytes, off_t *pos);
int kern_file_write(file_t *fp, void *buf, size_t nbytes, off_t *pos);
int kern_file_sync(file_t *fp);
...
int ref_file(file_t *fp);
int rele_file(file_t *fp);
...
#define flock_set_start(fl, start)	do { (fl)->l_start = (start); } while(0)

static inline loff_t flock_end(flock_t *fl)
{
	return (fl->l_len == 0 ? OFFSET_MAX : (fl->l_start + fl->l_len));
}

static inline void flock_set_end(flock_t *fl, loff_t end)
{
	if (end == OFFSET_MAX)
                fl->l_len = 0;
        else
