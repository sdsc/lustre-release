 ...
 *
 * page_t has to cover both of them, because core Lustre code is based on
 ...
 * common header that contains only "page type". Common page_t operations
...
struct xnu_page_ops {
	void *(*page_map)        (page_t *);
	void  (*page_unmap)      (page_t *);
	void *(*page_address)    (page_t *);
};
...
page_t *alloc_page(u_int32_t flags);
void __free_page(page_t *page);
void get_page(page_t *page);
int cfs_put_page_testzero(page_t *page);
