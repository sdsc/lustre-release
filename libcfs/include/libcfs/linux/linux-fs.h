...
# define do_fsync(fp, flag)    ((fp)->f_op->fsync(fp, 0, LLONG_MAX, flag))
#elif defined(HAVE_FILE_FSYNC_2ARGS)
# define do_fsync(fp, flag)    ((fp)->f_op->fsync(fp, flag))
#else
# define do_fsync(fp, flag)    ((fp)->f_op->fsync(fp, (fp)->f_dentry, flag))
#endif
#define filp_read(fp, buf, size, pos)   (fp)->f_op->read((fp), (buf), (size), pos)
#define filp_write(fp, buf, size, pos)  (fp)->f_op->write((fp), (buf), (size), pos)
#define filp_fsync(fp)                  do_fsync(fp, 1)


typedef struct file_lock flock_t;
...
ssize_t user_write (file_t *filp, const char *buf, size_t count, loff_t *offset);
