...
#ifdef CONFIG_NUMA     /* NUMA is disabled */

#define nodemask_size()                     sizeof(nodemask_t)
...
#define node_online(i)                      1
#define num_online_nodes()                  1
#define for_each_online_node(i)             for (i = 0; i < 1; i++)
#define num_possible_nodes()                1
#define for_each_node(i)                    for (i = 0; i < 1; i++)

#define node_set(i, mask)                   \
do {                                        \
        LASSERT(i == 0);                    \
        (mask) = 1;                         \
} while (0)

#define node_clear(i, mask)                 \
do {                                        \
        LASSERT(i == 0);                    \
        (mask) = 0;                         \
} while (0)

#define node_isset(i, mask)                 \
({                                          \
        LASSERT(i == 0);                    \
        ((mask) != 0);                      \
})

#define nodes_clear(mask)                   \
do {                                        \
        (mask) = 0;                         \
} while (0)

#define nodes_empty(mask)                 ((mask) == 0)
#define nodes_weight(mask)                ((mask) == 1)
#define for_each_node_mask(i, mask)         for (i = 0; i < 1; i++)
